(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))I(i);new MutationObserver(i=>{for(const v of i)if(v.type==="childList")for(const k of v.addedNodes)k.tagName==="LINK"&&k.rel==="modulepreload"&&I(k)}).observe(document,{childList:!0,subtree:!0});function y(i){const v={};return i.integrity&&(v.integrity=i.integrity),i.referrerPolicy&&(v.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?v.credentials="include":i.crossOrigin==="anonymous"?v.credentials="omit":v.credentials="same-origin",v}function I(i){if(i.ep)return;i.ep=!0;const v=y(i);fetch(i.href,v)}})();function Fs(b,r){const y=r.replace(/^#/,"");let I=null;for(const i of Object.keys(b))(y===i||y.startsWith(i))&&(I===null||i.length>I.length)&&(I=i);return I===null&&""in b&&(I=""),I}function Ws(b){return typeof b=="function"?{view:b,chrome:!0}:{view:b.view,chrome:b.chrome!==!1}}function Ys(b,r={}){const y=r.root??document.body,I=r.title??document.title??"",i=r.brandHref??"#",v=document.createElement("main"),k=document.createElement("header");k.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=I,w.setAttribute("aria-label",`${I} — home`),k.appendChild(w);const q=document.createElement("nav");q.className="cap-nav",q.setAttribute("aria-label","Views");for(const _ of r.nav??[]){const j=document.createElement("a");j.href=_.href,j.textContent=_.label,_.ariaLabel&&j.setAttribute("aria-label",_.ariaLabel),q.appendChild(j)}k.appendChild(q);const S=document.createElement("section");S.className="tk-content",v.appendChild(k),v.appendChild(S);const C=document.createElement("div");C.className="tk-bleed";const je=_=>{var j;for(const H of Array.from(q.querySelectorAll("a"))){const T=((j=H.getAttribute("href"))==null?void 0:j.replace(/^#/,""))??"";H.toggleAttribute("aria-current",_!==null&&_!==""&&T===_),H.hasAttribute("aria-current")&&H.setAttribute("aria-current","page")}};let z=0;const M=()=>{const _=Fs(b,location.hash);if(je(_),_===null){C.isConnected&&C.remove(),v.isConnected||y.appendChild(v),Ks(S,"Not found.");return}const{view:j,chrome:H}=Ws(b[_]),T=H?S:C;H?(C.isConnected&&C.remove(),v.isConnected||y.appendChild(v)):(v.isConnected&&v.remove(),C.isConnected||y.appendChild(C)),T.replaceChildren();const F=j(),Se=++z,le=F.mount(T);le instanceof Promise&&le.catch(Me=>{Se===z&&Gs(T,String(Me))})};window.addEventListener("hashchange",M),M()}function Ks(b,r){b.replaceChildren();const y=document.createElement("div");y.className="tk-empty",y.textContent=r,b.appendChild(y)}function Gs(b,r){b.replaceChildren();const y=document.createElement("div");y.className="tk-error",y.textContent=r,b.appendChild(y)}function Qs(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const y of r)y.unregister()}),window.caches&&caches.keys().then(r=>{for(const y of r)caches.delete(y)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Zs(){let b;try{b=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!b.ok)return null;let r;try{r=await b.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Xs=`
<div class="layout">
<aside class="sidebar">
...
`;function en(b){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null},y=e=>"pill pill-"+(e||"none"),I='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),v=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function k(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],N()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}r.jobs=e.rows||[],x(),q(),C()}function q(){if(!h.postingId)return;const e=r.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&Z())}let S=null;function C(){const e=r.jobs.some(t=>t.outreach_draft_status==="researching");e&&!S?S=setInterval(je,4e3):!e&&S&&(clearInterval(S),S=null)}async function je(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],s=new Map(r.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),n=t.some(a=>s.get(a.posting_id)!==a.outreach_draft_status)||t.length!==r.jobs.length;r.jobs=t,n&&(x(),q()),C()}function z(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Ae(),e==="jobs"?x():N()}async function M(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const s=document.getElementById("unscored-n");s.textContent="–",s.title=`stats failed: ${t.message}`;return}r.stats=e,_()}function _(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,K()}function j(e,t,s){const n=e[s]??"",a=t[s]??"";if(s==="headcount")return(n|0)-(a|0);if(s==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[n]??3)-(o[a]??3)}return String(n).localeCompare(String(a))}function H(e){return e.slice().sort((t,s)=>r.sort.dir*j(t,s,r.sort.k))}const T=new Set;let F=!1;function Se(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",T.has(e.dataset.v))})}function le(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(T.size&&!T.has(t.verdict||"__none__")||F&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Me=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],qt=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function st(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const nt=st("scout-hidden-cols"),at=st("scout-hidden-jcols");function ot(){return r.view==="jobs"?{cols:qt,hidden:at,key:"scout-hidden-jcols"}:{cols:Me,hidden:nt,key:"scout-hidden-cols"}}function ue(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=nt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=at.has(e.dataset.col)?"none":""})}function Ae(){const e=ot();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const s=ot(),n=t.dataset.col;s.hidden.has(n)?s.hidden.delete(n):s.hidden.add(n),localStorage.setItem(s.key,JSON.stringify([...s.hidden])),Ae(),ue()})})}function N(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=H(le());document.getElementById("empty").style.display=t.length?"none":"block";for(const s of t){const n=document.createElement("tr");n.dataset.id=s.company_id,n.innerHTML=`
      <td class="td-flag" data-col="flag"><button class="flag-btn${s.flagged?" is-on":""}" data-id="${s.company_id}" title="${s.flagged?"unflag":"flag"}">${I}</button></td>
    ...
    `;const L=()=>new Date().toISOString().slice(0,10);f.querySelector(".jt-nextup").onclick=()=>ut(d,!1),f.querySelector(".jt-applied").onclick=()=>fe(d,{applied_at:d.applied_at?"":L()}),f.querySelector(".jt-resp").onchange=E=>{G(E.target),fe(d,{response:E.target.value})},f.querySelector(".jt-inc").onclick=()=>fe(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:L()}),f.querySelector(".jt-dec").onclick=()=>{const E=Math.max(0,(d.outreach_count||0)-1);fe(d,{outreach_count:E,...E===0?{last_outreach_at:""}:{}})},e.appendChild(f)}e.querySelectorAll(".jt-resp").forEach(G),ue(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",m=>{m.target.closest("a, button, select")||rt(d.dataset.id)})})}function Vt(e){return e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="needs_work"?'<span class="draft-badge db-needswork" title="the draft finished below the depth bar — review, fix or regenerate">draft needs work</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function rt(e){let t=r.jobs.find(s=>s.posting_id===e);if(t||(await w(),t=r.jobs.find(s=>s.posting_id===e)),!t){u("posting not found — refresh");return}Pe(),Ne(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),ct("pursuit"),Z(),Q(),ee()}let He=null;function ct(e){He=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function me(){Pe(),Ne(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function Pe(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function Q(){if(!h.postingId)return;let e;try{const s=await fetch(`/api/postings/${h.postingId}/outreach`);if(!s.ok){ve();return}e=await s.json()}catch{ve();return}h.drafts=e.drafts||[],ve();const t=h.drafts[0];t&&t.status==="researching"?Dt():Pe()}function Dt(){h.poll||(h.poll=setInterval(Q,4e3))}function A(e,t,{multiline:s=!1}={}){if(!e)return;let n=e.value;e.addEventListener("focus",()=>{n=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=n,e.blur()):a.key==="Enter"&&(!s||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===n.trim()){e.value=n;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),n=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=n,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),u(`save failed: ${o.message}`)}})}async function dt(e,t,s){const n={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:s},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,summary:o.summary,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),x(),ae(e.posting_id,{title:o.title,location:o.location,summary:o.summary})}async function Ut(e,t){const s=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();e.url=n.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",v(e.url)),ae(e.posting_id,{url:n.url})}async function lt(e,t,s){const n={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:s};if(!String(n.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),k(),w()}async function Jt(e,t){const s=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();r.openId=n.company_id,ne(n),oe(n.company_id),k(),w()}async function zt(e,t){const s=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();e.notes=n.notes}function Z(){const e=h.row;if(!e)return;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${i(e.title||"")}">`;const t=O[e.response]||O[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${t.cls}">${i(t.label)}</span>`+(e.verdict?` <span class="${y(e.verdict)}">${i(e.verdict)}</span>`:"");const s=document.getElementById("pursuit-chat");s&&(s.style.display=r.meta&&r.meta.chat?"":"none",s.onclick=()=>Te("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
  ...
  `,Wt();const n=document.getElementById("pursuit-company-link");n&&n.addEventListener("click",()=>se(e.company_id)),A(document.getElementById("pursuit-title-input"),a=>dt(e,"title",a)),A(document.getElementById("pursuit-url-input"),a=>Ut(e,a)),A(document.getElementById("pursuit-notes-input"),a=>Yt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>A(a,o=>dt(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),ve(),te()}function Ft(e){return`
    ...
    </div>`}function Wt(){const e=h.row,t=()=>new Date().toISOString().slice(0,10),s=document.querySelector("#pursuit-body .pt-applied");s&&s.addEventListener("click",()=>W({applied_at:e.applied_at?"":t()}));const n=document.querySelector("#pursuit-body .pl-applied-date");n&&n.addEventListener("change",c=>W({applied_at:c.target.value}));const a=document.querySelector("#pursuit-body .pl-response");a&&a.addEventListener("change",c=>W({response:c.target.value}));const o=document.querySelector("#pursuit-body .pt-nextup");o&&o.addEventListener("click",()=>ut(h.row,!0))}async function ut(e,t){let s;try{s=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){u(`save failed: ${a.message}`);return}if(!s.ok){const a=(await s.text().catch(()=>"")).trim();u(`save failed: ${a||"HTTP "+s.status}`);return}const n=await s.json();e.next_up=n.next_up,x(),ae(e.posting_id,{next_up:n.next_up}),t&&Z(),u(e.next_up?"queued next up":"removed from the queue")}async function pt(e,t){const s={applied_at:e.applied_at||"",response:e.response||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",contacts:e.contacts||"",notes:e.notes||"",...t};let n;try{n=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)})}catch(o){return u(`save failed: ${o.message}`),null}if(!n.ok){const o=(await n.text().catch(()=>"")).trim();return u(`save failed: ${o||"HTTP "+n.status}`),null}const a=await n.json();return Object.assign(e,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),ae(e.posting_id,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function Yt(e){const t=h.row,s={applied_at:t.applied_at||"",response:t.response||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",contacts:t.contacts||"",notes:e},n=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();t.notes=a.notes,x()}async function W(e){await pt(h.row,e)&&(x(),Z(),u("tracking saved"))}async function fe(e,t){await pt(e,t)&&(x(),h.postingId===e.posting_id&&(h.row=e,Z()),u("tracking saved"))}function ve(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.row,s=h.drafts,n=s[0]||null,a=s.slice(1),o=`
    ...
    </div>`,p=n&&(Kt(n.status)||n.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${n?"Draft again":"Draft outreach"}</button>`,l=a.length?`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${n?mt(n,!1):""}</div><div class="draft-actions">${p}</div>`+l,Xt()}function Kt(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const Gt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',Qt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',ht=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${Gt}</button>`;function mt(e,t){const s=(f,B,L="")=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const f=Zt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      ${s("pill pill-yes","sent",t?"":ht)}
      ${e.sent_at?`<div class="draft-note">Sent ${i((e.sent_at||"").replace("T"," ").slice(0,16))}</div>`:""}
      <div class="draft-sentbody">${i(qe(e)||"(empty)")}</div>
      ${X(e)}
    </div>`;const n=qe(e),a=e.status==="no_hook",o=e.status==="needs_work",c=a?'<span class="pill pill-info">no honest hook</span>':o?'<span class="pill pill-maybe">needs work — below the depth bar</span>':'<span class="pill pill-maybe">awaiting review</span>';let p="";if(a)try{p=JSON.parse(e.hook||"{}").reasoning||""}catch{}const l=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${p?" "+i(p):""}</div>`:"",d=vt(e.critique);if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
      <div class="draft-head">${c}</div>
      ${l}
      ${d}
      <div class="draft-sentbody">${i(n||"(empty)")}</div>
      ${X(e)}
    </div>`;const m=n||a;return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    <div class="draft-head">${c}${n?ht:""}</div>
    ${l}
    ${d}
    ${m?`<textarea class="draft-textarea" id="draft-edit-${e.id}" spellcheck="false">${i(n)}</textarea>
    ${ft(e.lint)}
    ...
    ${X(e)}
  </div>`}function qe(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function X(e){let t="",s=null,n=null;try{s=JSON.parse(e.research||"null")}catch{}try{n=JSON.parse(e.hook||"null")}catch{}if(s&&typeof s=="object"){const a=(l,d)=>d?`<div class="tr-line"><span class="tr-key">${l}:</span> ${i(String(d))}</div>`:"",o=s.role||{},c=Array.isArray(s.hooks)?s.hooks:[],p=c.map(l=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function ft(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(s=>`<span class="lint-chip" title="${i(s.message||"")}"><code>${i(s.code||"")}</code>${i(s.message||"")}</span>`).join("")+"</div>":""}function vt(e){let t=null;try{t=JSON.parse(e||"null")}catch{return""}if(!t||typeof t!="object")return"";const s=t.depth==="deep"?"pill-yes":t.depth==="medium"?"pill-maybe":"pill-no",n={direct:"pill-yes",adjacent:"pill-info",standing:"pill-maybe"}[t.proof_tier]||"pill-no",a=t.proof_tier==="standing"?"standing creds":t.proof_tier,o=[t.depth?`<span class="pill ${s}">depth: ${i(t.depth)}</span>`:"",t.proof_tier?`<span class="pill ${n}">proof: ${i(a)}</span>`:""].filter(Boolean).join(""),c=Array.isArray(t.weaknesses)&&t.weaknesses.length?'<ul class="critique-list">'+t.weaknesses.map(d=>`<li>${i(String(d))}</li>`).join("")+"</ul>":"",p=String(t.experience_gaps||"").trim(),l=p?`<div class="critique-gap"><span class="cg-label">experience gap:</span> ${i(p)}</div>`:"";return!o&&!c&&!l?"":`<div class="draft-critique">
    ${o?`<div class="critique-chips">${o}</div>`:""}
    ${c}${l}
  </div>`}function Zt(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(s=>`<li>${i(s.claim||s.message||String(s))}${s.why?` <span class="vl-why">— ${i(s.why)}</span>`:""}</li>`).join("")+"</ul>":""}function Xt(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector(".oc-input");t&&(t.addEventListener("change",l=>W({contacts:l.target.value.trim()})),t.addEventListener("keydown",l=>{l.key==="Enter"&&(l.preventDefault(),l.target.blur())}));const s=()=>new Date().toISOString().slice(0,10),n=h.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>W({outreach_count:(n.outreach_count||0)+1,last_outreach_at:s()}));const o=e.querySelector(".pt-outreach-dec");o&&o.addEventListener("click",()=>{const l=Math.max(0,(n.outreach_count||0)-1);W({outreach_count:l,...l===0?{last_outreach_at:""}:{}})});const c=e.querySelector("#draft-start-btn");c&&c.addEventListener("click",()=>ge()),e.querySelectorAll(".draft-retry-btn").forEach(l=>l.addEventListener("click",()=>ge())),e.querySelectorAll(".draft-regen-btn").forEach(l=>l.addEventListener("click",()=>ge(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(l=>{const d=l.dataset.did,m=l.querySelector(".draft-textarea");m&&A(m,L=>ts(d,L),{multiline:!0});const f=l.querySelector(".draft-sent-btn");f&&f.addEventListener("click",()=>ss(d));const B=l.querySelector(".draft-copy-btn");B&&B.addEventListener("click",()=>{const L=l.querySelector(".draft-textarea"),E=l.querySelector(".draft-sentbody"),D=L?L.value:E?E.textContent:"";bs(D,"email copied")})});const p=e.querySelector("details.draft-history");p&&p.addEventListener("toggle",()=>{h.openHist=p.open})}async function ge(e=!1){const t=document.getElementById("outreach-section"),s=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));s&&(s.disabled=!0);let n;try{const o=e?"?regenerate=1":"";n=await fetch(`/api/postings/${h.postingId}/outreach${o}`,{method:"POST"})}catch(o){u(`draft failed: ${o.message}`),s&&(s.disabled=!1);return}if(n.status===202){let o={};try{o=await n.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&u(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await Q(),w();return}if(n.status===409){await Q(),u("a draft is already active");return}if(n.status===412){let o={};try{o=await n.json()}catch{}es(o.need,o.error),s&&(s.disabled=!1);return}if(n.status===503){const o=document.getElementById("outreach-section");if(o){const c=document.createElement("div");c.className="draft-note",c.textContent="Outreach engine not running in this build.",o.appendChild(c)}s&&(s.disabled=!1);return}const a=(await n.text().catch(()=>"")).trim();u(`draft failed: ${a||"HTTP "+n.status}`),s&&(s.disabled=!1)}function es(e,t){const s=document.getElementById("outreach-section");if(!s)return;const n=s.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"Discover sources",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,n?n.replaceWith(c):s.appendChild(c);const p=c.querySelector("#gate-fix-btn");p&&p.addEventListener("click",()=>a?re("outreach-template"):Je());const l=c.querySelector("#gate-retry-btn");l&&l.addEventListener("click",ge)}async function ts(e,t){const s=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json(),a=h.drafts.findIndex(p=>String(p.id)===String(e));a>=0&&(h.drafts[a]=n);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const p=c.querySelector(".lint-chips"),l=ft(n.lint);p?p.outerHTML=l||"":l&&o.insertAdjacentHTML("afterend",l)}}async function ss(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(n){u(`failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`failed: ${n||"HTTP "+t.status}`);return}u("marked sent"),await Q(),await w();const s=r.jobs.find(n=>n.posting_id===h.postingId);s&&ae(s.posting_id,{outreach_count:s.outreach_count,last_outreach_at:s.last_outreach_at,next_up:s.next_up})}async function ee(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){te();return}e=await t.json()}catch{te();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",te(),h.answers.some(t=>t.status==="generating")?ns():Ne()}function ns(){h.answersPoll||(h.answersPoll=setInterval(ee,4e3))}function Ne(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function te(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,s=h.answersStatus,n=t.some(d=>d.status==="generating"),a=t.length?`<div class="answers-list">${t.map(is).join("")}</div>`:"",o=!!h.detecting,c=n||o?" disabled":"",p=d=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":d}</button>`;let l;s==="ok"&&t.length?l=`<button class="btn ${t.some(m=>!gt(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${n?"Drafting…":"Draft answers"}</button>`+p("Re-detect"):s===""||s==="unreachable"?l=`<button class="btn btn-primary" id="answers-start-btn"${c}>${n?"Drafting…":"Draft answers"}</button>`+p("Re-detect questions"):l=p("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${i(as(s,t.length))}</div>`+a+`<div class="answers-actions">${l}</div>`,cs()}function as(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function gt(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function os(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function is(e){const t=gt(e),s=e.edited&&e.edited.trim(),n=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${i(rs(e.fail_reason))}</div>`:""}
  </div>`}function rs(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function cs(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",yt);const s=e.querySelector("#answers-redetect-btn");s&&s.addEventListener("click",ls),e.querySelectorAll(".answer-card[data-aid]").forEach(n=>{const a=n.dataset.aid,o=n.querySelector(".answer-textarea");o&&(A(o,p=>us(a,p),{multiline:!0}),o.addEventListener("input",()=>ds(n,o)));const c=n.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>ps(a))})}function ds(e,t){const s=e.querySelector(".answer-count");if(!s)return;const n=t.value.length,a=s.textContent.includes("/")?parseInt(s.textContent.split("/")[1],10):0;s.textContent=a?`${n} / ${a}`:`${n} chars`,s.classList.toggle("over",!!a&&n>a)}async function yt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let s;try{s=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){u(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(s.status===202){await ee();return}if(s.status===412){let a={};try{a=await s.json()}catch{}hs(a.error),t&&(t.disabled=!1);return}if(s.status===503){bt("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const n=(await s.text().catch(()=>"")).trim();u(`draft failed: ${n||"HTTP "+s.status}`),t&&(t.disabled=!1)}async function ls(){h.detecting=!0,te();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();u(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){u(`detect failed: ${e.message}`)}h.detecting=!1,await ee()}async function us(e,t){const s=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=n)}async function ps(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(s){u(`regenerate failed: ${s.message}`);return}if(t.status===503){bt("Answer generation isn't running in this build.");return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`regenerate failed: ${s||"HTTP "+t.status}`);return}await ee()}function hs(e){const t=document.getElementById("answers-section");if(!t)return;const s=t.querySelector(".answers-actions"),n=document.createElement("div");n.className="blocks-gate",n.innerHTML=`
    ...
    </div>`,s?s.replaceWith(n):t.appendChild(n);const a=n.querySelector("#answers-fix-btn");a&&a.addEventListener("click",Je);const o=n.querySelector("#answers-retry-btn");o&&o.addEventListener("click",yt)}function bt(e){const t=document.getElementById("answers-section");if(!t)return;const s=document.createElement("div");s.className="draft-note",s.textContent=e,t.appendChild(s)}async function se(e){r.openId=e;const t=document.getElementById("pane"),s=document.getElementById("scrim");t.classList.add("open"),s.classList.add("open"),t.setAttribute("aria-hidden","false"),ct("company"),document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';let n;try{const a=await fetch(`/api/companies/${e}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);n=await a.json()}catch(a){document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${i(a.message)}</div>`;return}ne(n),oe(e)}function ye(){r.openId=null,r.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function ne(e){r.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${i(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `;const B=document.getElementById("posting-add-btn");B&&B.addEventListener("click",()=>gs(e)),Re(),document.querySelectorAll("#ve-pick .ve-opt").forEach(g=>{g.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(J=>J.classList.remove("is-on")),g.classList.add("is-on")})});const L=document.getElementById("ve-save-btn");L&&L.addEventListener("click",()=>vs(e)),A(document.getElementById("pane-title-input"),g=>lt(e,"name",g)),document.querySelectorAll("#facts-body [data-k]").forEach(g=>A(g,J=>lt(e,g.dataset.k,J))),A(document.getElementById("pane-domain-input"),g=>Jt(e,g)),A(document.getElementById("pane-notes-input"),g=>zt(e,g),{multiline:!0});const E=document.getElementById("flag-toggle-btn");E&&E.addEventListener("click",()=>wt(e.company_id));const D=document.getElementById("review-stamp-btn");D&&D.addEventListener("click",()=>fs(e.company_id));const de=document.getElementById("rescore-btn");de&&de.addEventListener("click",()=>De("verdict",{company_ids:[e.company_id]}));const $=document.getElementById("reenrich-btn");$&&$.addEventListener("click",()=>De("enrich",{company_ids:[e.company_id]}))}function ms(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ae(e,t){const s=r.openDetail;if(!s||!r.openId)return;const n=(s.postings||[]).find(o=>String(o.id)===String(e));if(!n)return;Object.assign(n,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Oe(s),Re())}function Re(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||rt(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),Te("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function gs(e){const t=document.getElementById("posting-url"),s=document.getElementById("posting-title"),n=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){u("Enter a URL first."),t.focus();return}n.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:s.value.trim()})})}catch(l){u(`add failed: ${l.message}`),n.disabled=!1;return}if(!o.ok){const l=await o.text().catch(()=>"");u(`add failed: HTTP ${o.status}${l?" — "+l:""}`),n.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(l=>l.id!==c.id),e.postings.unshift(c);const p=document.getElementById("postings-list");p&&(p.innerHTML=Oe(e),Re()),t.value="",s.value="",n.disabled=!1,w(),u("link added")}async function oe(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(n){be(`<div class="muted">Failed to load trail: ${i(n.message)}</div>`);return}if(!t.ok){be(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const s=(await t.json()).events||[];if(s.length===0){be('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}be(s.map(ys).join(""))}function ys(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
    ...
    </div>`}function be(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let kt;function u(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(kt),kt=setTimeout(()=>t.classList.remove("show"),2200)}async function bs(e,t="copied"){if(!e){u("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const s=document.createElement("textarea");s.value=e,s.style.position="fixed",s.style.opacity="0",document.body.appendChild(s),s.select(),document.execCommand("copy"),document.body.removeChild(s)}u(t)}catch(s){u(`copy failed: ${s.message}`)}}r.meta={control:!1,brain:!1,verdict:!1};async function ws(){try{const n=await fetch("/api/meta");if(!n.ok)return;r.meta=await n.json()}catch{return}const e=r.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!r.meta.verdict,t.title=r.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const s=document.getElementById("open-chat");s&&(s.style.display=r.meta.chat?"":"none")}async function Ve(){let e;try{const n=await fetch("/api/runs");if(!n.ok)return;e=await n.json()}catch{return}const t=e.busy_stage||"",s=document.getElementById("run-busy");t?(s.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):s.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let we=null;async function De(e,t){if(r.meta&&r.meta.control===!1){u("control surface disabled");return}let s;try{s=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){u(`run failed: ${a.message}`);return}if(s.status===409){u("a job is already running");return}if(s.status===412){const a=await s.text();u(a.trim());return}if(!s.ok){u(`run failed: HTTP ${s.status}`);return}const{job_id:n}=await s.json();Lt(e,n)}async function ks(e){const t=new FormData;t.append("csv",e);let s;try{s=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){u(`upload failed: ${a.message}`);return}if(s.status===409){u("a job is already running");return}if(!s.ok){u(`upload failed: HTTP ${s.status}`);return}const{job_id:n}=await s.json();Lt("ingest",n)}const Es=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let ke=[],R=new Set,V="company";function Et(e){V=e,document.querySelectorAll("#add-kind .v-chip").forEach(n=>n.classList.toggle("is-on",n.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),s=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',s.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',s.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",$t()}function Ue(){return!!r.meta.capture&&document.getElementById("add-enrich").checked}function $t(){const e=document.getElementById("add-note");Ue()?e.innerHTML=V==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=V==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function $s(){Es.forEach(n=>{document.getElementById(n).value=""}),document.getElementById("add-vertical-filter").value="",R=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!r.meta.capture,t.classList.toggle("disabled",!r.meta.capture),t.title=r.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",r.meta.capture||(e.checked=!1),Et(r.view==="jobs"?"job":"company");const s=document.getElementById("add-stage");s.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(r.rows||[]).map(n=>`<option value="${i(n.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const n=await(await fetch("/api/facets")).json();(n.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,s.appendChild(o)}),ke=n.verticals||[]}catch{ke=[]}xt()}function Ee(){document.getElementById("add-scrim").classList.remove("open")}function xt(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),s=ke.filter(n=>!t||n.toLowerCase().includes(t));s.length?(e.innerHTML=s.map(n=>`<button type="button" class="vchip${R.has(n)?" sel":""}" data-v="${i(n)}">${i(n)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(n=>n.addEventListener("click",()=>{const a=n.dataset.v;R.has(a)?R.delete(a):R.add(a),n.classList.toggle("sel"),It()}))):e.innerHTML=`<div class="none">${ke.length?"no match":"no verticals in the set yet"}</div>`,It()}function It(){const e=R.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function _t(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Bt(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){u(V==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const s=document.getElementById("add-save"),n=s.textContent;s.disabled=!0,Ue()&&(s.textContent="reading page…");const a=()=>{s.disabled=!1,s.textContent=n},o=f=>document.getElementById(f).value.trim(),c=Ue();let p,l;c?(p="/api/capture",l={url:_t(t),kind:V==="company"?"company_page":"job_posting",fields:V==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...R].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):V==="company"?(p="/api/companies",l={website:t,name:o("add-name"),vertical:[...R].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(p="/api/postings",l={url:_t(t),title:o("add-title"),company:o("add-job-company")});let d;try{d=await fetch(p,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)})}catch(f){u(`add failed: ${f.message}`),a();return}if(!d.ok){let f=`HTTP ${d.status}`;try{const B=await d.text();try{f=JSON.parse(B).error||f}catch{f=B.trim()||f}}catch{}if(a(),d.status===409){u(f||"That company is already in the list."),e.focus(),e.select();return}u(`add failed: ${f}`);return}const m=await d.json();if(a(),c&&!m.company_id){u(m.note||"couldn't classify that page");return}if(Ee(),k(),M(),w(),V==="job"){const f=m.posting&&m.posting.title||"job link";u(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),z("jobs")}else c?(u(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`),se(m.company_id)):u("company added")}function Lt(e,t){we=t;const s=document.getElementById("drawer"),n=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",n.innerHTML="",s.classList.add("open"),Ve();const a=new EventSource(`/api/jobs/${t}/stream`),o=(c,p)=>{const l=document.createElement("div"),d=!p&&/^\s*warn:/i.test(c);l.className="ln"+(p?" ln-err":d?" ln-warn":""),l.textContent=d?c.replace(/^\s*warn:\s*/i,"⚠ "):c,n.appendChild(l),n.scrollTop=n.scrollHeight};a.addEventListener("line",c=>o(c.data,/error|failed/i.test(c.data))),a.addEventListener("end",c=>{a.close(),we=null,o(`— ${c.data} —`,c.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",u(`${e} ${c.data}`),k(),M(),Ve(),w(),r.openId&&se(r.openId)}),a.onerror=()=>{a.close()}}async function xs(){if(we)try{await fetch(`/api/jobs/${we}/cancel`,{method:"POST"})}catch{}}let ie=null;function Tt(e){return e==="outreach-template"?"outreach template":e==="outreach-doctrine"?"outreach doctrine":e==="playbook"?"playbook":e+".md"}async function re(e){ie=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Tt(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="",t.classList.add("open");try{const s=await fetch(`/api/${e}`);if(!s.ok){const a=(await s.text().catch(()=>"")).trim();document.getElementById("editor-text").value=s.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${a||"HTTP "+s.status}`;return}const n=await s.json();document.getElementById("editor-text").value=n.content||"",n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version)}catch(s){document.getElementById("editor-text").value="failed to load: "+s.message}}function $e(){document.getElementById("editor-scrim").classList.remove("open"),ie=null}const Is=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function Je(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{Fe(await(await fetch("/api/outreach/sources")).json())}catch(e){u(`failed to load sources: ${e.message}`)}}function ze(){document.getElementById("sources-scrim").classList.remove("open")}function Fe(e){const t=document.getElementById("sources-list");if(!t)return;const s=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):Is,n={};(e&&e.sources||[]).forEach(a=>{(n[a.need]=n[a.need]||[]).push(a)}),t.innerHTML=s.map(a=>{const o=n[a.key]||[],c=o.length?o.map(p=>`<li><span class="src-title">${i(p.title||p.page_id)}</span><button class="src-rm" data-need="${i(a.key)}" data-id="${i(p.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${i(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>Bs(a.dataset.need,a.dataset.id)))}async function _s(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(n){u(`refresh failed: ${n.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){u(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const s=await t.json();s.warning?u(s.warning):u("sources refreshed"),Fe(s),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function Bs(e,t){let s;try{s=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(n){u(`remove failed: ${n.message}`);return}if(!s.ok){u(`remove failed: ${(await s.text().catch(()=>"")).trim()||"HTTP "+s.status}`);return}Fe(await s.json())}async function Ls(){if(!ie)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${ie}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(n){u(`save failed: ${n.message}`);return}if(!t.ok){u(`save failed: HTTP ${t.status}`);return}const s=await t.json();s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version),u(`${Tt(ie)} saved`),$e(),M()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),N()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),x()}}),document.getElementById("tab-companies").onclick=()=>z("companies"),document.getElementById("tab-jobs").onclick=()=>z("jobs"),document.getElementById("q").oninput=N,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;T.has(t)?T.delete(t):T.add(t),Se(),N()})}),document.getElementById("flag-filter").addEventListener("click",e=>{F=!F,e.currentTarget.classList.toggle("is-on",F),N()}),document.getElementById("jq").oninput=x,document.getElementById("hide-rejected").addEventListener("click",e=>{U=!U,e.currentTarget.classList.toggle("is-on",U),x()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;P.has(t)?P.delete(t):P.add(t),e.classList.toggle("is-on",P.has(t)),x()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{pe=!pe,e.currentTarget.classList.toggle("is-on",pe),x()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{he=!he,e.currentTarget.classList.toggle("is-on",he),x()}),Ae(),ue(),document.getElementById("pane-close").onclick=ye,document.getElementById("scrim").onclick=ye,document.getElementById("pursuit-close").onclick=me,document.getElementById("pursuit-scrim").onclick=me,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){tt();return}if(Ns()){Ze();return}if(document.getElementById("profile-scrim").classList.contains("open")){Qe();return}if(document.getElementById("add-scrim").classList.contains("open")){Ee();return}if(document.getElementById("run-scrim").classList.contains("open")){xe();return}if(document.getElementById("help-scrim").classList.contains("open")){Ie();return}const t=document.getElementById("pane").classList.contains("open"),s=document.getElementById("pursuit-pane").classList.contains("open");if(t||s){if(He==="pursuit"&&s){me();return}if(He==="company"&&t){ye();return}if(t){ye();return}me();return}if(document.getElementById("sources-scrim").classList.contains("open")){ze();return}if(document.getElementById("editor-scrim").classList.contains("open")){$e();return}if(document.getElementById("settings-scrim").classList.contains("open")){Xe();return}});let We=null;const Ts={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Ct(e){if(r.meta&&r.meta.control===!1){u("control surface disabled");return}We=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Ts[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),s=r.stats||{},n=Math.max(0,(s.total_companies||0)-(s.enriched_ok||0));e==="verdict"&&n>0?(document.getElementById("run-warn-text").textContent=`${n} ${n===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${n===1?"it":"them"}. Run Enrich first to include ${n===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function xe(){document.getElementById("run-scrim").classList.remove("open"),We=null}document.getElementById("btn-enrich").onclick=()=>Ct("enrich"),document.getElementById("btn-verdict").onclick=()=>Ct("verdict"),document.getElementById("run-cancel").onclick=xe,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&xe()},document.getElementById("run-go").onclick=()=>{const e=We,t=document.getElementById("run-only-blanks").checked,s=parseInt(document.getElementById("run-workers-input").value,10);if(xe(),!e)return;const n={};t&&(n.only_blanks=!0),s>0&&(n.workers=s),De(e,n)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=$s;const Cs={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function jt(e){const t=Cs[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const s=document.getElementById("help-items");if(s.innerHTML="",t.intro){const n=document.createElement("p");n.className="help-intro",n.textContent=t.intro,s.appendChild(n)}t.items.forEach(n=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=n.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=n.desc;const p=document.createElement("a");p.className="help-link",p.textContent="Learn more →",p.onclick=()=>{Ie(),Mt(),Os(n.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(p),s.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Ie(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>jt("add"),document.getElementById("help-run").onclick=()=>jt("run"),document.getElementById("help-close").onclick=Ie,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Ie()},document.getElementById("add-cancel").onclick=Ee,document.getElementById("add-save").onclick=Bt,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Ee()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Et(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",$t),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Bt()))}),document.getElementById("add-vertical-filter").addEventListener("input",xt),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&ks(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=xs,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=$e,document.getElementById("editor-save").onclick=Ls,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&$e()},document.getElementById("sources-close").onclick=ze,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&ze()},document.getElementById("sources-refresh-btn").onclick=_s;function Ye(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const s=Math.round(t/60);if(s<90)return`${s}m ago`;const n=Math.round(s/60);return n<48?`${n}h ago`:`${Math.round(n/24)}d ago`}async function Ke(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}K()}const _e='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',ce='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',St='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',js='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',Ss='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',Ms='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',As='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>';function Y(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${i(e.note)}</span>`:""}</div>`:"",s=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${s} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function K(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,n=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(n){let $="off",g="";const J=t&&t.criteria_state;J==="current"?($="ok",g="current · verified "+Ye(t.verified_age_seconds)):J==="changed"?($="warn",g="changed — re-distill"):J==="unverified"?($="warn",g=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?($="warn",g="brain offline · using cache"):a&&($="ok",g="fetched "+Ye(t.age_seconds)),o=Y({icon:St,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:$,note:g,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:ce,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=Y({icon:St,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:_e,actTitle:"edit taste.md",actLabel:"edit taste"});const c=r.sources&&r.sources.sources||[],p=c.filter($=>$.need==="experience").length,l=c.filter($=>$.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";p>0?(d="ok",m=`${p} experience · ${l} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const f=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",B=Y({icon:As,nameHTML:f,dot:d,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:ce,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),L=Y({icon:js,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:_e,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),E=Y({icon:Ss,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:_e,actTitle:"edit the outreach email template",actLabel:"edit email template"}),D=Y({icon:Ms,nameHTML:'<span class="edit-link" data-act="edit-doctrine" title="edit the outreach doctrine">outreach doctrine</span>',desc:"How cold emails get written — the depth bar, show-don't-tell, the kill list.",act:"edit-doctrine",actIcon:_e,actTitle:"edit the outreach doctrine",actLabel:"edit outreach doctrine"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${L}${E}${D}
     </div>`;const de={"view-profile":()=>qs(r.profile),"refresh-profile":Ps,"edit-taste":()=>re("taste"),"edit-playbook":()=>re("playbook"),"edit-template":()=>re("outreach-template"),"edit-doctrine":()=>re("outreach-doctrine"),"view-sources":Je,"refresh-sources":Hs};e.querySelectorAll("[data-act]").forEach($=>{const g=$.dataset.act;g&&de[g]&&($.onclick=de[g])})}async function Ge(){try{r.sources=await(await fetch("/api/outreach/sources")).json()}catch{r.sources=null}K()}async function Hs(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(n){u(`refresh failed: ${n.message}`),Ge();return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`refresh failed: ${n||"HTTP "+t.status}`),Ge();return}const s=await t.json();s.warning?u(s.warning):u("outreach knowledge refreshed"),r.sources={sources:s.sources||[],needs:r.sources&&r.sources.needs||[]},K()}async function Ps(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),Ke();return}if(!t.ok){const s=await t.text().catch(()=>"");u(`refresh failed: ${(s||"").trim()||"HTTP "+t.status}`),Ke();return}r.profile=await t.json(),K(),u("company-fit brief refreshed"),M()}function qs(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${Ye(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Qe(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Qe,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Qe()};function Mt(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Be(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ze(){document.getElementById("docs-scrim").classList.remove("open")}function Ns(){return document.getElementById("docs-scrim").classList.contains("open")}function Be(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Os(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Be(e)}document.getElementById("open-docs").onclick=Mt,document.getElementById("docs-close").onclick=Ze,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Ze()};function Rs(){document.getElementById("settings-scrim").classList.add("open"),K()}function Xe(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Rs,document.getElementById("settings-close").onclick=Xe,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&Xe()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Be(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(s=>{const n=s.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);n.length&&Be(n[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(s=>t.observe(s))}(),r.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Vs(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Ds(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Le(e,t){const s=document.createElement("div");return s.className="chat-msg chat-"+e,s.textContent=t||"",s}function et(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Us(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=r.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(r.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function At(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const s of e||[]){const n=Vs(s.content);if(s.role==="user")n&&t.appendChild(Le("user",n));else if(s.role==="assistant"){const a=Ds(s.content);if(!n&&!a.length)continue;const o=Le("assistant",n);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Us()),et()}async function Te(e,t,s){if(!r.meta||!r.meta.chat){u("chat needs ANTHROPIC_API_KEY in the server env");return}r.chat.es&&(r.chat.es.close(),r.chat.es=null),r.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":s||"";const n=document.getElementById("chat-messages");n.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const p=await c.json();r.chat.threadId=p.thread.id,At(p.messages||[])}catch(o){n.innerHTML='<div class="chat-empty">Failed to open chat: '+i(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function tt(){r.chat.es&&(r.chat.es.close(),r.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Ce(e){r.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Ht(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Pt(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||r.chat.streaming||!r.chat.threadId)return;e.value="",Ht(),Ce(!0);const s=document.getElementById("chat-messages"),n=s.querySelector(".chat-empty");n&&n.remove(),s.appendChild(Le("user",t));const a=Le("assistant","");a.classList.add("chat-streaming"),s.appendChild(a),et();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Ce(!1)},p=r.chat.threadId;let l;try{l=await fetch("/api/chat/"+p+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!l.ok){c((await l.text().catch(()=>"")).trim()||"HTTP "+l.status);return}const d=new EventSource("/api/chat/"+p+"/stream");r.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,et()}),d.addEventListener("end",async m=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Ce(!1),r.chat.threadId===p&&await Js(),zs(),typeof m.data=="string"&&m.data.indexOf("error")===0&&u("chat: "+m.data)}),d.onerror=()=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Ce(!1)}}async function Js(){const e=r.chat.scope,t=r.chat.scopeId,s="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const n=await fetch("/api/chat/threads?"+s);if(!n.ok)return;const a=await n.json();At(a.messages||[])}catch{}}function zs(){k(),w(),M(),r.openId&&se(r.openId)}document.getElementById("open-chat").onclick=()=>Te("global","",""),document.getElementById("chat-close").onclick=tt,document.getElementById("chat-scrim").onclick=tt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Pt()}),document.getElementById("chat-input").addEventListener("input",Ht),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Pt())}),k(),w(),M(),ws(),Ve(),Ke(),Ge()}Ys({"":{view:()=>({mount(b){b.innerHTML=Xs,en()}}),chrome:!1}},{title:"scout"});Qs();Zs();
