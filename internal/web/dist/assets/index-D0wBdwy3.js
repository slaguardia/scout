(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))$(i);new MutationObserver(i=>{for(const v of i)if(v.type==="childList")for(const E of v.addedNodes)E.tagName==="LINK"&&E.rel==="modulepreload"&&$(E)}).observe(document,{childList:!0,subtree:!0});function g(i){const v={};return i.integrity&&(v.integrity=i.integrity),i.referrerPolicy&&(v.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?v.credentials="include":i.crossOrigin==="anonymous"?v.credentials="omit":v.credentials="same-origin",v}function $(i){if(i.ep)return;i.ep=!0;const v=g(i);fetch(i.href,v)}})();function En(y,r){const g=r.replace(/^#/,"");let $=null;for(const i of Object.keys(y))(g===i||g.startsWith(i))&&($===null||i.length>$.length)&&($=i);return $===null&&""in y&&($=""),$}function $n(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function xn(y,r={}){const g=r.root??document.body,$=r.title??document.title??"",i=r.brandHref??"#",v=document.createElement("main"),E=document.createElement("header");E.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=$,w.setAttribute("aria-label",`${$} — home`),E.appendChild(w);const j=document.createElement("nav");j.className="cap-nav",j.setAttribute("aria-label","Views");for(const k of r.nav??[]){const _=document.createElement("a");_.href=k.href,_.textContent=k.label,k.ariaLabel&&_.setAttribute("aria-label",k.ariaLabel),j.appendChild(_)}E.appendChild(j);const I=document.createElement("section");I.className="tk-content",v.appendChild(E),v.appendChild(I);const L=document.createElement("div");L.className="tk-bleed";const ke=k=>{var _;for(const M of Array.from(j.querySelectorAll("a"))){const R=((_=M.getAttribute("href"))==null?void 0:_.replace(/^#/,""))??"";M.toggleAttribute("aria-current",k!==null&&k!==""&&R===k),M.hasAttribute("aria-current")&&M.setAttribute("aria-current","page")}};let ee=0;const S=()=>{const k=En(y,location.hash);if(ke(k),k===null){L.isConnected&&L.remove(),v.isConnected||g.appendChild(v),In(I,"Not found.");return}const{view:_,chrome:M}=$n(y[k]),R=M?I:L;M?(L.isConnected&&L.remove(),v.isConnected||g.appendChild(v)):(v.isConnected&&v.remove(),L.isConnected||g.appendChild(L)),R.replaceChildren();const Ee=_(),te=++ee,J=Ee.mount(R);J instanceof Promise&&J.catch(ne=>{te===ee&&Bn(R,String(ne))})};window.addEventListener("hashchange",S),S()}function In(y,r){y.replaceChildren();const g=document.createElement("div");g.className="tk-empty",g.textContent=r,y.appendChild(g)}function Bn(y,r){y.replaceChildren();const g=document.createElement("div");g.className="tk-error",g.textContent=r,y.appendChild(g)}function _n(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const g of r)g.unregister()}),window.caches&&caches.keys().then(r=>{for(const g of r)caches.delete(g)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Tn(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let r;try{r=await y.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Cn=`
<div class="layout">
<aside class="sidebar">
...
`;function Ln(y){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},g=e=>"pill pill-"+(e||"none"),$='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),v=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function E(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],P()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],x(),p.postingId){const t=r.jobs.find(n=>n.posting_id===p.postingId);t&&(p.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&W())}}function j(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",$e(),e==="jobs"?x():P()}async function I(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,He()}function ke(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function ee(e){return e.slice().sort((t,n)=>r.sort.dir*ke(t,n,r.sort.k))}const S=new Set;let k=!1;function _(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",S.has(e.dataset.v))})}function M(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(S.size&&!S.has(t.verdict||"__none__")||k&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const R=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Ee=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function te(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const J=te("scout-hidden-cols"),ne=te("scout-hidden-jcols");function Ve(){return r.view==="jobs"?{cols:Ee,hidden:ne,key:"scout-hidden-jcols"}:{cols:R,hidden:J,key:"scout-hidden-cols"}}function se(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=J.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=ne.has(e.dataset.col)?"none":""})}function $e(){const e=Ve();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=Ve(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),$e(),se()})})}function P(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=ee(M());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
  ...
  `,Lt();const s=document.getElementById("pursuit-company-link");s&&s.addEventListener("click",()=>G(e.company_id)),H(document.getElementById("pursuit-title-input"),a=>Je(e,"title",a)),H(document.getElementById("pursuit-notes-input"),a=>jt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>H(a,o=>Je(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),Y(),D()}function Ct(e){return`
    ...
    </div>`,h=s&&(St(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?Ye(s,!1):""}</div><div class="draft-actions">${h}</div>`+u,At()}function St(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function Ye(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(m,f)=>`
    ...
    </div>`;if(e.status==="failed"){const m=Mt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    <button class="btn btn-primary" id="blocks-sync-btn">Sync blocks</button>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#blocks-sync-btn");a&&a.addEventListener("click",Pt)}async function Pt(){const e=document.getElementById("blocks-sync-btn");e&&(e.disabled=!0,e.textContent="Syncing…");let t;try{t=await fetch("/api/outreach/sync",{method:"POST"})}catch(n){l(`sync failed: ${n.message}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`sync failed: ${n||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}l("blocks synced"),Y()}async function qt(e){const t=document.getElementById(`draft-edit-${e}`);if(!t)return;let n;try{n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t.value})})}catch(c){l(`save failed: ${c.message}`);return}if(!n.ok){const c=(await n.text().catch(()=>"")).trim();l(`save failed: ${c||"HTTP "+n.status}`);return}const s=await n.json(),a=p.drafts.findIndex(c=>String(c.id)===String(e));a>=0&&(p.drafts[a]=s);const o=t.closest(".draft-card");if(o){const c=o.querySelector(".lint-chips"),h=Ge(s.lint);c?c.outerHTML=h||"":h&&t.insertAdjacentHTML("afterend",h)}l("saved")}async function Nt(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(n){l(`failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`failed: ${n||"HTTP "+t.status}`);return}l("marked sent"),await z(),w()}async function K(){if(!p.postingId)return;let e;try{const t=await fetch(`/api/postings/${p.postingId}/answers`);if(!t.ok){D();return}e=await t.json()}catch{D();return}p.answers=e.answers||[],p.answersStatus=e.questions_status||"",D(),p.answers.some(t=>t.status==="generating")?Ot():Be()}function Ot(){p.answersPoll||(p.answersPoll=setInterval(K,4e3))}function Be(){p.answersPoll&&(clearInterval(p.answersPoll),p.answersPoll=null)}function D(){const e=document.getElementById("answers-section");if(!e)return;const t=p.answers,n=p.answersStatus,s=t.some(d=>d.status==="generating"),a=t.length?`<div class="answers-list">${t.map(Ut).join("")}</div>`:"",o=!!p.detecting,c=s||o?" disabled":"",h=d=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":d}</button>`;let u;n==="ok"&&t.length?u=`<button class="btn ${t.some(m=>!Xe(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+h("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+h("Re-detect questions"):u=h("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${i(Rt(n,t.length))}</div>`+a+`<div class="answers-actions">${u}</div>`,Jt()}function Rt(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function Xe(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Vt(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function Ut(e){const t=Xe(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${i(Dt(e.fail_reason))}</div>`:""}
  </div>`}function Dt(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function Jt(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",zt);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",Wt),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(H(o,h=>Yt(a,h),{multiline:!0}),o.addEventListener("input",()=>Ft(s,o)));const c=s.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>Kt(a))})}function Ft(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function zt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${p.postingId}/answers`,{method:"POST"})}catch(a){l(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await K();return}if(n.status===412){let a={};try{a=await n.json()}catch{}Gt(a.missing_blocks||[]),t&&(t.disabled=!1);return}if(n.status===503){Ze("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();l(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function Wt(){p.detecting=!0,D();try{const e=await fetch(`/api/postings/${p.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();l(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){l(`detect failed: ${e.message}`)}p.detecting=!1,await K()}async function Yt(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=p.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(p.answers[a]=s)}async function Kt(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){l(`regenerate failed: ${n.message}`);return}if(t.status===503){Ze("Answer generation isn't running in this build.");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`regenerate failed: ${n||"HTTP "+t.status}`);return}await K()}function Gt(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
  ...
  `;const B=document.getElementById("posting-add-btn");B&&B.addEventListener("click",()=>en(e)),nt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Re=>Re.classList.remove("is-on")),b.classList.add("is-on")})});const T=document.getElementById("ve-save-btn");T&&T.addEventListener("click",()=>Zt(e)),H(document.getElementById("pane-title-input"),b=>Fe(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>H(b,Re=>Fe(e,b.dataset.k,Re))),H(document.getElementById("pane-domain-input"),b=>_t(e,b)),H(document.getElementById("pane-notes-input"),b=>Tt(e,b),{multiline:!0});const C=document.getElementById("flag-toggle-btn");C&&C.addEventListener("click",()=>et(e.company_id));const we=document.getElementById("review-stamp-btn");we&&we.addEventListener("click",()=>Xt(e.company_id));const wt=document.getElementById("rescore-btn");wt&&wt.addEventListener("click",()=>Te("verdict",{company_ids:[e.company_id]}));const kt=document.getElementById("reenrich-btn");kt&&kt.addEventListener("click",()=>Te("enrich",{company_ids:[e.company_id]}))}function Qt(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function nt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||(ce(),De(e.dataset.pid))})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),ye("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function en(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const h=document.getElementById("postings-list");h&&(h.innerHTML=tt(e),nt()),t.value="",n.value="",s.disabled=!1,w(),l("link added")}async function X(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){de(`<div class="muted">Failed to load trail: ${i(s.message)}</div>`);return}if(!t.ok){de(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){de('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}de(n.map(tn).join(""))}function tn(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
