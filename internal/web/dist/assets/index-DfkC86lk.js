...
`;function en(b){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null},y=e=>"pill pill-"+(e||"none"),_='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],N()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}r.jobs=e.rows||[],I(),q(),C()}function q(){if(!h.postingId)return;const e=r.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&Z())}let S=null;function C(){const e=r.jobs.some(t=>t.outreach_draft_status==="researching");e&&!S?S=setInterval(je,4e3):!e&&S&&(clearInterval(S),S=null)}async function je(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],s=new Map(r.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),n=t.some(a=>s.get(a.posting_id)!==a.outreach_draft_status)||t.length!==r.jobs.length;r.jobs=t,n&&(I(),q()),C()}function z(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Ae(),e==="jobs"?I():N()}async function M(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const s=document.getElementById("unscored-n");s.textContent="–",s.title=`stats failed: ${t.message}`;return}r.stats=e,B()}function B(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,K()}function j(e,t,s){const n=e[s]??"",a=t[s]??"";if(s==="headcount")return(n|0)-(a|0);if(s==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[n]??3)-(o[a]??3)}return String(n).localeCompare(String(a))}function H(e){return e.slice().sort((t,s)=>r.sort.dir*j(t,s,r.sort.k))}const T=new Set;let F=!1;function Se(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",T.has(e.dataset.v))})}function le(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(T.size&&!T.has(t.verdict||"__none__")||F&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Me=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],qt=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function st(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const nt=st("scout-hidden-cols"),at=st("scout-hidden-jcols");function ot(){return r.view==="jobs"?{cols:qt,hidden:at,key:"scout-hidden-jcols"}:{cols:Me,hidden:nt,key:"scout-hidden-cols"}}function ue(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=nt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=at.has(e.dataset.col)?"none":""})}function Ae(){const e=ot();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const s=ot(),n=t.dataset.col;s.hidden.has(n)?s.hidden.delete(n):s.hidden.add(n),localStorage.setItem(s.key,JSON.stringify([...s.hidden])),Ae(),ue()})})}function N(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=H(le());document.getElementById("empty").style.display=t.length?"none":"block";for(const s of t){const n=document.createElement("tr");n.dataset.id=s.company_id,n.innerHTML=`
  ...
  `,Wt();const n=document.getElementById("pursuit-company-link");n&&n.addEventListener("click",()=>se(e.company_id)),A(document.getElementById("pursuit-title-input"),a=>dt(e,"title",a)),A(document.getElementById("pursuit-url-input"),a=>Ut(e,a)),A(document.getElementById("pursuit-notes-input"),a=>Yt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>A(a,o=>dt(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),ve(),te()}function Ft(e){return`
    ...
    </div>`;if(e.status==="failed"){const E=Zt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    <div class="draft-head">${p}<span class="dh-time">${i(s)}</span>${a?ht:""}</div>
    ${d}
    ${m}
  ...
  `;const E=document.getElementById("posting-add-btn");E&&E.addEventListener("click",()=>gs(e)),Re(),document.querySelectorAll("#ve-pick .ve-opt").forEach(g=>{g.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(J=>J.classList.remove("is-on")),g.classList.add("is-on")})});const L=document.getElementById("ve-save-btn");L&&L.addEventListener("click",()=>vs(e)),A(document.getElementById("pane-title-input"),g=>lt(e,"name",g)),document.querySelectorAll("#facts-body [data-k]").forEach(g=>A(g,J=>lt(e,g.dataset.k,J))),A(document.getElementById("pane-domain-input"),g=>Jt(e,g)),A(document.getElementById("pane-notes-input"),g=>zt(e,g),{multiline:!0});const k=document.getElementById("flag-toggle-btn");k&&k.addEventListener("click",()=>wt(e.company_id));const D=document.getElementById("review-stamp-btn");D&&D.addEventListener("click",()=>fs(e.company_id));const de=document.getElementById("rescore-btn");de&&de.addEventListener("click",()=>De("verdict",{company_ids:[e.company_id]}));const x=document.getElementById("reenrich-btn");x&&x.addEventListener("click",()=>De("enrich",{company_ids:[e.company_id]}))}function ms(e){return`
    <div class="ie-grid">
  ...
  </div>`}function K(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,n=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(n){let x="off",g="";const J=t&&t.criteria_state;J==="current"?(x="ok",g="current · verified "+Ye(t.verified_age_seconds)):J==="changed"?(x="warn",g="changed — re-distill"):J==="unverified"?(x="warn",g=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(x="warn",g="brain offline · using cache"):a&&(x="ok",g="fetched "+Ye(t.age_seconds)),o=Y({icon:St,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:x,note:g,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:ce,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=Y({icon:St,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:_e,actTitle:"edit taste.md",actLabel:"edit taste"});const c=r.sources&&r.sources.sources||[],p=c.filter(x=>x.need==="experience").length,l=c.filter(x=>x.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";p>0?(d="ok",m=`${p} experience · ${l} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const v=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",E=Y({icon:As,nameHTML:v,dot:d,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:ce,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),L=Y({icon:js,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:_e,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),k=Y({icon:Ss,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:_e,actTitle:"edit the outreach email template",actLabel:"edit email template"}),D=Y({icon:Ms,nameHTML:'<span class="edit-link" data-act="edit-doctrine" title="edit the outreach doctrine">outreach doctrine</span>',desc:"How cold emails get written — the depth bar, show-don't-tell, the kill list.",act:"edit-doctrine",actIcon:_e,actTitle:"edit the outreach doctrine",actLabel:"edit outreach doctrine"});e.innerHTML=`<div class="settings-section">
