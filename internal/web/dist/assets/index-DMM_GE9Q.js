(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))T(r);new MutationObserver(r=>{for(const g of r)if(g.type==="childList")for(const _ of g.addedNodes)_.tagName==="LINK"&&_.rel==="modulepreload"&&T(_)}).observe(document,{childList:!0,subtree:!0});function v(r){const g={};return r.integrity&&(g.integrity=r.integrity),r.referrerPolicy&&(g.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?g.credentials="include":r.crossOrigin==="anonymous"?g.credentials="omit":g.credentials="same-origin",g}function T(r){if(r.ep)return;r.ep=!0;const g=v(r);fetch(r.href,g)}})();function zn(b,i){const v=i.replace(/^#/,"");let T=null;for(const r of Object.keys(b))(v===r||v.startsWith(r))&&(T===null||r.length>T.length)&&(T=r);return T===null&&""in b&&(T=""),T}function Fn(b){return typeof b=="function"?{view:b,chrome:!0}:{view:b.view,chrome:b.chrome!==!1}}function Kn(b,i={}){const v=i.root??document.body,T=i.title??document.title??"",r=i.brandHref??"#",g=document.createElement("main"),_=document.createElement("header");_.className="cap-head";const E=document.createElement("a");E.className="brand",E.href=r,E.textContent=T,E.setAttribute("aria-label",`${T} — home`),_.appendChild(E);const J=document.createElement("nav");J.className="cap-nav",J.setAttribute("aria-label","Views");for(const w of i.nav??[]){const M=document.createElement("a");M.href=w.href,M.textContent=w.label,w.ariaLabel&&M.setAttribute("aria-label",w.ariaLabel),J.appendChild(M)}_.appendChild(J);const H=document.createElement("section");H.className="tk-content",g.appendChild(_),g.appendChild(H);const j=document.createElement("div");j.className="tk-bleed";const Ve=w=>{var M;for(const N of Array.from(J.querySelectorAll("a"))){const z=((M=N.getAttribute("href"))==null?void 0:M.replace(/^#/,""))??"";N.toggleAttribute("aria-current",w!==null&&w!==""&&z===w),N.hasAttribute("aria-current")&&N.setAttribute("aria-current","page")}};let ie=0;const ee=()=>{const w=zn(b,location.hash);if(Ve(w),w===null){j.isConnected&&j.remove(),g.isConnected||v.appendChild(g),Wn(H,"Not found.");return}const{view:M,chrome:N}=Fn(b[w]),z=N?H:j;N?(j.isConnected&&j.remove(),g.isConnected||v.appendChild(g)):(g.isConnected&&g.remove(),j.isConnected||v.appendChild(j)),z.replaceChildren();const F=M(),te=++ie,ye=F.mount(z);ye instanceof Promise&&ye.catch(be=>{te===ie&&Yn(z,String(be))})};window.addEventListener("hashchange",ee),ee()}function Wn(b,i){b.replaceChildren();const v=document.createElement("div");v.className="tk-empty",v.textContent=i,b.appendChild(v)}function Yn(b,i){b.replaceChildren();const v=document.createElement("div");v.className="tk-error",v.textContent=i,b.appendChild(v)}function Gn(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(i=>{for(const v of i)v.unregister()}),window.caches&&caches.keys().then(i=>{for(const v of i)caches.delete(v)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Qn(){let b;try{b=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!b.ok)return null;let i;try{i=await b.json()}catch{return null}return typeof i.email=="string"&&i.email?{email:i.email}:null}const Zn=`
<div class="layout">
<aside class="sidebar">
...
`;function Xn(b){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],openDetail:null,anthropicKey:null},v=e=>"pill pill-"+(e||"none"),T='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),g=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function _(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],P()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],I(),J(),j()}function J(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&ae())}let H=null;function j(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!H?H=setInterval(Ve,4e3):!e&&H&&(clearInterval(H),H=null)}async function Ve(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],s=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),n=t.some(a=>s.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,n&&(I(),J()),j()}async function ie(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{})]),ys(),i.view==="jobs"&&I()}function ee(e){i.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Ue(),e==="jobs"?I():P()}async function w(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const s=document.getElementById("unscored-n");s.textContent="–",s.title=`stats failed: ${t.message}`;return}i.stats=e,M()}function M(){const e=i.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,D()}function N(e,t,s){const n=e[s]??"",a=t[s]??"";if(s==="headcount")return(n|0)-(a|0);if(s==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[n]??3)-(o[a]??3)}return String(n).localeCompare(String(a))}function z(e){return e.slice().sort((t,s)=>i.sort.dir*N(t,s,i.sort.k))}const F=new Set;let te=!1;function ye(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",F.has(e.dataset.v))})}function be(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(F.size&&!F.has(t.verdict||"__none__")||te&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const ps=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],hs=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function kt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Et=kt("scout-hidden-cols"),$t=kt("scout-hidden-jcols");function It(){return i.view==="jobs"?{cols:hs,hidden:$t,key:"scout-hidden-jcols"}:{cols:ps,hidden:Et,key:"scout-hidden-cols"}}function se(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Et.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=$t.has(e.dataset.col)?"none":""})}function Ue(){const e=It();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const s=It(),n=t.dataset.col;s.hidden.has(n)?s.hidden.delete(n):s.hidden.add(n),localStorage.setItem(s.key,JSON.stringify([...s.hidden])),Ue(),se()})})}function xt(e){return`
    ...
    `}function _t(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>Wt(t.dataset.id))}const ms=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],fs=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function Bt(e,t,s=7){const n=document.querySelector(e);if(!n)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let c=0;c<s;c++){const d=t.map(([p,l])=>{const m=l.endsWith("%")?Math.round(parseFloat(l)*a[c%a.length])+"%":l;return`<td${p?` data-col="${p}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${d}</tr>`}n.innerHTML=o,se()}function P(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=z(be());document.getElementById("empty").style.display=t.length?"none":"block";for(const s of t){const n=document.createElement("tr");n.dataset.id=s.company_id,n.innerHTML=xt(s),n.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||pe(n.dataset.id)}),e.appendChild(n),_t(n)}se()}async function gs(e){const s=await(await fetch("/api/companies")).json();i.rows=s.rows||[];const n=document.querySelector("#t tbody"),a=z(be()).map(c=>c.company_id),o=[...n.querySelectorAll("tr")].map(c=>c.dataset.id);if(a.length!==o.length||a.some((c,d)=>c!==o[d])){P();return}for(const c of e){const d=i.rows.find(l=>l.company_id===c),p=n.querySelector(`tr[data-id="${CSS.escape(c)}"]`);if(!d||!p){P();return}p.innerHTML=xt(d),_t(p)}se()}const S=new Set;let we=!1,ke=!1,Z=!0;function vs(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=Z&&!S.has("rejected");return i.jobs.filter(s=>{const n=R(s.stage_history);return!(t&&n==="rejected"||S.size&&!S.has(n)||we&&!s.next_up||ke&&(s.outreach_count|0)>0||e&&!(s.title+" "+s.company+" "+(s.location||"")+" "+(s.description||"")+" "+(s.contacts||"")).toLowerCase().includes(e))})}function ys(){const e=document.getElementById("stage-chips");if(e){for(const t of[...S])i.applicationStages.includes(t)||S.delete(t);e.innerHTML=i.applicationStages.map(t=>`<button class="v-chip${S.has(t)?" is-on":""}" data-stage="${r(t)}">${r(t)}</button>`).join("")}}const bs=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function Lt(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(s=>({position:String((s==null?void 0:s.position)||"").trim(),email:String((s==null?void 0:s.email)||"").trim()})).filter(s=>s.position||s.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const s=t.match(bs),n=s?s[0]:"";let a=n?t.replace(n,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:n}})}function ws(e){const t=(e||[]).map(s=>({position:(s.position||"").trim(),email:(s.email||"").trim()})).filter(s=>s.position||s.email);return t.length?JSON.stringify(t):""}const Ee=()=>new Date().toISOString().slice(0,10);function X(e){if(e=String(e||"").trim(),!e)return[];try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(s=>({stage:String((s==null?void 0:s.stage)||"").trim(),date:String((s==null?void 0:s.date)||"").trim()})).filter(s=>s.stage)}catch{}return[]}function $e(e){const t=(e||[]).map(s=>({stage:(s.stage||"").trim(),date:(s.date||"").trim()})).filter(s=>s.stage);return t.sort((s,n)=>(s.date||"9999-99-99").localeCompare(n.date||"9999-99-99")),t.length?JSON.stringify(t):""}function R(e){const t=X(e);return t.length?t[t.length-1].stage:""}function Tt(e){const t=X(e);return t.length?t[t.length-1].date:""}function ks(e){const t=[["","none"]];for(const s of i.applicationStages)t.push([s,s]);return e&&!i.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function Ct(e){const t=[["","none"]];for(const s of i.outreachStatuses)t.push([s,s]);return e&&!i.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}function Es(e){const t=Lt(e);return t.length?t.map(s=>{const n=r(s.position||s.email);if(!s.email)return n;const a=r(s.position?`${s.position} — ${s.email}`:s.email);return`<a href="mailto:${r(s.email)}" title="${a}">${n}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function St(e){return`<div class="cc-row">
    ...
    </div>`}function $s(e){return`<div class="outreach-contacts" id="contacts-editor">
      <label class="cc-label">contacts</label>
      <div class="cc-list">${Lt(e).map(St).join("")}</div>
      <button class="btn cc-add" type="button">+ add contact</button>
    </div>`}function jt(e){const t=R(e.stage_history);if(!t)return i.applicationStages.length+1;const s=i.applicationStages.indexOf(t);return s<0?i.applicationStages.length:s}function Is(e,t,s){if(s==="verdict"){const n={yes:0,maybe:1,no:2,"":3};return(n[e.verdict]??3)-(n[t.verdict]??3)}if(s==="application")return jt(e)-jt(t);if(s==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(s==="created_at"||s==="last_outreach_at"){const n=e[s]||"",a=t[s]||"";return!n&&!a?0:n?a?String(a).localeCompare(String(n)):-i.jsort.dir:i.jsort.dir}return String(e[s]??"").localeCompare(String(t[s]??""))}function Ie(e){const t=e.options[e.selectedIndex],s=getComputedStyle(e),n=Ie._c||(Ie._c=document.createElement("canvas").getContext("2d"));n.font=`${s.fontWeight} ${s.fontSize} ${s.fontFamily}`;const a=n.measureText(t?t.text:"").width;e.style.width=Math.ceil(a+24)+"px"}function I(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=vs().sort((l,m)=>i.jsort.dir*Is(l,m,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const s=Z&&!S.has("rejected")?i.jobs.filter(l=>R(l.stage_history)==="rejected").length:0,n=document.getElementById("hidden-rej-n");n.textContent=s,n.style.display=s?"":"none";const a=i.jobs.filter(l=>l.next_up).length,o=document.getElementById("next-up-n");o.textContent=a,o.style.display=a?"":"none";const c=i.jobs.filter(l=>!(l.outreach_count|0)&&!(Z&&!S.has("rejected")&&R(l.stage_history)==="rejected")).length,d=document.getElementById("not-reached-n");d.textContent=c,d.style.display=c?"":"none";const p=document.getElementById("jobs-hidden-note");p.style.display=s?"":"none",s&&(p.innerHTML=`${s} rejected application${s>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{Z=!1,document.getElementById("hide-rejected").classList.remove("is-on"),I()});for(const l of t){const m=R(l.stage_history),f=Tt(l.stage_history),y=document.createElement("tr");y.dataset.id=l.posting_id;const B=ks(m).map(([x,U])=>`<option value="${r(x)}"${m===x?" selected":""}>${r(U)}</option>`).join(""),A=l.outreach_status||"",V=Ct(A).map(([x,U])=>`<option value="${r(x)}"${A===x?" selected":""}>${r(U)}</option>`).join("");y.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${l.next_up?" is-on":""}" title="${l.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${l.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${r(l.title||l.company)}</span>${_s(l.outreach_draft_status)}${l.title?`<div class="small dim">${r(l.company)}</div>`:""}</div></div></td>
      <td data-col="application"><div class="jt-stage"><select class="jt-stage-sel${m?" has-stage":""}" title="application stage — pick a new stage to record it (dated today)">${B}</select>${f?`<span class="jt-stage-date" title="when this stage was reached">${r(f)}</span>`:""}</div></td>
      <td class="small" data-col="outreach"><div class="jt-out"><span class="jt-stepper"><button class="jt-dec" title="undo one outreach"${l.outreach_count?"":" disabled"}>−</button><span class="jt-oc${l.outreach_count?"":" dim"}">${l.outreach_count||0}</span><button class="jt-inc" title="log one outreach (today)">+</button></span><select class="jt-ostatus${A?" has-status":""}" title="outreach reply status">${V}</select></div></td>
      <td class="small" data-col="last_outreach">${l.last_outreach_at?r(l.last_outreach_at):'<span class="dim">—</span>'}</td>
      <td class="small td-contacts" data-col="contacts">${Es(l.contacts)}</td>
      <td data-col="link"><a href="${g(l.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `,y.querySelector(".jt-nextup").onclick=()=>Rt(l,!1),y.querySelector(".jt-stage-sel").onchange=x=>xs(l,x.target.value),y.querySelector(".jt-ostatus").onchange=x=>_e(l,{outreach_status:x.target.value}),y.querySelector(".jt-inc").onclick=()=>_e(l,{outreach_count:(l.outreach_count||0)+1,last_outreach_at:Ee()}),y.querySelector(".jt-dec").onclick=()=>{const x=Math.max(0,(l.outreach_count||0)-1);_e(l,{outreach_count:x,...x===0?{last_outreach_at:""}:{}})},e.appendChild(y)}e.querySelectorAll(".jt-stage-sel, .jt-ostatus").forEach(Ie),se(),e.querySelectorAll("tr").forEach(l=>{l.addEventListener("click",m=>{m.target.closest("a, button, select")||Mt(l.dataset.id)})})}function xs(e,t){if(t=(t||"").trim(),!t||t===R(e.stage_history)){I();return}const s=X(e.stage_history);s.push({stage:t,date:Ee()}),_e(e,{stage_history:$e(s)})}function _s(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function Mt(e){let t=i.jobs.find(s=>s.posting_id===e);if(t||(await E(),t=i.jobs.find(s=>s.posting_id===e)),!t){u("posting not found — refresh");return}ze(),Ye(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),At("pursuit"),ae(),re(),le()}let Je=null;function At(e){Je=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function xe(){ze(),Ye(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function ze(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function re(){if(!h.postingId)return;let e;try{const s=await fetch(`/api/postings/${h.postingId}/outreach`);if(!s.ok){Be();return}e=await s.json()}catch{Be();return}h.drafts=e.drafts||[],Be();const t=h.drafts[0];t&&t.status==="researching"?Bs():ze()}function Bs(){h.poll||(h.poll=setInterval(re,4e3))}function q(e,t,{multiline:s=!1}={}){if(!e)return;let n=e.value;e.addEventListener("focus",()=>{n=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=n,e.blur()):a.key==="Enter"&&(!s||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===n.trim()){e.value=n;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),n=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=n,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),u(`save failed: ${o.message}`)}})}async function Ht(e,t,s){const n={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:s},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),I(),me(e.posting_id,{title:o.title,location:o.location})}async function Ls(e,t){const s=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();e.url=n.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",g(e.url)),me(e.posting_id,{url:n.url})}function Ts(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>Ss(e))}async function Cs(e,t){const s=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();e.company_id=n.company_id,e.company=n.company_name,ae(),E()}let ce=null;function Ss(e){ce=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const s=document.getElementById("relink-search");s&&(s.value=""),Pt(""),document.getElementById("relink-scrim").classList.add("open"),s&&s.focus()}function ne(){document.getElementById("relink-scrim").classList.remove("open"),ce=null}function Pt(e){const t=document.getElementById("relink-results");if(!t)return;const s=e.trim().toLowerCase();let n=(i.rows||[]).slice();if(s?(n=n.filter(o=>(o.name||"").toLowerCase().includes(s)),n.sort((o,c)=>{const d=(o.name||"").toLowerCase().startsWith(s)?0:1,p=(c.name||"").toLowerCase().startsWith(s)?0:1;return d-p||(o.name||"").localeCompare(c.name||"")})):n.sort((o,c)=>(o.name||"").localeCompare(c.name||"")),n=n.slice(0,60),!n.length){t.innerHTML=`<div class="relink-empty">${(i.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const a=ce?ce.company_id:"";t.innerHTML=n.map(o=>{const c=o.company_id===a,d=[o.vertical,o.location].filter(Boolean).map(r).join(" · ");return`<button type="button" class="relink-result${c?" is-current":""}"
      ...
      </button>`}).join("")}async function qt(e){const t=ce;if(!t){ne();return}if(e===t.company_id){ne();return}try{await Cs(t,e),ne(),u(`moved to ${t.company}`)}catch(s){u(`move failed: ${s.message}`)}}async function Ot(e,t,s){const n={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:s};if(!String(n.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),_(),E()}async function js(e,t){const s=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();i.openId=n.company_id,he(n),fe(n.company_id),_(),E()}async function Ms(e,t){const s=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();e.notes=n.notes}let Nt=null;function ae(){const e=h.row;if(!e)return;const t=document.getElementById("pursuit-body"),n=!!t&&Nt===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${r(e.title||"")}">`;const a=R(e.stage_history);document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a?"pill-stage":"pill-none"}">${r(a||"—")}</span>`+(e.verdict?` <span class="${v(e.verdict)}">${r(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>Re("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    <section class="pane-section role-head">
      <div id="role-body">${As(e)}</div>
    </section>
      ...
      <div class="pipeline-grid">
        ...
        <div class="pipeline-row">
          ...
          <select class="input pl-ostatus" title="outreach reply status — separate from the application stage">
            ${Ct(e.outreach_status||"").map(([d,p])=>`<option value="${r(d)}"${(e.outreach_status||"")===d?" selected":""}>${r(p)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
  ...
  `,Ps();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>pe(e.company_id)),Ts(e),q(document.getElementById("pursuit-title-input"),d=>Ht(e,"title",d)),q(document.getElementById("pursuit-url-input"),d=>Ls(e,d)),q(document.getElementById("pursuit-notes-input"),d=>qs(d),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(d=>q(d,p=>Ht(e,d.dataset.k,p),{multiline:d.tagName==="TEXTAREA"})),Be(),ue(),t&&(t.scrollTop=n),Nt=e.posting_id}function As(e){return`
    ...
    </div>`}function Hs(e){const t=X(e.stage_history),s=t.map((a,o)=>`
    ...
    </div>`}function Ps(){const e=h.row;document.querySelectorAll("#pursuit-body .stage-entry .stage-date").forEach(a=>{a.addEventListener("change",o=>{const c=+o.target.closest(".stage-entry").dataset.i,d=X(e.stage_history);d[c]&&(d[c].date=o.target.value,oe({stage_history:$e(d)}))})}),document.querySelectorAll("#pursuit-body .stage-entry .stage-del").forEach(a=>{a.addEventListener("click",o=>{const c=+o.target.closest(".stage-entry").dataset.i,d=X(e.stage_history);d.splice(c,1),oe({stage_history:$e(d)})})});const t=document.querySelector("#pursuit-body .stage-add-btn");t&&t.addEventListener("click",()=>{const a=document.querySelector("#pursuit-body .stage-add-sel"),o=document.querySelector("#pursuit-body .stage-add-date"),c=(a.value||"").trim();if(!c)return;const d=X(e.stage_history);d.push({stage:c,date:o.value||Ee()}),oe({stage_history:$e(d)})});const s=document.querySelector("#pursuit-body .pl-ostatus");s&&s.addEventListener("change",a=>oe({outreach_status:a.target.value}));const n=document.querySelector("#pursuit-body .pt-nextup");n&&n.addEventListener("click",()=>Rt(h.row,!0))}async function Rt(e,t){let s;try{s=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){u(`save failed: ${a.message}`);return}if(!s.ok){const a=(await s.text().catch(()=>"")).trim();u(`save failed: ${a||"HTTP "+s.status}`);return}const n=await s.json();e.next_up=n.next_up,I(),me(e.posting_id,{next_up:n.next_up}),t&&ae(),u(e.next_up?"queued next up":"removed from the queue")}async function Fe(e,t){const s={stage_history:e.stage_history||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",outreach_status:e.outreach_status||"",contacts:e.contacts||"",notes:e.notes||"",...t};let n;try{n=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)})}catch(o){return u(`save failed: ${o.message}`),null}if(!n.ok){const o=(await n.text().catch(()=>"")).trim();return u(`save failed: ${o||"HTTP "+n.status}`),null}const a=await n.json();return Object.assign(e,{stage_history:a.stage_history,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,outreach_status:a.outreach_status,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),me(e.posting_id,{stage_history:a.stage_history,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function qs(e){const t=h.row,s={stage_history:t.stage_history||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",outreach_status:t.outreach_status||"",contacts:t.contacts||"",notes:e},n=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();t.notes=a.notes,I()}async function oe(e){await Fe(h.row,e)&&(I(),ae(),u("tracking saved"))}async function Dt(e){await Fe(h.row,{contacts:e})&&I()}async function _e(e,t){await Fe(e,t)&&(I(),h.postingId===e.posting_id&&(h.row=e,ae()),u("tracking saved"))}function Be(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.row,s=h.drafts,n=s[0]||null,a=s.slice(1),o=`
    ...
    ${$s(t.contacts)}`,d=n&&(Os(n.status)||n.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${n?"Draft again":"Draft outreach"}</button>`,p=a.length?`
    <details class="draft-history" ${h.openHist?"open":""}>
      <summary>${a.length} earlier draft${a.length>1?"s":""}</summary>
      <div id="draft-history-body">${a.map(l=>Ut(l,!0)).join("")}</div>
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${n?Ut(n,!1):""}</div><div class="draft-actions">${d}</div>`+p,Js()}function Os(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const Ns='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',Rs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',Vt=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${Ns}</button>`,Ke=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],Ds='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function Vs(e){let t=Ke.findIndex(n=>n.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${Ke.map((n,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",c=a<t?Ds:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${n.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${Ke[t].active}…</span></div>
  </div>`}function Ut(e,t){const s=(l,m,f="")=>`
    <div class="draft-head">
      <span class="${l}">${m}</span>${f}
    ...
    </div>`;if(e.status==="failed"){const l=Us(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      <div class="draft-sentbody">${r(We(e)||"(empty)")}</div>
      ${de(e)}
    </div>`;if(e.status==="sent")return`<div class="draft-card dc-sent" data-did="${e.id}">
      ${s("pill pill-yes","sent",t?"":Vt)}
      ${e.sent_at?`<div class="draft-note">Sent ${r((e.sent_at||"").replace("T"," ").slice(0,16))}</div>`:""}
      <div class="draft-sentbody">${r(We(e)||"(empty)")}</div>
      ${de(e)}
    </div>`;const n=We(e),a=e.status==="no_hook",o=a?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let c="";if(a)try{c=JSON.parse(e.hook||"{}").reasoning||""}catch{}const d=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${c?" "+r(c):""}</div>`:"";if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
      <div class="draft-head">${o}</div>
      ${d}
      <div class="draft-sentbody">${r(n||"(empty)")}</div>
      ${de(e)}
    </div>`;const p=n||a;return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    <div class="draft-head">${o}${n?Vt:""}</div>
    ${d}
    ${p?`<textarea class="draft-textarea" id="draft-edit-${e.id}" spellcheck="false">${r(n)}</textarea>
    ${Jt(e.lint)}
    <div class="draft-actions">
      <button class="btn btn-primary draft-sent-btn" title="mark this email sent — bumps the outreach count">${Rs}Mark sent</button>
    ...
    ${de(e)}
  </div>`}function We(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function de(e){let t="",s=null,n=null;try{s=JSON.parse(e.research||"null")}catch{}try{n=JSON.parse(e.hook||"null")}catch{}if(s&&typeof s=="object"){const a=(p,l)=>l?`<div class="tr-line"><span class="tr-key">${p}:</span> ${r(String(l))}</div>`:"",o=s.role||{},c=Array.isArray(s.hooks)?s.hooks:[],d=c.map(p=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function Jt(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(s=>`<span class="lint-chip" title="${r(s.message||"")}"><code>${r(s.code||"")}</code>${r(s.message||"")}</span>`).join("")+"</div>":""}function Us(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(s=>`<li>${r(s.claim||s.message||String(s))}${s.why?` <span class="vl-why">— ${r(s.why)}</span>`:""}</li>`).join("")+"</ul>":""}function Js(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#contacts-editor");if(t){const p=t.querySelector(".cc-list"),l=()=>ws(Array.from(p.querySelectorAll(".cc-row")).map(f=>({position:f.querySelector(".cc-pos").value,email:f.querySelector(".cc-email").value}))),m=f=>{f.querySelectorAll(".cc-pos, .cc-email").forEach(y=>{y.addEventListener("change",()=>Dt(l())),y.addEventListener("keydown",B=>{B.key==="Enter"&&(B.preventDefault(),B.target.blur())})}),f.querySelector(".cc-del").addEventListener("click",()=>{f.remove(),Dt(l())})};p.querySelectorAll(".cc-row").forEach(m),t.querySelector(".cc-add").addEventListener("click",()=>{const f=document.createElement("div");f.innerHTML=St({position:"",email:""});const y=f.firstElementChild;p.appendChild(y),m(y),y.querySelector(".cc-pos").focus()})}const s=()=>new Date().toISOString().slice(0,10),n=h.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>oe({outreach_count:(n.outreach_count||0)+1,last_outreach_at:s()}));const o=e.querySelector(".pt-outreach-dec");o&&o.addEventListener("click",()=>{const p=Math.max(0,(n.outreach_count||0)-1);oe({outreach_count:p,...p===0?{last_outreach_at:""}:{}})});const c=e.querySelector("#draft-start-btn");c&&c.addEventListener("click",()=>Le()),e.querySelectorAll(".draft-retry-btn").forEach(p=>p.addEventListener("click",()=>Le())),e.querySelectorAll(".draft-regen-btn").forEach(p=>p.addEventListener("click",()=>Le(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(p=>{const l=p.dataset.did,m=p.querySelector(".draft-textarea");m&&q(m,B=>Fs(l,B),{multiline:!0});const f=p.querySelector(".draft-sent-btn");f&&f.addEventListener("click",()=>Ks(l));const y=p.querySelector(".draft-copy-btn");y&&y.addEventListener("click",()=>{const B=p.querySelector(".draft-textarea"),A=p.querySelector(".draft-sentbody"),V=B?B.value:A?A.textContent:"";un(V,"email copied")})});const d=e.querySelector("details.draft-history");d&&d.addEventListener("toggle",()=>{h.openHist=d.open})}async function Le(e=!1){const t=document.getElementById("outreach-section"),s=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));s&&(s.disabled=!0);let n;try{const o=e?"?regenerate=1":"";n=await fetch(`/api/postings/${h.postingId}/outreach${o}`,{method:"POST"})}catch(o){u(`draft failed: ${o.message}`),s&&(s.disabled=!1);return}if(n.status===202){let o={};try{o=await n.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&u(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await re(),E();return}if(n.status===409){await re(),u("a draft is already active");return}if(n.status===412){let o={};try{o=await n.json()}catch{}zs(o.need,o.error),s&&(s.disabled=!1);return}if(n.status===503){const o=document.getElementById("outreach-section");if(o){const c=document.createElement("div");c.className="draft-note",c.textContent="Outreach engine not running in this build.",o.appendChild(c)}s&&(s.disabled=!1);return}const a=(await n.text().catch(()=>"")).trim();u(`draft failed: ${a||"HTTP "+n.status}`),s&&(s.disabled=!1)}function zs(e,t){const s=document.getElementById("outreach-section");if(!s)return;const n=s.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"Discover sources",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,n?n.replaceWith(c):s.appendChild(c);const d=c.querySelector("#gate-fix-btn");d&&d.addEventListener("click",()=>a?Y("outreach-template"):nt());const p=c.querySelector("#gate-retry-btn");p&&p.addEventListener("click",Le)}async function Fs(e,t){const s=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json(),a=h.drafts.findIndex(d=>String(d.id)===String(e));a>=0&&(h.drafts[a]=n);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const d=c.querySelector(".lint-chips"),p=Jt(n.lint);d?d.outerHTML=p||"":p&&o.insertAdjacentHTML("afterend",p)}}async function Ks(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(n){u(`failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`failed: ${n||"HTTP "+t.status}`);return}u("marked sent"),await re(),await E();const s=i.jobs.find(n=>n.posting_id===h.postingId);s&&me(s.posting_id,{outreach_count:s.outreach_count,last_outreach_at:s.last_outreach_at,next_up:s.next_up})}async function le(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){ue();return}e=await t.json()}catch{ue();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",ue(),h.answers.some(t=>t.status==="generating")?Ws():Ye()}function Ws(){h.answersPoll||(h.answersPoll=setInterval(le,4e3))}function Ye(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function ue(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,s=h.answersStatus,n=t.some(l=>l.status==="generating"),a=t.length?`<div class="answers-list">${t.map(Qs).join("")}</div>`:"",o=!!h.detecting,c=n||o?" disabled":"",d=l=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":l}</button>`;let p;s==="ok"&&t.length?p=`<button class="btn ${t.some(m=>!zt(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${n?"Drafting…":"Draft answers"}</button>`+d("Re-detect"):s===""||s==="unreachable"?p=`<button class="btn btn-primary" id="answers-start-btn"${c}>${n?"Drafting…":"Draft answers"}</button>`+d("Re-detect questions"):p=d("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r(Ys(s,t.length))}</div>`+a+`<div class="answers-actions">${p}</div>`,Xs()}function Ys(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function zt(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Gs(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function Qs(e){const t=zt(e),s=e.edited&&e.edited.trim(),n=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
  ...
  </div>`}function Zs(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function Xs(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",Ft);const s=e.querySelector("#answers-redetect-btn");s&&s.addEventListener("click",tn),e.querySelectorAll(".answer-card[data-aid]").forEach(n=>{const a=n.dataset.aid,o=n.querySelector(".answer-textarea");o&&(q(o,d=>sn(a,d),{multiline:!0}),o.addEventListener("input",()=>en(n,o)));const c=n.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>nn(a))})}function en(e,t){const s=e.querySelector(".answer-count");if(!s)return;const n=t.value.length,a=s.textContent.includes("/")?parseInt(s.textContent.split("/")[1],10):0;s.textContent=a?`${n} / ${a}`:`${n} chars`,s.classList.toggle("over",!!a&&n>a)}async function Ft(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let s;try{s=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){u(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(s.status===202){await le();return}if(s.status===412){let a={};try{a=await s.json()}catch{}an(a.error),t&&(t.disabled=!1);return}if(s.status===503){Kt("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const n=(await s.text().catch(()=>"")).trim();u(`draft failed: ${n||"HTTP "+s.status}`),t&&(t.disabled=!1)}async function tn(){h.detecting=!0,ue();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();u(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){u(`detect failed: ${e.message}`)}h.detecting=!1,await le()}async function sn(e,t){const s=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=n)}async function nn(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(s){u(`regenerate failed: ${s.message}`);return}if(t.status===503){Kt("Answer generation isn't running in this build.");return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`regenerate failed: ${s||"HTTP "+t.status}`);return}await le()}function an(e){const t=document.getElementById("answers-section");if(!t)return;const s=t.querySelector(".answers-actions"),n=document.createElement("div");n.className="blocks-gate",n.innerHTML=`
    ...
    </div>`,s?s.replaceWith(n):t.appendChild(n);const a=n.querySelector("#answers-fix-btn");a&&a.addEventListener("click",nt);const o=n.querySelector("#answers-retry-btn");o&&o.addEventListener("click",Ft)}function Kt(e){const t=document.getElementById("answers-section");if(!t)return;const s=document.createElement("div");s.className="draft-note",s.textContent=e,t.appendChild(s)}async function pe(e){var d,p;const t=document.getElementById("pane"),s=document.getElementById("scrim"),n=i.openId===e&&t.classList.contains("open"),a=n?((d=document.getElementById("pane-body"))==null?void 0:d.scrollTop)??0:0,o=n?(p=document.getElementById("trace-body"))==null?void 0:p.innerHTML:null;i.openId=e,t.classList.add("open"),s.classList.add("open"),t.setAttribute("aria-hidden","false"),At("company"),n||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let c;try{const l=await fetch(`/api/companies/${e}`);if(!l.ok)throw new Error(`HTTP ${l.status}`);c=await l.json()}catch(l){n||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${r(l.message)}</div>`);return}if(i.openId===e){if(he(c),n){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const l=document.getElementById("pane-body");l&&(l.scrollTop=a)}fe(e)}}function Te(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function he(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${r(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
    <span class="${v(e.has_verdict?e.verdict:"")}">${r(e.has_verdict?e.verdict:"unscored")}</span>
  `;const t=document.getElementById("pane-chat");t&&(t.style.display=i.meta&&i.meta.chat?"":"none",t.onclick=()=>Re("company",e.company_id,e.name));const s=e.model==="manual",n=e.has_verdict?`
    <dl class="kv">
  ...
  `:'<div class="muted">No enrichment yet. Run <code>scout enrich</code>.</div>',c=!i.meta||i.meta.control!==!1,d=c&&i.meta&&i.meta.verdict?'<button class="h3-action" id="rescore-btn" title="re-score just this company — replaces the current verdict, manual or not">↻ re-score</button>':"",p=c&&e.domain?'<button class="h3-action" id="reenrich-btn" title="re-fetch this company’s site now">↻ re-enrich</button>':"",l=Object.keys(e.raw_json||{}).sort(),m=l.length===0?"":`
    <details class="raw-json">
      <summary>Raw row <span class="dim">(${l.length} fields)</span></summary>
      <table><tbody>
        ${l.map(k=>`<tr><td class="k">${r(k)}</td><td>${r(e.raw_json[k])}</td></tr>`).join("")}
      </tbody></table>
    </details>
  `,f=`
    <div class="flag-bar">
  ...
  `;const y=document.getElementById("posting-add-btn");y&&y.addEventListener("click",()=>dn(e)),Qe(),document.querySelectorAll("#ve-pick .ve-opt").forEach(k=>{k.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Q=>Q.classList.remove("is-on")),k.classList.add("is-on")})});const B=document.getElementById("ve-save-btn");B&&B.addEventListener("click",()=>cn(e)),q(document.getElementById("pane-title-input"),k=>Ot(e,"name",k)),document.querySelectorAll("#facts-body [data-k]").forEach(k=>q(k,Q=>Ot(e,k.dataset.k,Q))),q(document.getElementById("pane-domain-input"),k=>js(e,k)),q(document.getElementById("pane-notes-input"),k=>Ms(e,k),{multiline:!0});const A=document.getElementById("flag-toggle-btn");A&&A.addEventListener("click",()=>Wt(e.company_id));const V=document.getElementById("review-stamp-btn");V&&V.addEventListener("click",()=>rn(e.company_id));const x=document.getElementById("rescore-btn");x&&x.addEventListener("click",()=>et("verdict",{company_ids:[e.company_id]}));const U=document.getElementById("reenrich-btn");U&&U.addEventListener("click",()=>et("enrich",{company_ids:[e.company_id]}))}function on(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function rn(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let s;try{s=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){u(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!s.ok){const o=await s.text().catch(()=>"");u(`failed: HTTP ${s.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const n=await s.json(),a=i.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=n.reviewed_at,P()),i.openId===e&&(he(n),fe(e)),u("reviewed")}async function Wt(e){const t=i.rows.find(o=>o.company_id===e),s=!(t&&t.flagged);let n;try{n=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:s})})}catch(o){u(`failed: ${o.message}`);return}if(!n.ok){const o=await n.text().catch(()=>"");u(`failed: HTTP ${n.status}${o?" — "+o:""}`);return}const a=await n.json();t&&(t.flagged=a.flagged,P()),i.openId===e&&(he(a),fe(e)),E(),u(a.flagged?"flagged":"unflagged")}async function cn(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){u("Pick yes, maybe, or no.");return}const s=t.dataset.v,n=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:s,reason:n})})}catch(d){u(`save failed: ${d.message}`),a.disabled=!1;return}if(!o.ok){const d=await o.text().catch(()=>"");u(`save failed: HTTP ${o.status}${d?" — "+d:""}`),a.disabled=!1;return}const c=await o.json();he(c),fe(c.company_id),_(),w(),E(),u("verdict saved")}function Ge(e){const t=e.postings||[];return t.length?t.map(s=>{const n=[s.location,s.source==="capture"?"captured":"added",(s.created_at||"").slice(0,10)].filter(Boolean).map(r).join(" · "),a=R(s.stage_history),o=Tt(s.stage_history),c=[s.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?"pill-stage":"pill-none"}">${r(a||"—")}</span>`,`<span class="pt-meta">${a?o?r(o):"tracked":"not applied"}</span>`,`<span class="pt-meta">${s.outreach_count?`${s.outreach_count} sent · last ${r(s.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join(""),d=i.meta&&i.meta.chat?`<button class="pcard-chat" data-pid="${r(s.id)}" data-ptitle="${r(s.title||"")}" title="chat about this role">chat</button>`:"";return`
    <div class="brain-node posting-card" data-pid="${r(s.id)}" title="open the pursuit — tracking, outreach, drafts">
      ...
      <div class="pcard-status">${c}${d}<span class="pcard-open">open →</span></div>
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function me(e,t){const s=i.openDetail;if(!s||!i.openId)return;const n=(s.postings||[]).find(o=>String(o.id)===String(e));if(!n)return;Object.assign(n,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Ge(s),Qe())}function Qe(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||Mt(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),Re("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function dn(e){const t=document.getElementById("posting-url"),s=document.getElementById("posting-title"),n=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){u("Enter a URL first."),t.focus();return}n.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:s.value.trim()})})}catch(p){u(`add failed: ${p.message}`),n.disabled=!1;return}if(!o.ok){const p=await o.text().catch(()=>"");u(`add failed: HTTP ${o.status}${p?" — "+p:""}`),n.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(p=>p.id!==c.id),e.postings.unshift(c);const d=document.getElementById("postings-list");d&&(d.innerHTML=Ge(e),Qe()),t.value="",s.value="",n.disabled=!1,E(),u("link added")}async function fe(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(n){Ce(`<div class="muted">Failed to load trail: ${r(n.message)}</div>`);return}if(!t.ok){Ce(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const s=(await t.json()).events||[];if(s.length===0){Ce('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Ce(s.map(ln).join(""))}function ln(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(r);return e.run_id&&t.push("run "+r(e.run_id.slice(0,8))),`
    ...
    </div>`}function Ce(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let Yt;function u(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(Yt),Yt=setTimeout(()=>t.classList.remove("show"),2200)}async function un(e,t="copied"){if(!e){u("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const s=document.createElement("textarea");s.value=e,s.style.position="fixed",s.style.opacity="0",document.body.appendChild(s),s.select(),document.execCommand("copy"),document.body.removeChild(s)}u(t)}catch(s){u(`copy failed: ${s.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function Ze(){try{const n=await fetch("/api/meta");if(!n.ok)return;i.meta=await n.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const s=document.getElementById("open-chat");s&&(s.style.display=i.meta.chat?"":"none")}async function Xe(){let e;try{const n=await fetch("/api/runs");if(!n.ok)return;e=await n.json()}catch{return}const t=e.busy_stage||"",s=document.getElementById("run-busy");t?(s.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):s.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Se=null;async function et(e,t){if(i.meta&&i.meta.control===!1){u("control surface disabled");return}let s;try{s=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){u(`run failed: ${a.message}`);return}if(s.status===409){u("a job is already running");return}if(s.status===412){const a=await s.text();u(a.trim());return}if(!s.ok){u(`run failed: HTTP ${s.status}`);return}const{job_id:n}=await s.json();ss(e,n,t)}async function pn(e){const t=new FormData;t.append("csv",e);let s;try{s=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){u(`upload failed: ${a.message}`);return}if(s.status===409){u("a job is already running");return}if(!s.ok){u(`upload failed: HTTP ${s.status}`);return}const{job_id:n}=await s.json();ss("ingest",n)}const hn=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let je=[],K=new Set,W="company";function Gt(e){W=e,document.querySelectorAll("#add-kind .v-chip").forEach(n=>n.classList.toggle("is-on",n.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),s=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',s.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',s.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",Qt()}function tt(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function Qt(){const e=document.getElementById("add-note");tt()?e.innerHTML=W==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and description — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=W==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function mn(){hn.forEach(n=>{document.getElementById(n).value=""}),document.getElementById("add-vertical-filter").value="",K=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),Gt(i.view==="jobs"?"job":"company");const s=document.getElementById("add-stage");s.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(n=>`<option value="${r(n.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const n=await(await fetch("/api/facets")).json();(n.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,s.appendChild(o)}),je=n.verticals||[]}catch{je=[]}Zt()}function Me(){document.getElementById("add-scrim").classList.remove("open")}function Zt(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),s=je.filter(n=>!t||n.toLowerCase().includes(t));s.length?(e.innerHTML=s.map(n=>`<button type="button" class="vchip${K.has(n)?" sel":""}" data-v="${r(n)}">${r(n)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(n=>n.addEventListener("click",()=>{const a=n.dataset.v;K.has(a)?K.delete(a):K.add(a),n.classList.toggle("sel"),Xt()}))):e.innerHTML=`<div class="none">${je.length?"no match":"no verticals in the set yet"}</div>`,Xt()}function Xt(){const e=K.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function es(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function ts(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){u(W==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const s=document.getElementById("add-save"),n=s.textContent;s.disabled=!0,tt()&&(s.textContent="reading page…");const a=()=>{s.disabled=!1,s.textContent=n},o=f=>document.getElementById(f).value.trim(),c=tt();let d,p;c?(d="/api/capture",p={url:es(t),kind:W==="company"?"company_page":"job_posting",fields:W==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...K].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):W==="company"?(d="/api/companies",p={website:t,name:o("add-name"),vertical:[...K].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(d="/api/postings",p={url:es(t),title:o("add-title"),company:o("add-job-company")});let l;try{l=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)})}catch(f){u(`add failed: ${f.message}`),a();return}if(!l.ok){let f=`HTTP ${l.status}`;try{const y=await l.text();try{f=JSON.parse(y).error||f}catch{f=y.trim()||f}}catch{}if(a(),l.status===409){u(f||"That company is already in the list."),e.focus(),e.select();return}u(`add failed: ${f}`);return}const m=await l.json();if(a(),c&&!m.company_id){u(m.note||"couldn't classify that page");return}if(Me(),_(),w(),E(),W==="job"){const f=m.posting&&m.posting.title||"job link";u(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),ee("jobs")}else c?(u(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`),pe(m.company_id)):u("company added")}function ss(e,t,s){Se=t;const n=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="",n.classList.add("open"),Xe();const o=new EventSource(`/api/jobs/${t}/stream`),c=(d,p)=>{const l=document.createElement("div"),m=!p&&/^\s*warn:/i.test(d);l.className="ln"+(p?" ln-err":m?" ln-warn":""),l.textContent=m?d.replace(/^\s*warn:\s*/i,"⚠ "):d,a.appendChild(l),a.scrollTop=a.scrollHeight};o.addEventListener("line",d=>c(d.data,/error|failed/i.test(d.data))),o.addEventListener("end",d=>{o.close(),Se=null,c(`— ${d.data} —`,d.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",u(`${e} ${d.data}`),s&&Array.isArray(s.company_ids)&&s.company_ids.length>0?gs(s.company_ids):_(),w(),Xe(),E(),i.openId&&pe(i.openId)}),o.onerror=()=>{o.close()}}async function fn(){if(Se)try{await fetch(`/api/jobs/${Se}/cancel`,{method:"POST"})}catch{}}let C=null;const gn={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function st(e){return e==="application-stages"||e==="outreach-statuses"}function Ae(e){if(e==="outreach-template")return"outreach template";if(e==="taste-filter")return"pre-filter rules";if(e==="playbook")return"playbook";if(e==="application-stages")return"application stages";if(e==="outreach-statuses")return"outreach statuses";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(gn[t]||t)+" prompt"}return e+".md"}async function Y(e){C=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Ae(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const s=!!e&&e.startsWith("outreach-prompts/"),n=s?e.slice(17):"",a=e==="taste-filter"||s&&n!=="fill";document.getElementById("editor-toggle-row").style.display=a?"":"none",document.getElementById("editor-reset").style.display=s?"":"none",a&&(document.getElementById("editor-toggle-label").textContent=e==="taste-filter"?"Enable the pre-filter (off → bulk verdict runs score every company; the rules below are kept either way)":"Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const d=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${d||"HTTP "+o.status}`;return}const c=await o.json();st(e)?(document.getElementById("editor-title").textContent="edit "+Ae(e)+" — one per line",document.getElementById("editor-text").value=(c.statuses||[]).join(`
`)):document.getElementById("editor-text").value=c.content||"",a&&(document.getElementById("editor-enabled").checked=c.enabled!==!1),c.taste_version&&(document.getElementById("editor-ver").textContent="version "+c.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function He(){document.getElementById("editor-scrim").classList.remove("open"),C=null}const vn=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function nt(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{ot(await(await fetch("/api/outreach/sources")).json())}catch(e){u(`failed to load sources: ${e.message}`)}}function at(){document.getElementById("sources-scrim").classList.remove("open")}function ot(e){const t=document.getElementById("sources-list");if(!t)return;const s=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):vn,n={};(e&&e.sources||[]).forEach(a=>{(n[a.need]=n[a.need]||[]).push(a)}),t.innerHTML=s.map(a=>{const o=n[a.key]||[],c=o.length?o.map(d=>`<li><span class="src-title">${r(d.title||d.page_id)}</span><button class="src-rm" data-need="${r(a.key)}" data-id="${r(d.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${r(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>bn(a.dataset.need,a.dataset.id)))}async function yn(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(n){u(`refresh failed: ${n.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){u(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const s=await t.json();s.warning?u(s.warning):u("sources refreshed"),ot(s),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function bn(e,t){let s;try{s=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(n){u(`remove failed: ${n.message}`);return}if(!s.ok){u(`remove failed: ${(await s.text().catch(()=>"")).trim()||"HTTP "+s.status}`);return}ot(await s.json())}async function wn(){if(!C)return;const e=document.getElementById("editor-text").value;let t;if(st(C))t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)};else{t={content:e};const o=C.startsWith("outreach-prompts/")&&C!=="outreach-prompts/fill";(C==="taste-filter"||o)&&(t.enabled=document.getElementById("editor-enabled").checked)}let s;try{s=await fetch(`/api/${C}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){u(`save failed: ${o.message}`);return}if(!s.ok){u(`save failed: ${(await s.text().catch(()=>"")).trim()||"HTTP "+s.status}`);return}const n=await s.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version);const a=st(C);u(`${Ae(C)} saved`),He(),a&&ie(),w()}async function kn(){if(!C||!C.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${C}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(s){u(`reset failed: ${s.message}`);return}if(!e.ok){u(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",u(`${Ae(C)} reset to default`)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;i.sort.k===t?i.sort.dir*=-1:(i.sort.k=t,i.sort.dir=1),P()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;i.jsort.k===t?i.jsort.dir*=-1:(i.jsort.k=t,i.jsort.dir=1),I()}}),document.getElementById("tab-companies").onclick=()=>ee("companies"),document.getElementById("tab-jobs").onclick=()=>ee("jobs"),document.getElementById("q").oninput=P,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;F.has(t)?F.delete(t):F.add(t),ye(),P()})}),document.getElementById("flag-filter").addEventListener("click",e=>{te=!te,e.currentTarget.classList.toggle("is-on",te),P()}),document.getElementById("jq").oninput=I,document.getElementById("hide-rejected").addEventListener("click",e=>{Z=!Z,e.currentTarget.classList.toggle("is-on",Z),I()}),document.getElementById("stage-chips").addEventListener("click",e=>{const t=e.target.closest(".v-chip[data-stage]");if(!t)return;const s=t.dataset.stage;S.has(s)?S.delete(s):S.add(s),t.classList.toggle("is-on",S.has(s)),I()}),document.getElementById("next-up-filter").addEventListener("click",e=>{we=!we,e.currentTarget.classList.toggle("is-on",we),I()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{ke=!ke,e.currentTarget.classList.toggle("is-on",ke),I()}),Ue(),se(),document.getElementById("pane-close").onclick=Te,document.getElementById("scrim").onclick=Te,document.getElementById("pursuit-close").onclick=xe,document.getElementById("pursuit-scrim").onclick=xe,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){vt();return}if(An()){mt();return}if(document.getElementById("profile-scrim").classList.contains("open")){ht();return}if(document.getElementById("add-scrim").classList.contains("open")){Me();return}if(document.getElementById("run-scrim").classList.contains("open")){Pe();return}if(document.getElementById("help-scrim").classList.contains("open")){qe();return}if(document.getElementById("relink-scrim").classList.contains("open")){ne();return}const t=document.getElementById("pane").classList.contains("open"),s=document.getElementById("pursuit-pane").classList.contains("open");if(t||s){if(Je==="pursuit"&&s){xe();return}if(Je==="company"&&t){Te();return}if(t){Te();return}xe();return}if(document.getElementById("key-scrim").classList.contains("open")){pt();return}if(document.getElementById("sources-scrim").classList.contains("open")){at();return}if(document.getElementById("editor-scrim").classList.contains("open")){He();return}if(document.getElementById("settings-scrim").classList.contains("open")){ft();return}});let it=null;const En={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function ns(e){if(i.meta&&i.meta.control===!1){u("control surface disabled");return}it=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=En[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),s=i.stats||{},n=Math.max(0,(s.total_companies||0)-(s.enriched_ok||0));e==="verdict"&&n>0?(document.getElementById("run-warn-text").textContent=`${n} ${n===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${n===1?"it":"them"}. Run Enrich first to include ${n===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Pe(){document.getElementById("run-scrim").classList.remove("open"),it=null}document.getElementById("btn-enrich").onclick=()=>ns("enrich"),document.getElementById("btn-verdict").onclick=()=>ns("verdict"),document.getElementById("run-cancel").onclick=Pe,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Pe()},document.getElementById("run-go").onclick=()=>{const e=it,t=document.getElementById("run-only-blanks").checked,s=parseInt(document.getElementById("run-workers-input").value,10);if(Pe(),!e)return;const n={};t&&(n.only_blanks=!0),s>0&&(n.workers=s),et(e,n)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=mn;const $n={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function as(e){const t=$n[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const s=document.getElementById("help-items");if(s.innerHTML="",t.intro){const n=document.createElement("p");n.className="help-intro",n.textContent=t.intro,s.appendChild(n)}t.items.forEach(n=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=n.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=n.desc;const d=document.createElement("a");d.className="help-link",d.textContent="Learn more →",d.onclick=()=>{qe(),cs(),Hn(n.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(d),s.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function qe(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>as("add"),document.getElementById("help-run").onclick=()=>as("run"),document.getElementById("help-close").onclick=qe,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&qe()},document.getElementById("add-cancel").onclick=Me,document.getElementById("add-save").onclick=ts,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Me()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Gt(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Qt),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),ts()))}),document.getElementById("add-vertical-filter").addEventListener("input",Zt),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&pn(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=fn,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=He,document.getElementById("editor-save").onclick=wn,document.getElementById("editor-reset").onclick=kn,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&He()},document.getElementById("sources-close").onclick=at,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&at()},document.getElementById("sources-refresh-btn").onclick=yn,document.getElementById("key-cancel").onclick=pt,document.getElementById("key-save").onclick=rs,document.getElementById("key-remove").onclick=Sn,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&pt()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),rs())}),document.getElementById("relink-cancel").onclick=ne,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&ne()},document.getElementById("relink-search").addEventListener("input",e=>{Pt(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&qt(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&qt(t.dataset.id)});function rt(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const s=Math.round(t/60);if(s<90)return`${s}m ago`;const n=Math.round(s/60);return n<48?`${n}h ago`:`${Math.round(n/24)}d ago`}async function ct(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}D()}const G='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',ge='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',In='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',os='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',xn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',_n='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',dt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',Bn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>',Ln='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>';function O(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${r(e.note)}</span>`:""}</div>`:"",s=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${s} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function D(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,n=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(n){let $="off",L="";const ve=t&&t.criteria_state;ve==="current"?($="ok",L="current · verified "+rt(t.verified_age_seconds)):ve==="changed"?($="warn",L="changed — re-distill"):ve==="unverified"?($="warn",L=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?($="warn",L="brain offline · using cache"):a&&($="ok",L="fetched "+rt(t.age_seconds)),o=O({icon:os,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:$,note:L,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:ge,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=O({icon:os,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:G,actTitle:"edit taste.md",actLabel:"edit taste"});const c=i.sources&&i.sources.sources||[],d=c.filter($=>$.need==="experience").length,p=c.filter($=>$.need==="voice").length;let l="off",m="not discovered yet — refresh from the brain";d>0?(l="ok",m=`${d} experience · ${p} voice`):c.length>0&&(l="warn",m="no experience yet — refresh");const f=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",y=O({icon:Bn,nameHTML:f,dot:l,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:ge,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),B=O({icon:xn,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:G,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),A=O({icon:_n,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:G,actTitle:"edit the outreach email template",actLabel:"edit email template"}),V=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],x=V.map(([$,L,ve])=>O({icon:dt,nameHTML:`<span class="edit-link" data-act="edit-prompt-${$}" title="edit the ${L.replace(/^\d+ · /,"")} prompt">${L}</span>`,desc:ve,act:`edit-prompt-${$}`,actIcon:G,actTitle:`edit the ${L} prompt`,actLabel:`edit ${L} prompt`})).join(""),U=!i.stats||i.stats.taste_filter_enabled!==!1,k=O({icon:Ln,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage. Toggle it off in the editor to score every company.",dot:U?"ok":"off",note:U?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:G,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),Q=i.anthropicKey;let yt="off",bt="not set — verdict, capture & outreach disabled";Q&&Q.key_source==="db"?(yt="ok",bt="set here · active"):Q&&Q.key_source==="env"&&(yt="ok",bt="from the environment");const Vn=O({icon:In,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:yt,note:bt,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:G,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),Un=O({icon:dt,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:G,actTitle:"edit the application stages",actLabel:"edit application stages"}),Jn=O({icon:dt,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:G,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${Vn}
     </div>`;const wt={"view-profile":()=>Mn(i.profile),"refresh-profile":jn,"edit-taste":()=>Y("taste"),"edit-taste-filter":()=>Y("taste-filter"),"edit-application-stages":()=>Y("application-stages"),"edit-outreach-statuses":()=>Y("outreach-statuses"),"edit-playbook":()=>Y("playbook"),"edit-template":()=>Y("outreach-template"),"view-sources":nt,"refresh-sources":Tn,"edit-anthropic-key":Cn};for(const[$]of V)wt[`edit-prompt-${$}`]=()=>Y(`outreach-prompts/${$}`);e.querySelectorAll("[data-act]").forEach($=>{const L=$.dataset.act;L&&wt[L]&&($.onclick=wt[L])})}async function lt(){try{i.sources=await(await fetch("/api/outreach/sources")).json()}catch{i.sources=null}D()}async function Tn(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(n){u(`refresh failed: ${n.message}`),lt();return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`refresh failed: ${n||"HTTP "+t.status}`),lt();return}const s=await t.json();s.warning?u(s.warning):u("outreach knowledge refreshed"),i.sources={sources:s.sources||[],needs:i.sources&&i.sources.needs||[]},D()}async function is(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}D()}async function Cn(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await is(),ut();const e=document.getElementById("key-input");e&&e.focus()}function ut(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const s=document.getElementById("key-remove");s&&(s.style.display=e.key_source==="db"?"":"none");const n=document.getElementById("key-restart-hint");if(n){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);n.style.display=a?"":"none"}}function pt(){document.getElementById("key-scrim").classList.remove("open")}async function rs(){const e=(document.getElementById("key-input").value||"").trim();if(!e){u("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const s=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let n;try{n=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){u(`save failed: ${a.message}`),s();return}if(!n.ok){u((await n.text().catch(()=>"")).trim()||`HTTP ${n.status}`),s();return}i.anthropicKey=await n.json(),document.getElementById("key-input").value="",s(),u("Anthropic key saved"),await Ze(),ut(),D()}async function Sn(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(s){u(`remove failed: ${s.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){u((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),u(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await Ze(),ut(),D()}async function jn(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),ct();return}if(!t.ok){const s=await t.text().catch(()=>"");u(`refresh failed: ${(s||"").trim()||"HTTP "+t.status}`),ct();return}i.profile=await t.json(),D(),u("company-fit brief refreshed"),w()}function Mn(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${rt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function ht(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=ht,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&ht()};function cs(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Oe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function mt(){document.getElementById("docs-scrim").classList.remove("open")}function An(){return document.getElementById("docs-scrim").classList.contains("open")}function Oe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Hn(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Oe(e)}document.getElementById("open-docs").onclick=cs,document.getElementById("docs-close").onclick=mt,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&mt()};function Pn(){document.getElementById("settings-scrim").classList.add("open"),D()}function ft(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Pn,document.getElementById("settings-close").onclick=ft,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&ft()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Oe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(s=>{const n=s.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);n.length&&Oe(n[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(s=>t.observe(s))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function qn(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function On(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ne(e,t){const s=document.createElement("div");return s.className="chat-msg chat-"+e,s.textContent=t||"",s}function gt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Nn(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function ds(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const s of e||[]){const n=qn(s.content);if(s.role==="user")n&&t.appendChild(Ne("user",n));else if(s.role==="assistant"){const a=On(s.content);if(!n&&!a.length)continue;const o=Ne("assistant",n);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Nn()),gt()}async function Re(e,t,s){if(!i.meta||!i.meta.chat){u("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":s||"";const n=document.getElementById("chat-messages");n.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const d=await c.json();i.chat.threadId=d.thread.id,ds(d.messages||[])}catch(o){n.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function vt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function De(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function ls(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function us(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",ls(),De(!0);const s=document.getElementById("chat-messages"),n=s.querySelector(".chat-empty");n&&n.remove(),s.appendChild(Ne("user",t));const a=Ne("assistant","");a.classList.add("chat-streaming"),s.appendChild(a),gt();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,De(!1)},d=i.chat.threadId;let p;try{p=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!p.ok){c((await p.text().catch(()=>"")).trim()||"HTTP "+p.status);return}const l=new EventSource("/api/chat/"+d+"/stream");i.chat.es=l,l.addEventListener("delta",m=>{o+=m.data,a.textContent=o,gt()}),l.addEventListener("end",async m=>{l.close(),i.chat.es===l&&(i.chat.es=null),a.classList.remove("chat-streaming"),De(!1),i.chat.threadId===d&&await Rn(),Dn(),typeof m.data=="string"&&m.data.indexOf("error")===0&&u("chat: "+m.data)}),l.onerror=()=>{l.close(),i.chat.es===l&&(i.chat.es=null),a.classList.remove("chat-streaming"),De(!1)}}async function Rn(){const e=i.chat.scope,t=i.chat.scopeId,s="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const n=await fetch("/api/chat/threads?"+s);if(!n.ok)return;const a=await n.json();ds(a.messages||[])}catch{}}function Dn(){_(),E(),w(),i.openId&&pe(i.openId)}document.getElementById("open-chat").onclick=()=>Re("global","",""),document.getElementById("chat-close").onclick=vt,document.getElementById("chat-scrim").onclick=vt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),us()}),document.getElementById("chat-input").addEventListener("input",ls),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),us())}),Bt("#t tbody",ms),Bt("#jt tbody",fs),_(),E(),w(),Ze(),Xe(),ct(),lt(),is(),ie()}Kn({"":{view:()=>({mount(b){b.innerHTML=Zn,Xn()}}),chrome:!1}},{title:"scout"});Gn();Qn();
