(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))L(r);new MutationObserver(r=>{for(const g of r)if(g.type==="childList")for(const x of g.addedNodes)x.tagName==="LINK"&&x.rel==="modulepreload"&&L(x)}).observe(document,{childList:!0,subtree:!0});function y(r){const g={};return r.integrity&&(g.integrity=r.integrity),r.referrerPolicy&&(g.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?g.credentials="include":r.crossOrigin==="anonymous"?g.credentials="omit":g.credentials="same-origin",g}function L(r){if(r.ep)return;r.ep=!0;const g=y(r);fetch(r.href,g)}})();function Ea(v,i){const y=i.replace(/^#/,"");let L=null;for(const r of Object.keys(v))(y===r||y.startsWith(r))&&(L===null||r.length>L.length)&&(L=r);return L===null&&""in v&&(L=""),L}function $a(v){return typeof v=="function"?{view:v,chrome:!0}:{view:v.view,chrome:v.chrome!==!1}}function Ia(v,i={}){const y=i.root??document.body,L=i.title??document.title??"",r=i.brandHref??"#",g=document.createElement("main"),x=document.createElement("header");x.className="cap-head";const b=document.createElement("a");b.className="brand",b.href=r,b.textContent=L,b.setAttribute("aria-label",`${L} — home`),x.appendChild(b);const V=document.createElement("nav");V.className="cap-nav",V.setAttribute("aria-label","Views");for(const w of i.nav??[]){const j=document.createElement("a");j.href=w.href,j.textContent=w.label,w.ariaLabel&&j.setAttribute("aria-label",w.ariaLabel),V.appendChild(j)}x.appendChild(V);const M=document.createElement("section");M.className="tk-content",g.appendChild(x),g.appendChild(M);const S=document.createElement("div");S.className="tk-bleed";const Je=w=>{var j;for(const q of Array.from(V.querySelectorAll("a"))){const U=((j=q.getAttribute("href"))==null?void 0:j.replace(/^#/,""))??"";q.toggleAttribute("aria-current",w!==null&&w!==""&&U===w),q.hasAttribute("aria-current")&&q.setAttribute("aria-current","page")}};let pe=0;const X=()=>{const w=Ea(v,location.hash);if(Je(w),w===null){S.isConnected&&S.remove(),g.isConnected||y.appendChild(g),xa(M,"Not found.");return}const{view:j,chrome:q}=$a(v[w]),U=q?M:S;q?(S.isConnected&&S.remove(),g.isConnected||y.appendChild(g)):(g.isConnected&&g.remove(),S.isConnected||y.appendChild(S)),U.replaceChildren();const A=j(),z=++pe,O=A.mount(U);O instanceof Promise&&O.catch(Fe=>{z===pe&&_a(U,String(Fe))})};window.addEventListener("hashchange",X),X()}function xa(v,i){v.replaceChildren();const y=document.createElement("div");y.className="tk-empty",y.textContent=i,v.appendChild(y)}function _a(v,i){v.replaceChildren();const y=document.createElement("div");y.className="tk-error",y.textContent=i,v.appendChild(y)}function Ba(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(i=>{for(const y of i)y.unregister()}),window.caches&&caches.keys().then(i=>{for(const y of i)caches.delete(y)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function La(){let v;try{v=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!v.ok)return null;let i;try{i=await v.json()}catch{return null}return typeof i.email=="string"&&i.email?{email:i.email}:null}const Ta=`
<div class="layout">
<aside class="sidebar">
...
`;function Ca(v){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],openDetail:null,anthropicKey:null},y=e=>"pill pill-"+(e||"none"),L='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),g=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function x(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],N()}async function b(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],_(),V(),S()}function V(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&te())}let M=null;function S(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!M?M=setInterval(Je,4e3):!e&&M&&(clearInterval(M),M=null)}async function Je(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&(_(),V()),S()}async function pe(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{})]),Vt(),i.view==="jobs"&&_()}function X(e,{render:t=!0}={}){i.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Pt(),t&&(e==="jobs"?_():N())}async function w(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}i.stats=e,j()}function j(){D()}function q(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function U(e){return e.slice().sort((t,n)=>i.sort.dir*q(t,n,i.sort.k))}const A=new Set;let z=!1,O=!1;const Fe=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function Pn(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+Fe.map(([t,n,s])=>F("data-v",t,n,s,A.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+F("data-toggle","flagged","⚑ Flagged","",z)+F("data-toggle","enriched","Enriched","",O),St())}function St(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const c of i.rows){const d=c.verdict||"__none__";e[d]=(e[d]|0)+1,c.flagged&&t++,c.enriched&&n++}Ye("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=A.size+(z?1:0)+(O?1:0);Jt("fdrop-cfilters-btn",o,o>0)}function jt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(A.size&&!A.has(t.verdict||"__none__")||z&&!t.flagged||O&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const qn=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],On=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Mt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const At=Mt("scout-hidden-cols"),Ht=Mt("scout-hidden-jcols");function Ke(){return i.view==="jobs"?{cols:On,hidden:Ht,key:"scout-hidden-jcols"}:{cols:qn,hidden:At,key:"scout-hidden-cols"}}function se(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=At.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Ht.has(e.dataset.col)?"none":""})}function Pt(){const e=Ke(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>F("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),qt())}function qt(){const e=Ke(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Ot(e){return`
    ...
    </div>`}function Yn(e){return`<div class="outreach-contacts" id="contacts-editor">
    ...
    </div>`}function en(e){const t=R(e.stage_history);if(!t)return i.applicationStages.length+1;const n=i.applicationStages.indexOf(t);return n<0?i.applicationStages.length:n}function Gn(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return en(e)-en(t);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function _(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=Vn().sort((a,o)=>i.jsort.dir*Gn(a,o,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",Ut();const n=I&&!I.has("rejected")?i.jobs.filter(a=>R(a.stage_history)==="rejected").length:0,s=document.getElementById("jobs-hidden-note");s.style.display=n?"":"none",n&&(s.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{I.add("rejected"),Vt(),_()});for(const a of t){const o=R(a.stage_history),c=Yt(a.stage_history),d=document.createElement("tr");d.dataset.id=a.posting_id;const u=Fn(o).map(([f,k])=>`<option value="${r(f)}"${o===f?" selected":""}>${r(k)}</option>`).join(""),p=a.outreach_status||"",m=Gt(p).map(([f,k])=>`<option value="${r(f)}"${p===f?" selected":""}>${r(k)}</option>`).join("");d.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${a.next_up?" is-on":""}" title="${a.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${a.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${r(a.title||a.company)}</span>${Zn(a.outreach_draft_status)}${a.title?`<div class="small dim">${r(a.company)}</div>`:""}</div></div></td>
    ...
    <section class="pane-section">
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row pipeline-stage-row">
          ...
          <div class="pl-stage-wrap">${ps(e)}</div>
        </div>
  ...
  `,hs();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>de(e.company_id)),ns(e),H(document.getElementById("pursuit-title-input"),p=>sn(e,"title",p)),H(document.getElementById("pursuit-url-input"),p=>es(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>ts(e,d)),H(document.getElementById("pursuit-notes-input"),p=>ms(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>H(p,m=>sn(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>rs(e)),Se(),ye(),t&&(t.scrollTop=s),cn=e.posting_id}function us(e){return`
    ...
    </div>`}function ps(e){const t=ee(e.stage_history),n=t.map((a,o)=>`
    ...
    ${Yn(t.contacts)}`,d=s&&(fs(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?pn(s,!1):""}</div><div class="draft-actions">${d}</div>`+u,ks()}function fs(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const gs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',ys='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',un=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${gs}</button>`,nt=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],vs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function bs(e){let t=nt.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${nt.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",c=a<t?vs:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    ...
    <div class="draft-actions">
      <button class="btn btn-primary draft-sent-btn" title="mark this email sent — bumps the outreach count">${ys}Mark sent</button>
    ...
    </div>`,s?s.replaceWith(c):n.appendChild(c);const d=c.querySelector("#gate-fix-btn");d&&d.addEventListener("click",()=>a?G("outreach-template"):pt());const u=c.querySelector("#gate-retry-btn");u&&u.addEventListener("click",je)}async function $s(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.drafts.findIndex(d=>String(d.id)===String(e));a>=0&&(h.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const d=c.querySelector(".lint-chips"),u=hn(s.lint);d?d.outerHTML=u||"":u&&o.insertAdjacentHTML("afterend",u)}}async function Is(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){l(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`failed: ${s||"HTTP "+t.status}`);return}l("marked sent"),await me(),await b();const n=i.jobs.find(s=>s.posting_id===h.postingId);n&&le(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function ce(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){ye();return}e=await t.json()}catch{ye();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",ye(),h.answers.some(t=>t.status==="generating")?xs():at()}function xs(){h.answersPoll||(h.answersPoll=setInterval(ce,4e3))}function at(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function ye(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(p=>p.status==="generating"),a=t.length?`<div class="answers-list">${t.map(Ls).join("")}</div>`:"",o=!!h.detecting,c=s||o?" disabled":"",d=p=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":p}</button>`;let u;n==="ok"&&t.length?u=(t.some(m=>!mn(m)&&m.status!=="generating")?`<button class="btn" id="answers-start-btn"${c}>${s?"Drafting…":"Draft all blank"}</button>`:"")+d("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+d("Re-detect questions"):u=d("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r(_s(n,t.length))}</div>`+a+`<div class="answers-actions">${u}</div>`,Cs()}function _s(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function mn(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Bs(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function Ls(e){const t=mn(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`,d=!!t,u=d?"Regenerate":"Generate",p=d?"re-draft this answer (discards the current text)":"draft an answer to just this question";return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
  ...
  `;const k=document.getElementById("posting-add-btn");k&&k.addEventListener("click",()=>Ns(e)),it(),document.querySelectorAll("#ve-pick .ve-opt").forEach($=>{$.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Z=>Z.classList.remove("is-on")),$.classList.add("is-on")})});const C=document.getElementById("ve-save-btn");C&&C.addEventListener("click",()=>Os(e)),H(document.getElementById("pane-title-input"),$=>rn(e,"name",$)),document.querySelectorAll("#facts-body [data-k]").forEach($=>H($,Z=>rn(e,$.dataset.k,Z))),H(document.getElementById("pane-domain-input"),$=>ds(e,$)),H(document.getElementById("pane-notes-input"),$=>ls(e,$),{multiline:!0});const ne=document.getElementById("flag-toggle-btn");ne&&ne.addEventListener("click",()=>vn(e.company_id));const ue=document.getElementById("review-stamp-btn");ue&&ue.addEventListener("click",()=>qs(e.company_id));const Ee=document.getElementById("rescore-btn");Ee&&Ee.addEventListener("click",()=>dt("verdict",{company_ids:[e.company_id]}));const ze=document.getElementById("reenrich-btn");ze&&ze.addEventListener("click",()=>dt("enrich",{company_ids:[e.company_id]}));const $e=document.getElementById("company-delete-btn");$e&&$e.addEventListener("click",()=>os(e))}function Ps(e){return`
    <div class="ie-grid">
...
`)):document.getElementById("editor-text").value=c.content||"",a&&(document.getElementById("editor-enabled").checked=c.enabled!==!1),c.taste_version&&(document.getElementById("editor-ver").textContent="version "+c.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Oe(){document.getElementById("editor-scrim").classList.remove("open"),T=null}const Ks=[{key:"experience",hard:!0},{key:"voice",hard:!1},{key:"logistics",hard:!1}];async function pt(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{mt(await(await fetch("/api/outreach/sources")).json())}catch(e){l(`failed to load sources: ${e.message}`)}}function ht(){document.getElementById("sources-scrim").classList.remove("open")}function mt(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):Ks,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],c=o.length?o.map(d=>`<li><span class="src-title">${r(d.title||d.page_id)}</span><button class="src-rm" data-need="${r(a.key)}" data-id="${r(d.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function D(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,s=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let E="off",B="";const Ie=t&&t.criteria_state;Ie==="current"?(E="ok",B="current · verified "+gt(t.verified_age_seconds)):Ie==="changed"?(E="warn",B="changed — re-distill"):Ie==="unverified"?(E="warn",B=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(E="warn",B="brain offline · using cache"):a&&(E="ok",B="fetched "+gt(t.age_seconds)),o=P({icon:Tn,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:E,note:B,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:ke,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=P({icon:Tn,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:Q,actTitle:"edit taste.md",actLabel:"edit taste"});const c=i.sources&&i.sources.sources||[],d=c.filter(E=>E.need==="experience").length,u=c.filter(E=>E.need==="voice").length,p=c.filter(E=>E.need==="logistics").length;let m="off",f="not discovered yet — refresh from the brain";d>0?(m="ok",f=`${d} experience · ${u} voice · ${p} logistics`):c.length>0&&(m="warn",f="no experience yet — refresh");const k=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience, voice + logistics">outreach knowledge</span>':"outreach knowledge",C=P({icon:sa,nameHTML:k,dot:m,note:f,desc:"Your experience, voice + logistics, discovered from the brain to ground outreach and application answers.",act:"refresh-sources",actID:"refresh-sources",actIcon:ke,actTitle:"re-discover experience, voice + logistics from the brain",actLabel:"refresh outreach knowledge"}),ne=P({icon:ta,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:Q,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),ue=P({icon:na,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:Q,actTitle:"edit the outreach email template",actLabel:"edit email template"}),Ee=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],ze=Ee.map(([E,B,Ie])=>P({icon:vt,nameHTML:`<span class="edit-link" data-act="edit-prompt-${E}" title="edit the ${B.replace(/^\d+ · /,"")} prompt">${B}</span>`,desc:Ie,act:`edit-prompt-${E}`,actIcon:Q,actTitle:`edit the ${B} prompt`,actLabel:`edit ${B} prompt`})).join(""),$e=!i.stats||i.stats.taste_filter_enabled!==!1,$=P({icon:aa,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage. Toggle it off in the editor to score every company.",dot:$e?"ok":"off",note:$e?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:Q,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),Z=i.anthropicKey;let Lt="off",Tt="not set — verdict, capture & outreach disabled";Z&&Z.key_source==="db"?(Lt="ok",Tt="set here · active"):Z&&Z.key_source==="env"&&(Lt="ok",Tt="from the environment");const ba=P({icon:ea,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:Lt,note:Tt,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:Q,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),wa=P({icon:vt,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:Q,actTitle:"edit the application stages",actLabel:"edit application stages"}),ka=P({icon:vt,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:Q,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${ba}
     </div>`;const Ct={"view-profile":()=>da(i.profile),"refresh-profile":ca,"edit-taste":()=>G("taste"),"edit-taste-filter":()=>G("taste-filter"),"edit-application-stages":()=>G("application-stages"),"edit-outreach-statuses":()=>G("outreach-statuses"),"edit-playbook":()=>G("playbook"),"edit-template":()=>G("outreach-template"),"view-sources":pt,"refresh-sources":oa,"edit-anthropic-key":ia};for(const[E]of Ee)Ct[`edit-prompt-${E}`]=()=>G(`outreach-prompts/${E}`);e.querySelectorAll("[data-act]").forEach(E=>{const B=E.dataset.act;B&&Ct[B]&&(E.onclick=Ct[B])})}async function bt(){try{i.sources=await(await fetch("/api/outreach/sources")).json()}catch{i.sources=null}D()}async function oa(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){l(`refresh failed: ${s.message}`),bt();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`refresh failed: ${s||"HTTP "+t.status}`),bt();return}const n=await t.json();n.warning?l(n.warning):l("outreach knowledge refreshed"),i.sources={sources:n.sources||[],needs:i.sources&&i.sources.needs||[]},D()}async function Cn(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}D()}async function ia(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await Cn(),wt();const e=document.getElementById("key-input");e&&e.focus()}function wt(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function kt(){document.getElementById("key-scrim").classList.remove("open")}async function Sn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){l("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){l(`save failed: ${a.message}`),n();return}if(!s.ok){l((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),l("Anthropic key saved"),await rt(),wt(),D()}async function ra(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){l(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){l((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),l(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await rt(),wt(),D()}async function ca(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),yt();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),yt();return}i.profile=await t.json(),D(),l("company-fit brief refreshed"),w()}function da(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${gt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Et(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Et,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Et()};function jn(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");De(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function $t(){document.getElementById("docs-scrim").classList.remove("open")}function la(){return document.getElementById("docs-scrim").classList.contains("open")}function De(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function ua(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),De(e)}document.getElementById("open-docs").onclick=jn,document.getElementById("docs-close").onclick=$t,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&$t()};function pa(){document.getElementById("settings-scrim").classList.add("open"),D()}function It(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=pa,document.getElementById("settings-close").onclick=It,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&It()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),De(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&De(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function ha(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function ma(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ve(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function xt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function fa(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Mn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=ha(n.content);if(n.role==="user")s&&t.appendChild(Ve("user",s));else if(n.role==="assistant"){const a=ma(n.content);if(!s&&!a.length)continue;const o=Ve("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(fa()),xt()}async function _t(e,t,n){if(!i.meta||!i.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const d=await c.json();i.chat.threadId=d.thread.id,Mn(d.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Bt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Ue(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function An(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Hn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",An(),Ue(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Ve("user",t));const a=Ve("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),xt();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Ue(!1)},d=i.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");i.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,a.textContent=o,xt()}),p.addEventListener("end",async m=>{p.close(),i.chat.es===p&&(i.chat.es=null),a.classList.remove("chat-streaming"),Ue(!1),i.chat.threadId===d&&await ga(),ya(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),p.onerror=()=>{p.close(),i.chat.es===p&&(i.chat.es=null),a.classList.remove("chat-streaming"),Ue(!1)}}async function ga(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Mn(a.messages||[])}catch{}}function ya(){x(),b(),w(),i.openId&&de(i.openId)}document.getElementById("open-chat").onclick=()=>_t("global","",""),document.getElementById("chat-close").onclick=Bt,document.getElementById("chat-scrim").onclick=Bt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Hn()}),document.getElementById("chat-input").addEventListener("input",An),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Hn())}),Rt("#t tbody",Nn),Rt("#jt tbody",Rn);const va=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();X(va==="jobs"?"jobs":"companies",{render:!1}),x(),b(),w(),rt(),ct(),yt(),bt(),Cn(),pe()}Ia({"":{view:()=>({mount(v){v.innerHTML=Ta,Ca()}}),chrome:!1}},{title:"scout"});Ba();La();
