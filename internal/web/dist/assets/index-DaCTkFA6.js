...
`;function ra(b){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],openDetail:null,anthropicKey:null},y=e=>"pill pill-"+(e||"none"),T='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),g=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function x(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],P()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],$(),J(),j()}function J(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&X())}let H=null;function j(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!H?H=setInterval(Ve,4e3):!e&&H&&(clearInterval(H),H=null)}async function Ve(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&($(),J()),j()}async function re(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{})]),En(),i.view==="jobs"&&$()}function te(e){i.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Ue(),e==="jobs"?$():P()}async function k(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}i.stats=e,M()}function M(){const e=i.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,D()}function N(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function z(e){return e.slice().sort((t,n)=>i.sort.dir*N(t,n,i.sort.k))}const F=new Set;let ne=!1;function be(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",F.has(e.dataset.v))})}function we(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(F.size&&!F.has(t.verdict||"__none__")||ne&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const gn=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],yn=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function $t(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const It=$t("scout-hidden-cols"),xt=$t("scout-hidden-jcols");function _t(){return i.view==="jobs"?{cols:yn,hidden:xt,key:"scout-hidden-jcols"}:{cols:gn,hidden:It,key:"scout-hidden-cols"}}function se(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=It.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=xt.has(e.dataset.col)?"none":""})}function Ue(){const e=_t();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=_t(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),Ue(),se()})})}function Bt(e){return`
    ...
    `}function Lt(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>Qt(t.dataset.id))}const vn=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],bn=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function Tt(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let c=0;c<n;c++){const l=t.map(([u,d])=>{const m=d.endsWith("%")?Math.round(parseFloat(d)*a[c%a.length])+"%":d;return`<td${u?` data-col="${u}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${l}</tr>`}s.innerHTML=o,se()}function P(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=z(we());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=Bt(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||he(s.dataset.id)}),e.appendChild(s),Lt(s)}se()}async function wn(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=z(we()).map(c=>c.company_id),o=[...s.querySelectorAll("tr")].map(c=>c.dataset.id);if(a.length!==o.length||a.some((c,l)=>c!==o[l])){P();return}for(const c of e){const l=i.rows.find(d=>d.company_id===c),u=s.querySelector(`tr[data-id="${CSS.escape(c)}"]`);if(!l||!u){P();return}u.innerHTML=Bt(l),Lt(u)}se()}const S=new Set;let ke=!1,Ee=!1,Q=!0;function kn(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=Q&&!S.has("rejected");return i.jobs.filter(n=>{const s=R(n.stage_history);return!(t&&s==="rejected"||S.size&&!S.has(s)||ke&&!n.next_up||Ee&&(n.outreach_count|0)>0||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.description||"")+" "+(n.contacts||"")).toLowerCase().includes(e))})}function En(){const e=document.getElementById("stage-chips");if(e){for(const t of[...S])i.applicationStages.includes(t)||S.delete(t);e.innerHTML=i.applicationStages.map(t=>`<button class="v-chip${S.has(t)?" is-on":""}" data-stage="${r(t)}">${r(t)}</button>`).join("")}}const $n=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function Ct(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match($n),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}function In(e){const t=(e||[]).map(n=>({position:(n.position||"").trim(),email:(n.email||"").trim()})).filter(n=>n.position||n.email);return t.length?JSON.stringify(t):""}const $e=()=>new Date().toISOString().slice(0,10);function Z(e){if(e=String(e||"").trim(),!e)return[];try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({stage:String((n==null?void 0:n.stage)||"").trim(),date:String((n==null?void 0:n.date)||"").trim()})).filter(n=>n.stage)}catch{}return[]}function Ie(e){const t=(e||[]).map(n=>({stage:(n.stage||"").trim(),date:(n.date||"").trim()})).filter(n=>n.stage);return t.sort((n,s)=>(n.date||"9999-99-99").localeCompare(s.date||"9999-99-99")),t.length?JSON.stringify(t):""}function R(e){const t=Z(e);return t.length?t[t.length-1].stage:""}function St(e){const t=Z(e);return t.length?t[t.length-1].date:""}function xn(e){const t=[["","none"]];for(const n of i.applicationStages)t.push([n,n]);return e&&!i.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function jt(e){const t=[["","none"]];for(const n of i.outreachStatuses)t.push([n,n]);return e&&!i.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const _n=8;function Mt(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%_n}function Je(e){return Mt(e,i.applicationStages)}function Bn(e){return Mt(e,i.outreachStatuses)}function Ln(e){const t=Ct(e);return t.length?t.map(n=>{const s=r(n.position||n.email);if(!n.email)return s;const a=r(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${r(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function At(e){return`<div class="cc-row">
    ...
    </div>`}function Tn(e){return`<div class="outreach-contacts" id="contacts-editor">
    ...
    </div>`}function Ht(e){const t=R(e.stage_history);if(!t)return i.applicationStages.length+1;const n=i.applicationStages.indexOf(t);return n<0?i.applicationStages.length:n}function Cn(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return Ht(e)-Ht(t);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function $(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=kn().sort((d,m)=>i.jsort.dir*Cn(d,m,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=Q&&!S.has("rejected")?i.jobs.filter(d=>R(d.stage_history)==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=i.jobs.filter(d=>d.next_up).length,o=document.getElementById("next-up-n");o.textContent=a,o.style.display=a?"":"none";const c=i.jobs.filter(d=>!(d.outreach_count|0)&&!(Q&&!S.has("rejected")&&R(d.stage_history)==="rejected")).length,l=document.getElementById("not-reached-n");l.textContent=c,l.style.display=c?"":"none";const u=document.getElementById("jobs-hidden-note");u.style.display=n?"":"none",n&&(u.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{Q=!1,document.getElementById("hide-rejected").classList.remove("is-on"),$()});for(const d of t){const m=R(d.stage_history),f=St(d.stage_history),v=document.createElement("tr");v.dataset.id=d.posting_id;const B=xn(m).map(([_,U])=>`<option value="${r(_)}"${m===_?" selected":""}>${r(U)}</option>`).join(""),A=d.outreach_status||"",V=jt(A).map(([_,U])=>`<option value="${r(_)}"${A===_?" selected":""}>${r(U)}</option>`).join("");v.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${d.next_up?" is-on":""}" title="${d.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${d.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${r(d.title||d.company)}</span>${jn(d.outreach_draft_status)}${d.title?`<div class="small dim">${r(d.company)}</div>`:""}</div></div></td>
      ...
      <td class="small" data-col="outreach"><div class="jt-out"><span class="jt-stepper"><button class="jt-dec" title="undo one outreach"${d.outreach_count?"":" disabled"}>−</button><span class="jt-oc${d.outreach_count?"":" dim"}">${d.outreach_count||0}</span><button class="jt-inc" title="log one outreach (today)">+</button></span><select class="jt-ostatus ${Bn(A)}" title="outreach reply status">${V}</select></div></td>
      <td class="small" data-col="last_outreach">${d.last_outreach_at?r(d.last_outreach_at):'<span class="dim">—</span>'}</td>
      <td class="small td-contacts" data-col="contacts">${Ln(d.contacts)}</td>
      <td data-col="link"><a href="${g(d.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `,v.querySelector(".jt-nextup").onclick=()=>Ut(d,!1),v.querySelector(".jt-stage-sel").onchange=_=>Sn(d,_.target.value),v.querySelector(".jt-ostatus").onchange=_=>Be(d,{outreach_status:_.target.value}),v.querySelector(".jt-inc").onclick=()=>Be(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:$e()}),v.querySelector(".jt-dec").onclick=()=>{const _=Math.max(0,(d.outreach_count||0)-1);Be(d,{outreach_count:_,..._===0?{last_outreach_at:""}:{}})},e.appendChild(v)}se(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",m=>{m.target.closest("a, button, select")||Pt(d.dataset.id)})})}function Sn(e,t){if(t=(t||"").trim(),!t||t===R(e.stage_history)){$();return}const n=Z(e.stage_history);n.push({stage:t,date:$e()}),Be(e,{stage_history:Ie(n)})}function jn(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function Pt(e){let t=i.jobs.find(n=>n.posting_id===e);if(t||(await E(),t=i.jobs.find(n=>n.posting_id===e)),!t){p("posting not found — refresh");return}Fe(),Qe(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),Ot("pursuit"),X(),ce(),ue()}let ze=null;function Ot(e){ze=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function xe(){Fe(),Qe(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function Fe(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function ce(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){Le();return}e=await n.json()}catch{Le();return}h.drafts=e.drafts||[],Le();const t=h.drafts[0];t&&t.status==="researching"?Mn():Fe()}function Mn(){h.poll||(h.poll=setInterval(ce,4e3))}function O(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),p(`save failed: ${o.message}`)}})}async function qt(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),$(),ie(e.posting_id,{title:o.title,location:o.location})}async function An(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",g(e.url)),ie(e.posting_id,{url:s.url})}async function Hn(e,t){if(t.disabled)return;const n=t.textContent;t.disabled=!0,t.textContent="re-enriching…";let s;try{s=await fetch(`/api/postings/${e.posting_id}/recapture`,{method:"POST"})}catch(o){t.disabled=!1,t.textContent=n,p(`re-enrich failed: ${o.message}`);return}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();let c=o||"HTTP "+s.status;try{c=JSON.parse(o).error||c}catch{}t.disabled=!1,t.textContent=n,p(`re-enrich failed: ${c}`);return}const a=await s.json();Object.assign(e,{title:a.title,location:a.location,employment_type:a.employment_type,workplace_type:a.workplace_type,department:a.department,comp_range:a.comp_range,description:a.description,posted_at:a.posted_at,url:a.url,questions_status:a.questions_status}),$(),X(),ie(e.posting_id,{title:a.title,location:a.location,url:a.url}),p("re-enriched from the posting link")}function Pn(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>qn(e))}async function On(e,t){const n=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.company_id=s.company_id,e.company=s.company_name,X(),E()}let de=null;function qn(e){de=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const n=document.getElementById("relink-search");n&&(n.value=""),Nt(""),document.getElementById("relink-scrim").classList.add("open"),n&&n.focus()}function ae(){document.getElementById("relink-scrim").classList.remove("open"),de=null}let Ke=null;function Nn(e){Ke=e;const t=(e.postings||[]).length,n=t?` and its ${t} job ${t===1?"posting":"postings"}`:"",s=document.getElementById("delcompany-summary");s&&(s.innerHTML=`Delete <strong>${r(e.name||"this company")}</strong>${n}?`);const a=document.getElementById("delcompany-confirm");a&&(a.disabled=!1),document.getElementById("delcompany-scrim").classList.add("open")}function _e(){document.getElementById("delcompany-scrim").classList.remove("open"),Ke=null}async function Rn(){const e=Ke;if(!e)return;const t=document.getElementById("delcompany-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e.company_id}`,{method:"DELETE"})}catch(a){p(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");p(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=e.name||"company";_e(),i.openId===e.company_id&&me(),x(),E(),k(),p(`deleted ${s}`)}function Nt(e){const t=document.getElementById("relink-results");if(!t)return;const n=e.trim().toLowerCase();let s=(i.rows||[]).slice();if(n?(s=s.filter(o=>(o.name||"").toLowerCase().includes(n)),s.sort((o,c)=>{const l=(o.name||"").toLowerCase().startsWith(n)?0:1,u=(c.name||"").toLowerCase().startsWith(n)?0:1;return l-u||(o.name||"").localeCompare(c.name||"")})):s.sort((o,c)=>(o.name||"").localeCompare(c.name||"")),s=s.slice(0,60),!s.length){t.innerHTML=`<div class="relink-empty">${(i.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const a=de?de.company_id:"";t.innerHTML=s.map(o=>{const c=o.company_id===a,l=[o.vertical,o.location].filter(Boolean).map(r).join(" · ");return`<button type="button" class="relink-result${c?" is-current":""}"
      ...
      </button>`}).join("")}async function Rt(e){const t=de;if(!t){ae();return}if(e===t.company_id){ae();return}try{await On(t,e),ae(),p(`moved to ${t.company}`)}catch(n){p(`move failed: ${n.message}`)}}async function Dt(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),x(),E()}async function Dn(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();i.openId=s.company_id,fe(s),ge(s.company_id),x(),E()}async function Vn(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}let Vt=null;function X(){const e=h.row;if(!e)return;const t=document.getElementById("pursuit-body"),s=!!t&&Vt===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${r(e.title||"")}">`;const a=R(e.stage_history);document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a?Je(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`+(e.verdict?` <span class="${y(e.verdict)}">${r(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>bt("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    ...
    <section class="pane-section">
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row pipeline-stage-row">
          ...
          <div class="pl-stage-wrap">${Jn(e)}</div>
        </div>
  ...
  `,zn();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>he(e.company_id)),Pn(e),O(document.getElementById("pursuit-title-input"),u=>qt(e,"title",u)),O(document.getElementById("pursuit-url-input"),u=>An(e,u));const l=document.getElementById("pursuit-reenrich");l&&l.addEventListener("click",()=>Hn(e,l)),O(document.getElementById("pursuit-notes-input"),u=>Fn(u),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(u=>O(u,d=>qt(e,u.dataset.k,d),{multiline:u.tagName==="TEXTAREA"})),Le(),pe(),t&&(t.scrollTop=s),Vt=e.posting_id}function Un(e){return`
    ...
    </div>`}function Jn(e){const t=Z(e.stage_history),n=t.map((a,o)=>`
    ...
    </div>`}function zn(){const e=h.row;document.querySelectorAll("#pursuit-body .stage-entry .stage-date").forEach(a=>{a.addEventListener("change",o=>{const c=+o.target.closest(".stage-entry").dataset.i,l=Z(e.stage_history);l[c]&&(l[c].date=o.target.value,oe({stage_history:Ie(l)}))})}),document.querySelectorAll("#pursuit-body .stage-entry .stage-del").forEach(a=>{a.addEventListener("click",o=>{const c=+o.target.closest(".stage-entry").dataset.i,l=Z(e.stage_history);l.splice(c,1),oe({stage_history:Ie(l)})})});const t=document.querySelector("#pursuit-body .stage-add-btn");t&&t.addEventListener("click",()=>{const a=document.querySelector("#pursuit-body .stage-add-sel"),o=document.querySelector("#pursuit-body .stage-add-date"),c=(a.value||"").trim();if(!c)return;const l=Z(e.stage_history);l.push({stage:c,date:o.value||$e()}),oe({stage_history:Ie(l)})});const n=document.querySelector("#pursuit-body .pl-ostatus");n&&n.addEventListener("change",a=>oe({outreach_status:a.target.value}));const s=document.querySelector("#pursuit-body .pt-nextup");s&&s.addEventListener("click",()=>Ut(h.row,!0))}async function Ut(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){p(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();p(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,$(),ie(e.posting_id,{next_up:s.next_up}),t&&X(),p(e.next_up?"queued next up":"removed from the queue")}async function We(e,t){const n={stage_history:e.stage_history||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",outreach_status:e.outreach_status||"",contacts:e.contacts||"",notes:e.notes||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return p(`save failed: ${o.message}`),null}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();return p(`save failed: ${o||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{stage_history:a.stage_history,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,outreach_status:a.outreach_status,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),ie(e.posting_id,{stage_history:a.stage_history,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function Fn(e){const t=h.row,n={stage_history:t.stage_history||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",outreach_status:t.outreach_status||"",contacts:t.contacts||"",notes:e},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,$()}async function oe(e){await We(h.row,e)&&($(),X(),p("tracking saved"))}async function Jt(e){await We(h.row,{contacts:e})&&$()}async function Be(e,t){await We(e,t)&&($(),h.postingId===e.posting_id&&(h.row=e,X()),p("tracking saved"))}function Le(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.row,n=h.drafts,s=n[0]||null,a=n.slice(1),o=`
    ...
    ${Tn(t.contacts)}`,l=s&&(Kn(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?Ft(s,!1):""}</div><div class="draft-actions">${l}</div>`+u,Xn()}function Kn(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const Wn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',Yn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',zt=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${Wn}</button>`,Ye=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],Gn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function Qn(e){let t=Ye.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${Ye.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",c=a<t?Gn:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${Ye[t].active}…</span></div>
  </div>`}function Ft(e,t){const n=(d,m,f="")=>`
    ...
    </div>`;if(e.status==="failed"){const d=Zn(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    <div class="draft-head">${o}${s?zt:""}</div>
    ${l}
    ...
    ${le(e)}
  </div>`}function Ge(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function le(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,d)=>d?`<div class="tr-line"><span class="tr-key">${u}:</span> ${r(String(d))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],l=c.map(u=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function Kt(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${r(n.message||"")}"><code>${r(n.code||"")}</code>${r(n.message||"")}</span>`).join("")+"</div>":""}function Zn(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${r(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${r(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function Xn(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#contacts-editor");if(t){const u=t.querySelector(".cc-list"),d=()=>In(Array.from(u.querySelectorAll(".cc-row")).map(f=>({position:f.querySelector(".cc-pos").value,email:f.querySelector(".cc-email").value}))),m=f=>{f.querySelectorAll(".cc-pos, .cc-email").forEach(v=>{v.addEventListener("change",()=>Jt(d())),v.addEventListener("keydown",B=>{B.key==="Enter"&&(B.preventDefault(),B.target.blur())})}),f.querySelector(".cc-del").addEventListener("click",()=>{f.remove(),Jt(d())})};u.querySelectorAll(".cc-row").forEach(m),t.querySelector(".cc-add").addEventListener("click",()=>{const f=document.createElement("div");f.innerHTML=At({position:"",email:""});const v=f.firstElementChild;u.appendChild(v),m(v),v.querySelector(".cc-pos").focus()})}const n=()=>new Date().toISOString().slice(0,10),s=h.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>oe({outreach_count:(s.outreach_count||0)+1,last_outreach_at:n()}));const o=e.querySelector(".pt-outreach-dec");o&&o.addEventListener("click",()=>{const u=Math.max(0,(s.outreach_count||0)-1);oe({outreach_count:u,...u===0?{last_outreach_at:""}:{}})});const c=e.querySelector("#draft-start-btn");c&&c.addEventListener("click",()=>Te()),e.querySelectorAll(".draft-retry-btn").forEach(u=>u.addEventListener("click",()=>Te())),e.querySelectorAll(".draft-regen-btn").forEach(u=>u.addEventListener("click",()=>Te(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(u=>{const d=u.dataset.did,m=u.querySelector(".draft-textarea");m&&O(m,B=>ts(d,B),{multiline:!0});const f=u.querySelector(".draft-sent-btn");f&&f.addEventListener("click",()=>ns(d));const v=u.querySelector(".draft-copy-btn");v&&v.addEventListener("click",()=>{const B=u.querySelector(".draft-textarea"),A=u.querySelector(".draft-sentbody"),V=B?B.value:A?A.textContent:"";bs(V,"email copied")})});const l=e.querySelector("details.draft-history");l&&l.addEventListener("toggle",()=>{h.openHist=l.open})}async function Te(e=!1){const t=document.getElementById("outreach-section"),n=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));n&&(n.disabled=!0);let s;try{const o=e?"?regenerate=1":"";s=await fetch(`/api/postings/${h.postingId}/outreach${o}`,{method:"POST"})}catch(o){p(`draft failed: ${o.message}`),n&&(n.disabled=!1);return}if(s.status===202){let o={};try{o=await s.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&p(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await ce(),E();return}if(s.status===409){await ce(),p("a draft is already active");return}if(s.status===412){let o={};try{o=await s.json()}catch{}es(o.need,o.error),n&&(n.disabled=!1);return}if(s.status===503){const o=document.getElementById("outreach-section");if(o){const c=document.createElement("div");c.className="draft-note",c.textContent="Outreach engine not running in this build.",o.appendChild(c)}n&&(n.disabled=!1);return}const a=(await s.text().catch(()=>"")).trim();p(`draft failed: ${a||"HTTP "+s.status}`),n&&(n.disabled=!1)}function es(e,t){const n=document.getElementById("outreach-section");if(!n)return;const s=n.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"Discover sources",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,s?s.replaceWith(c):n.appendChild(c);const l=c.querySelector("#gate-fix-btn");l&&l.addEventListener("click",()=>a?Y("outreach-template"):ot());const u=c.querySelector("#gate-retry-btn");u&&u.addEventListener("click",Te)}async function ts(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.drafts.findIndex(l=>String(l.id)===String(e));a>=0&&(h.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const l=c.querySelector(".lint-chips"),u=Kt(s.lint);l?l.outerHTML=u||"":u&&o.insertAdjacentHTML("afterend",u)}}async function ns(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){p(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();p(`failed: ${s||"HTTP "+t.status}`);return}p("marked sent"),await ce(),await E();const n=i.jobs.find(s=>s.posting_id===h.postingId);n&&ie(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function ue(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){pe();return}e=await t.json()}catch{pe();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",pe(),h.answers.some(t=>t.status==="generating")?ss():Qe()}function ss(){h.answersPoll||(h.answersPoll=setInterval(ue,4e3))}function Qe(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function pe(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(d=>d.status==="generating"),a=t.length?`<div class="answers-list">${t.map(is).join("")}</div>`:"",o=!!h.detecting,c=s||o?" disabled":"",l=d=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":d}</button>`;let u;n==="ok"&&t.length?u=`<button class="btn ${t.some(m=>!Wt(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect questions"):u=l("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r(as(n,t.length))}</div>`+a+`<div class="answers-actions">${u}</div>`,cs()}function as(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function Wt(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function os(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function is(e){const t=Wt(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
  ...
  </div>`}function rs(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function cs(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",Yt);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",ls),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(O(o,l=>us(a,l),{multiline:!0}),o.addEventListener("input",()=>ds(s,o)));const c=s.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>ps(a))})}function ds(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function Yt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){p(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await ue();return}if(n.status===412){let a={};try{a=await n.json()}catch{}hs(a.error),t&&(t.disabled=!1);return}if(n.status===503){Gt("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();p(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function ls(){h.detecting=!0,pe();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();p(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){p(`detect failed: ${e.message}`)}h.detecting=!1,await ue()}async function us(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=s)}async function ps(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){p(`regenerate failed: ${n.message}`);return}if(t.status===503){Gt("Answer generation isn't running in this build.");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();p(`regenerate failed: ${n||"HTTP "+t.status}`);return}await ue()}function hs(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",ot);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",Yt)}function Gt(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function he(e){var l,u;const t=document.getElementById("pane"),n=document.getElementById("scrim"),s=i.openId===e&&t.classList.contains("open"),a=s?((l=document.getElementById("pane-body"))==null?void 0:l.scrollTop)??0:0,o=s?(u=document.getElementById("trace-body"))==null?void 0:u.innerHTML:null;i.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),Ot("company"),s||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let c;try{const d=await fetch(`/api/companies/${e}`);if(!d.ok)throw new Error(`HTTP ${d.status}`);c=await d.json()}catch(d){s||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${r(d.message)}</div>`);return}if(i.openId===e){if(fe(c),s){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const d=document.getElementById("pane-body");d&&(d.scrollTop=a)}ge(e)}}function me(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function fe(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${r(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `;const v=document.getElementById("posting-add-btn");v&&v.addEventListener("click",()=>ys(e)),Xe(),document.querySelectorAll("#ve-pick .ve-opt").forEach(w=>{w.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(ee=>ee.classList.remove("is-on")),w.classList.add("is-on")})});const B=document.getElementById("ve-save-btn");B&&B.addEventListener("click",()=>gs(e)),O(document.getElementById("pane-title-input"),w=>Dt(e,"name",w)),document.querySelectorAll("#facts-body [data-k]").forEach(w=>O(w,ee=>Dt(e,w.dataset.k,ee))),O(document.getElementById("pane-domain-input"),w=>Dn(e,w)),O(document.getElementById("pane-notes-input"),w=>Vn(e,w),{multiline:!0});const A=document.getElementById("flag-toggle-btn");A&&A.addEventListener("click",()=>Qt(e.company_id));const V=document.getElementById("review-stamp-btn");V&&V.addEventListener("click",()=>fs(e.company_id));const _=document.getElementById("rescore-btn");_&&_.addEventListener("click",()=>nt("verdict",{company_ids:[e.company_id]}));const U=document.getElementById("reenrich-btn");U&&U.addEventListener("click",()=>nt("enrich",{company_ids:[e.company_id]}));const De=document.getElementById("company-delete-btn");De&&De.addEventListener("click",()=>Nn(e))}function ms(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function fs(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){p(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");p(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=i.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,P()),i.openId===e&&(fe(s),ge(e)),p("reviewed")}async function Qt(e){const t=i.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){p(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");p(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,P()),i.openId===e&&(fe(a),ge(e)),E(),p(a.flagged?"flagged":"unflagged")}async function gs(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){p("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(l){p(`save failed: ${l.message}`),a.disabled=!1;return}if(!o.ok){const l=await o.text().catch(()=>"");p(`save failed: HTTP ${o.status}${l?" — "+l:""}`),a.disabled=!1;return}const c=await o.json();fe(c),ge(c.company_id),x(),k(),E(),p("verdict saved")}function Ze(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(r).join(" · "),a=R(n.stage_history),o=St(n.stage_history),c=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?Je(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`,`<span class="pt-meta">${a?o?r(o):"tracked":"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${r(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ie(e,t){const n=i.openDetail;if(!n||!i.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Ze(n),Xe())}function Xe(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||Pt(e.dataset.pid)})})}async function ys(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){p("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){p(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");p(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const l=document.getElementById("postings-list");l&&(l.innerHTML=Ze(e),Xe()),t.value="",n.value="",s.disabled=!1,E(),p("link added")}async function ge(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){Ce(`<div class="muted">Failed to load trail: ${r(s.message)}</div>`);return}if(!t.ok){Ce(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Ce('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Ce(n.map(vs).join(""))}function vs(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(r);return e.run_id&&t.push("run "+r(e.run_id.slice(0,8))),`
    ...
    </div>`}function Ce(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let Zt;function p(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(Zt),Zt=setTimeout(()=>t.classList.remove("show"),2200)}async function bs(e,t="copied"){if(!e){p("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}p(t)}catch(n){p(`copy failed: ${n.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function et(){try{const s=await fetch("/api/meta");if(!s.ok)return;i.meta=await s.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=i.meta.chat?"":"none")}async function tt(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Se=null;async function nt(e,t){if(i.meta&&i.meta.control===!1){p("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){p(`run failed: ${a.message}`);return}if(n.status===409){p("a job is already running");return}if(n.status===412){const a=await n.text();p(a.trim());return}if(!n.ok){p(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();on(e,s,t)}async function ws(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){p(`upload failed: ${a.message}`);return}if(n.status===409){p("a job is already running");return}if(!n.ok){p(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();on("ingest",s)}const ks=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let je=[],K=new Set,W="company";function Xt(e){W=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",en()}function st(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function en(){const e=document.getElementById("add-note");st()?e.innerHTML=W==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and description — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=W==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function Es(){ks.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",K=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),Xt(i.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(s=>`<option value="${r(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),je=s.verticals||[]}catch{je=[]}tn()}function Me(){document.getElementById("add-scrim").classList.remove("open")}function tn(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=je.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${K.has(s)?" sel":""}" data-v="${r(s)}">${r(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;K.has(a)?K.delete(a):K.add(a),s.classList.toggle("sel"),nn()}))):e.innerHTML=`<div class="none">${je.length?"no match":"no verticals in the set yet"}</div>`,nn()}function nn(){const e=K.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function sn(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function an(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){p(W==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,st()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=f=>document.getElementById(f).value.trim(),c=st();let l,u;c?(l="/api/capture",u={url:sn(t),kind:W==="company"?"company_page":"job_posting",fields:W==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...K].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):W==="company"?(l="/api/companies",u={website:t,name:o("add-name"),vertical:[...K].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(l="/api/postings",u={url:sn(t),title:o("add-title"),company:o("add-job-company")});let d;try{d=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(f){p(`add failed: ${f.message}`),a();return}if(!d.ok){let f=`HTTP ${d.status}`;try{const v=await d.text();try{f=JSON.parse(v).error||f}catch{f=v.trim()||f}}catch{}if(a(),d.status===409){p(f||"That company is already in the list."),e.focus(),e.select();return}p(`add failed: ${f}`);return}const m=await d.json();if(a(),c&&!m.company_id){p(m.note||"couldn't classify that page");return}if(Me(),x(),k(),E(),W==="job"){const f=m.posting&&m.posting.title||"job link";p(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),te("jobs")}else c?(p(m.note||(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`)),he(m.company_id)):p("company added")}function on(e,t,n){Se=t;const s=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="",s.classList.add("open"),tt();const o=new EventSource(`/api/jobs/${t}/stream`),c=(l,u)=>{const d=document.createElement("div"),m=!u&&/^\s*warn:/i.test(l);d.className="ln"+(u?" ln-err":m?" ln-warn":""),d.textContent=m?l.replace(/^\s*warn:\s*/i,"⚠ "):l,a.appendChild(d),a.scrollTop=a.scrollHeight};o.addEventListener("line",l=>c(l.data,/error|failed/i.test(l.data))),o.addEventListener("end",l=>{o.close(),Se=null,c(`— ${l.data} —`,l.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",p(`${e} ${l.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?wn(n.company_ids):x(),k(),tt(),E(),i.openId&&he(i.openId)}),o.onerror=()=>{o.close()}}async function $s(){if(Se)try{await fetch(`/api/jobs/${Se}/cancel`,{method:"POST"})}catch{}}let C=null;const Is={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function at(e){return e==="application-stages"||e==="outreach-statuses"}function Ae(e){if(e==="outreach-template")return"outreach template";if(e==="taste-filter")return"pre-filter rules";if(e==="playbook")return"playbook";if(e==="application-stages")return"application stages";if(e==="outreach-statuses")return"outreach statuses";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(Is[t]||t)+" prompt"}return e+".md"}async function Y(e){C=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Ae(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=!!e&&e.startsWith("outreach-prompts/"),s=n?e.slice(17):"",a=e==="taste-filter"||n&&s!=="fill";document.getElementById("editor-toggle-row").style.display=a?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",a&&(document.getElementById("editor-toggle-label").textContent=e==="taste-filter"?"Enable the pre-filter (off → bulk verdict runs score every company; the rules below are kept either way)":"Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const l=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${l||"HTTP "+o.status}`;return}const c=await o.json();at(e)?(document.getElementById("editor-title").textContent="edit "+Ae(e)+" — one per line",document.getElementById("editor-text").value=(c.statuses||[]).join(`
`)):document.getElementById("editor-text").value=c.content||"",a&&(document.getElementById("editor-enabled").checked=c.enabled!==!1),c.taste_version&&(document.getElementById("editor-ver").textContent="version "+c.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function He(){document.getElementById("editor-scrim").classList.remove("open"),C=null}const xs=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function ot(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{rt(await(await fetch("/api/outreach/sources")).json())}catch(e){p(`failed to load sources: ${e.message}`)}}function it(){document.getElementById("sources-scrim").classList.remove("open")}function rt(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):xs,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],c=o.length?o.map(l=>`<li><span class="src-title">${r(l.title||l.page_id)}</span><button class="src-rm" data-need="${r(a.key)}" data-id="${r(l.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${r(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>Bs(a.dataset.need,a.dataset.id)))}async function _s(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){p(`refresh failed: ${s.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){p(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const n=await t.json();n.warning?p(n.warning):p("sources refreshed"),rt(n),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function Bs(e,t){let n;try{n=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(s){p(`remove failed: ${s.message}`);return}if(!n.ok){p(`remove failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}rt(await n.json())}async function Ls(){if(!C)return;const e=document.getElementById("editor-text").value;let t;if(at(C))t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)};else{t={content:e};const o=C.startsWith("outreach-prompts/")&&C!=="outreach-prompts/fill";(C==="taste-filter"||o)&&(t.enabled=document.getElementById("editor-enabled").checked)}let n;try{n=await fetch(`/api/${C}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){p(`save failed: ${o.message}`);return}if(!n.ok){p(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const s=await n.json();s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version);const a=at(C);p(`${Ae(C)} saved`),He(),a&&re(),k()}async function Ts(){if(!C||!C.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${C}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){p(`reset failed: ${n.message}`);return}if(!e.ok){p(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",p(`${Ae(C)} reset to default`)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;i.sort.k===t?i.sort.dir*=-1:(i.sort.k=t,i.sort.dir=1),P()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;i.jsort.k===t?i.jsort.dir*=-1:(i.jsort.k=t,i.jsort.dir=1),$()}}),document.getElementById("tab-companies").onclick=()=>te("companies"),document.getElementById("tab-jobs").onclick=()=>te("jobs"),document.getElementById("q").oninput=P,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;F.has(t)?F.delete(t):F.add(t),be(),P()})}),document.getElementById("flag-filter").addEventListener("click",e=>{ne=!ne,e.currentTarget.classList.toggle("is-on",ne),P()}),document.getElementById("jq").oninput=$,document.getElementById("hide-rejected").addEventListener("click",e=>{Q=!Q,e.currentTarget.classList.toggle("is-on",Q),$()}),document.getElementById("stage-chips").addEventListener("click",e=>{const t=e.target.closest(".v-chip[data-stage]");if(!t)return;const n=t.dataset.stage;S.has(n)?S.delete(n):S.add(n),t.classList.toggle("is-on",S.has(n)),$()}),document.getElementById("next-up-filter").addEventListener("click",e=>{ke=!ke,e.currentTarget.classList.toggle("is-on",ke),$()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{Ee=!Ee,e.currentTarget.classList.toggle("is-on",Ee),$()}),Ue(),se(),document.getElementById("pane-close").onclick=me,document.getElementById("scrim").onclick=me,document.getElementById("pursuit-close").onclick=xe,document.getElementById("pursuit-scrim").onclick=xe,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){wt();return}if(Vs()){gt();return}if(document.getElementById("profile-scrim").classList.contains("open")){ft();return}if(document.getElementById("add-scrim").classList.contains("open")){Me();return}if(document.getElementById("run-scrim").classList.contains("open")){Pe();return}if(document.getElementById("help-scrim").classList.contains("open")){Oe();return}if(document.getElementById("relink-scrim").classList.contains("open")){ae();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){_e();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(ze==="pursuit"&&n){xe();return}if(ze==="company"&&t){me();return}if(t){me();return}xe();return}if(document.getElementById("key-scrim").classList.contains("open")){mt();return}if(document.getElementById("sources-scrim").classList.contains("open")){it();return}if(document.getElementById("editor-scrim").classList.contains("open")){He();return}if(document.getElementById("settings-scrim").classList.contains("open")){yt();return}});let ct=null;const Cs={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function rn(e){if(i.meta&&i.meta.control===!1){p("control surface disabled");return}ct=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Cs[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Pe(){document.getElementById("run-scrim").classList.remove("open"),ct=null}document.getElementById("btn-enrich").onclick=()=>rn("enrich"),document.getElementById("btn-verdict").onclick=()=>rn("verdict"),document.getElementById("run-cancel").onclick=Pe,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Pe()},document.getElementById("run-go").onclick=()=>{const e=ct,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(Pe(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),nt(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=Es;const Ss={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function cn(e){const t=Ss[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const l=document.createElement("a");l.className="help-link",l.textContent="Learn more →",l.onclick=()=>{Oe(),pn(),Us(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(l),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Oe(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>cn("add"),document.getElementById("help-run").onclick=()=>cn("run"),document.getElementById("help-close").onclick=Oe,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Oe()},document.getElementById("add-cancel").onclick=Me,document.getElementById("add-save").onclick=an,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Me()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Xt(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",en),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),an()))}),document.getElementById("add-vertical-filter").addEventListener("input",tn),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&ws(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=$s,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=He,document.getElementById("editor-save").onclick=Ls,document.getElementById("editor-reset").onclick=Ts,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&He()},document.getElementById("sources-close").onclick=it,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&it()},document.getElementById("sources-refresh-btn").onclick=_s,document.getElementById("key-cancel").onclick=mt,document.getElementById("key-save").onclick=un,document.getElementById("key-remove").onclick=Ns,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&mt()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),un())}),document.getElementById("delcompany-cancel").onclick=_e,document.getElementById("delcompany-confirm").onclick=Rn,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&_e()},document.getElementById("relink-cancel").onclick=ae,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&ae()},document.getElementById("relink-search").addEventListener("input",e=>{Nt(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&Rt(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&Rt(t.dataset.id)});function dt(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function lt(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}D()}const G='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',ye='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',js='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',dn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',Ms='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',As='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',ut='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',Hs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>',Ps='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>';function q(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${r(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function D(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,s=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let I="off",L="";const ve=t&&t.criteria_state;ve==="current"?(I="ok",L="current · verified "+dt(t.verified_age_seconds)):ve==="changed"?(I="warn",L="changed — re-distill"):ve==="unverified"?(I="warn",L=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(I="warn",L="brain offline · using cache"):a&&(I="ok",L="fetched "+dt(t.age_seconds)),o=q({icon:dn,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:I,note:L,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:ye,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=q({icon:dn,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:G,actTitle:"edit taste.md",actLabel:"edit taste"});const c=i.sources&&i.sources.sources||[],l=c.filter(I=>I.need==="experience").length,u=c.filter(I=>I.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";l>0?(d="ok",m=`${l} experience · ${u} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const f=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",v=q({icon:Hs,nameHTML:f,dot:d,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:ye,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),B=q({icon:Ms,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:G,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),A=q({icon:As,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:G,actTitle:"edit the outreach email template",actLabel:"edit email template"}),V=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],_=V.map(([I,L,ve])=>q({icon:ut,nameHTML:`<span class="edit-link" data-act="edit-prompt-${I}" title="edit the ${L.replace(/^\d+ · /,"")} prompt">${L}</span>`,desc:ve,act:`edit-prompt-${I}`,actIcon:G,actTitle:`edit the ${L} prompt`,actLabel:`edit ${L} prompt`})).join(""),U=!i.stats||i.stats.taste_filter_enabled!==!1,De=q({icon:Ps,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage. Toggle it off in the editor to score every company.",dot:U?"ok":"off",note:U?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:G,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),w=i.anthropicKey;let ee="off",kt="not set — verdict, capture & outreach disabled";w&&w.key_source==="db"?(ee="ok",kt="set here · active"):w&&w.key_source==="env"&&(ee="ok",kt="from the environment");const Gs=q({icon:js,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:ee,note:kt,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:G,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),Qs=q({icon:ut,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:G,actTitle:"edit the application stages",actLabel:"edit application stages"}),Zs=q({icon:ut,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:G,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${Gs}
     </div>`;const Et={"view-profile":()=>Ds(i.profile),"refresh-profile":Rs,"edit-taste":()=>Y("taste"),"edit-taste-filter":()=>Y("taste-filter"),"edit-application-stages":()=>Y("application-stages"),"edit-outreach-statuses":()=>Y("outreach-statuses"),"edit-playbook":()=>Y("playbook"),"edit-template":()=>Y("outreach-template"),"view-sources":ot,"refresh-sources":Os,"edit-anthropic-key":qs};for(const[I]of V)Et[`edit-prompt-${I}`]=()=>Y(`outreach-prompts/${I}`);e.querySelectorAll("[data-act]").forEach(I=>{const L=I.dataset.act;L&&Et[L]&&(I.onclick=Et[L])})}async function pt(){try{i.sources=await(await fetch("/api/outreach/sources")).json()}catch{i.sources=null}D()}async function Os(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){p(`refresh failed: ${s.message}`),pt();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();p(`refresh failed: ${s||"HTTP "+t.status}`),pt();return}const n=await t.json();n.warning?p(n.warning):p("outreach knowledge refreshed"),i.sources={sources:n.sources||[],needs:i.sources&&i.sources.needs||[]},D()}async function ln(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}D()}async function qs(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await ln(),ht();const e=document.getElementById("key-input");e&&e.focus()}function ht(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function mt(){document.getElementById("key-scrim").classList.remove("open")}async function un(){const e=(document.getElementById("key-input").value||"").trim();if(!e){p("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){p(`save failed: ${a.message}`),n();return}if(!s.ok){p((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),p("Anthropic key saved"),await et(),ht(),D()}async function Ns(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){p(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){p((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),p(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await et(),ht(),D()}async function Rs(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){p(`refresh failed: ${n.message}`),lt();return}if(!t.ok){const n=await t.text().catch(()=>"");p(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),lt();return}i.profile=await t.json(),D(),p("company-fit brief refreshed"),k()}function Ds(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${dt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function ft(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=ft,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&ft()};function pn(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");qe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function gt(){document.getElementById("docs-scrim").classList.remove("open")}function Vs(){return document.getElementById("docs-scrim").classList.contains("open")}function qe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Us(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),qe(e)}document.getElementById("open-docs").onclick=pn,document.getElementById("docs-close").onclick=gt,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&gt()};function Js(){document.getElementById("settings-scrim").classList.add("open"),D()}function yt(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Js,document.getElementById("settings-close").onclick=yt,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&yt()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),qe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&qe(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function zs(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Fs(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ne(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function vt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Ks(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function hn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=zs(n.content);if(n.role==="user")s&&t.appendChild(Ne("user",s));else if(n.role==="assistant"){const a=Fs(n.content);if(!s&&!a.length)continue;const o=Ne("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Ks()),vt()}async function bt(e,t,n){if(!i.meta||!i.meta.chat){p("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const l=await c.json();i.chat.threadId=l.thread.id,hn(l.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function wt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Re(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function mn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function fn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",mn(),Re(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Ne("user",t));const a=Ne("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),vt();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Re(!1)},l=i.chat.threadId;let u;try{u=await fetch("/api/chat/"+l+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const d=new EventSource("/api/chat/"+l+"/stream");i.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,vt()}),d.addEventListener("end",async m=>{d.close(),i.chat.es===d&&(i.chat.es=null),a.classList.remove("chat-streaming"),Re(!1),i.chat.threadId===l&&await Ws(),Ys(),typeof m.data=="string"&&m.data.indexOf("error")===0&&p("chat: "+m.data)}),d.onerror=()=>{d.close(),i.chat.es===d&&(i.chat.es=null),a.classList.remove("chat-streaming"),Re(!1)}}async function Ws(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();hn(a.messages||[])}catch{}}function Ys(){x(),E(),k(),i.openId&&he(i.openId)}document.getElementById("open-chat").onclick=()=>bt("global","",""),document.getElementById("chat-close").onclick=wt,document.getElementById("chat-scrim").onclick=wt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),fn()}),document.getElementById("chat-input").addEventListener("input",mn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),fn())}),Tt("#t tbody",vn),Tt("#jt tbody",bn),x(),E(),k(),et(),tt(),lt(),pt(),ln(),re()}ta({"":{view:()=>({mount(b){b.innerHTML=ia,ra()}}),chrome:!1}},{title:"scout"});aa();oa();
