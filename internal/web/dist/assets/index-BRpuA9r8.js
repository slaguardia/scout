(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))$(i);new MutationObserver(i=>{for(const f of i)if(f.type==="childList")for(const E of f.addedNodes)E.tagName==="LINK"&&E.rel==="modulepreload"&&$(E)}).observe(document,{childList:!0,subtree:!0});function v(i){const f={};return i.integrity&&(f.integrity=i.integrity),i.referrerPolicy&&(f.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?f.credentials="include":i.crossOrigin==="anonymous"?f.credentials="omit":f.credentials="same-origin",f}function $(i){if(i.ep)return;i.ep=!0;const f=v(i);fetch(i.href,f)}})();function Mn(y,r){const v=r.replace(/^#/,"");let $=null;for(const i of Object.keys(y))(v===i||v.startsWith(i))&&($===null||i.length>$.length)&&($=i);return $===null&&""in y&&($=""),$}function An(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function Hn(y,r={}){const v=r.root??document.body,$=r.title??document.title??"",i=r.brandHref??"#",f=document.createElement("main"),E=document.createElement("header");E.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=$,w.setAttribute("aria-label",`${$} — home`),E.appendChild(w);const j=document.createElement("nav");j.className="cap-nav",j.setAttribute("aria-label","Views");for(const k of r.nav??[]){const T=document.createElement("a");T.href=k.href,T.textContent=k.label,k.ariaLabel&&T.setAttribute("aria-label",k.ariaLabel),j.appendChild(T)}E.appendChild(j);const _=document.createElement("section");_.className="tk-content",f.appendChild(E),f.appendChild(_);const L=document.createElement("div");L.className="tk-bleed";const Le=k=>{var T;for(const M of Array.from(j.querySelectorAll("a"))){const V=((T=M.getAttribute("href"))==null?void 0:T.replace(/^#/,""))??"";M.toggleAttribute("aria-current",k!==null&&k!==""&&V===k),M.hasAttribute("aria-current")&&M.setAttribute("aria-current","page")}};let ie=0;const S=()=>{const k=Mn(y,location.hash);if(Le(k),k===null){L.isConnected&&L.remove(),f.isConnected||v.appendChild(f),Pn(_,"Not found.");return}const{view:T,chrome:M}=An(y[k]),V=M?_:L;M?(L.isConnected&&L.remove(),f.isConnected||v.appendChild(f)):(f.isConnected&&f.remove(),L.isConnected||v.appendChild(L)),V.replaceChildren();const Ce=T(),re=++ie,W=Ce.mount(V);W instanceof Promise&&W.catch(ce=>{re===ie&&qn(V,String(ce))})};window.addEventListener("hashchange",S),S()}function Pn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-empty",v.textContent=r,y.appendChild(v)}function qn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-error",v.textContent=r,y.appendChild(v)}function Nn(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const v of r)v.unregister()}),window.caches&&caches.keys().then(r=>{for(const v of r)caches.delete(v)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function On(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let r;try{r=await y.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Rn=`
<div class="layout">
<aside class="sidebar">
...
`;function Vn(y){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null},v=e=>"pill pill-"+(e||"none"),$='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function E(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],P()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],I(),h.postingId){const t=r.jobs.find(n=>n.posting_id===h.postingId);t&&(h.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&G())}}function j(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",je(),e==="jobs"?I():P()}async function _(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,F()}function Le(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function ie(e){return e.slice().sort((t,n)=>r.sort.dir*Le(t,n,r.sort.k))}const S=new Set;let k=!1;function T(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",S.has(e.dataset.v))})}function M(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(S.size&&!S.has(t.verdict||"__none__")||k&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const V=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Ce=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function re(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const W=re("scout-hidden-cols"),ce=re("scout-hidden-jcols");function tt(){return r.view==="jobs"?{cols:Ce,hidden:ce,key:"scout-hidden-jcols"}:{cols:V,hidden:W,key:"scout-hidden-cols"}}function de(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=W.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=ce.has(e.dataset.col)?"none":""})}function je(){const e=tt();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=tt(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),je(),de()})})}function P(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=ie(M());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
    ...
    `,e.appendChild(s)}de(),e.querySelectorAll("tr").forEach(n=>{n.addEventListener("click",s=>{s.target.closest("a, .flag-btn")||X(n.dataset.id)})}),e.querySelectorAll(".flag-btn").forEach(n=>{n.addEventListener("click",()=>ft(n.dataset.id))})}const A=new Set;let le=!1,ue=!1,U=!0;function jt(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=U&&!A.has("rejected");return r.jobs.filter(n=>!(t&&n.response==="rejected"||A.size&&!A.has(n.response||"")||le&&!n.next_up||ue&&(n.outreach_count|0)>0||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const St=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function nt(e){const t=String(e||"").split(",").map(n=>n.trim()).filter(Boolean);return t.length?t.map(n=>{const s=n.match(St);return s?`<a href="mailto:${i(s[0])}" title="${i(n)}">${i(n)}</a>`:i(n)}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}const q={offer:{cls:"pill-yes",label:"offer",order:0},interview:{cls:"pill-info",label:"interview",order:1},screening:{cls:"pill-maybe",label:"screening call",order:2},"":{cls:"pill-none",label:"—",order:3},rejected:{cls:"pill-no",label:"rejected",order:4}};function Mt(e,t,n){var s,a;if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[e.verdict]??3)-(o[t.verdict]??3)}if(n==="response")return(((s=q[e.response])==null?void 0:s.order)??3)-(((a=q[t.response])==null?void 0:a.order)??3);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="applied_at"||n==="last_outreach_at"){const o=e[n]||"",c=t[n]||"";return!o&&!c?0:o?c?String(c).localeCompare(String(o)):-r.jsort.dir:r.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function K(e){const t=e.options[e.selectedIndex],n=getComputedStyle(e),s=K._c||(K._c=document.createElement("canvas").getContext("2d"));s.font=`${n.fontWeight} ${n.fontSize} ${n.fontFamily}`;const a=s.measureText(t?t.text:"").width;e.style.width=Math.ceil(a+35)+"px"}function I(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=jt().sort((d,m)=>r.jsort.dir*Mt(d,m,r.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=U&&!A.has("rejected")?r.jobs.filter(d=>d.response==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=r.jobs.filter(d=>d.next_up).length,o=document.getElementById("next-up-n");o.textContent=a,o.style.display=a?"":"none";const c=r.jobs.filter(d=>!(d.outreach_count|0)&&!(U&&!A.has("rejected")&&d.response==="rejected")).length,p=document.getElementById("not-reached-n");p.textContent=c,p.style.display=c?"":"none";const u=document.getElementById("jobs-hidden-note");u.style.display=n?"":"none",n&&(u.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{U=!1,document.getElementById("hide-rejected").classList.remove("is-on"),I()});for(const d of t){const m=q[d.response]||q[""],g=document.createElement("tr");g.dataset.id=d.posting_id;const C=[["","none"],["screening","screening"],["interview","interview"],["offer","offer"],["rejected","rejected"]].map(([x,D])=>`<option value="${x}"${(d.response||"")===x?" selected":""}>${D}</option>`).join("");g.innerHTML=`
      ...
      <td class="small td-contacts" data-col="contacts">${nt(d.contacts)}</td>
      <td data-col="link"><a href="${f(d.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `;const R=()=>new Date().toISOString().slice(0,10);g.querySelector(".jt-nextup").onclick=()=>rt(d,!1),g.querySelector(".jt-applied").onclick=()=>he(d,{applied_at:d.applied_at?"":R()}),g.querySelector(".jt-resp").onchange=x=>{K(x.target),he(d,{response:x.target.value})},g.querySelector(".jt-inc").onclick=()=>he(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:R()}),g.querySelector(".jt-dec").onclick=()=>{const x=Math.max(0,(d.outreach_count||0)-1);he(d,{outreach_count:x,...x===0?{last_outreach_at:""}:{}})},e.appendChild(g)}e.querySelectorAll(".jt-resp").forEach(K),de(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",m=>{m.target.closest("a, button, select")||st(d.dataset.id)})})}function At(e){return e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function st(e){let t=r.jobs.find(n=>n.posting_id===e);if(t||(await w(),t=r.jobs.find(n=>n.posting_id===e)),!t){l("posting not found — refresh");return}Me(),He(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),at("pursuit"),G(),Y(),Q()}let Se=null;function at(e){Se=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function pe(){Me(),He(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function Me(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function Y(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){me();return}e=await n.json()}catch{me();return}h.drafts=e.drafts||[],me();const t=h.drafts[0];t&&t.status==="researching"?Ht():Me()}function Ht(){h.poll||(h.poll=setInterval(Y,4e3))}function H(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),l(`save failed: ${o.message}`)}})}async function ot(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,summary:o.summary,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),I(),te(e.posting_id,{title:o.title,location:o.location,summary:o.summary})}async function Pt(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",f(e.url)),te(e.posting_id,{url:s.url})}async function it(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),E(),w()}async function qt(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();r.openId=s.company_id,ee(s),ne(s.company_id),E(),w()}async function Nt(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}function G(){const e=h.row;if(!e)return;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${i(e.title||"")}">`;const t=q[e.response]||q[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${t.cls}">${i(t.label)}</span>`+(e.verdict?` <span class="${v(e.verdict)}">${i(e.verdict)}</span>`:"");const n=document.getElementById("pursuit-chat");n&&(n.style.display=r.meta&&r.meta.chat?"":"none",n.onclick=()=>_e("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
  ...
  `,Rt();const s=document.getElementById("pursuit-company-link");s&&s.addEventListener("click",()=>X(e.company_id)),H(document.getElementById("pursuit-title-input"),a=>ot(e,"title",a)),H(document.getElementById("pursuit-url-input"),a=>Pt(e,a)),H(document.getElementById("pursuit-notes-input"),a=>Vt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>H(a,o=>ot(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),me(),Z()}function Ot(e){return`
    ...
    <div class="outreach-contacts">
      ...
      <div class="oc-rendered">${nt(t.contacts)}</div>
    ...
    <details class="draft-history" ${h.openHist?"open":""}>
      ...
      <div id="draft-history-body">${a.map(d=>dt(d,!0)).join("")}</div>
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?dt(s,!1):""}</div><div class="draft-actions">${p}</div>`+u,Jt()}function Ut(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function dt(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(m,g)=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const m=Dt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      <div class="draft-sentbody">${i(lt(e)||"(empty)")}</div>
      ${fe(e)}
    </div>`;const a=lt(e),o=e.status==="no_hook",c=o?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let p="";if(o)try{p=JSON.parse(e.hook||"{}").reasoning||""}catch{}const u=o?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${p?" "+i(p):""}</div>`:"";if(t)return`<div class="draft-card ${o?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    ${fe(e)}
  </div>`}function lt(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function fe(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,d)=>d?`<div class="tr-line"><span class="tr-key">${u}:</span> ${i(String(d))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],p=c.map(u=>`
      <div class="tr-line">
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",Ve);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",ht)}function mt(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function X(e){r.openId=e;const t=document.getElementById("pane"),n=document.getElementById("scrim");t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),at("company"),document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';let s;try{const a=await fetch(`/api/companies/${e}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);s=await a.json()}catch(a){document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${i(a.message)}</div>`;return}ee(s),ne(e)}function ge(){r.openId=null,r.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function ee(e){r.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${i(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `;const C=document.getElementById("posting-add-btn");C&&C.addEventListener("click",()=>dn(e)),qe(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(B=>B.classList.remove("is-on")),b.classList.add("is-on")})});const R=document.getElementById("ve-save-btn");R&&R.addEventListener("click",()=>cn(e)),H(document.getElementById("pane-title-input"),b=>it(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>H(b,B=>it(e,b.dataset.k,B))),H(document.getElementById("pane-domain-input"),b=>qt(e,b)),H(document.getElementById("pane-notes-input"),b=>Nt(e,b),{multiline:!0});const x=document.getElementById("flag-toggle-btn");x&&x.addEventListener("click",()=>ft(e.company_id));const D=document.getElementById("review-stamp-btn");D&&D.addEventListener("click",()=>rn(e.company_id));const ae=document.getElementById("rescore-btn");ae&&ae.addEventListener("click",()=>Oe("verdict",{company_ids:[e.company_id]}));const oe=document.getElementById("reenrich-btn");oe&&oe.addEventListener("click",()=>Oe("enrich",{company_ids:[e.company_id]}))}function on(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function te(e,t){const n=r.openDetail;if(!n||!r.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Pe(n),qe())}function qe(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||st(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),_e("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function dn(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const p=document.getElementById("postings-list");p&&(p.innerHTML=Pe(e),qe()),t.value="",n.value="",s.disabled=!1,w(),l("link added")}async function ne(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){ve(`<div class="muted">Failed to load trail: ${i(s.message)}</div>`);return}if(!t.ok){ve(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){ve('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}ve(n.map(ln).join(""))}function ln(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
    ...
    </div>`}function ve(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let gt;function l(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(gt),gt=setTimeout(()=>t.classList.remove("show"),2200)}r.meta={control:!1,brain:!1,verdict:!1};async function un(){try{const s=await fetch("/api/meta");if(!s.ok)return;r.meta=await s.json()}catch{return}const e=r.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!r.meta.verdict,t.title=r.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=r.meta.chat?"":"none")}async function Ne(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let ye=null;async function Oe(e,t){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){l(`run failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(n.status===412){const a=await n.text();l(a.trim());return}if(!n.ok){l(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();$t(e,s)}async function pn(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){l(`upload failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(!n.ok){l(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();$t("ingest",s)}const hn=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let be=[],N=new Set,O="company";function vt(e){O=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",yt()}function Re(){return!!r.meta.capture&&document.getElementById("add-enrich").checked}function yt(){const e=document.getElementById("add-note");Re()?e.innerHTML=O==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=O==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function mn(){hn.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",N=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!r.meta.capture,t.classList.toggle("disabled",!r.meta.capture),t.title=r.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",r.meta.capture||(e.checked=!1),vt(r.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(r.rows||[]).map(s=>`<option value="${i(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),be=s.verticals||[]}catch{be=[]}bt()}function we(){document.getElementById("add-scrim").classList.remove("open")}function bt(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=be.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${N.has(s)?" sel":""}" data-v="${i(s)}">${i(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;N.has(a)?N.delete(a):N.add(a),s.classList.toggle("sel"),wt()}))):e.innerHTML=`<div class="none">${be.length?"no match":"no verticals in the set yet"}</div>`,wt()}function wt(){const e=N.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function kt(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Et(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){l(O==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,Re()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=g=>document.getElementById(g).value.trim(),c=Re();let p,u;c?(p="/api/capture",u={url:kt(t),kind:O==="company"?"company_page":"job_posting",fields:O==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...N].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):O==="company"?(p="/api/companies",u={website:t,name:o("add-name"),vertical:[...N].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(p="/api/postings",u={url:kt(t),title:o("add-title"),company:o("add-job-company")});let d;try{d=await fetch(p,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(g){l(`add failed: ${g.message}`),a();return}if(!d.ok){let g=`HTTP ${d.status}`;try{const C=await d.text();try{g=JSON.parse(C).error||g}catch{g=C.trim()||g}}catch{}if(a(),d.status===409){l(g||"That company is already in the list."),e.focus(),e.select();return}l(`add failed: ${g}`);return}const m=await d.json();if(a(),c&&!m.company_id){l(m.note||"couldn't classify that page");return}if(we(),E(),_(),w(),O==="job"){const g=m.posting&&m.posting.title||"job link";l(`tracking: ${g} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),j("jobs")}else c?(l(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`),X(m.company_id)):l("company added")}function $t(e,t){ye=t;const n=document.getElementById("drawer"),s=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",s.innerHTML="",n.classList.add("open"),Ne();const a=new EventSource(`/api/jobs/${t}/stream`),o=(c,p)=>{const u=document.createElement("div"),d=!p&&/^\s*warn:/i.test(c);u.className="ln"+(p?" ln-err":d?" ln-warn":""),u.textContent=d?c.replace(/^\s*warn:\s*/i,"⚠ "):c,s.appendChild(u),s.scrollTop=s.scrollHeight};a.addEventListener("line",c=>o(c.data,/error|failed/i.test(c.data))),a.addEventListener("end",c=>{a.close(),ye=null,o(`— ${c.data} —`,c.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",l(`${e} ${c.data}`),E(),_(),Ne(),w(),r.openId&&X(r.openId)}),a.onerror=()=>{a.close()}}async function fn(){if(ye)try{await fetch(`/api/jobs/${ye}/cancel`,{method:"POST"})}catch{}}let se=null;async function ke(e){se=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+e+".md",document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="",t.classList.add("open");try{const n=await fetch(`/api/${e}`);if(!n.ok){const a=(await n.text().catch(()=>"")).trim();document.getElementById("editor-text").value=n.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${a||"HTTP "+n.status}`;return}const s=await n.json();document.getElementById("editor-text").value=s.content||"",s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version)}catch(n){document.getElementById("editor-text").value="failed to load: "+n.message}}function Ee(){document.getElementById("editor-scrim").classList.remove("open"),se=null}const gn=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function Ve(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{De(await(await fetch("/api/outreach/sources")).json())}catch(e){l(`failed to load sources: ${e.message}`)}}function Ue(){document.getElementById("sources-scrim").classList.remove("open")}function De(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):gn,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],c=o.length?o.map(p=>`<li><span class="src-title">${i(p.title||p.page_id)}</span><button class="src-rm" data-need="${i(a.key)}" data-id="${i(p.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${i(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>yn(a.dataset.need,a.dataset.id)))}async function vn(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){l(`refresh failed: ${s.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){l(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const n=await t.json();n.warning?l(n.warning):l("sources refreshed"),De(n),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function yn(e,t){let n;try{n=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(s){l(`remove failed: ${s.message}`);return}if(!n.ok){l(`remove failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}De(await n.json())}async function bn(){if(!se)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${se}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(s){l(`save failed: ${s.message}`);return}if(!t.ok){l(`save failed: HTTP ${t.status}`);return}const n=await t.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version),l(`${se}.md saved`),Ee(),_()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),P()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),I()}}),document.getElementById("tab-companies").onclick=()=>j("companies"),document.getElementById("tab-jobs").onclick=()=>j("jobs"),document.getElementById("q").oninput=P,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;S.has(t)?S.delete(t):S.add(t),T(),P()})}),document.getElementById("flag-filter").addEventListener("click",e=>{k=!k,e.currentTarget.classList.toggle("is-on",k),P()}),document.getElementById("jq").oninput=I,document.getElementById("hide-rejected").addEventListener("click",e=>{U=!U,e.currentTarget.classList.toggle("is-on",U),I()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;A.has(t)?A.delete(t):A.add(t),e.classList.toggle("is-on",A.has(t)),I()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{le=!le,e.currentTarget.classList.toggle("is-on",le),I()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{ue=!ue,e.currentTarget.classList.toggle("is-on",ue),I()}),je(),de(),document.getElementById("pane-close").onclick=ge,document.getElementById("scrim").onclick=ge,document.getElementById("pursuit-close").onclick=pe,document.getElementById("pursuit-scrim").onclick=pe,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){Xe();return}if(xn()){Ge();return}if(document.getElementById("profile-scrim").classList.contains("open")){Ye();return}if(document.getElementById("add-scrim").classList.contains("open")){we();return}if(document.getElementById("run-scrim").classList.contains("open")){$e();return}if(document.getElementById("help-scrim").classList.contains("open")){Ie();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(Se==="pursuit"&&n){pe();return}if(Se==="company"&&t){ge();return}if(t){ge();return}pe();return}if(document.getElementById("sources-scrim").classList.contains("open")){Ue();return}if(document.getElementById("editor-scrim").classList.contains("open")){Ee();return}if(document.getElementById("settings-scrim").classList.contains("open")){Qe();return}});let Je=null;const wn={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function It(e){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}Je=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=wn[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=r.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function $e(){document.getElementById("run-scrim").classList.remove("open"),Je=null}document.getElementById("btn-enrich").onclick=()=>It("enrich"),document.getElementById("btn-verdict").onclick=()=>It("verdict"),document.getElementById("run-cancel").onclick=$e,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&$e()},document.getElementById("run-go").onclick=()=>{const e=Je,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if($e(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),Oe(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=mn;const kn={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function xt(e){const t=kn[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const p=document.createElement("a");p.className="help-link",p.textContent="Learn more →",p.onclick=()=>{Ie(),_t(),Bn(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(p),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Ie(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>xt("add"),document.getElementById("help-run").onclick=()=>xt("run"),document.getElementById("help-close").onclick=Ie,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Ie()},document.getElementById("add-cancel").onclick=we,document.getElementById("add-save").onclick=Et,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&we()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>vt(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",yt),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Et()))}),document.getElementById("add-vertical-filter").addEventListener("input",bt),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&pn(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=fn,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=Ee,document.getElementById("editor-save").onclick=bn,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Ee()},document.getElementById("sources-close").onclick=Ue,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&Ue()},document.getElementById("sources-refresh-btn").onclick=vn;function ze(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function Fe(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}F()}const We='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',Bt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>';function F(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,s=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o="";if(s){let B="off",J="";const et=t&&t.criteria_state;et==="current"?(B="ok",J="current · verified "+ze(t.verified_age_seconds)):et==="changed"?(B="warn",J="changed — re-distill"):et==="unverified"?(B="warn",J=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(B="warn",J="brain offline · using cache"):a&&(B="ok",J="fetched "+ze(t.age_seconds)),o+=`<div class="crit-row">
      <span class="crit-what"><span class="pf-dot ${B}"></span>${a?'<span class="edit-link" id="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief"}</span>
      <button class="crit-edit" id="refresh-profile" title="re-distill the company-fit brief from the brain" aria-label="refresh company-fit brief">${Bt}</button></div>`,J&&(o+=`<div class="crit-note dim small">${i(J)}</div>`)}else o+=`<div class="crit-row">
    ...
    <button class="crit-edit" id="edit-template" title="edit the outreach email template" aria-label="edit email template">${We}</button></div>`;const c=r.sources&&r.sources.sources||[],p=c.filter(B=>B.need==="experience").length,u=c.filter(B=>B.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";p>0?(d="ok",m=`${p} experience · ${u} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const g=c.length?'<span class="edit-link" id="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge";o+=`<div class="crit-row">
    <span class="crit-what"><span class="pf-dot ${d}"></span>${g}</span>
    <button class="crit-edit" id="refresh-sources" title="re-discover experience + voice from the brain" aria-label="refresh outreach knowledge">${Bt}</button></div>`,o+=`<div class="crit-note dim small">${i(m)}</div>`,e.innerHTML=o;const C=document.getElementById("view-profile");C&&(C.onclick=()=>In(r.profile));const R=document.getElementById("refresh-profile");R&&(R.onclick=$n);const x=document.getElementById("edit-taste");x&&(x.onclick=()=>ke("taste"));const D=document.getElementById("edit-playbook");D&&(D.onclick=()=>ke("playbook"));const ae=document.getElementById("edit-template");ae&&(ae.onclick=()=>ke("outreach-template"));const oe=document.getElementById("view-sources");oe&&(oe.onclick=Ve);const b=document.getElementById("refresh-sources");b&&(b.onclick=En)}async function Ke(){try{r.sources=await(await fetch("/api/outreach/sources")).json()}catch{r.sources=null}F()}async function En(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){l(`refresh failed: ${s.message}`),Ke();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`refresh failed: ${s||"HTTP "+t.status}`),Ke();return}const n=await t.json();n.warning?l(n.warning):l("outreach knowledge refreshed"),r.sources={sources:n.sources||[],needs:r.sources&&r.sources.needs||[]},F()}async function $n(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),Fe();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Fe();return}r.profile=await t.json(),F(),l("company-fit brief refreshed"),_()}function In(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${ze(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Ye(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ye,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ye()};function _t(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");xe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ge(){document.getElementById("docs-scrim").classList.remove("open")}function xn(){return document.getElementById("docs-scrim").classList.contains("open")}function xe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Bn(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),xe(e)}document.getElementById("open-docs").onclick=_t,document.getElementById("docs-close").onclick=Ge,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Ge()};function _n(){document.getElementById("settings-scrim").classList.add("open"),F()}function Qe(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=_n,document.getElementById("settings-close").onclick=Qe,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&Qe()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),xe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&xe(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),r.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Tn(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Ln(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Be(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function Ze(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Cn(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=r.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(r.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Tt(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=Tn(n.content);if(n.role==="user")s&&t.appendChild(Be("user",s));else if(n.role==="assistant"){const a=Ln(n.content);if(!s&&!a.length)continue;const o=Be("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Cn()),Ze()}async function _e(e,t,n){if(!r.meta||!r.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}r.chat.es&&(r.chat.es.close(),r.chat.es=null),r.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const p=await c.json();r.chat.threadId=p.thread.id,Tt(p.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+i(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Xe(){r.chat.es&&(r.chat.es.close(),r.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Te(e){r.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Lt(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Ct(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||r.chat.streaming||!r.chat.threadId)return;e.value="",Lt(),Te(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Be("user",t));const a=Be("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),Ze();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Te(!1)},p=r.chat.threadId;let u;try{u=await fetch("/api/chat/"+p+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const d=new EventSource("/api/chat/"+p+"/stream");r.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,Ze()}),d.addEventListener("end",async m=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Te(!1),r.chat.threadId===p&&await jn(),Sn(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),d.onerror=()=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Te(!1)}}async function jn(){const e=r.chat.scope,t=r.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Tt(a.messages||[])}catch{}}function Sn(){E(),w(),_(),r.openId&&X(r.openId)}document.getElementById("open-chat").onclick=()=>_e("global","",""),document.getElementById("chat-close").onclick=Xe,document.getElementById("chat-scrim").onclick=Xe,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Ct()}),document.getElementById("chat-input").addEventListener("input",Lt),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Ct())}),E(),w(),_(),un(),Ne(),Fe(),Ke()}Hn({"":{view:()=>({mount(y){y.innerHTML=Rn,Vn()}}),chrome:!1}},{title:"scout"});Nn();On();
