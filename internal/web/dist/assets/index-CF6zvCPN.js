(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))L(r);new MutationObserver(r=>{for(const v of r)if(v.type==="childList")for(const I of v.addedNodes)I.tagName==="LINK"&&I.rel==="modulepreload"&&L(I)}).observe(document,{childList:!0,subtree:!0});function g(r){const v={};return r.integrity&&(v.integrity=r.integrity),r.referrerPolicy&&(v.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?v.credentials="include":r.crossOrigin==="anonymous"?v.credentials="omit":v.credentials="same-origin",v}function L(r){if(r.ep)return;r.ep=!0;const v=g(r);fetch(r.href,v)}})();function _s(y,i){const g=i.replace(/^#/,"");let L=null;for(const r of Object.keys(y))(g===r||g.startsWith(r))&&(L===null||r.length>L.length)&&(L=r);return L===null&&""in y&&(L=""),L}function Bs(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function Ls(y,i={}){const g=i.root??document.body,L=i.title??document.title??"",r=i.brandHref??"#",v=document.createElement("main"),I=document.createElement("header");I.className="cap-head";const E=document.createElement("a");E.className="brand",E.href=r,E.textContent=L,E.setAttribute("aria-label",`${L} — home`),I.appendChild(E);const D=document.createElement("nav");D.className="cap-nav",D.setAttribute("aria-label","Views");for(const T of i.nav??[]){const M=document.createElement("a");M.href=T.href,M.textContent=T.label,T.ariaLabel&&M.setAttribute("aria-label",T.ariaLabel),D.appendChild(M)}I.appendChild(D);const A=document.createElement("section");A.className="tk-content",v.appendChild(I),v.appendChild(A);const j=document.createElement("div");j.className="tk-bleed";const Ae=T=>{var M;for(const P of Array.from(D.querySelectorAll("a"))){const C=((M=P.getAttribute("href"))==null?void 0:M.replace(/^#/,""))??"";P.toggleAttribute("aria-current",T!==null&&T!==""&&C===T),P.hasAttribute("aria-current")&&P.setAttribute("aria-current","page")}};let Y=0;const H=()=>{const T=_s(y,location.hash);if(Ae(T),T===null){j.isConnected&&j.remove(),v.isConnected||g.appendChild(v),Ts(A,"Not found.");return}const{view:M,chrome:P}=Bs(y[T]),C=P?A:j;P?(j.isConnected&&j.remove(),v.isConnected||g.appendChild(v)):(v.isConnected&&v.remove(),j.isConnected||g.appendChild(j)),C.replaceChildren();const G=M(),He=++Y,te=G.mount(C);te instanceof Promise&&te.catch(Pe=>{He===Y&&Cs(C,String(Pe))})};window.addEventListener("hashchange",H),H()}function Ts(y,i){y.replaceChildren();const g=document.createElement("div");g.className="tk-empty",g.textContent=i,y.appendChild(g)}function Cs(y,i){y.replaceChildren();const g=document.createElement("div");g.className="tk-error",g.textContent=i,y.appendChild(g)}function Ss(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(i=>{for(const g of i)g.unregister()}),window.caches&&caches.keys().then(i=>{for(const g of i)caches.delete(g)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function js(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let i;try{i=await y.json()}catch{return null}return typeof i.email=="string"&&i.email?{email:i.email}:null}const Ms=`
<div class="layout">
<aside class="sidebar">
...
`;function As(y){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null,anthropicKey:null},g=e=>"pill pill-"+(e||"none"),L='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),v=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function I(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],q()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],x(),D(),j()}function D(){if(!m.postingId)return;const e=i.jobs.find(t=>t.posting_id===m.postingId);e&&(m.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&Z())}let A=null;function j(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!A?A=setInterval(Ae,4e3):!e&&A&&(clearInterval(A),A=null)}async function Ae(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&(x(),D()),j()}function Y(e){i.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",qe(),e==="jobs"?x():q()}async function H(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}i.stats=e,T()}function T(){const e=i.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,R()}function M(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function P(e){return e.slice().sort((t,n)=>i.sort.dir*M(t,n,i.sort.k))}const C=new Set;let G=!1;function He(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",C.has(e.dataset.v))})}function te(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(C.size&&!C.has(t.verdict||"__none__")||G&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Pe=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],en=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function mt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const ft=mt("scout-hidden-cols"),vt=mt("scout-hidden-jcols");function gt(){return i.view==="jobs"?{cols:en,hidden:vt,key:"scout-hidden-jcols"}:{cols:Pe,hidden:ft,key:"scout-hidden-cols"}}function Q(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=ft.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=vt.has(e.dataset.col)?"none":""})}function qe(){const e=gt();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=gt(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),qe(),Q()})})}function yt(e){return`
  ...
  `,bn();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>ce(e.company_id)),mn(e),O(document.getElementById("pursuit-title-input"),l=>xt(e,"title",l)),O(document.getElementById("pursuit-url-input"),l=>hn(e,l)),O(document.getElementById("pursuit-notes-input"),l=>wn(l),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(l=>O(l,u=>xt(e,l.dataset.k,u),{multiline:l.tagName==="TEXTAREA"})),be(),re(),t&&(t.scrollTop=s),Bt=e.posting_id}function yn(e){return`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?St(s,!1):""}</div><div class="draft-actions">${l}</div>`+u,Bn()}function kn(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const En='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',$n='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',Ct=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${En}</button>`,Ve=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],In='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function xn(e){let t=Ve.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${Ve.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",c=a<t?In:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${Ve[t].active}…</span></div>
    ...
    <div class="draft-head">
      ...
      <div class="draft-sentbody">${r(De(e)||"(empty)")}</div>
      ${oe(e)}
      ...
      <div class="draft-sentbody">${r(De(e)||"(empty)")}</div>
      ${oe(e)}
    </div>`;const s=De(e),a=e.status==="no_hook",o=a?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let c="";if(a)try{c=JSON.parse(e.hook||"{}").reasoning||""}catch{}const l=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${c?" "+r(c):""}</div>`:"";if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    ${oe(e)}
  </div>`}function De(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function oe(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,d)=>d?`<div class="tr-line"><span class="tr-key">${u}:</span> ${r(String(d))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],l=c.map(u=>`
      <div class="tr-line">
  ...
  `;const b=document.getElementById("posting-add-btn");b&&b.addEventListener("click",()=>zn(e)),ze(),document.querySelectorAll("#ve-pick .ve-opt").forEach(k=>{k.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(K=>K.classList.remove("is-on")),k.classList.add("is-on")})});const w=document.getElementById("ve-save-btn");w&&w.addEventListener("click",()=>Jn(e)),O(document.getElementById("pane-title-input"),k=>_t(e,"name",k)),document.querySelectorAll("#facts-body [data-k]").forEach(k=>O(k,K=>_t(e,k.dataset.k,K))),O(document.getElementById("pane-domain-input"),k=>vn(e,k)),O(document.getElementById("pane-notes-input"),k=>gn(e,k),{multiline:!0});const _=document.getElementById("flag-toggle-btn");_&&_.addEventListener("click",()=>Pt(e.company_id));const V=document.getElementById("review-stamp-btn");V&&V.addEventListener("click",()=>Un(e.company_id));const Me=document.getElementById("rescore-btn");Me&&Me.addEventListener("click",()=>We("verdict",{company_ids:[e.company_id]}));const he=document.getElementById("reenrich-btn");he&&he.addEventListener("click",()=>We("enrich",{company_ids:[e.company_id]}))}function Dn(e){return`
    <div class="ie-grid">
