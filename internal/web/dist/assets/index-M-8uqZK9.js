(function(){const v=document.createElement("link").relList;if(v&&v.supports&&v.supports("modulepreload"))return;for(const g of document.querySelectorAll('link[rel="modulepreload"]'))i(g);new MutationObserver(g=>{for(const I of g)if(I.type==="childList")for(const r of I.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function $(g){const I={};return g.integrity&&(I.integrity=g.integrity),g.referrerPolicy&&(I.referrerPolicy=g.referrerPolicy),g.crossOrigin==="use-credentials"?I.credentials="include":g.crossOrigin==="anonymous"?I.credentials="omit":I.credentials="same-origin",I}function i(g){if(g.ep)return;g.ep=!0;const I=$(g);fetch(g.href,I)}})();function Da(b,v){const $=v.replace(/^#/,"");let i=null;for(const g of Object.keys(b))($===g||$.startsWith(g))&&(i===null||g.length>i.length)&&(i=g);return i===null&&""in b&&(i=""),i}function Va(b){return typeof b=="function"?{view:b,chrome:!0}:{view:b.view,chrome:b.chrome!==!1}}function Ua(b,v={}){const $=v.root??document.body,i=v.title??document.title??"",g=v.brandHref??"#",I=document.createElement("main"),r=document.createElement("header");r.className="cap-head";const S=document.createElement("a");S.className="brand",S.href=g,S.textContent=i,S.setAttribute("aria-label",`${i} — home`),r.appendChild(S);const q=document.createElement("nav");q.className="cap-nav",q.setAttribute("aria-label","Views");for(const M of v.nav??[]){const O=document.createElement("a");O.href=M.href,O.textContent=M.label,M.ariaLabel&&O.setAttribute("aria-label",M.ariaLabel),q.appendChild(O)}r.appendChild(q);const _=document.createElement("section");_.className="tk-content",I.appendChild(r),I.appendChild(_);const D=document.createElement("div");D.className="tk-bleed";const ne=M=>{var O;for(const C of Array.from(q.querySelectorAll("a"))){const se=((O=C.getAttribute("href"))==null?void 0:O.replace(/^#/,""))??"";C.toggleAttribute("aria-current",M!==null&&M!==""&&se===M),C.hasAttribute("aria-current")&&C.setAttribute("aria-current","page")}};let ge=0;const Ce=()=>{const M=Da(b,location.hash);if(ne(M),M===null){D.isConnected&&D.remove(),I.isConnected||$.appendChild(I),Fa(_,"Not found.");return}const{view:O,chrome:C}=Va(b[M]),se=C?_:D;C?(D.isConnected&&D.remove(),I.isConnected||$.appendChild(I)):(I.isConnected&&I.remove(),D.isConnected||$.appendChild(D)),se.replaceChildren();const Ze=O(),Se=++ge,ve=Ze.mount(se);ve instanceof Promise&&ve.catch(V=>{Se===ge&&za(se,String(V))})};window.addEventListener("hashchange",Ce),Ce()}function Fa(b,v){b.replaceChildren();const $=document.createElement("div");$.className="tk-empty",$.textContent=v,b.appendChild($)}function za(b,v){b.replaceChildren();const $=document.createElement("div");$.className="tk-error",$.textContent=v,b.appendChild($)}function Ja(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(v=>{for(const $ of v)$.unregister()}),window.caches&&caches.keys().then(v=>{for(const $ of v)caches.delete($)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Ka(){let b;try{b=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!b.ok)return null;let v;try{v=await b.json()}catch{return null}return typeof v.email=="string"&&v.email?{email:v.email}:null}const Wa=`
<div class="layout">
<aside class="sidebar">
...
`;function Ya(b){const v={k:"verdict",dir:1},$={k:"created_at",dir:1},i={rows:[],sort:{...v},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...$},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null},g=e=>"pill pill-"+(e||"none"),I='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),S=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function q(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],z()}async function _(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],A(),D(),ge()}function D(){if(!m.postingId)return;const e=i.jobs.find(t=>t.posting_id===m.postingId);e&&(m.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&ae())}let ne=null;function ge(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!ne?ne=setInterval(Ce,4e3):!e&&ne&&(clearInterval(ne),ne=null)}async function Ce(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&(A(),D()),ge()}async function M(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(i.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(i.followupTemplate=e.content)}).catch(()=>{})]),Jt(),i.view==="jobs"&&A()}function O(e,{render:t=!0}={}){i.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Rt(),t&&(e==="jobs"?A():z())}async function C(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}i.stats=e,se()}function se(){oe()}function Ze(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function Se(e){return e.slice().sort((t,n)=>i.sort.dir*Ze(t,n,i.sort.k))}function ve(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(s=>{s.getAttribute(t)===n.k?s.dataset.sort=n.dir<0?"desc":"asc":delete s.dataset.sort})}const V=new Set;let ie=!1,ce=!1;const Zn=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function Qn(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+Zn.map(([t,n,s])=>J("data-v",t,n,s,V.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+J("data-toggle","flagged","⚑ Flagged","",ie)+J("data-toggle","enriched","Enriched","",ce),Ht())}function Ht(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const c of i.rows){const l=c.verdict||"__none__";e[l]=(e[l]|0)+1,c.flagged&&t++,c.enriched&&n++}et("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=V.size+(ie?1:0)+(ce?1:0);Yt("fdrop-cfilters-btn",o,o>0)}function Pt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(V.size&&!V.has(t.verdict||"__none__")||ie&&!t.flagged||ce&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Xn=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],es=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function qt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Ot=qt("scout-hidden-cols"),Nt=qt("scout-hidden-jcols");function Qe(){return i.view==="jobs"?{cols:es,hidden:Nt,key:"scout-hidden-jcols"}:{cols:Xn,hidden:Ot,key:"scout-hidden-cols"}}function re(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Ot.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Nt.has(e.dataset.col)?"none":""})}function Rt(){const e=Qe(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>J("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Dt())}function Dt(){const e=Qe(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Vt(e){return`
    ...
    `}function Ut(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>In(t.dataset.id))}const ts=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],ns=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function Ft(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let c=0;c<n;c++){const l=t.map(([p,u])=>{const h=u.endsWith("%")?Math.round(parseFloat(u)*a[c%a.length])+"%":u;return`<td${p?` data-col="${p}"`:""}><span class="skel" style="width:${h}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${l}</tr>`}s.innerHTML=o,re()}function z(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=Se(Pt());Ht(),document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=Vt(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||fe(s.dataset.id)}),e.appendChild(s),Ut(s)}ve("t","data-k",i.sort),re()}async function ss(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=Se(Pt()).map(c=>c.company_id),o=[...s.querySelectorAll("tr")].map(c=>c.dataset.id);if(a.length!==o.length||a.some((c,l)=>c!==o[l])){z();return}for(const c of e){const l=i.rows.find(u=>u.company_id===c),p=s.querySelector(`tr[data-id="${CSS.escape(c)}"]`);if(!l||!p){z();return}p.innerHTML=Vt(l),Ut(p)}re()}let B=null,Xe=null,le=!1,de=!1,ue=!1;const Y=new Set;function zt(){const e=i.applicationStages;if(B===null)B=new Set(["",...e.filter(t=>t!=="rejected")]);else{for(const t of[...B])t!==""&&!e.includes(t)&&B.delete(t);if(Xe)for(const t of e)t!=="rejected"&&!Xe.has(t)&&B.add(t)}Xe=new Set(e)}function as(){zt();const e=document.getElementById("jq").value.trim().toLowerCase();return i.jobs.filter(t=>{const n=t.application_status||"";return!(!B.has(n)||le&&!t.next_up||de&&(t.outreach_count|0)>0||ue&&!(t.followups_due|0)||Y.size&&!Y.has(t.outreach_status||"")||e&&!(t.title+" "+t.company+" "+(t.location||"")+" "+(t.description||"")+" "+(t.contacts||"")).toLowerCase().includes(e))})}function J(e,t,n,s,a){return`<button class="fdrop-item${a?" is-checked":""}" ${e}="${r(t)}" role="menuitemcheckbox" aria-checked="${a}"><span class="fdrop-check" aria-hidden="true"></span>`+(s?`<span class="fdrop-dot ${s}"></span>`:"")+`<span class="fdrop-label">${r(n)}</span><span class="fdrop-item-count" data-count></span></button>`}function Jt(){zt();const e=document.getElementById("fdrop-jfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Application stage</div>'+J("data-stage","","not applied","",B.has(""))+i.applicationStages.map(t=>J("data-stage",t,t,je(t),B.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Quick filters</div>'+J("data-toggle","nextup","★ Next up","",le)+J("data-toggle","notreached","Not reached out","",de)+'<div class="fdrop-sep"></div><div class="fdrop-head">Reply status</div>'+[["","none",""]].concat(i.outreachStatuses.map(t=>[t,t,tn(t)])).map(([t,n,s])=>J("data-status",t,n,s,Y.has(t))).join(""),Kt())}function Kt(){const e={},t={};let n=0,s=0;for(const l of i.jobs){const p=l.application_status||"";e[p]=(e[p]|0)+1;const u=l.outreach_status||"";t[u]=(t[u]|0)+1,l.next_up&&n++,l.outreach_count|0||s++}et("#fdrop-jfilters-menu [data-stage]","data-stage",e),et("#fdrop-jfilters-menu [data-status]","data-status",t),Wt("nextup",n),Wt("notreached",s);const a=["",...i.applicationStages.filter(l=>l!=="rejected")],c=(B&&B.size===a.length&&a.every(l=>B.has(l))?0:B?B.size:0)+(le?1:0)+(de?1:0)+Y.size;Yt("fdrop-jfilters-btn",c,c>0)}function et(e,t,n){document.querySelectorAll(e).forEach(s=>{const a=s.querySelector("[data-count]");if(a){const o=n[s.getAttribute(t)]|0;a.textContent=o||""}})}function Wt(e,t){const n=document.querySelector(`#fdrop-jfilters-menu [data-toggle="${e}"] [data-count]`);n&&(n.textContent=t||"")}function Yt(e,t,n){const s=document.getElementById(e);if(!s)return;s.classList.toggle("is-active",n);const a=s.querySelector(".fdrop-count");if(a){const o=n&&t>0;a.textContent=o?t:"",a.style.display=o?"":"none"}}function G(e,t){e.classList.toggle("is-checked",t),e.setAttribute("aria-checked",String(t))}function tt(){document.querySelectorAll(".fdrop.is-open").forEach(e=>{e.classList.remove("is-open");const t=e.querySelector(".fdrop-btn");t&&t.setAttribute("aria-expanded","false")})}function Gt(e){const t=e.querySelector(".fdrop-btn"),n=e.querySelector(".fdrop-menu");if(!t||!n)return;const s=t.getBoundingClientRect();n.style.left=Math.round(s.left)+"px",n.style.top=Math.round(s.bottom+4)+"px",n.style.minWidth=Math.round(s.width)+"px",n.style.maxHeight=Math.max(160,Math.round(window.innerHeight-s.bottom-12))+"px"}function os(e){const t=e.querySelector(".fdrop-btn");e.classList.add("is-open"),t&&t.setAttribute("aria-expanded","true"),Gt(e)}function Zt(){const e=document.querySelector(".fdrop.is-open");e&&Gt(e)}window.addEventListener("scroll",Zt,!0),window.addEventListener("resize",Zt);const is=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function cs(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(is),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}const be=()=>new Date().toISOString().slice(0,10);function Qt(e){const t=[["","none"]];for(const n of i.applicationStages)t.push([n,n]);return e&&!i.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function Xt(e){const t=[["","none"]];for(const n of i.outreachStatuses)t.push([n,n]);return e&&!i.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const rs=8;function en(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%rs}function je(e){return en(e,i.applicationStages)}function tn(e){return en(e,i.outreachStatuses)}function ls(e){const t=cs(e);return t.length?t.map(n=>{const s=r(n.position||n.email);if(!n.email)return s;const a=r(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${r(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function nn(e){const t=e.application_status||"";if(!t)return i.applicationStages.length+1;const n=i.applicationStages.indexOf(t);return n<0?i.applicationStages.length:n}function ds(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return nn(e)-nn(t);if(n==="followups_due")return(t.followups_due|0)-(e.followups_due|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function us(){const e=document.getElementById("jobs-followup-banner");if(!e)return;const t=i.jobs.reduce((n,s)=>n+(s.followups_due|0),0);if(!t){e.style.display="none",ue=!1;return}e.style.display="",e.classList.toggle("is-filtered",ue),e.innerHTML=`<span class="fb-icon">⏰</span><span class="fb-text"><strong>${t}</strong> follow-up${t>1?"s":""} due</span><button class="btn fb-toggle">${ue?"show all jobs":"show only these"}</button>`,e.querySelector(".fb-toggle").onclick=()=>{ue=!ue,A()}}function A(){const e=document.querySelector("#jt tbody");e.innerHTML="",us();const t=as().sort((a,o)=>i.jsort.dir*ds(a,o,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",Kt();const n=B&&!B.has("rejected")?i.jobs.filter(a=>(a.application_status||"")==="rejected").length:0,s=document.getElementById("jobs-hidden-note");s.style.display=n?"":"none",n&&(s.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{B.add("rejected"),Jt(),A()});for(const a of t){const o=a.application_status||"",c=document.createElement("tr");c.dataset.id=a.posting_id;const l=Qt(o).map(([h,y])=>`<option value="${r(h)}"${o===h?" selected":""}>${r(y)}</option>`).join(""),p=a.outreach_status||"",u=Xt(p).map(([h,y])=>`<option value="${r(h)}"${p===h?" selected":""}>${r(y)}</option>`).join("");c.innerHTML=`
      ...
      <td class="small" data-col="outreach"><div class="jt-out"><select class="jt-ostatus ${tn(p)}" title="outreach reply status">${u}</select>${a.followups_due?`<span class="followup-badge" title="${a.followups_due} follow-up${a.followups_due>1?"s":""} due — open to act">⏰ ${a.followups_due}</span>`:""}</div></td>
    ...
    `,c.querySelector(".jt-nextup").onclick=()=>pn(a,!1),c.querySelector(".jt-stage-sel").onchange=h=>fn(a,{application_status:h.target.value}),c.querySelector(".jt-ostatus").onchange=h=>fn(a,{outreach_status:h.target.value}),e.appendChild(c)}ve("jt","data-jk",i.jsort),re(),e.querySelectorAll("tr").forEach(a=>{a.addEventListener("click",o=>{o.target.closest("a, button, select")||sn(a.dataset.id)})})}function ps(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const m={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1,contacts:[],outreach:[],contactsLoaded:!1};async function sn(e){let t=i.jobs.find(n=>n.posting_id===e);if(t||(await _(),t=i.jobs.find(n=>n.posting_id===e)),!t){d("posting not found — refresh");return}st(),rt(),m.postingId=e,m.row=t,m.drafts=[],m.openHist=!1,m.answers=[],m.detecting=!1,m.contacts=[],m.outreach=[],m.contactsLoaded=!1,m.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),on("pursuit"),ae(),ke(),he(),an()}async function an(){const e=m.postingId,t=m.row&&m.row.company_id;if(!(!e||!t)){try{const[n,s]=await Promise.all([fetch(`/api/companies/${t}/contacts`).then(a=>a.ok?a.json():[]),fetch(`/api/postings/${e}/outreach-log`).then(a=>a.ok?a.json():[])]);if(m.postingId!==e)return;m.contacts=Array.isArray(n)?n:[],m.outreach=Array.isArray(s)?s:[]}catch{}m.contactsLoaded=!0,me()}}let nt=null;function on(e){nt=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function we(){st(),rt(),m.postingId=null,m.row=null,m.drafts=[],m.answers=[],m.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function st(){m.poll&&(clearInterval(m.poll),m.poll=null)}async function ke(){if(!m.postingId)return;let e;try{const n=await fetch(`/api/postings/${m.postingId}/outreach`);if(!n.ok){me();return}e=await n.json()}catch{me();return}m.drafts=e.drafts||[],me();const t=m.drafts[0];t&&t.status==="researching"?ms():st()}function ms(){m.poll||(m.poll=setInterval(ke,4e3))}function U(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),d(`save failed: ${o.message}`)}})}async function cn(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),A(),ye(e.posting_id,{title:o.title,location:o.location})}async function hs(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",S(e.url)),ye(e.posting_id,{url:s.url})}async function fs(e,t){if(t.disabled)return;const n=t.textContent;t.disabled=!0,t.textContent="re-enriching…";let s;try{s=await fetch(`/api/postings/${e.posting_id}/recapture`,{method:"POST"})}catch(o){t.disabled=!1,t.textContent=n,d(`re-enrich failed: ${o.message}`);return}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();let c=o||"HTTP "+s.status;try{c=JSON.parse(o).error||c}catch{}t.disabled=!1,t.textContent=n,d(`re-enrich failed: ${c}`);return}const a=await s.json();Object.assign(e,{title:a.title,location:a.location,employment_type:a.employment_type,workplace_type:a.workplace_type,department:a.department,comp_range:a.comp_range,description:a.description,posted_at:a.posted_at,url:a.url,questions_status:a.questions_status}),A(),ae(),ye(e.posting_id,{title:a.title,location:a.location,url:a.url}),d("re-enriched from the posting link")}function ys(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>vs(e))}async function gs(e,t){const n=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.company_id=s.company_id,e.company=s.company_name,ae(),_()}let Ee=null;function vs(e){Ee=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const n=document.getElementById("relink-search");n&&(n.value=""),rn(""),document.getElementById("relink-scrim").classList.add("open"),n&&n.focus()}function pe(){document.getElementById("relink-scrim").classList.remove("open"),Ee=null}let at=null;function bs(e){at=e;const t=(e.postings||[]).length,n=t?` and its ${t} job ${t===1?"posting":"postings"}`:"",s=document.getElementById("delcompany-summary");s&&(s.innerHTML=`Delete <strong>${r(e.name||"this company")}</strong>${n}?`);const a=document.getElementById("delcompany-confirm");a&&(a.disabled=!1),document.getElementById("delcompany-scrim").classList.add("open")}function Me(){document.getElementById("delcompany-scrim").classList.remove("open"),at=null}async function ws(){const e=at;if(!e)return;const t=document.getElementById("delcompany-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e.company_id}`,{method:"DELETE"})}catch(a){d(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");d(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=e.name||"company";Me(),i.openId===e.company_id&&xe(),q(),_(),C(),d(`deleted ${s}`)}let ot=null;function ks(e){ot=e;const t=(e.title||"").trim()||"this posting",n=e.company?` at <strong>${r(e.company)}</strong>`:"",s=document.getElementById("deljob-summary");s&&(s.innerHTML=`Delete <strong>${r(t)}</strong>${n}?`);const a=document.getElementById("deljob-confirm");a&&(a.disabled=!1),document.getElementById("deljob-scrim").classList.add("open")}function Ae(){document.getElementById("deljob-scrim").classList.remove("open"),ot=null}async function Es(){const e=ot;if(!e)return;const t=document.getElementById("deljob-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${e.posting_id}`,{method:"DELETE"})}catch(a){d(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");d(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=(e.title||"").trim()||"posting";Ae(),we(),_(),i.openId===e.company_id&&fe(e.company_id),d(`deleted ${s}`)}function rn(e){const t=document.getElementById("relink-results");if(!t)return;const n=e.trim().toLowerCase();let s=(i.rows||[]).slice();if(n?(s=s.filter(o=>(o.name||"").toLowerCase().includes(n)),s.sort((o,c)=>{const l=(o.name||"").toLowerCase().startsWith(n)?0:1,p=(c.name||"").toLowerCase().startsWith(n)?0:1;return l-p||(o.name||"").localeCompare(c.name||"")})):s.sort((o,c)=>(o.name||"").localeCompare(c.name||"")),s=s.slice(0,60),!s.length){t.innerHTML=`<div class="relink-empty">${(i.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const a=Ee?Ee.company_id:"";t.innerHTML=s.map(o=>{const c=o.company_id===a,l=[o.vertical,o.location].filter(Boolean).map(r).join(" · ");return`<button type="button" class="relink-result${c?" is-current":""}"
      ...
      </button>`}).join("")}async function ln(e){const t=Ee;if(!t){pe();return}if(e===t.company_id){pe();return}try{await gs(t,e),pe(),d(`moved to ${t.company}`)}catch(n){d(`move failed: ${n.message}`)}}async function dn(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),q(),_()}async function $s(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();i.openId=s.company_id,_e(s),Be(s.company_id),q(),_()}async function Is(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}let un=null;function ae(){const e=m.row;if(!e)return;const t=document.getElementById("pursuit-body"),s=!!t&&un===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${r(e.title||"")}">`;const a=e.application_status||"";document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a?je(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`+(e.verdict?` <span class="${g(e.verdict)}">${r(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>Mt("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row">
          ...
          <select class="input pl-appstatus" title="application stage">
            ${Qt(e.application_status||"").map(([u,h])=>`<option value="${r(u)}"${(e.application_status||"")===u?" selected":""}>${r(h)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
          ...
          <select class="input pl-ostatus" title="outreach reply status — separate from the application stage">
            ${Xt(e.outreach_status||"").map(([u,h])=>`<option value="${r(u)}"${(e.outreach_status||"")===u?" selected":""}>${r(h)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
  ...
  `,_s();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>fe(e.company_id)),ys(e),U(document.getElementById("pursuit-title-input"),u=>cn(e,"title",u)),U(document.getElementById("pursuit-url-input"),u=>hs(e,u));const l=document.getElementById("pursuit-reenrich");l&&l.addEventListener("click",()=>fs(e,l)),U(document.getElementById("pursuit-notes-input"),u=>Bs(u),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(u=>U(u,h=>cn(e,u.dataset.k,h),{multiline:u.tagName==="TEXTAREA"}));const p=document.getElementById("job-delete-btn");p&&p.addEventListener("click",()=>ks(e)),me(),Ie(),t&&(t.scrollTop=s),un=e.posting_id}function xs(e){return`
    ...
    </div>`;if(!m.contactsLoaded)return`<div class="contacts-mgr">${t}<div class="loading-row"><span class="spinner"></span><span>loading contacts…</span></div></div>`;const n=m.contacts.map(Ss).join(""),s=m.contacts.length?"":`<div class="cc-empty dim">No contacts yet — add the people you're reaching out to at ${r(e.company)}.</div>`;return`<div class="contacts-mgr">
    ${t}
  ...
  </div>`}function Ss(e){const t=m.outreach.filter(o=>o.contact_id===e.id),n=t[0]||null,s=e.role?`<span class="cc-role">${r(e.role)}</span>`:"",a=e.email?`<a class="cc-mail" href="mailto:${r(e.email)}" title="${r(e.email)}">${r(e.email)}</a>`:"";return`<div class="contact-card" data-cid="${e.id}">
    <div class="cc-head">
    ...
    ${t.length?`<details class="cc-history"><summary>${t.length} send${t.length>1?"s":""}</summary><div class="cc-entries">${t.map(Ms).join("")}</div></details>`:""}
      ...
      ${n}
    </div>`}async function K(e,t,n){let s;try{s=await fetch(t,{method:e,headers:n?{"Content-Type":"application/json"}:{},body:n?JSON.stringify(n):void 0})}catch(a){return d(`save failed: ${a.message}`),null}if(!s.ok){const a=(await s.text().catch(()=>"")).trim();return d(`save failed: ${a||"HTTP "+s.status}`),null}try{return await s.json()}catch{return{}}}async function Z(){await _(),await an()}function As(){const e=document.getElementById("outreach-section");if(!e)return;const t=m.postingId,n=e.querySelector(".fu-interval");n&&n.addEventListener("change",async()=>{const a=Math.max(0,Math.min(90,parseInt(n.value,10)||0));await K("PUT","/api/followup-interval",{days:a})&&(i.followupInterval=a,d("follow-up interval saved"))});const s=e.querySelector(".cc-addwrap");if(s){const a=s.querySelector(".cc-addform");s.querySelector(".cc-addbtn").addEventListener("click",()=>{a.style.display="",s.querySelector(".cc-addbtn").style.display="none",a.querySelector(".cc-f-name").focus()}),s.querySelector(".cc-f-cancel").addEventListener("click",()=>me()),s.querySelector(".cc-f-save").addEventListener("click",async()=>{const o={name:a.querySelector(".cc-f-name").value,role:a.querySelector(".cc-f-role").value,email:a.querySelector(".cc-f-email").value};await K("POST",`/api/companies/${m.row.company_id}/contacts`,o)&&(d("contact added"),Z())})}e.querySelectorAll(".contact-card").forEach(a=>{const o=a.dataset.cid,c=a.querySelector(".cc-editform");a.querySelector(".cc-edit").addEventListener("click",()=>{c.style.display=c.style.display==="none"?"":"none",c.style.display!=="none"&&c.querySelector(".cc-e-name").focus()});const l=a.querySelector(".cc-e-cancel");l&&l.addEventListener("click",()=>{c.style.display="none"});const p=a.querySelector(".cc-e-save");p&&p.addEventListener("click",async()=>{const E={name:c.querySelector(".cc-e-name").value,role:c.querySelector(".cc-e-role").value,email:c.querySelector(".cc-e-email").value};await K("PUT",`/api/contacts/${o}`,E)&&(d("contact saved"),Z())}),a.querySelector(".cc-arch").addEventListener("click",async()=>{await K("DELETE",`/api/contacts/${o}`)&&(d("contact removed"),Z())});const u=a.querySelector(".cc-logform");a.querySelector(".cc-log").addEventListener("click",()=>{u.style.display=u.style.display==="none"?"":"none",u.style.display!=="none"&&u.querySelector(".cc-l-date").focus()});const h=a.querySelector(".cc-l-cancel");h&&h.addEventListener("click",()=>{u.style.display="none"});const y=a.querySelector(".cc-l-save");y&&y.addEventListener("click",async()=>{const E={contact_id:o,sent_at:u.querySelector(".cc-l-date").value||be(),body:u.querySelector(".cc-l-body").value};await K("POST",`/api/postings/${t}/outreach-log`,E)&&(d("outreach logged"),Z())});const w=a.querySelector(".cc-followup");w&&w.addEventListener("click",()=>{const E=m.contacts.find(k=>k.id===o),j=m.outreach.filter(k=>k.contact_id===o)[0]||null,f=Ts(E,j);pt(f,"follow-up copied — paste into your email"),u.style.display="",u.querySelector(".cc-l-body").value=f,u.querySelector(".cc-l-date").value=be()});const L=(E,j)=>{const f=m.outreach.find(k=>String(k.id)===String(E))||{};return{sent_at:f.sent_at||"",body:f.body||"",note:f.note||"",followup_due_at:f.followup_due_at||"",done:!!f.followup_done_at,...j}},T=a.querySelector(".fu-date");T&&T.addEventListener("change",async()=>{await K("PUT",`/api/outreach-log/${T.dataset.eid}`,L(T.dataset.eid,{followup_due_at:T.value,done:!1}))&&(d("follow-up updated"),Z())});const H=a.querySelector(".cc-fu-done");H&&H.addEventListener("click",async()=>{await K("PUT",`/api/outreach-log/${H.dataset.eid}`,L(H.dataset.eid,{done:!0}))&&(d("marked followed up"),Z())});const R=a.querySelector(".cc-fu-set");R&&R.addEventListener("click",async()=>{const E=Ls(be(),i.followupInterval||5);await K("PUT",`/api/outreach-log/${R.dataset.eid}`,L(R.dataset.eid,{followup_due_at:E,done:!1}))&&(d("reminder set"),Z())}),a.querySelectorAll(".cc-e-del").forEach(E=>E.addEventListener("click",async()=>{const j=E.closest(".cc-entry").dataset.eid;await K("DELETE",`/api/outreach-log/${j}`)&&(d("send deleted"),Z())}))})}function Hs(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const yn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',Ps='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',gn=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${yn}</button>`,it=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],qs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function Os(e){let t=it.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${it.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",c=a<t?qs:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${it[t].active}…</span></div>
  </div>`}function vn(e,t){const n=(u,h,y="")=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const u=Ns(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      <div class="draft-sentbody">${r(ct(e)||"(empty)")}</div>
      ${$e(e)}
    </div>`;if(e.status==="sent")return`<div class="draft-card dc-sent" data-did="${e.id}">
      ${n("pill pill-yes","sent",t?"":gn)}
      ${e.sent_at?`<div class="draft-note">Sent ${r((e.sent_at||"").replace("T"," ").slice(0,16))}</div>`:""}
      <div class="draft-sentbody">${r(ct(e)||"(empty)")}</div>
      ${$e(e)}
    </div>`;const s=ct(e),a=e.status==="no_hook",o=a?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let c="";if(a)try{c=JSON.parse(e.hook||"{}").reasoning||""}catch{}const l=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${c?" "+r(c):""}</div>`:"";if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    <div class="draft-head">${o}${s?gn:""}</div>
    ${l}
    ${p?`<textarea class="draft-textarea" id="draft-edit-${e.id}" spellcheck="false">${r(s)}</textarea>
    ${bn(e.lint)}
    <div class="draft-actions">
      <button class="btn btn-primary draft-sent-btn" title="mark this email sent — bumps the outreach count">${Ps}Mark sent</button>
    ...
    ${$e(e)}
  </div>`}function ct(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function $e(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(p,u)=>u?`<div class="tr-line"><span class="tr-key">${p}:</span> ${r(String(u))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],l=c.map(p=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function bn(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${r(n.message||"")}"><code>${r(n.code||"")}</code>${r(n.message||"")}</span>`).join("")+"</div>":""}function Ns(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${r(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${r(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function Rs(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#draft-start-btn");t&&t.addEventListener("click",()=>He(!1,Ds())),e.querySelectorAll(".draft-retry-btn").forEach(s=>s.addEventListener("click",()=>He())),e.querySelectorAll(".draft-regen-btn").forEach(s=>s.addEventListener("click",()=>He(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(s=>{const a=s.dataset.did,o=s.querySelector(".draft-textarea");o&&U(o,p=>Us(a,p),{multiline:!0});const c=s.querySelector(".draft-sent-btn");c&&c.addEventListener("click",()=>Fs(a));const l=s.querySelector(".draft-copy-btn");l&&l.addEventListener("click",()=>{const p=s.querySelector(".draft-textarea"),u=s.querySelector(".draft-sentbody"),h=p?p.value:u?u.textContent:"";pt(h,"email copied")})});const n=e.querySelector("details.draft-history");n&&n.addEventListener("toggle",()=>{m.openHist=n.open})}function Ds(){const e=document.getElementById("draft-skip-research");return!!(e&&e.checked)}async function He(e=!1,t=!1){const n=document.getElementById("outreach-section"),s=n&&(n.querySelector("#draft-start-btn")||n.querySelector(".draft-retry-btn")||n.querySelector(".draft-regen-btn"));s&&(s.disabled=!0);let a;try{const c=new URLSearchParams;e&&c.set("regenerate","1"),t&&c.set("research","0");const l=c.toString()?`?${c.toString()}`:"";a=await fetch(`/api/postings/${m.postingId}/outreach${l}`,{method:"POST"})}catch(c){d(`draft failed: ${c.message}`),s&&(s.disabled=!1);return}if(a.status===202){let c={};try{c=await a.json()}catch{}Array.isArray(c.degraded)&&c.degraded.length&&d(`drafting without ${c.degraded.join(", ")} — quality degrades, integrity unaffected`),await ke(),_();return}if(a.status===409){await ke(),d("a draft is already active");return}if(a.status===412){let c={};try{c=await a.json()}catch{}Vs(c.need,c.error),s&&(s.disabled=!1);return}if(a.status===503){const c=document.getElementById("outreach-section");if(c){const l=document.createElement("div");l.className="draft-note",l.textContent="Outreach engine not running in this build.",c.appendChild(l)}s&&(s.disabled=!1);return}const o=(await a.text().catch(()=>"")).trim();d(`draft failed: ${o||"HTTP "+a.status}`),s&&(s.disabled=!1)}function Vs(e,t){const n=document.getElementById("outreach-section");if(!n)return;const s=n.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"View brain knowledge",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,s?s.replaceWith(c):n.appendChild(c);const l=c.querySelector("#gate-fix-btn");l&&l.addEventListener("click",()=>a?ee("outreach-template"):On());const p=c.querySelector("#gate-retry-btn");p&&p.addEventListener("click",He)}async function Us(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=m.drafts.findIndex(l=>String(l.id)===String(e));a>=0&&(m.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const l=c.querySelector(".lint-chips"),p=bn(s.lint);l?l.outerHTML=p||"":p&&o.insertAdjacentHTML("afterend",p)}}async function Fs(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){d(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();d(`failed: ${s||"HTTP "+t.status}`);return}d("marked sent"),await ke(),await _();const n=i.jobs.find(s=>s.posting_id===m.postingId);n&&ye(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function he(){if(!m.postingId)return;let e;try{const t=await fetch(`/api/postings/${m.postingId}/answers`);if(!t.ok){Ie();return}e=await t.json()}catch{Ie();return}m.answers=e.answers||[],m.answersStatus=e.questions_status||"",Ie(),m.answers.some(t=>t.status==="generating")?zs():rt()}function zs(){m.answersPoll||(m.answersPoll=setInterval(he,4e3))}function rt(){m.answersPoll&&(clearInterval(m.answersPoll),m.answersPoll=null)}function Ie(){const e=document.getElementById("answers-section");if(!e)return;const t=m.answers,n=m.answersStatus,s=t.some(u=>u.status==="generating"),a=t.length?`<div class="answers-list">${t.map(Ws).join("")}</div>`:"",o=!!m.detecting,c=s||o?" disabled":"",l=u=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":u}</button>`;let p;n==="ok"&&t.length?p=(t.some(h=>!wn(h)&&h.status!=="generating")?`<button class="btn" id="answers-start-btn"${c}>${s?"Drafting…":"Draft all blank"}</button>`:"")+l("Re-detect"):n===""||n==="unreachable"?p=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect questions"):p=l("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r(Js(n,t.length))}</div>`+a+`<div class="answers-actions">${p}</div>`,Gs()}function Js(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function wn(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Ks(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function Ws(e){const t=wn(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`,l=!!t,p=l?"Regenerate":"Generate",u=l?"re-draft this answer (discards the current text)":"draft an answer to just this question";return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    <div class="answer-foot">
      ...
      ${s||!l?"":`<button class="answer-copy-btn dh-copy" title="copy this answer to the clipboard" aria-label="copy answer">${yn}</button>`}
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${r(Ys(e.fail_reason))}</div>`:""}
  </div>`}function Ys(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function Gs(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",kn);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",Qs),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(U(o,u=>Xs(a,u),{multiline:!0}),o.addEventListener("input",()=>Zs(s,o)));const c=s.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>ea(a));const l=s.querySelector(".answer-copy-btn");l&&l.addEventListener("click",()=>{o&&pt(o.value,"answer copied")});const p=s.querySelector(".answer-remove-btn");p&&p.addEventListener("click",()=>ta(a))})}function Zs(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function kn(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${m.postingId}/answers`,{method:"POST"})}catch(a){d(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await he();return}if(n.status===412){let a={};try{a=await n.json()}catch{}En(a.error),t&&(t.disabled=!1);return}if(n.status===503){$n("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();d(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function Qs(){m.detecting=!0,Ie();try{const e=await fetch(`/api/postings/${m.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();d(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){d(`detect failed: ${e.message}`)}m.detecting=!1,await he()}async function Xs(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=m.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(m.answers[a]=s)}async function ea(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){d(`regenerate failed: ${n.message}`);return}if(t.status===503){$n("Answer generation isn't running in this build.");return}if(t.status===412){let n={};try{n=await t.json()}catch{}En(n.error);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();d(`regenerate failed: ${n||"HTTP "+t.status}`);return}await he()}async function ta(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"DELETE"})}catch(n){d(`remove failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();d(`remove failed: ${n||"HTTP "+t.status}`);return}await he()}function En(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",On);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",kn)}function $n(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function fe(e){var l,p;const t=document.getElementById("pane"),n=document.getElementById("scrim"),s=i.openId===e&&t.classList.contains("open"),a=s?((l=document.getElementById("pane-body"))==null?void 0:l.scrollTop)??0:0,o=s?(p=document.getElementById("trace-body"))==null?void 0:p.innerHTML:null;i.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),on("company"),s||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let c;try{const u=await fetch(`/api/companies/${e}`);if(!u.ok)throw new Error(`HTTP ${u.status}`);c=await u.json()}catch(u){s||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${r(u.message)}</div>`);return}if(i.openId===e){if(_e(c),s){if(o!=null){const h=document.getElementById("trace-body");h&&(h.innerHTML=o)}const u=document.getElementById("pane-body");u&&(u.scrollTop=a)}Be(e)}}function xe(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function _e(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${r(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `;const w=document.getElementById("posting-add-btn");w&&w.addEventListener("click",()=>oa(e)),dt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(f=>{f.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(k=>k.classList.remove("is-on")),f.classList.add("is-on")})});const L=document.getElementById("ve-save-btn");L&&L.addEventListener("click",()=>aa(e)),U(document.getElementById("pane-title-input"),f=>dn(e,"name",f)),document.querySelectorAll("#facts-body [data-k]").forEach(f=>U(f,k=>dn(e,f.dataset.k,k))),U(document.getElementById("pane-domain-input"),f=>$s(e,f)),U(document.getElementById("pane-notes-input"),f=>Is(e,f),{multiline:!0});const T=document.getElementById("flag-toggle-btn");T&&T.addEventListener("click",()=>In(e.company_id));const H=document.getElementById("review-stamp-btn");H&&H.addEventListener("click",()=>sa(e.company_id));const R=document.getElementById("rescore-btn");R&&R.addEventListener("click",()=>ft("verdict",{company_ids:[e.company_id]}));const E=document.getElementById("reenrich-btn");E&&E.addEventListener("click",()=>ft("enrich",{company_ids:[e.company_id]}));const j=document.getElementById("company-delete-btn");j&&j.addEventListener("click",()=>bs(e))}function na(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function sa(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){d(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");d(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=i.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,z()),i.openId===e&&(_e(s),Be(e)),d("reviewed")}async function In(e){const t=i.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){d(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");d(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,z()),i.openId===e&&(_e(a),Be(e)),_(),d(a.flagged?"flagged":"unflagged")}async function aa(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){d("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(l){d(`save failed: ${l.message}`),a.disabled=!1;return}if(!o.ok){const l=await o.text().catch(()=>"");d(`save failed: HTTP ${o.status}${l?" — "+l:""}`),a.disabled=!1;return}const c=await o.json();_e(c),Be(c.company_id),q(),C(),_(),d("verdict saved")}function lt(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(r).join(" · "),a=n.application_status||"",o=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?je(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`,`<span class="pt-meta">${a?"tracked":"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${r(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ye(e,t){const n=i.openDetail;if(!n||!i.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=lt(n),dt())}function dt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||sn(e.dataset.pid)})})}async function oa(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){d("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(p){d(`add failed: ${p.message}`),s.disabled=!1;return}if(!o.ok){const p=await o.text().catch(()=>"");d(`add failed: HTTP ${o.status}${p?" — "+p:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(p=>p.id!==c.id),e.postings.unshift(c);const l=document.getElementById("postings-list");l&&(l.innerHTML=lt(e),dt()),t.value="",n.value="",s.disabled=!1,_(),d("link added")}async function Be(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){Pe(`<div class="muted">Failed to load trail: ${r(s.message)}</div>`);return}if(!t.ok){Pe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Pe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Pe(n.map(ia).join(""))}function ia(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(r);return e.run_id&&t.push("run "+r(e.run_id.slice(0,8))),`
    ...
    </div>`}function Pe(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let xn;function d(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(xn),xn=setTimeout(()=>t.classList.remove("show"),2200)}let ut;const ca=6e3;function qe(){clearTimeout(ut),ut=void 0}function _n(){qe(),document.getElementById("drawer").classList.remove("open")}function Bn(){qe(),ut=setTimeout(_n,ca)}async function pt(e,t="copied"){if(!e){d("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}d(t)}catch(n){d(`copy failed: ${n.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function mt(){try{const s=await fetch("/api/meta");if(!s.ok)return;i.meta=await s.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=i.meta.chat?"":"none")}async function ht(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Le=null;async function ft(e,t){if(i.meta&&i.meta.control===!1){d("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){d(`run failed: ${a.message}`);return}if(n.status===409){d("a job is already running");return}if(n.status===412){const a=await n.text();d(a.trim());return}if(!n.ok){d(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();An(e,s,t)}async function ra(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){d(`upload failed: ${a.message}`);return}if(n.status===409){d("a job is already running");return}if(!n.ok){d(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();An("ingest",s)}const la=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let Oe=[],Q=new Set,X="company";function Ln(e){X=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",Tn()}function yt(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function Tn(){const e=document.getElementById("add-note");yt()?e.innerHTML=X==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and description — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=X==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function da(){la.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",Q=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),Ln(i.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(s=>`<option value="${r(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),Oe=s.verticals||[]}catch{Oe=[]}Cn()}function Ne(){document.getElementById("add-scrim").classList.remove("open")}function Cn(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=Oe.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${Q.has(s)?" sel":""}" data-v="${r(s)}">${r(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;Q.has(a)?Q.delete(a):Q.add(a),s.classList.toggle("sel"),Sn()}))):e.innerHTML=`<div class="none">${Oe.length?"no match":"no verticals in the set yet"}</div>`,Sn()}function Sn(){const e=Q.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function jn(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Mn(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){d(X==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,yt()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=y=>document.getElementById(y).value.trim(),c=yt();let l,p;c?(l="/api/capture",p={url:jn(t),kind:X==="company"?"company_page":"job_posting",fields:X==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...Q].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):X==="company"?(l="/api/companies",p={website:t,name:o("add-name"),vertical:[...Q].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(l="/api/postings",p={url:jn(t),title:o("add-title"),company:o("add-job-company")});let u;try{u=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)})}catch(y){d(`add failed: ${y.message}`),a();return}if(!u.ok){let y=`HTTP ${u.status}`;try{const w=await u.text();try{y=JSON.parse(w).error||y}catch{y=w.trim()||y}}catch{}if(a(),u.status===409){d(y||"That company is already in the list."),e.focus(),e.select();return}d(`add failed: ${y}`);return}const h=await u.json();if(a(),c&&!h.company_id){d(h.note||"couldn't classify that page");return}if(Ne(),q(),C(),_(),X==="job"){const y=h.posting&&h.posting.title||"job link";d(`tracking: ${y} @ ${h.company_name}${h.posting_updated?" (refreshed)":""}`),O("jobs")}else c?(d(h.note||(h.company_created?`company added: ${h.company_name}`:`${h.company_name} is already in the list`)),fe(h.company_id)):d("company added")}function An(e,t,n){Le=t,qe();const s=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="";const o=document.getElementById("drawer-summary");o.hidden=!0,o.innerHTML="",s.classList.add("open"),ht();const c={yes:0,maybe:0,no:0},l=/^(.+?)\s*→\s*(yes|maybe|no)\s*—\s*([\s\S]*)$/i,p=new EventSource(`/api/jobs/${t}/stream`),u=(h,y)=>{const w=document.createElement("div");let L;if(!y&&(L=h.match(l))){const T=L[2].toLowerCase();c[T]++,w.className="ln ln-verdict";const H=document.createElement("span");H.className="pill pill-"+T,H.textContent=T;const R=document.createElement("span");R.className="lv-text";const E=document.createElement("span");E.className="lv-name",E.textContent=L[1].trim(),R.appendChild(E);const j=(L[3]||"").trim();if(j){const f=document.createElement("span");f.className="lv-reason",f.textContent=j,R.append(" ",f)}w.append(H,R)}else if(!y&&/^(scoring|enriching|ingesting)\b/i.test(h))w.className="ln ln-head",w.textContent=h;else if(!y&&/^·\s/.test(h))w.className="ln ln-pick",w.textContent=h;else{const T=!y&&/^\s*warn:/i.test(h);w.className="ln"+(y?" ln-err":T?" ln-warn":""),w.textContent=T?h.replace(/^\s*warn:\s*/i,"⚠ "):h}a.appendChild(w),a.scrollTop=a.scrollHeight};p.addEventListener("line",h=>u(h.data,/error|failed/i.test(h.data))),p.addEventListener("end",h=>{if(p.close(),Le=null,u(`— ${h.data} —`,h.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",c.yes+c.maybe+c.no>0){for(const w of["yes","maybe","no"]){if(!c[w])continue;const L=document.createElement("span");L.className="pill pill-"+w,L.textContent=`${c[w]} ${w}`,o.appendChild(L)}o.hidden=!1}Bn(),d(`${e} ${h.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?ss(n.company_ids):q(),C(),ht(),_(),i.openId&&fe(i.openId)}),p.onerror=()=>{p.close()}}async function ua(){if(Le)try{await fetch(`/api/jobs/${Le}/cancel`,{method:"POST"})}catch{}}let N=null;const pa={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function gt(e){return e==="application-stages"||e==="outreach-statuses"}function Re(e){if(e==="outreach-template")return"outreach template";if(e==="followup-template")return"follow-up template";if(e==="playbook")return"playbook";if(e==="application-stages")return"application stages";if(e==="outreach-statuses")return"outreach statuses";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(pa[t]||t)+" prompt"}return e+".md"}async function ee(e){N=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Re(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=!!e&&e.startsWith("outreach-prompts/"),s=n?e.slice(17):"",a=n&&s!=="fill";document.getElementById("editor-toggle-row").style.display=a?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",a&&(document.getElementById("editor-toggle-label").textContent="Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const l=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${l||"HTTP "+o.status}`;return}const c=await o.json();gt(e)?(document.getElementById("editor-title").textContent="edit "+Re(e)+" — one per line",document.getElementById("editor-text").value=(c.statuses||[]).join(`
`)):document.getElementById("editor-text").value=c.content||"",a&&(document.getElementById("editor-enabled").checked=c.enabled!==!1),c.taste_version&&(document.getElementById("editor-ver").textContent="version "+c.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function De(){document.getElementById("editor-scrim").classList.remove("open"),N=null}let x=null,vt=[],bt=[];const ma=["location.allowed","verticals.excluded","verticals.allowed"],wt={"verticals.excluded":"pf-vertical-tags","verticals.allowed":"pf-vertical-tags"};function te(e){const[t,n]=e.split(".");return x[t]&&x[t][n]||[]}function Ve(e,t){const[n,s]=e.split(".");(x[n]=x[n]||{})[s]=t}function Ue(e,t){const n=t.toLowerCase();return e.some(s=>String(s).toLowerCase()===n)}function kt(){ma.forEach(e=>{const t=document.querySelector(`.pf-chips[data-field="${e}"]`);if(!t)return;const n=wt[e]?` list="${wt[e]}"`:"",s=wt[e]?"type to search…":"type &amp; Enter…";t.innerHTML=te(e).map((a,o)=>`<span class="pf-chip">${r(a)}<button class="pf-chip-x" data-field="${e}" data-i="${o}" title="remove" aria-label="remove ${r(a)}">×</button></span>`).join("")+`<input class="pf-chip-input" data-field="${e}"${n} type="text" placeholder="${s}" spellcheck="false" autocomplete="off" />`})}function ha(e,t){var s;const n=(t||"").trim();if(n){const a=te(e);Ue(a,n)||Ve(e,[...a,n])}kt(),(s=document.querySelector(`.pf-chip-input[data-field="${e}"]`))==null||s.focus()}function Hn(e,t){const n=te(e).slice();n.splice(t,1),Ve(e,n),kt()}function Pn(){const e=document.getElementById("pf-stages");if(!e)return;const t=te("funding_stage.allowed");e.innerHTML=bt.map(n=>{const s=Ue(t,n.value),a=n.count?` <span class="pf-stage-n">${n.count}</span>`:"";return`<button class="pf-stage${s?" is-on":""}" data-stage="${r(n.value)}">${r(n.value)}${a}</button>`}).join("")}function fa(e){const t=te("funding_stage.allowed");Ve("funding_stage.allowed",Ue(t,e)?t.filter(n=>String(n).toLowerCase()!==e.toLowerCase()):[...t,e]),Pn()}function ya(){const e=document.getElementById("pf-vertical-tags");e&&(e.innerHTML=vt.map(t=>`<option value="${r(t.value)}" label="${t.count}"></option>`).join(""))}function ga(){return{location:{allowed:[],remote_ok:!0},headcount:{min:0,max:0},verticals:{allowed:[],excluded:[]},funding_stage:{allowed:[]}}}async function qn(e=!1){document.getElementById("prefilter-scrim").classList.add("open");try{const[t,n]=await Promise.all([(await fetch("/api/taste-filter"+(e?"?default=1":""))).json(),vt.length||bt.length?Promise.resolve(null):fetch("/api/filter-options").then(s=>s.json()).catch(()=>null)]);n&&(vt=n.verticals||[],bt=n.stages||[]),x=Object.assign(ga(),t.rules||{}),x.location=Object.assign({allowed:[],remote_ok:!0},x.location),x.headcount=Object.assign({min:0,max:0},x.headcount),x.verticals=Object.assign({allowed:[],excluded:[]},x.verticals),x.funding_stage=Object.assign({allowed:[]},x.funding_stage),e||(document.getElementById("pf-enabled").checked=t.enabled!==!1),document.getElementById("pf-remote-ok").checked=!!x.location.remote_ok,document.getElementById("pf-hc-min").value=String(x.headcount.min||0),document.getElementById("pf-hc-max").value=String(x.headcount.max||0),ya(),kt(),Pn()}catch(t){d(`failed to load pre-filter: ${t.message}`)}}function Fe(){document.getElementById("prefilter-scrim").classList.remove("open"),x=null}async function va(){if(!x)return;x.location.remote_ok=document.getElementById("pf-remote-ok").checked,x.headcount.min=Math.max(0,parseInt(document.getElementById("pf-hc-min").value,10)||0),x.headcount.max=Math.max(0,parseInt(document.getElementById("pf-hc-max").value,10)||0),document.querySelectorAll(".pf-chip-input").forEach(n=>{const s=n.value.trim();s&&!Ue(te(n.dataset.field),s)&&Ve(n.dataset.field,[...te(n.dataset.field),s])});const e=document.getElementById("pf-enabled").checked;let t;try{t=await fetch("/api/taste-filter",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({rules:x,enabled:e})})}catch(n){d(`save failed: ${n.message}`);return}if(!t.ok){d(`save failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`);return}d("pre-filter saved"),Fe(),C()}const ba=[{key:"experience",hard:!0},{key:"voice",hard:!1},{key:"logistics",hard:!1}];async function On(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{wa(await(await fetch("/api/outreach/sources")).json())}catch(e){d(`failed to load sources: ${e.message}`)}}function Et(){document.getElementById("sources-scrim").classList.remove("open")}function wa(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):ba,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],c=o.length?o.map(l=>`<li><span class="src-title">${r(l.title||l.page_id)}</span></li>`).join(""):`<li class="dim small">${a.hard?"none yet — add an experience page to your brain":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${r(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join("")}async function ka(){if(!N)return;const e=document.getElementById("editor-text").value;let t;gt(N)?t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)}:(t={content:e},N.startsWith("outreach-prompts/")&&N!=="outreach-prompts/fill"&&(t.enabled=document.getElementById("editor-enabled").checked));let n;try{n=await fetch(`/api/${N}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){d(`save failed: ${o.message}`);return}if(!n.ok){d(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const s=await n.json();s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version);const a=gt(N);N==="followup-template"&&(i.followupTemplate=e),d(`${Re(N)} saved`),De(),a&&M(),C()}async function Ea(){if(!N||!N.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${N}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){d(`reset failed: ${n.message}`);return}if(!e.ok){d(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",d(`${Re(N)} reset to default`)}function Nn(e,t,n){e.k!==t?(e.k=t,e.dir=1):e.dir===1?e.dir=-1:Object.assign(e,n)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{Nn(i.sort,e.dataset.k,v),z()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{Nn(i.jsort,e.dataset.jk,$),A()}}),document.getElementById("tab-companies").onclick=()=>O("companies"),document.getElementById("tab-jobs").onclick=()=>O("jobs"),document.getElementById("q").oninput=z,document.getElementById("fdrop-cfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.dataset.toggle==="flagged")ie=!ie,G(t,ie);else if(t.dataset.toggle==="enriched")ce=!ce,G(t,ce);else if(t.hasAttribute("data-v")){const n=t.getAttribute("data-v");V.has(n)?V.delete(n):V.add(n),G(t,V.has(n))}else return;z()}}),document.getElementById("fdrop-columns-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item[data-col]");if(!t)return;const n=Qe(),s=t.getAttribute("data-col");n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),G(t,!n.hidden.has(s)),re(),Dt()}),document.getElementById("jq").oninput=A;for(const e of["fdrop-cfilters","fdrop-columns","fdrop-jfilters"]){const t=document.getElementById(e);t.querySelector(".fdrop-btn").addEventListener("click",s=>{s.stopPropagation();const a=t.classList.contains("is-open");tt(),a||os(t)}),t.querySelector(".fdrop-menu").addEventListener("click",s=>s.stopPropagation())}document.addEventListener("click",tt),document.getElementById("fdrop-jfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.hasAttribute("data-stage")){const n=t.getAttribute("data-stage");B.has(n)?B.delete(n):B.add(n),G(t,B.has(n))}else if(t.dataset.toggle==="nextup")le=!le,G(t,le);else if(t.dataset.toggle==="notreached")de=!de,G(t,de);else if(t.hasAttribute("data-status")){const n=t.getAttribute("data-status");Y.has(n)?Y.delete(n):Y.add(n),G(t,Y.has(n))}else return;A()}}),Qn(),Rt(),re(),document.getElementById("pane-close").onclick=xe,document.getElementById("scrim").onclick=xe,document.getElementById("pursuit-close").onclick=we,document.getElementById("pursuit-scrim").onclick=we,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.querySelector(".fdrop.is-open")){tt();return}if(document.getElementById("chat-pane").classList.contains("open")){At();return}if(ja()){Ct();return}if(document.getElementById("profile-scrim").classList.contains("open")){Tt();return}if(document.getElementById("add-scrim").classList.contains("open")){Ne();return}if(document.getElementById("run-scrim").classList.contains("open")){ze();return}if(document.getElementById("help-scrim").classList.contains("open")){Je();return}if(document.getElementById("relink-scrim").classList.contains("open")){pe();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){Me();return}if(document.getElementById("deljob-scrim").classList.contains("open")){Ae();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(nt==="pursuit"&&n){we();return}if(nt==="company"&&t){xe();return}if(t){xe();return}we();return}if(document.getElementById("key-scrim").classList.contains("open")){Lt();return}if(document.getElementById("sources-scrim").classList.contains("open")){Et();return}if(document.getElementById("prefilter-scrim").classList.contains("open")){Fe();return}if(document.getElementById("editor-scrim").classList.contains("open")){De();return}if(document.getElementById("settings-scrim").classList.contains("open")){St();return}});let $t=null;const $a={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Rn(e){if(i.meta&&i.meta.control===!1){d("control surface disabled");return}$t=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=$a[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function ze(){document.getElementById("run-scrim").classList.remove("open"),$t=null}document.getElementById("btn-enrich").onclick=()=>Rn("enrich"),document.getElementById("btn-verdict").onclick=()=>Rn("verdict"),document.getElementById("run-cancel").onclick=ze,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&ze()},document.getElementById("run-go").onclick=()=>{const e=$t,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(ze(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),ft(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=da;const Ia={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Dn(e){const t=Ia[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const l=document.createElement("a");l.className="help-link",l.textContent="Learn more →",l.onclick=()=>{Je(),Jn(),Ma(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(l),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Je(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Dn("add"),document.getElementById("help-run").onclick=()=>Dn("run"),document.getElementById("help-close").onclick=Je,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Je()},document.getElementById("add-cancel").onclick=Ne,document.getElementById("add-save").onclick=Mn,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Ne()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Ln(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Tn),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Mn()))}),document.getElementById("add-vertical-filter").addEventListener("input",Cn),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&ra(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=ua,document.getElementById("drawer-close").onclick=_n,(()=>{const e=document.getElementById("drawer");e.addEventListener("mouseenter",qe),e.addEventListener("mouseleave",()=>{!Le&&e.classList.contains("open")&&Bn()})})(),document.getElementById("editor-cancel").onclick=De,document.getElementById("editor-save").onclick=ka,document.getElementById("editor-reset").onclick=Ea,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&De()},document.getElementById("sources-close").onclick=Et,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&Et()},document.getElementById("pf-cancel").onclick=Fe,document.getElementById("pf-save").onclick=va,document.getElementById("pf-reset").onclick=()=>qn(!0),document.getElementById("prefilter-scrim").addEventListener("click",e=>{var a;if(e.target.id==="prefilter-scrim"){Fe();return}const t=e.target.closest(".pf-stage");if(t){fa(t.dataset.stage);return}const n=e.target.closest(".pf-chip-x");if(n){Hn(n.dataset.field,parseInt(n.dataset.i,10));return}const s=e.target.closest(".pf-chips");s&&e.target===s&&((a=s.querySelector(".pf-chip-input"))==null||a.focus())}),document.getElementById("prefilter-scrim").addEventListener("keydown",e=>{const t=e.target.closest(".pf-chip-input");if(t){if(e.key==="Enter"||e.key===",")e.preventDefault(),ha(t.dataset.field,t.value);else if(e.key==="Backspace"&&!t.value){const n=te(t.dataset.field);n.length&&Hn(t.dataset.field,n.length-1)}}}),document.getElementById("key-cancel").onclick=Lt,document.getElementById("key-save").onclick=zn,document.getElementById("key-remove").onclick=Ta,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&Lt()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),zn())}),document.getElementById("delcompany-cancel").onclick=Me,document.getElementById("delcompany-confirm").onclick=ws,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&Me()},document.getElementById("deljob-cancel").onclick=Ae,document.getElementById("deljob-confirm").onclick=Es,document.getElementById("deljob-scrim").onclick=e=>{e.target.id==="deljob-scrim"&&Ae()},document.getElementById("relink-cancel").onclick=pe,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&pe()},document.getElementById("relink-search").addEventListener("input",e=>{rn(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&ln(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&ln(t.dataset.id)});function It(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function xt(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}oe()}const W='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',Ke='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',xa='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',Vn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',_a='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',Un='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',_t='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',Ba='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>';function F(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${r(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"",s=e.act?`<button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>`:"";return`<div class="settings-item">
    ...
    ${s}
  </div>`}function oe(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,s=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let k="off",P="";const Te=t&&t.criteria_state;Te==="current"?(k="ok",P="current · verified "+It(t.verified_age_seconds)):Te==="changed"?(k="warn",P="changed — re-distill"):Te==="unverified"?(k="warn",P=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(k="warn",P="brain offline · using cache"):a&&(k="ok",P="fetched "+It(t.age_seconds)),o=F({icon:Vn,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:k,note:P,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:Ke,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=F({icon:Vn,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:W,actTitle:"edit taste.md",actLabel:"edit taste"});const c=F({icon:_a,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:W,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),l=F({icon:Un,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:W,actTitle:"edit the outreach email template",actLabel:"edit email template"}),p=F({icon:Un,nameHTML:'<span class="edit-link" data-act="edit-followup-template" title="edit the follow-up template">follow-up template</span>',desc:"Copy-paste follow-up — variables {{contact_name}}, {{role}}, {{company}}, {{last_sent}}, {{last_message}}.",act:"edit-followup-template",actIcon:W,actTitle:"edit the follow-up template",actLabel:"edit follow-up template"}),u=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],h=u.map(([k,P,Te])=>F({icon:_t,nameHTML:`<span class="edit-link" data-act="edit-prompt-${k}" title="edit the ${P.replace(/^\d+ · /,"")} prompt">${P}</span>`,desc:Te,act:`edit-prompt-${k}`,actIcon:W,actTitle:`edit the ${P} prompt`,actLabel:`edit ${P} prompt`})).join(""),y=!i.stats||i.stats.taste_filter_enabled!==!1,w=F({icon:Ba,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate that narrows bulk verdict runs before the paid LLM — location, headcount, vertical, stage. Re-scoring one company by hand ignores it.",dot:y?"ok":"off",note:y?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:W,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),L=i.anthropicKey;let T="off",H="not set — verdict, capture & outreach disabled";L&&L.key_source==="db"?(T="ok",H="set here · active"):L&&L.key_source==="env"&&(T="ok",H="from the environment");const R=F({icon:xa,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:T,note:H,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:W,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),E=F({icon:_t,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:W,actTitle:"edit the application stages",actLabel:"edit application stages"}),j=F({icon:_t,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:W,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${R}
     </div>`;const f={"view-profile":()=>Sa(i.profile),"refresh-profile":Ca,"edit-taste":()=>ee("taste"),"edit-taste-filter":()=>qn(),"edit-application-stages":()=>ee("application-stages"),"edit-outreach-statuses":()=>ee("outreach-statuses"),"edit-playbook":()=>ee("playbook"),"edit-template":()=>ee("outreach-template"),"edit-followup-template":()=>ee("followup-template"),"edit-anthropic-key":La};for(const[k]of u)f[`edit-prompt-${k}`]=()=>ee(`outreach-prompts/${k}`);e.querySelectorAll("[data-act]").forEach(k=>{const P=k.dataset.act;P&&f[P]&&(k.onclick=f[P])})}async function Fn(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}oe()}async function La(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await Fn(),Bt();const e=document.getElementById("key-input");e&&e.focus()}function Bt(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function Lt(){document.getElementById("key-scrim").classList.remove("open")}async function zn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){d("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){d(`save failed: ${a.message}`),n();return}if(!s.ok){d((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),d("Anthropic key saved"),await mt(),Bt(),oe()}async function Ta(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){d(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){d((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),d(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await mt(),Bt(),oe()}async function Ca(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){d(`refresh failed: ${n.message}`),xt();return}if(!t.ok){const n=await t.text().catch(()=>"");d(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),xt();return}i.profile=await t.json(),oe(),d("company-fit brief refreshed"),C()}function Sa(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${It(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Tt(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Tt,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Tt()};function Jn(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");We(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ct(){document.getElementById("docs-scrim").classList.remove("open")}function ja(){return document.getElementById("docs-scrim").classList.contains("open")}function We(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Ma(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),We(e)}document.getElementById("open-docs").onclick=Jn,document.getElementById("docs-close").onclick=Ct,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Ct()};function Aa(){document.getElementById("settings-scrim").classList.add("open"),oe()}function St(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Aa,document.getElementById("settings-close").onclick=St,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&St()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),We(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&We(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Ha(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Pa(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ye(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function jt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function qa(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Kn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=Ha(n.content);if(n.role==="user")s&&t.appendChild(Ye("user",s));else if(n.role==="assistant"){const a=Pa(n.content);if(!s&&!a.length)continue;const o=Ye("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(qa()),jt()}async function Mt(e,t,n){if(!i.meta||!i.meta.chat){d("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const l=await c.json();i.chat.threadId=l.thread.id,Kn(l.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function At(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Ge(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Wn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Yn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",Wn(),Ge(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Ye("user",t));const a=Ye("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),jt();let o="";const c=h=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+h,Ge(!1)},l=i.chat.threadId;let p;try{p=await fetch("/api/chat/"+l+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(h){c(h.message);return}if(!p.ok){c((await p.text().catch(()=>"")).trim()||"HTTP "+p.status);return}const u=new EventSource("/api/chat/"+l+"/stream");i.chat.es=u,u.addEventListener("delta",h=>{o+=h.data,a.textContent=o,jt()}),u.addEventListener("end",async h=>{u.close(),i.chat.es===u&&(i.chat.es=null),a.classList.remove("chat-streaming"),Ge(!1),i.chat.threadId===l&&await Oa(),Na(),typeof h.data=="string"&&h.data.indexOf("error")===0&&d("chat: "+h.data)}),u.onerror=()=>{u.close(),i.chat.es===u&&(i.chat.es=null),a.classList.remove("chat-streaming"),Ge(!1)}}async function Oa(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Kn(a.messages||[])}catch{}}function Na(){q(),_(),C(),i.openId&&fe(i.openId)}document.getElementById("open-chat").onclick=()=>Mt("global","",""),document.getElementById("chat-close").onclick=At,document.getElementById("chat-scrim").onclick=At,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Yn()}),document.getElementById("chat-input").addEventListener("input",Wn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Yn())}),Ft("#t tbody",ts),Ft("#jt tbody",ns);const Ra=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();O(Ra==="jobs"?"jobs":"companies",{render:!1});function Gn(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||Gn()}),window.addEventListener("pageshow",e=>{e.persisted&&Gn()}),q(),_(),C(),mt(),ht(),xt(),Fn(),M()}Ua({"":{view:()=>({mount(b){b.innerHTML=Wa,Ya()}}),chrome:!1}},{title:"scout"});Ja();Ka();
