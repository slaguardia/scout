(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))T(r);new MutationObserver(r=>{for(const g of r)if(g.type==="childList")for(const x of g.addedNodes)x.tagName==="LINK"&&x.rel==="modulepreload"&&T(x)}).observe(document,{childList:!0,subtree:!0});function y(r){const g={};return r.integrity&&(g.integrity=r.integrity),r.referrerPolicy&&(g.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?g.credentials="include":r.crossOrigin==="anonymous"?g.credentials="omit":g.credentials="same-origin",g}function T(r){if(r.ep)return;r.ep=!0;const g=y(r);fetch(r.href,g)}})();function aa(b,i){const y=i.replace(/^#/,"");let T=null;for(const r of Object.keys(b))(y===r||y.startsWith(r))&&(T===null||r.length>T.length)&&(T=r);return T===null&&""in b&&(T=""),T}function oa(b){return typeof b=="function"?{view:b,chrome:!0}:{view:b.view,chrome:b.chrome!==!1}}function ia(b,i={}){const y=i.root??document.body,T=i.title??document.title??"",r=i.brandHref??"#",g=document.createElement("main"),x=document.createElement("header");x.className="cap-head";const k=document.createElement("a");k.className="brand",k.href=r,k.textContent=T,k.setAttribute("aria-label",`${T} — home`),x.appendChild(k);const J=document.createElement("nav");J.className="cap-nav",J.setAttribute("aria-label","Views");for(const E of i.nav??[]){const M=document.createElement("a");M.href=E.href,M.textContent=E.label,E.ariaLabel&&M.setAttribute("aria-label",E.ariaLabel),J.appendChild(M)}x.appendChild(J);const P=document.createElement("section");P.className="tk-content",g.appendChild(x),g.appendChild(P);const j=document.createElement("div");j.className="tk-bleed";const Ve=E=>{var M;for(const N of Array.from(J.querySelectorAll("a"))){const z=((M=N.getAttribute("href"))==null?void 0:M.replace(/^#/,""))??"";N.toggleAttribute("aria-current",E!==null&&E!==""&&z===E),N.hasAttribute("aria-current")&&N.setAttribute("aria-current","page")}};let de=0;const ne=()=>{const E=aa(b,location.hash);if(Ve(E),E===null){j.isConnected&&j.remove(),g.isConnected||y.appendChild(g),ra(P,"Not found.");return}const{view:M,chrome:N}=oa(b[E]),z=N?P:j;N?(j.isConnected&&j.remove(),g.isConnected||y.appendChild(g)):(g.isConnected&&g.remove(),j.isConnected||y.appendChild(j)),z.replaceChildren();const F=M(),se=++de,Q=F.mount(z);Q instanceof Promise&&Q.catch(Ue=>{se===de&&ca(z,String(Ue))})};window.addEventListener("hashchange",ne),ne()}function ra(b,i){b.replaceChildren();const y=document.createElement("div");y.className="tk-empty",y.textContent=i,b.appendChild(y)}function ca(b,i){b.replaceChildren();const y=document.createElement("div");y.className="tk-error",y.textContent=i,b.appendChild(y)}function da(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(i=>{for(const y of i)y.unregister()}),window.caches&&caches.keys().then(i=>{for(const y of i)caches.delete(y)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function la(){let b;try{b=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!b.ok)return null;let i;try{i=await b.json()}catch{return null}return typeof i.email=="string"&&i.email?{email:i.email}:null}const ua=`
<div class="layout">
<aside class="sidebar">
...
`;function pa(b){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],openDetail:null,anthropicKey:null},y=e=>"pill pill-"+(e||"none"),T='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),g=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function x(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],A()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],$(),J(),j()}function J(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&ee())}let P=null;function j(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!P?P=setInterval(Ve,4e3):!e&&P&&(clearInterval(P),P=null)}async function Ve(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&($(),J()),j()}async function de(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{})]),xn(),i.view==="jobs"&&$()}function ne(e){i.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Je(),e==="jobs"?$():A()}async function E(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}i.stats=e,M()}function M(){const e=i.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,D()}function N(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function z(e){return e.slice().sort((t,n)=>i.sort.dir*N(t,n,i.sort.k))}const F=new Set;let se=!1,Q=!1;function Ue(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",F.has(e.dataset.v))})}function xt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(F.size&&!F.has(t.verdict||"__none__")||se&&!t.flagged||Q&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const bn=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],wn=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Bt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const _t=Bt("scout-hidden-cols"),Lt=Bt("scout-hidden-jcols");function Tt(){return i.view==="jobs"?{cols:wn,hidden:Lt,key:"scout-hidden-jcols"}:{cols:bn,hidden:_t,key:"scout-hidden-cols"}}function ae(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=_t.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Lt.has(e.dataset.col)?"none":""})}function Je(){const e=Tt();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=Tt(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),Je(),ae()})})}function Ct(e){return`
    ...
    `}function St(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>en(t.dataset.id))}const kn=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],En=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function jt(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let c=0;c<n;c++){const l=t.map(([p,d])=>{const m=d.endsWith("%")?Math.round(parseFloat(d)*a[c%a.length])+"%":d;return`<td${p?` data-col="${p}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${l}</tr>`}s.innerHTML=o,ae()}function A(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=z(xt());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=Ct(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||re(s.dataset.id)}),e.appendChild(s),St(s)}ae()}async function $n(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=z(xt()).map(c=>c.company_id),o=[...s.querySelectorAll("tr")].map(c=>c.dataset.id);if(a.length!==o.length||a.some((c,l)=>c!==o[l])){A();return}for(const c of e){const l=i.rows.find(d=>d.company_id===c),p=s.querySelector(`tr[data-id="${CSS.escape(c)}"]`);if(!l||!p){A();return}p.innerHTML=Ct(l),St(p)}ae()}const S=new Set;let ke=!1,Ee=!1,Z=!0;function In(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=Z&&!S.has("rejected");return i.jobs.filter(n=>{const s=R(n.stage_history);return!(t&&s==="rejected"||S.size&&!S.has(s)||ke&&!n.next_up||Ee&&(n.outreach_count|0)>0||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.description||"")+" "+(n.contacts||"")).toLowerCase().includes(e))})}function xn(){const e=document.getElementById("stage-chips");if(e){for(const t of[...S])i.applicationStages.includes(t)||S.delete(t);e.innerHTML=i.applicationStages.map(t=>`<button class="v-chip${S.has(t)?" is-on":""}" data-stage="${r(t)}">${r(t)}</button>`).join("")}}const Bn=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function Mt(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(Bn),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}function _n(e){const t=(e||[]).map(n=>({position:(n.position||"").trim(),email:(n.email||"").trim()})).filter(n=>n.position||n.email);return t.length?JSON.stringify(t):""}const $e=()=>new Date().toISOString().slice(0,10);function X(e){if(e=String(e||"").trim(),!e)return[];try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({stage:String((n==null?void 0:n.stage)||"").trim(),date:String((n==null?void 0:n.date)||"").trim()})).filter(n=>n.stage)}catch{}return[]}function Ie(e){const t=(e||[]).map(n=>({stage:(n.stage||"").trim(),date:(n.date||"").trim()})).filter(n=>n.stage);return t.sort((n,s)=>(n.date||"9999-99-99").localeCompare(s.date||"9999-99-99")),t.length?JSON.stringify(t):""}function R(e){const t=X(e);return t.length?t[t.length-1].stage:""}function At(e){const t=X(e);return t.length?t[t.length-1].date:""}function Ln(e){const t=[["","none"]];for(const n of i.applicationStages)t.push([n,n]);return e&&!i.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function Ht(e){const t=[["","none"]];for(const n of i.outreachStatuses)t.push([n,n]);return e&&!i.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const Tn=8;function Pt(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%Tn}function ze(e){return Pt(e,i.applicationStages)}function Cn(e){return Pt(e,i.outreachStatuses)}function Sn(e){const t=Mt(e);return t.length?t.map(n=>{const s=r(n.position||n.email);if(!n.email)return s;const a=r(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${r(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function Ot(e){return`<div class="cc-row">
    ...
    </div>`}function jn(e){return`<div class="outreach-contacts" id="contacts-editor">
      <label class="cc-label">contacts</label>
      <div class="cc-list">${Mt(e).map(Ot).join("")}</div>
      <button class="btn cc-add" type="button">+ add contact</button>
    </div>`}function qt(e){const t=R(e.stage_history);if(!t)return i.applicationStages.length+1;const n=i.applicationStages.indexOf(t);return n<0?i.applicationStages.length:n}function Mn(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return qt(e)-qt(t);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function $(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=In().sort((d,m)=>i.jsort.dir*Mn(d,m,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=Z&&!S.has("rejected")?i.jobs.filter(d=>R(d.stage_history)==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=i.jobs.filter(d=>d.next_up).length,o=document.getElementById("next-up-n");o.textContent=a,o.style.display=a?"":"none";const c=i.jobs.filter(d=>!(d.outreach_count|0)&&!(Z&&!S.has("rejected")&&R(d.stage_history)==="rejected")).length,l=document.getElementById("not-reached-n");l.textContent=c,l.style.display=c?"":"none";const p=document.getElementById("jobs-hidden-note");p.style.display=n?"":"none",n&&(p.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{Z=!1,document.getElementById("hide-rejected").classList.remove("is-on"),$()});for(const d of t){const m=R(d.stage_history),f=At(d.stage_history),v=document.createElement("tr");v.dataset.id=d.posting_id;const _=Ln(m).map(([B,U])=>`<option value="${r(B)}"${m===B?" selected":""}>${r(U)}</option>`).join(""),H=d.outreach_status||"",V=Ht(H).map(([B,U])=>`<option value="${r(B)}"${H===B?" selected":""}>${r(U)}</option>`).join("");v.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${d.next_up?" is-on":""}" title="${d.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${d.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${r(d.title||d.company)}</span>${Hn(d.outreach_draft_status)}${d.title?`<div class="small dim">${r(d.company)}</div>`:""}</div></div></td>
      ...
      <td class="small" data-col="outreach"><div class="jt-out"><span class="jt-stepper"><button class="jt-dec" title="undo one outreach"${d.outreach_count?"":" disabled"}>−</button><span class="jt-oc${d.outreach_count?"":" dim"}">${d.outreach_count||0}</span><button class="jt-inc" title="log one outreach (today)">+</button></span><select class="jt-ostatus ${Cn(H)}" title="outreach reply status">${V}</select></div></td>
      <td class="small" data-col="last_outreach">${d.last_outreach_at?r(d.last_outreach_at):'<span class="dim">—</span>'}</td>
      <td class="small td-contacts" data-col="contacts">${Sn(d.contacts)}</td>
      <td data-col="link"><a href="${g(d.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `,v.querySelector(".jt-nextup").onclick=()=>Ft(d,!1),v.querySelector(".jt-stage-sel").onchange=B=>An(d,B.target.value),v.querySelector(".jt-ostatus").onchange=B=>_e(d,{outreach_status:B.target.value}),v.querySelector(".jt-inc").onclick=()=>_e(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:$e()}),v.querySelector(".jt-dec").onclick=()=>{const B=Math.max(0,(d.outreach_count||0)-1);_e(d,{outreach_count:B,...B===0?{last_outreach_at:""}:{}})},e.appendChild(v)}ae(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",m=>{m.target.closest("a, button, select")||Nt(d.dataset.id)})})}function An(e,t){if(t=(t||"").trim(),!t||t===R(e.stage_history)){$();return}const n=X(e.stage_history);n.push({stage:t,date:$e()}),_e(e,{stage_history:Ie(n)})}function Hn(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function Nt(e){let t=i.jobs.find(n=>n.posting_id===e);if(t||(await k(),t=i.jobs.find(n=>n.posting_id===e)),!t){u("posting not found — refresh");return}Ke(),Xe(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),Rt("pursuit"),ee(),ue(),me()}let Fe=null;function Rt(e){Fe=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function le(){Ke(),Xe(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function Ke(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function ue(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){Le();return}e=await n.json()}catch{Le();return}h.drafts=e.drafts||[],Le();const t=h.drafts[0];t&&t.status==="researching"?Pn():Ke()}function Pn(){h.poll||(h.poll=setInterval(ue,4e3))}function O(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),u(`save failed: ${o.message}`)}})}async function Dt(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),$(),ce(e.posting_id,{title:o.title,location:o.location})}async function On(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",g(e.url)),ce(e.posting_id,{url:s.url})}async function qn(e,t){if(t.disabled)return;const n=t.textContent;t.disabled=!0,t.textContent="re-enriching…";let s;try{s=await fetch(`/api/postings/${e.posting_id}/recapture`,{method:"POST"})}catch(o){t.disabled=!1,t.textContent=n,u(`re-enrich failed: ${o.message}`);return}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();let c=o||"HTTP "+s.status;try{c=JSON.parse(o).error||c}catch{}t.disabled=!1,t.textContent=n,u(`re-enrich failed: ${c}`);return}const a=await s.json();Object.assign(e,{title:a.title,location:a.location,employment_type:a.employment_type,workplace_type:a.workplace_type,department:a.department,comp_range:a.comp_range,description:a.description,posted_at:a.posted_at,url:a.url,questions_status:a.questions_status}),$(),ee(),ce(e.posting_id,{title:a.title,location:a.location,url:a.url}),u("re-enriched from the posting link")}function Nn(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>Dn(e))}async function Rn(e,t){const n=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.company_id=s.company_id,e.company=s.company_name,ee(),k()}let pe=null;function Dn(e){pe=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const n=document.getElementById("relink-search");n&&(n.value=""),Vt(""),document.getElementById("relink-scrim").classList.add("open"),n&&n.focus()}function oe(){document.getElementById("relink-scrim").classList.remove("open"),pe=null}let We=null;function Vn(e){We=e;const t=(e.postings||[]).length,n=t?` and its ${t} job ${t===1?"posting":"postings"}`:"",s=document.getElementById("delcompany-summary");s&&(s.innerHTML=`Delete <strong>${r(e.name||"this company")}</strong>${n}?`);const a=document.getElementById("delcompany-confirm");a&&(a.disabled=!1),document.getElementById("delcompany-scrim").classList.add("open")}function xe(){document.getElementById("delcompany-scrim").classList.remove("open"),We=null}async function Un(){const e=We;if(!e)return;const t=document.getElementById("delcompany-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e.company_id}`,{method:"DELETE"})}catch(a){u(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");u(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=e.name||"company";xe(),i.openId===e.company_id&&ge(),x(),k(),E(),u(`deleted ${s}`)}let Ye=null;function Jn(e){Ye=e;const t=(e.title||"").trim()||"this posting",n=e.company?` at <strong>${r(e.company)}</strong>`:"",s=document.getElementById("deljob-summary");s&&(s.innerHTML=`Delete <strong>${r(t)}</strong>${n}?`);const a=document.getElementById("deljob-confirm");a&&(a.disabled=!1),document.getElementById("deljob-scrim").classList.add("open")}function Be(){document.getElementById("deljob-scrim").classList.remove("open"),Ye=null}async function zn(){const e=Ye;if(!e)return;const t=document.getElementById("deljob-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${e.posting_id}`,{method:"DELETE"})}catch(a){u(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");u(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=(e.title||"").trim()||"posting";Be(),le(),k(),i.openId===e.company_id&&re(e.company_id),u(`deleted ${s}`)}function Vt(e){const t=document.getElementById("relink-results");if(!t)return;const n=e.trim().toLowerCase();let s=(i.rows||[]).slice();if(n?(s=s.filter(o=>(o.name||"").toLowerCase().includes(n)),s.sort((o,c)=>{const l=(o.name||"").toLowerCase().startsWith(n)?0:1,p=(c.name||"").toLowerCase().startsWith(n)?0:1;return l-p||(o.name||"").localeCompare(c.name||"")})):s.sort((o,c)=>(o.name||"").localeCompare(c.name||"")),s=s.slice(0,60),!s.length){t.innerHTML=`<div class="relink-empty">${(i.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const a=pe?pe.company_id:"";t.innerHTML=s.map(o=>{const c=o.company_id===a,l=[o.vertical,o.location].filter(Boolean).map(r).join(" · ");return`<button type="button" class="relink-result${c?" is-current":""}"
      ...
      </button>`}).join("")}async function Ut(e){const t=pe;if(!t){oe();return}if(e===t.company_id){oe();return}try{await Rn(t,e),oe(),u(`moved to ${t.company}`)}catch(n){u(`move failed: ${n.message}`)}}async function Jt(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),x(),k()}async function Fn(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();i.openId=s.company_id,ye(s),ve(s.company_id),x(),k()}async function Kn(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}let zt=null;function ee(){const e=h.row;if(!e)return;const t=document.getElementById("pursuit-body"),s=!!t&&zt===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${r(e.title||"")}">`;const a=R(e.stage_history);document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a?ze(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`+(e.verdict?` <span class="${y(e.verdict)}">${r(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>kt("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    <section class="pane-section role-head">
      <div id="role-body">${Wn(e)}</div>
    </section>
    ...
    <section class="pane-section">
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row pipeline-stage-row">
          ...
          <div class="pl-stage-wrap">${Yn(e)}</div>
        </div>
        <div class="pipeline-row">
          ...
          <select class="input pl-ostatus" title="outreach reply status — separate from the application stage">
            ${Ht(e.outreach_status||"").map(([d,m])=>`<option value="${r(d)}"${(e.outreach_status||"")===d?" selected":""}>${r(m)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
  ...
  `,Gn();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>re(e.company_id)),Nn(e),O(document.getElementById("pursuit-title-input"),d=>Dt(e,"title",d)),O(document.getElementById("pursuit-url-input"),d=>On(e,d));const l=document.getElementById("pursuit-reenrich");l&&l.addEventListener("click",()=>qn(e,l)),O(document.getElementById("pursuit-notes-input"),d=>Qn(d),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(d=>O(d,m=>Dt(e,d.dataset.k,m),{multiline:d.tagName==="TEXTAREA"}));const p=document.getElementById("job-delete-btn");p&&p.addEventListener("click",()=>Jn(e)),Le(),fe(),t&&(t.scrollTop=s),zt=e.posting_id}function Wn(e){return`
    ...
    </div>`}function Yn(e){const t=X(e.stage_history),n=t.map((a,o)=>`
    ...
    </div>`}function Gn(){const e=h.row;document.querySelectorAll("#pursuit-body .stage-entry .stage-date").forEach(a=>{a.addEventListener("change",o=>{const c=+o.target.closest(".stage-entry").dataset.i,l=X(e.stage_history);l[c]&&(l[c].date=o.target.value,ie({stage_history:Ie(l)}))})}),document.querySelectorAll("#pursuit-body .stage-entry .stage-del").forEach(a=>{a.addEventListener("click",o=>{const c=+o.target.closest(".stage-entry").dataset.i,l=X(e.stage_history);l.splice(c,1),ie({stage_history:Ie(l)})})});const t=document.querySelector("#pursuit-body .stage-add-btn");t&&t.addEventListener("click",()=>{const a=document.querySelector("#pursuit-body .stage-add-sel"),o=document.querySelector("#pursuit-body .stage-add-date"),c=(a.value||"").trim();if(!c)return;const l=X(e.stage_history);l.push({stage:c,date:o.value||$e()}),ie({stage_history:Ie(l)})});const n=document.querySelector("#pursuit-body .pl-ostatus");n&&n.addEventListener("change",a=>ie({outreach_status:a.target.value}));const s=document.querySelector("#pursuit-body .pt-nextup");s&&s.addEventListener("click",()=>Ft(h.row,!0))}async function Ft(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){u(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();u(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,$(),ce(e.posting_id,{next_up:s.next_up}),t&&ee(),u(e.next_up?"queued next up":"removed from the queue")}async function Ge(e,t){const n={stage_history:e.stage_history||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",outreach_status:e.outreach_status||"",contacts:e.contacts||"",notes:e.notes||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return u(`save failed: ${o.message}`),null}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();return u(`save failed: ${o||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{stage_history:a.stage_history,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,outreach_status:a.outreach_status,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),ce(e.posting_id,{stage_history:a.stage_history,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function Qn(e){const t=h.row,n={stage_history:t.stage_history||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",outreach_status:t.outreach_status||"",contacts:t.contacts||"",notes:e},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,$()}async function ie(e){await Ge(h.row,e)&&($(),ee(),u("tracking saved"))}async function Kt(e){await Ge(h.row,{contacts:e})&&$()}async function _e(e,t){await Ge(e,t)&&($(),h.postingId===e.posting_id&&(h.row=e,ee()),u("tracking saved"))}function Le(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.row,n=h.drafts,s=n[0]||null,a=n.slice(1),o=`
    ...
    ${jn(t.contacts)}`,l=s&&(Zn(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,p=a.length?`
    <details class="draft-history" ${h.openHist?"open":""}>
      <summary>${a.length} earlier draft${a.length>1?"s":""}</summary>
      <div id="draft-history-body">${a.map(d=>Yt(d,!0)).join("")}</div>
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?Yt(s,!1):""}</div><div class="draft-actions">${l}</div>`+p,as()}function Zn(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const Xn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',es='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',Wt=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${Xn}</button>`,Qe=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],ts='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function ns(e){let t=Qe.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${Qe.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",c=a<t?ts:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${Qe[t].active}…</span></div>
  </div>`}function Yt(e,t){const n=(d,m,f="")=>`
    ...
    </div>`;if(e.status==="failed"){const d=ss(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    <div class="draft-head">${o}${s?Wt:""}</div>
    ${l}
    ...
    ${he(e)}
  </div>`}function Ze(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function he(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(p,d)=>d?`<div class="tr-line"><span class="tr-key">${p}:</span> ${r(String(d))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],l=c.map(p=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function Gt(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${r(n.message||"")}"><code>${r(n.code||"")}</code>${r(n.message||"")}</span>`).join("")+"</div>":""}function ss(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${r(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${r(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function as(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#contacts-editor");if(t){const p=t.querySelector(".cc-list"),d=()=>_n(Array.from(p.querySelectorAll(".cc-row")).map(f=>({position:f.querySelector(".cc-pos").value,email:f.querySelector(".cc-email").value}))),m=f=>{f.querySelectorAll(".cc-pos, .cc-email").forEach(v=>{v.addEventListener("change",()=>Kt(d())),v.addEventListener("keydown",_=>{_.key==="Enter"&&(_.preventDefault(),_.target.blur())})}),f.querySelector(".cc-del").addEventListener("click",()=>{f.remove(),Kt(d())})};p.querySelectorAll(".cc-row").forEach(m),t.querySelector(".cc-add").addEventListener("click",()=>{const f=document.createElement("div");f.innerHTML=Ot({position:"",email:""});const v=f.firstElementChild;p.appendChild(v),m(v),v.querySelector(".cc-pos").focus()})}const n=()=>new Date().toISOString().slice(0,10),s=h.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>ie({outreach_count:(s.outreach_count||0)+1,last_outreach_at:n()}));const o=e.querySelector(".pt-outreach-dec");o&&o.addEventListener("click",()=>{const p=Math.max(0,(s.outreach_count||0)-1);ie({outreach_count:p,...p===0?{last_outreach_at:""}:{}})});const c=e.querySelector("#draft-start-btn");c&&c.addEventListener("click",()=>Te()),e.querySelectorAll(".draft-retry-btn").forEach(p=>p.addEventListener("click",()=>Te())),e.querySelectorAll(".draft-regen-btn").forEach(p=>p.addEventListener("click",()=>Te(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(p=>{const d=p.dataset.did,m=p.querySelector(".draft-textarea");m&&O(m,_=>is(d,_),{multiline:!0});const f=p.querySelector(".draft-sent-btn");f&&f.addEventListener("click",()=>rs(d));const v=p.querySelector(".draft-copy-btn");v&&v.addEventListener("click",()=>{const _=p.querySelector(".draft-textarea"),H=p.querySelector(".draft-sentbody"),V=_?_.value:H?H.textContent:"";Is(V,"email copied")})});const l=e.querySelector("details.draft-history");l&&l.addEventListener("toggle",()=>{h.openHist=l.open})}async function Te(e=!1){const t=document.getElementById("outreach-section"),n=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));n&&(n.disabled=!0);let s;try{const o=e?"?regenerate=1":"";s=await fetch(`/api/postings/${h.postingId}/outreach${o}`,{method:"POST"})}catch(o){u(`draft failed: ${o.message}`),n&&(n.disabled=!1);return}if(s.status===202){let o={};try{o=await s.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&u(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await ue(),k();return}if(s.status===409){await ue(),u("a draft is already active");return}if(s.status===412){let o={};try{o=await s.json()}catch{}os(o.need,o.error),n&&(n.disabled=!1);return}if(s.status===503){const o=document.getElementById("outreach-section");if(o){const c=document.createElement("div");c.className="draft-note",c.textContent="Outreach engine not running in this build.",o.appendChild(c)}n&&(n.disabled=!1);return}const a=(await s.text().catch(()=>"")).trim();u(`draft failed: ${a||"HTTP "+s.status}`),n&&(n.disabled=!1)}function os(e,t){const n=document.getElementById("outreach-section");if(!n)return;const s=n.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"Discover sources",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,s?s.replaceWith(c):n.appendChild(c);const l=c.querySelector("#gate-fix-btn");l&&l.addEventListener("click",()=>a?Y("outreach-template"):rt());const p=c.querySelector("#gate-retry-btn");p&&p.addEventListener("click",Te)}async function is(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.drafts.findIndex(l=>String(l.id)===String(e));a>=0&&(h.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const l=c.querySelector(".lint-chips"),p=Gt(s.lint);l?l.outerHTML=p||"":p&&o.insertAdjacentHTML("afterend",p)}}async function rs(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){u(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`failed: ${s||"HTTP "+t.status}`);return}u("marked sent"),await ue(),await k();const n=i.jobs.find(s=>s.posting_id===h.postingId);n&&ce(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function me(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){fe();return}e=await t.json()}catch{fe();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",fe(),h.answers.some(t=>t.status==="generating")?cs():Xe()}function cs(){h.answersPoll||(h.answersPoll=setInterval(me,4e3))}function Xe(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function fe(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(d=>d.status==="generating"),a=t.length?`<div class="answers-list">${t.map(us).join("")}</div>`:"",o=!!h.detecting,c=s||o?" disabled":"",l=d=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":d}</button>`;let p;n==="ok"&&t.length?p=`<button class="btn ${t.some(m=>!Qt(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect"):n===""||n==="unreachable"?p=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect questions"):p=l("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r(ds(n,t.length))}</div>`+a+`<div class="answers-actions">${p}</div>`,hs()}function ds(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function Qt(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function ls(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function us(e){const t=Qt(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${r(ps(e.fail_reason))}</div>`:""}
  </div>`}function ps(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function hs(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",Zt);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",fs),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(O(o,l=>gs(a,l),{multiline:!0}),o.addEventListener("input",()=>ms(s,o)));const c=s.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>ys(a))})}function ms(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function Zt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){u(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await me();return}if(n.status===412){let a={};try{a=await n.json()}catch{}vs(a.error),t&&(t.disabled=!1);return}if(n.status===503){Xt("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();u(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function fs(){h.detecting=!0,fe();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();u(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){u(`detect failed: ${e.message}`)}h.detecting=!1,await me()}async function gs(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=s)}async function ys(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){u(`regenerate failed: ${n.message}`);return}if(t.status===503){Xt("Answer generation isn't running in this build.");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`regenerate failed: ${n||"HTTP "+t.status}`);return}await me()}function vs(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",rt);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",Zt)}function Xt(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function re(e){var l,p;const t=document.getElementById("pane"),n=document.getElementById("scrim"),s=i.openId===e&&t.classList.contains("open"),a=s?((l=document.getElementById("pane-body"))==null?void 0:l.scrollTop)??0:0,o=s?(p=document.getElementById("trace-body"))==null?void 0:p.innerHTML:null;i.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),Rt("company"),s||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let c;try{const d=await fetch(`/api/companies/${e}`);if(!d.ok)throw new Error(`HTTP ${d.status}`);c=await d.json()}catch(d){s||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${r(d.message)}</div>`);return}if(i.openId===e){if(ye(c),s){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const d=document.getElementById("pane-body");d&&(d.scrollTop=a)}ve(e)}}function ge(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function ye(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${r(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `;const v=document.getElementById("posting-add-btn");v&&v.addEventListener("click",()=>Es(e)),tt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(w=>{w.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(te=>te.classList.remove("is-on")),w.classList.add("is-on")})});const _=document.getElementById("ve-save-btn");_&&_.addEventListener("click",()=>ks(e)),O(document.getElementById("pane-title-input"),w=>Jt(e,"name",w)),document.querySelectorAll("#facts-body [data-k]").forEach(w=>O(w,te=>Jt(e,w.dataset.k,te))),O(document.getElementById("pane-domain-input"),w=>Fn(e,w)),O(document.getElementById("pane-notes-input"),w=>Kn(e,w),{multiline:!0});const H=document.getElementById("flag-toggle-btn");H&&H.addEventListener("click",()=>en(e.company_id));const V=document.getElementById("review-stamp-btn");V&&V.addEventListener("click",()=>ws(e.company_id));const B=document.getElementById("rescore-btn");B&&B.addEventListener("click",()=>at("verdict",{company_ids:[e.company_id]}));const U=document.getElementById("reenrich-btn");U&&U.addEventListener("click",()=>at("enrich",{company_ids:[e.company_id]}));const De=document.getElementById("company-delete-btn");De&&De.addEventListener("click",()=>Vn(e))}function bs(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function ws(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){u(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");u(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=i.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,A()),i.openId===e&&(ye(s),ve(e)),u("reviewed")}async function en(e){const t=i.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){u(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");u(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,A()),i.openId===e&&(ye(a),ve(e)),k(),u(a.flagged?"flagged":"unflagged")}async function ks(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){u("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(l){u(`save failed: ${l.message}`),a.disabled=!1;return}if(!o.ok){const l=await o.text().catch(()=>"");u(`save failed: HTTP ${o.status}${l?" — "+l:""}`),a.disabled=!1;return}const c=await o.json();ye(c),ve(c.company_id),x(),E(),k(),u("verdict saved")}function et(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(r).join(" · "),a=R(n.stage_history),o=At(n.stage_history),c=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?ze(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`,`<span class="pt-meta">${a?o?r(o):"tracked":"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${r(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ce(e,t){const n=i.openDetail;if(!n||!i.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=et(n),tt())}function tt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||Nt(e.dataset.pid)})})}async function Es(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){u("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(p){u(`add failed: ${p.message}`),s.disabled=!1;return}if(!o.ok){const p=await o.text().catch(()=>"");u(`add failed: HTTP ${o.status}${p?" — "+p:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(p=>p.id!==c.id),e.postings.unshift(c);const l=document.getElementById("postings-list");l&&(l.innerHTML=et(e),tt()),t.value="",n.value="",s.disabled=!1,k(),u("link added")}async function ve(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){Ce(`<div class="muted">Failed to load trail: ${r(s.message)}</div>`);return}if(!t.ok){Ce(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Ce('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Ce(n.map($s).join(""))}function $s(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(r);return e.run_id&&t.push("run "+r(e.run_id.slice(0,8))),`
    ...
    </div>`}function Ce(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let tn;function u(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(tn),tn=setTimeout(()=>t.classList.remove("show"),2200)}async function Is(e,t="copied"){if(!e){u("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}u(t)}catch(n){u(`copy failed: ${n.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function nt(){try{const s=await fetch("/api/meta");if(!s.ok)return;i.meta=await s.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=i.meta.chat?"":"none")}async function st(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Se=null;async function at(e,t){if(i.meta&&i.meta.control===!1){u("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){u(`run failed: ${a.message}`);return}if(n.status===409){u("a job is already running");return}if(n.status===412){const a=await n.text();u(a.trim());return}if(!n.ok){u(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();dn(e,s,t)}async function xs(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){u(`upload failed: ${a.message}`);return}if(n.status===409){u("a job is already running");return}if(!n.ok){u(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();dn("ingest",s)}const Bs=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let je=[],K=new Set,W="company";function nn(e){W=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",sn()}function ot(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function sn(){const e=document.getElementById("add-note");ot()?e.innerHTML=W==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and description — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=W==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function _s(){Bs.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",K=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),nn(i.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(s=>`<option value="${r(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),je=s.verticals||[]}catch{je=[]}an()}function Me(){document.getElementById("add-scrim").classList.remove("open")}function an(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=je.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${K.has(s)?" sel":""}" data-v="${r(s)}">${r(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;K.has(a)?K.delete(a):K.add(a),s.classList.toggle("sel"),on()}))):e.innerHTML=`<div class="none">${je.length?"no match":"no verticals in the set yet"}</div>`,on()}function on(){const e=K.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function rn(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function cn(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){u(W==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,ot()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=f=>document.getElementById(f).value.trim(),c=ot();let l,p;c?(l="/api/capture",p={url:rn(t),kind:W==="company"?"company_page":"job_posting",fields:W==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...K].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):W==="company"?(l="/api/companies",p={website:t,name:o("add-name"),vertical:[...K].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(l="/api/postings",p={url:rn(t),title:o("add-title"),company:o("add-job-company")});let d;try{d=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)})}catch(f){u(`add failed: ${f.message}`),a();return}if(!d.ok){let f=`HTTP ${d.status}`;try{const v=await d.text();try{f=JSON.parse(v).error||f}catch{f=v.trim()||f}}catch{}if(a(),d.status===409){u(f||"That company is already in the list."),e.focus(),e.select();return}u(`add failed: ${f}`);return}const m=await d.json();if(a(),c&&!m.company_id){u(m.note||"couldn't classify that page");return}if(Me(),x(),E(),k(),W==="job"){const f=m.posting&&m.posting.title||"job link";u(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),ne("jobs")}else c?(u(m.note||(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`)),re(m.company_id)):u("company added")}function dn(e,t,n){Se=t;const s=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="",s.classList.add("open"),st();const o=new EventSource(`/api/jobs/${t}/stream`),c=(l,p)=>{const d=document.createElement("div"),m=!p&&/^\s*warn:/i.test(l);d.className="ln"+(p?" ln-err":m?" ln-warn":""),d.textContent=m?l.replace(/^\s*warn:\s*/i,"⚠ "):l,a.appendChild(d),a.scrollTop=a.scrollHeight};o.addEventListener("line",l=>c(l.data,/error|failed/i.test(l.data))),o.addEventListener("end",l=>{o.close(),Se=null,c(`— ${l.data} —`,l.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",u(`${e} ${l.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?$n(n.company_ids):x(),E(),st(),k(),i.openId&&re(i.openId)}),o.onerror=()=>{o.close()}}async function Ls(){if(Se)try{await fetch(`/api/jobs/${Se}/cancel`,{method:"POST"})}catch{}}let C=null;const Ts={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function it(e){return e==="application-stages"||e==="outreach-statuses"}function Ae(e){if(e==="outreach-template")return"outreach template";if(e==="taste-filter")return"pre-filter rules";if(e==="playbook")return"playbook";if(e==="application-stages")return"application stages";if(e==="outreach-statuses")return"outreach statuses";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(Ts[t]||t)+" prompt"}return e+".md"}async function Y(e){C=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Ae(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=!!e&&e.startsWith("outreach-prompts/"),s=n?e.slice(17):"",a=e==="taste-filter"||n&&s!=="fill";document.getElementById("editor-toggle-row").style.display=a?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",a&&(document.getElementById("editor-toggle-label").textContent=e==="taste-filter"?"Enable the pre-filter (off → bulk verdict runs score every company; the rules below are kept either way)":"Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const l=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${l||"HTTP "+o.status}`;return}const c=await o.json();it(e)?(document.getElementById("editor-title").textContent="edit "+Ae(e)+" — one per line",document.getElementById("editor-text").value=(c.statuses||[]).join(`
`)):document.getElementById("editor-text").value=c.content||"",a&&(document.getElementById("editor-enabled").checked=c.enabled!==!1),c.taste_version&&(document.getElementById("editor-ver").textContent="version "+c.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function He(){document.getElementById("editor-scrim").classList.remove("open"),C=null}const Cs=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function rt(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{dt(await(await fetch("/api/outreach/sources")).json())}catch(e){u(`failed to load sources: ${e.message}`)}}function ct(){document.getElementById("sources-scrim").classList.remove("open")}function dt(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):Cs,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],c=o.length?o.map(l=>`<li><span class="src-title">${r(l.title||l.page_id)}</span><button class="src-rm" data-need="${r(a.key)}" data-id="${r(l.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${r(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>js(a.dataset.need,a.dataset.id)))}async function Ss(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){u(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const n=await t.json();n.warning?u(n.warning):u("sources refreshed"),dt(n),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function js(e,t){let n;try{n=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(s){u(`remove failed: ${s.message}`);return}if(!n.ok){u(`remove failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}dt(await n.json())}async function Ms(){if(!C)return;const e=document.getElementById("editor-text").value;let t;if(it(C))t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)};else{t={content:e};const o=C.startsWith("outreach-prompts/")&&C!=="outreach-prompts/fill";(C==="taste-filter"||o)&&(t.enabled=document.getElementById("editor-enabled").checked)}let n;try{n=await fetch(`/api/${C}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){u(`save failed: ${o.message}`);return}if(!n.ok){u(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const s=await n.json();s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version);const a=it(C);u(`${Ae(C)} saved`),He(),a&&de(),E()}async function As(){if(!C||!C.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${C}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){u(`reset failed: ${n.message}`);return}if(!e.ok){u(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",u(`${Ae(C)} reset to default`)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;i.sort.k===t?i.sort.dir*=-1:(i.sort.k=t,i.sort.dir=1),A()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;i.jsort.k===t?i.jsort.dir*=-1:(i.jsort.k=t,i.jsort.dir=1),$()}}),document.getElementById("tab-companies").onclick=()=>ne("companies"),document.getElementById("tab-jobs").onclick=()=>ne("jobs"),document.getElementById("q").oninput=A,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;F.has(t)?F.delete(t):F.add(t),Ue(),A()})}),document.getElementById("flag-filter").addEventListener("click",e=>{se=!se,e.currentTarget.classList.toggle("is-on",se),A()}),document.getElementById("enriched-filter").addEventListener("click",e=>{Q=!Q,e.currentTarget.classList.toggle("is-on",Q),A()}),document.getElementById("jq").oninput=$,document.getElementById("hide-rejected").addEventListener("click",e=>{Z=!Z,e.currentTarget.classList.toggle("is-on",Z),$()}),document.getElementById("stage-chips").addEventListener("click",e=>{const t=e.target.closest(".v-chip[data-stage]");if(!t)return;const n=t.dataset.stage;S.has(n)?S.delete(n):S.add(n),t.classList.toggle("is-on",S.has(n)),$()}),document.getElementById("next-up-filter").addEventListener("click",e=>{ke=!ke,e.currentTarget.classList.toggle("is-on",ke),$()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{Ee=!Ee,e.currentTarget.classList.toggle("is-on",Ee),$()}),Je(),ae(),document.getElementById("pane-close").onclick=ge,document.getElementById("scrim").onclick=ge,document.getElementById("pursuit-close").onclick=le,document.getElementById("pursuit-scrim").onclick=le,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){Et();return}if(Ks()){vt();return}if(document.getElementById("profile-scrim").classList.contains("open")){yt();return}if(document.getElementById("add-scrim").classList.contains("open")){Me();return}if(document.getElementById("run-scrim").classList.contains("open")){Pe();return}if(document.getElementById("help-scrim").classList.contains("open")){Oe();return}if(document.getElementById("relink-scrim").classList.contains("open")){oe();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){xe();return}if(document.getElementById("deljob-scrim").classList.contains("open")){Be();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(Fe==="pursuit"&&n){le();return}if(Fe==="company"&&t){ge();return}if(t){ge();return}le();return}if(document.getElementById("key-scrim").classList.contains("open")){gt();return}if(document.getElementById("sources-scrim").classList.contains("open")){ct();return}if(document.getElementById("editor-scrim").classList.contains("open")){He();return}if(document.getElementById("settings-scrim").classList.contains("open")){bt();return}});let lt=null;const Hs={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function ln(e){if(i.meta&&i.meta.control===!1){u("control surface disabled");return}lt=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Hs[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Pe(){document.getElementById("run-scrim").classList.remove("open"),lt=null}document.getElementById("btn-enrich").onclick=()=>ln("enrich"),document.getElementById("btn-verdict").onclick=()=>ln("verdict"),document.getElementById("run-cancel").onclick=Pe,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Pe()},document.getElementById("run-go").onclick=()=>{const e=lt,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(Pe(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),at(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=_s;const Ps={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function un(e){const t=Ps[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const l=document.createElement("a");l.className="help-link",l.textContent="Learn more →",l.onclick=()=>{Oe(),fn(),Ws(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(l),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Oe(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>un("add"),document.getElementById("help-run").onclick=()=>un("run"),document.getElementById("help-close").onclick=Oe,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Oe()},document.getElementById("add-cancel").onclick=Me,document.getElementById("add-save").onclick=cn,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Me()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>nn(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",sn),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),cn()))}),document.getElementById("add-vertical-filter").addEventListener("input",an),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&xs(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=Ls,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=He,document.getElementById("editor-save").onclick=Ms,document.getElementById("editor-reset").onclick=As,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&He()},document.getElementById("sources-close").onclick=ct,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&ct()},document.getElementById("sources-refresh-btn").onclick=Ss,document.getElementById("key-cancel").onclick=gt,document.getElementById("key-save").onclick=mn,document.getElementById("key-remove").onclick=Js,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&gt()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),mn())}),document.getElementById("delcompany-cancel").onclick=xe,document.getElementById("delcompany-confirm").onclick=Un,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&xe()},document.getElementById("deljob-cancel").onclick=Be,document.getElementById("deljob-confirm").onclick=zn,document.getElementById("deljob-scrim").onclick=e=>{e.target.id==="deljob-scrim"&&Be()},document.getElementById("relink-cancel").onclick=oe,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&oe()},document.getElementById("relink-search").addEventListener("input",e=>{Vt(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&Ut(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&Ut(t.dataset.id)});function ut(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function pt(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}D()}const G='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',be='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',Os='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',pn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',qs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',Ns='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',ht='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',Rs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>',Ds='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>';function q(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${r(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function D(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,s=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let I="off",L="";const we=t&&t.criteria_state;we==="current"?(I="ok",L="current · verified "+ut(t.verified_age_seconds)):we==="changed"?(I="warn",L="changed — re-distill"):we==="unverified"?(I="warn",L=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(I="warn",L="brain offline · using cache"):a&&(I="ok",L="fetched "+ut(t.age_seconds)),o=q({icon:pn,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:I,note:L,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:be,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=q({icon:pn,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:G,actTitle:"edit taste.md",actLabel:"edit taste"});const c=i.sources&&i.sources.sources||[],l=c.filter(I=>I.need==="experience").length,p=c.filter(I=>I.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";l>0?(d="ok",m=`${l} experience · ${p} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const f=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",v=q({icon:Rs,nameHTML:f,dot:d,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:be,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),_=q({icon:qs,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:G,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),H=q({icon:Ns,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:G,actTitle:"edit the outreach email template",actLabel:"edit email template"}),V=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],B=V.map(([I,L,we])=>q({icon:ht,nameHTML:`<span class="edit-link" data-act="edit-prompt-${I}" title="edit the ${L.replace(/^\d+ · /,"")} prompt">${L}</span>`,desc:we,act:`edit-prompt-${I}`,actIcon:G,actTitle:`edit the ${L} prompt`,actLabel:`edit ${L} prompt`})).join(""),U=!i.stats||i.stats.taste_filter_enabled!==!1,De=q({icon:Ds,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage. Toggle it off in the editor to score every company.",dot:U?"ok":"off",note:U?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:G,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),w=i.anthropicKey;let te="off",$t="not set — verdict, capture & outreach disabled";w&&w.key_source==="db"?(te="ok",$t="set here · active"):w&&w.key_source==="env"&&(te="ok",$t="from the environment");const ta=q({icon:Os,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:te,note:$t,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:G,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),na=q({icon:ht,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:G,actTitle:"edit the application stages",actLabel:"edit application stages"}),sa=q({icon:ht,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:G,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${ta}
     </div>`;const It={"view-profile":()=>Fs(i.profile),"refresh-profile":zs,"edit-taste":()=>Y("taste"),"edit-taste-filter":()=>Y("taste-filter"),"edit-application-stages":()=>Y("application-stages"),"edit-outreach-statuses":()=>Y("outreach-statuses"),"edit-playbook":()=>Y("playbook"),"edit-template":()=>Y("outreach-template"),"view-sources":rt,"refresh-sources":Vs,"edit-anthropic-key":Us};for(const[I]of V)It[`edit-prompt-${I}`]=()=>Y(`outreach-prompts/${I}`);e.querySelectorAll("[data-act]").forEach(I=>{const L=I.dataset.act;L&&It[L]&&(I.onclick=It[L])})}async function mt(){try{i.sources=await(await fetch("/api/outreach/sources")).json()}catch{i.sources=null}D()}async function Vs(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),mt();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`refresh failed: ${s||"HTTP "+t.status}`),mt();return}const n=await t.json();n.warning?u(n.warning):u("outreach knowledge refreshed"),i.sources={sources:n.sources||[],needs:i.sources&&i.sources.needs||[]},D()}async function hn(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}D()}async function Us(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await hn(),ft();const e=document.getElementById("key-input");e&&e.focus()}function ft(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function gt(){document.getElementById("key-scrim").classList.remove("open")}async function mn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){u("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){u(`save failed: ${a.message}`),n();return}if(!s.ok){u((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),u("Anthropic key saved"),await nt(),ft(),D()}async function Js(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){u(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){u((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),u(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await nt(),ft(),D()}async function zs(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){u(`refresh failed: ${n.message}`),pt();return}if(!t.ok){const n=await t.text().catch(()=>"");u(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),pt();return}i.profile=await t.json(),D(),u("company-fit brief refreshed"),E()}function Fs(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${ut(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function yt(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=yt,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&yt()};function fn(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");qe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function vt(){document.getElementById("docs-scrim").classList.remove("open")}function Ks(){return document.getElementById("docs-scrim").classList.contains("open")}function qe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Ws(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),qe(e)}document.getElementById("open-docs").onclick=fn,document.getElementById("docs-close").onclick=vt,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&vt()};function Ys(){document.getElementById("settings-scrim").classList.add("open"),D()}function bt(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Ys,document.getElementById("settings-close").onclick=bt,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&bt()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),qe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&qe(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Gs(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Qs(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ne(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function wt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Zs(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function gn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=Gs(n.content);if(n.role==="user")s&&t.appendChild(Ne("user",s));else if(n.role==="assistant"){const a=Qs(n.content);if(!s&&!a.length)continue;const o=Ne("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Zs()),wt()}async function kt(e,t,n){if(!i.meta||!i.meta.chat){u("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const l=await c.json();i.chat.threadId=l.thread.id,gn(l.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Et(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Re(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function yn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function vn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",yn(),Re(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Ne("user",t));const a=Ne("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),wt();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Re(!1)},l=i.chat.threadId;let p;try{p=await fetch("/api/chat/"+l+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!p.ok){c((await p.text().catch(()=>"")).trim()||"HTTP "+p.status);return}const d=new EventSource("/api/chat/"+l+"/stream");i.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,wt()}),d.addEventListener("end",async m=>{d.close(),i.chat.es===d&&(i.chat.es=null),a.classList.remove("chat-streaming"),Re(!1),i.chat.threadId===l&&await Xs(),ea(),typeof m.data=="string"&&m.data.indexOf("error")===0&&u("chat: "+m.data)}),d.onerror=()=>{d.close(),i.chat.es===d&&(i.chat.es=null),a.classList.remove("chat-streaming"),Re(!1)}}async function Xs(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();gn(a.messages||[])}catch{}}function ea(){x(),k(),E(),i.openId&&re(i.openId)}document.getElementById("open-chat").onclick=()=>kt("global","",""),document.getElementById("chat-close").onclick=Et,document.getElementById("chat-scrim").onclick=Et,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),vn()}),document.getElementById("chat-input").addEventListener("input",yn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),vn())}),jt("#t tbody",kn),jt("#jt tbody",En),x(),k(),E(),nt(),st(),pt(),mt(),hn(),de()}ia({"":{view:()=>({mount(b){b.innerHTML=ua,pa()}}),chrome:!1}},{title:"scout"});da();la();
