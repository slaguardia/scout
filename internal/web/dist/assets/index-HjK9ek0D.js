(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))x(i);new MutationObserver(i=>{for(const g of i)if(g.type==="childList")for(const k of g.addedNodes)k.tagName==="LINK"&&k.rel==="modulepreload"&&x(k)}).observe(document,{childList:!0,subtree:!0});function y(i){const g={};return i.integrity&&(g.integrity=i.integrity),i.referrerPolicy&&(g.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?g.credentials="include":i.crossOrigin==="anonymous"?g.credentials="omit":g.credentials="same-origin",g}function x(i){if(i.ep)return;i.ep=!0;const g=y(i);fetch(i.href,g)}})();function Zn(b,r){const y=r.replace(/^#/,"");let x=null;for(const i of Object.keys(b))(y===i||y.startsWith(i))&&(x===null||i.length>x.length)&&(x=i);return x===null&&""in b&&(x=""),x}function Xn(b){return typeof b=="function"?{view:b,chrome:!0}:{view:b.view,chrome:b.chrome!==!1}}function es(b,r={}){const y=r.root??document.body,x=r.title??document.title??"",i=r.brandHref??"#",g=document.createElement("main"),k=document.createElement("header");k.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=x,w.setAttribute("aria-label",`${x} — home`),k.appendChild(w);const N=document.createElement("nav");N.className="cap-nav",N.setAttribute("aria-label","Views");for(const I of r.nav??[]){const j=document.createElement("a");j.href=I.href,j.textContent=I.label,I.ariaLabel&&j.setAttribute("aria-label",I.ariaLabel),N.appendChild(j)}k.appendChild(N);const S=document.createElement("section");S.className="tk-content",g.appendChild(k),g.appendChild(S);const T=document.createElement("div");T.className="tk-bleed";const je=I=>{var j;for(const A of Array.from(N.querySelectorAll("a"))){const C=((j=A.getAttribute("href"))==null?void 0:j.replace(/^#/,""))??"";A.toggleAttribute("aria-current",I!==null&&I!==""&&C===I),A.hasAttribute("aria-current")&&A.setAttribute("aria-current","page")}};let z=0;const M=()=>{const I=Zn(b,location.hash);if(je(I),I===null){T.isConnected&&T.remove(),g.isConnected||y.appendChild(g),ts(S,"Not found.");return}const{view:j,chrome:A}=Xn(b[I]),C=A?S:T;A?(T.isConnected&&T.remove(),g.isConnected||y.appendChild(g)):(g.isConnected&&g.remove(),T.isConnected||y.appendChild(T)),C.replaceChildren();const F=j(),Se=++z,G=F.mount(C);G instanceof Promise&&G.catch(Me=>{Se===z&&ns(C,String(Me))})};window.addEventListener("hashchange",M),M()}function ts(b,r){b.replaceChildren();const y=document.createElement("div");y.className="tk-empty",y.textContent=r,b.appendChild(y)}function ns(b,r){b.replaceChildren();const y=document.createElement("div");y.className="tk-error",y.textContent=r,b.appendChild(y)}function ss(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const y of r)y.unregister()}),window.caches&&caches.keys().then(r=>{for(const y of r)caches.delete(y)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function as(){let b;try{b=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!b.ok)return null;let r;try{r=await b.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const os=`
<div class="layout">
<aside class="sidebar">
...
`;function is(b){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null},y=e=>"pill pill-"+(e||"none"),x='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),g=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function k(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],H()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}r.jobs=e.rows||[],$(),N(),T()}function N(){if(!h.postingId)return;const e=r.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&ee())}let S=null;function T(){const e=r.jobs.some(t=>t.outreach_draft_status==="researching");e&&!S?S=setInterval(je,4e3):!e&&S&&(clearInterval(S),S=null)}async function je(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(r.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==r.jobs.length;r.jobs=t,s&&($(),N()),T()}function z(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Ae(),e==="jobs"?$():H()}async function M(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,I()}function I(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,K()}function j(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function A(e){return e.slice().sort((t,n)=>r.sort.dir*j(t,n,r.sort.k))}const C=new Set;let F=!1;function Se(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",C.has(e.dataset.v))})}function G(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(C.size&&!C.has(t.verdict||"__none__")||F&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Me=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Rt=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function st(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const at=st("scout-hidden-cols"),ot=st("scout-hidden-jcols");function it(){return r.view==="jobs"?{cols:Rt,hidden:ot,key:"scout-hidden-jcols"}:{cols:Me,hidden:at,key:"scout-hidden-cols"}}function Q(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=at.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=ot.has(e.dataset.col)?"none":""})}function Ae(){const e=it();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=it(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),Ae(),Q()})})}function rt(e){return`
      ...
      <td data-col="verdict"><span class="${y(e.verdict)}">${i(e.verdict||"—")}</span></td>
      <td><span class="row-name" data-id="${e.company_id}">${i(e.name)}</span></td>
      <td class="reason" data-col="reason">${i(e.reason||"")}</td>
      <td data-col="vertical">${i(e.vertical||"")}</td>
      <td data-col="location">${i(e.location||"")}</td>
      <td data-col="hc">${e.headcount||""}</td>
      <td data-col="stage">${i(e.stage||"")}</td>
      <td data-col="reviewed" class="muted" title="${i(e.reviewed_at||"never reviewed")}">${e.reviewed_at?i(e.reviewed_at.slice(0,10)):"—"}</td>
      <td data-col="site">${e.website_url?`<a href="${g(e.website_url)}" target="_blank" rel="noopener">about ↗</a>`:""}</td>
    `}function ct(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>$t(t.dataset.id))}function H(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=A(G());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=rt(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||ae(s.dataset.id)}),e.appendChild(s),ct(s)}Q()}async function Vt(e){const n=await(await fetch("/api/companies")).json();r.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=A(G()).map(c=>c.company_id),o=[...s.querySelectorAll("tr")].map(c=>c.dataset.id);if(a.length!==o.length||a.some((c,u)=>c!==o[u])){H();return}for(const c of e){const u=r.rows.find(d=>d.company_id===c),l=s.querySelector(`tr[data-id="${CSS.escape(c)}"]`);if(!u||!l){H();return}l.innerHTML=rt(u),ct(l)}Q()}const q=new Set;let pe=!1,he=!1,U=!0;function Dt(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=U&&!q.has("rejected");return r.jobs.filter(n=>!(t&&n.response==="rejected"||q.size&&!q.has(n.response||"")||pe&&!n.next_up||he&&(n.outreach_count|0)>0||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const Ut=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function dt(e){const t=String(e||"").split(",").map(n=>n.trim()).filter(Boolean);return t.length?t.map(n=>{const s=n.match(Ut);return s?`<a href="mailto:${i(s[0])}" title="${i(n)}">${i(n)}</a>`:i(n)}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}const O={offer:{cls:"pill-yes",label:"offer",order:0},interview:{cls:"pill-info",label:"interview",order:1},screening:{cls:"pill-maybe",label:"screening call",order:2},"":{cls:"pill-none",label:"—",order:3},rejected:{cls:"pill-no",label:"rejected",order:4}};function Jt(e,t,n){var s,a;if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[e.verdict]??3)-(o[t.verdict]??3)}if(n==="response")return(((s=O[e.response])==null?void 0:s.order)??3)-(((a=O[t.response])==null?void 0:a.order)??3);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="applied_at"||n==="last_outreach_at"){const o=e[n]||"",c=t[n]||"";return!o&&!c?0:o?c?String(c).localeCompare(String(o)):-r.jsort.dir:r.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function Z(e){const t=e.options[e.selectedIndex],n=getComputedStyle(e),s=Z._c||(Z._c=document.createElement("canvas").getContext("2d"));s.font=`${n.fontWeight} ${n.fontSize} ${n.fontFamily}`;const a=s.measureText(t?t.text:"").width;e.style.width=Math.ceil(a+35)+"px"}function $(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=Dt().sort((d,m)=>r.jsort.dir*Jt(d,m,r.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=U&&!q.has("rejected")?r.jobs.filter(d=>d.response==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=r.jobs.filter(d=>d.next_up).length,o=document.getElementById("next-up-n");o.textContent=a,o.style.display=a?"":"none";const c=r.jobs.filter(d=>!(d.outreach_count|0)&&!(U&&!q.has("rejected")&&d.response==="rejected")).length,u=document.getElementById("not-reached-n");u.textContent=c,u.style.display=c?"":"none";const l=document.getElementById("jobs-hidden-note");l.style.display=n?"":"none",n&&(l.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{U=!1,document.getElementById("hide-rejected").classList.remove("is-on"),$()});for(const d of t){const m=O[d.response]||O[""],f=document.createElement("tr");f.dataset.id=d.posting_id;const _=[["","none"],["screening","screening"],["interview","interview"],["offer","offer"],["rejected","rejected"]].map(([E,D])=>`<option value="${E}"${(d.response||"")===E?" selected":""}>${D}</option>`).join("");f.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${d.next_up?" is-on":""}" title="${d.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${d.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${i(d.title||d.company)}</span>${zt(d.outreach_draft_status)}${d.title?`<div class="small dim">${i(d.company)}</div>`:""}</div></div></td>
      ...
      <td class="small td-contacts" data-col="contacts">${dt(d.contacts)}</td>
      <td data-col="link"><a href="${g(d.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `;const B=()=>new Date().toISOString().slice(0,10);f.querySelector(".jt-nextup").onclick=()=>mt(d,!1),f.querySelector(".jt-applied").onclick=()=>fe(d,{applied_at:d.applied_at?"":B()}),f.querySelector(".jt-resp").onchange=E=>{Z(E.target),fe(d,{response:E.target.value})},f.querySelector(".jt-inc").onclick=()=>fe(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:B()}),f.querySelector(".jt-dec").onclick=()=>{const E=Math.max(0,(d.outreach_count||0)-1);fe(d,{outreach_count:E,...E===0?{last_outreach_at:""}:{}})},e.appendChild(f)}e.querySelectorAll(".jt-resp").forEach(Z),Q(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",m=>{m.target.closest("a, button, select")||lt(d.dataset.id)})})}function zt(e){return e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="needs_work"?'<span class="draft-badge db-needswork" title="the draft finished below the depth bar — review, fix or regenerate">draft needs work</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function lt(e){let t=r.jobs.find(n=>n.posting_id===e);if(t||(await w(),t=r.jobs.find(n=>n.posting_id===e)),!t){p("posting not found — refresh");return}Pe(),Ne(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),ut("pursuit"),ee(),X(),ne()}let He=null;function ut(e){He=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function me(){Pe(),Ne(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function Pe(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function X(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){ve();return}e=await n.json()}catch{ve();return}h.drafts=e.drafts||[],ve();const t=h.drafts[0];t&&t.status==="researching"?Ft():Pe()}function Ft(){h.poll||(h.poll=setInterval(X,4e3))}function P(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),p(`save failed: ${o.message}`)}})}async function pt(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,summary:o.summary,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),$(),ie(e.posting_id,{title:o.title,location:o.location,summary:o.summary})}async function Wt(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",g(e.url)),ie(e.posting_id,{url:s.url})}async function ht(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),k(),w()}async function Yt(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();r.openId=s.company_id,oe(s),re(s.company_id),k(),w()}async function Kt(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}function ee(){const e=h.row;if(!e)return;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${i(e.title||"")}">`;const t=O[e.response]||O[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${t.cls}">${i(t.label)}</span>`+(e.verdict?` <span class="${y(e.verdict)}">${i(e.verdict)}</span>`:"");const n=document.getElementById("pursuit-chat");n&&(n.style.display=r.meta&&r.meta.chat?"":"none",n.onclick=()=>Le("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    <section class="pane-section role-head">
      <div id="role-body">${Gt(e)}</div>
    </section>
  ...
  `,Qt();const s=document.getElementById("pursuit-company-link");s&&s.addEventListener("click",()=>ae(e.company_id)),P(document.getElementById("pursuit-title-input"),a=>pt(e,"title",a)),P(document.getElementById("pursuit-url-input"),a=>Wt(e,a)),P(document.getElementById("pursuit-notes-input"),a=>Zt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>P(a,o=>pt(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),ve(),se()}function Gt(e){return`
    ...
    </div>`}function Qt(){const e=h.row,t=()=>new Date().toISOString().slice(0,10),n=document.querySelector("#pursuit-body .pt-applied");n&&n.addEventListener("click",()=>W({applied_at:e.applied_at?"":t()}));const s=document.querySelector("#pursuit-body .pl-applied-date");s&&s.addEventListener("change",c=>W({applied_at:c.target.value}));const a=document.querySelector("#pursuit-body .pl-response");a&&a.addEventListener("change",c=>W({response:c.target.value}));const o=document.querySelector("#pursuit-body .pt-nextup");o&&o.addEventListener("click",()=>mt(h.row,!0))}async function mt(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){p(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();p(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,$(),ie(e.posting_id,{next_up:s.next_up}),t&&ee(),p(e.next_up?"queued next up":"removed from the queue")}async function ft(e,t){const n={applied_at:e.applied_at||"",response:e.response||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",contacts:e.contacts||"",notes:e.notes||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return p(`save failed: ${o.message}`),null}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();return p(`save failed: ${o||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),ie(e.posting_id,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function Zt(e){const t=h.row,n={applied_at:t.applied_at||"",response:t.response||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",contacts:t.contacts||"",notes:e},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,$()}async function W(e){await ft(h.row,e)&&($(),ee(),p("tracking saved"))}async function fe(e,t){await ft(e,t)&&($(),h.postingId===e.posting_id&&(h.row=e,ee()),p("tracking saved"))}function ve(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.row,n=h.drafts,s=n[0]||null,a=n.slice(1),o=`
    ...
    <div class="outreach-contacts">
      ...
      <div class="oc-rendered">${dt(t.contacts)}</div>
    </div>`,u=s&&(Xt(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,l=a.length?`
    ...
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const f=nn(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ${n("pill pill-no","failed")}
      ${e.fail_reason?`<div class="draft-note">${i(e.fail_reason)}</div>`:""}
      ${f}
      ${bt(e.critique)}
      ${te(e)}
      ${t?"":`<div class="draft-actions"><button class="btn btn-primary draft-retry-btn">${le}Retry</button></div>`}
      ...
      ${n("pill pill-yes","sent",t?"":vt)}
      ${e.sent_at?`<div class="draft-note">Sent ${i((e.sent_at||"").replace("T"," ").slice(0,16))}</div>`:""}
      <div class="draft-sentbody">${i(qe(e)||"(empty)")}</div>
      ${te(e)}
    </div>`;const s=qe(e),a=e.status==="no_hook",o=e.status==="needs_work",c=a?'<span class="pill pill-info">no honest hook</span>':o?'<span class="pill pill-maybe">needs work — below the depth bar</span>':'<span class="pill pill-maybe">awaiting review</span>';let u="";if(a)try{u=JSON.parse(e.hook||"{}").reasoning||""}catch{}const l=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${u?" "+i(u):""}</div>`:"",d=bt(e.critique);if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
      <div class="draft-head">${c}</div>
      ${l}
      ${d}
      <div class="draft-sentbody">${i(s||"(empty)")}</div>
      ${te(e)}
    </div>`;const m=s||a;return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    <div class="draft-head">${c}${s?vt:""}</div>
    ${l}
    ${d}
    ${m?`<textarea class="draft-textarea" id="draft-edit-${e.id}" spellcheck="false">${i(s)}</textarea>
    ${yt(e.lint)}
    <div class="draft-actions">
      <button class="btn btn-primary draft-sent-btn" title="mark this email sent — bumps the outreach count">${tn}Mark sent</button>
      <button class="btn draft-regen-btn" title="discard this draft (kept in history) and re-run — picks up backfilled info">${le}Regenerate</button>
    </div>`:`<div class="draft-actions">
      <button class="btn draft-regen-btn" title="re-run the draft — picks up backfilled info">${le}Regenerate</button>
    </div>`}
    ${te(e)}
  </div>`}function qe(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function te(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(l,d)=>d?`<div class="tr-line"><span class="tr-key">${l}:</span> ${i(String(d))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],u=c.map(l=>`
      <div class="tr-line">
        <span class="tr-key">${i(l.type||"hook")}</span>
        ${g(l.source_url)!=="#"?` · <a href="${g(l.source_url)}" target="_blank" rel="noopener">source</a>`:""}
      ...
      </div></details>`}return t}function yt(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${i(n.message||"")}"><code>${i(n.code||"")}</code>${i(n.message||"")}</span>`).join("")+"</div>":""}function bt(e){let t=null;try{t=JSON.parse(e||"null")}catch{return""}if(!t||typeof t!="object")return"";const n=t.depth==="deep"?"pill-yes":t.depth==="medium"?"pill-maybe":"pill-no",s={direct:"pill-yes",adjacent:"pill-info",standing:"pill-maybe"}[t.proof_tier]||"pill-no",a=t.proof_tier==="standing"?"standing creds":t.proof_tier,o=[t.depth?`<span class="pill ${n}">depth: ${i(t.depth)}</span>`:"",t.proof_tier?`<span class="pill ${s}">proof: ${i(a)}</span>`:""].filter(Boolean).join(""),c=Array.isArray(t.weaknesses)&&t.weaknesses.length?'<ul class="critique-list">'+t.weaknesses.map(d=>`<li>${i(String(d))}</li>`).join("")+"</ul>":"",u=String(t.experience_gaps||"").trim(),l=u?`<div class="critique-gap"><span class="cg-label">experience gap:</span> ${i(u)}</div>`:"";return!o&&!c&&!l?"":`<div class="draft-critique">
    ${o?`<div class="critique-chips">${o}</div>`:""}
    ${c}${l}
  </div>`}function nn(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${i(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${i(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function sn(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector(".oc-input");t&&(t.addEventListener("change",l=>W({contacts:l.target.value.trim()})),t.addEventListener("keydown",l=>{l.key==="Enter"&&(l.preventDefault(),l.target.blur())}));const n=()=>new Date().toISOString().slice(0,10),s=h.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>W({outreach_count:(s.outreach_count||0)+1,last_outreach_at:n()}));const o=e.querySelector(".pt-outreach-dec");o&&o.addEventListener("click",()=>{const l=Math.max(0,(s.outreach_count||0)-1);W({outreach_count:l,...l===0?{last_outreach_at:""}:{}})});const c=e.querySelector("#draft-start-btn");c&&c.addEventListener("click",()=>ge()),e.querySelectorAll(".draft-retry-btn").forEach(l=>l.addEventListener("click",()=>ge())),e.querySelectorAll(".draft-regen-btn").forEach(l=>l.addEventListener("click",()=>ge(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(l=>{const d=l.dataset.did,m=l.querySelector(".draft-textarea");m&&P(m,B=>on(d,B),{multiline:!0});const f=l.querySelector(".draft-sent-btn");f&&f.addEventListener("click",()=>rn(d));const _=l.querySelector(".draft-copy-btn");_&&_.addEventListener("click",()=>{const B=l.querySelector(".draft-textarea"),E=l.querySelector(".draft-sentbody"),D=B?B.value:E?E.textContent:"";xn(D,"email copied")})});const u=e.querySelector("details.draft-history");u&&u.addEventListener("toggle",()=>{h.openHist=u.open})}async function ge(e=!1){const t=document.getElementById("outreach-section"),n=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));n&&(n.disabled=!0);let s;try{const o=e?"?regenerate=1":"";s=await fetch(`/api/postings/${h.postingId}/outreach${o}`,{method:"POST"})}catch(o){p(`draft failed: ${o.message}`),n&&(n.disabled=!1);return}if(s.status===202){let o={};try{o=await s.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&p(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await X(),w();return}if(s.status===409){await X(),p("a draft is already active");return}if(s.status===412){let o={};try{o=await s.json()}catch{}an(o.need,o.error),n&&(n.disabled=!1);return}if(s.status===503){const o=document.getElementById("outreach-section");if(o){const c=document.createElement("div");c.className="draft-note",c.textContent="Outreach engine not running in this build.",o.appendChild(c)}n&&(n.disabled=!1);return}const a=(await s.text().catch(()=>"")).trim();p(`draft failed: ${a||"HTTP "+s.status}`),n&&(n.disabled=!1)}function an(e,t){const n=document.getElementById("outreach-section");if(!n)return;const s=n.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"Discover sources",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,s?s.replaceWith(c):n.appendChild(c);const u=c.querySelector("#gate-fix-btn");u&&u.addEventListener("click",()=>a?Y("outreach-template"):Je());const l=c.querySelector("#gate-retry-btn");l&&l.addEventListener("click",ge)}async function on(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.drafts.findIndex(u=>String(u.id)===String(e));a>=0&&(h.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const u=c.querySelector(".lint-chips"),l=yt(s.lint);u?u.outerHTML=l||"":l&&o.insertAdjacentHTML("afterend",l)}}async function rn(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){p(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();p(`failed: ${s||"HTTP "+t.status}`);return}p("marked sent"),await X(),await w();const n=r.jobs.find(s=>s.posting_id===h.postingId);n&&ie(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function ne(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){se();return}e=await t.json()}catch{se();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",se(),h.answers.some(t=>t.status==="generating")?cn():Ne()}function cn(){h.answersPoll||(h.answersPoll=setInterval(ne,4e3))}function Ne(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function se(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(d=>d.status==="generating"),a=t.length?`<div class="answers-list">${t.map(un).join("")}</div>`:"",o=!!h.detecting,c=s||o?" disabled":"",u=d=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":d}</button>`;let l;n==="ok"&&t.length?l=`<button class="btn ${t.some(m=>!wt(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+u("Re-detect"):n===""||n==="unreachable"?l=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+u("Re-detect questions"):l=u("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${i(dn(n,t.length))}</div>`+a+`<div class="answers-actions">${l}</div>`,hn()}function dn(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function wt(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function ln(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function un(e){const t=wt(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${i(pn(e.fail_reason))}</div>`:""}
  </div>`}function pn(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function hn(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",kt);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",fn),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(P(o,u=>vn(a,u),{multiline:!0}),o.addEventListener("input",()=>mn(s,o)));const c=s.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>gn(a))})}function mn(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function kt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){p(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await ne();return}if(n.status===412){let a={};try{a=await n.json()}catch{}yn(a.error),t&&(t.disabled=!1);return}if(n.status===503){Et("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();p(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function fn(){h.detecting=!0,se();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();p(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){p(`detect failed: ${e.message}`)}h.detecting=!1,await ne()}async function vn(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=s)}async function gn(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){p(`regenerate failed: ${n.message}`);return}if(t.status===503){Et("Answer generation isn't running in this build.");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();p(`regenerate failed: ${n||"HTTP "+t.status}`);return}await ne()}function yn(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",Je);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",kt)}function Et(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function ae(e){r.openId=e;const t=document.getElementById("pane"),n=document.getElementById("scrim");t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),ut("company"),document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';let s;try{const a=await fetch(`/api/companies/${e}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);s=await a.json()}catch(a){document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${i(a.message)}</div>`;return}oe(s),re(e)}function ye(){r.openId=null,r.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function oe(e){r.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${i(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `:'<div class="muted">Not yet scored. Run <code>scout verdict</code>, or set one by hand below.</div>',a=`
    ...
    <dl class="kv">
      <dt>url</dt><dd>${e.website_url?`<a href="${g(e.website_url)}" target="_blank" rel="noopener">${i(e.website_url)} ↗</a>`:'<span class="muted">—</span>'}</dd>
  ...
  `;const _=document.getElementById("posting-add-btn");_&&_.addEventListener("click",()=>En(e)),Re(),document.querySelectorAll("#ve-pick .ve-opt").forEach(v=>{v.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(L=>L.classList.remove("is-on")),v.classList.add("is-on")})});const B=document.getElementById("ve-save-btn");B&&B.addEventListener("click",()=>kn(e)),P(document.getElementById("pane-title-input"),v=>ht(e,"name",v)),document.querySelectorAll("#facts-body [data-k]").forEach(v=>P(v,L=>ht(e,v.dataset.k,L))),P(document.getElementById("pane-domain-input"),v=>Yt(e,v)),P(document.getElementById("pane-notes-input"),v=>Kt(e,v),{multiline:!0});const E=document.getElementById("flag-toggle-btn");E&&E.addEventListener("click",()=>$t(e.company_id));const D=document.getElementById("review-stamp-btn");D&&D.addEventListener("click",()=>wn(e.company_id));const Te=document.getElementById("rescore-btn");Te&&Te.addEventListener("click",()=>De("verdict",{company_ids:[e.company_id]}));const ue=document.getElementById("reenrich-btn");ue&&ue.addEventListener("click",()=>De("enrich",{company_ids:[e.company_id]}))}function bn(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function wn(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){p(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");p(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=r.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,H()),r.openId===e&&(oe(s),re(e)),p("reviewed")}async function $t(e){const t=r.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){p(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");p(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,H()),r.openId===e&&(oe(a),re(e)),w(),p(a.flagged?"flagged":"unflagged")}async function kn(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){p("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(u){p(`save failed: ${u.message}`),a.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");p(`save failed: HTTP ${o.status}${u?" — "+u:""}`),a.disabled=!1;return}const c=await o.json();oe(c),re(c.company_id),k(),M(),w(),p("verdict saved")}function Oe(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(i).join(" · "),a=O[n.response]||O[""],o=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a.cls}">${i(a.label)}</span>`,`<span class="pt-meta">${n.applied_at?`applied ${i(n.applied_at)}`:"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${i(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join(""),c=r.meta&&r.meta.chat?`<button class="pcard-chat" data-pid="${i(n.id)}" data-ptitle="${i(n.title||"")}" title="chat about this role">chat</button>`:"";return`
    <div class="brain-node posting-card" data-pid="${i(n.id)}" title="open the pursuit — tracking, outreach, drafts">
      <div class="n"><a href="${g(n.url)}" target="_blank" rel="noopener">${i(n.title||n.url)} ↗</a></div>
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ie(e,t){const n=r.openDetail;if(!n||!r.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Oe(n),Re())}function Re(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||lt(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),Le("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function En(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){p("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(l){p(`add failed: ${l.message}`),s.disabled=!1;return}if(!o.ok){const l=await o.text().catch(()=>"");p(`add failed: HTTP ${o.status}${l?" — "+l:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(l=>l.id!==c.id),e.postings.unshift(c);const u=document.getElementById("postings-list");u&&(u.innerHTML=Oe(e),Re()),t.value="",n.value="",s.disabled=!1,w(),p("link added")}async function re(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){be(`<div class="muted">Failed to load trail: ${i(s.message)}</div>`);return}if(!t.ok){be(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){be('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}be(n.map($n).join(""))}function $n(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
    ...
    </div>`}function be(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let xt;function p(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(xt),xt=setTimeout(()=>t.classList.remove("show"),2200)}async function xn(e,t="copied"){if(!e){p("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}p(t)}catch(n){p(`copy failed: ${n.message}`)}}r.meta={control:!1,brain:!1,verdict:!1};async function In(){try{const s=await fetch("/api/meta");if(!s.ok)return;r.meta=await s.json()}catch{return}const e=r.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!r.meta.verdict,t.title=r.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=r.meta.chat?"":"none")}async function Ve(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let we=null;async function De(e,t){if(r.meta&&r.meta.control===!1){p("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){p(`run failed: ${a.message}`);return}if(n.status===409){p("a job is already running");return}if(n.status===412){const a=await n.text();p(a.trim());return}if(!n.ok){p(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();jt(e,s,t)}async function _n(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){p(`upload failed: ${a.message}`);return}if(n.status===409){p("a job is already running");return}if(!n.ok){p(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();jt("ingest",s)}const Bn=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let ke=[],R=new Set,V="company";function It(e){V=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",_t()}function Ue(){return!!r.meta.capture&&document.getElementById("add-enrich").checked}function _t(){const e=document.getElementById("add-note");Ue()?e.innerHTML=V==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=V==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function Ln(){Bn.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",R=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!r.meta.capture,t.classList.toggle("disabled",!r.meta.capture),t.title=r.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",r.meta.capture||(e.checked=!1),It(r.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(r.rows||[]).map(s=>`<option value="${i(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),ke=s.verticals||[]}catch{ke=[]}Bt()}function Ee(){document.getElementById("add-scrim").classList.remove("open")}function Bt(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=ke.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${R.has(s)?" sel":""}" data-v="${i(s)}">${i(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;R.has(a)?R.delete(a):R.add(a),s.classList.toggle("sel"),Lt()}))):e.innerHTML=`<div class="none">${ke.length?"no match":"no verticals in the set yet"}</div>`,Lt()}function Lt(){const e=R.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Ct(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Tt(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){p(V==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,Ue()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=f=>document.getElementById(f).value.trim(),c=Ue();let u,l;c?(u="/api/capture",l={url:Ct(t),kind:V==="company"?"company_page":"job_posting",fields:V==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...R].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):V==="company"?(u="/api/companies",l={website:t,name:o("add-name"),vertical:[...R].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(u="/api/postings",l={url:Ct(t),title:o("add-title"),company:o("add-job-company")});let d;try{d=await fetch(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)})}catch(f){p(`add failed: ${f.message}`),a();return}if(!d.ok){let f=`HTTP ${d.status}`;try{const _=await d.text();try{f=JSON.parse(_).error||f}catch{f=_.trim()||f}}catch{}if(a(),d.status===409){p(f||"That company is already in the list."),e.focus(),e.select();return}p(`add failed: ${f}`);return}const m=await d.json();if(a(),c&&!m.company_id){p(m.note||"couldn't classify that page");return}if(Ee(),k(),M(),w(),V==="job"){const f=m.posting&&m.posting.title||"job link";p(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),z("jobs")}else c?(p(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`),ae(m.company_id)):p("company added")}function jt(e,t,n){we=t;const s=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="",s.classList.add("open"),Ve();const o=new EventSource(`/api/jobs/${t}/stream`),c=(u,l)=>{const d=document.createElement("div"),m=!l&&/^\s*warn:/i.test(u);d.className="ln"+(l?" ln-err":m?" ln-warn":""),d.textContent=m?u.replace(/^\s*warn:\s*/i,"⚠ "):u,a.appendChild(d),a.scrollTop=a.scrollHeight};o.addEventListener("line",u=>c(u.data,/error|failed/i.test(u.data))),o.addEventListener("end",u=>{o.close(),we=null,c(`— ${u.data} —`,u.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",p(`${e} ${u.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?Vt(n.company_ids):k(),M(),Ve(),w(),r.openId&&ae(r.openId)}),o.onerror=()=>{o.close()}}async function Cn(){if(we)try{await fetch(`/api/jobs/${we}/cancel`,{method:"POST"})}catch{}}let ce=null;function St(e){return e==="outreach-template"?"outreach template":e==="outreach-doctrine"?"outreach doctrine":e==="taste-filter"?"pre-filter rules":e==="playbook"?"playbook":e+".md"}async function Y(e){ce=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+St(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="",t.classList.add("open");try{const n=await fetch(`/api/${e}`);if(!n.ok){const a=(await n.text().catch(()=>"")).trim();document.getElementById("editor-text").value=n.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${a||"HTTP "+n.status}`;return}const s=await n.json();document.getElementById("editor-text").value=s.content||"",s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version)}catch(n){document.getElementById("editor-text").value="failed to load: "+n.message}}function $e(){document.getElementById("editor-scrim").classList.remove("open"),ce=null}const Tn=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function Je(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{Fe(await(await fetch("/api/outreach/sources")).json())}catch(e){p(`failed to load sources: ${e.message}`)}}function ze(){document.getElementById("sources-scrim").classList.remove("open")}function Fe(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):Tn,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],c=o.length?o.map(u=>`<li><span class="src-title">${i(u.title||u.page_id)}</span><button class="src-rm" data-need="${i(a.key)}" data-id="${i(u.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${i(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>Sn(a.dataset.need,a.dataset.id)))}async function jn(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){p(`refresh failed: ${s.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){p(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const n=await t.json();n.warning?p(n.warning):p("sources refreshed"),Fe(n),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function Sn(e,t){let n;try{n=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(s){p(`remove failed: ${s.message}`);return}if(!n.ok){p(`remove failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}Fe(await n.json())}async function Mn(){if(!ce)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${ce}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(s){p(`save failed: ${s.message}`);return}if(!t.ok){p(`save failed: HTTP ${t.status}`);return}const n=await t.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version),p(`${St(ce)} saved`),$e(),M()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),H()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),$()}}),document.getElementById("tab-companies").onclick=()=>z("companies"),document.getElementById("tab-jobs").onclick=()=>z("jobs"),document.getElementById("q").oninput=H,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;C.has(t)?C.delete(t):C.add(t),Se(),H()})}),document.getElementById("flag-filter").addEventListener("click",e=>{F=!F,e.currentTarget.classList.toggle("is-on",F),H()}),document.getElementById("jq").oninput=$,document.getElementById("hide-rejected").addEventListener("click",e=>{U=!U,e.currentTarget.classList.toggle("is-on",U),$()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;q.has(t)?q.delete(t):q.add(t),e.classList.toggle("is-on",q.has(t)),$()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{pe=!pe,e.currentTarget.classList.toggle("is-on",pe),$()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{he=!he,e.currentTarget.classList.toggle("is-on",he),$()}),Ae(),Q(),document.getElementById("pane-close").onclick=ye,document.getElementById("scrim").onclick=ye,document.getElementById("pursuit-close").onclick=me,document.getElementById("pursuit-scrim").onclick=me,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){tt();return}if(Jn()){Ze();return}if(document.getElementById("profile-scrim").classList.contains("open")){Qe();return}if(document.getElementById("add-scrim").classList.contains("open")){Ee();return}if(document.getElementById("run-scrim").classList.contains("open")){xe();return}if(document.getElementById("help-scrim").classList.contains("open")){Ie();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(He==="pursuit"&&n){me();return}if(He==="company"&&t){ye();return}if(t){ye();return}me();return}if(document.getElementById("sources-scrim").classList.contains("open")){ze();return}if(document.getElementById("editor-scrim").classList.contains("open")){$e();return}if(document.getElementById("settings-scrim").classList.contains("open")){Xe();return}});let We=null;const An={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Mt(e){if(r.meta&&r.meta.control===!1){p("control surface disabled");return}We=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=An[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=r.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function xe(){document.getElementById("run-scrim").classList.remove("open"),We=null}document.getElementById("btn-enrich").onclick=()=>Mt("enrich"),document.getElementById("btn-verdict").onclick=()=>Mt("verdict"),document.getElementById("run-cancel").onclick=xe,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&xe()},document.getElementById("run-go").onclick=()=>{const e=We,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(xe(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),De(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=Ln;const Hn={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function At(e){const t=Hn[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const u=document.createElement("a");u.className="help-link",u.textContent="Learn more →",u.onclick=()=>{Ie(),Pt(),zn(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(u),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Ie(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>At("add"),document.getElementById("help-run").onclick=()=>At("run"),document.getElementById("help-close").onclick=Ie,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Ie()},document.getElementById("add-cancel").onclick=Ee,document.getElementById("add-save").onclick=Tt,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Ee()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>It(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",_t),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Tt()))}),document.getElementById("add-vertical-filter").addEventListener("input",Bt),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&_n(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=Cn,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=$e,document.getElementById("editor-save").onclick=Mn,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&$e()},document.getElementById("sources-close").onclick=ze,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&ze()},document.getElementById("sources-refresh-btn").onclick=jn;function Ye(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function Ke(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}K()}const de='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',le='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',Ht='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',Pn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',qn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',Nn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',On='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>',Rn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>';function J(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${i(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function K(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,s=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let v="off",L="";const nt=t&&t.criteria_state;nt==="current"?(v="ok",L="current · verified "+Ye(t.verified_age_seconds)):nt==="changed"?(v="warn",L="changed — re-distill"):nt==="unverified"?(v="warn",L=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(v="warn",L="brain offline · using cache"):a&&(v="ok",L="fetched "+Ye(t.age_seconds)),o=J({icon:Ht,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:v,note:L,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:le,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=J({icon:Ht,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:de,actTitle:"edit taste.md",actLabel:"edit taste"});const c=r.sources&&r.sources.sources||[],u=c.filter(v=>v.need==="experience").length,l=c.filter(v=>v.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";u>0?(d="ok",m=`${u} experience · ${l} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const f=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",_=J({icon:On,nameHTML:f,dot:d,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:le,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),B=J({icon:Pn,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:de,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),E=J({icon:qn,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:de,actTitle:"edit the outreach email template",actLabel:"edit email template"}),D=J({icon:Nn,nameHTML:'<span class="edit-link" data-act="edit-doctrine" title="edit the outreach doctrine">outreach doctrine</span>',desc:"How cold emails get written — the depth bar, show-don't-tell, the kill list.",act:"edit-doctrine",actIcon:de,actTitle:"edit the outreach doctrine",actLabel:"edit outreach doctrine"}),Te=J({icon:Rn,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage.",act:"edit-taste-filter",actIcon:de,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${B}${Te}${E}${D}
     </div>`;const ue={"view-profile":()=>Un(r.profile),"refresh-profile":Dn,"edit-taste":()=>Y("taste"),"edit-taste-filter":()=>Y("taste-filter"),"edit-playbook":()=>Y("playbook"),"edit-template":()=>Y("outreach-template"),"edit-doctrine":()=>Y("outreach-doctrine"),"view-sources":Je,"refresh-sources":Vn};e.querySelectorAll("[data-act]").forEach(v=>{const L=v.dataset.act;L&&ue[L]&&(v.onclick=ue[L])})}async function Ge(){try{r.sources=await(await fetch("/api/outreach/sources")).json()}catch{r.sources=null}K()}async function Vn(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){p(`refresh failed: ${s.message}`),Ge();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();p(`refresh failed: ${s||"HTTP "+t.status}`),Ge();return}const n=await t.json();n.warning?p(n.warning):p("outreach knowledge refreshed"),r.sources={sources:n.sources||[],needs:r.sources&&r.sources.needs||[]},K()}async function Dn(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){p(`refresh failed: ${n.message}`),Ke();return}if(!t.ok){const n=await t.text().catch(()=>"");p(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Ke();return}r.profile=await t.json(),K(),p("company-fit brief refreshed"),M()}function Un(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${Ye(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Qe(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Qe,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Qe()};function Pt(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");_e(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ze(){document.getElementById("docs-scrim").classList.remove("open")}function Jn(){return document.getElementById("docs-scrim").classList.contains("open")}function _e(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function zn(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),_e(e)}document.getElementById("open-docs").onclick=Pt,document.getElementById("docs-close").onclick=Ze,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Ze()};function Fn(){document.getElementById("settings-scrim").classList.add("open"),K()}function Xe(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Fn,document.getElementById("settings-close").onclick=Xe,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&Xe()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),_e(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&_e(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),r.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Wn(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Yn(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Be(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function et(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Kn(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=r.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(r.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function qt(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=Wn(n.content);if(n.role==="user")s&&t.appendChild(Be("user",s));else if(n.role==="assistant"){const a=Yn(n.content);if(!s&&!a.length)continue;const o=Be("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Kn()),et()}async function Le(e,t,n){if(!r.meta||!r.meta.chat){p("chat needs ANTHROPIC_API_KEY in the server env");return}r.chat.es&&(r.chat.es.close(),r.chat.es=null),r.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const u=await c.json();r.chat.threadId=u.thread.id,qt(u.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+i(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function tt(){r.chat.es&&(r.chat.es.close(),r.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Ce(e){r.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Nt(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Ot(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||r.chat.streaming||!r.chat.threadId)return;e.value="",Nt(),Ce(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Be("user",t));const a=Be("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),et();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Ce(!1)},u=r.chat.threadId;let l;try{l=await fetch("/api/chat/"+u+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!l.ok){c((await l.text().catch(()=>"")).trim()||"HTTP "+l.status);return}const d=new EventSource("/api/chat/"+u+"/stream");r.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,et()}),d.addEventListener("end",async m=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Ce(!1),r.chat.threadId===u&&await Gn(),Qn(),typeof m.data=="string"&&m.data.indexOf("error")===0&&p("chat: "+m.data)}),d.onerror=()=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Ce(!1)}}async function Gn(){const e=r.chat.scope,t=r.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();qt(a.messages||[])}catch{}}function Qn(){k(),w(),M(),r.openId&&ae(r.openId)}document.getElementById("open-chat").onclick=()=>Le("global","",""),document.getElementById("chat-close").onclick=tt,document.getElementById("chat-scrim").onclick=tt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Ot()}),document.getElementById("chat-input").addEventListener("input",Nt),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Ot())}),k(),w(),M(),In(),Ve(),Ke(),Ge()}es({"":{view:()=>({mount(b){b.innerHTML=os,is()}}),chrome:!1}},{title:"scout"});ss();as();
