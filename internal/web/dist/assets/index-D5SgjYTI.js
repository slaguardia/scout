...
`;function Kt(g){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},f=e=>"pill pill-"+(e||"none"),E='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',o=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),m=e=>/^https?:\/\//i.test(String(e??""))?o(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],A()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],_(),p.postingId){const t=r.jobs.find(n=>n.posting_id===p.postingId);t&&(p.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&J())}}function j(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",fe(),e==="jobs"?_():A()}async function I(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Ie()}function he(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[s]??3)-(i[a]??3)}return String(s).localeCompare(String(a))}function K(e){return e.slice().sort((t,n)=>r.sort.dir*he(t,n,r.sort.k))}const C=new Set;let w=!1;function B(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",C.has(e.dataset.v))})}function S(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(C.size&&!C.has(t.verdict||"__none__")||w&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const N=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],me=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Y(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const V=Y("scout-hidden-cols"),G=Y("scout-hidden-jcols");function je(){return r.view==="jobs"?{cols:me,hidden:G,key:"scout-hidden-jcols"}:{cols:N,hidden:V,key:"scout-hidden-cols"}}function Q(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=V.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=G.has(e.dataset.col)?"none":""})}function fe(){const e=je();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=je(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),fe(),Q()})})}function A(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=K(S());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
  ...
  `,ht();const n=document.getElementById("pursuit-view-company");n&&n.addEventListener("click",()=>te(e.company_id)),F(document.getElementById("pursuit-title-input"),s=>Me(e,"title",s)),document.querySelectorAll("#role-body [data-k]").forEach(s=>F(s,a=>Me(e,s.dataset.k,a),{multiline:s.tagName==="TEXTAREA"})),z()}function pt(e){return`
    ...
    </details>`:"";e.innerHTML=i+`<div id="draft-current">${s?qe(s,!1):""}</div><div class="draft-actions">${d}</div>`+u,vt()}function mt(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function qe(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(y,v)=>`
    ...
    ${ee(e)}
  </div>`}function Ne(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function ee(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,h)=>h?`<div class="tr-line"><span class="tr-key">${u}:</span> ${o(String(h))}</div>`:"",i=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],d=c.map(u=>`
      <div class="tr-line">
  ...
  `;const v=document.getElementById("posting-add-btn");v&&v.addEventListener("click",()=>xt(e)),De(),document.querySelectorAll("#ve-pick .ve-opt").forEach(x=>{x.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Te=>Te.classList.remove("is-on")),x.classList.add("is-on")})});const b=document.getElementById("ve-save-btn");b&&b.addEventListener("click",()=>$t(e)),F(document.getElementById("pane-title-input"),x=>Ae(e,"name",x)),document.querySelectorAll("#facts-body [data-k]").forEach(x=>F(x,Te=>Ae(e,x.dataset.k,Te)));const T=document.getElementById("flag-toggle-btn");T&&T.addEventListener("click",()=>Ve(e.company_id));const at=document.getElementById("review-stamp-btn");at&&at.addEventListener("click",()=>Et(e.company_id));const ot=document.getElementById("rescore-btn");ot&&ot.addEventListener("click",()=>be("verdict",{company_ids:[e.company_id]}));const it=document.getElementById("reenrich-btn");it&&it.addEventListener("click",()=>be("enrich",{company_ids:[e.company_id]}))}function kt(e){const t=e.domain?`<a href="https://${o(e.domain)}" target="_blank" rel="noopener">${o(e.domain)} ↗</a>`:'<span class="muted">no domain</span>';return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function De(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||(ne(),Se(e.dataset.pid))})})}async function xt(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!i.ok){const u=await i.text().catch(()=>"");l(`add failed: HTTP ${i.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await i.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const d=document.getElementById("postings-list");d&&(d.innerHTML=Ue(e),De()),t.value="",n.value="",s.disabled=!1,k(),l("link added")}async function ae(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){oe(`<div class="muted">Failed to load trail: ${o(s.message)}</div>`);return}if(!t.ok){oe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){oe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}oe(n.map(_t).join(""))}function _t(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(o);return e.run_id&&t.push("run "+o(e.run_id.slice(0,8))),`
    ...
    <button class="crit-edit" id="edit-sender" title="edit outreach identity" aria-label="edit outreach identity">${_e}</button></div>`,n>0&&(c+=`<div class="stale">
      ...
      ${n} verdicts stale</div>`),e.innerHTML=c;const d=document.getElementById("view-profile");d&&(d.onclick=()=>qt(r.profile));const u=document.getElementById("refresh-profile");u&&(u.onclick=Pt);const h=document.getElementById("edit-taste");h&&(h.onclick=()=>Xe("taste"));const y=document.getElementById("edit-playbook");y&&(y.onclick=()=>Xe("playbook"));const v=document.getElementById("edit-sender");v&&(v.onclick=Ct)}async function Pt(){const e=document.getElementById("refresh-profile");e&&(e.textContent="refreshing…",e.style.pointerEvents="none");let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),xe();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),xe();return}r.profile=await t.json(),Ie(),l("brain profile refreshed"),I()}function qt(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${nt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Be(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Be,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Be()};function st(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");pe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Le(){document.getElementById("docs-scrim").classList.remove("open")}function Nt(){return document.getElementById("docs-scrim").classList.contains("open")}function pe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Ot(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),pe(e)}document.getElementById("open-docs").onclick=st,document.getElementById("docs-close").onclick=Le,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Le()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),pe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,i)=>a.boundingClientRect.top-i.boundingClientRect.top);s.length&&pe(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),$(),k(),I(),It(),ye(),xe()}Ut({"":{view:()=>({mount(g){g.innerHTML=Wt,Kt()}}),chrome:!1}},{title:"scout"});Jt();zt();
