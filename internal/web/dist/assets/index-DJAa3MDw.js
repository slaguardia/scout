(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))_(o);new MutationObserver(o=>{for(const m of o)if(m.type==="childList")for(const $ of m.addedNodes)$.tagName==="LINK"&&$.rel==="modulepreload"&&_($)}).observe(document,{childList:!0,subtree:!0});function f(o){const m={};return o.integrity&&(m.integrity=o.integrity),o.referrerPolicy&&(m.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?m.credentials="include":o.crossOrigin==="anonymous"?m.credentials="omit":m.credentials="same-origin",m}function _(o){if(o.ep)return;o.ep=!0;const m=f(o);fetch(o.href,m)}})();function Dt(g,r){const f=r.replace(/^#/,"");let _=null;for(const o of Object.keys(g))(f===o||f.startsWith(o))&&(_===null||o.length>_.length)&&(_=o);return _===null&&""in g&&(_=""),_}function Jt(g){return typeof g=="function"?{view:g,chrome:!0}:{view:g.view,chrome:g.chrome!==!1}}function Ft(g,r={}){const f=r.root??document.body,_=r.title??document.title??"",o=r.brandHref??"#",m=document.createElement("main"),$=document.createElement("header");$.className="cap-head";const E=document.createElement("a");E.className="brand",E.href=o,E.textContent=_,E.setAttribute("aria-label",`${_} — home`),$.appendChild(E);const j=document.createElement("nav");j.className="cap-nav",j.setAttribute("aria-label","Views");for(const k of r.nav??[]){const B=document.createElement("a");B.href=k.href,B.textContent=k.label,k.ariaLabel&&B.setAttribute("aria-label",k.ariaLabel),j.appendChild(B)}$.appendChild(j);const I=document.createElement("section");I.className="tk-content",m.appendChild($),m.appendChild(I);const L=document.createElement("div");L.className="tk-bleed";const he=k=>{var B;for(const S of Array.from(j.querySelectorAll("a"))){const O=((B=S.getAttribute("href"))==null?void 0:B.replace(/^#/,""))??"";S.toggleAttribute("aria-current",k!==null&&k!==""&&O===k),S.hasAttribute("aria-current")&&S.setAttribute("aria-current","page")}};let G=0;const C=()=>{const k=Dt(g,location.hash);if(he(k),k===null){L.isConnected&&L.remove(),m.isConnected||f.appendChild(m),zt(I,"Not found.");return}const{view:B,chrome:S}=Jt(g[k]),O=S?I:L;S?(L.isConnected&&L.remove(),m.isConnected||f.appendChild(m)):(m.isConnected&&m.remove(),L.isConnected||f.appendChild(L)),O.replaceChildren();const me=B(),Q=++G,U=me.mount(O);U instanceof Promise&&U.catch(X=>{Q===G&&Wt(O,String(X))})};window.addEventListener("hashchange",C),C()}function zt(g,r){g.replaceChildren();const f=document.createElement("div");f.className="tk-empty",f.textContent=r,g.appendChild(f)}function Wt(g,r){g.replaceChildren();const f=document.createElement("div");f.className="tk-error",f.textContent=r,g.appendChild(f)}function Yt(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const f of r)f.unregister()}),window.caches&&caches.keys().then(r=>{for(const f of r)caches.delete(f)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Kt(){let g;try{g=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!g.ok)return null;let r;try{r=await g.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Gt=`
<div class="layout">
<aside class="sidebar">
...
`;function Qt(g){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},f=e=>"pill pill-"+(e||"none"),_='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',o=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),m=e=>/^https?:\/\//i.test(String(e??""))?o(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],M()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],x(),p.postingId){const t=r.jobs.find(n=>n.posting_id===p.postingId);t&&(p.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&F())}}function j(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",fe(),e==="jobs"?x():M()}async function I(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Ie()}function he(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[s]??3)-(i[a]??3)}return String(s).localeCompare(String(a))}function G(e){return e.slice().sort((t,n)=>r.sort.dir*he(t,n,r.sort.k))}const C=new Set;let k=!1;function B(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",C.has(e.dataset.v))})}function S(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(C.size&&!C.has(t.verdict||"__none__")||k&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const O=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],me=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Q(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const U=Q("scout-hidden-cols"),X=Q("scout-hidden-jcols");function je(){return r.view==="jobs"?{cols:me,hidden:X,key:"scout-hidden-jcols"}:{cols:O,hidden:U,key:"scout-hidden-cols"}}function Z(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=U.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=X.has(e.dataset.col)?"none":""})}function fe(){const e=je();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=je(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),fe(),Z()})})}function M(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=G(S());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
    ...
    `,e.appendChild(s)}Z(),e.querySelectorAll("tr").forEach(n=>{n.addEventListener("click",s=>{s.target.closest("a, .flag-btn")||se(n.dataset.id)})}),e.querySelectorAll(".flag-btn").forEach(n=>{n.addEventListener("click",()=>Ve(n.dataset.id))})}const A=new Set;let ee=!1,R=!0;function rt(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=R&&!A.has("rejected");return r.jobs.filter(n=>!(t&&n.response==="rejected"||A.size&&!A.has(n.response||"")||ee&&!n.next_up||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const ct=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function Ce(e){const t=String(e||"").split(",").map(n=>n.trim()).filter(Boolean);return t.length?t.map(n=>{const s=n.match(ct);return s?`<a href="mailto:${o(s[0])}" title="${o(n)}">${o(n)}</a>`:o(n)}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}const H={offer:{cls:"pill-yes",label:"offer",order:0},interview:{cls:"pill-info",label:"interview",order:1},screening:{cls:"pill-maybe",label:"screening call",order:2},"":{cls:"pill-none",label:"—",order:3},rejected:{cls:"pill-no",label:"rejected",order:4}};function dt(e,t,n){var s,a;if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[e.verdict]??3)-(i[t.verdict]??3)}if(n==="response")return(((s=H[e.response])==null?void 0:s.order)??3)-(((a=H[t.response])==null?void 0:a.order)??3);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="applied_at"||n==="last_outreach_at"){const i=e[n]||"",d=t[n]||"";return!i&&!d?0:i?d?String(d).localeCompare(String(i)):-r.jsort.dir:r.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function D(e){const t=e.options[e.selectedIndex],n=getComputedStyle(e),s=D._c||(D._c=document.createElement("canvas").getContext("2d"));s.font=`${n.fontWeight} ${n.fontSize} ${n.fontFamily}`;const a=s.measureText(t?t.text:"").width;e.style.width=Math.ceil(a+35)+"px"}function x(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=rt().sort((c,u)=>r.jsort.dir*dt(c,u,r.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=R&&!A.has("rejected")?r.jobs.filter(c=>c.response==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=r.jobs.filter(c=>c.next_up).length,i=document.getElementById("next-up-n");i.textContent=a,i.style.display=a?"":"none";const d=document.getElementById("jobs-hidden-note");d.style.display=n?"":"none",n&&(d.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{R=!1,document.getElementById("hide-rejected").classList.remove("is-on"),x()});for(const c of t){const u=H[c.response]||H[""],h=document.createElement("tr");h.dataset.id=c.posting_id;const y=[["","none"],["screening","screening"],["interview","interview"],["offer","offer"],["rejected","rejected"]].map(([b,T])=>`<option value="${b}"${(c.response||"")===b?" selected":""}>${T}</option>`).join("");h.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${c.next_up?" is-on":""}" title="${c.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${c.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${o(c.title||c.company)}</span>${lt(c.outreach_draft_status)}${c.title?`<div class="small dim">${o(c.company)}</div>`:""}</div></div></td>
      ...
      <td class="small td-contacts" data-col="contacts">${Ce(c.contacts)}</td>
      <td data-col="link"><a href="${m(c.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `;const v=()=>new Date().toISOString().slice(0,10);h.querySelector(".jt-nextup").onclick=()=>He(c,!1),h.querySelector(".jt-applied").onclick=()=>te(c,{applied_at:c.applied_at?"":v()}),h.querySelector(".jt-resp").onchange=b=>{D(b.target),te(c,{response:b.target.value})},h.querySelector(".jt-inc").onclick=()=>te(c,{outreach_count:(c.outreach_count||0)+1,last_outreach_at:v()}),h.querySelector(".jt-dec").onclick=()=>{const b=Math.max(0,(c.outreach_count||0)-1);te(c,{outreach_count:b,...b===0?{last_outreach_at:""}:{}})},e.appendChild(h)}e.querySelectorAll(".jt-resp").forEach(D),Z(),e.querySelectorAll("tr").forEach(c=>{c.addEventListener("click",u=>{u.target.closest("a, button, select")||Se(c.dataset.id)})})}function lt(e){return e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const p={postingId:null,row:null,drafts:[],poll:null,openHist:!1};async function Se(e){let t=r.jobs.find(n=>n.posting_id===e);if(t||(await E(),t=r.jobs.find(n=>n.posting_id===e)),!t){l("posting not found — refresh");return}ge(),p.postingId=e,p.row=t,p.drafts=[],p.openHist=!1,document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),F(),J()}function ve(){ge(),p.postingId=null,p.row=null,p.drafts=[],document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function ge(){p.poll&&(clearInterval(p.poll),p.poll=null)}async function J(){if(!p.postingId)return;let e;try{const n=await fetch(`/api/postings/${p.postingId}/outreach`);if(!n.ok){z();return}e=await n.json()}catch{z();return}p.drafts=e.drafts||[],z();const t=p.drafts[0];t&&t.status==="researching"?ut():ge()}function ut(){p.poll||(p.poll=setInterval(J,4e3))}function P(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(i){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),l(`save failed: ${i.message}`)}})}async function Me(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const i=await a.json();Object.assign(e,{title:i.title,location:i.location,summary:i.summary,employment_type:i.employment_type,workplace_type:i.workplace_type,department:i.department,comp_range:i.comp_range,description:i.description}),x()}async function Ae(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const i=await a.json();Object.assign(e,{name:i.name,headcount:i.headcount,funding_stage:i.funding_stage,location:i.location,vertical:i.vertical}),$(),E()}async function pt(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();r.openId=s.company_id,W(s),Y(s.company_id),$(),E()}async function ht(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}function F(){const e=p.row;if(!e)return;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${o(e.title||"")}">`;const t=H[e.response]||H[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${t.cls}">${o(t.label)}</span>`+(e.verdict?` <span class="${f(e.verdict)}">${o(e.verdict)}</span>`:""),document.getElementById("pursuit-body").innerHTML=`
  ...
  `,ft();const n=document.getElementById("pursuit-company-link");n&&n.addEventListener("click",()=>se(e.company_id)),P(document.getElementById("pursuit-title-input"),s=>Me(e,"title",s)),P(document.getElementById("pursuit-notes-input"),s=>vt(s),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(s=>P(s,a=>Me(e,s.dataset.k,a),{multiline:s.tagName==="TEXTAREA"})),z()}function mt(e){return`
    ...
    </div>`}function ft(){const e=p.row,t=()=>new Date().toISOString().slice(0,10),n=document.querySelector("#pursuit-body .pt-applied");n&&n.addEventListener("click",()=>V({applied_at:e.applied_at?"":t()}));const s=document.querySelector("#pursuit-body .pl-applied-date");s&&s.addEventListener("change",d=>V({applied_at:d.target.value}));const a=document.querySelector("#pursuit-body .pl-response");a&&a.addEventListener("change",d=>V({response:d.target.value}));const i=document.querySelector("#pursuit-body .pt-nextup");i&&i.addEventListener("click",()=>He(p.row,!0))}async function He(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){l(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();l(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,x(),t&&F(),l(e.next_up?"queued next up":"removed from the queue")}async function Pe(e,t){const n={applied_at:e.applied_at||"",response:e.response||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",contacts:e.contacts||"",notes:e.notes||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(i){return l(`save failed: ${i.message}`),null}if(!s.ok){const i=(await s.text().catch(()=>"")).trim();return l(`save failed: ${i||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),a}async function vt(e){const t=p.row,n={applied_at:t.applied_at||"",response:t.response||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",contacts:t.contacts||"",notes:e},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,x()}async function V(e){await Pe(p.row,e)&&(x(),F(),l("tracking saved"))}async function te(e,t){await Pe(e,t)&&(x(),p.postingId===e.posting_id&&(p.row=e,F()),l("tracking saved"))}function z(){const e=document.getElementById("outreach-section");if(!e)return;const t=p.row,n=p.drafts,s=n[0]||null,a=n.slice(1),i=`
    ...
    </div>`,c=s&&(gt(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    <details class="draft-history" ${p.openHist?"open":""}>
      <summary>${a.length} earlier draft${a.length>1?"s":""}</summary>
      <div id="draft-history-body">${a.map(h=>qe(h,!0)).join("")}</div>
    </details>`:"";e.innerHTML=i+`<div id="draft-current">${s?qe(s,!1):""}</div><div class="draft-actions">${c}</div>`+u,bt()}function gt(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function qe(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(y,v)=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const y=yt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      <div class="draft-sentbody">${o(Ne(e)||"(empty)")}</div>
      ${ne(e)}
    </div>`;const a=Ne(e),i=e.status==="no_hook",d=i?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let c="";if(i)try{c=JSON.parse(e.hook||"{}").reasoning||""}catch{}const u=i?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${c?" "+o(c):""}</div>`:"";if(t)return`<div class="draft-card ${i?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    ${ne(e)}
  </div>`}function Ne(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function ne(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,h)=>h?`<div class="tr-line"><span class="tr-key">${u}:</span> ${o(String(h))}</div>`:"",i=n.role||{},d=Array.isArray(n.hooks)?n.hooks:[],c=d.map(u=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function Oe(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${o(n.message||"")}"><code>${o(n.code||"")}</code>${o(n.message||"")}</span>`).join("")+"</div>":""}function yt(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${o(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${o(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function bt(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector(".oc-input");t&&(t.addEventListener("change",u=>V({contacts:u.target.value.trim()})),t.addEventListener("keydown",u=>{u.key==="Enter"&&(u.preventDefault(),u.target.blur())}));const n=()=>new Date().toISOString().slice(0,10),s=p.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>V({outreach_count:(s.outreach_count||0)+1,last_outreach_at:n()}));const i=e.querySelector(".pt-outreach-dec");i&&i.addEventListener("click",()=>{const u=Math.max(0,(s.outreach_count||0)-1);V({outreach_count:u,...u===0?{last_outreach_at:""}:{}})});const d=e.querySelector("#draft-start-btn");d&&d.addEventListener("click",Re),e.querySelectorAll(".draft-retry-btn").forEach(u=>u.addEventListener("click",Re)),e.querySelectorAll(".draft-card[data-did]").forEach(u=>{const h=u.dataset.did,y=u.querySelector(".draft-save-btn");y&&y.addEventListener("click",()=>Et(h));const v=u.querySelector(".draft-sent-btn");v&&v.addEventListener("click",()=>$t(h))});const c=e.querySelector("details.draft-history");c&&c.addEventListener("toggle",()=>{p.openHist=c.open})}async function Re(){const e=document.getElementById("outreach-section"),t=e&&(e.querySelector("#draft-start-btn")||e.querySelector(".draft-retry-btn"));t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${p.postingId}/outreach`,{method:"POST"})}catch(a){l(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await J();return}if(n.status===409){await J(),l("a draft is already active");return}if(n.status===412){let a={};try{a=await n.json()}catch{}wt(a.missing_blocks||[]),t&&(t.disabled=!1);return}if(n.status===503){const a=document.getElementById("outreach-section");if(a){const i=document.createElement("div");i.className="draft-note",i.textContent="Outreach engine not running in this build.",a.appendChild(i)}t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();l(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}function wt(e){const t=document.getElementById("outreach-section");if(!t)return;const n=t.querySelector(".draft-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    <button class="btn btn-primary" id="blocks-sync-btn">Sync blocks</button>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#blocks-sync-btn");a&&a.addEventListener("click",kt)}async function kt(){const e=document.getElementById("blocks-sync-btn");e&&(e.disabled=!0,e.textContent="Syncing…");let t;try{t=await fetch("/api/outreach/sync",{method:"POST"})}catch(n){l(`sync failed: ${n.message}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`sync failed: ${n||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}l("blocks synced"),z()}async function Et(e){const t=document.getElementById(`draft-edit-${e}`);if(!t)return;let n;try{n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t.value})})}catch(d){l(`save failed: ${d.message}`);return}if(!n.ok){const d=(await n.text().catch(()=>"")).trim();l(`save failed: ${d||"HTTP "+n.status}`);return}const s=await n.json(),a=p.drafts.findIndex(d=>String(d.id)===String(e));a>=0&&(p.drafts[a]=s);const i=t.closest(".draft-card");if(i){const d=i.querySelector(".lint-chips"),c=Oe(s.lint);d?d.outerHTML=c||"":c&&t.insertAdjacentHTML("afterend",c)}l("saved")}async function $t(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(n){l(`failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`failed: ${n||"HTTP "+t.status}`);return}l("marked sent"),await J(),E()}async function se(e){r.openId=e;const t=document.getElementById("pane"),n=document.getElementById("scrim");t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';let s;try{const a=await fetch(`/api/companies/${e}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);s=await a.json()}catch(a){document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${o(a.message)}</div>`;return}W(s),Y(e)}function ae(){r.openId=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function W(e){document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${o(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `:'<div class="muted">No enrichment yet. Run <code>scout enrich</code>.</div>',i=!r.meta||r.meta.control!==!1,d=i&&r.meta&&r.meta.verdict?'<button class="h3-action" id="rescore-btn" title="re-score just this company — replaces the current verdict, manual or not">↻ re-score</button>':"",c=i&&e.domain?'<button class="h3-action" id="reenrich-btn" title="re-fetch this company’s site now">↻ re-enrich</button>':"",u=Object.keys(e.raw_json||{}).sort(),h=u.length===0?"":`
    <details class="raw-json">
      <summary>Raw row <span class="dim">(${u.length} fields)</span></summary>
      <table><tbody>
        ${u.map(w=>`<tr><td class="k">${o(w)}</td><td>${o(e.raw_json[w])}</td></tr>`).join("")}
      </tbody></table>
    </details>
  `,y=`
    <div class="flag-bar">
  ...
  `;const v=document.getElementById("posting-add-btn");v&&v.addEventListener("click",()=>Bt(e)),De(),document.querySelectorAll("#ve-pick .ve-opt").forEach(w=>{w.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Te=>Te.classList.remove("is-on")),w.classList.add("is-on")})});const b=document.getElementById("ve-save-btn");b&&b.addEventListener("click",()=>It(e)),P(document.getElementById("pane-title-input"),w=>Ae(e,"name",w)),document.querySelectorAll("#facts-body [data-k]").forEach(w=>P(w,Te=>Ae(e,w.dataset.k,Te))),P(document.getElementById("pane-domain-input"),w=>pt(e,w)),P(document.getElementById("pane-notes-input"),w=>ht(e,w),{multiline:!0});const T=document.getElementById("flag-toggle-btn");T&&T.addEventListener("click",()=>Ve(e.company_id));const at=document.getElementById("review-stamp-btn");at&&at.addEventListener("click",()=>xt(e.company_id));const ot=document.getElementById("rescore-btn");ot&&ot.addEventListener("click",()=>be("verdict",{company_ids:[e.company_id]}));const it=document.getElementById("reenrich-btn");it&&it.addEventListener("click",()=>be("enrich",{company_ids:[e.company_id]}))}function _t(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function xt(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(i){l(`failed: ${i.message}`),t&&(t.disabled=!1);return}if(!n.ok){const i=await n.text().catch(()=>"");l(`failed: HTTP ${n.status}${i?" — "+i:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=r.rows.find(i=>i.company_id===e);a&&(a.reviewed_at=s.reviewed_at,M()),r.openId===e&&(W(s),Y(e)),l("reviewed")}async function Ve(e){const t=r.rows.find(i=>i.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(i){l(`failed: ${i.message}`);return}if(!s.ok){const i=await s.text().catch(()=>"");l(`failed: HTTP ${s.status}${i?" — "+i:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,M()),r.openId===e&&(W(a),Y(e)),E(),l(a.flagged?"flagged":"unflagged")}async function It(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){l("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(c){l(`save failed: ${c.message}`),a.disabled=!1;return}if(!i.ok){const c=await i.text().catch(()=>"");l(`save failed: HTTP ${i.status}${c?" — "+c:""}`),a.disabled=!1;return}const d=await i.json();W(d),Y(d.company_id),$(),I(),E(),l("verdict saved")}function Ue(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(o).join(" · "),a=H[n.response]||H[""],i=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a.cls}">${o(a.label)}</span>`,`<span class="pt-meta">${n.applied_at?`applied ${o(n.applied_at)}`:"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${o(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function De(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||(ae(),Se(e.dataset.pid))})})}async function Bt(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!i.ok){const u=await i.text().catch(()=>"");l(`add failed: HTTP ${i.status}${u?" — "+u:""}`),s.disabled=!1;return}const d=await i.json();e.postings=(e.postings||[]).filter(u=>u.id!==d.id),e.postings.unshift(d);const c=document.getElementById("postings-list");c&&(c.innerHTML=Ue(e),De()),t.value="",n.value="",s.disabled=!1,E(),l("link added")}async function Y(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){oe(`<div class="muted">Failed to load trail: ${o(s.message)}</div>`);return}if(!t.ok){oe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){oe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}oe(n.map(Lt).join(""))}function Lt(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(o);return e.run_id&&t.push("run "+o(e.run_id.slice(0,8))),`
    <div class="trail-event">
    ...
    </div>`}function oe(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let Je;function l(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(Je),Je=setTimeout(()=>t.classList.remove("show"),2200)}r.meta={control:!1,brain:!1,verdict:!1};async function Tt(){try{const n=await fetch("/api/meta");if(!n.ok)return;r.meta=await n.json()}catch{return}const e=r.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!r.meta.verdict,t.title=r.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable"}async function ye(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let ie=null;async function be(e,t){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){l(`run failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(n.status===412){const a=await n.text();l(a.trim());return}if(!n.ok){l(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Qe(e,s)}async function jt(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){l(`upload failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(!n.ok){l(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Qe("ingest",s)}const Ct=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let re=[],q=new Set,N="company";function Fe(e){N=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",ze()}function we(){return!!r.meta.capture&&document.getElementById("add-enrich").checked}function ze(){const e=document.getElementById("add-note");we()?e.innerHTML=N==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=N==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function St(){Ct.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",q=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!r.meta.capture,t.classList.toggle("disabled",!r.meta.capture),t.title=r.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",r.meta.capture||(e.checked=!1),Fe(r.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(r.rows||[]).map(s=>`<option value="${o(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const i=document.createElement("option");i.value=a,i.textContent=a,n.appendChild(i)}),re=s.verticals||[]}catch{re=[]}We()}function ce(){document.getElementById("add-scrim").classList.remove("open")}function We(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=re.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${q.has(s)?" sel":""}" data-v="${o(s)}">${o(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;q.has(a)?q.delete(a):q.add(a),s.classList.toggle("sel"),Ye()}))):e.innerHTML=`<div class="none">${re.length?"no match":"no verticals in the set yet"}</div>`,Ye()}function Ye(){const e=q.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Ke(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Ge(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){l(N==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,we()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},i=v=>document.getElementById(v).value.trim(),d=we();let c,u;d?(c="/api/capture",u={url:Ke(t),kind:N==="company"?"company_page":"job_posting",fields:N==="company"?{name:i("add-name"),location:i("add-location"),headcount:i("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...q].join(", ")}:{name:i("add-job-company"),title:i("add-title")}}):N==="company"?(c="/api/companies",u={website:t,name:i("add-name"),vertical:[...q].join(", "),location:i("add-location"),headcount:i("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(c="/api/postings",u={url:Ke(t),title:i("add-title"),company:i("add-job-company")});let h;try{h=await fetch(c,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(v){l(`add failed: ${v.message}`),a();return}if(!h.ok){let v=`HTTP ${h.status}`;try{const b=await h.text();try{v=JSON.parse(b).error||v}catch{v=b.trim()||v}}catch{}if(a(),h.status===409){l(v||"That company is already in the list."),e.focus(),e.select();return}l(`add failed: ${v}`);return}const y=await h.json();if(a(),d&&!y.company_id){l(y.note||"couldn't classify that page");return}if(ce(),$(),I(),E(),N==="job"){const v=y.posting&&y.posting.title||"job link";l(`tracking: ${v} @ ${y.company_name}${y.posting_updated?" (refreshed)":""}`),j("jobs")}else d?(l(y.company_created?`company added: ${y.company_name}`:`${y.company_name} is already in the list`),se(y.company_id)):l("company added")}function Qe(e,t){ie=t;const n=document.getElementById("drawer"),s=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",s.innerHTML="",n.classList.add("open"),ye();const a=new EventSource(`/api/jobs/${t}/stream`),i=(d,c)=>{const u=document.createElement("div"),h=!c&&/^\s*warn:/i.test(d);u.className="ln"+(c?" ln-err":h?" ln-warn":""),u.textContent=h?d.replace(/^\s*warn:\s*/i,"⚠ "):d,s.appendChild(u),s.scrollTop=s.scrollHeight};a.addEventListener("line",d=>i(d.data,/error|failed/i.test(d.data))),a.addEventListener("end",d=>{a.close(),ie=null,i(`— ${d.data} —`,d.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",l(`${e} ${d.data}`),$(),I(),ye(),E(),r.openId&&se(r.openId)}),a.onerror=()=>{a.close()}}async function Mt(){if(ie)try{await fetch(`/api/jobs/${ie}/cancel`,{method:"POST"})}catch{}}let K=null;async function Xe(e){K=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+e+".md",document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="",t.classList.add("open");try{const s=await(await fetch(`/api/${e}`)).json();document.getElementById("editor-text").value=s.content||"",s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version)}catch(n){document.getElementById("editor-text").value="failed to load: "+n.message}}function de(){document.getElementById("editor-scrim").classList.remove("open"),K=null}const ke=["subject_name","signature","lens","hook_prefs","arc"];let Ze=!1;async function At(){document.getElementById("sender-scrim").classList.add("open");let t={};try{t=await(await fetch("/api/outreach/sender")).json()}catch(n){l(`failed to load identity: ${n.message}`)}for(const n of ke){const s=document.getElementById("snd-"+n);s&&(s.value=t[n]||"")}if(!Ze){for(const n of ke){const s=document.getElementById("snd-"+n);P(s,a=>Ht(n,a),{multiline:s&&s.tagName==="TEXTAREA"})}Ze=!0}}async function Ht(e,t){const n={};for(const a of ke){const i=document.getElementById("snd-"+a);n[a]=i?i.value:""}n[e]=t;const s=await fetch("/api/outreach/sender",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status)}function Ee(){document.getElementById("sender-scrim").classList.remove("open")}async function Pt(){if(!K)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${K}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(s){l(`save failed: ${s.message}`);return}if(!t.ok){l(`save failed: HTTP ${t.status}`);return}const n=await t.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version),l(`${K}.md saved`),de(),I()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),M()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),x()}}),document.getElementById("tab-companies").onclick=()=>j("companies"),document.getElementById("tab-jobs").onclick=()=>j("jobs"),document.getElementById("q").oninput=M,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;C.has(t)?C.delete(t):C.add(t),B(),M()})}),document.getElementById("flag-filter").addEventListener("click",e=>{k=!k,e.currentTarget.classList.toggle("is-on",k),M()}),document.getElementById("jq").oninput=x,document.getElementById("hide-rejected").addEventListener("click",e=>{R=!R,e.currentTarget.classList.toggle("is-on",R),x()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;A.has(t)?A.delete(t):A.add(t),e.classList.toggle("is-on",A.has(t)),x()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{ee=!ee,e.currentTarget.classList.toggle("is-on",ee),x()}),fe(),Z(),document.getElementById("pane-close").onclick=ae,document.getElementById("scrim").onclick=ae,document.getElementById("pursuit-close").onclick=ve,document.getElementById("pursuit-scrim").onclick=ve,document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(Vt()){Le();return}if(document.getElementById("profile-scrim").classList.contains("open")){Be();return}if(document.getElementById("add-scrim").classList.contains("open")){ce();return}if(document.getElementById("run-scrim").classList.contains("open")){le();return}if(document.getElementById("help-scrim").classList.contains("open")){ue();return}if(document.getElementById("pane").classList.contains("open")){ae();return}if(document.getElementById("pursuit-pane").classList.contains("open")){ve();return}if(document.getElementById("sender-scrim").classList.contains("open")){Ee();return}document.getElementById("editor-scrim").classList.contains("open")&&de()}});let $e=null;const qt={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function et(e){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}$e=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=qt[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=r.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function le(){document.getElementById("run-scrim").classList.remove("open"),$e=null}document.getElementById("btn-enrich").onclick=()=>et("enrich"),document.getElementById("btn-verdict").onclick=()=>et("verdict"),document.getElementById("run-cancel").onclick=le,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&le()},document.getElementById("run-go").onclick=()=>{const e=$e,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(le(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),be(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=St;const Nt={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function tt(e){const t=Nt[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const i=document.createElement("div");i.className="help-item-name",i.textContent=s.name;const d=document.createElement("div");d.className="help-item-desc",d.textContent=s.desc;const c=document.createElement("a");c.className="help-link",c.textContent="Learn more →",c.onclick=()=>{ue(),st(),Ut(s.sec)},a.appendChild(i),a.appendChild(d),a.appendChild(c),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function ue(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>tt("add"),document.getElementById("help-run").onclick=()=>tt("run"),document.getElementById("help-close").onclick=ue,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&ue()},document.getElementById("add-cancel").onclick=ce,document.getElementById("add-save").onclick=Ge,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&ce()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Fe(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",ze),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Ge()))}),document.getElementById("add-vertical-filter").addEventListener("input",We),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&jt(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=Mt,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=de,document.getElementById("editor-save").onclick=Pt,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&de()},document.getElementById("sender-close").onclick=Ee,document.getElementById("sender-scrim").onclick=e=>{e.target.id==="sender-scrim"&&Ee()};function nt(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function _e(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}Ie()}const xe='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>';function Ie(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,n=r.stats&&r.stats.stale_verdicts||0,a=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),i=t&&typeof t.body=="string";let d="";if(a){let b="off",T="";t&&!t.reachable&&i?(b="warn",T="brain offline · using cache"):t&&t.stale?(b="warn",T="cached · stale"):i&&(b="ok",T="fetched "+nt(t.age_seconds)),d+=`<div class="crit-row">
      ...
      </span></div>`,T&&(d+=`<div class="crit-note dim small">${o(T)}</div>`)}else d+=`<div class="crit-row">
      <span class="crit-what">taste</span>
      <button class="crit-edit" id="edit-taste" title="edit taste.md" aria-label="edit taste">${xe}</button></div>`,t&&t.configured&&(d+='<div class="crit-note dim small">brain offline — local fallback</div>');d+=`<div class="crit-row">
    <span class="crit-what">playbook</span>
    <button class="crit-edit" id="edit-playbook" title="edit playbook.md" aria-label="edit playbook">${xe}</button></div>`,d+=`<div class="crit-row">
    <span class="crit-what">outreach identity</span>
    <button class="crit-edit" id="edit-sender" title="edit outreach identity" aria-label="edit outreach identity">${xe}</button></div>`,n>0&&(d+=`<div class="stale">
      ...
      ${n} verdicts stale</div>`),e.innerHTML=d;const c=document.getElementById("view-profile");c&&(c.onclick=()=>Rt(r.profile));const u=document.getElementById("refresh-profile");u&&(u.onclick=Ot);const h=document.getElementById("edit-taste");h&&(h.onclick=()=>Xe("taste"));const y=document.getElementById("edit-playbook");y&&(y.onclick=()=>Xe("playbook"));const v=document.getElementById("edit-sender");v&&(v.onclick=At)}async function Ot(){const e=document.getElementById("refresh-profile");e&&(e.textContent="refreshing…",e.style.pointerEvents="none");let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),_e();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),_e();return}r.profile=await t.json(),Ie(),l("brain profile refreshed"),I()}function Rt(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${nt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Be(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Be,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Be()};function st(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");pe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Le(){document.getElementById("docs-scrim").classList.remove("open")}function Vt(){return document.getElementById("docs-scrim").classList.contains("open")}function pe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Ut(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),pe(e)}document.getElementById("open-docs").onclick=st,document.getElementById("docs-close").onclick=Le,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Le()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),pe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,i)=>a.boundingClientRect.top-i.boundingClientRect.top);s.length&&pe(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),$(),E(),I(),Tt(),ye(),_e()}Ft({"":{view:()=>({mount(g){g.innerHTML=Gt,Qt()}}),chrome:!1}},{title:"scout"});Yt();Kt();
