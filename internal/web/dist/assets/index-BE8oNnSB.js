(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))x(o);new MutationObserver(o=>{for(const m of o)if(m.type==="childList")for(const $ of m.addedNodes)$.tagName==="LINK"&&$.rel==="modulepreload"&&x($)}).observe(document,{childList:!0,subtree:!0});function f(o){const m={};return o.integrity&&(m.integrity=o.integrity),o.referrerPolicy&&(m.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?m.credentials="include":o.crossOrigin==="anonymous"?m.credentials="omit":m.credentials="same-origin",m}function x(o){if(o.ep)return;o.ep=!0;const m=f(o);fetch(o.href,m)}})();function Ut(g,r){const f=r.replace(/^#/,"");let x=null;for(const o of Object.keys(g))(f===o||f.startsWith(o))&&(x===null||o.length>x.length)&&(x=o);return x===null&&""in g&&(x=""),x}function Dt(g){return typeof g=="function"?{view:g,chrome:!0}:{view:g.view,chrome:g.chrome!==!1}}function Jt(g,r={}){const f=r.root??document.body,x=r.title??document.title??"",o=r.brandHref??"#",m=document.createElement("main"),$=document.createElement("header");$.className="cap-head";const E=document.createElement("a");E.className="brand",E.href=o,E.textContent=x,E.setAttribute("aria-label",`${x} — home`),$.appendChild(E);const j=document.createElement("nav");j.className="cap-nav",j.setAttribute("aria-label","Views");for(const k of r.nav??[]){const B=document.createElement("a");B.href=k.href,B.textContent=k.label,k.ariaLabel&&B.setAttribute("aria-label",k.ariaLabel),j.appendChild(B)}$.appendChild(j);const I=document.createElement("section");I.className="tk-content",m.appendChild($),m.appendChild(I);const L=document.createElement("div");L.className="tk-bleed";const he=k=>{var B;for(const S of Array.from(j.querySelectorAll("a"))){const N=((B=S.getAttribute("href"))==null?void 0:B.replace(/^#/,""))??"";S.toggleAttribute("aria-current",k!==null&&k!==""&&N===k),S.hasAttribute("aria-current")&&S.setAttribute("aria-current","page")}};let G=0;const C=()=>{const k=Ut(g,location.hash);if(he(k),k===null){L.isConnected&&L.remove(),m.isConnected||f.appendChild(m),Ft(I,"Not found.");return}const{view:B,chrome:S}=Dt(g[k]),N=S?I:L;S?(L.isConnected&&L.remove(),m.isConnected||f.appendChild(m)):(m.isConnected&&m.remove(),L.isConnected||f.appendChild(L)),N.replaceChildren();const me=B(),Q=++G,U=me.mount(N);U instanceof Promise&&U.catch(X=>{Q===G&&zt(N,String(X))})};window.addEventListener("hashchange",C),C()}function Ft(g,r){g.replaceChildren();const f=document.createElement("div");f.className="tk-empty",f.textContent=r,g.appendChild(f)}function zt(g,r){g.replaceChildren();const f=document.createElement("div");f.className="tk-error",f.textContent=r,g.appendChild(f)}function Wt(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const f of r)f.unregister()}),window.caches&&caches.keys().then(r=>{for(const f of r)caches.delete(f)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Yt(){let g;try{g=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!g.ok)return null;let r;try{r=await g.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Kt=`
<div class="layout">
<aside class="sidebar">
...
`;function Gt(g){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},f=e=>"pill pill-"+(e||"none"),x='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',o=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),m=e=>/^https?:\/\//i.test(String(e??""))?o(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],M()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],_(),p.postingId){const t=r.jobs.find(n=>n.posting_id===p.postingId);t&&(p.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&F())}}function j(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",fe(),e==="jobs"?_():M()}async function I(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Ie()}function he(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[s]??3)-(i[a]??3)}return String(s).localeCompare(String(a))}function G(e){return e.slice().sort((t,n)=>r.sort.dir*he(t,n,r.sort.k))}const C=new Set;let k=!1;function B(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",C.has(e.dataset.v))})}function S(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(C.size&&!C.has(t.verdict||"__none__")||k&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const N=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],me=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Q(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const U=Q("scout-hidden-cols"),X=Q("scout-hidden-jcols");function je(){return r.view==="jobs"?{cols:me,hidden:X,key:"scout-hidden-jcols"}:{cols:N,hidden:U,key:"scout-hidden-cols"}}function Z(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=U.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=X.has(e.dataset.col)?"none":""})}function fe(){const e=je();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=je(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),fe(),Z()})})}function M(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=G(S());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
  ...
  `,ft();const n=document.getElementById("pursuit-view-company");n&&n.addEventListener("click",()=>se(e.company_id)),O(document.getElementById("pursuit-title-input"),s=>Me(e,"title",s)),document.querySelectorAll("#role-body [data-k]").forEach(s=>O(s,a=>Me(e,s.dataset.k,a),{multiline:s.tagName==="TEXTAREA"})),z()}function mt(e){return`
    ...
    </div>`,c=s&&(vt(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    ...
    </details>`:"";e.innerHTML=i+`<div id="draft-current">${s?qe(s,!1):""}</div><div class="draft-actions">${c}</div>`+u,yt()}function vt(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function qe(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(y,v)=>`
    ...
    </div>`;if(e.status==="failed"){const y=gt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
  ...
  `;const v=document.getElementById("posting-add-btn");v&&v.addEventListener("click",()=>It(e)),De(),document.querySelectorAll("#ve-pick .ve-opt").forEach(w=>{w.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Te=>Te.classList.remove("is-on")),w.classList.add("is-on")})});const b=document.getElementById("ve-save-btn");b&&b.addEventListener("click",()=>_t(e)),O(document.getElementById("pane-title-input"),w=>Ae(e,"name",w)),document.querySelectorAll("#facts-body [data-k]").forEach(w=>O(w,Te=>Ae(e,w.dataset.k,Te))),O(document.getElementById("pane-domain-input"),w=>pt(e,w)),O(document.getElementById("pane-notes-input"),w=>ht(e,w),{multiline:!0});const T=document.getElementById("flag-toggle-btn");T&&T.addEventListener("click",()=>Ve(e.company_id));const at=document.getElementById("review-stamp-btn");at&&at.addEventListener("click",()=>xt(e.company_id));const ot=document.getElementById("rescore-btn");ot&&ot.addEventListener("click",()=>be("verdict",{company_ids:[e.company_id]}));const it=document.getElementById("reenrich-btn");it&&it.addEventListener("click",()=>be("enrich",{company_ids:[e.company_id]}))}function $t(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function De(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||(ae(),Se(e.dataset.pid))})})}async function It(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!i.ok){const u=await i.text().catch(()=>"");l(`add failed: HTTP ${i.status}${u?" — "+u:""}`),s.disabled=!1;return}const d=await i.json();e.postings=(e.postings||[]).filter(u=>u.id!==d.id),e.postings.unshift(d);const c=document.getElementById("postings-list");c&&(c.innerHTML=Ue(e),De()),t.value="",n.value="",s.disabled=!1,E(),l("link added")}async function Y(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){oe(`<div class="muted">Failed to load trail: ${o(s.message)}</div>`);return}if(!t.ok){oe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){oe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}oe(n.map(Bt).join(""))}function Bt(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(o);return e.run_id&&t.push("run "+o(e.run_id.slice(0,8))),`
    <div class="trail-event">
      ...
      <button class="crit-edit" id="edit-taste" title="edit taste.md" aria-label="edit taste">${_e}</button></div>`,t&&t.configured&&(d+='<div class="crit-note dim small">brain offline — local fallback</div>');d+=`<div class="crit-row">
    <span class="crit-what">playbook</span>
    <button class="crit-edit" id="edit-playbook" title="edit playbook.md" aria-label="edit playbook">${_e}</button></div>`,d+=`<div class="crit-row">
    <span class="crit-what">outreach identity</span>
    <button class="crit-edit" id="edit-sender" title="edit outreach identity" aria-label="edit outreach identity">${_e}</button></div>`,n>0&&(d+=`<div class="stale">
      ...
      ${n} verdicts stale</div>`),e.innerHTML=d;const c=document.getElementById("view-profile");c&&(c.onclick=()=>Ot(r.profile));const u=document.getElementById("refresh-profile");u&&(u.onclick=Nt);const h=document.getElementById("edit-taste");h&&(h.onclick=()=>Xe("taste"));const y=document.getElementById("edit-playbook");y&&(y.onclick=()=>Xe("playbook"));const v=document.getElementById("edit-sender");v&&(v.onclick=Mt)}async function Nt(){const e=document.getElementById("refresh-profile");e&&(e.textContent="refreshing…",e.style.pointerEvents="none");let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),xe();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),xe();return}r.profile=await t.json(),Ie(),l("brain profile refreshed"),I()}function Ot(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${nt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Be(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Be,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Be()};function st(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");pe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Le(){document.getElementById("docs-scrim").classList.remove("open")}function Rt(){return document.getElementById("docs-scrim").classList.contains("open")}function pe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Vt(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),pe(e)}document.getElementById("open-docs").onclick=st,document.getElementById("docs-close").onclick=Le,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Le()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),pe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,i)=>a.boundingClientRect.top-i.boundingClientRect.top);s.length&&pe(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),$(),E(),I(),Lt(),ye(),xe()}Jt({"":{view:()=>({mount(g){g.innerHTML=Kt,Gt()}}),chrome:!1}},{title:"scout"});Wt();Yt();
