(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))_(i);new MutationObserver(i=>{for(const f of i)if(f.type==="childList")for(const E of f.addedNodes)E.tagName==="LINK"&&E.rel==="modulepreload"&&_(E)}).observe(document,{childList:!0,subtree:!0});function y(i){const f={};return i.integrity&&(f.integrity=i.integrity),i.referrerPolicy&&(f.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?f.credentials="include":i.crossOrigin==="anonymous"?f.credentials="omit":f.credentials="same-origin",f}function _(i){if(i.ep)return;i.ep=!0;const f=y(i);fetch(i.href,f)}})();function Jn(b,r){const y=r.replace(/^#/,"");let _=null;for(const i of Object.keys(b))(y===i||y.startsWith(i))&&(_===null||i.length>_.length)&&(_=i);return _===null&&""in b&&(_=""),_}function zn(b){return typeof b=="function"?{view:b,chrome:!0}:{view:b.view,chrome:b.chrome!==!1}}function Fn(b,r={}){const y=r.root??document.body,_=r.title??document.title??"",i=r.brandHref??"#",f=document.createElement("main"),E=document.createElement("header");E.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=_,w.setAttribute("aria-label",`${_} — home`),E.appendChild(w);const q=document.createElement("nav");q.className="cap-nav",q.setAttribute("aria-label","Views");for(const B of r.nav??[]){const j=document.createElement("a");j.href=B.href,j.textContent=B.label,B.ariaLabel&&j.setAttribute("aria-label",B.ariaLabel),q.appendChild(j)}E.appendChild(q);const S=document.createElement("section");S.className="tk-content",f.appendChild(E),f.appendChild(S);const C=document.createElement("div");C.className="tk-bleed";const Ce=B=>{var j;for(const A of Array.from(q.querySelectorAll("a"))){const L=((j=A.getAttribute("href"))==null?void 0:j.replace(/^#/,""))??"";A.toggleAttribute("aria-current",B!==null&&B!==""&&L===B),A.hasAttribute("aria-current")&&A.setAttribute("aria-current","page")}};let z=0;const M=()=>{const B=Jn(b,location.hash);if(Ce(B),B===null){C.isConnected&&C.remove(),f.isConnected||y.appendChild(f),Wn(S,"Not found.");return}const{view:j,chrome:A}=zn(b[B]),L=A?S:C;A?(C.isConnected&&C.remove(),f.isConnected||y.appendChild(f)):(f.isConnected&&f.remove(),C.isConnected||y.appendChild(C)),L.replaceChildren();const F=j(),je=++z,de=F.mount(L);de instanceof Promise&&de.catch(Se=>{je===z&&Kn(L,String(Se))})};window.addEventListener("hashchange",M),M()}function Wn(b,r){b.replaceChildren();const y=document.createElement("div");y.className="tk-empty",y.textContent=r,b.appendChild(y)}function Kn(b,r){b.replaceChildren();const y=document.createElement("div");y.className="tk-error",y.textContent=r,b.appendChild(y)}function Yn(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const y of r)y.unregister()}),window.caches&&caches.keys().then(r=>{for(const y of r)caches.delete(y)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Gn(){let b;try{b=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!b.ok)return null;let r;try{r=await b.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Qn=`
<div class="layout">
<aside class="sidebar">
...
`;function Zn(b){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null},y=e=>"pill pill-"+(e||"none"),_='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function E(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],N()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}r.jobs=e.rows||[],x(),q(),C()}function q(){if(!h.postingId)return;const e=r.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&Z())}let S=null;function C(){const e=r.jobs.some(t=>t.outreach_draft_status==="researching");e&&!S?S=setInterval(Ce,4e3):!e&&S&&(clearInterval(S),S=null)}async function Ce(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(r.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==r.jobs.length;r.jobs=t,s&&(x(),q()),C()}function z(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Me(),e==="jobs"?x():N()}async function M(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,B()}function B(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Y()}function j(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function A(e){return e.slice().sort((t,n)=>r.sort.dir*j(t,n,r.sort.k))}const L=new Set;let F=!1;function je(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",L.has(e.dataset.v))})}function de(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(L.size&&!L.has(t.verdict||"__none__")||F&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Se=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Pt=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function tt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const nt=tt("scout-hidden-cols"),st=tt("scout-hidden-jcols");function at(){return r.view==="jobs"?{cols:Pt,hidden:st,key:"scout-hidden-jcols"}:{cols:Se,hidden:nt,key:"scout-hidden-cols"}}function le(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=nt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=st.has(e.dataset.col)?"none":""})}function Me(){const e=at();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=at(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),Me(),le()})})}function N(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=A(de());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
    ...
    `;const T=()=>new Date().toISOString().slice(0,10);g.querySelector(".jt-nextup").onclick=()=>lt(d,!1),g.querySelector(".jt-applied").onclick=()=>me(d,{applied_at:d.applied_at?"":T()}),g.querySelector(".jt-resp").onchange=$=>{G($.target),me(d,{response:$.target.value})},g.querySelector(".jt-inc").onclick=()=>me(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:T()}),g.querySelector(".jt-dec").onclick=()=>{const $=Math.max(0,(d.outreach_count||0)-1);me(d,{outreach_count:$,...$===0?{last_outreach_at:""}:{}})},e.appendChild(g)}e.querySelectorAll(".jt-resp").forEach(G),le(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",m=>{m.target.closest("a, button, select")||it(d.dataset.id)})})}function Rt(e){return e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="needs_work"?'<span class="draft-badge db-needswork" title="the draft finished below the depth bar — review, fix or regenerate">draft needs work</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function it(e){let t=r.jobs.find(n=>n.posting_id===e);if(t||(await w(),t=r.jobs.find(n=>n.posting_id===e)),!t){l("posting not found — refresh");return}He(),qe(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),rt("pursuit"),Z(),Q(),ee()}let Ae=null;function rt(e){Ae=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function he(){He(),qe(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function He(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function Q(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){fe();return}e=await n.json()}catch{fe();return}h.drafts=e.drafts||[],fe();const t=h.drafts[0];t&&t.status==="researching"?Vt():He()}function Vt(){h.poll||(h.poll=setInterval(Q,4e3))}function P(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),l(`save failed: ${o.message}`)}})}async function ct(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,summary:o.summary,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),x(),ae(e.posting_id,{title:o.title,location:o.location,summary:o.summary})}async function Dt(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",f(e.url)),ae(e.posting_id,{url:s.url})}async function dt(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),E(),w()}async function Ut(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();r.openId=s.company_id,se(s),oe(s.company_id),E(),w()}async function Jt(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}function Z(){const e=h.row;if(!e)return;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${i(e.title||"")}">`;const t=O[e.response]||O[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${t.cls}">${i(t.label)}</span>`+(e.verdict?` <span class="${y(e.verdict)}">${i(e.verdict)}</span>`:"");const n=document.getElementById("pursuit-chat");n&&(n.style.display=r.meta&&r.meta.chat?"":"none",n.onclick=()=>Le("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    <section class="pane-section role-head">
      <div id="role-body">${zt(e)}</div>
    </section>
  ...
  `,Ft();const s=document.getElementById("pursuit-company-link");s&&s.addEventListener("click",()=>ne(e.company_id)),P(document.getElementById("pursuit-title-input"),a=>ct(e,"title",a)),P(document.getElementById("pursuit-url-input"),a=>Dt(e,a)),P(document.getElementById("pursuit-notes-input"),a=>Wt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>P(a,o=>ct(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),fe(),te()}function zt(e){return`
    ...
    </div>`,p=s&&(Kt(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?pt(s,!1):""}</div><div class="draft-actions">${p}</div>`+u,Gt()}function Kt(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}function pt(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(k,T)=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const k=Yt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      <div class="draft-sentbody">${i(Pe(e)||"(empty)")}</div>
      ${X(e)}
    ...
    ${X(e)}
  </div>`}function Pe(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function X(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,d)=>d?`<div class="tr-line"><span class="tr-key">${u}:</span> ${i(String(d))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],p=c.map(u=>`
      <div class="tr-line">
    ...
    </div>`,s?s.replaceWith(c):n.appendChild(c);const p=c.querySelector("#gate-fix-btn");p&&p.addEventListener("click",()=>a?re("outreach-template"):Ue());const u=c.querySelector("#gate-retry-btn");u&&u.addEventListener("click",ge)}async function Zt(e){const t=document.getElementById(`draft-edit-${e}`);if(!t)return;let n;try{n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t.value})})}catch(c){l(`save failed: ${c.message}`);return}if(!n.ok){const c=(await n.text().catch(()=>"")).trim();l(`save failed: ${c||"HTTP "+n.status}`);return}const s=await n.json(),a=h.drafts.findIndex(c=>String(c.id)===String(e));a>=0&&(h.drafts[a]=s);const o=t.closest(".draft-card");if(o){const c=o.querySelector(".lint-chips"),p=ht(s.lint);c?c.outerHTML=p||"":p&&t.insertAdjacentHTML("afterend",p)}l("saved")}async function Xt(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){l(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`failed: ${s||"HTTP "+t.status}`);return}l("marked sent"),await Q(),await w();const n=r.jobs.find(s=>s.posting_id===h.postingId);n&&ae(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function ee(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){te();return}e=await t.json()}catch{te();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",te(),h.answers.some(t=>t.status==="generating")?en():qe()}function en(){h.answersPoll||(h.answersPoll=setInterval(ee,4e3))}function qe(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function te(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(d=>d.status==="generating"),a=t.length?`<div class="answers-list">${t.map(sn).join("")}</div>`:"",o=!!h.detecting,c=s||o?" disabled":"",p=d=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":d}</button>`;let u;n==="ok"&&t.length?u=`<button class="btn ${t.some(m=>!ft(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+p("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+p("Re-detect questions"):u=p("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${i(tn(n,t.length))}</div>`+a+`<div class="answers-actions">${u}</div>`,on()}function tn(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function ft(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function nn(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function sn(e){const t=ft(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${i(an(e.fail_reason))}</div>`:""}
  </div>`}function an(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function on(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",gt);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",cn),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(P(o,p=>dn(a,p),{multiline:!0}),o.addEventListener("input",()=>rn(s,o)));const c=s.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>ln(a))})}function rn(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function gt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){l(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await ee();return}if(n.status===412){let a={};try{a=await n.json()}catch{}un(a.error),t&&(t.disabled=!1);return}if(n.status===503){vt("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();l(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function cn(){h.detecting=!0,te();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();l(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){l(`detect failed: ${e.message}`)}h.detecting=!1,await ee()}async function dn(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=s)}async function ln(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){l(`regenerate failed: ${n.message}`);return}if(t.status===503){vt("Answer generation isn't running in this build.");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`regenerate failed: ${n||"HTTP "+t.status}`);return}await ee()}function un(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",Ue);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",gt)}function vt(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function ne(e){r.openId=e;const t=document.getElementById("pane"),n=document.getElementById("scrim");t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),rt("company"),document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';let s;try{const a=await fetch(`/api/companies/${e}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);s=await a.json()}catch(a){document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${i(a.message)}</div>`;return}se(s),oe(e)}function ve(){r.openId=null,r.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function se(e){r.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${i(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `;const k=document.getElementById("posting-add-btn");k&&k.addEventListener("click",()=>fn(e)),Oe(),document.querySelectorAll("#ve-pick .ve-opt").forEach(v=>{v.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(J=>J.classList.remove("is-on")),v.classList.add("is-on")})});const T=document.getElementById("ve-save-btn");T&&T.addEventListener("click",()=>mn(e)),P(document.getElementById("pane-title-input"),v=>dt(e,"name",v)),document.querySelectorAll("#facts-body [data-k]").forEach(v=>P(v,J=>dt(e,v.dataset.k,J))),P(document.getElementById("pane-domain-input"),v=>Ut(e,v)),P(document.getElementById("pane-notes-input"),v=>Jt(e,v),{multiline:!0});const $=document.getElementById("flag-toggle-btn");$&&$.addEventListener("click",()=>yt(e.company_id));const D=document.getElementById("review-stamp-btn");D&&D.addEventListener("click",()=>hn(e.company_id));const ce=document.getElementById("rescore-btn");ce&&ce.addEventListener("click",()=>Ve("verdict",{company_ids:[e.company_id]}));const I=document.getElementById("reenrich-btn");I&&I.addEventListener("click",()=>Ve("enrich",{company_ids:[e.company_id]}))}function pn(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function hn(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){l(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");l(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=r.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,N()),r.openId===e&&(se(s),oe(e)),l("reviewed")}async function yt(e){const t=r.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){l(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");l(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,N()),r.openId===e&&(se(a),oe(e)),w(),l(a.flagged?"flagged":"unflagged")}async function mn(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){l("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(p){l(`save failed: ${p.message}`),a.disabled=!1;return}if(!o.ok){const p=await o.text().catch(()=>"");l(`save failed: HTTP ${o.status}${p?" — "+p:""}`),a.disabled=!1;return}const c=await o.json();se(c),oe(c.company_id),E(),M(),w(),l("verdict saved")}function Ne(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(i).join(" · "),a=O[n.response]||O[""],o=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a.cls}">${i(a.label)}</span>`,`<span class="pt-meta">${n.applied_at?`applied ${i(n.applied_at)}`:"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${i(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join(""),c=r.meta&&r.meta.chat?`<button class="pcard-chat" data-pid="${i(n.id)}" data-ptitle="${i(n.title||"")}" title="chat about this role">chat</button>`:"";return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ae(e,t){const n=r.openDetail;if(!n||!r.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Ne(n),Oe())}function Oe(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||it(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),Le("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function fn(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const p=document.getElementById("postings-list");p&&(p.innerHTML=Ne(e),Oe()),t.value="",n.value="",s.disabled=!1,w(),l("link added")}async function oe(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){ye(`<div class="muted">Failed to load trail: ${i(s.message)}</div>`);return}if(!t.ok){ye(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){ye('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}ye(n.map(gn).join(""))}function gn(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
    ...
    </div>`}function ye(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let bt;function l(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(bt),bt=setTimeout(()=>t.classList.remove("show"),2200)}async function vn(e,t="copied"){if(!e){l("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}l(t)}catch(n){l(`copy failed: ${n.message}`)}}r.meta={control:!1,brain:!1,verdict:!1};async function yn(){try{const s=await fetch("/api/meta");if(!s.ok)return;r.meta=await s.json()}catch{return}const e=r.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!r.meta.verdict,t.title=r.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=r.meta.chat?"":"none")}async function Re(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let be=null;async function Ve(e,t){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){l(`run failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(n.status===412){const a=await n.text();l(a.trim());return}if(!n.ok){l(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();_t(e,s)}async function bn(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){l(`upload failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(!n.ok){l(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();_t("ingest",s)}const wn=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let we=[],R=new Set,V="company";function wt(e){V=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",kt()}function De(){return!!r.meta.capture&&document.getElementById("add-enrich").checked}function kt(){const e=document.getElementById("add-note");De()?e.innerHTML=V==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=V==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function kn(){wn.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",R=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!r.meta.capture,t.classList.toggle("disabled",!r.meta.capture),t.title=r.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",r.meta.capture||(e.checked=!1),wt(r.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(r.rows||[]).map(s=>`<option value="${i(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),we=s.verticals||[]}catch{we=[]}Et()}function ke(){document.getElementById("add-scrim").classList.remove("open")}function Et(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=we.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${R.has(s)?" sel":""}" data-v="${i(s)}">${i(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;R.has(a)?R.delete(a):R.add(a),s.classList.toggle("sel"),$t()}))):e.innerHTML=`<div class="none">${we.length?"no match":"no verticals in the set yet"}</div>`,$t()}function $t(){const e=R.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function It(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function xt(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){l(V==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,De()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=g=>document.getElementById(g).value.trim(),c=De();let p,u;c?(p="/api/capture",u={url:It(t),kind:V==="company"?"company_page":"job_posting",fields:V==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...R].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):V==="company"?(p="/api/companies",u={website:t,name:o("add-name"),vertical:[...R].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(p="/api/postings",u={url:It(t),title:o("add-title"),company:o("add-job-company")});let d;try{d=await fetch(p,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(g){l(`add failed: ${g.message}`),a();return}if(!d.ok){let g=`HTTP ${d.status}`;try{const k=await d.text();try{g=JSON.parse(k).error||g}catch{g=k.trim()||g}}catch{}if(a(),d.status===409){l(g||"That company is already in the list."),e.focus(),e.select();return}l(`add failed: ${g}`);return}const m=await d.json();if(a(),c&&!m.company_id){l(m.note||"couldn't classify that page");return}if(ke(),E(),M(),w(),V==="job"){const g=m.posting&&m.posting.title||"job link";l(`tracking: ${g} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),z("jobs")}else c?(l(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`),ne(m.company_id)):l("company added")}function _t(e,t){be=t;const n=document.getElementById("drawer"),s=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",s.innerHTML="",n.classList.add("open"),Re();const a=new EventSource(`/api/jobs/${t}/stream`),o=(c,p)=>{const u=document.createElement("div"),d=!p&&/^\s*warn:/i.test(c);u.className="ln"+(p?" ln-err":d?" ln-warn":""),u.textContent=d?c.replace(/^\s*warn:\s*/i,"⚠ "):c,s.appendChild(u),s.scrollTop=s.scrollHeight};a.addEventListener("line",c=>o(c.data,/error|failed/i.test(c.data))),a.addEventListener("end",c=>{a.close(),be=null,o(`— ${c.data} —`,c.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",l(`${e} ${c.data}`),E(),M(),Re(),w(),r.openId&&ne(r.openId)}),a.onerror=()=>{a.close()}}async function En(){if(be)try{await fetch(`/api/jobs/${be}/cancel`,{method:"POST"})}catch{}}let ie=null;function Bt(e){return e==="outreach-template"?"outreach template":e==="outreach-doctrine"?"outreach doctrine":e==="playbook"?"playbook":e+".md"}async function re(e){ie=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Bt(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="",t.classList.add("open");try{const n=await fetch(`/api/${e}`);if(!n.ok){const a=(await n.text().catch(()=>"")).trim();document.getElementById("editor-text").value=n.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${a||"HTTP "+n.status}`;return}const s=await n.json();document.getElementById("editor-text").value=s.content||"",s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version)}catch(n){document.getElementById("editor-text").value="failed to load: "+n.message}}function Ee(){document.getElementById("editor-scrim").classList.remove("open"),ie=null}const $n=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function Ue(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{ze(await(await fetch("/api/outreach/sources")).json())}catch(e){l(`failed to load sources: ${e.message}`)}}function Je(){document.getElementById("sources-scrim").classList.remove("open")}function ze(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):$n,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],c=o.length?o.map(p=>`<li><span class="src-title">${i(p.title||p.page_id)}</span><button class="src-rm" data-need="${i(a.key)}" data-id="${i(p.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${i(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>xn(a.dataset.need,a.dataset.id)))}async function In(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){l(`refresh failed: ${s.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){l(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const n=await t.json();n.warning?l(n.warning):l("sources refreshed"),ze(n),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function xn(e,t){let n;try{n=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(s){l(`remove failed: ${s.message}`);return}if(!n.ok){l(`remove failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}ze(await n.json())}async function _n(){if(!ie)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${ie}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(s){l(`save failed: ${s.message}`);return}if(!t.ok){l(`save failed: HTTP ${t.status}`);return}const n=await t.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version),l(`${Bt(ie)} saved`),Ee(),M()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),N()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),x()}}),document.getElementById("tab-companies").onclick=()=>z("companies"),document.getElementById("tab-jobs").onclick=()=>z("jobs"),document.getElementById("q").oninput=N,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;L.has(t)?L.delete(t):L.add(t),je(),N()})}),document.getElementById("flag-filter").addEventListener("click",e=>{F=!F,e.currentTarget.classList.toggle("is-on",F),N()}),document.getElementById("jq").oninput=x,document.getElementById("hide-rejected").addEventListener("click",e=>{U=!U,e.currentTarget.classList.toggle("is-on",U),x()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;H.has(t)?H.delete(t):H.add(t),e.classList.toggle("is-on",H.has(t)),x()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{ue=!ue,e.currentTarget.classList.toggle("is-on",ue),x()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{pe=!pe,e.currentTarget.classList.toggle("is-on",pe),x()}),Me(),le(),document.getElementById("pane-close").onclick=ve,document.getElementById("scrim").onclick=ve,document.getElementById("pursuit-close").onclick=he,document.getElementById("pursuit-scrim").onclick=he,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){et();return}if(Pn()){Qe();return}if(document.getElementById("profile-scrim").classList.contains("open")){Ge();return}if(document.getElementById("add-scrim").classList.contains("open")){ke();return}if(document.getElementById("run-scrim").classList.contains("open")){$e();return}if(document.getElementById("help-scrim").classList.contains("open")){Ie();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(Ae==="pursuit"&&n){he();return}if(Ae==="company"&&t){ve();return}if(t){ve();return}he();return}if(document.getElementById("sources-scrim").classList.contains("open")){Je();return}if(document.getElementById("editor-scrim").classList.contains("open")){Ee();return}if(document.getElementById("settings-scrim").classList.contains("open")){Ze();return}});let Fe=null;const Bn={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Lt(e){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}Fe=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Bn[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=r.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function $e(){document.getElementById("run-scrim").classList.remove("open"),Fe=null}document.getElementById("btn-enrich").onclick=()=>Lt("enrich"),document.getElementById("btn-verdict").onclick=()=>Lt("verdict"),document.getElementById("run-cancel").onclick=$e,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&$e()},document.getElementById("run-go").onclick=()=>{const e=Fe,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if($e(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),Ve(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=kn;const Ln={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Tt(e){const t=Ln[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const p=document.createElement("a");p.className="help-link",p.textContent="Learn more →",p.onclick=()=>{Ie(),St(),qn(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(p),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Ie(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Tt("add"),document.getElementById("help-run").onclick=()=>Tt("run"),document.getElementById("help-close").onclick=Ie,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Ie()},document.getElementById("add-cancel").onclick=ke,document.getElementById("add-save").onclick=xt,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&ke()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>wt(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",kt),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),xt()))}),document.getElementById("add-vertical-filter").addEventListener("input",Et),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&bn(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=En,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=Ee,document.getElementById("editor-save").onclick=_n,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Ee()},document.getElementById("sources-close").onclick=Je,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&Je()},document.getElementById("sources-refresh-btn").onclick=In;function We(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function Ke(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}Y()}const xe='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',Ct='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',jt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',Tn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',Cn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',jn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',Sn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>';function K(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${i(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function Y(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,s=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let I="off",v="";const J=t&&t.criteria_state;J==="current"?(I="ok",v="current · verified "+We(t.verified_age_seconds)):J==="changed"?(I="warn",v="changed — re-distill"):J==="unverified"?(I="warn",v=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(I="warn",v="brain offline · using cache"):a&&(I="ok",v="fetched "+We(t.age_seconds)),o=K({icon:jt,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:I,note:v,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:Ct,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=K({icon:jt,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:xe,actTitle:"edit taste.md",actLabel:"edit taste"});const c=r.sources&&r.sources.sources||[],p=c.filter(I=>I.need==="experience").length,u=c.filter(I=>I.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";p>0?(d="ok",m=`${p} experience · ${u} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const g=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",k=K({icon:Sn,nameHTML:g,dot:d,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:Ct,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),T=K({icon:Tn,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:xe,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),$=K({icon:Cn,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:xe,actTitle:"edit the outreach email template",actLabel:"edit email template"}),D=K({icon:jn,nameHTML:'<span class="edit-link" data-act="edit-doctrine" title="edit the outreach doctrine">outreach doctrine</span>',desc:"How cold emails get written — the depth bar, show-don't-tell, the kill list.",act:"edit-doctrine",actIcon:xe,actTitle:"edit the outreach doctrine",actLabel:"edit outreach doctrine"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${T}${$}${D}
     </div>`;const ce={"view-profile":()=>Hn(r.profile),"refresh-profile":An,"edit-taste":()=>re("taste"),"edit-playbook":()=>re("playbook"),"edit-template":()=>re("outreach-template"),"edit-doctrine":()=>re("outreach-doctrine"),"view-sources":Ue,"refresh-sources":Mn};e.querySelectorAll("[data-act]").forEach(I=>{const v=I.dataset.act;v&&ce[v]&&(I.onclick=ce[v])})}async function Ye(){try{r.sources=await(await fetch("/api/outreach/sources")).json()}catch{r.sources=null}Y()}async function Mn(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){l(`refresh failed: ${s.message}`),Ye();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`refresh failed: ${s||"HTTP "+t.status}`),Ye();return}const n=await t.json();n.warning?l(n.warning):l("outreach knowledge refreshed"),r.sources={sources:n.sources||[],needs:r.sources&&r.sources.needs||[]},Y()}async function An(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),Ke();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Ke();return}r.profile=await t.json(),Y(),l("company-fit brief refreshed"),M()}function Hn(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${We(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Ge(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ge,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ge()};function St(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");_e(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Qe(){document.getElementById("docs-scrim").classList.remove("open")}function Pn(){return document.getElementById("docs-scrim").classList.contains("open")}function _e(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function qn(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),_e(e)}document.getElementById("open-docs").onclick=St,document.getElementById("docs-close").onclick=Qe,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Qe()};function Nn(){document.getElementById("settings-scrim").classList.add("open"),Y()}function Ze(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Nn,document.getElementById("settings-close").onclick=Ze,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&Ze()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),_e(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&_e(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),r.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function On(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Rn(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Be(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function Xe(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Vn(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=r.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(r.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Mt(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=On(n.content);if(n.role==="user")s&&t.appendChild(Be("user",s));else if(n.role==="assistant"){const a=Rn(n.content);if(!s&&!a.length)continue;const o=Be("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Vn()),Xe()}async function Le(e,t,n){if(!r.meta||!r.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}r.chat.es&&(r.chat.es.close(),r.chat.es=null),r.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const p=await c.json();r.chat.threadId=p.thread.id,Mt(p.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+i(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function et(){r.chat.es&&(r.chat.es.close(),r.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Te(e){r.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function At(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Ht(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||r.chat.streaming||!r.chat.threadId)return;e.value="",At(),Te(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Be("user",t));const a=Be("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),Xe();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Te(!1)},p=r.chat.threadId;let u;try{u=await fetch("/api/chat/"+p+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const d=new EventSource("/api/chat/"+p+"/stream");r.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,Xe()}),d.addEventListener("end",async m=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Te(!1),r.chat.threadId===p&&await Dn(),Un(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),d.onerror=()=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Te(!1)}}async function Dn(){const e=r.chat.scope,t=r.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Mt(a.messages||[])}catch{}}function Un(){E(),w(),M(),r.openId&&ne(r.openId)}document.getElementById("open-chat").onclick=()=>Le("global","",""),document.getElementById("chat-close").onclick=et,document.getElementById("chat-scrim").onclick=et,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Ht()}),document.getElementById("chat-input").addEventListener("input",At),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Ht())}),E(),w(),M(),yn(),Re(),Ke(),Ye()}Fn({"":{view:()=>({mount(b){b.innerHTML=Qn,Zn()}}),chrome:!1}},{title:"scout"});Yn();Gn();
