(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))I(i);new MutationObserver(i=>{for(const f of i)if(f.type==="childList")for(const E of f.addedNodes)E.tagName==="LINK"&&E.rel==="modulepreload"&&I(E)}).observe(document,{childList:!0,subtree:!0});function v(i){const f={};return i.integrity&&(f.integrity=i.integrity),i.referrerPolicy&&(f.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?f.credentials="include":i.crossOrigin==="anonymous"?f.credentials="omit":f.credentials="same-origin",f}function I(i){if(i.ep)return;i.ep=!0;const f=v(i);fetch(i.href,f)}})();function qn(y,r){const v=r.replace(/^#/,"");let I=null;for(const i of Object.keys(y))(v===i||v.startsWith(i))&&(I===null||i.length>I.length)&&(I=i);return I===null&&""in y&&(I=""),I}function Nn(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function On(y,r={}){const v=r.root??document.body,I=r.title??document.title??"",i=r.brandHref??"#",f=document.createElement("main"),E=document.createElement("header");E.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=I,w.setAttribute("aria-label",`${I} — home`),E.appendChild(w);const j=document.createElement("nav");j.className="cap-nav",j.setAttribute("aria-label","Views");for(const k of r.nav??[]){const T=document.createElement("a");T.href=k.href,T.textContent=k.label,k.ariaLabel&&T.setAttribute("aria-label",k.ariaLabel),j.appendChild(T)}E.appendChild(j);const L=document.createElement("section");L.className="tk-content",f.appendChild(E),f.appendChild(L);const C=document.createElement("div");C.className="tk-bleed";const Te=k=>{var T;for(const M of Array.from(j.querySelectorAll("a"))){const V=((T=M.getAttribute("href"))==null?void 0:T.replace(/^#/,""))??"";M.toggleAttribute("aria-current",k!==null&&k!==""&&V===k),M.hasAttribute("aria-current")&&M.setAttribute("aria-current","page")}};let oe=0;const S=()=>{const k=qn(y,location.hash);if(Te(k),k===null){C.isConnected&&C.remove(),f.isConnected||v.appendChild(f),Rn(L,"Not found.");return}const{view:T,chrome:M}=Nn(y[k]),V=M?L:C;M?(C.isConnected&&C.remove(),f.isConnected||v.appendChild(f)):(f.isConnected&&f.remove(),C.isConnected||v.appendChild(C)),V.replaceChildren();const Ce=T(),ie=++oe,W=Ce.mount(V);W instanceof Promise&&W.catch(re=>{ie===oe&&Vn(V,String(re))})};window.addEventListener("hashchange",S),S()}function Rn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-empty",v.textContent=r,y.appendChild(v)}function Vn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-error",v.textContent=r,y.appendChild(v)}function Dn(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const v of r)v.unregister()}),window.caches&&caches.keys().then(r=>{for(const v of r)caches.delete(v)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Un(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let r;try{r=await y.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Jn=`
<div class="layout">
<aside class="sidebar">
...
`;function zn(y){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null},v=e=>"pill pill-"+(e||"none"),I='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function E(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],q()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],x(),h.postingId){const t=r.jobs.find(n=>n.posting_id===h.postingId);t&&(h.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&G())}}function j(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",je(),e==="jobs"?x():q()}async function L(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,C()}function C(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,F()}function Te(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function oe(e){return e.slice().sort((t,n)=>r.sort.dir*Te(t,n,r.sort.k))}const S=new Set;let k=!1;function T(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",S.has(e.dataset.v))})}function M(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(S.size&&!S.has(t.verdict||"__none__")||k&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const V=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Ce=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function ie(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const W=ie("scout-hidden-cols"),re=ie("scout-hidden-jcols");function et(){return r.view==="jobs"?{cols:Ce,hidden:re,key:"scout-hidden-jcols"}:{cols:V,hidden:W,key:"scout-hidden-cols"}}function ce(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=W.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=re.has(e.dataset.col)?"none":""})}function je(){const e=et();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=et(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),je(),ce()})})}function q(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=oe(M());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
  ...
  `,Vt();const s=document.getElementById("pursuit-company-link");s&&s.addEventListener("click",()=>X(e.company_id)),H(document.getElementById("pursuit-title-input"),a=>at(e,"title",a)),H(document.getElementById("pursuit-url-input"),a=>qt(e,a)),H(document.getElementById("pursuit-notes-input"),a=>Dt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>H(a,o=>at(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),he(),Z()}function Rt(e){return`
    ...
    </div>`,p=s&&(Ut(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?ct(s,!1):""}</div><div class="draft-actions">${p}</div>`+u,zt()}function Ut(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function ct(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(m,g)=>`
    ...
    </div>`,s?s.replaceWith(c):n.appendChild(c);const p=c.querySelector("#gate-fix-btn");p&&p.addEventListener("click",()=>a?we("outreach-template"):Ve());const u=c.querySelector("#gate-retry-btn");u&&u.addEventListener("click",Ae)}async function Wt(e){const t=document.getElementById(`draft-edit-${e}`);if(!t)return;let n;try{n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t.value})})}catch(c){l(`save failed: ${c.message}`);return}if(!n.ok){const c=(await n.text().catch(()=>"")).trim();l(`save failed: ${c||"HTTP "+n.status}`);return}const s=await n.json(),a=h.drafts.findIndex(c=>String(c.id)===String(e));a>=0&&(h.drafts[a]=s);const o=t.closest(".draft-card");if(o){const c=o.querySelector(".lint-chips"),p=lt(s.lint);c?c.outerHTML=p||"":p&&t.insertAdjacentHTML("afterend",p)}l("saved")}async function Kt(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){l(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`failed: ${s||"HTTP "+t.status}`);return}l("marked sent"),await Y(),await w();const n=r.jobs.find(s=>s.posting_id===h.postingId);n&&te(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function Q(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){Z();return}e=await t.json()}catch{Z();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",Z(),h.answers.some(t=>t.status==="generating")?Yt():He()}function Yt(){h.answersPoll||(h.answersPoll=setInterval(Q,4e3))}function He(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function Z(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(d=>d.status==="generating"),a=t.length?`<div class="answers-list">${t.map(Zt).join("")}</div>`:"",o=!!h.detecting,c=s||o?" disabled":"",p=d=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":d}</button>`;let u;n==="ok"&&t.length?u=`<button class="btn ${t.some(m=>!ut(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+p("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+p("Re-detect questions"):u=p("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${i(Gt(n,t.length))}</div>`+a+`<div class="answers-actions">${u}</div>`,en()}function Gt(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function ut(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Qt(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function Zt(e){const t=ut(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${i(Xt(e.fail_reason))}</div>`:""}
  </div>`}function Xt(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function en(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",pt);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",nn),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(H(o,p=>sn(a,p),{multiline:!0}),o.addEventListener("input",()=>tn(s,o)));const c=s.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>an(a))})}function tn(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function pt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){l(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await Q();return}if(n.status===412){let a={};try{a=await n.json()}catch{}on(a.error),t&&(t.disabled=!1);return}if(n.status===503){ht("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();l(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function nn(){h.detecting=!0,Z();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();l(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){l(`detect failed: ${e.message}`)}h.detecting=!1,await Q()}async function sn(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=s)}async function an(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){l(`regenerate failed: ${n.message}`);return}if(t.status===503){ht("Answer generation isn't running in this build.");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`regenerate failed: ${n||"HTTP "+t.status}`);return}await Q()}function on(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
  ...
  `;const P=document.getElementById("posting-add-btn");P&&P.addEventListener("click",()=>ln(e)),qe(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Le=>Le.classList.remove("is-on")),b.classList.add("is-on")})});const U=document.getElementById("ve-save-btn");U&&U.addEventListener("click",()=>dn(e)),H(document.getElementById("pane-title-input"),b=>ot(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>H(b,Le=>ot(e,b.dataset.k,Le))),H(document.getElementById("pane-domain-input"),b=>Nt(e,b)),H(document.getElementById("pane-notes-input"),b=>Ot(e,b),{multiline:!0});const _=document.getElementById("flag-toggle-btn");_&&_.addEventListener("click",()=>mt(e.company_id));const J=document.getElementById("review-stamp-btn");J&&J.addEventListener("click",()=>cn(e.company_id));const $=document.getElementById("rescore-btn");$&&$.addEventListener("click",()=>Oe("verdict",{company_ids:[e.company_id]}));const B=document.getElementById("reenrich-btn");B&&B.addEventListener("click",()=>Oe("enrich",{company_ids:[e.company_id]}))}function rn(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function te(e,t){const n=r.openDetail;if(!n||!r.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Pe(n),qe())}function qe(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||nt(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),_e("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function ln(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const p=document.getElementById("postings-list");p&&(p.innerHTML=Pe(e),qe()),t.value="",n.value="",s.disabled=!1,w(),l("link added")}async function ne(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){ge(`<div class="muted">Failed to load trail: ${i(s.message)}</div>`);return}if(!t.ok){ge(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){ge('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}ge(n.map(un).join(""))}function un(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
     ...
     <div class="settings-section">
       ...
       ${U}${_}
     </div>`;const J={"view-profile":()=>Ln(r.profile),"refresh-profile":Bn,"edit-taste":()=>we("taste"),"edit-playbook":()=>we("playbook"),"edit-template":()=>we("outreach-template"),"view-sources":Ve,"refresh-sources":_n};e.querySelectorAll("[data-act]").forEach($=>{const B=$.dataset.act;B&&J[B]&&($.onclick=J[B])})}async function Ke(){try{r.sources=await(await fetch("/api/outreach/sources")).json()}catch{r.sources=null}F()}async function _n(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){l(`refresh failed: ${s.message}`),Ke();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`refresh failed: ${s||"HTTP "+t.status}`),Ke();return}const n=await t.json();n.warning?l(n.warning):l("outreach knowledge refreshed"),r.sources={sources:n.sources||[],needs:r.sources&&r.sources.needs||[]},F()}async function Bn(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),Fe();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Fe();return}r.profile=await t.json(),F(),l("company-fit brief refreshed"),L()}function Ln(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${ze(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Ye(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ye,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ye()};function Lt(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Ie(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ge(){document.getElementById("docs-scrim").classList.remove("open")}function Tn(){return document.getElementById("docs-scrim").classList.contains("open")}function Ie(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Cn(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ie(e)}document.getElementById("open-docs").onclick=Lt,document.getElementById("docs-close").onclick=Ge,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Ge()};function jn(){document.getElementById("settings-scrim").classList.add("open"),F()}function Qe(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=jn,document.getElementById("settings-close").onclick=Qe,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&Qe()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ie(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&Ie(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),r.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Sn(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Mn(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function xe(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function Ze(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function An(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=r.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(r.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Tt(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=Sn(n.content);if(n.role==="user")s&&t.appendChild(xe("user",s));else if(n.role==="assistant"){const a=Mn(n.content);if(!s&&!a.length)continue;const o=xe("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(An()),Ze()}async function _e(e,t,n){if(!r.meta||!r.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}r.chat.es&&(r.chat.es.close(),r.chat.es=null),r.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const p=await c.json();r.chat.threadId=p.thread.id,Tt(p.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+i(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Xe(){r.chat.es&&(r.chat.es.close(),r.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Be(e){r.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Ct(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function jt(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||r.chat.streaming||!r.chat.threadId)return;e.value="",Ct(),Be(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(xe("user",t));const a=xe("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),Ze();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Be(!1)},p=r.chat.threadId;let u;try{u=await fetch("/api/chat/"+p+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const d=new EventSource("/api/chat/"+p+"/stream");r.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,Ze()}),d.addEventListener("end",async m=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Be(!1),r.chat.threadId===p&&await Hn(),Pn(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),d.onerror=()=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Be(!1)}}async function Hn(){const e=r.chat.scope,t=r.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Tt(a.messages||[])}catch{}}function Pn(){E(),w(),L(),r.openId&&X(r.openId)}document.getElementById("open-chat").onclick=()=>_e("global","",""),document.getElementById("chat-close").onclick=Xe,document.getElementById("chat-scrim").onclick=Xe,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),jt()}),document.getElementById("chat-input").addEventListener("input",Ct),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),jt())}),E(),w(),L(),pn(),Ne(),Fe(),Ke()}On({"":{view:()=>({mount(y){y.innerHTML=Jn,zn()}}),chrome:!1}},{title:"scout"});Dn();Un();
