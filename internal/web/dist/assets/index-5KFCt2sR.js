...
`;function Yt(g){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},f=e=>"pill pill-"+(e||"none"),x='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',o=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),m=e=>/^https?:\/\//i.test(String(e??""))?o(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],A()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],_(),p.postingId){const t=r.jobs.find(n=>n.posting_id===p.postingId);t&&(p.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&F())}}function j(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",fe(),e==="jobs"?_():A()}async function I(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Ie()}function he(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[s]??3)-(i[a]??3)}return String(s).localeCompare(String(a))}function G(e){return e.slice().sort((t,n)=>r.sort.dir*he(t,n,r.sort.k))}const C=new Set;let w=!1;function B(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",C.has(e.dataset.v))})}function S(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(C.size&&!C.has(t.verdict||"__none__")||w&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const N=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],me=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Q(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const U=Q("scout-hidden-cols"),X=Q("scout-hidden-jcols");function je(){return r.view==="jobs"?{cols:me,hidden:X,key:"scout-hidden-jcols"}:{cols:N,hidden:U,key:"scout-hidden-cols"}}function Z(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=U.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=X.has(e.dataset.col)?"none":""})}function fe(){const e=je();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=je(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),fe(),Z()})})}function A(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=G(S());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
    ...
    `,e.appendChild(s)}Z(),e.querySelectorAll("tr").forEach(n=>{n.addEventListener("click",s=>{s.target.closest("a, .flag-btn")||se(n.dataset.id)})}),e.querySelectorAll(".flag-btn").forEach(n=>{n.addEventListener("click",()=>Ve(n.dataset.id))})}const H=new Set;let ee=!1,O=!0;function rt(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=O&&!H.has("rejected");return r.jobs.filter(n=>!(t&&n.response==="rejected"||H.size&&!H.has(n.response||"")||ee&&!n.next_up||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const dt=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function Ce(e){const t=String(e||"").split(",").map(n=>n.trim()).filter(Boolean);return t.length?t.map(n=>{const s=n.match(dt);return s?`<a href="mailto:${o(s[0])}" title="${o(n)}">${o(n)}</a>`:o(n)}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}const P={offer:{cls:"pill-yes",label:"offer",order:0},interview:{cls:"pill-info",label:"interview",order:1},screening:{cls:"pill-maybe",label:"screening call",order:2},"":{cls:"pill-none",label:"—",order:3},rejected:{cls:"pill-no",label:"rejected",order:4}};function ct(e,t,n){var s,a;if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[e.verdict]??3)-(i[t.verdict]??3)}if(n==="response")return(((s=P[e.response])==null?void 0:s.order)??3)-(((a=P[t.response])==null?void 0:a.order)??3);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="applied_at"||n==="last_outreach_at"){const i=e[n]||"",c=t[n]||"";return!i&&!c?0:i?c?String(c).localeCompare(String(i)):-r.jsort.dir:r.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function D(e){const t=e.options[e.selectedIndex],n=getComputedStyle(e),s=D._c||(D._c=document.createElement("canvas").getContext("2d"));s.font=`${n.fontWeight} ${n.fontSize} ${n.fontFamily}`;const a=s.measureText(t?t.text:"").width;e.style.width=Math.ceil(a+35)+"px"}function _(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=rt().sort((d,u)=>r.jsort.dir*ct(d,u,r.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=O&&!H.has("rejected")?r.jobs.filter(d=>d.response==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=r.jobs.filter(d=>d.next_up).length,i=document.getElementById("next-up-n");i.textContent=a,i.style.display=a?"":"none";const c=document.getElementById("jobs-hidden-note");c.style.display=n?"":"none",n&&(c.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{O=!1,document.getElementById("hide-rejected").classList.remove("is-on"),_()});for(const d of t){const u=P[d.response]||P[""],h=document.createElement("tr");h.dataset.id=d.posting_id;const y=[["","none"],["screening","screening"],["interview","interview"],["offer","offer"],["rejected","rejected"]].map(([b,T])=>`<option value="${b}"${(d.response||"")===b?" selected":""}>${T}</option>`).join("");h.innerHTML=`
      <td><button class="jt-nextup${d.next_up?" is-on":""}" title="${d.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${d.next_up?"★":"☆"}</button><span class="row-name">${o(d.title||d.company)}</span>${lt(d.outreach_draft_status)}
    ...
    `;const v=()=>new Date().toISOString().slice(0,10);h.querySelector(".jt-nextup").onclick=()=>He(d,!1),h.querySelector(".jt-applied").onclick=()=>te(d,{applied_at:d.applied_at?"":v()}),h.querySelector(".jt-resp").onchange=b=>{D(b.target),te(d,{response:b.target.value})},h.querySelector(".jt-inc").onclick=()=>te(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:v()}),h.querySelector(".jt-dec").onclick=()=>{const b=Math.max(0,(d.outreach_count||0)-1);te(d,{outreach_count:b,...b===0?{last_outreach_at:""}:{}})},e.appendChild(h)}e.querySelectorAll(".jt-resp").forEach(D),Z(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",u=>{u.target.closest("a, button, select")||Se(d.dataset.id)})})}function lt(e){return e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const p={postingId:null,row:null,drafts:[],poll:null,openHist:!1};async function Se(e){let t=r.jobs.find(n=>n.posting_id===e);if(t||(await k(),t=r.jobs.find(n=>n.posting_id===e)),!t){l("posting not found — refresh");return}ge(),p.postingId=e,p.row=t,p.drafts=[],p.openHist=!1,document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),F(),J()}function ve(){ge(),p.postingId=null,p.row=null,p.drafts=[],document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function ge(){p.poll&&(clearInterval(p.poll),p.poll=null)}async function J(){if(!p.postingId)return;let e;try{const n=await fetch(`/api/postings/${p.postingId}/outreach`);if(!n.ok){z();return}e=await n.json()}catch{z();return}p.drafts=e.drafts||[],z();const t=p.drafts[0];t&&t.status==="researching"?ut():ge()}function ut(){p.poll||(p.poll=setInterval(J,4e3))}function R(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(i){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),l(`save failed: ${i.message}`)}})}async function Me(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const i=await a.json();Object.assign(e,{title:i.title,location:i.location,summary:i.summary,employment_type:i.employment_type,workplace_type:i.workplace_type,department:i.department,comp_range:i.comp_range,description:i.description}),_()}async function Ae(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const i=await a.json();Object.assign(e,{name:i.name,headcount:i.headcount,funding_stage:i.funding_stage,location:i.location,vertical:i.vertical}),$(),k()}async function pt(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();r.openId=s.company_id,W(s),K(s.company_id),$(),k()}function F(){const e=p.row;if(!e)return;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${o(e.title||"")}">`;const t=P[e.response]||P[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${t.cls}">${o(t.label)}</span>`+(e.verdict?` <span class="${f(e.verdict)}">${o(e.verdict)}</span>`:""),document.getElementById("pursuit-body").innerHTML=`
  ...
  `,mt();const n=document.getElementById("pursuit-view-company");n&&n.addEventListener("click",()=>se(e.company_id)),R(document.getElementById("pursuit-title-input"),s=>Me(e,"title",s)),document.querySelectorAll("#role-body [data-k]").forEach(s=>R(s,a=>Me(e,s.dataset.k,a),{multiline:s.tagName==="TEXTAREA"})),z()}function ht(e){return`
    ...
    </div>`,d=s&&(ft(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    ...
    </details>`:"";e.innerHTML=i+`<div id="draft-current">${s?qe(s,!1):""}</div><div class="draft-actions">${d}</div>`+u,gt()}function ft(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function qe(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(y,v)=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const y=vt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      ${ne(e)}
    </div>`;const a=Ne(e),i=e.status==="no_hook",c=i?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let d="";if(i)try{d=JSON.parse(e.hook||"{}").reasoning||""}catch{}const u=i?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${d?" "+o(d):""}</div>`:"";if(t)return`<div class="draft-card ${i?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    ${ne(e)}
  </div>`}function Ne(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function ne(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,h)=>h?`<div class="tr-line"><span class="tr-key">${u}:</span> ${o(String(h))}</div>`:"",i=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],d=c.map(u=>`
      <div class="tr-line">
    ...
    <button class="btn btn-primary" id="blocks-sync-btn">Sync blocks</button>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#blocks-sync-btn");a&&a.addEventListener("click",bt)}async function bt(){const e=document.getElementById("blocks-sync-btn");e&&(e.disabled=!0,e.textContent="Syncing…");let t;try{t=await fetch("/api/outreach/sync",{method:"POST"})}catch(n){l(`sync failed: ${n.message}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`sync failed: ${n||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}l("blocks synced"),z()}async function wt(e){const t=document.getElementById(`draft-edit-${e}`);if(!t)return;let n;try{n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t.value})})}catch(c){l(`save failed: ${c.message}`);return}if(!n.ok){const c=(await n.text().catch(()=>"")).trim();l(`save failed: ${c||"HTTP "+n.status}`);return}const s=await n.json(),a=p.drafts.findIndex(c=>String(c.id)===String(e));a>=0&&(p.drafts[a]=s);const i=t.closest(".draft-card");if(i){const c=i.querySelector(".lint-chips"),d=Oe(s.lint);c?c.outerHTML=d||"":d&&t.insertAdjacentHTML("afterend",d)}l("saved")}async function kt(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(n){l(`failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`failed: ${n||"HTTP "+t.status}`);return}l("marked sent"),await J(),k()}async function se(e){r.openId=e;const t=document.getElementById("pane"),n=document.getElementById("scrim");t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';let s;try{const a=await fetch(`/api/companies/${e}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);s=await a.json()}catch(a){document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${o(a.message)}</div>`;return}W(s),K(e)}function ae(){r.openId=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function W(e){document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${o(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `:'<div class="muted">No enrichment yet. Run <code>scout enrich</code>.</div>',i=!r.meta||r.meta.control!==!1,c=i&&r.meta&&r.meta.verdict?'<button class="h3-action" id="rescore-btn" title="re-score just this company — replaces the current verdict, manual or not">↻ re-score</button>':"",d=i&&e.domain?'<button class="h3-action" id="reenrich-btn" title="re-fetch this company’s site now">↻ re-enrich</button>':"",u=Object.keys(e.raw_json||{}).sort(),h=u.length===0?"":`
    <details class="raw-json">
      ...
      <table><tbody>
        ${u.map(E=>`<tr><td class="k">${o(E)}</td><td>${o(e.raw_json[E])}</td></tr>`).join("")}
      </tbody></table>
    </details>
  `,y=`
    <div class="flag-bar">
  ...
  `;const v=document.getElementById("posting-add-btn");v&&v.addEventListener("click",()=>_t(e)),De(),document.querySelectorAll("#ve-pick .ve-opt").forEach(E=>{E.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Te=>Te.classList.remove("is-on")),E.classList.add("is-on")})});const b=document.getElementById("ve-save-btn");b&&b.addEventListener("click",()=>xt(e)),R(document.getElementById("pane-title-input"),E=>Ae(e,"name",E)),document.querySelectorAll("#facts-body [data-k]").forEach(E=>R(E,Te=>Ae(e,E.dataset.k,Te))),R(document.getElementById("pane-domain-input"),E=>pt(e,E));const T=document.getElementById("flag-toggle-btn");T&&T.addEventListener("click",()=>Ve(e.company_id));const at=document.getElementById("review-stamp-btn");at&&at.addEventListener("click",()=>$t(e.company_id));const ot=document.getElementById("rescore-btn");ot&&ot.addEventListener("click",()=>be("verdict",{company_ids:[e.company_id]}));const it=document.getElementById("reenrich-btn");it&&it.addEventListener("click",()=>be("enrich",{company_ids:[e.company_id]}))}function Et(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function $t(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(i){l(`failed: ${i.message}`),t&&(t.disabled=!1);return}if(!n.ok){const i=await n.text().catch(()=>"");l(`failed: HTTP ${n.status}${i?" — "+i:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=r.rows.find(i=>i.company_id===e);a&&(a.reviewed_at=s.reviewed_at,A()),r.openId===e&&(W(s),K(e)),l("reviewed")}async function Ve(e){const t=r.rows.find(i=>i.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(i){l(`failed: ${i.message}`);return}if(!s.ok){const i=await s.text().catch(()=>"");l(`failed: HTTP ${s.status}${i?" — "+i:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,A()),r.openId===e&&(W(a),K(e)),k(),l(a.flagged?"flagged":"unflagged")}async function xt(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){l("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(d){l(`save failed: ${d.message}`),a.disabled=!1;return}if(!i.ok){const d=await i.text().catch(()=>"");l(`save failed: HTTP ${i.status}${d?" — "+d:""}`),a.disabled=!1;return}const c=await i.json();W(c),K(c.company_id),$(),I(),k(),l("verdict saved")}function Ue(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(o).join(" · "),a=P[n.response]||P[""],i=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a.cls}">${o(a.label)}</span>`,`<span class="pt-meta">${n.applied_at?`applied ${o(n.applied_at)}`:"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${o(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function De(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||(ae(),Se(e.dataset.pid))})})}async function _t(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!i.ok){const u=await i.text().catch(()=>"");l(`add failed: HTTP ${i.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await i.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const d=document.getElementById("postings-list");d&&(d.innerHTML=Ue(e),De()),t.value="",n.value="",s.disabled=!1,k(),l("link added")}async function K(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){oe(`<div class="muted">Failed to load trail: ${o(s.message)}</div>`);return}if(!t.ok){oe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){oe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}oe(n.map(It).join(""))}function It(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(o);return e.run_id&&t.push("run "+o(e.run_id.slice(0,8))),`
    <div class="trail-event">
      ...
      <button class="crit-edit" id="edit-taste" title="edit taste.md" aria-label="edit taste">${_e}</button></div>`,t&&t.configured&&(c+='<div class="crit-note dim small">brain offline — local fallback</div>');c+=`<div class="crit-row">
    <span class="crit-what">playbook</span>
    <button class="crit-edit" id="edit-playbook" title="edit playbook.md" aria-label="edit playbook">${_e}</button></div>`,c+=`<div class="crit-row">
    <span class="crit-what">outreach identity</span>
    <button class="crit-edit" id="edit-sender" title="edit outreach identity" aria-label="edit outreach identity">${_e}</button></div>`,n>0&&(c+=`<div class="stale">
      ...
      ${n} verdicts stale</div>`),e.innerHTML=c;const d=document.getElementById("view-profile");d&&(d.onclick=()=>Nt(r.profile));const u=document.getElementById("refresh-profile");u&&(u.onclick=qt);const h=document.getElementById("edit-taste");h&&(h.onclick=()=>Xe("taste"));const y=document.getElementById("edit-playbook");y&&(y.onclick=()=>Xe("playbook"));const v=document.getElementById("edit-sender");v&&(v.onclick=St)}async function qt(){const e=document.getElementById("refresh-profile");e&&(e.textContent="refreshing…",e.style.pointerEvents="none");let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),xe();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),xe();return}r.profile=await t.json(),Ie(),l("brain profile refreshed"),I()}function Nt(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${nt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Be(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Be,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Be()};function st(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");pe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Le(){document.getElementById("docs-scrim").classList.remove("open")}function Ot(){return document.getElementById("docs-scrim").classList.contains("open")}function pe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Rt(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),pe(e)}document.getElementById("open-docs").onclick=st,document.getElementById("docs-close").onclick=Le,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Le()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),pe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,i)=>a.boundingClientRect.top-i.boundingClientRect.top);s.length&&pe(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),$(),k(),I(),Bt(),ye(),xe()}Dt({"":{view:()=>({mount(g){g.innerHTML=Kt,Yt()}}),chrome:!1}},{title:"scout"});zt();Wt();
