(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))I(i);new MutationObserver(i=>{for(const f of i)if(f.type==="childList")for(const $ of f.addedNodes)$.tagName==="LINK"&&$.rel==="modulepreload"&&I($)}).observe(document,{childList:!0,subtree:!0});function v(i){const f={};return i.integrity&&(f.integrity=i.integrity),i.referrerPolicy&&(f.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?f.credentials="include":i.crossOrigin==="anonymous"?f.credentials="omit":f.credentials="same-origin",f}function I(i){if(i.ep)return;i.ep=!0;const f=v(i);fetch(i.href,f)}})();function An(y,r){const v=r.replace(/^#/,"");let I=null;for(const i of Object.keys(y))(v===i||v.startsWith(i))&&(I===null||i.length>I.length)&&(I=i);return I===null&&""in y&&(I=""),I}function Hn(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function Pn(y,r={}){const v=r.root??document.body,I=r.title??document.title??"",i=r.brandHref??"#",f=document.createElement("main"),$=document.createElement("header");$.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=I,w.setAttribute("aria-label",`${I} — home`),$.appendChild(w);const M=document.createElement("nav");M.className="cap-nav",M.setAttribute("aria-label","Views");for(const k of r.nav??[]){const C=document.createElement("a");C.href=k.href,C.textContent=k.label,k.ariaLabel&&C.setAttribute("aria-label",k.ariaLabel),M.appendChild(C)}$.appendChild(M);const _=document.createElement("section");_.className="tk-content",f.appendChild($),f.appendChild(_);const L=document.createElement("div");L.className="tk-bleed";const Ie=k=>{var C;for(const H of Array.from(M.querySelectorAll("a"))){const V=((C=H.getAttribute("href"))==null?void 0:C.replace(/^#/,""))??"";H.toggleAttribute("aria-current",k!==null&&k!==""&&V===k),H.hasAttribute("aria-current")&&H.setAttribute("aria-current","page")}};let ne=0;const A=()=>{const k=An(y,location.hash);if(Ie(k),k===null){L.isConnected&&L.remove(),f.isConnected||v.appendChild(f),qn(_,"Not found.");return}const{view:C,chrome:H}=Hn(y[k]),V=H?_:L;H?(L.isConnected&&L.remove(),f.isConnected||v.appendChild(f)):(f.isConnected&&f.remove(),L.isConnected||v.appendChild(L)),V.replaceChildren();const Be=C(),se=++ne,F=Be.mount(V);F instanceof Promise&&F.catch(ae=>{se===ne&&Nn(V,String(ae))})};window.addEventListener("hashchange",A),A()}function qn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-empty",v.textContent=r,y.appendChild(v)}function Nn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-error",v.textContent=r,y.appendChild(v)}function On(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const v of r)v.unregister()}),window.caches&&caches.keys().then(r=>{for(const v of r)caches.delete(v)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Rn(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let r;try{r=await y.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Vn=`
<div class="layout">
<aside class="sidebar">
...
`;function Un(y){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},v=e=>"pill pill-"+(e||"none"),I='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],q()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],B(),h.postingId){const t=r.jobs.find(n=>n.posting_id===h.postingId);t&&(h.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&K())}}function M(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",_e(),e==="jobs"?B():q()}async function _(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Re()}function Ie(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function ne(e){return e.slice().sort((t,n)=>r.sort.dir*Ie(t,n,r.sort.k))}const A=new Set;let k=!1;function C(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",A.has(e.dataset.v))})}function H(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(A.size&&!A.has(t.verdict||"__none__")||k&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const V=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Be=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function se(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const F=se("scout-hidden-cols"),ae=se("scout-hidden-jcols");function ze(){return r.view==="jobs"?{cols:Be,hidden:ae,key:"scout-hidden-jcols"}:{cols:V,hidden:F,key:"scout-hidden-cols"}}function oe(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=F.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=ae.has(e.dataset.col)?"none":""})}function _e(){const e=ze();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=ze(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),_e(),oe()})})}function q(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=ne(H());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
  ...
  `,Ot();const s=document.getElementById("pursuit-company-link");s&&s.addEventListener("click",()=>Q(e.company_id)),T(document.getElementById("pursuit-title-input"),a=>Ye(e,"title",a)),T(document.getElementById("pursuit-url-input"),a=>Ht(e,a)),T(document.getElementById("pursuit-notes-input"),a=>Rt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>T(a,o=>Ye(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),Y(),J()}function Nt(e){return`
  ...
  `;const j=document.getElementById("posting-add-btn");j&&j.addEventListener("click",()=>ln(e)),rt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Fe=>Fe.classList.remove("is-on")),b.classList.add("is-on")})});const S=document.getElementById("ve-save-btn");S&&S.addEventListener("click",()=>dn(e)),T(document.getElementById("pane-title-input"),b=>Ge(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>T(b,Fe=>Ge(e,b.dataset.k,Fe))),T(document.getElementById("pane-domain-input"),b=>Pt(e,b)),T(document.getElementById("pane-notes-input"),b=>qt(e,b),{multiline:!0});const E=document.getElementById("flag-toggle-btn");E&&E.addEventListener("click",()=>ot(e.company_id));const te=document.getElementById("review-stamp-btn");te&&te.addEventListener("click",()=>cn(e.company_id));const Ct=document.getElementById("rescore-btn");Ct&&Ct.addEventListener("click",()=>Se("verdict",{company_ids:[e.company_id]}));const Lt=document.getElementById("reenrich-btn");Lt&&Lt.addEventListener("click",()=>Se("enrich",{company_ids:[e.company_id]}))}function rn(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function rt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||(le(),Ke(e.dataset.pid))})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),$e("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function ln(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const p=document.getElementById("postings-list");p&&(p.innerHTML=it(e),rt()),t.value="",n.value="",s.disabled=!1,w(),l("link added")}async function Z(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){ue(`<div class="muted">Failed to load trail: ${i(s.message)}</div>`);return}if(!t.ok){ue(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){ue('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}ue(n.map(un).join(""))}function un(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
    <div class="trail-event">
      ...
      </span></div>`}).join(""),e.querySelectorAll(".cfg-slot").forEach(n=>{const s=+n.dataset.i,a=n.querySelector(".cfg-up"),o=n.querySelector(".cfg-down"),c=n.querySelector(".cfg-rm");a&&(a.onclick=()=>wt(s,-1)),o&&(o.onclick=()=>wt(s,1)),c&&(c.onclick=()=>qe(p=>{p.splice(s,1)}))})}function wt(e,t){qe(n=>{const s=e+t;s<0||s>=n.length||([n[e],n[s]]=[n[s],n[e]])})}function wn(){const e=document.getElementById("cfg-add-select"),[t,n]=(e.value||"").split(":");qe(s=>{s.push(t==="locked"?{kind:"locked",block:n}:{kind:"model",source:n})})}async function qe(e){if(!x)return;const t=x.structure||[],n=JSON.parse(JSON.stringify(t));e(n),x.structure=n,ve(),await ge()||(x.structure=t,ve())}async function kn(){if(!ee)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${ee}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(s){l(`save failed: ${s.message}`);return}if(!t.ok){l(`save failed: HTTP ${t.status}`);return}const n=await t.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version),l(`${ee}.md saved`),fe(),_()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),q()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),B()}}),document.getElementById("tab-companies").onclick=()=>M("companies"),document.getElementById("tab-jobs").onclick=()=>M("jobs"),document.getElementById("q").oninput=q,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;A.has(t)?A.delete(t):A.add(t),C(),q()})}),document.getElementById("flag-filter").addEventListener("click",e=>{k=!k,e.currentTarget.classList.toggle("is-on",k),q()}),document.getElementById("jq").oninput=B,document.getElementById("hide-rejected").addEventListener("click",e=>{U=!U,e.currentTarget.classList.toggle("is-on",U),B()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;P.has(t)?P.delete(t):P.add(t),e.classList.toggle("is-on",P.has(t)),B()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{ie=!ie,e.currentTarget.classList.toggle("is-on",ie),B()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{re=!re,e.currentTarget.classList.toggle("is-on",re),B()}),_e(),oe(),document.getElementById("pane-close").onclick=le,document.getElementById("scrim").onclick=le,document.getElementById("pursuit-close").onclick=Ce,document.getElementById("pursuit-scrim").onclick=Ce,document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(document.getElementById("chat-pane").classList.contains("open")){Je();return}if(_n()){Ue();return}if(document.getElementById("profile-scrim").classList.contains("open")){Ve();return}if(document.getElementById("add-scrim").classList.contains("open")){me();return}if(document.getElementById("run-scrim").classList.contains("open")){ye();return}if(document.getElementById("help-scrim").classList.contains("open")){be();return}if(document.getElementById("pane").classList.contains("open")){le();return}if(document.getElementById("pursuit-pane").classList.contains("open")){Ce();return}if(document.getElementById("config-scrim").classList.contains("open")){Pe();return}if(document.getElementById("sender-scrim").classList.contains("open")){He();return}document.getElementById("editor-scrim").classList.contains("open")&&fe()}});let Ne=null;const En={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function kt(e){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}Ne=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=En[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=r.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function ye(){document.getElementById("run-scrim").classList.remove("open"),Ne=null}document.getElementById("btn-enrich").onclick=()=>kt("enrich"),document.getElementById("btn-verdict").onclick=()=>kt("verdict"),document.getElementById("run-cancel").onclick=ye,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&ye()},document.getElementById("run-go").onclick=()=>{const e=Ne,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(ye(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),Se(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=fn;const $n={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Et(e){const t=$n[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const p=document.createElement("a");p.className="help-link",p.textContent="Learn more →",p.onclick=()=>{be(),xt(),Cn(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(p),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function be(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Et("add"),document.getElementById("help-run").onclick=()=>Et("run"),document.getElementById("help-close").onclick=be,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&be()},document.getElementById("add-cancel").onclick=me,document.getElementById("add-save").onclick=mt,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&me()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>dt(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",lt),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),mt()))}),document.getElementById("add-vertical-filter").addEventListener("input",ut),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&hn(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=gn,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=fe,document.getElementById("editor-save").onclick=kn,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&fe()},document.getElementById("sender-close").onclick=He,document.getElementById("sender-scrim").onclick=e=>{e.target.id==="sender-scrim"&&He()},document.getElementById("config-close").onclick=Pe,document.getElementById("config-scrim").onclick=e=>{e.target.id==="config-scrim"&&Pe()};function $t(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function Oe(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}Re()}const we='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',xn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>';function Re(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,n=r.stats&&r.stats.stale_verdicts||0,a=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),o=t&&typeof t.body=="string";let c="";if(a){let S="off",E="";t&&!t.reachable&&o?(S="warn",E="brain offline · using cache"):t&&t.stale?(S="warn",E="cached · stale"):o&&(S="ok",E="fetched "+$t(t.age_seconds)),c+=`<div class="crit-row">
      <span class="crit-what"><span class="pf-dot ${S}"></span>${o?'<span class="edit-link" id="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief"}</span>
      <button class="crit-edit" id="refresh-profile" title="re-distill the company-fit brief from the brain" aria-label="refresh company-fit brief">${xn}</button></div>`,E&&(c+=`<div class="crit-note dim small">${i(E)}</div>`)}else c+=`<div class="crit-row">
      <span class="crit-what">taste</span>
      <button class="crit-edit" id="edit-taste" title="edit taste.md" aria-label="edit taste">${we}</button></div>`,t&&t.configured&&(c+='<div class="crit-note dim small">brain offline — local fallback</div>');c+=`<div class="crit-row">
    <span class="crit-what">playbook</span>
    <button class="crit-edit" id="edit-playbook" title="edit playbook.md" aria-label="edit playbook">${we}</button></div>`,c+=`<div class="crit-row">
    <span class="crit-what">outreach identity</span>
    <button class="crit-edit" id="edit-sender" title="edit outreach identity" aria-label="edit outreach identity">${we}</button></div>`,c+=`<div class="crit-row">
    <span class="crit-what">outreach config</span>
    <button class="crit-edit" id="edit-config" title="edit outreach config (lint knobs + email structure)" aria-label="edit outreach config">${we}</button></div>`,n>0&&(c+=`<div class="stale">
      ...
      ${n} verdicts stale</div>`),e.innerHTML=c;const p=document.getElementById("view-profile");p&&(p.onclick=()=>Bn(r.profile));const u=document.getElementById("refresh-profile");u&&(u.onclick=In);const d=document.getElementById("edit-taste");d&&(d.onclick=()=>gt("taste"));const m=document.getElementById("edit-playbook");m&&(m.onclick=()=>gt("playbook"));const g=document.getElementById("edit-sender");g&&(g.onclick=vn);const j=document.getElementById("edit-config");j&&(j.onclick=bn)}async function In(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),Oe();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Oe();return}r.profile=await t.json(),Re(),l("company-fit brief refreshed"),_()}function Bn(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${$t(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Ve(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ve,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ve()};function xt(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");ke(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ue(){document.getElementById("docs-scrim").classList.remove("open")}function _n(){return document.getElementById("docs-scrim").classList.contains("open")}function ke(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Cn(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),ke(e)}document.getElementById("open-docs").onclick=xt,document.getElementById("docs-close").onclick=Ue,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Ue()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),ke(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&ke(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),r.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Ln(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Tn(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ee(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function De(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function jn(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=r.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(r.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function It(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=Ln(n.content);if(n.role==="user")s&&t.appendChild(Ee("user",s));else if(n.role==="assistant"){const a=Tn(n.content);if(!s&&!a.length)continue;const o=Ee("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(jn()),De()}async function $e(e,t,n){if(!r.meta||!r.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}r.chat.es&&(r.chat.es.close(),r.chat.es=null),r.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const p=await c.json();r.chat.threadId=p.thread.id,It(p.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+i(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Je(){r.chat.es&&(r.chat.es.close(),r.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function xe(e){r.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Bt(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function _t(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||r.chat.streaming||!r.chat.threadId)return;e.value="",Bt(),xe(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Ee("user",t));const a=Ee("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),De();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,xe(!1)},p=r.chat.threadId;let u;try{u=await fetch("/api/chat/"+p+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const d=new EventSource("/api/chat/"+p+"/stream");r.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,De()}),d.addEventListener("end",async m=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),xe(!1),r.chat.threadId===p&&await Sn(),Mn(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),d.onerror=()=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),xe(!1)}}async function Sn(){const e=r.chat.scope,t=r.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();It(a.messages||[])}catch{}}function Mn(){$(),w(),_(),r.openId&&Q(r.openId)}document.getElementById("open-chat").onclick=()=>$e("global","",""),document.getElementById("chat-close").onclick=Je,document.getElementById("chat-scrim").onclick=Je,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),_t()}),document.getElementById("chat-input").addEventListener("input",Bt),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),_t())}),$(),w(),_(),pn(),je(),Oe()}Pn({"":{view:()=>({mount(y){y.innerHTML=Vn,Un()}}),chrome:!1}},{title:"scout"});On();Rn();
