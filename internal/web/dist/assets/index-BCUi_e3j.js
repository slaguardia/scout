(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))I(i);new MutationObserver(i=>{for(const f of i)if(f.type==="childList")for(const k of f.addedNodes)k.tagName==="LINK"&&k.rel==="modulepreload"&&I(k)}).observe(document,{childList:!0,subtree:!0});function v(i){const f={};return i.integrity&&(f.integrity=i.integrity),i.referrerPolicy&&(f.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?f.credentials="include":i.crossOrigin==="anonymous"?f.credentials="omit":f.credentials="same-origin",f}function I(i){if(i.ep)return;i.ep=!0;const f=v(i);fetch(i.href,f)}})();function Vn(y,r){const v=r.replace(/^#/,"");let I=null;for(const i of Object.keys(y))(v===i||v.startsWith(i))&&(I===null||i.length>I.length)&&(I=i);return I===null&&""in y&&(I=""),I}function Dn(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function Un(y,r={}){const v=r.root??document.body,I=r.title??document.title??"",i=r.brandHref??"#",f=document.createElement("main"),k=document.createElement("header");k.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=I,w.setAttribute("aria-label",`${I} — home`),k.appendChild(w);const q=document.createElement("nav");q.className="cap-nav",q.setAttribute("aria-label","Views");for(const x of r.nav??[]){const C=document.createElement("a");C.href=x.href,C.textContent=x.label,x.ariaLabel&&C.setAttribute("aria-label",x.ariaLabel),q.appendChild(C)}k.appendChild(q);const j=document.createElement("section");j.className="tk-content",f.appendChild(k),f.appendChild(j);const T=document.createElement("div");T.className="tk-bleed";const Te=x=>{var C;for(const M of Array.from(q.querySelectorAll("a"))){const L=((C=M.getAttribute("href"))==null?void 0:C.replace(/^#/,""))??"";M.toggleAttribute("aria-current",x!==null&&x!==""&&L===x),M.hasAttribute("aria-current")&&M.setAttribute("aria-current","page")}};let z=0;const S=()=>{const x=Vn(y,location.hash);if(Te(x),x===null){T.isConnected&&T.remove(),f.isConnected||v.appendChild(f),Jn(j,"Not found.");return}const{view:C,chrome:M}=Dn(y[x]),L=M?j:T;M?(T.isConnected&&T.remove(),f.isConnected||v.appendChild(f)):(f.isConnected&&f.remove(),T.isConnected||v.appendChild(T)),L.replaceChildren();const F=C(),Ce=++z,re=F.mount(L);re instanceof Promise&&re.catch(je=>{Ce===z&&zn(L,String(je))})};window.addEventListener("hashchange",S),S()}function Jn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-empty",v.textContent=r,y.appendChild(v)}function zn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-error",v.textContent=r,y.appendChild(v)}function Fn(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const v of r)v.unregister()}),window.caches&&caches.keys().then(r=>{for(const v of r)caches.delete(v)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Wn(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let r;try{r=await y.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Kn=`
<div class="layout">
<aside class="sidebar">
...
`;function Yn(y){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null},v=e=>"pill pill-"+(e||"none"),I='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function k(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],N()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}r.jobs=e.rows||[],$(),q(),T()}function q(){if(!h.postingId)return;const e=r.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&Q())}let j=null;function T(){const e=r.jobs.some(t=>t.outreach_draft_status==="researching");e&&!j?j=setInterval(Te,4e3):!e&&j&&(clearInterval(j),j=null)}async function Te(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(r.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==r.jobs.length;r.jobs=t,s&&($(),q()),T()}function z(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Se(),e==="jobs"?$():N()}async function S(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,x()}function x(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,K()}function C(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function M(e){return e.slice().sort((t,n)=>r.sort.dir*C(t,n,r.sort.k))}const L=new Set;let F=!1;function Ce(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",L.has(e.dataset.v))})}function re(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(L.size&&!L.has(t.verdict||"__none__")||F&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const je=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Ht=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function tt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const nt=tt("scout-hidden-cols"),st=tt("scout-hidden-jcols");function at(){return r.view==="jobs"?{cols:Ht,hidden:st,key:"scout-hidden-jcols"}:{cols:je,hidden:nt,key:"scout-hidden-cols"}}function ce(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=nt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=st.has(e.dataset.col)?"none":""})}function Se(){const e=at();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=at(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),Se(),ce()})})}function N(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=M(re());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
    ...
    `,e.appendChild(s)}ce(),e.querySelectorAll("tr").forEach(n=>{n.addEventListener("click",s=>{s.target.closest("a, .flag-btn")||te(n.dataset.id)})}),e.querySelectorAll(".flag-btn").forEach(n=>{n.addEventListener("click",()=>vt(n.dataset.id))})}const A=new Set;let de=!1,le=!1,D=!0;function Pt(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=D&&!A.has("rejected");return r.jobs.filter(n=>!(t&&n.response==="rejected"||A.size&&!A.has(n.response||"")||de&&!n.next_up||le&&(n.outreach_count|0)>0||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const qt=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function ot(e){const t=String(e||"").split(",").map(n=>n.trim()).filter(Boolean);return t.length?t.map(n=>{const s=n.match(qt);return s?`<a href="mailto:${i(s[0])}" title="${i(n)}">${i(n)}</a>`:i(n)}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}const O={offer:{cls:"pill-yes",label:"offer",order:0},interview:{cls:"pill-info",label:"interview",order:1},screening:{cls:"pill-maybe",label:"screening call",order:2},"":{cls:"pill-none",label:"—",order:3},rejected:{cls:"pill-no",label:"rejected",order:4}};function Nt(e,t,n){var s,a;if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[e.verdict]??3)-(o[t.verdict]??3)}if(n==="response")return(((s=O[e.response])==null?void 0:s.order)??3)-(((a=O[t.response])==null?void 0:a.order)??3);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="applied_at"||n==="last_outreach_at"){const o=e[n]||"",c=t[n]||"";return!o&&!c?0:o?c?String(c).localeCompare(String(o)):-r.jsort.dir:r.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function Y(e){const t=e.options[e.selectedIndex],n=getComputedStyle(e),s=Y._c||(Y._c=document.createElement("canvas").getContext("2d"));s.font=`${n.fontWeight} ${n.fontSize} ${n.fontFamily}`;const a=s.measureText(t?t.text:"").width;e.style.width=Math.ceil(a+35)+"px"}function $(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=Pt().sort((d,m)=>r.jsort.dir*Nt(d,m,r.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=D&&!A.has("rejected")?r.jobs.filter(d=>d.response==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=r.jobs.filter(d=>d.next_up).length,o=document.getElementById("next-up-n");o.textContent=a,o.style.display=a?"":"none";const c=r.jobs.filter(d=>!(d.outreach_count|0)&&!(D&&!A.has("rejected")&&d.response==="rejected")).length,p=document.getElementById("not-reached-n");p.textContent=c,p.style.display=c?"":"none";const u=document.getElementById("jobs-hidden-note");u.style.display=n?"":"none",n&&(u.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{D=!1,document.getElementById("hide-rejected").classList.remove("is-on"),$()});for(const d of t){const m=O[d.response]||O[""],g=document.createElement("tr");g.dataset.id=d.posting_id;const P=[["","none"],["screening","screening"],["interview","interview"],["offer","offer"],["rejected","rejected"]].map(([_,J])=>`<option value="${_}"${(d.response||"")===_?" selected":""}>${J}</option>`).join("");g.innerHTML=`
      ...
      <td data-col="response"><select class="jt-resp ${m.cls}" title="furthest response reached">${P}</select></td>
  ...
  `,zt();const s=document.getElementById("pursuit-company-link");s&&s.addEventListener("click",()=>te(e.company_id)),H(document.getElementById("pursuit-title-input"),a=>ct(e,"title",a)),H(document.getElementById("pursuit-url-input"),a=>Vt(e,a)),H(document.getElementById("pursuit-notes-input"),a=>Ft(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>H(a,o=>ct(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),he(),ee()}function Jt(e){return`
  ...
  `;const P=document.getElementById("posting-add-btn");P&&P.addEventListener("click",()=>mn(e)),Ne(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Le=>Le.classList.remove("is-on")),b.classList.add("is-on")})});const U=document.getElementById("ve-save-btn");U&&U.addEventListener("click",()=>hn(e)),H(document.getElementById("pane-title-input"),b=>dt(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>H(b,Le=>dt(e,b.dataset.k,Le))),H(document.getElementById("pane-domain-input"),b=>Dt(e,b)),H(document.getElementById("pane-notes-input"),b=>Ut(e,b),{multiline:!0});const _=document.getElementById("flag-toggle-btn");_&&_.addEventListener("click",()=>vt(e.company_id));const J=document.getElementById("review-stamp-btn");J&&J.addEventListener("click",()=>pn(e.company_id));const E=document.getElementById("rescore-btn");E&&E.addEventListener("click",()=>Re("verdict",{company_ids:[e.company_id]}));const B=document.getElementById("reenrich-btn");B&&B.addEventListener("click",()=>Re("enrich",{company_ids:[e.company_id]}))}function un(e){return`
    <div class="ie-grid">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function K(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,s=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let E="off",B="";const b=t&&t.criteria_state;b==="current"?(E="ok",B="current · verified "+Fe(t.verified_age_seconds)):b==="changed"?(E="warn",B="changed — re-distill"):b==="unverified"?(E="warn",B=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(E="warn",B="brain offline · using cache"):a&&(E="ok",B="fetched "+Fe(t.age_seconds)),o=ie({icon:Ct,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:E,note:B,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:Tt,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=ie({icon:Ct,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:Ke,actTitle:"edit taste.md",actLabel:"edit taste"});const c=r.sources&&r.sources.sources||[],p=c.filter(E=>E.need==="experience").length,u=c.filter(E=>E.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";p>0?(d="ok",m=`${p} experience · ${u} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const g=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",P=ie({icon:Tn,nameHTML:g,dot:d,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:Tt,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),U=ie({icon:Bn,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:Ke,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),_=ie({icon:Ln,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:Ke,actTitle:"edit the outreach email template",actLabel:"edit email template"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${U}${_}
     </div>`;const J={"view-profile":()=>Sn(r.profile),"refresh-profile":jn,"edit-taste":()=>we("taste"),"edit-playbook":()=>we("playbook"),"edit-template":()=>we("outreach-template"),"view-sources":De,"refresh-sources":Cn};e.querySelectorAll("[data-act]").forEach(E=>{const B=E.dataset.act;B&&J[B]&&(E.onclick=J[B])})}async function Ye(){try{r.sources=await(await fetch("/api/outreach/sources")).json()}catch{r.sources=null}K()}async function Cn(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){l(`refresh failed: ${s.message}`),Ye();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`refresh failed: ${s||"HTTP "+t.status}`),Ye();return}const n=await t.json();n.warning?l(n.warning):l("outreach knowledge refreshed"),r.sources={sources:n.sources||[],needs:r.sources&&r.sources.needs||[]},K()}async function jn(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),We();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),We();return}r.profile=await t.json(),K(),l("company-fit brief refreshed"),S()}function Sn(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${Fe(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Ge(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ge,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ge()};function jt(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Ie(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Qe(){document.getElementById("docs-scrim").classList.remove("open")}function Mn(){return document.getElementById("docs-scrim").classList.contains("open")}function Ie(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function An(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ie(e)}document.getElementById("open-docs").onclick=jt,document.getElementById("docs-close").onclick=Qe,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Qe()};function Hn(){document.getElementById("settings-scrim").classList.add("open"),K()}function Ze(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Hn,document.getElementById("settings-close").onclick=Ze,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&Ze()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ie(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&Ie(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),r.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Pn(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function qn(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function xe(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function Xe(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Nn(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=r.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(r.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function St(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=Pn(n.content);if(n.role==="user")s&&t.appendChild(xe("user",s));else if(n.role==="assistant"){const a=qn(n.content);if(!s&&!a.length)continue;const o=xe("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Nn()),Xe()}async function _e(e,t,n){if(!r.meta||!r.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}r.chat.es&&(r.chat.es.close(),r.chat.es=null),r.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const p=await c.json();r.chat.threadId=p.thread.id,St(p.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+i(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function et(){r.chat.es&&(r.chat.es.close(),r.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Be(e){r.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Mt(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function At(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||r.chat.streaming||!r.chat.threadId)return;e.value="",Mt(),Be(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(xe("user",t));const a=xe("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),Xe();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Be(!1)},p=r.chat.threadId;let u;try{u=await fetch("/api/chat/"+p+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const d=new EventSource("/api/chat/"+p+"/stream");r.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,Xe()}),d.addEventListener("end",async m=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Be(!1),r.chat.threadId===p&&await On(),Rn(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),d.onerror=()=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),Be(!1)}}async function On(){const e=r.chat.scope,t=r.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();St(a.messages||[])}catch{}}function Rn(){k(),w(),S(),r.openId&&te(r.openId)}document.getElementById("open-chat").onclick=()=>_e("global","",""),document.getElementById("chat-close").onclick=et,document.getElementById("chat-scrim").onclick=et,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),At()}),document.getElementById("chat-input").addEventListener("input",Mt),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),At())}),k(),w(),S(),gn(),Oe(),We(),Ye()}Un({"":{view:()=>({mount(y){y.innerHTML=Kn,Yn()}}),chrome:!1}},{title:"scout"});Fn();Wn();
