(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))C(r);new MutationObserver(r=>{for(const v of r)if(v.type==="childList")for(const L of v.addedNodes)L.tagName==="LINK"&&L.rel==="modulepreload"&&C(L)}).observe(document,{childList:!0,subtree:!0});function y(r){const v={};return r.integrity&&(v.integrity=r.integrity),r.referrerPolicy&&(v.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?v.credentials="include":r.crossOrigin==="anonymous"?v.credentials="omit":v.credentials="same-origin",v}function C(r){if(r.ep)return;r.ep=!0;const v=y(r);fetch(r.href,v)}})();function Hs(g,i){const y=i.replace(/^#/,"");let C=null;for(const r of Object.keys(g))(y===r||y.startsWith(r))&&(C===null||r.length>C.length)&&(C=r);return C===null&&""in g&&(C=""),C}function Ps(g){return typeof g=="function"?{view:g,chrome:!0}:{view:g.view,chrome:g.chrome!==!1}}function qs(g,i={}){const y=i.root??document.body,C=i.title??document.title??"",r=i.brandHref??"#",v=document.createElement("main"),L=document.createElement("header");L.className="cap-head";const k=document.createElement("a");k.className="brand",k.href=r,k.textContent=C,k.setAttribute("aria-label",`${C} — home`),L.appendChild(k);const D=document.createElement("nav");D.className="cap-nav",D.setAttribute("aria-label","Views");for(const x of i.nav??[]){const B=document.createElement("a");B.href=x.href,B.textContent=x.label,x.ariaLabel&&B.setAttribute("aria-label",x.ariaLabel),D.appendChild(B)}L.appendChild(D);const M=document.createElement("section");M.className="tk-content",v.appendChild(L),v.appendChild(M);const j=document.createElement("div");j.className="tk-bleed";const Oe=x=>{var B;for(const q of Array.from(D.querySelectorAll("a"))){const Y=((B=q.getAttribute("href"))==null?void 0:B.replace(/^#/,""))??"";q.toggleAttribute("aria-current",x!==null&&x!==""&&Y===x),q.hasAttribute("aria-current")&&q.setAttribute("aria-current","page")}};let ve=0;const X=()=>{const x=Hs(g,location.hash);if(Oe(x),x===null){j.isConnected&&j.remove(),v.isConnected||y.appendChild(v),Os(M,"Not found.");return}const{view:B,chrome:q}=Ps(g[x]),Y=q?M:j;q?(j.isConnected&&j.remove(),v.isConnected||y.appendChild(v)):(v.isConnected&&v.remove(),j.isConnected||y.appendChild(j)),Y.replaceChildren();const ye=B(),V=++ve,G=ye.mount(Y);G instanceof Promise&&G.catch(Ne=>{V===ve&&Ns(Y,String(Ne))})};window.addEventListener("hashchange",X),X()}function Os(g,i){g.replaceChildren();const y=document.createElement("div");y.className="tk-empty",y.textContent=i,g.appendChild(y)}function Ns(g,i){g.replaceChildren();const y=document.createElement("div");y.className="tk-error",y.textContent=i,g.appendChild(y)}function Rs(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(i=>{for(const y of i)y.unregister()}),window.caches&&caches.keys().then(i=>{for(const y of i)caches.delete(y)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Us(){let g;try{g=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!g.ok)return null;let i;try{i=await g.json()}catch{return null}return typeof i.email=="string"&&i.email?{email:i.email}:null}const Ds=`
<div class="layout">
<aside class="sidebar">
...
`;function Vs(g){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},followups:[],followUpInterval:7,openDetail:null,anthropicKey:null},y=e=>"pill pill-"+(e||"none"),C='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),v=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function L(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],A()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],$(),D(),j()}function D(){if(!m.postingId)return;const e=i.jobs.find(t=>t.posting_id===m.postingId);e&&(m.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&Z())}let M=null;function j(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!M?M=setInterval(Oe,4e3):!e&&M&&(clearInterval(M),M=null)}async function Oe(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&($(),D()),j()}async function ve(){try{const t=await fetch("/api/settings/follow-up-interval");if(!t.ok)return;const n=await t.json();typeof n.days=="number"&&n.days>0&&(i.followUpInterval=n.days)}catch{}const e=document.getElementById("fu-interval");e&&(e.value=i.followUpInterval),i.view==="jobs"&&$()}async function X(){let e;try{const t=await fetch("/api/follow-ups");if(!t.ok){i.followups=[],De();return}e=await t.json()}catch{i.followups=[],De();return}if(i.followups=e.follow_ups||[],typeof e.interval_days=="number"&&e.interval_days>0){i.followUpInterval=e.interval_days;const t=document.getElementById("fu-interval");t&&(t.value=i.followUpInterval)}De(),we()}function x(e){i.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("tab-followups").classList.toggle("active",e==="follow-ups"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("followups-view").style.display=e==="follow-ups"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",document.getElementById("block-columns").style.display=e==="follow-ups"?"none":"",Re(),e==="jobs"?$():e==="follow-ups"?X():A()}async function B(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}i.stats=e,q()}function q(){const e=i.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,N()}function Y(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function ye(e){return e.slice().sort((t,n)=>i.sort.dir*Y(t,n,i.sort.k))}const V=new Set;let G=!1;function Ne(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",V.has(e.dataset.v))})}function gt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(V.size&&!V.has(t.verdict||"__none__")||G&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const rn=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],cn=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function bt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const wt=bt("scout-hidden-cols"),kt=bt("scout-hidden-jcols");function Et(){return i.view==="jobs"?{cols:cn,hidden:kt,key:"scout-hidden-jcols"}:{cols:rn,hidden:wt,key:"scout-hidden-cols"}}function ee(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=wt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=kt.has(e.dataset.col)?"none":""})}function Re(){const e=Et();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=Et(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),Re(),ee()})})}function $t(e){return`
    ...
    `}function It(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>Dt(t.dataset.id))}const dn=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],ln=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function _t(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let c=0;c<n;c++){const l=t.map(([p,d])=>{const h=d.endsWith("%")?Math.round(parseFloat(d)*a[c%a.length])+"%":d;return`<td${p?` data-col="${p}"`:""}><span class="skel" style="width:${h}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${l}</tr>`}s.innerHTML=o,ee()}function A(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=ye(gt());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=$t(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||le(s.dataset.id)}),e.appendChild(s),It(s)}ee()}async function un(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=ye(gt()).map(c=>c.company_id),o=[...s.querySelectorAll("tr")].map(c=>c.dataset.id);if(a.length!==o.length||a.some((c,l)=>c!==o[l])){A();return}for(const c of e){const l=i.rows.find(d=>d.company_id===c),p=s.querySelector(`tr[data-id="${CSS.escape(c)}"]`);if(!l||!p){A();return}p.innerHTML=$t(l),It(p)}ee()}const O=new Set;let ge=!1,be=!1,Q=!0;function pn(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=Q&&!O.has("rejected");return i.jobs.filter(n=>!(t&&n.response==="rejected"||O.size&&!O.has(n.response||"")||ge&&!n.next_up||be&&(n.outreach_count|0)>0||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const hn=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function xt(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(hn),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}function mn(e){const t=(e||[]).map(n=>({position:(n.position||"").trim(),email:(n.email||"").trim()})).filter(n=>n.position||n.email);return t.length?JSON.stringify(t):""}function Bt(e){const t=xt(e);return t.length?t.map(n=>{const s=r(n.position||n.email);if(!n.email)return s;const a=r(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${r(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function Lt(e){return`<div class="cc-row">
    ...
    `;const P=()=>new Date().toISOString().slice(0,10);f.querySelector(".jt-nextup").onclick=()=>At(d,!1),f.querySelector(".jt-applied").onclick=()=>ie(d,{applied_at:d.applied_at?"":P()}),f.querySelector(".jt-resp").onchange=_=>{ae(_.target),ie(d,{response:_.target.value})},f.querySelector(".jt-ostatus").onchange=_=>ie(d,{outreach_status:_.target.value}),f.querySelector(".jt-inc").onclick=()=>ie(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:P()}),f.querySelector(".jt-dec").onclick=()=>{const _=Math.max(0,(d.outreach_count||0)-1);ie(d,{outreach_count:_,..._===0?{last_outreach_at:""}:{}})},e.appendChild(f)}e.querySelectorAll(".jt-resp").forEach(ae),ee(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",h=>{h.target.closest("a, button, select")||ke(d.dataset.id)})}),we()}const yn=[["","none"],["awaiting","awaiting"],["replied","replied"],["no_response","no response"]];function gn(e){if(!e)return null;const t=Date.parse(new Date().toISOString().slice(0,10)),n=Date.parse(e);return isNaN(n)?null:Math.round((t-n)/864e5)}function Ue(e){if((e.outreach_status||"")!=="awaiting"||e.response)return!1;const t=gn(e.last_outreach_at);return t!==null&&t>=(i.followUpInterval||7)}function bn(){return'<span class="draft-badge db-followup" title="overdue for a follow-up — awaiting a reply past your cadence">follow-up due</span>'}function we(){const e=document.getElementById("followups-n");if(!e)return;const t=i.jobs.filter(Ue).length;e.textContent=t,e.style.display=t?"":"none"}function wn(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}function De(){const e=document.querySelector("#fut tbody");if(!e)return;e.innerHTML="";const t=i.followups||[];document.getElementById("followups-empty").style.display=t.length?"none":"block";const n=document.getElementById("fu-interval");n&&document.activeElement!==n&&(n.value=i.followUpInterval);for(const s of t){const a=document.createElement("tr");a.dataset.id=s.posting_id;const o=s.days_overdue|0,c=o<=0?"due today":`${o} day${o===1?"":"s"} overdue`,l=o>=14?"fu-od-high":o>=7?"fu-od-mid":"fu-od-low";a.innerHTML=`
      ...
      </td>`,a.querySelector(".fu-replied").onclick=()=>Tt(s,"replied"),a.querySelector(".fu-noresp").onclick=()=>Tt(s,"no_response"),a.querySelector(".fu-open").onclick=()=>ke(s.posting_id),e.appendChild(a)}e.querySelectorAll("tr").forEach(s=>{s.addEventListener("click",a=>{a.target.closest("a, button")||ke(s.dataset.id)})})}async function Tt(e,t){let n=i.jobs.find(a=>a.posting_id===e.posting_id);if(n||(await k(),n=i.jobs.find(a=>a.posting_id===e.posting_id)),!n){u("posting not found — refresh");return}await $e(n,{outreach_status:t})&&(u(t==="replied"?"marked replied":"marked no response"),await X(),i.view==="jobs"&&$(),m.postingId===e.posting_id&&(m.row=n,Z()),we())}const m={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function ke(e){let t=i.jobs.find(n=>n.posting_id===e);if(t||(await k(),t=i.jobs.find(n=>n.posting_id===e)),!t){u("posting not found — refresh");return}Je(),Ke(),m.postingId=e,m.row=t,m.drafts=[],m.openHist=!1,m.answers=[],m.detecting=!1,m.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),Ct("pursuit"),Z(),oe(),ce()}let Ve=null;function Ct(e){Ve=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function Ee(){Je(),Ke(),m.postingId=null,m.row=null,m.drafts=[],m.answers=[],m.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function Je(){m.poll&&(clearInterval(m.poll),m.poll=null)}async function oe(){if(!m.postingId)return;let e;try{const n=await fetch(`/api/postings/${m.postingId}/outreach`);if(!n.ok){Ie();return}e=await n.json()}catch{Ie();return}m.drafts=e.drafts||[],Ie();const t=m.drafts[0];t&&t.status==="researching"?kn():Je()}function kn(){m.poll||(m.poll=setInterval(oe,4e3))}function H(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),u(`save failed: ${o.message}`)}})}async function St(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,summary:o.summary,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),$(),pe(e.posting_id,{title:o.title,location:o.location,summary:o.summary})}async function En(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",v(e.url)),pe(e.posting_id,{url:s.url})}function $n(e){const t=document.getElementById("pursuit-company-view"),n=document.getElementById("pursuit-company-relink"),s=document.getElementById("pursuit-company-edit"),a=document.getElementById("pursuit-company-input"),o=document.getElementById("pursuit-company-save"),c=document.getElementById("pursuit-company-cancel");if(!t||!n||!s||!a||!o||!c)return;const l=document.getElementById("relink-company-names");l.innerHTML=(i.rows||[]).map(h=>`<option value="${r(h.name)}">`).join("");const p=h=>{n.hidden=!h,t.hidden=h,h&&(a.value=e.company||"",a.focus(),a.select())},d=async()=>{const h=a.value.trim();if(!h){p(!1);return}const f=i.rows||[],b=f.find(w=>w.name===h)||f.find(w=>w.name.toLowerCase()===h.toLowerCase());if(!b){u(`no company named "${h}" — add it first`);return}if(b.company_id===e.company_id){p(!1);return}try{await In(e,b.company_id),u(`moved to ${e.company}`)}catch(w){u(`move failed: ${w.message}`)}};s.addEventListener("click",()=>p(!0)),c.addEventListener("click",()=>p(!1)),o.addEventListener("click",d),a.addEventListener("keydown",h=>{h.key==="Enter"?(h.preventDefault(),d()):h.key==="Escape"&&(h.preventDefault(),p(!1))})}async function In(e,t){const n=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.company_id=s.company_id,e.company=s.company_name,Z(),k()}async function jt(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),L(),k()}async function _n(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();i.openId=s.company_id,ue(s),he(s.company_id),L(),k()}async function xn(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}let Mt=null;function Z(){const e=m.row;if(!e)return;const t=document.getElementById("pursuit-body"),s=!!t&&Mt===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${r(e.title||"")}">`;const a=J[e.response]||J[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a.cls}">${r(a.label)}</span>`+(e.verdict?` <span class="${y(e.verdict)}">${r(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>Pe("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    <section class="pane-section role-head">
      <div id="role-body">${Bn(e)}</div>
    </section>
  ...
  `,Ln();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>le(e.company_id)),$n(e),H(document.getElementById("pursuit-title-input"),l=>St(e,"title",l)),H(document.getElementById("pursuit-url-input"),l=>En(e,l)),H(document.getElementById("pursuit-notes-input"),l=>Tn(l),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(l=>H(l,p=>St(e,l.dataset.k,p),{multiline:l.tagName==="TEXTAREA"})),Ie(),de(),t&&(t.scrollTop=s),Mt=e.posting_id}function Bn(e){return`
    ...
    </div>`}function Ln(){const e=m.row,t=()=>new Date().toISOString().slice(0,10),n=document.querySelector("#pursuit-body .pt-applied");n&&n.addEventListener("click",()=>te({applied_at:e.applied_at?"":t()}));const s=document.querySelector("#pursuit-body .pl-applied-date");s&&s.addEventListener("change",l=>te({applied_at:l.target.value}));const a=document.querySelector("#pursuit-body .pl-response");a&&a.addEventListener("change",l=>te({response:l.target.value}));const o=document.querySelector("#pursuit-body .pl-ostatus");o&&o.addEventListener("change",l=>te({outreach_status:l.target.value}));const c=document.querySelector("#pursuit-body .pt-nextup");c&&c.addEventListener("click",()=>At(m.row,!0))}async function At(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){u(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();u(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,$(),pe(e.posting_id,{next_up:s.next_up}),t&&Z(),u(e.next_up?"queued next up":"removed from the queue")}async function $e(e,t){const n={applied_at:e.applied_at||"",response:e.response||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",outreach_status:e.outreach_status||"",contacts:e.contacts||"",notes:e.notes||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return u(`save failed: ${o.message}`),null}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();return u(`save failed: ${o||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,outreach_status:a.outreach_status,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),pe(e.posting_id,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function Tn(e){const t=m.row,n={applied_at:t.applied_at||"",response:t.response||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",outreach_status:t.outreach_status||"",contacts:t.contacts||"",notes:e},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,$()}async function te(e){await $e(m.row,e)&&($(),Z(),u("tracking saved"))}async function Ht(e){await $e(m.row,{contacts:e})&&$()}async function ie(e,t){await $e(e,t)&&($(),m.postingId===e.posting_id&&(m.row=e,Z()),u("tracking saved"))}function Ie(){const e=document.getElementById("outreach-section");if(!e)return;const t=m.row,n=m.drafts,s=n[0]||null,a=n.slice(1),o=`
    ...
    ${fn(t.contacts)}`,l=s&&(Cn(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,p=a.length?`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?qt(s,!1):""}</div><div class="draft-actions">${l}</div>`+p,Pn()}function Cn(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const Sn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',jn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',Pt=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${Sn}</button>`,Fe=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],Mn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function An(e){let t=Fe.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${Fe.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",c=a<t?Mn:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${Fe[t].active}…</span></div>
  </div>`}function qt(e,t){const n=(d,h,f="")=>`
    ...
    </div>`;if(e.status==="failed"){const d=Hn(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    ${re(e)}
      ...
      </div></details>`}return t}function Ot(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${r(n.message||"")}"><code>${r(n.code||"")}</code>${r(n.message||"")}</span>`).join("")+"</div>":""}function Hn(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${r(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${r(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function Pn(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#contacts-editor");if(t){const p=t.querySelector(".cc-list"),d=()=>mn(Array.from(p.querySelectorAll(".cc-row")).map(f=>({position:f.querySelector(".cc-pos").value,email:f.querySelector(".cc-email").value}))),h=f=>{f.querySelectorAll(".cc-pos, .cc-email").forEach(b=>{b.addEventListener("change",()=>Ht(d())),b.addEventListener("keydown",w=>{w.key==="Enter"&&(w.preventDefault(),w.target.blur())})}),f.querySelector(".cc-del").addEventListener("click",()=>{f.remove(),Ht(d())})};p.querySelectorAll(".cc-row").forEach(h),t.querySelector(".cc-add").addEventListener("click",()=>{const f=document.createElement("div");f.innerHTML=Lt({position:"",email:""});const b=f.firstElementChild;p.appendChild(b),h(b),b.querySelector(".cc-pos").focus()})}const n=()=>new Date().toISOString().slice(0,10),s=m.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>te({outreach_count:(s.outreach_count||0)+1,last_outreach_at:n()}));const o=e.querySelector(".pt-outreach-dec");o&&o.addEventListener("click",()=>{const p=Math.max(0,(s.outreach_count||0)-1);te({outreach_count:p,...p===0?{last_outreach_at:""}:{}})});const c=e.querySelector("#draft-start-btn");c&&c.addEventListener("click",()=>_e()),e.querySelectorAll(".draft-retry-btn").forEach(p=>p.addEventListener("click",()=>_e())),e.querySelectorAll(".draft-regen-btn").forEach(p=>p.addEventListener("click",()=>_e(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(p=>{const d=p.dataset.did,h=p.querySelector(".draft-textarea");h&&H(h,w=>On(d,w),{multiline:!0});const f=p.querySelector(".draft-sent-btn");f&&f.addEventListener("click",()=>Nn(d));const b=p.querySelector(".draft-copy-btn");b&&b.addEventListener("click",()=>{const w=p.querySelector(".draft-textarea"),R=p.querySelector(".draft-sentbody"),P=w?w.value:R?R.textContent:"";ns(P,"email copied")})});const l=e.querySelector("details.draft-history");l&&l.addEventListener("toggle",()=>{m.openHist=l.open})}async function _e(e=!1){const t=document.getElementById("outreach-section"),n=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));n&&(n.disabled=!0);let s;try{const o=e?"?regenerate=1":"";s=await fetch(`/api/postings/${m.postingId}/outreach${o}`,{method:"POST"})}catch(o){u(`draft failed: ${o.message}`),n&&(n.disabled=!1);return}if(s.status===202){let o={};try{o=await s.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&u(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await oe(),k();return}if(s.status===409){await oe(),u("a draft is already active");return}if(s.status===412){let o={};try{o=await s.json()}catch{}qn(o.need,o.error),n&&(n.disabled=!1);return}if(s.status===503){const o=document.getElementById("outreach-section");if(o){const c=document.createElement("div");c.className="draft-note",c.textContent="Outreach engine not running in this build.",o.appendChild(c)}n&&(n.disabled=!1);return}const a=(await s.text().catch(()=>"")).trim();u(`draft failed: ${a||"HTTP "+s.status}`),n&&(n.disabled=!1)}function qn(e,t){const n=document.getElementById("outreach-section");if(!n)return;const s=n.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"Discover sources",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,s?s.replaceWith(c):n.appendChild(c);const l=c.querySelector("#gate-fix-btn");l&&l.addEventListener("click",()=>a?ne("outreach-template"):tt());const p=c.querySelector("#gate-retry-btn");p&&p.addEventListener("click",_e)}async function On(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=m.drafts.findIndex(l=>String(l.id)===String(e));a>=0&&(m.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const l=c.querySelector(".lint-chips"),p=Ot(s.lint);l?l.outerHTML=p||"":p&&o.insertAdjacentHTML("afterend",p)}}async function Nn(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){u(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`failed: ${s||"HTTP "+t.status}`);return}u("marked sent"),await oe(),await k();const n=i.jobs.find(s=>s.posting_id===m.postingId);n&&pe(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function ce(){if(!m.postingId)return;let e;try{const t=await fetch(`/api/postings/${m.postingId}/answers`);if(!t.ok){de();return}e=await t.json()}catch{de();return}m.answers=e.answers||[],m.answersStatus=e.questions_status||"",de(),m.answers.some(t=>t.status==="generating")?Rn():Ke()}function Rn(){m.answersPoll||(m.answersPoll=setInterval(ce,4e3))}function Ke(){m.answersPoll&&(clearInterval(m.answersPoll),m.answersPoll=null)}function de(){const e=document.getElementById("answers-section");if(!e)return;const t=m.answers,n=m.answersStatus,s=t.some(d=>d.status==="generating"),a=t.length?`<div class="answers-list">${t.map(Vn).join("")}</div>`:"",o=!!m.detecting,c=s||o?" disabled":"",l=d=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":d}</button>`;let p;n==="ok"&&t.length?p=`<button class="btn ${t.some(h=>!Nt(h)&&h.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect"):n===""||n==="unreachable"?p=`<button class="btn btn-primary" id="answers-start-btn"${c}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect questions"):p=l("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r(Un(n,t.length))}</div>`+a+`<div class="answers-actions">${p}</div>`,Fn()}function Un(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function Nt(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Dn(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function Vn(e){const t=Nt(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${r(Jn(e.fail_reason))}</div>`:""}
  </div>`}function Jn(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function Fn(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",Rt);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",Kn),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(H(o,l=>Wn(a,l),{multiline:!0}),o.addEventListener("input",()=>zn(s,o)));const c=s.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>Yn(a))})}function zn(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function Rt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${m.postingId}/answers`,{method:"POST"})}catch(a){u(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await ce();return}if(n.status===412){let a={};try{a=await n.json()}catch{}Gn(a.error),t&&(t.disabled=!1);return}if(n.status===503){Ut("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();u(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function Kn(){m.detecting=!0,de();try{const e=await fetch(`/api/postings/${m.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();u(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){u(`detect failed: ${e.message}`)}m.detecting=!1,await ce()}async function Wn(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=m.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(m.answers[a]=s)}async function Yn(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){u(`regenerate failed: ${n.message}`);return}if(t.status===503){Ut("Answer generation isn't running in this build.");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`regenerate failed: ${n||"HTTP "+t.status}`);return}await ce()}function Gn(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
  ...
  `:'<div class="muted">Not yet scored. Run <code>scout verdict</code>, or set one by hand below.</div>',a=`
    <div class="verdict-edit" id="verdict-edit">
      ...
      <div class="ve-pick" id="ve-pick">
        ${["yes","maybe","no"].map(E=>`<button type="button" class="ve-opt${e.has_verdict&&e.verdict===E?" is-on":""}" data-v="${E}">${E}</button>`).join("")}
      </div>
      <div class="prow">
  ...
  `:'<div class="muted">No enrichment yet. Run <code>scout enrich</code>.</div>',c=!i.meta||i.meta.control!==!1,l=c&&i.meta&&i.meta.verdict?'<button class="h3-action" id="rescore-btn" title="re-score just this company — replaces the current verdict, manual or not">↻ re-score</button>':"",p=c&&e.domain?'<button class="h3-action" id="reenrich-btn" title="re-fetch this company’s site now">↻ re-enrich</button>':"",d=Object.keys(e.raw_json||{}).sort(),h=d.length===0?"":`
    <details class="raw-json">
      ...
      <table><tbody>
        ${d.map(E=>`<tr><td class="k">${r(E)}</td><td>${r(e.raw_json[E])}</td></tr>`).join("")}
      </tbody></table>
    </details>
  `,f=`
    <div class="flag-bar">
  ...
  `;const b=document.getElementById("posting-add-btn");b&&b.addEventListener("click",()=>es(e)),Ye(),document.querySelectorAll("#ve-pick .ve-opt").forEach(E=>{E.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(W=>W.classList.remove("is-on")),E.classList.add("is-on")})});const w=document.getElementById("ve-save-btn");w&&w.addEventListener("click",()=>Xn(e)),H(document.getElementById("pane-title-input"),E=>jt(e,"name",E)),document.querySelectorAll("#facts-body [data-k]").forEach(E=>H(E,W=>jt(e,E.dataset.k,W))),H(document.getElementById("pane-domain-input"),E=>_n(e,E)),H(document.getElementById("pane-notes-input"),E=>xn(e,E),{multiline:!0});const R=document.getElementById("flag-toggle-btn");R&&R.addEventListener("click",()=>Dt(e.company_id));const P=document.getElementById("review-stamp-btn");P&&P.addEventListener("click",()=>Zn(e.company_id));const _=document.getElementById("rescore-btn");_&&_.addEventListener("click",()=>Ze("verdict",{company_ids:[e.company_id]}));const U=document.getElementById("reenrich-btn");U&&U.addEventListener("click",()=>Ze("enrich",{company_ids:[e.company_id]}))}function Qn(e){return`
    <div class="ie-grid">
    ...
    </div>`}function Be(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let Vt;function u(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(Vt),Vt=setTimeout(()=>t.classList.remove("show"),2200)}async function ns(e,t="copied"){if(!e){u("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}u(t)}catch(n){u(`copy failed: ${n.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function Ge(){try{const s=await fetch("/api/meta");if(!s.ok)return;i.meta=await s.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=i.meta.chat?"":"none")}async function Qe(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Le=null;async function Ze(e,t){if(i.meta&&i.meta.control===!1){u("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){u(`run failed: ${a.message}`);return}if(n.status===409){u("a job is already running");return}if(n.status===412){const a=await n.text();u(a.trim());return}if(!n.ok){u(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Gt(e,s,t)}async function ss(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){u(`upload failed: ${a.message}`);return}if(n.status===409){u("a job is already running");return}if(!n.ok){u(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Gt("ingest",s)}const as=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let Te=[],F=new Set,z="company";function Jt(e){z=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",Ft()}function Xe(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function Ft(){const e=document.getElementById("add-note");Xe()?e.innerHTML=z==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=z==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function os(){as.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",F=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),Jt(i.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(s=>`<option value="${r(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),Te=s.verticals||[]}catch{Te=[]}zt()}function Ce(){document.getElementById("add-scrim").classList.remove("open")}function zt(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=Te.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${F.has(s)?" sel":""}" data-v="${r(s)}">${r(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;F.has(a)?F.delete(a):F.add(a),s.classList.toggle("sel"),Kt()}))):e.innerHTML=`<div class="none">${Te.length?"no match":"no verticals in the set yet"}</div>`,Kt()}function Kt(){const e=F.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Wt(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Yt(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){u(z==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,Xe()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=f=>document.getElementById(f).value.trim(),c=Xe();let l,p;c?(l="/api/capture",p={url:Wt(t),kind:z==="company"?"company_page":"job_posting",fields:z==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...F].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):z==="company"?(l="/api/companies",p={website:t,name:o("add-name"),vertical:[...F].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(l="/api/postings",p={url:Wt(t),title:o("add-title"),company:o("add-job-company")});let d;try{d=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)})}catch(f){u(`add failed: ${f.message}`),a();return}if(!d.ok){let f=`HTTP ${d.status}`;try{const b=await d.text();try{f=JSON.parse(b).error||f}catch{f=b.trim()||f}}catch{}if(a(),d.status===409){u(f||"That company is already in the list."),e.focus(),e.select();return}u(`add failed: ${f}`);return}const h=await d.json();if(a(),c&&!h.company_id){u(h.note||"couldn't classify that page");return}if(Ce(),L(),B(),k(),z==="job"){const f=h.posting&&h.posting.title||"job link";u(`tracking: ${f} @ ${h.company_name}${h.posting_updated?" (refreshed)":""}`),x("jobs")}else c?(u(h.company_created?`company added: ${h.company_name}`:`${h.company_name} is already in the list`),le(h.company_id)):u("company added")}function Gt(e,t,n){Le=t;const s=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="",s.classList.add("open"),Qe();const o=new EventSource(`/api/jobs/${t}/stream`),c=(l,p)=>{const d=document.createElement("div"),h=!p&&/^\s*warn:/i.test(l);d.className="ln"+(p?" ln-err":h?" ln-warn":""),d.textContent=h?l.replace(/^\s*warn:\s*/i,"⚠ "):l,a.appendChild(d),a.scrollTop=a.scrollHeight};o.addEventListener("line",l=>c(l.data,/error|failed/i.test(l.data))),o.addEventListener("end",l=>{o.close(),Le=null,c(`— ${l.data} —`,l.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",u(`${e} ${l.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?un(n.company_ids):L(),B(),Qe(),k(),i.openId&&le(i.openId)}),o.onerror=()=>{o.close()}}async function is(){if(Le)try{await fetch(`/api/jobs/${Le}/cancel`,{method:"POST"})}catch{}}let S=null;const rs={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function et(e){if(e==="outreach-template")return"outreach template";if(e==="taste-filter")return"pre-filter rules";if(e==="playbook")return"playbook";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(rs[t]||t)+" prompt"}return e+".md"}async function ne(e){S=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+et(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=!!e&&e.startsWith("outreach-prompts/"),s=n?e.slice(17):"",a=e==="taste-filter"||n&&s!=="fill";document.getElementById("editor-toggle-row").style.display=a?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",a&&(document.getElementById("editor-toggle-label").textContent=e==="taste-filter"?"Enable the pre-filter (off → bulk verdict runs score every company; the rules below are kept either way)":"Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const l=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${l||"HTTP "+o.status}`;return}const c=await o.json();document.getElementById("editor-text").value=c.content||"",a&&(document.getElementById("editor-enabled").checked=c.enabled!==!1),c.taste_version&&(document.getElementById("editor-ver").textContent="version "+c.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Se(){document.getElementById("editor-scrim").classList.remove("open"),S=null}const cs=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function tt(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{st(await(await fetch("/api/outreach/sources")).json())}catch(e){u(`failed to load sources: ${e.message}`)}}function nt(){document.getElementById("sources-scrim").classList.remove("open")}function st(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):cs,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],c=o.length?o.map(l=>`<li><span class="src-title">${r(l.title||l.page_id)}</span><button class="src-rm" data-need="${r(a.key)}" data-id="${r(l.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${r(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>ls(a.dataset.need,a.dataset.id)))}async function ds(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){u(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const n=await t.json();n.warning?u(n.warning):u("sources refreshed"),st(n),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function ls(e,t){let n;try{n=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(s){u(`remove failed: ${s.message}`);return}if(!n.ok){u(`remove failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}st(await n.json())}async function us(){if(!S)return;const t={content:document.getElementById("editor-text").value},n=S.startsWith("outreach-prompts/")&&S!=="outreach-prompts/fill";(S==="taste-filter"||n)&&(t.enabled=document.getElementById("editor-enabled").checked);let s;try{s=await fetch(`/api/${S}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){u(`save failed: ${o.message}`);return}if(!s.ok){u(`save failed: HTTP ${s.status}`);return}const a=await s.json();a.taste_version&&(document.getElementById("editor-ver").textContent="version "+a.taste_version),u(`${et(S)} saved`),Se(),B()}async function ps(){if(!S||!S.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${S}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){u(`reset failed: ${n.message}`);return}if(!e.ok){u(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",u(`${et(S)} reset to default`)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;i.sort.k===t?i.sort.dir*=-1:(i.sort.k=t,i.sort.dir=1),A()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;i.jsort.k===t?i.jsort.dir*=-1:(i.jsort.k=t,i.jsort.dir=1),$()}}),document.getElementById("tab-companies").onclick=()=>x("companies"),document.getElementById("tab-jobs").onclick=()=>x("jobs"),document.getElementById("tab-followups").onclick=()=>x("follow-ups"),(()=>{const e=document.getElementById("fu-interval");if(!e)return;const t=async()=>{const n=parseInt(e.value,10);if(!Number.isInteger(n)||n<1){e.value=i.followUpInterval;return}if(n!==i.followUpInterval)try{const s=await fetch("/api/settings/follow-up-interval",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({days:n})});if(!s.ok){u("couldn’t save cadence"),e.value=i.followUpInterval;return}const a=await s.json();i.followUpInterval=a.days,e.value=i.followUpInterval,await X(),i.view==="jobs"&&$(),we(),u(`follow up after ${i.followUpInterval} days`)}catch(s){u(`couldn’t save cadence: ${s.message}`),e.value=i.followUpInterval}};e.addEventListener("change",t)})(),document.getElementById("q").oninput=A,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;V.has(t)?V.delete(t):V.add(t),Ne(),A()})}),document.getElementById("flag-filter").addEventListener("click",e=>{G=!G,e.currentTarget.classList.toggle("is-on",G),A()}),document.getElementById("jq").oninput=$,document.getElementById("hide-rejected").addEventListener("click",e=>{Q=!Q,e.currentTarget.classList.toggle("is-on",Q),$()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;O.has(t)?O.delete(t):O.add(t),e.classList.toggle("is-on",O.has(t)),$()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{ge=!ge,e.currentTarget.classList.toggle("is-on",ge),$()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{be=!be,e.currentTarget.classList.toggle("is-on",be),$()}),Re(),ee(),document.getElementById("pane-close").onclick=xe,document.getElementById("scrim").onclick=xe,document.getElementById("pursuit-close").onclick=Ee,document.getElementById("pursuit-scrim").onclick=Ee,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){mt();return}if(xs()){ut();return}if(document.getElementById("profile-scrim").classList.contains("open")){lt();return}if(document.getElementById("add-scrim").classList.contains("open")){Ce();return}if(document.getElementById("run-scrim").classList.contains("open")){je();return}if(document.getElementById("help-scrim").classList.contains("open")){Me();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(Ve==="pursuit"&&n){Ee();return}if(Ve==="company"&&t){xe();return}if(t){xe();return}Ee();return}if(document.getElementById("key-scrim").classList.contains("open")){dt();return}if(document.getElementById("sources-scrim").classList.contains("open")){nt();return}if(document.getElementById("editor-scrim").classList.contains("open")){Se();return}if(document.getElementById("settings-scrim").classList.contains("open")){pt();return}});let at=null;const hs={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Qt(e){if(i.meta&&i.meta.control===!1){u("control surface disabled");return}at=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=hs[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function je(){document.getElementById("run-scrim").classList.remove("open"),at=null}document.getElementById("btn-enrich").onclick=()=>Qt("enrich"),document.getElementById("btn-verdict").onclick=()=>Qt("verdict"),document.getElementById("run-cancel").onclick=je,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&je()},document.getElementById("run-go").onclick=()=>{const e=at,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(je(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),Ze(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=os;const ms={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Zt(e){const t=ms[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const l=document.createElement("a");l.className="help-link",l.textContent="Learn more →",l.onclick=()=>{Me(),nn(),Bs(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(l),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Me(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Zt("add"),document.getElementById("help-run").onclick=()=>Zt("run"),document.getElementById("help-close").onclick=Me,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Me()},document.getElementById("add-cancel").onclick=Ce,document.getElementById("add-save").onclick=Yt,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Ce()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Jt(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Ft),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Yt()))}),document.getElementById("add-vertical-filter").addEventListener("input",zt),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&ss(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=is,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=Se,document.getElementById("editor-save").onclick=us,document.getElementById("editor-reset").onclick=ps,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Se()},document.getElementById("sources-close").onclick=nt,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&nt()},document.getElementById("sources-refresh-btn").onclick=ds,document.getElementById("key-cancel").onclick=dt,document.getElementById("key-save").onclick=tn,document.getElementById("key-remove").onclick=$s,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&dt()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),tn())});function ot(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function it(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}N()}const se='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',me='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',fs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',Xt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',vs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',ys='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',gs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',bs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>',ws='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>';function K(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${r(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function N(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,s=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let I="off",T="";const fe=t&&t.criteria_state;fe==="current"?(I="ok",T="current · verified "+ot(t.verified_age_seconds)):fe==="changed"?(I="warn",T="changed — re-distill"):fe==="unverified"?(I="warn",T=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(I="warn",T="brain offline · using cache"):a&&(I="ok",T="fetched "+ot(t.age_seconds)),o=K({icon:Xt,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:I,note:T,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:me,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=K({icon:Xt,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:se,actTitle:"edit taste.md",actLabel:"edit taste"});const c=i.sources&&i.sources.sources||[],l=c.filter(I=>I.need==="experience").length,p=c.filter(I=>I.need==="voice").length;let d="off",h="not discovered yet — refresh from the brain";l>0?(d="ok",h=`${l} experience · ${p} voice`):c.length>0&&(d="warn",h="no experience yet — refresh");const f=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",b=K({icon:bs,nameHTML:f,dot:d,note:h,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:me,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),w=K({icon:vs,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:se,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),R=K({icon:ys,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:se,actTitle:"edit the outreach email template",actLabel:"edit email template"}),P=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],_=P.map(([I,T,fe])=>K({icon:gs,nameHTML:`<span class="edit-link" data-act="edit-prompt-${I}" title="edit the ${T.replace(/^\d+ · /,"")} prompt">${T}</span>`,desc:fe,act:`edit-prompt-${I}`,actIcon:se,actTitle:`edit the ${T} prompt`,actLabel:`edit ${T} prompt`})).join(""),U=!i.stats||i.stats.taste_filter_enabled!==!1,E=K({icon:ws,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage. Toggle it off in the editor to score every company.",dot:U?"ok":"off",note:U?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:se,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),W=i.anthropicKey;let ft="off",vt="not set — verdict, capture & outreach disabled";W&&W.key_source==="db"?(ft="ok",vt="set here · active"):W&&W.key_source==="env"&&(ft="ok",vt="from the environment");const As=K({icon:fs,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:ft,note:vt,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:se,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${As}
     </div>`;const yt={"view-profile":()=>_s(i.profile),"refresh-profile":Is,"edit-taste":()=>ne("taste"),"edit-taste-filter":()=>ne("taste-filter"),"edit-playbook":()=>ne("playbook"),"edit-template":()=>ne("outreach-template"),"view-sources":tt,"refresh-sources":ks,"edit-anthropic-key":Es};for(const[I]of P)yt[`edit-prompt-${I}`]=()=>ne(`outreach-prompts/${I}`);e.querySelectorAll("[data-act]").forEach(I=>{const T=I.dataset.act;T&&yt[T]&&(I.onclick=yt[T])})}async function rt(){try{i.sources=await(await fetch("/api/outreach/sources")).json()}catch{i.sources=null}N()}async function ks(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),rt();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`refresh failed: ${s||"HTTP "+t.status}`),rt();return}const n=await t.json();n.warning?u(n.warning):u("outreach knowledge refreshed"),i.sources={sources:n.sources||[],needs:i.sources&&i.sources.needs||[]},N()}async function en(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}N()}async function Es(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await en(),ct();const e=document.getElementById("key-input");e&&e.focus()}function ct(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function dt(){document.getElementById("key-scrim").classList.remove("open")}async function tn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){u("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){u(`save failed: ${a.message}`),n();return}if(!s.ok){u((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),u("Anthropic key saved"),await Ge(),ct(),N()}async function $s(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){u(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){u((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),u(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await Ge(),ct(),N()}async function Is(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){u(`refresh failed: ${n.message}`),it();return}if(!t.ok){const n=await t.text().catch(()=>"");u(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),it();return}i.profile=await t.json(),N(),u("company-fit brief refreshed"),B()}function _s(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${ot(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function lt(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=lt,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&lt()};function nn(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Ae(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function ut(){document.getElementById("docs-scrim").classList.remove("open")}function xs(){return document.getElementById("docs-scrim").classList.contains("open")}function Ae(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Bs(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ae(e)}document.getElementById("open-docs").onclick=nn,document.getElementById("docs-close").onclick=ut,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&ut()};function Ls(){document.getElementById("settings-scrim").classList.add("open"),N()}function pt(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Ls,document.getElementById("settings-close").onclick=pt,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&pt()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ae(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&Ae(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Ts(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Cs(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function He(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function ht(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Ss(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function sn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=Ts(n.content);if(n.role==="user")s&&t.appendChild(He("user",s));else if(n.role==="assistant"){const a=Cs(n.content);if(!s&&!a.length)continue;const o=He("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Ss()),ht()}async function Pe(e,t,n){if(!i.meta||!i.meta.chat){u("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const l=await c.json();i.chat.threadId=l.thread.id,sn(l.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function mt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function qe(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function an(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function on(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",an(),qe(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(He("user",t));const a=He("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),ht();let o="";const c=h=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+h,qe(!1)},l=i.chat.threadId;let p;try{p=await fetch("/api/chat/"+l+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(h){c(h.message);return}if(!p.ok){c((await p.text().catch(()=>"")).trim()||"HTTP "+p.status);return}const d=new EventSource("/api/chat/"+l+"/stream");i.chat.es=d,d.addEventListener("delta",h=>{o+=h.data,a.textContent=o,ht()}),d.addEventListener("end",async h=>{d.close(),i.chat.es===d&&(i.chat.es=null),a.classList.remove("chat-streaming"),qe(!1),i.chat.threadId===l&&await js(),Ms(),typeof h.data=="string"&&h.data.indexOf("error")===0&&u("chat: "+h.data)}),d.onerror=()=>{d.close(),i.chat.es===d&&(i.chat.es=null),a.classList.remove("chat-streaming"),qe(!1)}}async function js(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();sn(a.messages||[])}catch{}}function Ms(){L(),k(),B(),i.openId&&le(i.openId)}document.getElementById("open-chat").onclick=()=>Pe("global","",""),document.getElementById("chat-close").onclick=mt,document.getElementById("chat-scrim").onclick=mt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),on()}),document.getElementById("chat-input").addEventListener("input",an),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),on())}),_t("#t tbody",dn),_t("#jt tbody",ln),L(),k(),B(),Ge(),Qe(),it(),rt(),en(),ve()}qs({"":{view:()=>({mount(g){g.innerHTML=Ds,Vs()}}),chrome:!1}},{title:"scout"});Rs();Us();
