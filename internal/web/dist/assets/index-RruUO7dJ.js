...
`;function sn(b){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null},y=e=>"pill pill-"+(e||"none"),_='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function E(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],N()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}r.jobs=e.rows||[],I(),q(),T()}function q(){if(!h.postingId)return;const e=r.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&Z())}let S=null;function T(){const e=r.jobs.some(t=>t.outreach_draft_status==="researching");e&&!S?S=setInterval(Se,4e3):!e&&S&&(clearInterval(S),S=null)}async function Se(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],s=new Map(r.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),n=t.some(a=>s.get(a.posting_id)!==a.outreach_draft_status)||t.length!==r.jobs.length;r.jobs=t,n&&(I(),q()),T()}function z(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",He(),e==="jobs"?I():N()}async function M(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const s=document.getElementById("unscored-n");s.textContent="–",s.title=`stats failed: ${t.message}`;return}r.stats=e,B()}function B(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Y()}function j(e,t,s){const n=e[s]??"",a=t[s]??"";if(s==="headcount")return(n|0)-(a|0);if(s==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[n]??3)-(o[a]??3)}return String(n).localeCompare(String(a))}function A(e){return e.slice().sort((t,s)=>r.sort.dir*j(t,s,r.sort.k))}const L=new Set;let F=!1;function Me(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",L.has(e.dataset.v))})}function le(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(L.size&&!L.has(t.verdict||"__none__")||F&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Ae=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Nt=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function at(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const ot=at("scout-hidden-cols"),it=at("scout-hidden-jcols");function rt(){return r.view==="jobs"?{cols:Nt,hidden:it,key:"scout-hidden-jcols"}:{cols:Ae,hidden:ot,key:"scout-hidden-cols"}}function ue(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=ot.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=it.has(e.dataset.col)?"none":""})}function He(){const e=rt();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const s=rt(),n=t.dataset.col;s.hidden.has(n)?s.hidden.delete(n):s.hidden.add(n),localStorage.setItem(s.key,JSON.stringify([...s.hidden])),He(),ue()})})}function N(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=A(le());document.getElementById("empty").style.display=t.length?"none":"block";for(const s of t){const n=document.createElement("tr");n.dataset.id=s.company_id,n.innerHTML=`
  ...
  `,Kt();const n=document.getElementById("pursuit-company-link");n&&n.addEventListener("click",()=>se(e.company_id)),P(document.getElementById("pursuit-title-input"),a=>ut(e,"title",a)),P(document.getElementById("pursuit-url-input"),a=>Jt(e,a)),P(document.getElementById("pursuit-notes-input"),a=>Yt(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>P(a,o=>ut(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),ve(),te()}function Wt(e){return`
    ...
    </div>`,p=n&&(Gt(n.status)||n.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${n?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    ...
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${n?vt(n,!1):""}</div><div class="draft-actions">${p}</div>`+u,es()}function Gt(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const ft='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',Qt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12.5 13.5h-9a1 1 0 01-1-1v-9a1 1 0 011-1h7l3 3v7a1 1 0 01-1 1z"/><path d="M10.5 13.5V9.5h-5v4M5.5 2.5v3h4"/></svg>',Zt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>';function vt(e,t){const s=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),n=(k,C)=>`
    <div class="draft-head">
      <span class="${k}">${C}</span>
      <span class="dh-time">${i(s)}</span>
    ...
    </div>`;if(e.status==="failed"){const k=Xt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    <div class="draft-actions">
      <button class="btn draft-copy-btn" title="copy the email to the clipboard">${ft}Copy</button>
      <button class="btn draft-save-btn" title="save your edits">${Qt}Save</button>
  ...
  </div>`}function cs(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function ds(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",wt);const s=e.querySelector("#answers-redetect-btn");s&&s.addEventListener("click",us),e.querySelectorAll(".answer-card[data-aid]").forEach(n=>{const a=n.dataset.aid,o=n.querySelector(".answer-textarea");o&&(P(o,p=>ps(a,p),{multiline:!0}),o.addEventListener("input",()=>ls(n,o)));const c=n.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>hs(a))})}function ls(e,t){const s=e.querySelector(".answer-count");if(!s)return;const n=t.value.length,a=s.textContent.includes("/")?parseInt(s.textContent.split("/")[1],10):0;s.textContent=a?`${n} / ${a}`:`${n} chars`,s.classList.toggle("over",!!a&&n>a)}async function wt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let s;try{s=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){l(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(s.status===202){await ee();return}if(s.status===412){let a={};try{a=await s.json()}catch{}ms(a.error),t&&(t.disabled=!1);return}if(s.status===503){kt("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const n=(await s.text().catch(()=>"")).trim();l(`draft failed: ${n||"HTTP "+s.status}`),t&&(t.disabled=!1)}async function us(){h.detecting=!0,te();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();l(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){l(`detect failed: ${e.message}`)}h.detecting=!1,await ee()}async function ps(e,t){const s=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=n)}async function hs(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(s){l(`regenerate failed: ${s.message}`);return}if(t.status===503){kt("Answer generation isn't running in this build.");return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`regenerate failed: ${s||"HTTP "+t.status}`);return}await ee()}function ms(e){const t=document.getElementById("answers-section");if(!t)return;const s=t.querySelector(".answers-actions"),n=document.createElement("div");n.className="blocks-gate",n.innerHTML=`
  ...
  `;const k=document.getElementById("posting-add-btn");k&&k.addEventListener("click",()=>ys(e)),Ve(),document.querySelectorAll("#ve-pick .ve-opt").forEach(g=>{g.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(J=>J.classList.remove("is-on")),g.classList.add("is-on")})});const C=document.getElementById("ve-save-btn");C&&C.addEventListener("click",()=>gs(e)),P(document.getElementById("pane-title-input"),g=>pt(e,"name",g)),document.querySelectorAll("#facts-body [data-k]").forEach(g=>P(g,J=>pt(e,g.dataset.k,J))),P(document.getElementById("pane-domain-input"),g=>zt(e,g)),P(document.getElementById("pane-notes-input"),g=>Ft(e,g),{multiline:!0});const $=document.getElementById("flag-toggle-btn");$&&$.addEventListener("click",()=>Et(e.company_id));const D=document.getElementById("review-stamp-btn");D&&D.addEventListener("click",()=>vs(e.company_id));const de=document.getElementById("rescore-btn");de&&de.addEventListener("click",()=>Ue("verdict",{company_ids:[e.company_id]}));const x=document.getElementById("reenrich-btn");x&&x.addEventListener("click",()=>Ue("enrich",{company_ids:[e.company_id]}))}function fs(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function vs(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let s;try{s=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){l(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!s.ok){const o=await s.text().catch(()=>"");l(`failed: HTTP ${s.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const n=await s.json(),a=r.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=n.reviewed_at,N()),r.openId===e&&(ne(n),oe(e)),l("reviewed")}async function Et(e){const t=r.rows.find(o=>o.company_id===e),s=!(t&&t.flagged);let n;try{n=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:s})})}catch(o){l(`failed: ${o.message}`);return}if(!n.ok){const o=await n.text().catch(()=>"");l(`failed: HTTP ${n.status}${o?" — "+o:""}`);return}const a=await n.json();t&&(t.flagged=a.flagged,N()),r.openId===e&&(ne(a),oe(e)),w(),l(a.flagged?"flagged":"unflagged")}async function gs(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){l("Pick yes, maybe, or no.");return}const s=t.dataset.v,n=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:s,reason:n})})}catch(p){l(`save failed: ${p.message}`),a.disabled=!1;return}if(!o.ok){const p=await o.text().catch(()=>"");l(`save failed: HTTP ${o.status}${p?" — "+p:""}`),a.disabled=!1;return}const c=await o.json();ne(c),oe(c.company_id),E(),M(),w(),l("verdict saved")}function Re(e){const t=e.postings||[];return t.length?t.map(s=>{const n=[s.location,s.source==="capture"?"captured":"added",(s.created_at||"").slice(0,10)].filter(Boolean).map(i).join(" · "),a=O[s.response]||O[""],o=[s.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a.cls}">${i(a.label)}</span>`,`<span class="pt-meta">${s.applied_at?`applied ${i(s.applied_at)}`:"not applied"}</span>`,`<span class="pt-meta">${s.outreach_count?`${s.outreach_count} sent · last ${i(s.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join(""),c=r.meta&&r.meta.chat?`<button class="pcard-chat" data-pid="${i(s.id)}" data-ptitle="${i(s.title||"")}" title="chat about this role">chat</button>`:"";return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ae(e,t){const s=r.openDetail;if(!s||!r.openId)return;const n=(s.postings||[]).find(o=>String(o.id)===String(e));if(!n)return;Object.assign(n,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Re(s),Ve())}function Ve(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||dt(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),Te("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function ys(e){const t=document.getElementById("posting-url"),s=document.getElementById("posting-title"),n=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}n.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:s.value.trim()})})}catch(u){l(`add failed: ${u.message}`),n.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),n.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const p=document.getElementById("postings-list");p&&(p.innerHTML=Re(e),Ve()),t.value="",s.value="",n.disabled=!1,w(),l("link added")}async function oe(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(n){be(`<div class="muted">Failed to load trail: ${i(n.message)}</div>`);return}if(!t.ok){be(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const s=(await t.json()).events||[];if(s.length===0){be('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}be(s.map(bs).join(""))}function bs(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
  ...
  </div>`}function Y(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,n=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(n){let x="off",g="";const J=t&&t.criteria_state;J==="current"?(x="ok",g="current · verified "+Ye(t.verified_age_seconds)):J==="changed"?(x="warn",g="changed — re-distill"):J==="unverified"?(x="warn",g=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(x="warn",g="brain offline · using cache"):a&&(x="ok",g="fetched "+Ye(t.age_seconds)),o=K({icon:At,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:x,note:g,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:ce,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=K({icon:At,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:_e,actTitle:"edit taste.md",actLabel:"edit taste"});const c=r.sources&&r.sources.sources||[],p=c.filter(x=>x.need==="experience").length,u=c.filter(x=>x.need==="voice").length;let d="off",m="not discovered yet — refresh from the brain";p>0?(d="ok",m=`${p} experience · ${u} voice`):c.length>0&&(d="warn",m="no experience yet — refresh");const v=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",k=K({icon:Hs,nameHTML:v,dot:d,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:ce,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),C=K({icon:Ss,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:_e,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),$=K({icon:Ms,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:_e,actTitle:"edit the outreach email template",actLabel:"edit email template"}),D=K({icon:As,nameHTML:'<span class="edit-link" data-act="edit-doctrine" title="edit the outreach doctrine">outreach doctrine</span>',desc:"How cold emails get written — the depth bar, show-don't-tell, the kill list.",act:"edit-doctrine",actIcon:_e,actTitle:"edit the outreach doctrine",actLabel:"edit outreach doctrine"});e.innerHTML=`<div class="settings-section">
