(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))x(i);new MutationObserver(i=>{for(const f of i)if(f.type==="childList")for(const $ of f.addedNodes)$.tagName==="LINK"&&$.rel==="modulepreload"&&x($)}).observe(document,{childList:!0,subtree:!0});function v(i){const f={};return i.integrity&&(f.integrity=i.integrity),i.referrerPolicy&&(f.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?f.credentials="include":i.crossOrigin==="anonymous"?f.credentials="omit":f.credentials="same-origin",f}function x(i){if(i.ep)return;i.ep=!0;const f=v(i);fetch(i.href,f)}})();function Pn(y,r){const v=r.replace(/^#/,"");let x=null;for(const i of Object.keys(y))(v===i||v.startsWith(i))&&(x===null||i.length>x.length)&&(x=i);return x===null&&""in y&&(x=""),x}function qn(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function Nn(y,r={}){const v=r.root??document.body,x=r.title??document.title??"",i=r.brandHref??"#",f=document.createElement("main"),$=document.createElement("header");$.className="cap-head";const w=document.createElement("a");w.className="brand",w.href=i,w.textContent=x,w.setAttribute("aria-label",`${x} — home`),$.appendChild(w);const M=document.createElement("nav");M.className="cap-nav",M.setAttribute("aria-label","Views");for(const k of r.nav??[]){const C=document.createElement("a");C.href=k.href,C.textContent=k.label,k.ariaLabel&&C.setAttribute("aria-label",k.ariaLabel),M.appendChild(C)}$.appendChild(M);const _=document.createElement("section");_.className="tk-content",f.appendChild($),f.appendChild(_);const L=document.createElement("div");L.className="tk-bleed";const Be=k=>{var C;for(const H of Array.from(M.querySelectorAll("a"))){const V=((C=H.getAttribute("href"))==null?void 0:C.replace(/^#/,""))??"";H.toggleAttribute("aria-current",k!==null&&k!==""&&V===k),H.hasAttribute("aria-current")&&H.setAttribute("aria-current","page")}};let ne=0;const A=()=>{const k=Pn(y,location.hash);if(Be(k),k===null){L.isConnected&&L.remove(),f.isConnected||v.appendChild(f),On(_,"Not found.");return}const{view:C,chrome:H}=qn(y[k]),V=H?_:L;H?(L.isConnected&&L.remove(),f.isConnected||v.appendChild(f)):(f.isConnected&&f.remove(),L.isConnected||v.appendChild(L)),V.replaceChildren();const _e=C(),se=++ne,F=_e.mount(V);F instanceof Promise&&F.catch(ae=>{se===ne&&Rn(V,String(ae))})};window.addEventListener("hashchange",A),A()}function On(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-empty",v.textContent=r,y.appendChild(v)}function Rn(y,r){y.replaceChildren();const v=document.createElement("div");v.className="tk-error",v.textContent=r,y.appendChild(v)}function Vn(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const v of r)v.unregister()}),window.caches&&caches.keys().then(r=>{for(const v of r)caches.delete(v)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Un(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let r;try{r=await y.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Dn=`
<div class="layout">
<aside class="sidebar">
...
`;function Jn(y){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},v=e=>"pill pill-"+(e||"none"),x='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',i=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?i(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],q()}async function w(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],B(),h.postingId){const t=r.jobs.find(n=>n.posting_id===h.postingId);t&&(h.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&K())}}function M(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Ce(),e==="jobs"?B():q()}async function _(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Ve()}function Be(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function ne(e){return e.slice().sort((t,n)=>r.sort.dir*Be(t,n,r.sort.k))}const A=new Set;let k=!1;function C(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",A.has(e.dataset.v))})}function H(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(A.size&&!A.has(t.verdict||"__none__")||k&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const V=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],_e=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function se(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const F=se("scout-hidden-cols"),ae=se("scout-hidden-jcols");function We(){return r.view==="jobs"?{cols:_e,hidden:ae,key:"scout-hidden-jcols"}:{cols:V,hidden:F,key:"scout-hidden-cols"}}function oe(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=F.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=ae.has(e.dataset.col)?"none":""})}function Ce(){const e=We();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=We(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),Ce(),oe()})})}function q(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=ne(H());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
  ...
  `,Vt();const s=document.getElementById("pursuit-company-link");s&&s.addEventListener("click",()=>Q(e.company_id)),T(document.getElementById("pursuit-title-input"),a=>Qe(e,"title",a)),T(document.getElementById("pursuit-url-input"),a=>qt(e,a)),T(document.getElementById("pursuit-notes-input"),a=>Ut(a),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(a=>T(a,o=>Qe(e,a.dataset.k,o),{multiline:a.tagName==="TEXTAREA"})),Y(),J()}function Rt(e){return`
    ...
    ${le(e)}
  </div>`}function nt(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function le(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,d)=>d?`<div class="tr-line"><span class="tr-key">${u}:</span> ${i(String(d))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],p=c.map(u=>`
      <div class="tr-line">
  ...
  `;const j=document.getElementById("posting-add-btn");j&&j.addEventListener("click",()=>pn(e)),dt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(ze=>ze.classList.remove("is-on")),b.classList.add("is-on")})});const S=document.getElementById("ve-save-btn");S&&S.addEventListener("click",()=>un(e)),T(document.getElementById("pane-title-input"),b=>Xe(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>T(b,ze=>Xe(e,b.dataset.k,ze))),T(document.getElementById("pane-domain-input"),b=>Nt(e,b)),T(document.getElementById("pane-notes-input"),b=>Ot(e,b),{multiline:!0});const E=document.getElementById("flag-toggle-btn");E&&E.addEventListener("click",()=>rt(e.company_id));const te=document.getElementById("review-stamp-btn");te&&te.addEventListener("click",()=>ln(e.company_id));const Tt=document.getElementById("rescore-btn");Tt&&Tt.addEventListener("click",()=>Me("verdict",{company_ids:[e.company_id]}));const jt=document.getElementById("reenrich-btn");jt&&jt.addEventListener("click",()=>Me("enrich",{company_ids:[e.company_id]}))}function dn(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function dt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||Ye(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),Ie("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function pn(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const p=document.getElementById("postings-list");p&&(p.innerHTML=ct(e),dt()),t.value="",n.value="",s.disabled=!1,w(),l("link added")}async function Z(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){pe(`<div class="muted">Failed to load trail: ${i(s.message)}</div>`);return}if(!t.ok){pe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){pe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}pe(n.map(hn).join(""))}function hn(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(i);return e.run_id&&t.push("run "+i(e.run_id.slice(0,8))),`
    <div class="trail-event">
    ...
    </div>`}function pe(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let lt;function l(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(lt),lt=setTimeout(()=>t.classList.remove("show"),2200)}r.meta={control:!1,brain:!1,verdict:!1};async function mn(){try{const s=await fetch("/api/meta");if(!s.ok)return;r.meta=await s.json()}catch{return}const e=r.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!r.meta.verdict,t.title=r.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=r.meta.chat?"":"none")}async function Se(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let he=null;async function Me(e,t){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){l(`run failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(n.status===412){const a=await n.text();l(a.trim());return}if(!n.ok){l(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();vt(e,s)}async function fn(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){l(`upload failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(!n.ok){l(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();vt("ingest",s)}const gn=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let me=[],O=new Set,R="company";function ut(e){R=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",pt()}function Ae(){return!!r.meta.capture&&document.getElementById("add-enrich").checked}function pt(){const e=document.getElementById("add-note");Ae()?e.innerHTML=R==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=R==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function vn(){gn.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",O=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!r.meta.capture,t.classList.toggle("disabled",!r.meta.capture),t.title=r.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",r.meta.capture||(e.checked=!1),ut(r.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(r.rows||[]).map(s=>`<option value="${i(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),me=s.verticals||[]}catch{me=[]}ht()}function fe(){document.getElementById("add-scrim").classList.remove("open")}function ht(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=me.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${O.has(s)?" sel":""}" data-v="${i(s)}">${i(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;O.has(a)?O.delete(a):O.add(a),s.classList.toggle("sel"),mt()}))):e.innerHTML=`<div class="none">${me.length?"no match":"no verticals in the set yet"}</div>`,mt()}function mt(){const e=O.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function ft(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function gt(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){l(R==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,Ae()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=g=>document.getElementById(g).value.trim(),c=Ae();let p,u;c?(p="/api/capture",u={url:ft(t),kind:R==="company"?"company_page":"job_posting",fields:R==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...O].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):R==="company"?(p="/api/companies",u={website:t,name:o("add-name"),vertical:[...O].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(p="/api/postings",u={url:ft(t),title:o("add-title"),company:o("add-job-company")});let d;try{d=await fetch(p,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(g){l(`add failed: ${g.message}`),a();return}if(!d.ok){let g=`HTTP ${d.status}`;try{const j=await d.text();try{g=JSON.parse(j).error||g}catch{g=j.trim()||g}}catch{}if(a(),d.status===409){l(g||"That company is already in the list."),e.focus(),e.select();return}l(`add failed: ${g}`);return}const m=await d.json();if(a(),c&&!m.company_id){l(m.note||"couldn't classify that page");return}if(fe(),$(),_(),w(),R==="job"){const g=m.posting&&m.posting.title||"job link";l(`tracking: ${g} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),M("jobs")}else c?(l(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`),Q(m.company_id)):l("company added")}function vt(e,t){he=t;const n=document.getElementById("drawer"),s=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",s.innerHTML="",n.classList.add("open"),Se();const a=new EventSource(`/api/jobs/${t}/stream`),o=(c,p)=>{const u=document.createElement("div"),d=!p&&/^\s*warn:/i.test(c);u.className="ln"+(p?" ln-err":d?" ln-warn":""),u.textContent=d?c.replace(/^\s*warn:\s*/i,"⚠ "):c,s.appendChild(u),s.scrollTop=s.scrollHeight};a.addEventListener("line",c=>o(c.data,/error|failed/i.test(c.data))),a.addEventListener("end",c=>{a.close(),he=null,o(`— ${c.data} —`,c.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",l(`${e} ${c.data}`),$(),_(),Se(),w(),r.openId&&Q(r.openId)}),a.onerror=()=>{a.close()}}async function yn(){if(he)try{await fetch(`/api/jobs/${he}/cancel`,{method:"POST"})}catch{}}let ee=null;async function yt(e){ee=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+e+".md",document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="",t.classList.add("open");try{const s=await(await fetch(`/api/${e}`)).json();document.getElementById("editor-text").value=s.content||"",s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version)}catch(n){document.getElementById("editor-text").value="failed to load: "+n.message}}function ge(){document.getElementById("editor-scrim").classList.remove("open"),ee=null}const He=["subject_name","signature","lens","hook_prefs","arc"];let bt=!1;async function bn(){document.getElementById("sender-scrim").classList.add("open");let t={};try{t=await(await fetch("/api/outreach/sender")).json()}catch(n){l(`failed to load identity: ${n.message}`)}for(const n of He){const s=document.getElementById("snd-"+n);s&&(s.value=t[n]||"")}if(!bt){for(const n of He){const s=document.getElementById("snd-"+n);T(s,a=>wn(n,a),{multiline:s&&s.tagName==="TEXTAREA"})}bt=!0}}async function wn(e,t){const n={};for(const a of He){const o=document.getElementById("snd-"+a);n[a]=o?o.value:""}n[e]=t;const s=await fetch("/api/outreach/sender",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status)}function Pe(){document.getElementById("sender-scrim").classList.remove("open")}let I=null,wt=!1;async function kn(){document.getElementById("config-scrim").classList.add("open");try{I=await(await fetch("/api/outreach/config")).json()}catch(e){l(`failed to load config: ${e.message}`);return}if(kt(),ye(),!wt){const e=document.getElementById("cfg-word_min"),t=document.getElementById("cfg-word_max"),n=document.getElementById("cfg-subject_format");e.addEventListener("change",()=>{I.word_min=parseInt(e.value,10),ve()}),t.addEventListener("change",()=>{I.word_max=parseInt(t.value,10),ve()}),n.addEventListener("change",()=>{I.subject_format=n.value,ve()}),n.addEventListener("keydown",s=>{s.key==="Enter"&&(s.preventDefault(),n.blur())}),document.getElementById("cfg-add-btn").addEventListener("click",En),wt=!0}}function qe(){document.getElementById("config-scrim").classList.remove("open")}function kt(){I&&(document.getElementById("cfg-word_min").value=I.word_min??"",document.getElementById("cfg-word_max").value=I.word_max??"",document.getElementById("cfg-subject_format").value=I.subject_format||"")}async function ve(){let e;try{e=await fetch("/api/outreach/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(I)})}catch(t){return l(`config save failed: ${t.message}`),!1}return e.ok?(I=await e.json(),kt(),ye(),!0):(l(`config save failed: ${(await e.text().catch(()=>"")).trim()||"HTTP "+e.status}`),!1)}function ye(){const e=document.getElementById("cfg-structure");if(!e)return;const t=I&&I.structure||[];if(!t.length){e.innerHTML='<div class="dim small">no slots — add at least one</div>';return}e.innerHTML=t.map((n,s)=>{const a=n.kind==="locked"?`${i(n.block)} <span class="cfg-kind">locked</span>`:`${i(n.source)} <span class="cfg-kind">model</span>`;return`<div class="cfg-slot" data-i="${s}">
      ...
      </span></div>`}).join(""),e.querySelectorAll(".cfg-slot").forEach(n=>{const s=+n.dataset.i,a=n.querySelector(".cfg-up"),o=n.querySelector(".cfg-down"),c=n.querySelector(".cfg-rm");a&&(a.onclick=()=>Et(s,-1)),o&&(o.onclick=()=>Et(s,1)),c&&(c.onclick=()=>Ne(p=>{p.splice(s,1)}))})}function Et(e,t){Ne(n=>{const s=e+t;s<0||s>=n.length||([n[e],n[s]]=[n[s],n[e]])})}function En(){const e=document.getElementById("cfg-add-select"),[t,n]=(e.value||"").split(":");Ne(s=>{s.push(t==="locked"?{kind:"locked",block:n}:{kind:"model",source:n})})}async function Ne(e){if(!I)return;const t=I.structure||[],n=JSON.parse(JSON.stringify(t));e(n),I.structure=n,ye(),await ve()||(I.structure=t,ye())}async function $n(){if(!ee)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${ee}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(s){l(`save failed: ${s.message}`);return}if(!t.ok){l(`save failed: HTTP ${t.status}`);return}const n=await t.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version),l(`${ee}.md saved`),ge(),_()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),q()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),B()}}),document.getElementById("tab-companies").onclick=()=>M("companies"),document.getElementById("tab-jobs").onclick=()=>M("jobs"),document.getElementById("q").oninput=q,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;A.has(t)?A.delete(t):A.add(t),C(),q()})}),document.getElementById("flag-filter").addEventListener("click",e=>{k=!k,e.currentTarget.classList.toggle("is-on",k),q()}),document.getElementById("jq").oninput=B,document.getElementById("hide-rejected").addEventListener("click",e=>{U=!U,e.currentTarget.classList.toggle("is-on",U),B()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;P.has(t)?P.delete(t):P.add(t),e.classList.toggle("is-on",P.has(t)),B()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{ie=!ie,e.currentTarget.classList.toggle("is-on",ie),B()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{re=!re,e.currentTarget.classList.toggle("is-on",re),B()}),Ce(),oe(),document.getElementById("pane-close").onclick=ue,document.getElementById("scrim").onclick=ue,document.getElementById("pursuit-close").onclick=ce,document.getElementById("pursuit-scrim").onclick=ce,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){Fe();return}if(Ln()){De();return}if(document.getElementById("profile-scrim").classList.contains("open")){Ue();return}if(document.getElementById("add-scrim").classList.contains("open")){fe();return}if(document.getElementById("run-scrim").classList.contains("open")){be();return}if(document.getElementById("help-scrim").classList.contains("open")){we();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(Le==="pursuit"&&n){ce();return}if(Le==="company"&&t){ue();return}if(t){ue();return}ce();return}if(document.getElementById("config-scrim").classList.contains("open")){qe();return}if(document.getElementById("sender-scrim").classList.contains("open")){Pe();return}document.getElementById("editor-scrim").classList.contains("open")&&ge()});let Oe=null;const In={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function $t(e){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}Oe=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=In[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=r.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function be(){document.getElementById("run-scrim").classList.remove("open"),Oe=null}document.getElementById("btn-enrich").onclick=()=>$t("enrich"),document.getElementById("btn-verdict").onclick=()=>$t("verdict"),document.getElementById("run-cancel").onclick=be,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&be()},document.getElementById("run-go").onclick=()=>{const e=Oe,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(be(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),Me(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=vn;const xn={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function It(e){const t=xn[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const p=document.createElement("a");p.className="help-link",p.textContent="Learn more →",p.onclick=()=>{we(),Bt(),Tn(s.sec)},a.appendChild(o),a.appendChild(c),a.appendChild(p),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function we(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>It("add"),document.getElementById("help-run").onclick=()=>It("run"),document.getElementById("help-close").onclick=we,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&we()},document.getElementById("add-cancel").onclick=fe,document.getElementById("add-save").onclick=gt,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&fe()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>ut(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",pt),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),gt()))}),document.getElementById("add-vertical-filter").addEventListener("input",ht),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&fn(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=yn,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=ge,document.getElementById("editor-save").onclick=$n,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&ge()},document.getElementById("sender-close").onclick=Pe,document.getElementById("sender-scrim").onclick=e=>{e.target.id==="sender-scrim"&&Pe()},document.getElementById("config-close").onclick=qe,document.getElementById("config-scrim").onclick=e=>{e.target.id==="config-scrim"&&qe()};function xt(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function Re(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}Ve()}const ke='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',Bn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>';function Ve(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,n=r.stats&&r.stats.stale_verdicts||0,a=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),o=t&&typeof t.body=="string";let c="";if(a){let S="off",E="";t&&!t.reachable&&o?(S="warn",E="brain offline · using cache"):t&&t.stale?(S="warn",E="cached · stale"):o&&(S="ok",E="fetched "+xt(t.age_seconds)),c+=`<div class="crit-row">
      ...
      <button class="crit-edit" id="refresh-profile" title="re-distill the company-fit brief from the brain" aria-label="refresh company-fit brief">${Bn}</button></div>`,E&&(c+=`<div class="crit-note dim small">${i(E)}</div>`)}else c+=`<div class="crit-row">
      <span class="crit-what">taste</span>
      <button class="crit-edit" id="edit-taste" title="edit taste.md" aria-label="edit taste">${ke}</button></div>`,t&&t.configured&&(c+='<div class="crit-note dim small">brain offline — local fallback</div>');c+=`<div class="crit-row">
    <span class="crit-what">playbook</span>
    <button class="crit-edit" id="edit-playbook" title="edit playbook.md" aria-label="edit playbook">${ke}</button></div>`,c+=`<div class="crit-row">
    <span class="crit-what">outreach identity</span>
    <button class="crit-edit" id="edit-sender" title="edit outreach identity" aria-label="edit outreach identity">${ke}</button></div>`,c+=`<div class="crit-row">
    <span class="crit-what">outreach config</span>
    <button class="crit-edit" id="edit-config" title="edit outreach config (lint knobs + email structure)" aria-label="edit outreach config">${ke}</button></div>`,n>0&&(c+=`<div class="stale">
      ...
      ${n} verdicts stale</div>`),e.innerHTML=c;const p=document.getElementById("view-profile");p&&(p.onclick=()=>Cn(r.profile));const u=document.getElementById("refresh-profile");u&&(u.onclick=_n);const d=document.getElementById("edit-taste");d&&(d.onclick=()=>yt("taste"));const m=document.getElementById("edit-playbook");m&&(m.onclick=()=>yt("playbook"));const g=document.getElementById("edit-sender");g&&(g.onclick=bn);const j=document.getElementById("edit-config");j&&(j.onclick=kn)}async function _n(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),Re();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Re();return}r.profile=await t.json(),Ve(),l("company-fit brief refreshed"),_()}function Cn(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${xt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Ue(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ue,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ue()};function Bt(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Ee(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function De(){document.getElementById("docs-scrim").classList.remove("open")}function Ln(){return document.getElementById("docs-scrim").classList.contains("open")}function Ee(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Tn(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ee(e)}document.getElementById("open-docs").onclick=Bt,document.getElementById("docs-close").onclick=De,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&De()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ee(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&Ee(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),r.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function jn(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Sn(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function $e(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function Je(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Mn(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=r.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(r.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function _t(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=jn(n.content);if(n.role==="user")s&&t.appendChild($e("user",s));else if(n.role==="assistant"){const a=Sn(n.content);if(!s&&!a.length)continue;const o=$e("assistant",s);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Mn()),Je()}async function Ie(e,t,n){if(!r.meta||!r.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}r.chat.es&&(r.chat.es.close(),r.chat.es=null),r.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const p=await c.json();r.chat.threadId=p.thread.id,_t(p.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+i(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Fe(){r.chat.es&&(r.chat.es.close(),r.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function xe(e){r.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Ct(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Lt(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||r.chat.streaming||!r.chat.threadId)return;e.value="",Ct(),xe(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild($e("user",t));const a=$e("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),Je();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,xe(!1)},p=r.chat.threadId;let u;try{u=await fetch("/api/chat/"+p+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const d=new EventSource("/api/chat/"+p+"/stream");r.chat.es=d,d.addEventListener("delta",m=>{o+=m.data,a.textContent=o,Je()}),d.addEventListener("end",async m=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),xe(!1),r.chat.threadId===p&&await An(),Hn(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),d.onerror=()=>{d.close(),r.chat.es===d&&(r.chat.es=null),a.classList.remove("chat-streaming"),xe(!1)}}async function An(){const e=r.chat.scope,t=r.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();_t(a.messages||[])}catch{}}function Hn(){$(),w(),_(),r.openId&&Q(r.openId)}document.getElementById("open-chat").onclick=()=>Ie("global","",""),document.getElementById("chat-close").onclick=Fe,document.getElementById("chat-scrim").onclick=Fe,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Lt()}),document.getElementById("chat-input").addEventListener("input",Ct),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Lt())}),$(),w(),_(),mn(),Se(),Re()}Nn({"":{view:()=>({mount(y){y.innerHTML=Dn,Jn()}}),chrome:!1}},{title:"scout"});Vn();Un();
