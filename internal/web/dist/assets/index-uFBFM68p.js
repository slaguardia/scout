(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))T(r);new MutationObserver(r=>{for(const g of r)if(g.type==="childList")for(const _ of g.addedNodes)_.tagName==="LINK"&&_.rel==="modulepreload"&&T(_)}).observe(document,{childList:!0,subtree:!0});function v(r){const g={};return r.integrity&&(g.integrity=r.integrity),r.referrerPolicy&&(g.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?g.credentials="include":r.crossOrigin==="anonymous"?g.credentials="omit":g.credentials="same-origin",g}function T(r){if(r.ep)return;r.ep=!0;const g=v(r);fetch(r.href,g)}})();function Wn(b,i){const v=i.replace(/^#/,"");let T=null;for(const r of Object.keys(b))(v===r||v.startsWith(r))&&(T===null||r.length>T.length)&&(T=r);return T===null&&""in b&&(T=""),T}function Yn(b){return typeof b=="function"?{view:b,chrome:!0}:{view:b.view,chrome:b.chrome!==!1}}function Gn(b,i={}){const v=i.root??document.body,T=i.title??document.title??"",r=i.brandHref??"#",g=document.createElement("main"),_=document.createElement("header");_.className="cap-head";const E=document.createElement("a");E.className="brand",E.href=r,E.textContent=T,E.setAttribute("aria-label",`${T} — home`),_.appendChild(E);const J=document.createElement("nav");J.className="cap-nav",J.setAttribute("aria-label","Views");for(const w of i.nav??[]){const M=document.createElement("a");M.href=w.href,M.textContent=w.label,w.ariaLabel&&M.setAttribute("aria-label",w.ariaLabel),J.appendChild(M)}_.appendChild(J);const H=document.createElement("section");H.className="tk-content",g.appendChild(_),g.appendChild(H);const j=document.createElement("div");j.className="tk-bleed";const De=w=>{var M;for(const N of Array.from(J.querySelectorAll("a"))){const z=((M=N.getAttribute("href"))==null?void 0:M.replace(/^#/,""))??"";N.toggleAttribute("aria-current",w!==null&&w!==""&&z===w),N.hasAttribute("aria-current")&&N.setAttribute("aria-current","page")}};let ie=0;const ee=()=>{const w=Wn(b,location.hash);if(De(w),w===null){j.isConnected&&j.remove(),g.isConnected||v.appendChild(g),Qn(H,"Not found.");return}const{view:M,chrome:N}=Yn(b[w]),z=N?H:j;N?(j.isConnected&&j.remove(),g.isConnected||v.appendChild(g)):(g.isConnected&&g.remove(),j.isConnected||v.appendChild(j)),z.replaceChildren();const F=M(),te=++ie,ye=F.mount(z);ye instanceof Promise&&ye.catch(be=>{te===ie&&Zn(z,String(be))})};window.addEventListener("hashchange",ee),ee()}function Qn(b,i){b.replaceChildren();const v=document.createElement("div");v.className="tk-empty",v.textContent=i,b.appendChild(v)}function Zn(b,i){b.replaceChildren();const v=document.createElement("div");v.className="tk-error",v.textContent=i,b.appendChild(v)}function Xn(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(i=>{for(const v of i)v.unregister()}),window.caches&&caches.keys().then(i=>{for(const v of i)caches.delete(v)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function ea(){let b;try{b=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!b.ok)return null;let i;try{i=await b.json()}catch{return null}return typeof i.email=="string"&&i.email?{email:i.email}:null}const ta=`
<div class="layout">
<aside class="sidebar">
...
`;function sa(b){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],openDetail:null,anthropicKey:null},v=e=>"pill pill-"+(e||"none"),T='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),g=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function _(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],P()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],I(),J(),j()}function J(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&ae())}let H=null;function j(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!H?H=setInterval(De,4e3):!e&&H&&(clearInterval(H),H=null)}async function De(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],s=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),n=t.some(a=>s.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,n&&(I(),J()),j()}async function ie(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{})]),bs(),i.view==="jobs"&&I()}function ee(e){i.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Ve(),e==="jobs"?I():P()}async function w(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const s=document.getElementById("unscored-n");s.textContent="–",s.title=`stats failed: ${t.message}`;return}i.stats=e,M()}function M(){const e=i.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,D()}function N(e,t,s){const n=e[s]??"",a=t[s]??"";if(s==="headcount")return(n|0)-(a|0);if(s==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[n]??3)-(o[a]??3)}return String(n).localeCompare(String(a))}function z(e){return e.slice().sort((t,s)=>i.sort.dir*N(t,s,i.sort.k))}const F=new Set;let te=!1;function ye(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",F.has(e.dataset.v))})}function be(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(F.size&&!F.has(t.verdict||"__none__")||te&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const hs=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],ms=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function kt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Et=kt("scout-hidden-cols"),$t=kt("scout-hidden-jcols");function It(){return i.view==="jobs"?{cols:ms,hidden:$t,key:"scout-hidden-jcols"}:{cols:hs,hidden:Et,key:"scout-hidden-cols"}}function se(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Et.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=$t.has(e.dataset.col)?"none":""})}function Ve(){const e=It();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const s=It(),n=t.dataset.col;s.hidden.has(n)?s.hidden.delete(n):s.hidden.add(n),localStorage.setItem(s.key,JSON.stringify([...s.hidden])),Ve(),se()})})}function xt(e){return`
    ...
    `,y.querySelector(".jt-nextup").onclick=()=>Dt(l,!1),y.querySelector(".jt-stage-sel").onchange=x=>Ls(l,x.target.value),y.querySelector(".jt-ostatus").onchange=x=>xe(l,{outreach_status:x.target.value}),y.querySelector(".jt-inc").onclick=()=>xe(l,{outreach_count:(l.outreach_count||0)+1,last_outreach_at:Ee()}),y.querySelector(".jt-dec").onclick=()=>{const x=Math.max(0,(l.outreach_count||0)-1);xe(l,{outreach_count:x,...x===0?{last_outreach_at:""}:{}})},e.appendChild(y)}se(),e.querySelectorAll("tr").forEach(l=>{l.addEventListener("click",m=>{m.target.closest("a, button, select")||At(l.dataset.id)})})}function Ls(e,t){if(t=(t||"").trim(),!t||t===R(e.stage_history)){I();return}const s=X(e.stage_history);s.push({stage:t,date:Ee()}),xe(e,{stage_history:$e(s)})}function Ts(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function At(e){let t=i.jobs.find(s=>s.posting_id===e);if(t||(await E(),t=i.jobs.find(s=>s.posting_id===e)),!t){u("posting not found — refresh");return}ze(),Ye(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),Ht("pursuit"),ae(),re(),le()}let Je=null;function Ht(e){Je=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function Ie(){ze(),Ye(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function ze(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function re(){if(!h.postingId)return;let e;try{const s=await fetch(`/api/postings/${h.postingId}/outreach`);if(!s.ok){_e();return}e=await s.json()}catch{_e();return}h.drafts=e.drafts||[],_e();const t=h.drafts[0];t&&t.status==="researching"?Cs():ze()}function Cs(){h.poll||(h.poll=setInterval(re,4e3))}function O(e,t,{multiline:s=!1}={}){if(!e)return;let n=e.value;e.addEventListener("focus",()=>{n=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=n,e.blur()):a.key==="Enter"&&(!s||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===n.trim()){e.value=n;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),n=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=n,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),u(`save failed: ${o.message}`)}})}async function Pt(e,t,s){const n={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:s},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),I(),me(e.posting_id,{title:o.title,location:o.location})}async function Ss(e,t){const s=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();e.url=n.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",g(e.url)),me(e.posting_id,{url:n.url})}function js(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>As(e))}async function Ms(e,t){const s=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();e.company_id=n.company_id,e.company=n.company_name,ae(),E()}let ce=null;function As(e){ce=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const s=document.getElementById("relink-search");s&&(s.value=""),Ot(""),document.getElementById("relink-scrim").classList.add("open"),s&&s.focus()}function ne(){document.getElementById("relink-scrim").classList.remove("open"),ce=null}function Ot(e){const t=document.getElementById("relink-results");if(!t)return;const s=e.trim().toLowerCase();let n=(i.rows||[]).slice();if(s?(n=n.filter(o=>(o.name||"").toLowerCase().includes(s)),n.sort((o,c)=>{const d=(o.name||"").toLowerCase().startsWith(s)?0:1,p=(c.name||"").toLowerCase().startsWith(s)?0:1;return d-p||(o.name||"").localeCompare(c.name||"")})):n.sort((o,c)=>(o.name||"").localeCompare(c.name||"")),n=n.slice(0,60),!n.length){t.innerHTML=`<div class="relink-empty">${(i.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const a=ce?ce.company_id:"";t.innerHTML=n.map(o=>{const c=o.company_id===a,d=[o.vertical,o.location].filter(Boolean).map(r).join(" · ");return`<button type="button" class="relink-result${c?" is-current":""}"
      ...
      </button>`}).join("")}async function qt(e){const t=ce;if(!t){ne();return}if(e===t.company_id){ne();return}try{await Ms(t,e),ne(),u(`moved to ${t.company}`)}catch(s){u(`move failed: ${s.message}`)}}async function Nt(e,t,s){const n={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:s};if(!String(n.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),_(),E()}async function Hs(e,t){const s=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();i.openId=n.company_id,he(n),fe(n.company_id),_(),E()}async function Ps(e,t){const s=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json();e.notes=n.notes}let Rt=null;function ae(){const e=h.row;if(!e)return;const t=document.getElementById("pursuit-body"),n=!!t&&Rt===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${r(e.title||"")}">`;const a=R(e.stage_history);document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a?Ue(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`+(e.verdict?` <span class="${v(e.verdict)}">${r(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>Ne("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    ...
    <section class="pane-section">
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row pipeline-stage-row">
          ...
          <div class="pl-stage-wrap">${qs(e)}</div>
        </div>
  ...
  `,Ns();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>pe(e.company_id)),js(e),O(document.getElementById("pursuit-title-input"),d=>Pt(e,"title",d)),O(document.getElementById("pursuit-url-input"),d=>Ss(e,d)),O(document.getElementById("pursuit-notes-input"),d=>Rs(d),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(d=>O(d,p=>Pt(e,d.dataset.k,p),{multiline:d.tagName==="TEXTAREA"})),_e(),ue(),t&&(t.scrollTop=n),Rt=e.posting_id}function Os(e){return`
    ...
    </div>`}function qs(e){const t=X(e.stage_history),s=t.map((a,o)=>`
    ...
    ${_s(t.contacts)}`,d=n&&(Ds(n.status)||n.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${n?"Draft again":"Draft outreach"}</button>`,p=a.length?`
    <details class="draft-history" ${h.openHist?"open":""}>
      <summary>${a.length} earlier draft${a.length>1?"s":""}</summary>
      <div id="draft-history-body">${a.map(l=>Jt(l,!0)).join("")}</div>
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${n?Jt(n,!1):""}</div><div class="draft-actions">${d}</div>`+p,Ks()}function Ds(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const Vs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',Us='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',Ut=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${Vs}</button>`,Ke=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],Js='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function zs(e){let t=Ke.findIndex(n=>n.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${Ke.map((n,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",c=a<t?Js:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${n.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${Ke[t].active}…</span></div>
  </div>`}function Jt(e,t){const s=(l,m,f="")=>`
    ...
    </div>`;if(e.status==="failed"){const l=Fs(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    ${de(e)}
      ...
      </div></details>`}return t}function zt(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(s=>`<span class="lint-chip" title="${r(s.message||"")}"><code>${r(s.code||"")}</code>${r(s.message||"")}</span>`).join("")+"</div>":""}function Fs(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(s=>`<li>${r(s.claim||s.message||String(s))}${s.why?` <span class="vl-why">— ${r(s.why)}</span>`:""}</li>`).join("")+"</ul>":""}function Ks(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#contacts-editor");if(t){const p=t.querySelector(".cc-list"),l=()=>ks(Array.from(p.querySelectorAll(".cc-row")).map(f=>({position:f.querySelector(".cc-pos").value,email:f.querySelector(".cc-email").value}))),m=f=>{f.querySelectorAll(".cc-pos, .cc-email").forEach(y=>{y.addEventListener("change",()=>Vt(l())),y.addEventListener("keydown",B=>{B.key==="Enter"&&(B.preventDefault(),B.target.blur())})}),f.querySelector(".cc-del").addEventListener("click",()=>{f.remove(),Vt(l())})};p.querySelectorAll(".cc-row").forEach(m),t.querySelector(".cc-add").addEventListener("click",()=>{const f=document.createElement("div");f.innerHTML=jt({position:"",email:""});const y=f.firstElementChild;p.appendChild(y),m(y),y.querySelector(".cc-pos").focus()})}const s=()=>new Date().toISOString().slice(0,10),n=h.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>oe({outreach_count:(n.outreach_count||0)+1,last_outreach_at:s()}));const o=e.querySelector(".pt-outreach-dec");o&&o.addEventListener("click",()=>{const p=Math.max(0,(n.outreach_count||0)-1);oe({outreach_count:p,...p===0?{last_outreach_at:""}:{}})});const c=e.querySelector("#draft-start-btn");c&&c.addEventListener("click",()=>Be()),e.querySelectorAll(".draft-retry-btn").forEach(p=>p.addEventListener("click",()=>Be())),e.querySelectorAll(".draft-regen-btn").forEach(p=>p.addEventListener("click",()=>Be(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(p=>{const l=p.dataset.did,m=p.querySelector(".draft-textarea");m&&O(m,B=>Ys(l,B),{multiline:!0});const f=p.querySelector(".draft-sent-btn");f&&f.addEventListener("click",()=>Gs(l));const y=p.querySelector(".draft-copy-btn");y&&y.addEventListener("click",()=>{const B=p.querySelector(".draft-textarea"),A=p.querySelector(".draft-sentbody"),V=B?B.value:A?A.textContent:"";mn(V,"email copied")})});const d=e.querySelector("details.draft-history");d&&d.addEventListener("toggle",()=>{h.openHist=d.open})}async function Be(e=!1){const t=document.getElementById("outreach-section"),s=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));s&&(s.disabled=!0);let n;try{const o=e?"?regenerate=1":"";n=await fetch(`/api/postings/${h.postingId}/outreach${o}`,{method:"POST"})}catch(o){u(`draft failed: ${o.message}`),s&&(s.disabled=!1);return}if(n.status===202){let o={};try{o=await n.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&u(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await re(),E();return}if(n.status===409){await re(),u("a draft is already active");return}if(n.status===412){let o={};try{o=await n.json()}catch{}Ws(o.need,o.error),s&&(s.disabled=!1);return}if(n.status===503){const o=document.getElementById("outreach-section");if(o){const c=document.createElement("div");c.className="draft-note",c.textContent="Outreach engine not running in this build.",o.appendChild(c)}s&&(s.disabled=!1);return}const a=(await n.text().catch(()=>"")).trim();u(`draft failed: ${a||"HTTP "+n.status}`),s&&(s.disabled=!1)}function Ws(e,t){const s=document.getElementById("outreach-section");if(!s)return;const n=s.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"Discover sources",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,n?n.replaceWith(c):s.appendChild(c);const d=c.querySelector("#gate-fix-btn");d&&d.addEventListener("click",()=>a?Y("outreach-template"):nt());const p=c.querySelector("#gate-retry-btn");p&&p.addEventListener("click",Be)}async function Ys(e,t){const s=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json(),a=h.drafts.findIndex(d=>String(d.id)===String(e));a>=0&&(h.drafts[a]=n);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const d=c.querySelector(".lint-chips"),p=zt(n.lint);d?d.outerHTML=p||"":p&&o.insertAdjacentHTML("afterend",p)}}async function Gs(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(n){u(`failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`failed: ${n||"HTTP "+t.status}`);return}u("marked sent"),await re(),await E();const s=i.jobs.find(n=>n.posting_id===h.postingId);s&&me(s.posting_id,{outreach_count:s.outreach_count,last_outreach_at:s.last_outreach_at,next_up:s.next_up})}async function le(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){ue();return}e=await t.json()}catch{ue();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",ue(),h.answers.some(t=>t.status==="generating")?Qs():Ye()}function Qs(){h.answersPoll||(h.answersPoll=setInterval(le,4e3))}function Ye(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function ue(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,s=h.answersStatus,n=t.some(l=>l.status==="generating"),a=t.length?`<div class="answers-list">${t.map(en).join("")}</div>`:"",o=!!h.detecting,c=n||o?" disabled":"",d=l=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":l}</button>`;let p;s==="ok"&&t.length?p=`<button class="btn ${t.some(m=>!Ft(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${c}>${n?"Drafting…":"Draft answers"}</button>`+d("Re-detect"):s===""||s==="unreachable"?p=`<button class="btn btn-primary" id="answers-start-btn"${c}>${n?"Drafting…":"Draft answers"}</button>`+d("Re-detect questions"):p=d("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r(Zs(s,t.length))}</div>`+a+`<div class="answers-actions">${p}</div>`,sn()}function Zs(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function Ft(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Xs(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function en(e){const t=Ft(e),s=e.edited&&e.edited.trim(),n=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
  ...
  </div>`}function tn(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function sn(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",Kt);const s=e.querySelector("#answers-redetect-btn");s&&s.addEventListener("click",an),e.querySelectorAll(".answer-card[data-aid]").forEach(n=>{const a=n.dataset.aid,o=n.querySelector(".answer-textarea");o&&(O(o,d=>on(a,d),{multiline:!0}),o.addEventListener("input",()=>nn(n,o)));const c=n.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>rn(a))})}function nn(e,t){const s=e.querySelector(".answer-count");if(!s)return;const n=t.value.length,a=s.textContent.includes("/")?parseInt(s.textContent.split("/")[1],10):0;s.textContent=a?`${n} / ${a}`:`${n} chars`,s.classList.toggle("over",!!a&&n>a)}async function Kt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let s;try{s=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){u(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(s.status===202){await le();return}if(s.status===412){let a={};try{a=await s.json()}catch{}cn(a.error),t&&(t.disabled=!1);return}if(s.status===503){Wt("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const n=(await s.text().catch(()=>"")).trim();u(`draft failed: ${n||"HTTP "+s.status}`),t&&(t.disabled=!1)}async function an(){h.detecting=!0,ue();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();u(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){u(`detect failed: ${e.message}`)}h.detecting=!1,await le()}async function on(e,t){const s=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const n=await s.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=n)}async function rn(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(s){u(`regenerate failed: ${s.message}`);return}if(t.status===503){Wt("Answer generation isn't running in this build.");return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`regenerate failed: ${s||"HTTP "+t.status}`);return}await le()}function cn(e){const t=document.getElementById("answers-section");if(!t)return;const s=t.querySelector(".answers-actions"),n=document.createElement("div");n.className="blocks-gate",n.innerHTML=`
    ...
    </div>`,s?s.replaceWith(n):t.appendChild(n);const a=n.querySelector("#answers-fix-btn");a&&a.addEventListener("click",nt);const o=n.querySelector("#answers-retry-btn");o&&o.addEventListener("click",Kt)}function Wt(e){const t=document.getElementById("answers-section");if(!t)return;const s=document.createElement("div");s.className="draft-note",s.textContent=e,t.appendChild(s)}async function pe(e){var d,p;const t=document.getElementById("pane"),s=document.getElementById("scrim"),n=i.openId===e&&t.classList.contains("open"),a=n?((d=document.getElementById("pane-body"))==null?void 0:d.scrollTop)??0:0,o=n?(p=document.getElementById("trace-body"))==null?void 0:p.innerHTML:null;i.openId=e,t.classList.add("open"),s.classList.add("open"),t.setAttribute("aria-hidden","false"),Ht("company"),n||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let c;try{const l=await fetch(`/api/companies/${e}`);if(!l.ok)throw new Error(`HTTP ${l.status}`);c=await l.json()}catch(l){n||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${r(l.message)}</div>`);return}if(i.openId===e){if(he(c),n){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const l=document.getElementById("pane-body");l&&(l.scrollTop=a)}fe(e)}}function Le(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function he(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${r(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
    <span class="${v(e.has_verdict?e.verdict:"")}">${r(e.has_verdict?e.verdict:"unscored")}</span>
  `;const t=document.getElementById("pane-chat");t&&(t.style.display=i.meta&&i.meta.chat?"":"none",t.onclick=()=>Ne("company",e.company_id,e.name));const s=e.model==="manual",n=e.has_verdict?`
    <dl class="kv">
  ...
  `;const y=document.getElementById("posting-add-btn");y&&y.addEventListener("click",()=>pn(e)),Qe(),document.querySelectorAll("#ve-pick .ve-opt").forEach(k=>{k.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Q=>Q.classList.remove("is-on")),k.classList.add("is-on")})});const B=document.getElementById("ve-save-btn");B&&B.addEventListener("click",()=>un(e)),O(document.getElementById("pane-title-input"),k=>Nt(e,"name",k)),document.querySelectorAll("#facts-body [data-k]").forEach(k=>O(k,Q=>Nt(e,k.dataset.k,Q))),O(document.getElementById("pane-domain-input"),k=>Hs(e,k)),O(document.getElementById("pane-notes-input"),k=>Ps(e,k),{multiline:!0});const A=document.getElementById("flag-toggle-btn");A&&A.addEventListener("click",()=>Yt(e.company_id));const V=document.getElementById("review-stamp-btn");V&&V.addEventListener("click",()=>ln(e.company_id));const x=document.getElementById("rescore-btn");x&&x.addEventListener("click",()=>et("verdict",{company_ids:[e.company_id]}));const U=document.getElementById("reenrich-btn");U&&U.addEventListener("click",()=>et("enrich",{company_ids:[e.company_id]}))}function dn(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function ln(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let s;try{s=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){u(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!s.ok){const o=await s.text().catch(()=>"");u(`failed: HTTP ${s.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const n=await s.json(),a=i.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=n.reviewed_at,P()),i.openId===e&&(he(n),fe(e)),u("reviewed")}async function Yt(e){const t=i.rows.find(o=>o.company_id===e),s=!(t&&t.flagged);let n;try{n=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:s})})}catch(o){u(`failed: ${o.message}`);return}if(!n.ok){const o=await n.text().catch(()=>"");u(`failed: HTTP ${n.status}${o?" — "+o:""}`);return}const a=await n.json();t&&(t.flagged=a.flagged,P()),i.openId===e&&(he(a),fe(e)),E(),u(a.flagged?"flagged":"unflagged")}async function un(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){u("Pick yes, maybe, or no.");return}const s=t.dataset.v,n=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:s,reason:n})})}catch(d){u(`save failed: ${d.message}`),a.disabled=!1;return}if(!o.ok){const d=await o.text().catch(()=>"");u(`save failed: HTTP ${o.status}${d?" — "+d:""}`),a.disabled=!1;return}const c=await o.json();he(c),fe(c.company_id),_(),w(),E(),u("verdict saved")}function Ge(e){const t=e.postings||[];return t.length?t.map(s=>{const n=[s.location,s.source==="capture"?"captured":"added",(s.created_at||"").slice(0,10)].filter(Boolean).map(r).join(" · "),a=R(s.stage_history),o=Tt(s.stage_history),c=[s.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?Ue(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`,`<span class="pt-meta">${a?o?r(o):"tracked":"not applied"}</span>`,`<span class="pt-meta">${s.outreach_count?`${s.outreach_count} sent · last ${r(s.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join(""),d=i.meta&&i.meta.chat?`<button class="pcard-chat" data-pid="${r(s.id)}" data-ptitle="${r(s.title||"")}" title="chat about this role">chat</button>`:"";return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function me(e,t){const s=i.openDetail;if(!s||!i.openId)return;const n=(s.postings||[]).find(o=>String(o.id)===String(e));if(!n)return;Object.assign(n,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Ge(s),Qe())}function Qe(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||At(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),Ne("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function pn(e){const t=document.getElementById("posting-url"),s=document.getElementById("posting-title"),n=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){u("Enter a URL first."),t.focus();return}n.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:s.value.trim()})})}catch(p){u(`add failed: ${p.message}`),n.disabled=!1;return}if(!o.ok){const p=await o.text().catch(()=>"");u(`add failed: HTTP ${o.status}${p?" — "+p:""}`),n.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(p=>p.id!==c.id),e.postings.unshift(c);const d=document.getElementById("postings-list");d&&(d.innerHTML=Ge(e),Qe()),t.value="",s.value="",n.disabled=!1,E(),u("link added")}async function fe(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(n){Te(`<div class="muted">Failed to load trail: ${r(n.message)}</div>`);return}if(!t.ok){Te(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const s=(await t.json()).events||[];if(s.length===0){Te('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Te(s.map(hn).join(""))}function hn(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(r);return e.run_id&&t.push("run "+r(e.run_id.slice(0,8))),`
...
`)):document.getElementById("editor-text").value=c.content||"",a&&(document.getElementById("editor-enabled").checked=c.enabled!==!1),c.taste_version&&(document.getElementById("editor-ver").textContent="version "+c.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Ae(){document.getElementById("editor-scrim").classList.remove("open"),C=null}const wn=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function nt(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{ot(await(await fetch("/api/outreach/sources")).json())}catch(e){u(`failed to load sources: ${e.message}`)}}function at(){document.getElementById("sources-scrim").classList.remove("open")}function ot(e){const t=document.getElementById("sources-list");if(!t)return;const s=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):wn,n={};(e&&e.sources||[]).forEach(a=>{(n[a.need]=n[a.need]||[]).push(a)}),t.innerHTML=s.map(a=>{const o=n[a.key]||[],c=o.length?o.map(d=>`<li><span class="src-title">${r(d.title||d.page_id)}</span><button class="src-rm" data-need="${r(a.key)}" data-id="${r(d.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
    ...
    <button class="crit-edit"${s} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function D(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,n=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(n){let $="off",L="";const ve=t&&t.criteria_state;ve==="current"?($="ok",L="current · verified "+rt(t.verified_age_seconds)):ve==="changed"?($="warn",L="changed — re-distill"):ve==="unverified"?($="warn",L=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?($="warn",L="brain offline · using cache"):a&&($="ok",L="fetched "+rt(t.age_seconds)),o=q({icon:is,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:$,note:L,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:ge,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=q({icon:is,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:G,actTitle:"edit taste.md",actLabel:"edit taste"});const c=i.sources&&i.sources.sources||[],d=c.filter($=>$.need==="experience").length,p=c.filter($=>$.need==="voice").length;let l="off",m="not discovered yet — refresh from the brain";d>0?(l="ok",m=`${d} experience · ${p} voice`):c.length>0&&(l="warn",m="no experience yet — refresh");const f=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",y=q({icon:Cn,nameHTML:f,dot:l,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:ge,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),B=q({icon:Ln,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:G,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),A=q({icon:Tn,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:G,actTitle:"edit the outreach email template",actLabel:"edit email template"}),V=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],x=V.map(([$,L,ve])=>q({icon:dt,nameHTML:`<span class="edit-link" data-act="edit-prompt-${$}" title="edit the ${L.replace(/^\d+ · /,"")} prompt">${L}</span>`,desc:ve,act:`edit-prompt-${$}`,actIcon:G,actTitle:`edit the ${L} prompt`,actLabel:`edit ${L} prompt`})).join(""),U=!i.stats||i.stats.taste_filter_enabled!==!1,k=q({icon:Sn,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage. Toggle it off in the editor to score every company.",dot:U?"ok":"off",note:U?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:G,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),Q=i.anthropicKey;let yt="off",bt="not set — verdict, capture & outreach disabled";Q&&Q.key_source==="db"?(yt="ok",bt="set here · active"):Q&&Q.key_source==="env"&&(yt="ok",bt="from the environment");const zn=q({icon:Bn,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:yt,note:bt,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:G,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),Fn=q({icon:dt,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:G,actTitle:"edit the application stages",actLabel:"edit application stages"}),Kn=q({icon:dt,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:G,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${zn}
     </div>`;const wt={"view-profile":()=>Pn(i.profile),"refresh-profile":Hn,"edit-taste":()=>Y("taste"),"edit-taste-filter":()=>Y("taste-filter"),"edit-application-stages":()=>Y("application-stages"),"edit-outreach-statuses":()=>Y("outreach-statuses"),"edit-playbook":()=>Y("playbook"),"edit-template":()=>Y("outreach-template"),"view-sources":nt,"refresh-sources":jn,"edit-anthropic-key":Mn};for(const[$]of V)wt[`edit-prompt-${$}`]=()=>Y(`outreach-prompts/${$}`);e.querySelectorAll("[data-act]").forEach($=>{const L=$.dataset.act;L&&wt[L]&&($.onclick=wt[L])})}async function lt(){try{i.sources=await(await fetch("/api/outreach/sources")).json()}catch{i.sources=null}D()}async function jn(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(n){u(`refresh failed: ${n.message}`),lt();return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`refresh failed: ${n||"HTTP "+t.status}`),lt();return}const s=await t.json();s.warning?u(s.warning):u("outreach knowledge refreshed"),i.sources={sources:s.sources||[],needs:i.sources&&i.sources.needs||[]},D()}async function rs(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}D()}async function Mn(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await rs(),ut();const e=document.getElementById("key-input");e&&e.focus()}function ut(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const s=document.getElementById("key-remove");s&&(s.style.display=e.key_source==="db"?"":"none");const n=document.getElementById("key-restart-hint");if(n){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);n.style.display=a?"":"none"}}function pt(){document.getElementById("key-scrim").classList.remove("open")}async function cs(){const e=(document.getElementById("key-input").value||"").trim();if(!e){u("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const s=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let n;try{n=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){u(`save failed: ${a.message}`),s();return}if(!n.ok){u((await n.text().catch(()=>"")).trim()||`HTTP ${n.status}`),s();return}i.anthropicKey=await n.json(),document.getElementById("key-input").value="",s(),u("Anthropic key saved"),await Ze(),ut(),D()}async function An(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(s){u(`remove failed: ${s.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){u((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),u(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await Ze(),ut(),D()}async function Hn(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),ct();return}if(!t.ok){const s=await t.text().catch(()=>"");u(`refresh failed: ${(s||"").trim()||"HTTP "+t.status}`),ct();return}i.profile=await t.json(),D(),u("company-fit brief refreshed"),w()}function Pn(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${rt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function ht(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=ht,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&ht()};function ds(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Oe(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function mt(){document.getElementById("docs-scrim").classList.remove("open")}function On(){return document.getElementById("docs-scrim").classList.contains("open")}function Oe(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function qn(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Oe(e)}document.getElementById("open-docs").onclick=ds,document.getElementById("docs-close").onclick=mt,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&mt()};function Nn(){document.getElementById("settings-scrim").classList.add("open"),D()}function ft(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Nn,document.getElementById("settings-close").onclick=ft,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&ft()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Oe(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(s=>{const n=s.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);n.length&&Oe(n[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(s=>t.observe(s))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function Rn(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function Dn(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function qe(e,t){const s=document.createElement("div");return s.className="chat-msg chat-"+e,s.textContent=t||"",s}function gt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function Vn(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function ls(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const s of e||[]){const n=Rn(s.content);if(s.role==="user")n&&t.appendChild(qe("user",n));else if(s.role==="assistant"){const a=Dn(s.content);if(!n&&!a.length)continue;const o=qe("assistant",n);if(a.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+a.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(Vn()),gt()}async function Ne(e,t,s){if(!i.meta||!i.meta.chat){u("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":s||"";const n=document.getElementById("chat-messages");n.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const d=await c.json();i.chat.threadId=d.thread.id,ls(d.messages||[])}catch(o){n.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function vt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Re(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function us(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function ps(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",us(),Re(!0);const s=document.getElementById("chat-messages"),n=s.querySelector(".chat-empty");n&&n.remove(),s.appendChild(qe("user",t));const a=qe("assistant","");a.classList.add("chat-streaming"),s.appendChild(a),gt();let o="";const c=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Re(!1)},d=i.chat.threadId;let p;try{p=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!p.ok){c((await p.text().catch(()=>"")).trim()||"HTTP "+p.status);return}const l=new EventSource("/api/chat/"+d+"/stream");i.chat.es=l,l.addEventListener("delta",m=>{o+=m.data,a.textContent=o,gt()}),l.addEventListener("end",async m=>{l.close(),i.chat.es===l&&(i.chat.es=null),a.classList.remove("chat-streaming"),Re(!1),i.chat.threadId===d&&await Un(),Jn(),typeof m.data=="string"&&m.data.indexOf("error")===0&&u("chat: "+m.data)}),l.onerror=()=>{l.close(),i.chat.es===l&&(i.chat.es=null),a.classList.remove("chat-streaming"),Re(!1)}}async function Un(){const e=i.chat.scope,t=i.chat.scopeId,s="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const n=await fetch("/api/chat/threads?"+s);if(!n.ok)return;const a=await n.json();ls(a.messages||[])}catch{}}function Jn(){_(),E(),w(),i.openId&&pe(i.openId)}document.getElementById("open-chat").onclick=()=>Ne("global","",""),document.getElementById("chat-close").onclick=vt,document.getElementById("chat-scrim").onclick=vt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),ps()}),document.getElementById("chat-input").addEventListener("input",us),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),ps())}),Bt("#t tbody",fs),Bt("#jt tbody",gs),_(),E(),w(),Ze(),Xe(),ct(),lt(),rs(),ie()}Gn({"":{view:()=>({mount(b){b.innerHTML=ta,sa()}}),chrome:!1}},{title:"scout"});Xn();ea();
