(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))_(r);new MutationObserver(r=>{for(const v of r)if(v.type==="childList")for(const $ of v.addedNodes)$.tagName==="LINK"&&$.rel==="modulepreload"&&_($)}).observe(document,{childList:!0,subtree:!0});function g(r){const v={};return r.integrity&&(v.integrity=r.integrity),r.referrerPolicy&&(v.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?v.credentials="include":r.crossOrigin==="anonymous"?v.credentials="omit":v.credentials="same-origin",v}function _(r){if(r.ep)return;r.ep=!0;const v=g(r);fetch(r.href,v)}})();function ws(y,i){const g=i.replace(/^#/,"");let _=null;for(const r of Object.keys(y))(g===r||g.startsWith(r))&&(_===null||r.length>_.length)&&(_=r);return _===null&&""in y&&(_=""),_}function ks(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function Es(y,i={}){const g=i.root??document.body,_=i.title??document.title??"",r=i.brandHref??"#",v=document.createElement("main"),$=document.createElement("header");$.className="cap-head";const k=document.createElement("a");k.className="brand",k.href=r,k.textContent=_,k.setAttribute("aria-label",`${_} — home`),$.appendChild(k);const D=document.createElement("nav");D.className="cap-nav",D.setAttribute("aria-label","Views");for(const B of i.nav??[]){const M=document.createElement("a");M.href=B.href,M.textContent=B.label,B.ariaLabel&&M.setAttribute("aria-label",B.ariaLabel),D.appendChild(M)}$.appendChild(D);const A=document.createElement("section");A.className="tk-content",v.appendChild($),v.appendChild(A);const S=document.createElement("div");S.className="tk-bleed";const Ae=B=>{var M;for(const P of Array.from(D.querySelectorAll("a"))){const T=((M=P.getAttribute("href"))==null?void 0:M.replace(/^#/,""))??"";P.toggleAttribute("aria-current",B!==null&&B!==""&&T===B),P.hasAttribute("aria-current")&&P.setAttribute("aria-current","page")}};let G=0;const H=()=>{const B=ws(y,location.hash);if(Ae(B),B===null){S.isConnected&&S.remove(),v.isConnected||g.appendChild(v),$s(A,"Not found.");return}const{view:M,chrome:P}=ks(y[B]),T=P?A:S;P?(S.isConnected&&S.remove(),v.isConnected||g.appendChild(v)):(v.isConnected&&v.remove(),S.isConnected||g.appendChild(S)),T.replaceChildren();const Q=M(),He=++G,ne=Q.mount(T);ne instanceof Promise&&ne.catch(Pe=>{He===G&&Is(T,String(Pe))})};window.addEventListener("hashchange",H),H()}function $s(y,i){y.replaceChildren();const g=document.createElement("div");g.className="tk-empty",g.textContent=i,y.appendChild(g)}function Is(y,i){y.replaceChildren();const g=document.createElement("div");g.className="tk-error",g.textContent=i,y.appendChild(g)}function xs(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(i=>{for(const g of i)g.unregister()}),window.caches&&caches.keys().then(i=>{for(const g of i)caches.delete(g)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function _s(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let i;try{i=await y.json()}catch{return null}return typeof i.email=="string"&&i.email?{email:i.email}:null}const Bs=`
<div class="layout">
<aside class="sidebar">
...
`;function Ls(y){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},openDetail:null,anthropicKey:null},g=e=>"pill pill-"+(e||"none"),_='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),v=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],q()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],I(),D(),S()}function D(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&oe())}let A=null;function S(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!A?A=setInterval(Ae,4e3):!e&&A&&(clearInterval(A),A=null)}async function Ae(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&(I(),D()),S()}function G(e){i.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",qe(),e==="jobs"?I():q()}async function H(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}i.stats=e,B()}function B(){const e=i.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,V()}function M(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function P(e){return e.slice().sort((t,n)=>i.sort.dir*M(t,n,i.sort.k))}const T=new Set;let Q=!1;function He(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",T.has(e.dataset.v))})}function ne(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(T.size&&!T.has(t.verdict||"__none__")||Q&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Pe=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Xt=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function ht(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const mt=ht("scout-hidden-cols"),ft=ht("scout-hidden-jcols");function vt(){return i.view==="jobs"?{cols:Xt,hidden:ft,key:"scout-hidden-jcols"}:{cols:Pe,hidden:mt,key:"scout-hidden-cols"}}function Z(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=mt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=ft.has(e.dataset.col)?"none":""})}function qe(){const e=vt();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=vt(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),qe(),Z()})})}function gt(e){return`
      <td class="td-flag" data-col="flag"><button class="flag-btn${e.flagged?" is-on":""}" data-id="${e.company_id}" title="${e.flagged?"unflag":"flag"}">${_}</button></td>
    ...
    `}function yt(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>Ht(t.dataset.id))}const en=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],tn=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function bt(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let d=0;d<n;d++){const l=t.map(([p,c])=>{const m=c.endsWith("%")?Math.round(parseFloat(c)*a[d%a.length])+"%":c;return`<td${p?` data-col="${p}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${l}</tr>`}s.innerHTML=o,Z()}function q(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=P(ne());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=gt(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||de(s.dataset.id)}),e.appendChild(s),yt(s)}Z()}async function nn(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=P(ne()).map(d=>d.company_id),o=[...s.querySelectorAll("tr")].map(d=>d.dataset.id);if(a.length!==o.length||a.some((d,l)=>d!==o[l])){q();return}for(const d of e){const l=i.rows.find(c=>c.company_id===d),p=s.querySelector(`tr[data-id="${CSS.escape(d)}"]`);if(!l||!p){q();return}p.innerHTML=gt(l),yt(p)}Z()}const R=new Set;let ve=!1,ge=!1,W=!0;function sn(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=W&&!R.has("rejected");return i.jobs.filter(n=>!(t&&n.response==="rejected"||R.size&&!R.has(n.response||"")||ve&&!n.next_up||ge&&(n.outreach_count|0)>0||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const an=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function wt(e){const t=String(e||"").split(",").map(n=>n.trim()).filter(Boolean);return t.length?t.map(n=>{const s=n.match(an);return s?`<a href="mailto:${r(s[0])}" title="${r(n)}">${r(n)}</a>`:r(n)}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}const U={offer:{cls:"pill-yes",label:"offer",order:0},interview:{cls:"pill-info",label:"interview",order:1},screening:{cls:"pill-maybe",label:"screening call",order:2},"":{cls:"pill-none",label:"—",order:3},rejected:{cls:"pill-no",label:"rejected",order:4}};function on(e,t,n){var s,a;if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[e.verdict]??3)-(o[t.verdict]??3)}if(n==="response")return(((s=U[e.response])==null?void 0:s.order)??3)-(((a=U[t.response])==null?void 0:a.order)??3);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="applied_at"||n==="last_outreach_at"){const o=e[n]||"",d=t[n]||"";return!o&&!d?0:o?d?String(d).localeCompare(String(o)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function se(e){const t=e.options[e.selectedIndex],n=getComputedStyle(e),s=se._c||(se._c=document.createElement("canvas").getContext("2d"));s.font=`${n.fontWeight} ${n.fontSize} ${n.fontFamily}`;const a=s.measureText(t?t.text:"").width;e.style.width=Math.ceil(a+35)+"px"}function I(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=sn().sort((c,m)=>i.jsort.dir*on(c,m,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=W&&!R.has("rejected")?i.jobs.filter(c=>c.response==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=i.jobs.filter(c=>c.next_up).length,o=document.getElementById("next-up-n");o.textContent=a,o.style.display=a?"":"none";const d=i.jobs.filter(c=>!(c.outreach_count|0)&&!(W&&!R.has("rejected")&&c.response==="rejected")).length,l=document.getElementById("not-reached-n");l.textContent=d,l.style.display=d?"":"none";const p=document.getElementById("jobs-hidden-note");p.style.display=n?"":"none",n&&(p.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{W=!1,document.getElementById("hide-rejected").classList.remove("is-on"),I()});for(const c of t){const m=U[c.response]||U[""],f=document.createElement("tr");f.dataset.id=c.posting_id;const L=[["","none"],["screening","screening"],["interview","interview"],["offer","offer"],["rejected","rejected"]].map(([E,N])=>`<option value="${E}"${(c.response||"")===E?" selected":""}>${N}</option>`).join("");f.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${c.next_up?" is-on":""}" title="${c.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${c.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${r(c.title||c.company)}</span>${rn(c.outreach_draft_status)}${c.title?`<div class="small dim">${r(c.company)}</div>`:""}</div></div></td>
      ...
      <td data-col="response"><select class="jt-resp ${m.cls}" title="furthest response reached">${L}</select></td>
      ...
      <td class="small td-contacts" data-col="contacts">${wt(c.contacts)}</td>
      <td data-col="link"><a href="${v(c.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `;const j=()=>new Date().toISOString().slice(0,10);f.querySelector(".jt-nextup").onclick=()=>_t(c,!1),f.querySelector(".jt-applied").onclick=()=>be(c,{applied_at:c.applied_at?"":j()}),f.querySelector(".jt-resp").onchange=E=>{se(E.target),be(c,{response:E.target.value})},f.querySelector(".jt-inc").onclick=()=>be(c,{outreach_count:(c.outreach_count||0)+1,last_outreach_at:j()}),f.querySelector(".jt-dec").onclick=()=>{const E=Math.max(0,(c.outreach_count||0)-1);be(c,{outreach_count:E,...E===0?{last_outreach_at:""}:{}})},e.appendChild(f)}e.querySelectorAll(".jt-resp").forEach(se),Z(),e.querySelectorAll("tr").forEach(c=>{c.addEventListener("click",m=>{m.target.closest("a, button, select")||kt(c.dataset.id)})})}function rn(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="needs_work"?'<span class="draft-badge db-needswork" title="the draft finished below the depth bar — review, fix or regenerate">draft needs work</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function kt(e){let t=i.jobs.find(n=>n.posting_id===e);if(t||(await k(),t=i.jobs.find(n=>n.posting_id===e)),!t){u("posting not found — refresh");return}Ne(),De(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),Et("pursuit"),oe(),ae(),re()}let Oe=null;function Et(e){Oe=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function ye(){Ne(),De(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function Ne(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function ae(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){we();return}e=await n.json()}catch{we();return}h.drafts=e.drafts||[],we();const t=h.drafts[0];t&&t.status==="researching"?cn():Ne()}function cn(){h.poll||(h.poll=setInterval(ae,4e3))}function O(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),u(`save failed: ${o.message}`)}})}async function $t(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,summary:o.summary,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),I(),ue(e.posting_id,{title:o.title,location:o.location,summary:o.summary})}async function dn(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",v(e.url)),ue(e.posting_id,{url:s.url})}async function It(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),$(),k()}async function ln(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();i.openId=s.company_id,le(s),pe(s.company_id),$(),k()}async function un(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}let xt=null;function oe(){const e=h.row;if(!e)return;const t=document.getElementById("pursuit-body"),s=!!t&&xt===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${r(e.title||"")}">`;const a=U[e.response]||U[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a.cls}">${r(a.label)}</span>`+(e.verdict?` <span class="${g(e.verdict)}">${r(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>Se("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    <section class="pane-section role-head">
      <div id="role-body">${pn(e)}</div>
    </section>
  ...
  `,hn();const d=document.getElementById("pursuit-company-link");d&&d.addEventListener("click",()=>de(e.company_id)),O(document.getElementById("pursuit-title-input"),l=>$t(e,"title",l)),O(document.getElementById("pursuit-url-input"),l=>dn(e,l)),O(document.getElementById("pursuit-notes-input"),l=>mn(l),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(l=>O(l,p=>$t(e,l.dataset.k,p),{multiline:l.tagName==="TEXTAREA"})),we(),ce(),t&&(t.scrollTop=s),xt=e.posting_id}function pn(e){return`
    ...
    </div>`}function hn(){const e=h.row,t=()=>new Date().toISOString().slice(0,10),n=document.querySelector("#pursuit-body .pt-applied");n&&n.addEventListener("click",()=>Y({applied_at:e.applied_at?"":t()}));const s=document.querySelector("#pursuit-body .pl-applied-date");s&&s.addEventListener("change",d=>Y({applied_at:d.target.value}));const a=document.querySelector("#pursuit-body .pl-response");a&&a.addEventListener("change",d=>Y({response:d.target.value}));const o=document.querySelector("#pursuit-body .pt-nextup");o&&o.addEventListener("click",()=>_t(h.row,!0))}async function _t(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){u(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();u(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,I(),ue(e.posting_id,{next_up:s.next_up}),t&&oe(),u(e.next_up?"queued next up":"removed from the queue")}async function Bt(e,t){const n={applied_at:e.applied_at||"",response:e.response||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",contacts:e.contacts||"",notes:e.notes||"",suggested_contacts:e.suggested_contacts||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return u(`save failed: ${o.message}`),null}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();return u(`save failed: ${o||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,contacts:a.contacts,notes:a.notes,suggested_contacts:a.suggested_contacts,next_up:a.next_up}),ue(e.posting_id,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function mn(e){const t=h.row,n={applied_at:t.applied_at||"",response:t.response||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",contacts:t.contacts||"",notes:e,suggested_contacts:t.suggested_contacts||""},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,I()}async function Y(e){await Bt(h.row,e)&&(I(),oe(),u("tracking saved"))}async function be(e,t){await Bt(e,t)&&(I(),h.postingId===e.posting_id&&(h.row=e,oe()),u("tracking saved"))}function we(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.row,n=h.drafts,s=n[0]||null,a=n.slice(1),o=`
    ...
    <div class="outreach-contacts">
      ...
      <div class="oc-rendered">${wt(t.contacts)}</div>
    </div>
    ...
    </div>`,l=s&&(fn(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,p=a.length?`
    <details class="draft-history" ${h.openHist?"open":""}>
      <summary>${a.length} earlier draft${a.length>1?"s":""}</summary>
      <div id="draft-history-body">${a.map(c=>Tt(c,!0)).join("")}</div>
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${s?Tt(s,!1):""}</div><div class="draft-actions">${l}</div>`+p,kn()}function fn(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const vn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',gn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',Lt=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${vn}</button>`,Re=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"},{key:"judge",label:"Review",active:"Judging against the doctrine"}],yn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function bn(e){let t=Re.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${Re.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",d=a<t?yn:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${d}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${Re[t].active}…</span></div>
  </div>`}function Tt(e,t){const n=(f,L,j="")=>`
    <div class="draft-head">
      <span class="${f}">${L}</span>${j}
    ...
    </div>`;if(e.status==="failed"){const f=wn(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    <div class="draft-head">${d}${s?Lt:""}</div>
    ${p}
    ${c}
    ...
    ${ie(e)}
  </div>`}function Ve(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function ie(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(p,c)=>c?`<div class="tr-line"><span class="tr-key">${p}:</span> ${r(String(c))}</div>`:"",o=n.role||{},d=Array.isArray(n.hooks)?n.hooks:[],l=d.map(p=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function Ct(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${r(n.message||"")}"><code>${r(n.code||"")}</code>${r(n.message||"")}</span>`).join("")+"</div>":""}function jt(e){let t=null;try{t=JSON.parse(e||"null")}catch{return""}if(!t||typeof t!="object")return"";const n=t.depth==="deep"?"pill-yes":t.depth==="medium"?"pill-maybe":"pill-no",s={direct:"pill-yes",adjacent:"pill-info",standing:"pill-maybe"}[t.proof_tier]||"pill-no",a=t.proof_tier==="standing"?"standing creds":t.proof_tier,o=[t.depth?`<span class="pill ${n}">depth: ${r(t.depth)}</span>`:"",t.proof_tier?`<span class="pill ${s}">proof: ${r(a)}</span>`:""].filter(Boolean).join(""),d=Array.isArray(t.weaknesses)&&t.weaknesses.length?'<ul class="critique-list">'+t.weaknesses.map(c=>`<li>${r(String(c))}</li>`).join("")+"</ul>":"",l=String(t.experience_gaps||"").trim(),p=l?`<div class="critique-gap"><span class="cg-label">experience gap:</span> ${r(l)}</div>`:"";return!o&&!d&&!p?"":`<div class="draft-critique">
    ${o?`<div class="critique-chips">${o}</div>`:""}
    ${d}${p}
  </div>`}function wn(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${r(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${r(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function kn(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector(".oc-input");t&&(t.addEventListener("change",c=>Y({contacts:c.target.value.trim()})),t.addEventListener("keydown",c=>{c.key==="Enter"&&(c.preventDefault(),c.target.blur())}));const n=e.querySelector(".sc-input");n&&(n.addEventListener("change",c=>Y({suggested_contacts:c.target.value.trim()})),n.addEventListener("keydown",c=>{c.key==="Enter"&&(c.preventDefault(),c.target.blur())}));const s=()=>new Date().toISOString().slice(0,10),a=h.row,o=e.querySelector(".pt-outreach");o&&o.addEventListener("click",()=>Y({outreach_count:(a.outreach_count||0)+1,last_outreach_at:s()}));const d=e.querySelector(".pt-outreach-dec");d&&d.addEventListener("click",()=>{const c=Math.max(0,(a.outreach_count||0)-1);Y({outreach_count:c,...c===0?{last_outreach_at:""}:{}})});const l=e.querySelector("#draft-start-btn");l&&l.addEventListener("click",()=>ke()),e.querySelectorAll(".draft-retry-btn").forEach(c=>c.addEventListener("click",()=>ke())),e.querySelectorAll(".draft-regen-btn").forEach(c=>c.addEventListener("click",()=>ke(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(c=>{const m=c.dataset.did,f=c.querySelector(".draft-textarea");f&&O(f,E=>$n(m,E),{multiline:!0});const L=c.querySelector(".draft-sent-btn");L&&L.addEventListener("click",()=>In(m));const j=c.querySelector(".draft-copy-btn");j&&j.addEventListener("click",()=>{const E=c.querySelector(".draft-textarea"),N=c.querySelector(".draft-sentbody"),te=E?E.value:N?N.textContent:"";Vn(te,"email copied")})});const p=e.querySelector("details.draft-history");p&&p.addEventListener("toggle",()=>{h.openHist=p.open})}async function ke(e=!1){const t=document.getElementById("outreach-section"),n=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));n&&(n.disabled=!0);let s;try{const o=e?"?regenerate=1":"";s=await fetch(`/api/postings/${h.postingId}/outreach${o}`,{method:"POST"})}catch(o){u(`draft failed: ${o.message}`),n&&(n.disabled=!1);return}if(s.status===202){let o={};try{o=await s.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&u(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await ae(),k();return}if(s.status===409){await ae(),u("a draft is already active");return}if(s.status===412){let o={};try{o=await s.json()}catch{}En(o.need,o.error),n&&(n.disabled=!1);return}if(s.status===503){const o=document.getElementById("outreach-section");if(o){const d=document.createElement("div");d.className="draft-note",d.textContent="Outreach engine not running in this build.",o.appendChild(d)}n&&(n.disabled=!1);return}const a=(await s.text().catch(()=>"")).trim();u(`draft failed: ${a||"HTTP "+s.status}`),n&&(n.disabled=!1)}function En(e,t){const n=document.getElementById("outreach-section");if(!n)return;const s=n.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"Discover sources",d=document.createElement("div");d.className="blocks-gate",d.innerHTML=`
    ...
    </div>`,s?s.replaceWith(d):n.appendChild(d);const l=d.querySelector("#gate-fix-btn");l&&l.addEventListener("click",()=>a?X("outreach-template"):Ge());const p=d.querySelector("#gate-retry-btn");p&&p.addEventListener("click",ke)}async function $n(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.drafts.findIndex(l=>String(l.id)===String(e));a>=0&&(h.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),d=o&&o.closest(".draft-card");if(d){const l=d.querySelector(".lint-chips"),p=Ct(s.lint);l?l.outerHTML=p||"":p&&o.insertAdjacentHTML("afterend",p)}}async function In(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){u(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`failed: ${s||"HTTP "+t.status}`);return}u("marked sent"),await ae(),await k();const n=i.jobs.find(s=>s.posting_id===h.postingId);n&&ue(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function re(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){ce();return}e=await t.json()}catch{ce();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",ce(),h.answers.some(t=>t.status==="generating")?xn():De()}function xn(){h.answersPoll||(h.answersPoll=setInterval(re,4e3))}function De(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function ce(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(c=>c.status==="generating"),a=t.length?`<div class="answers-list">${t.map(Ln).join("")}</div>`:"",o=!!h.detecting,d=s||o?" disabled":"",l=c=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":c}</button>`;let p;n==="ok"&&t.length?p=`<button class="btn ${t.some(m=>!St(m)&&m.status!=="generating")?"btn-primary":""}" id="answers-start-btn"${d}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect"):n===""||n==="unreachable"?p=`<button class="btn btn-primary" id="answers-start-btn"${d}>${s?"Drafting…":"Draft answers"}</button>`+l("Re-detect questions"):p=l("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r(_n(n,t.length))}</div>`+a+`<div class="answers-actions">${p}</div>`,Cn()}function _n(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function St(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Bn(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function Ln(e){const t=St(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,d=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`;return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${r(Tn(e.fail_reason))}</div>`:""}
  </div>`}function Tn(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function Cn(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",Mt);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",Sn),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(O(o,l=>Mn(a,l),{multiline:!0}),o.addEventListener("input",()=>jn(s,o)));const d=s.querySelector(".answer-regen-btn");d&&d.addEventListener("click",()=>An(a))})}function jn(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function Mt(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){u(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await re();return}if(n.status===412){let a={};try{a=await n.json()}catch{}Hn(a.error),t&&(t.disabled=!1);return}if(n.status===503){At("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();u(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function Sn(){h.detecting=!0,ce();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();u(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){u(`detect failed: ${e.message}`)}h.detecting=!1,await re()}async function Mn(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=s)}async function An(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){u(`regenerate failed: ${n.message}`);return}if(t.status===503){At("Answer generation isn't running in this build.");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();u(`regenerate failed: ${n||"HTTP "+t.status}`);return}await re()}function Hn(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",Ge);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",Mt)}function At(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function de(e){var l,p;const t=document.getElementById("pane"),n=document.getElementById("scrim"),s=i.openId===e&&t.classList.contains("open"),a=s?((l=document.getElementById("pane-body"))==null?void 0:l.scrollTop)??0:0,o=s?(p=document.getElementById("trace-body"))==null?void 0:p.innerHTML:null;i.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),Et("company"),s||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let d;try{const c=await fetch(`/api/companies/${e}`);if(!c.ok)throw new Error(`HTTP ${c.status}`);d=await c.json()}catch(c){s||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${r(c.message)}</div>`);return}if(i.openId===e){if(le(d),s){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const c=document.getElementById("pane-body");c&&(c.scrollTop=a)}pe(e)}}function Ee(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function le(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${r(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `;const L=document.getElementById("posting-add-btn");L&&L.addEventListener("click",()=>Nn(e)),Je(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(K=>K.classList.remove("is-on")),b.classList.add("is-on")})});const j=document.getElementById("ve-save-btn");j&&j.addEventListener("click",()=>On(e)),O(document.getElementById("pane-title-input"),b=>It(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>O(b,K=>It(e,b.dataset.k,K))),O(document.getElementById("pane-domain-input"),b=>ln(e,b)),O(document.getElementById("pane-notes-input"),b=>un(e,b),{multiline:!0});const E=document.getElementById("flag-toggle-btn");E&&E.addEventListener("click",()=>Ht(e.company_id));const N=document.getElementById("review-stamp-btn");N&&N.addEventListener("click",()=>qn(e.company_id));const te=document.getElementById("rescore-btn");te&&te.addEventListener("click",()=>Ke("verdict",{company_ids:[e.company_id]}));const me=document.getElementById("reenrich-btn");me&&me.addEventListener("click",()=>Ke("enrich",{company_ids:[e.company_id]}))}function Pn(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function qn(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){u(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");u(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=i.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,q()),i.openId===e&&(le(s),pe(e)),u("reviewed")}async function Ht(e){const t=i.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){u(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");u(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,q()),i.openId===e&&(le(a),pe(e)),k(),u(a.flagged?"flagged":"unflagged")}async function On(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){u("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(l){u(`save failed: ${l.message}`),a.disabled=!1;return}if(!o.ok){const l=await o.text().catch(()=>"");u(`save failed: HTTP ${o.status}${l?" — "+l:""}`),a.disabled=!1;return}const d=await o.json();le(d),pe(d.company_id),$(),H(),k(),u("verdict saved")}function Ue(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(r).join(" · "),a=U[n.response]||U[""],o=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a.cls}">${r(a.label)}</span>`,`<span class="pt-meta">${n.applied_at?`applied ${r(n.applied_at)}`:"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${r(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join(""),d=i.meta&&i.meta.chat?`<button class="pcard-chat" data-pid="${r(n.id)}" data-ptitle="${r(n.title||"")}" title="chat about this role">chat</button>`:"";return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ue(e,t){const n=i.openDetail;if(!n||!i.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=Ue(n),Je())}function Je(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||t.target.closest(".pcard-chat")||kt(e.dataset.pid)})}),document.querySelectorAll("#postings-list .pcard-chat").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),Se("posting",e.dataset.pid,e.dataset.ptitle||"")})})}async function Nn(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){u("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(p){u(`add failed: ${p.message}`),s.disabled=!1;return}if(!o.ok){const p=await o.text().catch(()=>"");u(`add failed: HTTP ${o.status}${p?" — "+p:""}`),s.disabled=!1;return}const d=await o.json();e.postings=(e.postings||[]).filter(p=>p.id!==d.id),e.postings.unshift(d);const l=document.getElementById("postings-list");l&&(l.innerHTML=Ue(e),Je()),t.value="",n.value="",s.disabled=!1,k(),u("link added")}async function pe(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){$e(`<div class="muted">Failed to load trail: ${r(s.message)}</div>`);return}if(!t.ok){$e(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){$e('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}$e(n.map(Rn).join(""))}function Rn(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(r);return e.run_id&&t.push("run "+r(e.run_id.slice(0,8))),`
    ...
    </div>`}function $e(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let Pt;function u(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(Pt),Pt=setTimeout(()=>t.classList.remove("show"),2200)}async function Vn(e,t="copied"){if(!e){u("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}u(t)}catch(n){u(`copy failed: ${n.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function ze(){try{const s=await fetch("/api/meta");if(!s.ok)return;i.meta=await s.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=i.meta.chat?"":"none")}async function Fe(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Ie=null;async function Ke(e,t){if(i.meta&&i.meta.control===!1){u("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){u(`run failed: ${a.message}`);return}if(n.status===409){u("a job is already running");return}if(n.status===412){const a=await n.text();u(a.trim());return}if(!n.ok){u(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Ut(e,s,t)}async function Dn(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){u(`upload failed: ${a.message}`);return}if(n.status===409){u("a job is already running");return}if(!n.ok){u(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Ut("ingest",s)}const Un=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let xe=[],J=new Set,z="company";function qt(e){z=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",Ot()}function We(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function Ot(){const e=document.getElementById("add-note");We()?e.innerHTML=z==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=z==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function Jn(){Un.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",J=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),qt(i.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(s=>`<option value="${r(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),xe=s.verticals||[]}catch{xe=[]}Nt()}function _e(){document.getElementById("add-scrim").classList.remove("open")}function Nt(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=xe.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${J.has(s)?" sel":""}" data-v="${r(s)}">${r(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;J.has(a)?J.delete(a):J.add(a),s.classList.toggle("sel"),Rt()}))):e.innerHTML=`<div class="none">${xe.length?"no match":"no verticals in the set yet"}</div>`,Rt()}function Rt(){const e=J.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Vt(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Dt(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){u(z==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,We()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=f=>document.getElementById(f).value.trim(),d=We();let l,p;d?(l="/api/capture",p={url:Vt(t),kind:z==="company"?"company_page":"job_posting",fields:z==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...J].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):z==="company"?(l="/api/companies",p={website:t,name:o("add-name"),vertical:[...J].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(l="/api/postings",p={url:Vt(t),title:o("add-title"),company:o("add-job-company")});let c;try{c=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)})}catch(f){u(`add failed: ${f.message}`),a();return}if(!c.ok){let f=`HTTP ${c.status}`;try{const L=await c.text();try{f=JSON.parse(L).error||f}catch{f=L.trim()||f}}catch{}if(a(),c.status===409){u(f||"That company is already in the list."),e.focus(),e.select();return}u(`add failed: ${f}`);return}const m=await c.json();if(a(),d&&!m.company_id){u(m.note||"couldn't classify that page");return}if(_e(),$(),H(),k(),z==="job"){const f=m.posting&&m.posting.title||"job link";u(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),G("jobs")}else d?(u(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`),de(m.company_id)):u("company added")}function Ut(e,t,n){Ie=t;const s=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="",s.classList.add("open"),Fe();const o=new EventSource(`/api/jobs/${t}/stream`),d=(l,p)=>{const c=document.createElement("div"),m=!p&&/^\s*warn:/i.test(l);c.className="ln"+(p?" ln-err":m?" ln-warn":""),c.textContent=m?l.replace(/^\s*warn:\s*/i,"⚠ "):l,a.appendChild(c),a.scrollTop=a.scrollHeight};o.addEventListener("line",l=>d(l.data,/error|failed/i.test(l.data))),o.addEventListener("end",l=>{o.close(),Ie=null,d(`— ${l.data} —`,l.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",u(`${e} ${l.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?nn(n.company_ids):$(),H(),Fe(),k(),i.openId&&de(i.openId)}),o.onerror=()=>{o.close()}}async function zn(){if(Ie)try{await fetch(`/api/jobs/${Ie}/cancel`,{method:"POST"})}catch{}}let C=null;const Fn={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check",judge:"judge"};function Ye(e){if(e==="outreach-template")return"outreach template";if(e==="taste-filter")return"pre-filter rules";if(e==="playbook")return"playbook";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(Fn[t]||t)+" prompt"}return e+".md"}async function X(e){C=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Ye(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=!!e&&e.startsWith("outreach-prompts/"),s=n?e.slice(17):"",a=e==="taste-filter"||n&&s!=="fill";document.getElementById("editor-toggle-row").style.display=a?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",a&&(document.getElementById("editor-toggle-label").textContent=e==="taste-filter"?"Enable the pre-filter (off → bulk verdict runs score every company; the rules below are kept either way)":"Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const l=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${l||"HTTP "+o.status}`;return}const d=await o.json();document.getElementById("editor-text").value=d.content||"",a&&(document.getElementById("editor-enabled").checked=d.enabled!==!1),d.taste_version&&(document.getElementById("editor-ver").textContent="version "+d.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Be(){document.getElementById("editor-scrim").classList.remove("open"),C=null}const Kn=[{key:"experience",hard:!0},{key:"voice",hard:!1}];async function Ge(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{Ze(await(await fetch("/api/outreach/sources")).json())}catch(e){u(`failed to load sources: ${e.message}`)}}function Qe(){document.getElementById("sources-scrim").classList.remove("open")}function Ze(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):Kn,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],d=o.length?o.map(l=>`<li><span class="src-title">${r(l.title||l.page_id)}</span><button class="src-rm" data-need="${r(a.key)}" data-id="${r(l.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${a.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${r(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${d}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(a=>a.addEventListener("click",()=>Yn(a.dataset.need,a.dataset.id)))}async function Wn(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){u(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const n=await t.json();n.warning?u(n.warning):u("sources refreshed"),Ze(n),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function Yn(e,t){let n;try{n=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(s){u(`remove failed: ${s.message}`);return}if(!n.ok){u(`remove failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}Ze(await n.json())}async function Gn(){if(!C)return;const t={content:document.getElementById("editor-text").value},n=C.startsWith("outreach-prompts/")&&C!=="outreach-prompts/fill";(C==="taste-filter"||n)&&(t.enabled=document.getElementById("editor-enabled").checked);let s;try{s=await fetch(`/api/${C}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){u(`save failed: ${o.message}`);return}if(!s.ok){u(`save failed: HTTP ${s.status}`);return}const a=await s.json();a.taste_version&&(document.getElementById("editor-ver").textContent="version "+a.taste_version),u(`${Ye(C)} saved`),Be(),H()}async function Qn(){if(!C||!C.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${C}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){u(`reset failed: ${n.message}`);return}if(!e.ok){u(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",u(`${Ye(C)} reset to default`)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;i.sort.k===t?i.sort.dir*=-1:(i.sort.k=t,i.sort.dir=1),q()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;i.jsort.k===t?i.jsort.dir*=-1:(i.jsort.k=t,i.jsort.dir=1),I()}}),document.getElementById("tab-companies").onclick=()=>G("companies"),document.getElementById("tab-jobs").onclick=()=>G("jobs"),document.getElementById("q").oninput=q,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;T.has(t)?T.delete(t):T.add(t),He(),q()})}),document.getElementById("flag-filter").addEventListener("click",e=>{Q=!Q,e.currentTarget.classList.toggle("is-on",Q),q()}),document.getElementById("jq").oninput=I,document.getElementById("hide-rejected").addEventListener("click",e=>{W=!W,e.currentTarget.classList.toggle("is-on",W),I()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;R.has(t)?R.delete(t):R.add(t),e.classList.toggle("is-on",R.has(t)),I()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{ve=!ve,e.currentTarget.classList.toggle("is-on",ve),I()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{ge=!ge,e.currentTarget.classList.toggle("is-on",ge),I()}),qe(),Z(),document.getElementById("pane-close").onclick=Ee,document.getElementById("scrim").onclick=Ee,document.getElementById("pursuit-close").onclick=ye,document.getElementById("pursuit-scrim").onclick=ye,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.getElementById("chat-pane").classList.contains("open")){dt();return}if(us()){it();return}if(document.getElementById("profile-scrim").classList.contains("open")){ot();return}if(document.getElementById("add-scrim").classList.contains("open")){_e();return}if(document.getElementById("run-scrim").classList.contains("open")){Le();return}if(document.getElementById("help-scrim").classList.contains("open")){Te();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(Oe==="pursuit"&&n){ye();return}if(Oe==="company"&&t){Ee();return}if(t){Ee();return}ye();return}if(document.getElementById("key-scrim").classList.contains("open")){at();return}if(document.getElementById("sources-scrim").classList.contains("open")){Qe();return}if(document.getElementById("editor-scrim").classList.contains("open")){Be();return}if(document.getElementById("settings-scrim").classList.contains("open")){rt();return}});let Xe=null;const Zn={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Jt(e){if(i.meta&&i.meta.control===!1){u("control surface disabled");return}Xe=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Zn[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Le(){document.getElementById("run-scrim").classList.remove("open"),Xe=null}document.getElementById("btn-enrich").onclick=()=>Jt("enrich"),document.getElementById("btn-verdict").onclick=()=>Jt("verdict"),document.getElementById("run-cancel").onclick=Le,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Le()},document.getElementById("run-go").onclick=()=>{const e=Xe,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(Le(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),Ke(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=Jn;const Xn={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function zt(e){const t=Xn[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const d=document.createElement("div");d.className="help-item-desc",d.textContent=s.desc;const l=document.createElement("a");l.className="help-link",l.textContent="Learn more →",l.onclick=()=>{Te(),Yt(),ps(s.sec)},a.appendChild(o),a.appendChild(d),a.appendChild(l),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Te(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>zt("add"),document.getElementById("help-run").onclick=()=>zt("run"),document.getElementById("help-close").onclick=Te,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Te()},document.getElementById("add-cancel").onclick=_e,document.getElementById("add-save").onclick=Dt,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&_e()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>qt(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Ot),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Dt()))}),document.getElementById("add-vertical-filter").addEventListener("input",Nt),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&Dn(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=zn,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=Be,document.getElementById("editor-save").onclick=Gn,document.getElementById("editor-reset").onclick=Qn,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Be()},document.getElementById("sources-close").onclick=Qe,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&Qe()},document.getElementById("sources-refresh-btn").onclick=Wn,document.getElementById("key-cancel").onclick=at,document.getElementById("key-save").onclick=Wt,document.getElementById("key-remove").onclick=cs,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&at()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),Wt())});function et(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function tt(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}V()}const ee='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',he='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',es='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',Ft='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',ts='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',ns='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',ss='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',as='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>',os='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>';function F(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${r(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function V(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,s=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let w="off",x="";const fe=t&&t.criteria_state;fe==="current"?(w="ok",x="current · verified "+et(t.verified_age_seconds)):fe==="changed"?(w="warn",x="changed — re-distill"):fe==="unverified"?(w="warn",x=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(w="warn",x="brain offline · using cache"):a&&(w="ok",x="fetched "+et(t.age_seconds)),o=F({icon:Ft,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:w,note:x,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:he,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=F({icon:Ft,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:ee,actTitle:"edit taste.md",actLabel:"edit taste"});const d=i.sources&&i.sources.sources||[],l=d.filter(w=>w.need==="experience").length,p=d.filter(w=>w.need==="voice").length;let c="off",m="not discovered yet — refresh from the brain";l>0?(c="ok",m=`${l} experience · ${p} voice`):d.length>0&&(c="warn",m="no experience yet — refresh");const f=d.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience + voice">outreach knowledge</span>':"outreach knowledge",L=F({icon:as,nameHTML:f,dot:c,note:m,desc:"Your experience + voice, discovered from the brain to ground outreach.",act:"refresh-sources",actID:"refresh-sources",actIcon:he,actTitle:"re-discover experience + voice from the brain",actLabel:"refresh outreach knowledge"}),j=F({icon:ts,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:ee,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),E=F({icon:ns,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:ee,actTitle:"edit the outreach email template",actLabel:"edit email template"}),N=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."],["judge","5 · Judge","Grades depth and proof; gates whether a draft is good enough to ship."]],te=N.map(([w,x,fe])=>F({icon:ss,nameHTML:`<span class="edit-link" data-act="edit-prompt-${w}" title="edit the ${x.replace(/^\d+ · /,"")} prompt">${x}</span>`,desc:fe,act:`edit-prompt-${w}`,actIcon:ee,actTitle:`edit the ${x} prompt`,actLabel:`edit ${x} prompt`})).join(""),me=!i.stats||i.stats.taste_filter_enabled!==!1,b=F({icon:os,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage. Toggle it off in the editor to score every company.",dot:me?"ok":"off",note:me?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:ee,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),K=i.anthropicKey;let lt="off",ut="not set — verdict, capture & outreach disabled";K&&K.key_source==="db"?(lt="ok",ut="set here · active"):K&&K.key_source==="env"&&(lt="ok",ut="from the environment");const bs=F({icon:es,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:lt,note:ut,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:ee,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${bs}
     </div>`;const pt={"view-profile":()=>ls(i.profile),"refresh-profile":ds,"edit-taste":()=>X("taste"),"edit-taste-filter":()=>X("taste-filter"),"edit-playbook":()=>X("playbook"),"edit-template":()=>X("outreach-template"),"view-sources":Ge,"refresh-sources":is,"edit-anthropic-key":rs};for(const[w]of N)pt[`edit-prompt-${w}`]=()=>X(`outreach-prompts/${w}`);e.querySelectorAll("[data-act]").forEach(w=>{const x=w.dataset.act;x&&pt[x]&&(w.onclick=pt[x])})}async function nt(){try{i.sources=await(await fetch("/api/outreach/sources")).json()}catch{i.sources=null}V()}async function is(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(s){u(`refresh failed: ${s.message}`),nt();return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();u(`refresh failed: ${s||"HTTP "+t.status}`),nt();return}const n=await t.json();n.warning?u(n.warning):u("outreach knowledge refreshed"),i.sources={sources:n.sources||[],needs:i.sources&&i.sources.needs||[]},V()}async function Kt(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}V()}async function rs(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await Kt(),st();const e=document.getElementById("key-input");e&&e.focus()}function st(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function at(){document.getElementById("key-scrim").classList.remove("open")}async function Wt(){const e=(document.getElementById("key-input").value||"").trim();if(!e){u("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){u(`save failed: ${a.message}`),n();return}if(!s.ok){u((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),u("Anthropic key saved"),await ze(),st(),V()}async function cs(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){u(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){u((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),u(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await ze(),st(),V()}async function ds(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){u(`refresh failed: ${n.message}`),tt();return}if(!t.ok){const n=await t.text().catch(()=>"");u(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),tt();return}i.profile=await t.json(),V(),u("company-fit brief refreshed"),H()}function ls(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${et(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function ot(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=ot,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&ot()};function Yt(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Ce(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function it(){document.getElementById("docs-scrim").classList.remove("open")}function us(){return document.getElementById("docs-scrim").classList.contains("open")}function Ce(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function ps(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ce(e)}document.getElementById("open-docs").onclick=Yt,document.getElementById("docs-close").onclick=it,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&it()};function hs(){document.getElementById("settings-scrim").classList.add("open"),V()}function rt(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=hs,document.getElementById("settings-close").onclick=rt,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&rt()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ce(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&Ce(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function ms(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function fs(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function je(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function ct(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function vs(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Gt(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=ms(n.content);if(n.role==="user")s&&t.appendChild(je("user",s));else if(n.role==="assistant"){const a=fs(n.content);if(!s&&!a.length)continue;const o=je("assistant",s);if(a.length){const d=document.createElement("div");d.className="chat-tools",d.textContent="· used "+a.join(", "),o.appendChild(d)}t.appendChild(o)}}t.children.length||t.appendChild(vs()),ct()}async function Se(e,t,n){if(!i.meta||!i.meta.chat){u("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),d=await fetch("/api/chat/threads?"+o);if(!d.ok)throw new Error((await d.text().catch(()=>"")).trim()||"HTTP "+d.status);const l=await d.json();i.chat.threadId=l.thread.id,Gt(l.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function dt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Me(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Qt(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Zt(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",Qt(),Me(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(je("user",t));const a=je("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),ct();let o="";const d=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Me(!1)},l=i.chat.threadId;let p;try{p=await fetch("/api/chat/"+l+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){d(m.message);return}if(!p.ok){d((await p.text().catch(()=>"")).trim()||"HTTP "+p.status);return}const c=new EventSource("/api/chat/"+l+"/stream");i.chat.es=c,c.addEventListener("delta",m=>{o+=m.data,a.textContent=o,ct()}),c.addEventListener("end",async m=>{c.close(),i.chat.es===c&&(i.chat.es=null),a.classList.remove("chat-streaming"),Me(!1),i.chat.threadId===l&&await gs(),ys(),typeof m.data=="string"&&m.data.indexOf("error")===0&&u("chat: "+m.data)}),c.onerror=()=>{c.close(),i.chat.es===c&&(i.chat.es=null),a.classList.remove("chat-streaming"),Me(!1)}}async function gs(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Gt(a.messages||[])}catch{}}function ys(){$(),k(),H(),i.openId&&de(i.openId)}document.getElementById("open-chat").onclick=()=>Se("global","",""),document.getElementById("chat-close").onclick=dt,document.getElementById("chat-scrim").onclick=dt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Zt()}),document.getElementById("chat-input").addEventListener("input",Qt),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Zt())}),bt("#t tbody",en),bt("#jt tbody",tn),$(),k(),H(),ze(),Fe(),tt(),nt(),Kt()}Es({"":{view:()=>({mount(y){y.innerHTML=Bs,Ls()}}),chrome:!1}},{title:"scout"});xs();_s();
