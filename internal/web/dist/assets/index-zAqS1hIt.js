(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))$(o);new MutationObserver(o=>{for(const f of o)if(f.type==="childList")for(const E of f.addedNodes)E.tagName==="LINK"&&E.rel==="modulepreload"&&$(E)}).observe(document,{childList:!0,subtree:!0});function g(o){const f={};return o.integrity&&(f.integrity=o.integrity),o.referrerPolicy&&(f.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?f.credentials="include":o.crossOrigin==="anonymous"?f.credentials="omit":f.credentials="same-origin",f}function $(o){if(o.ep)return;o.ep=!0;const f=g(o);fetch(o.href,f)}})();function Jt(y,r){const g=r.replace(/^#/,"");let $=null;for(const o of Object.keys(y))(g===o||g.startsWith(o))&&($===null||o.length>$.length)&&($=o);return $===null&&""in y&&($=""),$}function Ft(y){return typeof y=="function"?{view:y,chrome:!0}:{view:y.view,chrome:y.chrome!==!1}}function zt(y,r={}){const g=r.root??document.body,$=r.title??document.title??"",o=r.brandHref??"#",f=document.createElement("main"),E=document.createElement("header");E.className="cap-head";const k=document.createElement("a");k.className="brand",k.href=o,k.textContent=$,k.setAttribute("aria-label",`${$} — home`),E.appendChild(k);const C=document.createElement("nav");C.className="cap-nav",C.setAttribute("aria-label","Views");for(const w of r.nav??[]){const B=document.createElement("a");B.href=w.href,B.textContent=w.label,w.ariaLabel&&B.setAttribute("aria-label",w.ariaLabel),C.appendChild(B)}E.appendChild(C);const I=document.createElement("section");I.className="tk-content",f.appendChild(E),f.appendChild(I);const j=document.createElement("div");j.className="tk-bleed";const ve=w=>{var B;for(const M of Array.from(C.querySelectorAll("a"))){const R=((B=M.getAttribute("href"))==null?void 0:B.replace(/^#/,""))??"";M.toggleAttribute("aria-current",w!==null&&w!==""&&R===w),M.hasAttribute("aria-current")&&M.setAttribute("aria-current","page")}};let Q=0;const S=()=>{const w=Jt(y,location.hash);if(ve(w),w===null){j.isConnected&&j.remove(),f.isConnected||g.appendChild(f),Wt(I,"Not found.");return}const{view:B,chrome:M}=Ft(y[w]),R=M?I:j;M?(j.isConnected&&j.remove(),f.isConnected||g.appendChild(f)):(f.isConnected&&f.remove(),j.isConnected||g.appendChild(j)),R.replaceChildren();const ge=B(),X=++Q,D=ge.mount(R);D instanceof Promise&&D.catch(Z=>{X===Q&&Yt(R,String(Z))})};window.addEventListener("hashchange",S),S()}function Wt(y,r){y.replaceChildren();const g=document.createElement("div");g.className="tk-empty",g.textContent=r,y.appendChild(g)}function Yt(y,r){y.replaceChildren();const g=document.createElement("div");g.className="tk-error",g.textContent=r,y.appendChild(g)}function Kt(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const g of r)g.unregister()}),window.caches&&caches.keys().then(r=>{for(const g of r)caches.delete(g)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Gt(){let y;try{y=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!y.ok)return null;let r;try{r=await y.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Qt=`
<div class="layout">
<aside class="sidebar">
...
`;function Xt(y){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},g=e=>"pill pill-"+(e||"none"),$='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',o=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),f=e=>/^https?:\/\//i.test(String(e??""))?o(e):"#";async function E(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],H()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],_(),h.postingId){const t=r.jobs.find(n=>n.posting_id===h.postingId);t&&(h.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&z())}}function C(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",ye(),e==="jobs"?_():H()}async function I(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,j()}function j(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Te()}function ve(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[s]??3)-(i[a]??3)}return String(s).localeCompare(String(a))}function Q(e){return e.slice().sort((t,n)=>r.sort.dir*ve(t,n,r.sort.k))}const S=new Set;let w=!1;function B(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",S.has(e.dataset.v))})}function M(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(S.size&&!S.has(t.verdict||"__none__")||w&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const R=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],ge=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function X(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const D=X("scout-hidden-cols"),Z=X("scout-hidden-jcols");function Me(){return r.view==="jobs"?{cols:ge,hidden:Z,key:"scout-hidden-jcols"}:{cols:R,hidden:D,key:"scout-hidden-cols"}}function ee(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=D.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Z.has(e.dataset.col)?"none":""})}function ye(){const e=Me();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=Me(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),ye(),ee()})})}function H(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=Q(M());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
      ...
      <td data-col="verdict"><span class="${g(n.verdict)}">${o(n.verdict||"—")}</span></td>
    ...
    `,e.appendChild(s)}ee(),e.querySelectorAll("tr").forEach(n=>{n.addEventListener("click",s=>{s.target.closest("a, .flag-btn")||oe(n.dataset.id)})}),e.querySelectorAll(".flag-btn").forEach(n=>{n.addEventListener("click",()=>Je(n.dataset.id))})}const A=new Set;let te=!1,ne=!1,V=!0;function ct(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=V&&!A.has("rejected");return r.jobs.filter(n=>!(t&&n.response==="rejected"||A.size&&!A.has(n.response||"")||te&&!n.next_up||ne&&(n.outreach_count|0)>0||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const dt=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function Ae(e){const t=String(e||"").split(",").map(n=>n.trim()).filter(Boolean);return t.length?t.map(n=>{const s=n.match(dt);return s?`<a href="mailto:${o(s[0])}" title="${o(n)}">${o(n)}</a>`:o(n)}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}const P={offer:{cls:"pill-yes",label:"offer",order:0},interview:{cls:"pill-info",label:"interview",order:1},screening:{cls:"pill-maybe",label:"screening call",order:2},"":{cls:"pill-none",label:"—",order:3},rejected:{cls:"pill-no",label:"rejected",order:4}};function lt(e,t,n){var s,a;if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[e.verdict]??3)-(i[t.verdict]??3)}if(n==="response")return(((s=P[e.response])==null?void 0:s.order)??3)-(((a=P[t.response])==null?void 0:a.order)??3);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="applied_at"||n==="last_outreach_at"){const i=e[n]||"",d=t[n]||"";return!i&&!d?0:i?d?String(d).localeCompare(String(i)):-r.jsort.dir:r.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function J(e){const t=e.options[e.selectedIndex],n=getComputedStyle(e),s=J._c||(J._c=document.createElement("canvas").getContext("2d"));s.font=`${n.fontWeight} ${n.fontSize} ${n.fontFamily}`;const a=s.measureText(t?t.text:"").width;e.style.width=Math.ceil(a+35)+"px"}function _(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=ct().sort((c,v)=>r.jsort.dir*lt(c,v,r.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=V&&!A.has("rejected")?r.jobs.filter(c=>c.response==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=r.jobs.filter(c=>c.next_up).length,i=document.getElementById("next-up-n");i.textContent=a,i.style.display=a?"":"none";const d=r.jobs.filter(c=>!(c.outreach_count|0)&&!(V&&!A.has("rejected")&&c.response==="rejected")).length,p=document.getElementById("not-reached-n");p.textContent=d,p.style.display=d?"":"none";const u=document.getElementById("jobs-hidden-note");u.style.display=n?"":"none",n&&(u.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{V=!1,document.getElementById("hide-rejected").classList.remove("is-on"),_()});for(const c of t){const v=P[c.response]||P[""],m=document.createElement("tr");m.dataset.id=c.posting_id;const x=[["","none"],["screening","screening"],["interview","interview"],["offer","offer"],["rejected","rejected"]].map(([T,fe])=>`<option value="${T}"${(c.response||"")===T?" selected":""}>${fe}</option>`).join("");m.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${c.next_up?" is-on":""}" title="${c.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${c.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${o(c.title||c.company)}</span>${ut(c.outreach_draft_status)}${c.title?`<div class="small dim">${o(c.company)}</div>`:""}</div></div></td>
      <td class="small" data-col="applied"><button class="jt-applied${c.applied_at?" is-on":""}" title="${c.applied_at?"mark as not applied":"mark applied today"}">${c.applied_at?o(c.applied_at):"+ applied"}</button></td>
      <td data-col="response"><select class="jt-resp ${v.cls}" title="furthest response reached">${x}</select></td>
      ...
      <td class="small" data-col="last_outreach">${c.last_outreach_at?o(c.last_outreach_at):'<span class="dim">—</span>'}</td>
      <td class="small td-contacts" data-col="contacts">${Ae(c.contacts)}</td>
      <td data-col="link"><a href="${f(c.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `;const L=()=>new Date().toISOString().slice(0,10);m.querySelector(".jt-nextup").onclick=()=>Ne(c,!1),m.querySelector(".jt-applied").onclick=()=>se(c,{applied_at:c.applied_at?"":L()}),m.querySelector(".jt-resp").onchange=T=>{J(T.target),se(c,{response:T.target.value})},m.querySelector(".jt-inc").onclick=()=>se(c,{outreach_count:(c.outreach_count||0)+1,last_outreach_at:L()}),m.querySelector(".jt-dec").onclick=()=>{const T=Math.max(0,(c.outreach_count||0)-1);se(c,{outreach_count:T,...T===0?{last_outreach_at:""}:{}})},e.appendChild(m)}e.querySelectorAll(".jt-resp").forEach(J),ee(),e.querySelectorAll("tr").forEach(c=>{c.addEventListener("click",v=>{v.target.closest("a, button, select")||He(c.dataset.id)})})}function ut(e){return e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1};async function He(e){let t=r.jobs.find(n=>n.posting_id===e);if(t||(await k(),t=r.jobs.find(n=>n.posting_id===e)),!t){l("posting not found — refresh");return}we(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),z(),F()}function be(){we(),h.postingId=null,h.row=null,h.drafts=[],document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function we(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function F(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){W();return}e=await n.json()}catch{W();return}h.drafts=e.drafts||[],W();const t=h.drafts[0];t&&t.status==="researching"?pt():we()}function pt(){h.poll||(h.poll=setInterval(F,4e3))}function q(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(i){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),l(`save failed: ${i.message}`)}})}async function Pe(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const i=await a.json();Object.assign(e,{title:i.title,location:i.location,summary:i.summary,employment_type:i.employment_type,workplace_type:i.workplace_type,department:i.department,comp_range:i.comp_range,description:i.description}),_()}async function qe(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const i=await a.json();Object.assign(e,{name:i.name,headcount:i.headcount,funding_stage:i.funding_stage,location:i.location,vertical:i.vertical}),E(),k()}async function ht(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();r.openId=s.company_id,Y(s),K(s.company_id),E(),k()}async function mt(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}function z(){const e=h.row;if(!e)return;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${o(e.title||"")}">`;const t=P[e.response]||P[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${t.cls}">${o(t.label)}</span>`+(e.verdict?` <span class="${g(e.verdict)}">${o(e.verdict)}</span>`:""),document.getElementById("pursuit-body").innerHTML=`
    ...
    <section class="pane-section">
      ...
      <textarea class="ie ie-notes" id="pursuit-notes-input" rows="4" placeholder="Your notes on this role — ⌘/Ctrl+Enter or click away to save">${o(e.notes||"")}</textarea>
    </section>
  ...
  `,vt();const n=document.getElementById("pursuit-company-link");n&&n.addEventListener("click",()=>oe(e.company_id)),q(document.getElementById("pursuit-title-input"),s=>Pe(e,"title",s)),q(document.getElementById("pursuit-notes-input"),s=>gt(s),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(s=>q(s,a=>Pe(e,s.dataset.k,a),{multiline:s.tagName==="TEXTAREA"})),W()}function ft(e){return`
    <div class="role-url"><a href="${f(e.url)}" target="_blank" rel="noopener">${o(e.url)} ↗</a></div>
    <div class="ie-grid">
      <div class="prow">
    ...
    <div class="role-meta">
      ...
      <button type="button" class="role-company role-company-link" id="pursuit-company-link"
              title="open the company panel">${o(e.company)} ↗</button>
    </div>`}function vt(){const e=h.row,t=()=>new Date().toISOString().slice(0,10),n=document.querySelector("#pursuit-body .pt-applied");n&&n.addEventListener("click",()=>U({applied_at:e.applied_at?"":t()}));const s=document.querySelector("#pursuit-body .pl-applied-date");s&&s.addEventListener("change",d=>U({applied_at:d.target.value}));const a=document.querySelector("#pursuit-body .pl-response");a&&a.addEventListener("change",d=>U({response:d.target.value}));const i=document.querySelector("#pursuit-body .pt-nextup");i&&i.addEventListener("click",()=>Ne(h.row,!0))}async function Ne(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){l(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();l(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,_(),t&&z(),l(e.next_up?"queued next up":"removed from the queue")}async function Oe(e,t){const n={applied_at:e.applied_at||"",response:e.response||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",contacts:e.contacts||"",notes:e.notes||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(i){return l(`save failed: ${i.message}`),null}if(!s.ok){const i=(await s.text().catch(()=>"")).trim();return l(`save failed: ${i||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{applied_at:a.applied_at,response:a.response,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),a}async function gt(e){const t=h.row,n={applied_at:t.applied_at||"",response:t.response||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",contacts:t.contacts||"",notes:e},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,_()}async function U(e){await Oe(h.row,e)&&(_(),z(),l("tracking saved"))}async function se(e,t){await Oe(e,t)&&(_(),h.postingId===e.posting_id&&(h.row=e,z()),l("tracking saved"))}function W(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.row,n=h.drafts,s=n[0]||null,a=n.slice(1),i=`
    ...
    <div class="outreach-contacts">
      ...
      <div class="oc-rendered">${Ae(t.contacts)}</div>
    ...
    <details class="draft-history" ${h.openHist?"open":""}>
      ...
      <div id="draft-history-body">${a.map(c=>Re(c,!0)).join("")}</div>
    </details>`:"";e.innerHTML=i+`<div id="draft-current">${s?Re(s,!1):""}</div><div class="draft-actions">${p}</div>`+u,wt()}function yt(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function Re(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(v,m)=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const v=bt(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      <div class="draft-sentbody">${o(Ve(e)||"(empty)")}</div>
      ${ae(e)}
    </div>`;const a=Ve(e),i=e.status==="no_hook",d=i?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let p="";if(i)try{p=JSON.parse(e.hook||"{}").reasoning||""}catch{}const u=i?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${p?" "+o(p):""}</div>`:"";if(t)return`<div class="draft-card ${i?"dc-nohook":"dc-review"}" data-did="${e.id}">
      <div class="draft-head">${d}<span class="dh-time">${o(n)}</span></div>
      ${u}
      <div class="draft-sentbody">${o(a||"(empty)")}</div>
      ${ae(e)}
    </div>`;const c=a||i;return`<div class="draft-card ${i?"dc-nohook":"dc-review"}" data-did="${e.id}">
    <div class="draft-head">${d}<span class="dh-time">${o(n)}</span></div>
    ${u}
    ${c?`<textarea class="draft-textarea" id="draft-edit-${e.id}" spellcheck="false">${o(a)}</textarea>
    ${Ue(e.lint)}
    ...
    ${ae(e)}
  </div>`}function Ve(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function ae(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,c)=>c?`<div class="tr-line"><span class="tr-key">${u}:</span> ${o(String(c))}</div>`:"",i=n.role||{},d=Array.isArray(n.hooks)?n.hooks:[],p=d.map(u=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function Ue(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${o(n.message||"")}"><code>${o(n.code||"")}</code>${o(n.message||"")}</span>`).join("")+"</div>":""}function bt(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${o(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${o(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function wt(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector(".oc-input");t&&(t.addEventListener("change",u=>U({contacts:u.target.value.trim()})),t.addEventListener("keydown",u=>{u.key==="Enter"&&(u.preventDefault(),u.target.blur())}));const n=()=>new Date().toISOString().slice(0,10),s=h.row,a=e.querySelector(".pt-outreach");a&&a.addEventListener("click",()=>U({outreach_count:(s.outreach_count||0)+1,last_outreach_at:n()}));const i=e.querySelector(".pt-outreach-dec");i&&i.addEventListener("click",()=>{const u=Math.max(0,(s.outreach_count||0)-1);U({outreach_count:u,...u===0?{last_outreach_at:""}:{}})});const d=e.querySelector("#draft-start-btn");d&&d.addEventListener("click",De),e.querySelectorAll(".draft-retry-btn").forEach(u=>u.addEventListener("click",De)),e.querySelectorAll(".draft-card[data-did]").forEach(u=>{const c=u.dataset.did,v=u.querySelector(".draft-save-btn");v&&v.addEventListener("click",()=>$t(c));const m=u.querySelector(".draft-sent-btn");m&&m.addEventListener("click",()=>_t(c))});const p=e.querySelector("details.draft-history");p&&p.addEventListener("toggle",()=>{h.openHist=p.open})}async function De(){const e=document.getElementById("outreach-section"),t=e&&(e.querySelector("#draft-start-btn")||e.querySelector(".draft-retry-btn"));t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/outreach`,{method:"POST"})}catch(a){l(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await F();return}if(n.status===409){await F(),l("a draft is already active");return}if(n.status===412){let a={};try{a=await n.json()}catch{}kt(a.missing_blocks||[]),t&&(t.disabled=!1);return}if(n.status===503){const a=document.getElementById("outreach-section");if(a){const i=document.createElement("div");i.className="draft-note",i.textContent="Outreach engine not running in this build.",a.appendChild(i)}t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();l(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}function kt(e){const t=document.getElementById("outreach-section");if(!t)return;const n=t.querySelector(".draft-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    <button class="btn btn-primary" id="blocks-sync-btn">Sync blocks</button>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#blocks-sync-btn");a&&a.addEventListener("click",Et)}async function Et(){const e=document.getElementById("blocks-sync-btn");e&&(e.disabled=!0,e.textContent="Syncing…");let t;try{t=await fetch("/api/outreach/sync",{method:"POST"})}catch(n){l(`sync failed: ${n.message}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`sync failed: ${n||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}l("blocks synced"),W()}async function $t(e){const t=document.getElementById(`draft-edit-${e}`);if(!t)return;let n;try{n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t.value})})}catch(d){l(`save failed: ${d.message}`);return}if(!n.ok){const d=(await n.text().catch(()=>"")).trim();l(`save failed: ${d||"HTTP "+n.status}`);return}const s=await n.json(),a=h.drafts.findIndex(d=>String(d.id)===String(e));a>=0&&(h.drafts[a]=s);const i=t.closest(".draft-card");if(i){const d=i.querySelector(".lint-chips"),p=Ue(s.lint);d?d.outerHTML=p||"":p&&t.insertAdjacentHTML("afterend",p)}l("saved")}async function _t(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(n){l(`failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`failed: ${n||"HTTP "+t.status}`);return}l("marked sent"),await F(),k()}async function oe(e){r.openId=e;const t=document.getElementById("pane"),n=document.getElementById("scrim");t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';let s;try{const a=await fetch(`/api/companies/${e}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);s=await a.json()}catch(a){document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${o(a.message)}</div>`;return}Y(s),K(e)}function ie(){r.openId=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function Y(e){document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${o(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
    <span class="${g(e.has_verdict?e.verdict:"")}">${o(e.has_verdict?e.verdict:"unscored")}</span>
  `;const t=e.model==="manual",n=e.has_verdict?`
    <dl class="kv">
      <dt>verdict</dt><dd><span class="${g(e.verdict)}">${o(e.verdict)}</span>${t?' <span class="small muted">· set by hand</span>':""}</dd>
  ...
  `:'<div class="muted">Not yet scored. Run <code>scout verdict</code>, or set one by hand below.</div>',s=`
    ...
    <dl class="kv">
      <dt>url</dt><dd>${e.website_url?`<a href="${f(e.website_url)}" target="_blank" rel="noopener">${o(e.website_url)} ↗</a>`:'<span class="muted">—</span>'}</dd>
  ...
  `:'<div class="muted">No enrichment yet. Run <code>scout enrich</code>.</div>',i=!r.meta||r.meta.control!==!1,d=i&&r.meta&&r.meta.verdict?'<button class="h3-action" id="rescore-btn" title="re-score just this company — replaces the current verdict, manual or not">↻ re-score</button>':"",p=i&&e.domain?'<button class="h3-action" id="reenrich-btn" title="re-fetch this company’s site now">↻ re-enrich</button>':"",u=Object.keys(e.raw_json||{}).sort(),c=u.length===0?"":`
    <details class="raw-json">
      <summary>Raw row <span class="dim">(${u.length} fields)</span></summary>
      <table><tbody>
        ${u.map(b=>`<tr><td class="k">${o(b)}</td><td>${o(e.raw_json[b])}</td></tr>`).join("")}
      </tbody></table>
    </details>
  `,v=`
    <div class="flag-bar">
    ...
    <section class="pane-section">
      ...
      <textarea class="ie ie-notes" id="pane-notes-input" rows="4" placeholder="Your notes on this company — ⌘/Ctrl+Enter or click away to save">${o(e.notes||"")}</textarea>
    </section>
  ...
  `;const m=document.getElementById("posting-add-btn");m&&m.addEventListener("click",()=>Lt(e)),ze(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Se=>Se.classList.remove("is-on")),b.classList.add("is-on")})});const x=document.getElementById("ve-save-btn");x&&x.addEventListener("click",()=>Bt(e)),q(document.getElementById("pane-title-input"),b=>qe(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>q(b,Se=>qe(e,b.dataset.k,Se))),q(document.getElementById("pane-domain-input"),b=>ht(e,b)),q(document.getElementById("pane-notes-input"),b=>mt(e,b),{multiline:!0});const L=document.getElementById("flag-toggle-btn");L&&L.addEventListener("click",()=>Je(e.company_id));const T=document.getElementById("review-stamp-btn");T&&T.addEventListener("click",()=>It(e.company_id));const fe=document.getElementById("rescore-btn");fe&&fe.addEventListener("click",()=>Ee("verdict",{company_ids:[e.company_id]}));const rt=document.getElementById("reenrich-btn");rt&&rt.addEventListener("click",()=>Ee("enrich",{company_ids:[e.company_id]}))}function xt(e){return`
    <div class="ie-grid">
      <div class="ie-field"><label>website${e.domain?` · <a href="https://${o(e.domain)}" target="_blank" rel="noopener">open ↗</a>`:""}</label>
        <input class="ie" id="pane-domain-input" placeholder="acme.com" value="${o(e.domain||"")}"></div>
    ...
    </dl>`}async function It(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(i){l(`failed: ${i.message}`),t&&(t.disabled=!1);return}if(!n.ok){const i=await n.text().catch(()=>"");l(`failed: HTTP ${n.status}${i?" — "+i:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=r.rows.find(i=>i.company_id===e);a&&(a.reviewed_at=s.reviewed_at,H()),r.openId===e&&(Y(s),K(e)),l("reviewed")}async function Je(e){const t=r.rows.find(i=>i.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(i){l(`failed: ${i.message}`);return}if(!s.ok){const i=await s.text().catch(()=>"");l(`failed: HTTP ${s.status}${i?" — "+i:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,H()),r.openId===e&&(Y(a),K(e)),k(),l(a.flagged?"flagged":"unflagged")}async function Bt(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){l("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(p){l(`save failed: ${p.message}`),a.disabled=!1;return}if(!i.ok){const p=await i.text().catch(()=>"");l(`save failed: HTTP ${i.status}${p?" — "+p:""}`),a.disabled=!1;return}const d=await i.json();Y(d),K(d.company_id),E(),I(),k(),l("verdict saved")}function Fe(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(o).join(" · "),a=P[n.response]||P[""],i=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a.cls}">${o(a.label)}</span>`,`<span class="pt-meta">${n.applied_at?`applied ${o(n.applied_at)}`:"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${o(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    <div class="brain-node posting-card" data-pid="${o(n.id)}" title="open the pursuit — tracking, outreach, drafts">
      <div class="n"><a href="${f(n.url)}" target="_blank" rel="noopener">${o(n.title||n.url)} ↗</a></div>
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ze(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||(ie(),He(e.dataset.pid))})})}async function Lt(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!i.ok){const u=await i.text().catch(()=>"");l(`add failed: HTTP ${i.status}${u?" — "+u:""}`),s.disabled=!1;return}const d=await i.json();e.postings=(e.postings||[]).filter(u=>u.id!==d.id),e.postings.unshift(d);const p=document.getElementById("postings-list");p&&(p.innerHTML=Fe(e),ze()),t.value="",n.value="",s.disabled=!1,k(),l("link added")}async function K(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){re(`<div class="muted">Failed to load trail: ${o(s.message)}</div>`);return}if(!t.ok){re(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){re('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}re(n.map(Tt).join(""))}function Tt(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(o);return e.run_id&&t.push("run "+o(e.run_id.slice(0,8))),`
    <div class="trail-event">
      <div class="trail-head">
        <span class="${g(e.verdict)}">${o(e.verdict)}</span>
    ...
    </div>`}function re(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let We;function l(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(We),We=setTimeout(()=>t.classList.remove("show"),2200)}r.meta={control:!1,brain:!1,verdict:!1};async function jt(){try{const n=await fetch("/api/meta");if(!n.ok)return;r.meta=await n.json()}catch{return}const e=r.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!r.meta.verdict,t.title=r.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable"}async function ke(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let ce=null;async function Ee(e,t){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){l(`run failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(n.status===412){const a=await n.text();l(a.trim());return}if(!n.ok){l(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();et(e,s)}async function Ct(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){l(`upload failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(!n.ok){l(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();et("ingest",s)}const St=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let de=[],N=new Set,O="company";function Ye(e){O=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",Ke()}function $e(){return!!r.meta.capture&&document.getElementById("add-enrich").checked}function Ke(){const e=document.getElementById("add-note");$e()?e.innerHTML=O==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=O==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function Mt(){St.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",N=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!r.meta.capture,t.classList.toggle("disabled",!r.meta.capture),t.title=r.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",r.meta.capture||(e.checked=!1),Ye(r.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(r.rows||[]).map(s=>`<option value="${o(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const i=document.createElement("option");i.value=a,i.textContent=a,n.appendChild(i)}),de=s.verticals||[]}catch{de=[]}Ge()}function le(){document.getElementById("add-scrim").classList.remove("open")}function Ge(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=de.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${N.has(s)?" sel":""}" data-v="${o(s)}">${o(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;N.has(a)?N.delete(a):N.add(a),s.classList.toggle("sel"),Qe()}))):e.innerHTML=`<div class="none">${de.length?"no match":"no verticals in the set yet"}</div>`,Qe()}function Qe(){const e=N.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Xe(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Ze(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){l(O==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,$e()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},i=m=>document.getElementById(m).value.trim(),d=$e();let p,u;d?(p="/api/capture",u={url:Xe(t),kind:O==="company"?"company_page":"job_posting",fields:O==="company"?{name:i("add-name"),location:i("add-location"),headcount:i("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...N].join(", ")}:{name:i("add-job-company"),title:i("add-title")}}):O==="company"?(p="/api/companies",u={website:t,name:i("add-name"),vertical:[...N].join(", "),location:i("add-location"),headcount:i("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(p="/api/postings",u={url:Xe(t),title:i("add-title"),company:i("add-job-company")});let c;try{c=await fetch(p,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(m){l(`add failed: ${m.message}`),a();return}if(!c.ok){let m=`HTTP ${c.status}`;try{const x=await c.text();try{m=JSON.parse(x).error||m}catch{m=x.trim()||m}}catch{}if(a(),c.status===409){l(m||"That company is already in the list."),e.focus(),e.select();return}l(`add failed: ${m}`);return}const v=await c.json();if(a(),d&&!v.company_id){l(v.note||"couldn't classify that page");return}if(le(),E(),I(),k(),O==="job"){const m=v.posting&&v.posting.title||"job link";l(`tracking: ${m} @ ${v.company_name}${v.posting_updated?" (refreshed)":""}`),C("jobs")}else d?(l(v.company_created?`company added: ${v.company_name}`:`${v.company_name} is already in the list`),oe(v.company_id)):l("company added")}function et(e,t){ce=t;const n=document.getElementById("drawer"),s=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",s.innerHTML="",n.classList.add("open"),ke();const a=new EventSource(`/api/jobs/${t}/stream`),i=(d,p)=>{const u=document.createElement("div"),c=!p&&/^\s*warn:/i.test(d);u.className="ln"+(p?" ln-err":c?" ln-warn":""),u.textContent=c?d.replace(/^\s*warn:\s*/i,"⚠ "):d,s.appendChild(u),s.scrollTop=s.scrollHeight};a.addEventListener("line",d=>i(d.data,/error|failed/i.test(d.data))),a.addEventListener("end",d=>{a.close(),ce=null,i(`— ${d.data} —`,d.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",l(`${e} ${d.data}`),E(),I(),ke(),k(),r.openId&&oe(r.openId)}),a.onerror=()=>{a.close()}}async function At(){if(ce)try{await fetch(`/api/jobs/${ce}/cancel`,{method:"POST"})}catch{}}let G=null;async function tt(e){G=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+e+".md",document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="",t.classList.add("open");try{const s=await(await fetch(`/api/${e}`)).json();document.getElementById("editor-text").value=s.content||"",s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version)}catch(n){document.getElementById("editor-text").value="failed to load: "+n.message}}function ue(){document.getElementById("editor-scrim").classList.remove("open"),G=null}const _e=["subject_name","signature","lens","hook_prefs","arc"];let nt=!1;async function Ht(){document.getElementById("sender-scrim").classList.add("open");let t={};try{t=await(await fetch("/api/outreach/sender")).json()}catch(n){l(`failed to load identity: ${n.message}`)}for(const n of _e){const s=document.getElementById("snd-"+n);s&&(s.value=t[n]||"")}if(!nt){for(const n of _e){const s=document.getElementById("snd-"+n);q(s,a=>Pt(n,a),{multiline:s&&s.tagName==="TEXTAREA"})}nt=!0}}async function Pt(e,t){const n={};for(const a of _e){const i=document.getElementById("snd-"+a);n[a]=i?i.value:""}n[e]=t;const s=await fetch("/api/outreach/sender",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status)}function xe(){document.getElementById("sender-scrim").classList.remove("open")}async function qt(){if(!G)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${G}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(s){l(`save failed: ${s.message}`);return}if(!t.ok){l(`save failed: HTTP ${t.status}`);return}const n=await t.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version),l(`${G}.md saved`),ue(),I()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),H()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),_()}}),document.getElementById("tab-companies").onclick=()=>C("companies"),document.getElementById("tab-jobs").onclick=()=>C("jobs"),document.getElementById("q").oninput=H,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;S.has(t)?S.delete(t):S.add(t),B(),H()})}),document.getElementById("flag-filter").addEventListener("click",e=>{w=!w,e.currentTarget.classList.toggle("is-on",w),H()}),document.getElementById("jq").oninput=_,document.getElementById("hide-rejected").addEventListener("click",e=>{V=!V,e.currentTarget.classList.toggle("is-on",V),_()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;A.has(t)?A.delete(t):A.add(t),e.classList.toggle("is-on",A.has(t)),_()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{te=!te,e.currentTarget.classList.toggle("is-on",te),_()}),document.getElementById("not-reached-filter").addEventListener("click",e=>{ne=!ne,e.currentTarget.classList.toggle("is-on",ne),_()}),ye(),ee(),document.getElementById("pane-close").onclick=ie,document.getElementById("scrim").onclick=ie,document.getElementById("pursuit-close").onclick=be,document.getElementById("pursuit-scrim").onclick=be,document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(Ut()){Ce();return}if(document.getElementById("profile-scrim").classList.contains("open")){je();return}if(document.getElementById("add-scrim").classList.contains("open")){le();return}if(document.getElementById("run-scrim").classList.contains("open")){pe();return}if(document.getElementById("help-scrim").classList.contains("open")){he();return}if(document.getElementById("pane").classList.contains("open")){ie();return}if(document.getElementById("pursuit-pane").classList.contains("open")){be();return}if(document.getElementById("sender-scrim").classList.contains("open")){xe();return}document.getElementById("editor-scrim").classList.contains("open")&&ue()}});let Ie=null;const Nt={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function st(e){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}Ie=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Nt[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=r.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function pe(){document.getElementById("run-scrim").classList.remove("open"),Ie=null}document.getElementById("btn-enrich").onclick=()=>st("enrich"),document.getElementById("btn-verdict").onclick=()=>st("verdict"),document.getElementById("run-cancel").onclick=pe,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&pe()},document.getElementById("run-go").onclick=()=>{const e=Ie,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(pe(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),Ee(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=Mt;const Ot={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function at(e){const t=Ot[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const i=document.createElement("div");i.className="help-item-name",i.textContent=s.name;const d=document.createElement("div");d.className="help-item-desc",d.textContent=s.desc;const p=document.createElement("a");p.className="help-link",p.textContent="Learn more →",p.onclick=()=>{he(),it(),Dt(s.sec)},a.appendChild(i),a.appendChild(d),a.appendChild(p),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function he(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>at("add"),document.getElementById("help-run").onclick=()=>at("run"),document.getElementById("help-close").onclick=he,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&he()},document.getElementById("add-cancel").onclick=le,document.getElementById("add-save").onclick=Ze,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&le()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Ye(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Ke),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Ze()))}),document.getElementById("add-vertical-filter").addEventListener("input",Ge),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&Ct(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=At,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=ue,document.getElementById("editor-save").onclick=qt,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&ue()},document.getElementById("sender-close").onclick=xe,document.getElementById("sender-scrim").onclick=e=>{e.target.id==="sender-scrim"&&xe()};function ot(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function Be(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}Te()}const Le='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>';function Te(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,n=r.stats&&r.stats.stale_verdicts||0,a=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),i=t&&typeof t.body=="string";let d="";if(a){let x="off",L="";t&&!t.reachable&&i?(x="warn",L="brain offline · using cache"):t&&t.stale?(x="warn",L="cached · stale"):i&&(x="ok",L="fetched "+ot(t.age_seconds)),d+=`<div class="crit-row">
      <span class="crit-what"><span class="pf-dot ${x}"></span>brain profile</span>
      ...
      </span></div>`,L&&(d+=`<div class="crit-note dim small">${o(L)}</div>`)}else d+=`<div class="crit-row">
      <span class="crit-what">taste</span>
      <button class="crit-edit" id="edit-taste" title="edit taste.md" aria-label="edit taste">${Le}</button></div>`,t&&t.configured&&(d+='<div class="crit-note dim small">brain offline — local fallback</div>');d+=`<div class="crit-row">
    <span class="crit-what">playbook</span>
    <button class="crit-edit" id="edit-playbook" title="edit playbook.md" aria-label="edit playbook">${Le}</button></div>`,d+=`<div class="crit-row">
    <span class="crit-what">outreach identity</span>
    <button class="crit-edit" id="edit-sender" title="edit outreach identity" aria-label="edit outreach identity">${Le}</button></div>`,n>0&&(d+=`<div class="stale">
      ...
      ${n} verdicts stale</div>`),e.innerHTML=d;const p=document.getElementById("view-profile");p&&(p.onclick=()=>Vt(r.profile));const u=document.getElementById("refresh-profile");u&&(u.onclick=Rt);const c=document.getElementById("edit-taste");c&&(c.onclick=()=>tt("taste"));const v=document.getElementById("edit-playbook");v&&(v.onclick=()=>tt("playbook"));const m=document.getElementById("edit-sender");m&&(m.onclick=Ht)}async function Rt(){const e=document.getElementById("refresh-profile");e&&(e.textContent="refreshing…",e.style.pointerEvents="none");let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),Be();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Be();return}r.profile=await t.json(),Te(),l("brain profile refreshed"),I()}function Vt(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${ot(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function je(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=je,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&je()};function it(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");me(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ce(){document.getElementById("docs-scrim").classList.remove("open")}function Ut(){return document.getElementById("docs-scrim").classList.contains("open")}function me(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Dt(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),me(e)}document.getElementById("open-docs").onclick=it,document.getElementById("docs-close").onclick=Ce,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&Ce()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),me(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,i)=>a.boundingClientRect.top-i.boundingClientRect.top);s.length&&me(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),E(),k(),I(),jt(),ke(),Be()}zt({"":{view:()=>({mount(y){y.innerHTML=Qt,Xt()}}),chrome:!1}},{title:"scout"});Kt();Gt();
