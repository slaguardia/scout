(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))L(r);new MutationObserver(r=>{for(const g of r)if(g.type==="childList")for(const x of g.addedNodes)x.tagName==="LINK"&&x.rel==="modulepreload"&&L(x)}).observe(document,{childList:!0,subtree:!0});function y(r){const g={};return r.integrity&&(g.integrity=r.integrity),r.referrerPolicy&&(g.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?g.credentials="include":r.crossOrigin==="anonymous"?g.credentials="omit":g.credentials="same-origin",g}function L(r){if(r.ep)return;r.ep=!0;const g=y(r);fetch(r.href,g)}})();function ba(v,i){const y=i.replace(/^#/,"");let L=null;for(const r of Object.keys(v))(y===r||y.startsWith(r))&&(L===null||r.length>L.length)&&(L=r);return L===null&&""in v&&(L=""),L}function wa(v){return typeof v=="function"?{view:v,chrome:!0}:{view:v.view,chrome:v.chrome!==!1}}function ka(v,i={}){const y=i.root??document.body,L=i.title??document.title??"",r=i.brandHref??"#",g=document.createElement("main"),x=document.createElement("header");x.className="cap-head";const b=document.createElement("a");b.className="brand",b.href=r,b.textContent=L,b.setAttribute("aria-label",`${L} — home`),x.appendChild(b);const V=document.createElement("nav");V.className="cap-nav",V.setAttribute("aria-label","Views");for(const w of i.nav??[]){const j=document.createElement("a");j.href=w.href,j.textContent=w.label,w.ariaLabel&&j.setAttribute("aria-label",w.ariaLabel),V.appendChild(j)}x.appendChild(V);const M=document.createElement("section");M.className="tk-content",g.appendChild(x),g.appendChild(M);const S=document.createElement("div");S.className="tk-bleed";const Je=w=>{var j;for(const q of Array.from(V.querySelectorAll("a"))){const U=((j=q.getAttribute("href"))==null?void 0:j.replace(/^#/,""))??"";q.toggleAttribute("aria-current",w!==null&&w!==""&&U===w),q.hasAttribute("aria-current")&&q.setAttribute("aria-current","page")}};let pe=0;const X=()=>{const w=ba(v,location.hash);if(Je(w),w===null){S.isConnected&&S.remove(),g.isConnected||y.appendChild(g),Ea(M,"Not found.");return}const{view:j,chrome:q}=wa(v[w]),U=q?M:S;q?(S.isConnected&&S.remove(),g.isConnected||y.appendChild(g)):(g.isConnected&&g.remove(),S.isConnected||y.appendChild(S)),U.replaceChildren();const A=j(),z=++pe,O=A.mount(U);O instanceof Promise&&O.catch(Fe=>{z===pe&&$a(U,String(Fe))})};window.addEventListener("hashchange",X),X()}function Ea(v,i){v.replaceChildren();const y=document.createElement("div");y.className="tk-empty",y.textContent=i,v.appendChild(y)}function $a(v,i){v.replaceChildren();const y=document.createElement("div");y.className="tk-error",y.textContent=i,v.appendChild(y)}function Ia(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(i=>{for(const y of i)y.unregister()}),window.caches&&caches.keys().then(i=>{for(const y of i)caches.delete(y)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function xa(){let v;try{v=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!v.ok)return null;let i;try{i=await v.json()}catch{return null}return typeof i.email=="string"&&i.email?{email:i.email}:null}const _a=`
<div class="layout">
<aside class="sidebar">
...
`;function Ba(v){const i={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],openDetail:null,anthropicKey:null},y=e=>"pill pill-"+(e||"none"),L='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),g=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function x(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],N()}async function b(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],_(),V(),S()}function V(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&te())}let M=null;function S(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!M?M=setInterval(Je,4e3):!e&&M&&(clearInterval(M),M=null)}async function Je(){let e;try{const s=await fetch("/api/postings");if(!s.ok)return;e=await s.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(s=>[s.posting_id,s.outreach_draft_status])),a=t.some(s=>n.get(s.posting_id)!==s.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,a&&(_(),V()),S()}async function pe(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{})]),Ut(),i.view==="jobs"&&_()}function X(e,{render:t=!0}={}){i.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",qt(),t&&(e==="jobs"?_():N())}async function w(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}i.stats=e,j()}function j(){D()}function q(e,t,n){const a=e[n]??"",s=t[n]??"";if(n==="headcount")return(a|0)-(s|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[a]??3)-(o[s]??3)}return String(a).localeCompare(String(s))}function U(e){return e.slice().sort((t,n)=>i.sort.dir*q(t,n,i.sort.k))}const A=new Set;let z=!1,O=!1;const Fe=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function An(){const e=document.getElementById("fdrop-verdict-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+Fe.map(([t,n,a])=>F("data-v",t,n,a,A.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Filters</div>'+F("data-toggle","flagged","⚑ Flagged","",z)+F("data-toggle","enriched","Enriched","",O),jt())}function jt(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const c of i.rows){const d=c.verdict||"__none__";e[d]=(e[d]|0)+1,c.flagged&&t++,c.enriched&&n++}Ye("#fdrop-verdict-menu [data-v]","data-v",e);const a=document.querySelector('#fdrop-verdict-menu [data-toggle="flagged"] [data-count]');a&&(a.textContent=t||"");const s=document.querySelector('#fdrop-verdict-menu [data-toggle="enriched"] [data-count]');s&&(s.textContent=n||"");const o=A.size+(z?1:0)+(O?1:0);Ge("fdrop-verdict-btn",o,o>0)}function Mt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(A.size&&!A.has(t.verdict||"__none__")||z&&!t.flagged||O&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Hn=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],Pn=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function At(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Ht=At("scout-hidden-cols"),Pt=At("scout-hidden-jcols");function Ke(){return i.view==="jobs"?{cols:Pn,hidden:Pt,key:"scout-hidden-jcols"}:{cols:Hn,hidden:Ht,key:"scout-hidden-cols"}}function se(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Ht.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Pt.has(e.dataset.col)?"none":""})}function qt(){const e=Ke(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>F("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Ot())}function Ot(){const e=Ke(),t=e.cols.filter(a=>e.hidden.has(a.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Nt(e){return`
    ...
    `}function Rt(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>gn(t.dataset.id))}const qn=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],On=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function Dt(e,t,n=7){const a=document.querySelector(e);if(!a)return;const s=[1,.82,.7,.95,.76,.9,.85];let o="";for(let c=0;c<n;c++){const d=t.map(([u,p])=>{const m=p.endsWith("%")?Math.round(parseFloat(p)*s[c%s.length])+"%":p;return`<td${u?` data-col="${u}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${d}</tr>`}a.innerHTML=o,se()}function N(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=U(Mt());jt(),document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const a=document.createElement("tr");a.dataset.id=n.company_id,a.innerHTML=Nt(n),a.addEventListener("click",s=>{s.target.closest("a, .flag-btn")||de(a.dataset.id)}),e.appendChild(a),Rt(a)}se()}async function Nn(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const a=document.querySelector("#t tbody"),s=U(Mt()).map(c=>c.company_id),o=[...a.querySelectorAll("tr")].map(c=>c.dataset.id);if(s.length!==o.length||s.some((c,d)=>c!==o[d])){N();return}for(const c of e){const d=i.rows.find(p=>p.company_id===c),u=a.querySelector(`tr[data-id="${CSS.escape(c)}"]`);if(!d||!u){N();return}u.innerHTML=Nt(d),Rt(u)}se()}let I=null,We=null,ae=!1,oe=!1;const J=new Set;function Vt(){const e=i.applicationStages;if(I===null)I=new Set(e.filter(t=>t!=="rejected"));else{for(const t of[...I])e.includes(t)||I.delete(t);if(We)for(const t of e)t!=="rejected"&&!We.has(t)&&I.add(t)}We=new Set(e)}function Rn(){Vt();const e=document.getElementById("jq").value.trim().toLowerCase();return i.jobs.filter(t=>{const n=R(t.stage_history);return!(n&&!I.has(n)||ae&&!t.next_up||oe&&(t.outreach_count|0)>0||J.size&&!J.has(t.outreach_status||"")||e&&!(t.title+" "+t.company+" "+(t.location||"")+" "+(t.description||"")+" "+(t.contacts||"")).toLowerCase().includes(e))})}function F(e,t,n,a,s){return`<button class="fdrop-item${s?" is-checked":""}" ${e}="${r(t)}" role="menuitemcheckbox" aria-checked="${s}"><span class="fdrop-check" aria-hidden="true"></span>`+(a?`<span class="fdrop-dot ${a}"></span>`:"")+`<span class="fdrop-label">${r(n)}</span><span class="fdrop-item-count" data-count></span></button>`}function Ut(){Vt();const e=document.getElementById("fdrop-application-menu");e&&(e.innerHTML='<div class="fdrop-head">Application stage</div>'+i.applicationStages.map(n=>F("data-stage",n,n,Be(n),I.has(n))).join(""));const t=document.getElementById("fdrop-outreach-menu");t&&(t.innerHTML='<div class="fdrop-head">Quick filters</div>'+F("data-toggle","nextup","★ Next up","",ae)+F("data-toggle","notreached","Not reached out","",oe)+'<div class="fdrop-sep"></div><div class="fdrop-head">Reply status</div>'+[["","none",""]].concat(i.outreachStatuses.map(n=>[n,n,Gt(n)])).map(([n,a,s])=>F("data-status",n,a,s,J.has(n))).join("")),zt()}function zt(){const e={},t={};let n=0,a=0;for(const d of i.jobs){const u=R(d.stage_history);u&&(e[u]=(e[u]|0)+1);const p=d.outreach_status||"";t[p]=(t[p]|0)+1,d.next_up&&n++,d.outreach_count|0||a++}Ye("#fdrop-application-menu [data-stage]","data-stage",e),Ye("#fdrop-outreach-menu [data-status]","data-status",t),Jt('[data-toggle="nextup"]',n),Jt('[data-toggle="notreached"]',a);const s=i.applicationStages.filter(d=>d!=="rejected"),o=I&&I.size===s.length&&s.every(d=>I.has(d));Ge("fdrop-application-btn",o?0:I?I.size:0,!o);const c=(ae?1:0)+(oe?1:0)+J.size;Ge("fdrop-outreach-btn",c,c>0)}function Ye(e,t,n){document.querySelectorAll(e).forEach(a=>{const s=a.querySelector("[data-count]");if(s){const o=n[a.getAttribute(t)]|0;s.textContent=o||""}})}function Jt(e,t){const n=document.querySelector(`#fdrop-outreach-menu ${e} [data-count]`);n&&(n.textContent=t||"")}function Ge(e,t,n){const a=document.getElementById(e);if(!a)return;a.classList.toggle("is-active",n);const s=a.querySelector(".fdrop-count");if(s){const o=n&&t>0;s.textContent=o?t:"",s.style.display=o?"":"none"}}function K(e,t){e.classList.toggle("is-checked",t),e.setAttribute("aria-checked",String(t))}function Qe(){document.querySelectorAll(".fdrop.is-open").forEach(e=>{e.classList.remove("is-open");const t=e.querySelector(".fdrop-btn");t&&t.setAttribute("aria-expanded","false")})}const Dn=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function Ft(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(Dn),a=n?n[0]:"";let s=a?t.replace(a,""):t;return s=s.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:s,email:a}})}function Vn(e){const t=(e||[]).map(n=>({position:(n.position||"").trim(),email:(n.email||"").trim()})).filter(n=>n.position||n.email);return t.length?JSON.stringify(t):""}const xe=()=>new Date().toISOString().slice(0,10);function ee(e){if(e=String(e||"").trim(),!e)return[];try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({stage:String((n==null?void 0:n.stage)||"").trim(),date:String((n==null?void 0:n.date)||"").trim()})).filter(n=>n.stage)}catch{}return[]}function _e(e){const t=(e||[]).map(n=>({stage:(n.stage||"").trim(),date:(n.date||"").trim()})).filter(n=>n.stage);return t.sort((n,a)=>(n.date||"9999-99-99").localeCompare(a.date||"9999-99-99")),t.length?JSON.stringify(t):""}function R(e){const t=ee(e);return t.length?t[t.length-1].stage:""}function Kt(e){const t=ee(e);return t.length?t[t.length-1].date:""}function Un(e){const t=[["","none"]];for(const n of i.applicationStages)t.push([n,n]);return e&&!i.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function Wt(e){const t=[["","none"]];for(const n of i.outreachStatuses)t.push([n,n]);return e&&!i.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const zn=8;function Yt(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%zn}function Be(e){return Yt(e,i.applicationStages)}function Gt(e){return Yt(e,i.outreachStatuses)}function Jn(e){const t=Ft(e);return t.length?t.map(n=>{const a=r(n.position||n.email);if(!n.email)return a;const s=r(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${r(n.email)}" title="${s}">${a}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function Qt(e){return`<div class="cc-row">
      ...
      <div class="cc-list">${Ft(e).map(Qt).join("")}</div>
      <button class="btn cc-add" type="button">+ add contact</button>
    </div>`}function Zt(e){const t=R(e.stage_history);if(!t)return i.applicationStages.length+1;const n=i.applicationStages.indexOf(t);return n<0?i.applicationStages.length:n}function Kn(e,t,n){if(n==="verdict"){const a={yes:0,maybe:1,no:2,"":3};return(a[e.verdict]??3)-(a[t.verdict]??3)}if(n==="application")return Zt(e)-Zt(t);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="last_outreach_at"){const a=e[n]||"",s=t[n]||"";return!a&&!s?0:a?s?String(s).localeCompare(String(a)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function _(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=Rn().sort((s,o)=>i.jsort.dir*Kn(s,o,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",zt();const n=I&&!I.has("rejected")?i.jobs.filter(s=>R(s.stage_history)==="rejected").length:0,a=document.getElementById("jobs-hidden-note");a.style.display=n?"":"none",n&&(a.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{I.add("rejected"),Ut(),_()});for(const s of t){const o=R(s.stage_history),c=Kt(s.stage_history),d=document.createElement("tr");d.dataset.id=s.posting_id;const u=Un(o).map(([f,k])=>`<option value="${r(f)}"${o===f?" selected":""}>${r(k)}</option>`).join(""),p=s.outreach_status||"",m=Wt(p).map(([f,k])=>`<option value="${r(f)}"${p===f?" selected":""}>${r(k)}</option>`).join("");d.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${s.next_up?" is-on":""}" title="${s.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up">${s.next_up?"★":"☆"}</button><div class="jt-namecol"><span class="row-name">${r(s.title||s.company)}</span>${Yn(s.outreach_draft_status)}${s.title?`<div class="small dim">${r(s.company)}</div>`:""}</div></div></td>
      ...
      <td class="small" data-col="outreach"><div class="jt-out"><span class="jt-stepper"><button class="jt-dec" title="undo one outreach"${s.outreach_count?"":" disabled"}>−</button><span class="jt-oc${s.outreach_count?"":" dim"}">${s.outreach_count||0}</span><button class="jt-inc" title="log one outreach (today)">+</button></span><select class="jt-ostatus ${Gt(p)}" title="outreach reply status">${m}</select></div></td>
      <td class="small" data-col="last_outreach">${s.last_outreach_at?r(s.last_outreach_at):'<span class="dim">—</span>'}</td>
      <td class="small td-contacts" data-col="contacts">${Jn(s.contacts)}</td>
      <td data-col="link"><a href="${g(s.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `,d.querySelector(".jt-nextup").onclick=()=>rn(s,!1),d.querySelector(".jt-stage-sel").onchange=f=>Wn(s,f.target.value),d.querySelector(".jt-ostatus").onchange=f=>Ce(s,{outreach_status:f.target.value}),d.querySelector(".jt-inc").onclick=()=>Ce(s,{outreach_count:(s.outreach_count||0)+1,last_outreach_at:xe()}),d.querySelector(".jt-dec").onclick=()=>{const f=Math.max(0,(s.outreach_count||0)-1);Ce(s,{outreach_count:f,...f===0?{last_outreach_at:""}:{}})},e.appendChild(d)}se(),e.querySelectorAll("tr").forEach(s=>{s.addEventListener("click",o=>{o.target.closest("a, button, select")||Xt(s.dataset.id)})})}function Wn(e,t){if(t=(t||"").trim(),!t||t===R(e.stage_history)){_();return}const n=ee(e.stage_history);n.push({stage:t,date:xe()}),Ce(e,{stage_history:_e(n)})}function Yn(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1};async function Xt(e){let t=i.jobs.find(n=>n.posting_id===e);if(t||(await b(),t=i.jobs.find(n=>n.posting_id===e)),!t){l("posting not found — refresh");return}Xe(),ot(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),en("pursuit"),te(),me(),ce()}let Ze=null;function en(e){Ze=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function he(){Xe(),ot(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function Xe(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function me(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){Se();return}e=await n.json()}catch{Se();return}h.drafts=e.drafts||[],Se();const t=h.drafts[0];t&&t.status==="researching"?Gn():Xe()}function Gn(){h.poll||(h.poll=setInterval(me,4e3))}function H(e,t,{multiline:n=!1}={}){if(!e)return;let a=e.value;e.addEventListener("focus",()=>{a=e.value}),e.addEventListener("keydown",s=>{s.key==="Escape"?(s.preventDefault(),e.value=a,e.blur()):s.key==="Enter"&&(!n||s.metaKey||s.ctrlKey)&&(s.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const s=e.value.trim();if(s===a.trim()){e.value=a;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(s),a=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=a,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),l(`save failed: ${o.message}`)}})}async function tn(e,t,n){const a={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:n},s=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const o=await s.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),_(),le(e.posting_id,{title:o.title,location:o.location})}async function Qn(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();e.url=a.url;const s=document.querySelector("#role-body .role-url-open");s&&s.setAttribute("href",g(e.url)),le(e.posting_id,{url:a.url})}async function Zn(e,t){if(t.disabled)return;const n=t.textContent;t.disabled=!0,t.textContent="re-enriching…";let a;try{a=await fetch(`/api/postings/${e.posting_id}/recapture`,{method:"POST"})}catch(o){t.disabled=!1,t.textContent=n,l(`re-enrich failed: ${o.message}`);return}if(!a.ok){const o=(await a.text().catch(()=>"")).trim();let c=o||"HTTP "+a.status;try{c=JSON.parse(o).error||c}catch{}t.disabled=!1,t.textContent=n,l(`re-enrich failed: ${c}`);return}const s=await a.json();Object.assign(e,{title:s.title,location:s.location,employment_type:s.employment_type,workplace_type:s.workplace_type,department:s.department,comp_range:s.comp_range,description:s.description,posted_at:s.posted_at,url:s.url,questions_status:s.questions_status}),_(),te(),le(e.posting_id,{title:s.title,location:s.location,url:s.url}),l("re-enriched from the posting link")}function Xn(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>ts(e))}async function es(e,t){const n=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();e.company_id=a.company_id,e.company=a.company_name,te(),b()}let fe=null;function ts(e){fe=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const n=document.getElementById("relink-search");n&&(n.value=""),nn(""),document.getElementById("relink-scrim").classList.add("open"),n&&n.focus()}function ie(){document.getElementById("relink-scrim").classList.remove("open"),fe=null}let et=null;function ns(e){et=e;const t=(e.postings||[]).length,n=t?` and its ${t} job ${t===1?"posting":"postings"}`:"",a=document.getElementById("delcompany-summary");a&&(a.innerHTML=`Delete <strong>${r(e.name||"this company")}</strong>${n}?`);const s=document.getElementById("delcompany-confirm");s&&(s.disabled=!1),document.getElementById("delcompany-scrim").classList.add("open")}function Le(){document.getElementById("delcompany-scrim").classList.remove("open"),et=null}async function ss(){const e=et;if(!e)return;const t=document.getElementById("delcompany-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e.company_id}`,{method:"DELETE"})}catch(s){l(`delete failed: ${s.message}`),t&&(t.disabled=!1);return}if(!n.ok){const s=await n.text().catch(()=>"");l(`delete failed: HTTP ${n.status}${s?" — "+s:""}`),t&&(t.disabled=!1);return}const a=e.name||"company";Le(),i.openId===e.company_id&&ve(),x(),b(),w(),l(`deleted ${a}`)}let tt=null;function as(e){tt=e;const t=(e.title||"").trim()||"this posting",n=e.company?` at <strong>${r(e.company)}</strong>`:"",a=document.getElementById("deljob-summary");a&&(a.innerHTML=`Delete <strong>${r(t)}</strong>${n}?`);const s=document.getElementById("deljob-confirm");s&&(s.disabled=!1),document.getElementById("deljob-scrim").classList.add("open")}function Te(){document.getElementById("deljob-scrim").classList.remove("open"),tt=null}async function os(){const e=tt;if(!e)return;const t=document.getElementById("deljob-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${e.posting_id}`,{method:"DELETE"})}catch(s){l(`delete failed: ${s.message}`),t&&(t.disabled=!1);return}if(!n.ok){const s=await n.text().catch(()=>"");l(`delete failed: HTTP ${n.status}${s?" — "+s:""}`),t&&(t.disabled=!1);return}const a=(e.title||"").trim()||"posting";Te(),he(),b(),i.openId===e.company_id&&de(e.company_id),l(`deleted ${a}`)}function nn(e){const t=document.getElementById("relink-results");if(!t)return;const n=e.trim().toLowerCase();let a=(i.rows||[]).slice();if(n?(a=a.filter(o=>(o.name||"").toLowerCase().includes(n)),a.sort((o,c)=>{const d=(o.name||"").toLowerCase().startsWith(n)?0:1,u=(c.name||"").toLowerCase().startsWith(n)?0:1;return d-u||(o.name||"").localeCompare(c.name||"")})):a.sort((o,c)=>(o.name||"").localeCompare(c.name||"")),a=a.slice(0,60),!a.length){t.innerHTML=`<div class="relink-empty">${(i.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const s=fe?fe.company_id:"";t.innerHTML=a.map(o=>{const c=o.company_id===s,d=[o.vertical,o.location].filter(Boolean).map(r).join(" · ");return`<button type="button" class="relink-result${c?" is-current":""}"
      ...
      </button>`}).join("")}async function sn(e){const t=fe;if(!t){ie();return}if(e===t.company_id){ie();return}try{await es(t,e),ie(),l(`moved to ${t.company}`)}catch(n){l(`move failed: ${n.message}`)}}async function an(e,t,n){const a={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(a.name).trim())throw new Error("name is required");const s=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const o=await s.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),x(),b()}async function is(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();i.openId=a.company_id,be(a),we(a.company_id),x(),b()}async function rs(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();e.notes=a.notes}let on=null;function te(){const e=h.row;if(!e)return;const t=document.getElementById("pursuit-body"),a=!!t&&on===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${r(e.title||"")}">`;const s=R(e.stage_history);document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${s?Be(s)||"pill-stage":"pill-none"}">${r(s||"—")}</span>`+(e.verdict?` <span class="${y(e.verdict)}">${r(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>Bt("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    ...
    <section class="pane-section">
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row pipeline-stage-row">
          ...
          <div class="pl-stage-wrap">${ds(e)}</div>
        </div>
        <div class="pipeline-row">
          ...
          <select class="input pl-ostatus" title="outreach reply status — separate from the application stage">
            ${Wt(e.outreach_status||"").map(([p,m])=>`<option value="${r(p)}"${(e.outreach_status||"")===p?" selected":""}>${r(m)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
  ...
  `,ls();const c=document.getElementById("pursuit-company-link");c&&c.addEventListener("click",()=>de(e.company_id)),Xn(e),H(document.getElementById("pursuit-title-input"),p=>tn(e,"title",p)),H(document.getElementById("pursuit-url-input"),p=>Qn(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>Zn(e,d)),H(document.getElementById("pursuit-notes-input"),p=>us(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>H(p,m=>tn(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>as(e)),Se(),ye(),t&&(t.scrollTop=a),on=e.posting_id}function cs(e){return`
    ...
    </div>`}function ds(e){const t=ee(e.stage_history),n=t.map((s,o)=>`
    ...
    </div>`}function ls(){const e=h.row;document.querySelectorAll("#pursuit-body .stage-entry .stage-date").forEach(s=>{s.addEventListener("change",o=>{const c=+o.target.closest(".stage-entry").dataset.i,d=ee(e.stage_history);d[c]&&(d[c].date=o.target.value,re({stage_history:_e(d)}))})}),document.querySelectorAll("#pursuit-body .stage-entry .stage-del").forEach(s=>{s.addEventListener("click",o=>{const c=+o.target.closest(".stage-entry").dataset.i,d=ee(e.stage_history);d.splice(c,1),re({stage_history:_e(d)})})});const t=document.querySelector("#pursuit-body .stage-add-btn");t&&t.addEventListener("click",()=>{const s=document.querySelector("#pursuit-body .stage-add-sel"),o=document.querySelector("#pursuit-body .stage-add-date"),c=(s.value||"").trim();if(!c)return;const d=ee(e.stage_history);d.push({stage:c,date:o.value||xe()}),re({stage_history:_e(d)})});const n=document.querySelector("#pursuit-body .pl-ostatus");n&&n.addEventListener("change",s=>re({outreach_status:s.target.value}));const a=document.querySelector("#pursuit-body .pt-nextup");a&&a.addEventListener("click",()=>rn(h.row,!0))}async function rn(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(s){l(`save failed: ${s.message}`);return}if(!n.ok){const s=(await n.text().catch(()=>"")).trim();l(`save failed: ${s||"HTTP "+n.status}`);return}const a=await n.json();e.next_up=a.next_up,_(),le(e.posting_id,{next_up:a.next_up}),t&&te(),l(e.next_up?"queued next up":"removed from the queue")}async function nt(e,t){const n={stage_history:e.stage_history||"",outreach_count:e.outreach_count||0,last_outreach_at:e.last_outreach_at||"",outreach_status:e.outreach_status||"",contacts:e.contacts||"",notes:e.notes||"",...t};let a;try{a=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return l(`save failed: ${o.message}`),null}if(!a.ok){const o=(await a.text().catch(()=>"")).trim();return l(`save failed: ${o||"HTTP "+a.status}`),null}const s=await a.json();return Object.assign(e,{stage_history:s.stage_history,outreach_count:s.outreach_count,last_outreach_at:s.last_outreach_at,outreach_status:s.outreach_status,contacts:s.contacts,notes:s.notes,next_up:s.next_up}),le(e.posting_id,{stage_history:s.stage_history,outreach_count:s.outreach_count,last_outreach_at:s.last_outreach_at,next_up:s.next_up}),s}async function us(e){const t=h.row,n={stage_history:t.stage_history||"",outreach_count:t.outreach_count||0,last_outreach_at:t.last_outreach_at||"",outreach_status:t.outreach_status||"",contacts:t.contacts||"",notes:e},a=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const s=await a.json();t.notes=s.notes,_()}async function re(e){await nt(h.row,e)&&(_(),te(),l("tracking saved"))}async function cn(e){await nt(h.row,{contacts:e})&&_()}async function Ce(e,t){await nt(e,t)&&(_(),h.postingId===e.posting_id&&(h.row=e,te()),l("tracking saved"))}function Se(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.row,n=h.drafts,a=n[0]||null,s=n.slice(1),o=`
    ...
    ${Fn(t.contacts)}`,d=a&&(ps(a.status)||a.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${a?"Draft again":"Draft outreach"}</button>`,u=s.length?`
    <details class="draft-history" ${h.openHist?"open":""}>
      <summary>${s.length} earlier draft${s.length>1?"s":""}</summary>
      <div id="draft-history-body">${s.map(p=>ln(p,!0)).join("")}</div>
    </details>`:"";e.innerHTML=o+`<div id="draft-current">${a?ln(a,!1):""}</div><div class="draft-actions">${d}</div>`+u,vs()}function ps(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const hs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',ms='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',dn=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${hs}</button>`,st=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],fs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function gs(e){let t=st.findIndex(a=>a.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${st.map((a,s)=>{const o=s<t?"is-done":s===t?"is-active":"is-pending",c=s<t?fs:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${c}</span><span class="dp-name">${a.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${st[t].active}…</span></div>
  </div>`}function ln(e,t){const n=(p,m,f="")=>`
    <div class="draft-head">
      ...
      <div class="draft-sentbody">${r(at(e)||"(empty)")}</div>
      ${ge(e)}
    </div>`;const a=at(e),s=e.status==="no_hook",o=s?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let c="";if(s)try{c=JSON.parse(e.hook||"{}").reasoning||""}catch{}const d=s?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${c?" "+r(c):""}</div>`:"";if(t)return`<div class="draft-card ${s?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    <div class="draft-head">${o}${a?dn:""}</div>
    ${d}
    ${u?`<textarea class="draft-textarea" id="draft-edit-${e.id}" spellcheck="false">${r(a)}</textarea>
    ${un(e.lint)}
    <div class="draft-actions">
      <button class="btn btn-primary draft-sent-btn" title="mark this email sent — bumps the outreach count">${ms}Mark sent</button>
    ...
    ${ge(e)}
  </div>`}function at(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function ge(e){let t="",n=null,a=null;try{n=JSON.parse(e.research||"null")}catch{}try{a=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const s=(u,p)=>p?`<div class="tr-line"><span class="tr-key">${u}:</span> ${r(String(p))}</div>`:"",o=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],d=c.map(u=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function un(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${r(n.message||"")}"><code>${r(n.code||"")}</code>${r(n.message||"")}</span>`).join("")+"</div>":""}function ys(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${r(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${r(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function vs(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#contacts-editor");if(t){const u=t.querySelector(".cc-list"),p=()=>Vn(Array.from(u.querySelectorAll(".cc-row")).map(f=>({position:f.querySelector(".cc-pos").value,email:f.querySelector(".cc-email").value}))),m=f=>{f.querySelectorAll(".cc-pos, .cc-email").forEach(k=>{k.addEventListener("change",()=>cn(p())),k.addEventListener("keydown",C=>{C.key==="Enter"&&(C.preventDefault(),C.target.blur())})}),f.querySelector(".cc-del").addEventListener("click",()=>{f.remove(),cn(p())})};u.querySelectorAll(".cc-row").forEach(m),t.querySelector(".cc-add").addEventListener("click",()=>{const f=document.createElement("div");f.innerHTML=Qt({position:"",email:""});const k=f.firstElementChild;u.appendChild(k),m(k),k.querySelector(".cc-pos").focus()})}const n=()=>new Date().toISOString().slice(0,10),a=h.row,s=e.querySelector(".pt-outreach");s&&s.addEventListener("click",()=>re({outreach_count:(a.outreach_count||0)+1,last_outreach_at:n()}));const o=e.querySelector(".pt-outreach-dec");o&&o.addEventListener("click",()=>{const u=Math.max(0,(a.outreach_count||0)-1);re({outreach_count:u,...u===0?{last_outreach_at:""}:{}})});const c=e.querySelector("#draft-start-btn");c&&c.addEventListener("click",()=>je()),e.querySelectorAll(".draft-retry-btn").forEach(u=>u.addEventListener("click",()=>je())),e.querySelectorAll(".draft-regen-btn").forEach(u=>u.addEventListener("click",()=>je(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(u=>{const p=u.dataset.did,m=u.querySelector(".draft-textarea");m&&H(m,C=>ws(p,C),{multiline:!0});const f=u.querySelector(".draft-sent-btn");f&&f.addEventListener("click",()=>ks(p));const k=u.querySelector(".draft-copy-btn");k&&k.addEventListener("click",()=>{const C=u.querySelector(".draft-textarea"),ne=u.querySelector(".draft-sentbody"),ue=C?C.value:ne?ne.textContent:"";Os(ue,"email copied")})});const d=e.querySelector("details.draft-history");d&&d.addEventListener("toggle",()=>{h.openHist=d.open})}async function je(e=!1){const t=document.getElementById("outreach-section"),n=t&&(t.querySelector("#draft-start-btn")||t.querySelector(".draft-retry-btn")||t.querySelector(".draft-regen-btn"));n&&(n.disabled=!0);let a;try{const o=e?"?regenerate=1":"";a=await fetch(`/api/postings/${h.postingId}/outreach${o}`,{method:"POST"})}catch(o){l(`draft failed: ${o.message}`),n&&(n.disabled=!1);return}if(a.status===202){let o={};try{o=await a.json()}catch{}Array.isArray(o.degraded)&&o.degraded.length&&l(`drafting without ${o.degraded.join(", ")} — quality degrades, integrity unaffected`),await me(),b();return}if(a.status===409){await me(),l("a draft is already active");return}if(a.status===412){let o={};try{o=await a.json()}catch{}bs(o.need,o.error),n&&(n.disabled=!1);return}if(a.status===503){const o=document.getElementById("outreach-section");if(o){const c=document.createElement("div");c.className="draft-note",c.textContent="Outreach engine not running in this build.",o.appendChild(c)}n&&(n.disabled=!1);return}const s=(await a.text().catch(()=>"")).trim();l(`draft failed: ${s||"HTTP "+a.status}`),n&&(n.disabled=!1)}function bs(e,t){const n=document.getElementById("outreach-section");if(!n)return;const a=n.querySelector(".draft-actions"),s=e==="template",o=s?"Write email template":"Discover sources",c=document.createElement("div");c.className="blocks-gate",c.innerHTML=`
    ...
    </div>`,a?a.replaceWith(c):n.appendChild(c);const d=c.querySelector("#gate-fix-btn");d&&d.addEventListener("click",()=>s?G("outreach-template"):ht());const u=c.querySelector("#gate-retry-btn");u&&u.addEventListener("click",je)}async function ws(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json(),s=h.drafts.findIndex(d=>String(d.id)===String(e));s>=0&&(h.drafts[s]=a);const o=document.getElementById(`draft-edit-${e}`),c=o&&o.closest(".draft-card");if(c){const d=c.querySelector(".lint-chips"),u=un(a.lint);d?d.outerHTML=u||"":u&&o.insertAdjacentHTML("afterend",u)}}async function ks(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(a){l(`failed: ${a.message}`);return}if(!t.ok){const a=(await t.text().catch(()=>"")).trim();l(`failed: ${a||"HTTP "+t.status}`);return}l("marked sent"),await me(),await b();const n=i.jobs.find(a=>a.posting_id===h.postingId);n&&le(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function ce(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){ye();return}e=await t.json()}catch{ye();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",ye(),h.answers.some(t=>t.status==="generating")?Es():ot()}function Es(){h.answersPoll||(h.answersPoll=setInterval(ce,4e3))}function ot(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function ye(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,a=t.some(p=>p.status==="generating"),s=t.length?`<div class="answers-list">${t.map(xs).join("")}</div>`:"",o=!!h.detecting,c=a||o?" disabled":"",d=p=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":p}</button>`;let u;n==="ok"&&t.length?u=(t.some(m=>!pn(m)&&m.status!=="generating")?`<button class="btn" id="answers-start-btn"${c}>${a?"Drafting…":"Draft all blank"}</button>`:"")+d("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${c}>${a?"Drafting…":"Draft answers"}</button>`+d("Re-detect questions"):u=d("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${r($s(n,t.length))}</div>`+s+`<div class="answers-actions">${u}</div>`,Bs()}function $s(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function pn(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Is(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function xs(e){const t=pn(e),n=e.edited&&e.edited.trim(),a=e.status==="generating",s=t.length,o=e.max_length&&s>e.max_length,c=e.max_length?`<span class="answer-count${o?" over":""}">${s} / ${e.max_length}</span>`:`<span class="answer-count">${s} chars</span>`,d=!!t,u=d?"Regenerate":"Generate",p=d?"re-draft this answer (discards the current text)":"draft an answer to just this question";return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
  ...
  </div>`}function _s(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function Bs(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",hn);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",Ts),e.querySelectorAll(".answer-card[data-aid]").forEach(a=>{const s=a.dataset.aid,o=a.querySelector(".answer-textarea");o&&(H(o,u=>Cs(s,u),{multiline:!0}),o.addEventListener("input",()=>Ls(a,o)));const c=a.querySelector(".answer-regen-btn");c&&c.addEventListener("click",()=>Ss(s));const d=a.querySelector(".answer-remove-btn");d&&d.addEventListener("click",()=>js(s))})}function Ls(e,t){const n=e.querySelector(".answer-count");if(!n)return;const a=t.value.length,s=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=s?`${a} / ${s}`:`${a} chars`,n.classList.toggle("over",!!s&&a>s)}async function hn(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(s){l(`draft failed: ${s.message}`),t&&(t.disabled=!1);return}if(n.status===202){await ce();return}if(n.status===412){let s={};try{s=await n.json()}catch{}mn(s.error),t&&(t.disabled=!1);return}if(n.status===503){fn("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const a=(await n.text().catch(()=>"")).trim();l(`draft failed: ${a||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function Ts(){h.detecting=!0,ye();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();l(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){l(`detect failed: ${e.message}`)}h.detecting=!1,await ce()}async function Cs(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json(),s=h.answers.findIndex(o=>String(o.id)===String(e));s>=0&&(h.answers[s]=a)}async function Ss(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){l(`regenerate failed: ${n.message}`);return}if(t.status===503){fn("Answer generation isn't running in this build.");return}if(t.status===412){let n={};try{n=await t.json()}catch{}mn(n.error);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`regenerate failed: ${n||"HTTP "+t.status}`);return}await ce()}async function js(e){if(!confirm("Remove this question? Any answer drafted or written for it is discarded, and re-detecting won't bring it back."))return;let t;try{t=await fetch(`/api/answers/${e}`,{method:"DELETE"})}catch(n){l(`remove failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`remove failed: ${n||"HTTP "+t.status}`);return}await ce()}function mn(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),a=document.createElement("div");a.className="blocks-gate",a.innerHTML=`
    ...
    </div>`,n?n.replaceWith(a):t.appendChild(a);const s=a.querySelector("#answers-fix-btn");s&&s.addEventListener("click",ht);const o=a.querySelector("#answers-retry-btn");o&&o.addEventListener("click",hn)}function fn(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function de(e){var d,u;const t=document.getElementById("pane"),n=document.getElementById("scrim"),a=i.openId===e&&t.classList.contains("open"),s=a?((d=document.getElementById("pane-body"))==null?void 0:d.scrollTop)??0:0,o=a?(u=document.getElementById("trace-body"))==null?void 0:u.innerHTML:null;i.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),en("company"),a||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let c;try{const p=await fetch(`/api/companies/${e}`);if(!p.ok)throw new Error(`HTTP ${p.status}`);c=await p.json()}catch(p){a||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${r(p.message)}</div>`);return}if(i.openId===e){if(be(c),a){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const p=document.getElementById("pane-body");p&&(p.scrollTop=s)}we(e)}}function ve(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function be(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${r(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
    <span class="${y(e.has_verdict?e.verdict:"")}">${r(e.has_verdict?e.verdict:"unscored")}</span>
  `;const t=document.getElementById("pane-chat");t&&(t.style.display=i.meta&&i.meta.chat?"":"none",t.onclick=()=>Bt("company",e.company_id,e.name));const n=e.model==="manual",a=e.has_verdict?`
    <dl class="kv">
  ...
  `;const k=document.getElementById("posting-add-btn");k&&k.addEventListener("click",()=>Ps(e)),rt(),document.querySelectorAll("#ve-pick .ve-opt").forEach($=>{$.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Z=>Z.classList.remove("is-on")),$.classList.add("is-on")})});const C=document.getElementById("ve-save-btn");C&&C.addEventListener("click",()=>Hs(e)),H(document.getElementById("pane-title-input"),$=>an(e,"name",$)),document.querySelectorAll("#facts-body [data-k]").forEach($=>H($,Z=>an(e,$.dataset.k,Z))),H(document.getElementById("pane-domain-input"),$=>is(e,$)),H(document.getElementById("pane-notes-input"),$=>rs(e,$),{multiline:!0});const ne=document.getElementById("flag-toggle-btn");ne&&ne.addEventListener("click",()=>gn(e.company_id));const ue=document.getElementById("review-stamp-btn");ue&&ue.addEventListener("click",()=>As(e.company_id));const Ee=document.getElementById("rescore-btn");Ee&&Ee.addEventListener("click",()=>lt("verdict",{company_ids:[e.company_id]}));const ze=document.getElementById("reenrich-btn");ze&&ze.addEventListener("click",()=>lt("enrich",{company_ids:[e.company_id]}));const $e=document.getElementById("company-delete-btn");$e&&$e.addEventListener("click",()=>ns(e))}function Ms(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function le(e,t){const n=i.openDetail;if(!n||!i.openId)return;const a=(n.postings||[]).find(o=>String(o.id)===String(e));if(!a)return;Object.assign(a,t);const s=document.getElementById("postings-list");s&&(s.innerHTML=it(n),rt())}function rt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||Xt(e.dataset.pid)})})}async function Ps(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),a=document.getElementById("posting-add-btn"),s=t.value.trim();if(!s){l("Enter a URL first."),t.focus();return}a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:s,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),a.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),a.disabled=!1;return}const c=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const d=document.getElementById("postings-list");d&&(d.innerHTML=it(e),rt()),t.value="",n.value="",a.disabled=!1,b(),l("link added")}async function we(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(a){Me(`<div class="muted">Failed to load trail: ${r(a.message)}</div>`);return}if(!t.ok){Me(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Me('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Me(n.map(qs).join(""))}function qs(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(r);return e.run_id&&t.push("run "+r(e.run_id.slice(0,8))),`
    ...
    </div>`}function Me(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let yn;function l(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(yn),yn=setTimeout(()=>t.classList.remove("show"),2200)}async function Os(e,t="copied"){if(!e){l("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}l(t)}catch(n){l(`copy failed: ${n.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function ct(){try{const a=await fetch("/api/meta");if(!a.ok)return;i.meta=await a.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=i.meta.chat?"":"none")}async function dt(){let e;try{const a=await fetch("/api/runs");if(!a.ok)return;e=await a.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Ae=null;async function lt(e,t){if(i.meta&&i.meta.control===!1){l("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(s){l(`run failed: ${s.message}`);return}if(n.status===409){l("a job is already running");return}if(n.status===412){const s=await n.text();l(s.trim());return}if(!n.ok){l(`run failed: HTTP ${n.status}`);return}const{job_id:a}=await n.json();In(e,a,t)}async function Ns(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(s){l(`upload failed: ${s.message}`);return}if(n.status===409){l("a job is already running");return}if(!n.ok){l(`upload failed: HTTP ${n.status}`);return}const{job_id:a}=await n.json();In("ingest",a)}const Rs=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let He=[],W=new Set,Y="company";function vn(e){Y=e,document.querySelectorAll("#add-kind .v-chip").forEach(a=>a.classList.toggle("is-on",a.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",bn()}function ut(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function bn(){const e=document.getElementById("add-note");ut()?e.innerHTML=Y==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and description — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=Y==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function Ds(){Rs.forEach(a=>{document.getElementById(a).value=""}),document.getElementById("add-vertical-filter").value="",W=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),vn(i.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(a=>`<option value="${r(a.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const a=await(await fetch("/api/facets")).json();(a.funding_stages||[]).forEach(s=>{const o=document.createElement("option");o.value=s,o.textContent=s,n.appendChild(o)}),He=a.verticals||[]}catch{He=[]}wn()}function Pe(){document.getElementById("add-scrim").classList.remove("open")}function wn(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=He.filter(a=>!t||a.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(a=>`<button type="button" class="vchip${W.has(a)?" sel":""}" data-v="${r(a)}">${r(a)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(a=>a.addEventListener("click",()=>{const s=a.dataset.v;W.has(s)?W.delete(s):W.add(s),a.classList.toggle("sel"),kn()}))):e.innerHTML=`<div class="none">${He.length?"no match":"no verticals in the set yet"}</div>`,kn()}function kn(){const e=W.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function En(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function $n(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){l(Y==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),a=n.textContent;n.disabled=!0,ut()&&(n.textContent="reading page…");const s=()=>{n.disabled=!1,n.textContent=a},o=f=>document.getElementById(f).value.trim(),c=ut();let d,u;c?(d="/api/capture",u={url:En(t),kind:Y==="company"?"company_page":"job_posting",fields:Y==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...W].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):Y==="company"?(d="/api/companies",u={website:t,name:o("add-name"),vertical:[...W].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(d="/api/postings",u={url:En(t),title:o("add-title"),company:o("add-job-company")});let p;try{p=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(f){l(`add failed: ${f.message}`),s();return}if(!p.ok){let f=`HTTP ${p.status}`;try{const k=await p.text();try{f=JSON.parse(k).error||f}catch{f=k.trim()||f}}catch{}if(s(),p.status===409){l(f||"That company is already in the list."),e.focus(),e.select();return}l(`add failed: ${f}`);return}const m=await p.json();if(s(),c&&!m.company_id){l(m.note||"couldn't classify that page");return}if(Pe(),x(),w(),b(),Y==="job"){const f=m.posting&&m.posting.title||"job link";l(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),X("jobs")}else c?(l(m.note||(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`)),de(m.company_id)):l("company added")}function In(e,t,n){Ae=t;const a=document.getElementById("drawer"),s=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",s.innerHTML="",a.classList.add("open"),dt();const o=new EventSource(`/api/jobs/${t}/stream`),c=(d,u)=>{const p=document.createElement("div"),m=!u&&/^\s*warn:/i.test(d);p.className="ln"+(u?" ln-err":m?" ln-warn":""),p.textContent=m?d.replace(/^\s*warn:\s*/i,"⚠ "):d,s.appendChild(p),s.scrollTop=s.scrollHeight};o.addEventListener("line",d=>c(d.data,/error|failed/i.test(d.data))),o.addEventListener("end",d=>{o.close(),Ae=null,c(`— ${d.data} —`,d.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",l(`${e} ${d.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?Nn(n.company_ids):x(),w(),dt(),b(),i.openId&&de(i.openId)}),o.onerror=()=>{o.close()}}async function Vs(){if(Ae)try{await fetch(`/api/jobs/${Ae}/cancel`,{method:"POST"})}catch{}}let T=null;const Us={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function pt(e){return e==="application-stages"||e==="outreach-statuses"}function qe(e){if(e==="outreach-template")return"outreach template";if(e==="taste-filter")return"pre-filter rules";if(e==="playbook")return"playbook";if(e==="application-stages")return"application stages";if(e==="outreach-statuses")return"outreach statuses";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(Us[t]||t)+" prompt"}return e+".md"}async function G(e){T=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+qe(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=!!e&&e.startsWith("outreach-prompts/"),a=n?e.slice(17):"",s=e==="taste-filter"||n&&a!=="fill";document.getElementById("editor-toggle-row").style.display=s?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",s&&(document.getElementById("editor-toggle-label").textContent=e==="taste-filter"?"Enable the pre-filter (off → bulk verdict runs score every company; the rules below are kept either way)":"Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const d=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${d||"HTTP "+o.status}`;return}const c=await o.json();pt(e)?(document.getElementById("editor-title").textContent="edit "+qe(e)+" — one per line",document.getElementById("editor-text").value=(c.statuses||[]).join(`
`)):document.getElementById("editor-text").value=c.content||"",s&&(document.getElementById("editor-enabled").checked=c.enabled!==!1),c.taste_version&&(document.getElementById("editor-ver").textContent="version "+c.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Oe(){document.getElementById("editor-scrim").classList.remove("open"),T=null}const zs=[{key:"experience",hard:!0},{key:"voice",hard:!1},{key:"logistics",hard:!1}];async function ht(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{ft(await(await fetch("/api/outreach/sources")).json())}catch(e){l(`failed to load sources: ${e.message}`)}}function mt(){document.getElementById("sources-scrim").classList.remove("open")}function ft(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(s=>({key:s.Key||s.key,hard:s.Hard??s.hard})):zs,a={};(e&&e.sources||[]).forEach(s=>{(a[s.need]=a[s.need]||[]).push(s)}),t.innerHTML=n.map(s=>{const o=a[s.key]||[],c=o.length?o.map(d=>`<li><span class="src-title">${r(d.title||d.page_id)}</span><button class="src-rm" data-need="${r(s.key)}" data-id="${r(d.page_id)}" title="remove">✕</button></li>`).join(""):`<li class="dim small">${s.hard?"none yet — required for drafting":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${r(s.key)}${s.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${c}</ul></div>`}).join(""),t.querySelectorAll(".src-rm").forEach(s=>s.addEventListener("click",()=>Fs(s.dataset.need,s.dataset.id)))}async function Js(){const e=document.getElementById("sources-refresh-btn");e&&(e.disabled=!0,e.textContent="Discovering…");let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(a){l(`refresh failed: ${a.message}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}if(!t.ok){l(`refresh failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Refresh from brain");return}const n=await t.json();n.warning?l(n.warning):l("sources refreshed"),ft(n),e&&(e.disabled=!1,e.textContent="Refresh from brain")}async function Fs(e,t){let n;try{n=await fetch("/api/outreach/sources/remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({need:e,page_id:t})})}catch(a){l(`remove failed: ${a.message}`);return}if(!n.ok){l(`remove failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}ft(await n.json())}async function Ks(){if(!T)return;const e=document.getElementById("editor-text").value;let t;if(pt(T))t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)};else{t={content:e};const o=T.startsWith("outreach-prompts/")&&T!=="outreach-prompts/fill";(T==="taste-filter"||o)&&(t.enabled=document.getElementById("editor-enabled").checked)}let n;try{n=await fetch(`/api/${T}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){l(`save failed: ${o.message}`);return}if(!n.ok){l(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const a=await n.json();a.taste_version&&(document.getElementById("editor-ver").textContent="version "+a.taste_version);const s=pt(T);l(`${qe(T)} saved`),Oe(),s&&pe(),w()}async function Ws(){if(!T||!T.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${T}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){l(`reset failed: ${n.message}`);return}if(!e.ok){l(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",l(`${qe(T)} reset to default`)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;i.sort.k===t?i.sort.dir*=-1:(i.sort.k=t,i.sort.dir=1),N()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;i.jsort.k===t?i.jsort.dir*=-1:(i.jsort.k=t,i.jsort.dir=1),_()}}),document.getElementById("tab-companies").onclick=()=>X("companies"),document.getElementById("tab-jobs").onclick=()=>X("jobs"),document.getElementById("q").oninput=N,document.getElementById("fdrop-verdict-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.dataset.toggle==="flagged")z=!z,K(t,z);else if(t.dataset.toggle==="enriched")O=!O,K(t,O);else if(t.hasAttribute("data-v")){const n=t.getAttribute("data-v");A.has(n)?A.delete(n):A.add(n),K(t,A.has(n))}else return;N()}}),document.getElementById("fdrop-columns-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item[data-col]");if(!t)return;const n=Ke(),a=t.getAttribute("data-col");n.hidden.has(a)?n.hidden.delete(a):n.hidden.add(a),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),K(t,!n.hidden.has(a)),se(),Ot()}),document.getElementById("jq").oninput=_;for(const e of["fdrop-verdict","fdrop-columns","fdrop-application","fdrop-outreach"]){const t=document.getElementById(e),n=t.querySelector(".fdrop-btn");n.addEventListener("click",a=>{a.stopPropagation();const s=t.classList.contains("is-open");Qe(),s||(t.classList.add("is-open"),n.setAttribute("aria-expanded","true"))}),t.querySelector(".fdrop-menu").addEventListener("click",a=>a.stopPropagation())}document.addEventListener("click",Qe),document.getElementById("fdrop-application-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item[data-stage]");if(!t)return;const n=t.getAttribute("data-stage");I.has(n)?I.delete(n):I.add(n),K(t,I.has(n)),_()}),document.getElementById("fdrop-outreach-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.dataset.toggle==="nextup")ae=!ae,K(t,ae);else if(t.dataset.toggle==="notreached")oe=!oe,K(t,oe);else if(t.hasAttribute("data-status")){const n=t.getAttribute("data-status");J.has(n)?J.delete(n):J.add(n),K(t,J.has(n))}else return;_()}}),An(),qt(),se(),document.getElementById("pane-close").onclick=ve,document.getElementById("scrim").onclick=ve,document.getElementById("pursuit-close").onclick=he,document.getElementById("pursuit-scrim").onclick=he,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.querySelector(".fdrop.is-open")){Qe();return}if(document.getElementById("chat-pane").classList.contains("open")){Lt();return}if(ra()){It();return}if(document.getElementById("profile-scrim").classList.contains("open")){$t();return}if(document.getElementById("add-scrim").classList.contains("open")){Pe();return}if(document.getElementById("run-scrim").classList.contains("open")){Ne();return}if(document.getElementById("help-scrim").classList.contains("open")){Re();return}if(document.getElementById("relink-scrim").classList.contains("open")){ie();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){Le();return}if(document.getElementById("deljob-scrim").classList.contains("open")){Te();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(Ze==="pursuit"&&n){he();return}if(Ze==="company"&&t){ve();return}if(t){ve();return}he();return}if(document.getElementById("key-scrim").classList.contains("open")){Et();return}if(document.getElementById("sources-scrim").classList.contains("open")){mt();return}if(document.getElementById("editor-scrim").classList.contains("open")){Oe();return}if(document.getElementById("settings-scrim").classList.contains("open")){xt();return}});let gt=null;const Ys={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function xn(e){if(i.meta&&i.meta.control===!1){l("control surface disabled");return}gt=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Ys[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},a=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&a>0?(document.getElementById("run-warn-text").textContent=`${a} ${a===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${a===1?"it":"them"}. Run Enrich first to include ${a===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Ne(){document.getElementById("run-scrim").classList.remove("open"),gt=null}document.getElementById("btn-enrich").onclick=()=>xn("enrich"),document.getElementById("btn-verdict").onclick=()=>xn("verdict"),document.getElementById("run-cancel").onclick=Ne,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Ne()},document.getElementById("run-go").onclick=()=>{const e=gt,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(Ne(),!e)return;const a={};t&&(a.only_blanks=!0),n>0&&(a.workers=n),lt(e,a)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=Ds;const Gs={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function _n(e){const t=Gs[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const a=document.createElement("p");a.className="help-intro",a.textContent=t.intro,n.appendChild(a)}t.items.forEach(a=>{const s=document.createElement("div");s.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=a.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=a.desc;const d=document.createElement("a");d.className="help-link",d.textContent="Learn more →",d.onclick=()=>{Re(),Cn(),ca(a.sec)},s.appendChild(o),s.appendChild(c),s.appendChild(d),n.appendChild(s)}),document.getElementById("help-scrim").classList.add("open")}function Re(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>_n("add"),document.getElementById("help-run").onclick=()=>_n("run"),document.getElementById("help-close").onclick=Re,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Re()},document.getElementById("add-cancel").onclick=Pe,document.getElementById("add-save").onclick=$n,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Pe()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>vn(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",bn),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),$n()))}),document.getElementById("add-vertical-filter").addEventListener("input",wn),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&Ns(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=Vs,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=Oe,document.getElementById("editor-save").onclick=Ks,document.getElementById("editor-reset").onclick=Ws,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Oe()},document.getElementById("sources-close").onclick=mt,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&mt()},document.getElementById("sources-refresh-btn").onclick=Js,document.getElementById("key-cancel").onclick=Et,document.getElementById("key-save").onclick=Tn,document.getElementById("key-remove").onclick=aa,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&Et()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),Tn())}),document.getElementById("delcompany-cancel").onclick=Le,document.getElementById("delcompany-confirm").onclick=ss,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&Le()},document.getElementById("deljob-cancel").onclick=Te,document.getElementById("deljob-confirm").onclick=os,document.getElementById("deljob-scrim").onclick=e=>{e.target.id==="deljob-scrim"&&Te()},document.getElementById("relink-cancel").onclick=ie,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&ie()},document.getElementById("relink-search").addEventListener("input",e=>{nn(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&sn(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&sn(t.dataset.id)});function yt(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const a=Math.round(n/60);return a<48?`${a}h ago`:`${Math.round(a/24)}d ago`}async function vt(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}D()}const Q='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',ke='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',Qs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',Bn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',Zs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',Xs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',bt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',ea='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.6v2M8 12.4v2M14.4 8h-2M3.6 8h-2M12.5 3.5 11 5M5 11l-1.5 1.5M12.5 12.5 11 11M5 5 3.5 3.5"/><circle cx="8" cy="8" r="2.2"/></svg>',ta='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>';function P(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${r(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"";return`<div class="settings-item">
    ...
    <button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>
  </div>`}function D(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,a=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),s=t&&typeof t.body=="string";let o;if(a){let E="off",B="";const Ie=t&&t.criteria_state;Ie==="current"?(E="ok",B="current · verified "+yt(t.verified_age_seconds)):Ie==="changed"?(E="warn",B="changed — re-distill"):Ie==="unverified"?(E="warn",B=t&&!t.reachable&&s?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&s?(E="warn",B="brain offline · using cache"):s&&(E="ok",B="fetched "+yt(t.age_seconds)),o=P({icon:Bn,nameHTML:s?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:E,note:B,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:ke,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=P({icon:Bn,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:Q,actTitle:"edit taste.md",actLabel:"edit taste"});const c=i.sources&&i.sources.sources||[],d=c.filter(E=>E.need==="experience").length,u=c.filter(E=>E.need==="voice").length,p=c.filter(E=>E.need==="logistics").length;let m="off",f="not discovered yet — refresh from the brain";d>0?(m="ok",f=`${d} experience · ${u} voice · ${p} logistics`):c.length>0&&(m="warn",f="no experience yet — refresh");const k=c.length?'<span class="edit-link" data-act="view-sources" title="view discovered experience, voice + logistics">outreach knowledge</span>':"outreach knowledge",C=P({icon:ea,nameHTML:k,dot:m,note:f,desc:"Your experience, voice + logistics, discovered from the brain to ground outreach and application answers.",act:"refresh-sources",actID:"refresh-sources",actIcon:ke,actTitle:"re-discover experience, voice + logistics from the brain",actLabel:"refresh outreach knowledge"}),ne=P({icon:Zs,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:Q,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),ue=P({icon:Xs,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:Q,actTitle:"edit the outreach email template",actLabel:"edit email template"}),Ee=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],ze=Ee.map(([E,B,Ie])=>P({icon:bt,nameHTML:`<span class="edit-link" data-act="edit-prompt-${E}" title="edit the ${B.replace(/^\d+ · /,"")} prompt">${B}</span>`,desc:Ie,act:`edit-prompt-${E}`,actIcon:Q,actTitle:`edit the ${B} prompt`,actLabel:`edit ${B} prompt`})).join(""),$e=!i.stats||i.stats.taste_filter_enabled!==!1,$=P({icon:ta,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate before the LLM verdict — location, headcount, vertical, stage. Toggle it off in the editor to score every company.",dot:$e?"ok":"off",note:$e?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:Q,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),Z=i.anthropicKey;let Tt="off",Ct="not set — verdict, capture & outreach disabled";Z&&Z.key_source==="db"?(Tt="ok",Ct="set here · active"):Z&&Z.key_source==="env"&&(Tt="ok",Ct="from the environment");const ga=P({icon:Qs,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:Tt,note:Ct,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:Q,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),ya=P({icon:bt,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:Q,actTitle:"edit the application stages",actLabel:"edit application stages"}),va=P({icon:bt,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:Q,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${ga}
     </div>`;const St={"view-profile":()=>ia(i.profile),"refresh-profile":oa,"edit-taste":()=>G("taste"),"edit-taste-filter":()=>G("taste-filter"),"edit-application-stages":()=>G("application-stages"),"edit-outreach-statuses":()=>G("outreach-statuses"),"edit-playbook":()=>G("playbook"),"edit-template":()=>G("outreach-template"),"view-sources":ht,"refresh-sources":na,"edit-anthropic-key":sa};for(const[E]of Ee)St[`edit-prompt-${E}`]=()=>G(`outreach-prompts/${E}`);e.querySelectorAll("[data-act]").forEach(E=>{const B=E.dataset.act;B&&St[B]&&(E.onclick=St[B])})}async function wt(){try{i.sources=await(await fetch("/api/outreach/sources")).json()}catch{i.sources=null}D()}async function na(){const e=document.getElementById("refresh-sources");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/outreach/sources/refresh",{method:"POST"})}catch(a){l(`refresh failed: ${a.message}`),wt();return}if(!t.ok){const a=(await t.text().catch(()=>"")).trim();l(`refresh failed: ${a||"HTTP "+t.status}`),wt();return}const n=await t.json();n.warning?l(n.warning):l("outreach knowledge refreshed"),i.sources={sources:n.sources||[],needs:i.sources&&i.sources.needs||[]},D()}async function Ln(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}D()}async function sa(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await Ln(),kt();const e=document.getElementById("key-input");e&&e.focus()}function kt(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const a=document.getElementById("key-restart-hint");if(a){const s=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);a.style.display=s?"":"none"}}function Et(){document.getElementById("key-scrim").classList.remove("open")}async function Tn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){l("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let a;try{a=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(s){l(`save failed: ${s.message}`),n();return}if(!a.ok){l((await a.text().catch(()=>"")).trim()||`HTTP ${a.status}`),n();return}i.anthropicKey=await a.json(),document.getElementById("key-input").value="",n(),l("Anthropic key saved"),await ct(),kt(),D()}async function aa(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){l(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){l((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),l(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await ct(),kt(),D()}async function oa(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),vt();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),vt();return}i.profile=await t.json(),D(),l("company-fit brief refreshed"),w()}function ia(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${yt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function $t(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=$t,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&$t()};function Cn(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");De(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function It(){document.getElementById("docs-scrim").classList.remove("open")}function ra(){return document.getElementById("docs-scrim").classList.contains("open")}function De(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function ca(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),De(e)}document.getElementById("open-docs").onclick=Cn,document.getElementById("docs-close").onclick=It,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&It()};function da(){document.getElementById("settings-scrim").classList.add("open"),D()}function xt(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=da,document.getElementById("settings-close").onclick=xt,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&xt()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),De(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const a=n.filter(s=>s.isIntersecting).sort((s,o)=>s.boundingClientRect.top-o.boundingClientRect.top);a.length&&De(a[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function la(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function ua(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ve(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function _t(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function pa(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Sn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const a=la(n.content);if(n.role==="user")a&&t.appendChild(Ve("user",a));else if(n.role==="assistant"){const s=ua(n.content);if(!a&&!s.length)continue;const o=Ve("assistant",a);if(s.length){const c=document.createElement("div");c.className="chat-tools",c.textContent="· used "+s.join(", "),o.appendChild(c)}t.appendChild(o)}}t.children.length||t.appendChild(pa()),_t()}async function Bt(e,t,n){if(!i.meta||!i.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const a=document.getElementById("chat-messages");a.innerHTML='<div class="chat-empty">loading…</div>';const s=document.getElementById("chat-pane");s.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),s.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),c=await fetch("/api/chat/threads?"+o);if(!c.ok)throw new Error((await c.text().catch(()=>"")).trim()||"HTTP "+c.status);const d=await c.json();i.chat.threadId=d.thread.id,Sn(d.messages||[])}catch(o){a.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Lt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Ue(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function jn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Mn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",jn(),Ue(!0);const n=document.getElementById("chat-messages"),a=n.querySelector(".chat-empty");a&&a.remove(),n.appendChild(Ve("user",t));const s=Ve("assistant","");s.classList.add("chat-streaming"),n.appendChild(s),_t();let o="";const c=m=>{s.classList.remove("chat-streaming"),s.textContent="⚠ "+m,Ue(!1)},d=i.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){c(m.message);return}if(!u.ok){c((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");i.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,s.textContent=o,_t()}),p.addEventListener("end",async m=>{p.close(),i.chat.es===p&&(i.chat.es=null),s.classList.remove("chat-streaming"),Ue(!1),i.chat.threadId===d&&await ha(),ma(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),p.onerror=()=>{p.close(),i.chat.es===p&&(i.chat.es=null),s.classList.remove("chat-streaming"),Ue(!1)}}async function ha(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const a=await fetch("/api/chat/threads?"+n);if(!a.ok)return;const s=await a.json();Sn(s.messages||[])}catch{}}function ma(){x(),b(),w(),i.openId&&de(i.openId)}document.getElementById("open-chat").onclick=()=>Bt("global","",""),document.getElementById("chat-close").onclick=Lt,document.getElementById("chat-scrim").onclick=Lt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Mn()}),document.getElementById("chat-input").addEventListener("input",jn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Mn())}),Dt("#t tbody",qn),Dt("#jt tbody",On);const fa=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();X(fa==="jobs"?"jobs":"companies",{render:!1}),x(),b(),w(),ct(),dt(),vt(),wt(),Ln(),pe()}ka({"":{view:()=>({mount(v){v.innerHTML=_a,Ba()}}),chrome:!1}},{title:"scout"});Ia();xa();
