(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))E(o);new MutationObserver(o=>{for(const m of o)if(m.type==="childList")for(const $ of m.addedNodes)$.tagName==="LINK"&&$.rel==="modulepreload"&&E($)}).observe(document,{childList:!0,subtree:!0});function v(o){const m={};return o.integrity&&(m.integrity=o.integrity),o.referrerPolicy&&(m.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?m.credentials="include":o.crossOrigin==="anonymous"?m.credentials="omit":m.credentials="same-origin",m}function E(o){if(o.ep)return;o.ep=!0;const m=v(o);fetch(o.href,m)}})();function At(g,r){const v=r.replace(/^#/,"");let E=null;for(const o of Object.keys(g))(v===o||v.startsWith(o))&&(E===null||o.length>E.length)&&(E=o);return E===null&&""in g&&(E=""),E}function Ht(g){return typeof g=="function"?{view:g,chrome:!0}:{view:g.view,chrome:g.chrome!==!1}}function Pt(g,r={}){const v=r.root??document.body,E=r.title??document.title??"",o=r.brandHref??"#",m=document.createElement("main"),$=document.createElement("header");$.className="cap-head";const k=document.createElement("a");k.className="brand",k.href=o,k.textContent=E,k.setAttribute("aria-label",`${E} — home`),$.appendChild(k);const j=document.createElement("nav");j.className="cap-nav",j.setAttribute("aria-label","Views");for(const w of r.nav??[]){const x=document.createElement("a");x.href=w.href,x.textContent=w.label,w.ariaLabel&&x.setAttribute("aria-label",w.ariaLabel),j.appendChild(x)}$.appendChild(j);const I=document.createElement("section");I.className="tk-content",m.appendChild($),m.appendChild(I);const L=document.createElement("div");L.className="tk-bleed";const pe=w=>{var x;for(const C of Array.from(j.querySelectorAll("a"))){const q=((x=C.getAttribute("href"))==null?void 0:x.replace(/^#/,""))??"";C.toggleAttribute("aria-current",w!==null&&w!==""&&q===w),C.hasAttribute("aria-current")&&C.setAttribute("aria-current","page")}};let F=0;const T=()=>{const w=At(g,location.hash);if(pe(w),w===null){L.isConnected&&L.remove(),m.isConnected||v.appendChild(m),qt(I,"Not found.");return}const{view:x,chrome:C}=Ht(g[w]),q=C?I:L;C?(L.isConnected&&L.remove(),m.isConnected||v.appendChild(m)):(m.isConnected&&m.remove(),L.isConnected||v.appendChild(L)),q.replaceChildren();const he=x(),z=++F,R=he.mount(q);R instanceof Promise&&R.catch(W=>{z===F&&Ot(q,String(W))})};window.addEventListener("hashchange",T),T()}function qt(g,r){g.replaceChildren();const v=document.createElement("div");v.className="tk-empty",v.textContent=r,g.appendChild(v)}function Ot(g,r){g.replaceChildren();const v=document.createElement("div");v.className="tk-error",v.textContent=r,g.appendChild(v)}function Nt(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(r=>{for(const v of r)v.unregister()}),window.caches&&caches.keys().then(r=>{for(const v of r)caches.delete(v)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Rt(){let g;try{g=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!g.ok)return null;let r;try{r=await g.json()}catch{return null}return typeof r.email=="string"&&r.email?{email:r.email}:null}const Vt=`
<div class="layout">
<aside class="sidebar">
...
`;function Ut(g){const r={rows:[],sort:{k:"verdict",dir:1},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{k:"created_at",dir:1}},v=e=>"pill pill-"+(e||"none"),E='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',o=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),m=e=>/^https?:\/\//i.test(String(e??""))?o(e):"#";async function $(){const t=await(await fetch("/api/companies")).json();r.rows=t.rows||[],M()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}if(r.jobs=e.rows||[],B(),p.postingId){const t=r.jobs.find(n=>n.posting_id===p.postingId);t&&(p.row=t,document.getElementById("pursuit-pane").classList.contains("open")&&U())}}function j(e){r.view=e,document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",me(),e==="jobs"?B():M()}async function I(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){const n=document.getElementById("unscored-n");n.textContent="–",n.title=`stats failed: ${t.message}`;return}r.stats=e,L()}function L(){const e=r.stats||{};document.getElementById("unscored-n").textContent=e.unscored??0,Ee()}function pe(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[s]??3)-(i[a]??3)}return String(s).localeCompare(String(a))}function F(e){return e.slice().sort((t,n)=>r.sort.dir*pe(t,n,r.sort.k))}const T=new Set;let w=!1;function x(){document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.classList.toggle("is-on",T.has(e.dataset.v))})}function C(){const e=document.getElementById("q").value.trim().toLowerCase();return r.rows.filter(t=>!(T.size&&!T.has(t.verdict||"__none__")||w&&!t.flagged||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const q=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],he=[{k:"applied",label:"applied"},{k:"response",label:"response"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function z(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const R=z("scout-hidden-cols"),W=z("scout-hidden-jcols");function Ie(){return r.view==="jobs"?{cols:he,hidden:W,key:"scout-hidden-jcols"}:{cols:q,hidden:R,key:"scout-hidden-cols"}}function K(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=R.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=W.has(e.dataset.col)?"none":""})}function me(){const e=Ie();document.getElementById("col-toggles").innerHTML=e.cols.map(t=>`<button class="col-chip${e.hidden.has(t.k)?"":" is-on"}" data-col="${t.k}" title="${e.hidden.has(t.k)?"show":"hide"} ${t.label}">${t.label}</button>`).join(""),document.querySelectorAll("#col-toggles .col-chip").forEach(t=>{t.addEventListener("click",()=>{const n=Ie(),s=t.dataset.col;n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),me(),K()})})}function M(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=F(C());document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=`
    ...
    `,e.appendChild(s)}K(),e.querySelectorAll("tr").forEach(n=>{n.addEventListener("click",s=>{s.target.closest("a, .flag-btn")||X(n.dataset.id)})}),e.querySelectorAll(".flag-btn").forEach(n=>{n.addEventListener("click",()=>Pe(n.dataset.id))})}const A=new Set;let Y=!1,O=!0;function nt(){const e=document.getElementById("jq").value.trim().toLowerCase(),t=O&&!A.has("rejected");return r.jobs.filter(n=>!(t&&n.response==="rejected"||A.size&&!A.has(n.response||"")||Y&&!n.next_up||e&&!(n.title+" "+n.company+" "+(n.location||"")+" "+(n.summary||"")+" "+(n.contacts||"")).toLowerCase().includes(e)))}const st=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function xe(e){const t=String(e||"").split(",").map(n=>n.trim()).filter(Boolean);return t.length?t.map(n=>{const s=n.match(st);return s?`<a href="mailto:${o(s[0])}" title="${o(n)}">${o(n)}</a>`:o(n)}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}const H={offer:{cls:"pill-yes",label:"offer",order:0},interview:{cls:"pill-info",label:"interview",order:1},screening:{cls:"pill-maybe",label:"screening call",order:2},"":{cls:"pill-none",label:"—",order:3},rejected:{cls:"pill-no",label:"rejected",order:4}};function at(e,t,n){var s,a;if(n==="verdict"){const i={yes:0,maybe:1,no:2,"":3};return(i[e.verdict]??3)-(i[t.verdict]??3)}if(n==="response")return(((s=H[e.response])==null?void 0:s.order)??3)-(((a=H[t.response])==null?void 0:a.order)??3);if(n==="outreach_count")return(t.outreach_count|0)-(e.outreach_count|0);if(n==="created_at"||n==="applied_at"||n==="last_outreach_at"){const i=e[n]||"",c=t[n]||"";return!i&&!c?0:i?c?String(c).localeCompare(String(i)):-r.jsort.dir:r.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function B(){const e=document.querySelector("#jt tbody");e.innerHTML="";const t=nt().sort((d,u)=>r.jsort.dir*at(d,u,r.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block";const n=O&&!A.has("rejected")?r.jobs.filter(d=>d.response==="rejected").length:0,s=document.getElementById("hidden-rej-n");s.textContent=n,s.style.display=n?"":"none";const a=r.jobs.filter(d=>d.next_up).length,i=document.getElementById("next-up-n");i.textContent=a,i.style.display=a?"":"none";const c=document.getElementById("jobs-hidden-note");c.style.display=n?"":"none",n&&(c.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{O=!1,document.getElementById("hide-rejected").classList.remove("is-on"),B()});for(const d of t){const u=H[d.response]||H[""],h=document.createElement("tr");h.dataset.id=d.posting_id;const y=[["","—"],["screening","screening"],["interview","interview"],["offer","offer"],["rejected","rejected"]].map(([b,ue])=>`<option value="${b}"${(d.response||"")===b?" selected":""}>${ue}</option>`).join("");h.innerHTML=`
      <td><span class="row-name">${o(d.title||d.company)}</span>${d.next_up?'<span class="draft-badge db-next" title="queued next up for outreach">next up</span>':""}${ot(d.outreach_draft_status)}
      ...
      <td class="small td-contacts" data-col="contacts">${xe(d.contacts)}</td>
      <td data-col="link"><a href="${m(d.url)}" target="_blank" rel="noopener">open ↗</a></td>
    `;const f=()=>new Date().toISOString().slice(0,10);h.querySelector(".jt-applied").onclick=()=>Q(d,{applied_at:d.applied_at?"":f()}),h.querySelector(".jt-resp").onchange=b=>Q(d,{response:b.target.value}),h.querySelector(".jt-inc").onclick=()=>Q(d,{outreach_count:(d.outreach_count||0)+1,last_outreach_at:f()}),h.querySelector(".jt-dec").onclick=()=>{const b=Math.max(0,(d.outreach_count||0)-1);Q(d,{outreach_count:b,...b===0?{last_outreach_at:""}:{}})},e.appendChild(h)}K(),e.querySelectorAll("tr").forEach(d=>{d.addEventListener("click",u=>{u.target.closest("a, button, select")||Le(d.dataset.id)})})}function ot(e){return e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const p={postingId:null,row:null,drafts:[],poll:null,openHist:!1};async function Le(e){let t=r.jobs.find(n=>n.posting_id===e);if(t||(await k(),t=r.jobs.find(n=>n.posting_id===e)),!t){l("posting not found — refresh");return}ve(),p.postingId=e,p.row=t,p.drafts=[],p.openHist=!1,document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),U(),V()}function fe(){ve(),p.postingId=null,p.row=null,p.drafts=[],document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function ve(){p.poll&&(clearInterval(p.poll),p.poll=null)}async function V(){if(!p.postingId)return;let e;try{const n=await fetch(`/api/postings/${p.postingId}/outreach`);if(!n.ok){D();return}e=await n.json()}catch{D();return}p.drafts=e.drafts||[],D();const t=p.drafts[0];t&&t.status==="researching"?it():ve()}function it(){p.poll||(p.poll=setInterval(V,4e3))}function G(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(i){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),l(`save failed: ${i.message}`)}})}async function je(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",summary:e.summary||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const i=await a.json();Object.assign(e,{title:i.title,location:i.location,summary:i.summary,employment_type:i.employment_type,workplace_type:i.workplace_type,department:i.department,comp_range:i.comp_range,description:i.description}),B()}async function Te(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const i=await a.json();Object.assign(e,{name:i.name,headcount:i.headcount,funding_stage:i.funding_stage,location:i.location,vertical:i.vertical}),$(),k()}function U(){const e=p.row;if(!e)return;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${o(e.title||"")}">`;const t=H[e.response]||H[""];document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${t.cls}">${o(t.label)}</span>`+(e.verdict?` <span class="${v(e.verdict)}">${o(e.verdict)}</span>`:""),document.getElementById("pursuit-body").innerHTML=`
  ...
  `,dt();const n=document.getElementById("pursuit-view-company");n&&n.addEventListener("click",()=>X(e.company_id)),G(document.getElementById("pursuit-title-input"),s=>je(e,"title",s)),document.querySelectorAll("#role-body [data-k]").forEach(s=>G(s,a=>je(e,s.dataset.k,a),{multiline:s.tagName==="TEXTAREA"})),D()}function rt(e){return`
    ...
    <div class="outreach-contacts">
      ...
      <div class="oc-rendered">${xe(t.contacts)}</div>
    </div>`,d=s&&(lt(s.status)||s.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${s?"Draft again":"Draft outreach"}</button>`,u=a.length?`
    <details class="draft-history" ${p.openHist?"open":""}>
      <summary>${a.length} earlier draft${a.length>1?"s":""}</summary>
      <div id="draft-history-body">${a.map(h=>Se(h,!0)).join("")}</div>
    </details>`:"";e.innerHTML=i+`<div id="draft-current">${s?Se(s,!1):""}</div><div class="draft-actions">${d}</div>`+u,pt()}function lt(e){return e==="researching"||e==="awaiting_review"||e==="no_hook"}function Se(e,t){const n=(e.updated_at||e.created_at||"").replace("T"," ").slice(0,16),s=(y,f)=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const y=ut(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      <div class="draft-sentbody">${o(Me(e)||"(empty)")}</div>
      ${Z(e)}
    </div>`;const a=Me(e),i=e.status==="no_hook",c=i?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let d="";if(i)try{d=JSON.parse(e.hook||"{}").reasoning||""}catch{}const u=i?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${d?" "+o(d):""}</div>`:"";if(t)return`<div class="draft-card ${i?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    ${Z(e)}
  </div>`}function Me(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function Z(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,h)=>h?`<div class="tr-line"><span class="tr-key">${u}:</span> ${o(String(h))}</div>`:"",i=n.role||{},c=Array.isArray(n.hooks)?n.hooks:[],d=c.map(u=>`
      <div class="tr-line">
    ...
    <button class="btn btn-primary" id="blocks-sync-btn">Sync blocks</button>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#blocks-sync-btn");a&&a.addEventListener("click",mt)}async function mt(){const e=document.getElementById("blocks-sync-btn");e&&(e.disabled=!0,e.textContent="Syncing…");let t;try{t=await fetch("/api/outreach/sync",{method:"POST"})}catch(n){l(`sync failed: ${n.message}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`sync failed: ${n||"HTTP "+t.status}`),e&&(e.disabled=!1,e.textContent="Sync blocks");return}l("blocks synced"),D()}async function ft(e){const t=document.getElementById(`draft-edit-${e}`);if(!t)return;let n;try{n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t.value})})}catch(c){l(`save failed: ${c.message}`);return}if(!n.ok){const c=(await n.text().catch(()=>"")).trim();l(`save failed: ${c||"HTTP "+n.status}`);return}const s=await n.json(),a=p.drafts.findIndex(c=>String(c.id)===String(e));a>=0&&(p.drafts[a]=s);const i=t.closest(".draft-card");if(i){const c=i.querySelector(".lint-chips"),d=Ae(s.lint);c?c.outerHTML=d||"":d&&t.insertAdjacentHTML("afterend",d)}l("saved")}async function vt(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(n){l(`failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`failed: ${n||"HTTP "+t.status}`);return}l("marked sent"),await V(),k()}async function X(e){r.openId=e;const t=document.getElementById("pane"),n=document.getElementById("scrim");t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';let s;try{const a=await fetch(`/api/companies/${e}`);if(!a.ok)throw new Error(`HTTP ${a.status}`);s=await a.json()}catch(a){document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${o(a.message)}</div>`;return}te(s),ne(e)}function ee(){r.openId=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function te(e){document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${o(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
  ...
  `;const f=document.getElementById("posting-add-btn");f&&f.addEventListener("click",()=>wt(e)),Oe(),document.querySelectorAll("#ve-pick .ve-opt").forEach(_=>{_.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(Be=>Be.classList.remove("is-on")),_.classList.add("is-on")})});const b=document.getElementById("ve-save-btn");b&&b.addEventListener("click",()=>bt(e)),G(document.getElementById("pane-title-input"),_=>Te(e,"name",_)),document.querySelectorAll("#facts-body [data-k]").forEach(_=>G(_,Be=>Te(e,_.dataset.k,Be)));const ue=document.getElementById("flag-toggle-btn");ue&&ue.addEventListener("click",()=>Pe(e.company_id));const Xe=document.getElementById("review-stamp-btn");Xe&&Xe.addEventListener("click",()=>yt(e.company_id));const et=document.getElementById("rescore-btn");et&&et.addEventListener("click",()=>ye("verdict",{company_ids:[e.company_id]}));const tt=document.getElementById("reenrich-btn");tt&&tt.addEventListener("click",()=>ye("enrich",{company_ids:[e.company_id]}))}function gt(e){const t=e.domain?`<a href="https://${o(e.domain)}" target="_blank" rel="noopener">${o(e.domain)} ↗</a>`:'<span class="muted">no domain</span>';return`
    <div class="ie-grid">
    ...
    </dl>`}async function yt(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(i){l(`failed: ${i.message}`),t&&(t.disabled=!1);return}if(!n.ok){const i=await n.text().catch(()=>"");l(`failed: HTTP ${n.status}${i?" — "+i:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=r.rows.find(i=>i.company_id===e);a&&(a.reviewed_at=s.reviewed_at,M()),r.openId===e&&(te(s),ne(e)),l("reviewed")}async function Pe(e){const t=r.rows.find(i=>i.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(i){l(`failed: ${i.message}`);return}if(!s.ok){const i=await s.text().catch(()=>"");l(`failed: HTTP ${s.status}${i?" — "+i:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,M()),r.openId===e&&(te(a),ne(e)),k(),l(a.flagged?"flagged":"unflagged")}async function bt(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){l("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(d){l(`save failed: ${d.message}`),a.disabled=!1;return}if(!i.ok){const d=await i.text().catch(()=>"");l(`save failed: HTTP ${i.status}${d?" — "+d:""}`),a.disabled=!1;return}const c=await i.json();te(c),ne(c.company_id),$(),I(),k(),l("verdict saved")}function qe(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(o).join(" · "),a=H[n.response]||H[""],i=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a.cls}">${o(a.label)}</span>`,`<span class="pt-meta">${n.applied_at?`applied ${o(n.applied_at)}`:"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${o(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function Oe(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||(ee(),Le(e.dataset.pid))})})}async function wt(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let i;try{i=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!i.ok){const u=await i.text().catch(()=>"");l(`add failed: HTTP ${i.status}${u?" — "+u:""}`),s.disabled=!1;return}const c=await i.json();e.postings=(e.postings||[]).filter(u=>u.id!==c.id),e.postings.unshift(c);const d=document.getElementById("postings-list");d&&(d.innerHTML=qe(e),Oe()),t.value="",n.value="",s.disabled=!1,k(),l("link added")}async function ne(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){se(`<div class="muted">Failed to load trail: ${o(s.message)}</div>`);return}if(!t.ok){se(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){se('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}se(n.map(kt).join(""))}function kt(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(o);return e.run_id&&t.push("run "+o(e.run_id.slice(0,8))),`
    <div class="trail-event">
    ...
    </div>`}function se(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let Ne;function l(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(Ne),Ne=setTimeout(()=>t.classList.remove("show"),2200)}r.meta={control:!1,brain:!1,verdict:!1};async function Et(){try{const n=await fetch("/api/meta");if(!n.ok)return;r.meta=await n.json()}catch{return}const e=r.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!r.meta.verdict,t.title=r.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable"}async function ge(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let ae=null;async function ye(e,t){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){l(`run failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(n.status===412){const a=await n.text();l(a.trim());return}if(!n.ok){l(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();ze(e,s)}async function $t(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){l(`upload failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(!n.ok){l(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();ze("ingest",s)}const _t=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let oe=[],P=new Set,S="company";function Re(e){S=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",Ve()}function be(){return!!r.meta.capture&&document.getElementById("add-enrich").checked}function Ve(){const e=document.getElementById("add-note");be()?e.innerHTML=S==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and summary — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=S==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function Bt(){_t.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",P=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!r.meta.capture,t.classList.toggle("disabled",!r.meta.capture),t.title=r.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",r.meta.capture||(e.checked=!1),Re(S);const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(r.rows||[]).map(s=>`<option value="${o(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const i=document.createElement("option");i.value=a,i.textContent=a,n.appendChild(i)}),oe=s.verticals||[]}catch{oe=[]}Ue()}function ie(){document.getElementById("add-scrim").classList.remove("open")}function Ue(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=oe.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${P.has(s)?" sel":""}" data-v="${o(s)}">${o(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;P.has(a)?P.delete(a):P.add(a),s.classList.toggle("sel"),De()}))):e.innerHTML=`<div class="none">${oe.length?"no match":"no verticals in the set yet"}</div>`,De()}function De(){const e=P.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Je(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Fe(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){l(S==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,be()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},i=f=>document.getElementById(f).value.trim(),c=be();let d,u;c?(d="/api/capture",u={url:Je(t),kind:S==="company"?"company_page":"job_posting",fields:S==="company"?{name:i("add-name"),location:i("add-location"),headcount:i("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...P].join(", ")}:{name:i("add-job-company"),title:i("add-title")}}):S==="company"?(d="/api/companies",u={website:t,name:i("add-name"),vertical:[...P].join(", "),location:i("add-location"),headcount:i("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(d="/api/postings",u={url:Je(t),title:i("add-title"),company:i("add-job-company")});let h;try{h=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(f){l(`add failed: ${f.message}`),a();return}if(!h.ok){let f=`HTTP ${h.status}`;try{const b=await h.text();try{f=JSON.parse(b).error||f}catch{f=b.trim()||f}}catch{}if(a(),h.status===409){l(f||"That company is already in the list."),e.focus(),e.select();return}l(`add failed: ${f}`);return}const y=await h.json();if(a(),c&&!y.company_id){l(y.note||"couldn't classify that page");return}if(ie(),$(),I(),k(),S==="job"){const f=y.posting&&y.posting.title||"job link";l(`tracking: ${f} @ ${y.company_name}${y.posting_updated?" (refreshed)":""}`),j("jobs")}else c?(l(y.company_created?`company added: ${y.company_name}`:`${y.company_name} is already in the list`),X(y.company_id)):l("company added")}function ze(e,t){ae=t;const n=document.getElementById("drawer"),s=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",s.innerHTML="",n.classList.add("open"),ge();const a=new EventSource(`/api/jobs/${t}/stream`),i=(c,d)=>{const u=document.createElement("div"),h=!d&&/^\s*warn:/i.test(c);u.className="ln"+(d?" ln-err":h?" ln-warn":""),u.textContent=h?c.replace(/^\s*warn:\s*/i,"⚠ "):c,s.appendChild(u),s.scrollTop=s.scrollHeight};a.addEventListener("line",c=>i(c.data,/error|failed/i.test(c.data))),a.addEventListener("end",c=>{a.close(),ae=null,i(`— ${c.data} —`,c.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",l(`${e} ${c.data}`),$(),I(),ge(),k(),r.openId&&X(r.openId)}),a.onerror=()=>{a.close()}}async function It(){if(ae)try{await fetch(`/api/jobs/${ae}/cancel`,{method:"POST"})}catch{}}let J=null;async function We(e){J=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+e+".md",document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="",t.classList.add("open");try{const s=await(await fetch(`/api/${e}`)).json();document.getElementById("editor-text").value=s.content||"",s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version)}catch(n){document.getElementById("editor-text").value="failed to load: "+n.message}}function re(){document.getElementById("editor-scrim").classList.remove("open"),J=null}async function xt(){if(!J)return;const e=document.getElementById("editor-text").value;let t;try{t=await fetch(`/api/${J}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e})})}catch(s){l(`save failed: ${s.message}`);return}if(!t.ok){l(`save failed: HTTP ${t.status}`);return}const n=await t.json();n.taste_version&&(document.getElementById("editor-ver").textContent="version "+n.taste_version),l(`${J}.md saved`),re(),I()}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{const t=e.dataset.k;r.sort.k===t?r.sort.dir*=-1:(r.sort.k=t,r.sort.dir=1),M()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{const t=e.dataset.jk;r.jsort.k===t?r.jsort.dir*=-1:(r.jsort.k=t,r.jsort.dir=1),B()}}),document.getElementById("tab-companies").onclick=()=>j("companies"),document.getElementById("tab-jobs").onclick=()=>j("jobs"),document.getElementById("q").oninput=M,document.querySelectorAll("#verdict-chips .v-chip[data-v]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.v;T.has(t)?T.delete(t):T.add(t),x(),M()})}),document.getElementById("flag-filter").addEventListener("click",e=>{w=!w,e.currentTarget.classList.toggle("is-on",w),M()}),document.getElementById("jq").oninput=B,document.getElementById("hide-rejected").addEventListener("click",e=>{O=!O,e.currentTarget.classList.toggle("is-on",O),B()}),document.querySelectorAll("#response-chips .v-chip[data-r]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.r;A.has(t)?A.delete(t):A.add(t),e.classList.toggle("is-on",A.has(t)),B()})}),document.getElementById("next-up-filter").addEventListener("click",e=>{Y=!Y,e.currentTarget.classList.toggle("is-on",Y),B()}),me(),K(),document.getElementById("pane-close").onclick=ee,document.getElementById("scrim").onclick=ee,document.getElementById("pursuit-close").onclick=fe,document.getElementById("pursuit-scrim").onclick=fe,document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(St()){_e();return}if(document.getElementById("profile-scrim").classList.contains("open")){$e();return}if(document.getElementById("add-scrim").classList.contains("open")){ie();return}if(document.getElementById("run-scrim").classList.contains("open")){de();return}if(document.getElementById("help-scrim").classList.contains("open")){ce();return}if(document.getElementById("pane").classList.contains("open")){ee();return}if(document.getElementById("pursuit-pane").classList.contains("open")){fe();return}document.getElementById("editor-scrim").classList.contains("open")&&re()}});let we=null;const Lt={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Ke(e){if(r.meta&&r.meta.control===!1){l("control surface disabled");return}we=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Lt[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=r.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function de(){document.getElementById("run-scrim").classList.remove("open"),we=null}document.getElementById("btn-enrich").onclick=()=>Ke("enrich"),document.getElementById("btn-verdict").onclick=()=>Ke("verdict"),document.getElementById("run-cancel").onclick=de,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&de()},document.getElementById("run-go").onclick=()=>{const e=we,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(de(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),ye(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=Bt;const jt={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Ye(e){const t=jt[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const i=document.createElement("div");i.className="help-item-name",i.textContent=s.name;const c=document.createElement("div");c.className="help-item-desc",c.textContent=s.desc;const d=document.createElement("a");d.className="help-link",d.textContent="Learn more →",d.onclick=()=>{ce(),Ze(),Mt(s.sec)},a.appendChild(i),a.appendChild(c),a.appendChild(d),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function ce(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Ye("add"),document.getElementById("help-run").onclick=()=>Ye("run"),document.getElementById("help-close").onclick=ce,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&ce()},document.getElementById("add-cancel").onclick=ie,document.getElementById("add-save").onclick=Fe,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&ie()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Re(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Ve),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Fe()))}),document.getElementById("add-vertical-filter").addEventListener("input",Ue),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&$t(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=It,document.getElementById("drawer-close").onclick=()=>document.getElementById("drawer").classList.remove("open"),document.getElementById("editor-cancel").onclick=re,document.getElementById("editor-save").onclick=xt,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&re()};function Ge(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function ke(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);r.profile=await e.json()}catch{r.profile=null}Ee()}const Qe='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>';function Ee(){const e=document.getElementById("criteria-stats");if(!e)return;const t=r.profile,n=r.stats&&r.stats.stale_verdicts||0,a=(t&&t.active_source||r.stats&&r.stats.taste_source||"").startsWith("brain:"),i=t&&typeof t.body=="string";let c="";if(a){let f="off",b="";t&&!t.reachable&&i?(f="warn",b="brain offline · using cache"):t&&t.stale?(f="warn",b="cached · stale"):i&&(f="ok",b="fetched "+Ge(t.age_seconds)),c+=`<div class="crit-row">
      ...
      <button class="crit-edit" id="edit-taste" title="edit taste.md" aria-label="edit taste">${Qe}</button></div>`,t&&t.configured&&(c+='<div class="crit-note dim small">brain offline — local fallback</div>');c+=`<div class="crit-row">
    <span class="crit-what">playbook</span>
    <button class="crit-edit" id="edit-playbook" title="edit playbook.md" aria-label="edit playbook">${Qe}</button></div>`,n>0&&(c+=`<div class="stale">
      ...
      ${n} verdicts stale</div>`),e.innerHTML=c;const d=document.getElementById("view-profile");d&&(d.onclick=()=>Ct(r.profile));const u=document.getElementById("refresh-profile");u&&(u.onclick=Tt);const h=document.getElementById("edit-taste");h&&(h.onclick=()=>We("taste"));const y=document.getElementById("edit-playbook");y&&(y.onclick=()=>We("playbook"))}async function Tt(){const e=document.getElementById("refresh-profile");e&&(e.textContent="refreshing…",e.style.pointerEvents="none");let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),ke();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),ke();return}r.profile=await t.json(),Ee(),l("brain profile refreshed"),I()}function Ct(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${Ge(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function $e(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=$e,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&$e()};function Ze(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");le(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function _e(){document.getElementById("docs-scrim").classList.remove("open")}function St(){return document.getElementById("docs-scrim").classList.contains("open")}function le(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Mt(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),le(e)}document.getElementById("open-docs").onclick=Ze,document.getElementById("docs-close").onclick=_e,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&_e()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),le(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,i)=>a.boundingClientRect.top-i.boundingClientRect.top);s.length&&le(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),$(),k(),I(),Et(),ge(),ke()}Pt({"":{view:()=>({mount(g){g.innerHTML=Vt,Ut()}}),chrome:!1}},{title:"scout"});Nt();Rt();
