...
func TestPostingTrackingAPI(t *testing.T) {
	...
	rec := put(p.ID, `{"application_status":"screening","outreach_status":"initial contact"}`)
	if rec.Code != http.StatusOK {
	...
	if got.ApplicationStatus != "screening" || got.OutreachStatus != "initial contact" || got.OutreachCount != 0 {
