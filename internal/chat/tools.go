	...
	defs := []struct {
		...
		},
		{
			anthropic.ToolDef{
				...
				Description: "Update a posting's application-tracking fields. Call this when the user reports applying, advancing a stage (heard back / screening / interview / offer / rejected), doing outreach, or adding a contact. Passing `stage` sets the posting's current application stage. Only the fields you pass are changed; omit the rest. Get the posting_id from capture_link or search.",
				InputSchema: objSchema(map[string]any{
