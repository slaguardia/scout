(function(){const v=document.createElement("link").relList;if(v&&v.supports&&v.supports("modulepreload"))return;for(const g of document.querySelectorAll('link[rel="modulepreload"]'))l(g);new MutationObserver(g=>{for(const S of g)if(S.type==="childList")for(const c of S.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&l(c)}).observe(document,{childList:!0,subtree:!0});function $(g){const S={};return g.integrity&&(S.integrity=g.integrity),g.referrerPolicy&&(S.referrerPolicy=g.referrerPolicy),g.crossOrigin==="use-credentials"?S.credentials="include":g.crossOrigin==="anonymous"?S.credentials="omit":S.credentials="same-origin",S}function l(g){if(g.ep)return;g.ep=!0;const S=$(g);fetch(g.href,S)}})();function ko(w,v){const $=v.replace(/^#/,"");let l=null;for(const g of Object.keys(w))($===g||$.startsWith(g))&&(l===null||g.length>l.length)&&(l=g);return l===null&&""in w&&(l=""),l}function Eo(w){return typeof w=="function"?{view:w,chrome:!0}:{view:w.view,chrome:w.chrome!==!1}}function $o(w,v={}){const $=v.root??document.body,l=v.title??document.title??"",g=v.brandHref??"#",S=document.createElement("main"),c=document.createElement("header");c.className="cap-head";const P=document.createElement("a");P.className="brand",P.href=g,P.textContent=l,P.setAttribute("aria-label",`${l} — home`),c.appendChild(P);const H=document.createElement("nav");H.className="cap-nav",H.setAttribute("aria-label","Views");for(const q of v.nav??[]){const L=document.createElement("a");L.href=q.href,L.textContent=q.label,q.ariaLabel&&L.setAttribute("aria-label",q.ariaLabel),H.appendChild(L)}c.appendChild(H);const k=document.createElement("section");k.className="tk-content",S.appendChild(c),S.appendChild(k);const U=document.createElement("div");U.className="tk-bleed";const ee=q=>{var L;for(const j of Array.from(H.querySelectorAll("a"))){const te=((L=j.getAttribute("href"))==null?void 0:L.replace(/^#/,""))??"";j.toggleAttribute("aria-current",q!==null&&q!==""&&te===q),j.hasAttribute("aria-current")&&j.setAttribute("aria-current","page")}};let be=0;const je=()=>{const q=ko(w,location.hash);if(ee(q),q===null){U.isConnected&&U.remove(),S.isConnected||$.appendChild(S),xo(k,"Not found.");return}const{view:L,chrome:j}=Eo(w[q]),te=j?k:U;j?(U.isConnected&&U.remove(),S.isConnected||$.appendChild(S)):(S.isConnected&&S.remove(),U.isConnected||$.appendChild(U)),te.replaceChildren();const tt=L(),Me=++be,we=tt.mount(te);we instanceof Promise&&we.catch(V=>{Me===be&&Io(te,String(V))})};window.addEventListener("hashchange",je),je()}function xo(w,v){w.replaceChildren();const $=document.createElement("div");$.className="tk-empty",$.textContent=v,w.appendChild($)}function Io(w,v){w.replaceChildren();const $=document.createElement("div");$.className="tk-error",$.textContent=v,w.appendChild($)}function So(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(v=>{for(const $ of v)$.unregister()}),window.caches&&caches.keys().then(v=>{for(const $ of v)caches.delete($)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function _o(){let w;try{w=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!w.ok)return null;let v;try{v=await w.json()}catch{return null}return typeof v.email=="string"&&v.email?{email:v.email}:null}const To=`
<div class="layout">
<aside class="sidebar">
...
`;function Lo(w){const v={k:"verdict",dir:1},$={k:"created_at",dir:1},l={rows:[],sort:{...v},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...$},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null,gmail:null,notifications:{notifications:[],unread:0,followups:[]},settingsGroup:"outreach"},g=e=>"pill pill-"+(e||"none"),S='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',c=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),P=e=>/^https?:\/\//i.test(String(e??""))?c(e):"#";async function H(){const t=await(await fetch("/api/companies")).json();l.rows=t.rows||[],F()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}l.jobs=e.rows||[],M(),U(),be()}function U(){if(!h.postingId)return;const e=l.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&se())}let ee=null;function be(){const e=l.jobs.some(t=>t.outreach_draft_status==="researching");e&&!ee?ee=setInterval(je,4e3):!e&&ee&&(clearInterval(ee),ee=null)}async function je(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(l.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==l.jobs.length;l.jobs=t,s&&(M(),U()),be()}async function q(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(l.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(l.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(l.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(l.followupTemplate=e.content)}).catch(()=>{})]),ot(),l.view==="jobs"&&M()}function L(e,{render:t=!0}={}){l.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("tab-inbox").classList.toggle("active",e==="inbox"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none";const n=(i,d)=>{const u=document.getElementById(i);u&&(u.style.display=d?"":"none")};n("settings-view",e==="settings"),n("inbox-view",e==="inbox"),n("docs-view",e==="docs"),document.getElementById("open-settings").classList.toggle("is-active",e==="settings");const s=document.getElementById("open-docs");s&&s.classList.toggle("is-active",e==="docs");const a=e==="companies"||e==="jobs";document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none";const o=document.getElementById("block-columns");o&&(o.style.display=a?"":"none"),Ft(),t&&(e==="jobs"?M():e==="settings"?G():e==="inbox"?(Yn(),oe()):e==="docs"?eo():F())}async function j(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}l.stats=e,te()}function te(){G()}function tt(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function Me(e){return e.slice().sort((t,n)=>l.sort.dir*tt(t,n,l.sort.k))}function we(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(s=>{s.getAttribute(t)===n.k?s.dataset.sort=n.dir<0?"desc":"asc":delete s.dataset.sort})}const V=new Set;let ie=!1,ce=!1;const ts=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function ns(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+ts.map(([t,n,s])=>ne("data-v",t,n,s,V.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+ne("data-toggle","flagged","⚑ Flagged","",ie)+ne("data-toggle","enriched","Enriched","",ce),Rt())}function Rt(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const i of l.rows){const d=i.verdict||"__none__";e[d]=(e[d]|0)+1,i.flagged&&t++,i.enriched&&n++}it("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=V.size+(ie?1:0)+(ce?1:0);en("fdrop-cfilters-btn",o,o>0)}function Dt(){const e=document.getElementById("q").value.trim().toLowerCase();return l.rows.filter(t=>!(V.size&&!V.has(t.verdict||"__none__")||ie&&!t.flagged||ce&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const ss=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],as=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Ut(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Vt=Ut("scout-hidden-cols"),Jt=Ut("scout-hidden-jcols");function nt(){return l.view==="jobs"?{cols:as,hidden:Jt,key:"scout-hidden-jcols"}:{cols:ss,hidden:Vt,key:"scout-hidden-cols"}}function le(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Vt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Jt.has(e.dataset.col)?"none":""})}function Ft(){const e=nt(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>ne("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Gt())}function Gt(){const e=nt(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function zt(e){return`
    ...
    `}function Wt(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>Tn(t.dataset.id))}const os=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],is=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function Kt(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let i=0;i<n;i++){const d=t.map(([u,p])=>{const m=p.endsWith("%")?Math.round(parseFloat(p)*a[i%a.length])+"%":p;return`<td${u?` data-col="${u}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${d}</tr>`}s.innerHTML=o,le()}function F(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=Me(Dt());Rt(),document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=zt(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||ge(s.dataset.id)}),e.appendChild(s),Wt(s)}we("t","data-k",l.sort),le()}async function cs(e){const n=await(await fetch("/api/companies")).json();l.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=Me(Dt()).map(i=>i.company_id),o=[...s.querySelectorAll("tr")].map(i=>i.dataset.id);if(a.length!==o.length||a.some((i,d)=>i!==o[d])){F();return}for(const i of e){const d=l.rows.find(p=>p.company_id===i),u=s.querySelector(`tr[data-id="${CSS.escape(i)}"]`);if(!d||!u){F();return}u.innerHTML=zt(d),Wt(u)}le()}let E=null,st=null,re=!1,de=!1,_=null,at=null;function Yt(){const e=l.applicationStages;if(E===null)E=new Set(["",...e]);else{for(const t of[...E])t!==""&&!e.includes(t)&&E.delete(t);if(st)for(const t of e)st.has(t)||E.add(t)}st=new Set(e)}function Zt(){const e=l.outreachStatuses;if(_===null)_=new Set(["",...e]);else{for(const t of[..._])t!==""&&!e.includes(t)&&_.delete(t);if(at)for(const t of e)at.has(t)||_.add(t)}at=new Set(e)}function ls(){Yt(),Zt();const e=document.getElementById("jq").value.trim().toLowerCase();return l.jobs.filter(t=>{const n=t.application_status||"";return!(!E.has(n)||re&&!t.next_up||de&&!(t.followups_due|0)||!_.has(t.outreach_status||"")||e&&!(t.title+" "+t.company+" "+(t.location||"")+" "+(t.description||"")+" "+(t.contacts||"")).toLowerCase().includes(e))})}function ne(e,t,n,s,a){return`<button class="fdrop-item${a?" is-checked":""}" ${e}="${c(t)}" role="menuitemcheckbox" aria-checked="${a}"><span class="fdrop-check" aria-hidden="true"></span>`+(s?`<span class="fdrop-dot ${s}"></span>`:"")+`<span class="fdrop-label">${c(n)}</span><span class="fdrop-item-count" data-count></span></button>`}function Qt(e,t,n){return`<div class="fdrop-head fdrop-head--toggle"><span>${e}</span><button type="button" class="fdrop-all" data-all="${t}">${n?"none":"all"}</button></div>`}function ot(){Yt(),Zt();const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=["",...l.applicationStages],n=["",...l.outreachStatuses];e.innerHTML=Qt("Application stage","stage",t.every(s=>E.has(s)))+ne("data-stage","","not applied","",E.has(""))+l.applicationStages.map(s=>ne("data-stage",s,s,Pe(s),E.has(s))).join("")+'<div class="fdrop-sep"></div>'+Qt("Reply status","status",n.every(s=>_.has(s)))+[["","not reached out",""]].concat(l.outreachStatuses.map(s=>[s,s,cn(s)])).map(([s,a,o])=>ne("data-status",s,a,o,_.has(s))).join(""),Xt()}function Xt(){const e={},t={};for(const d of l.jobs){const u=d.application_status||"";e[u]=(e[u]|0)+1;const p=d.outreach_status||"";t[p]=(t[p]|0)+1}it("#fdrop-jfilters-menu [data-stage]","data-stage",e),it("#fdrop-jfilters-menu [data-status]","data-status",t);const n=["",...l.applicationStages],s=E&&E.size===n.length&&n.every(d=>E.has(d)),a=["",...l.outreachStatuses],o=_&&_.size===a.length&&a.every(d=>_.has(d)),i=(s?0:E?E.size:0)+(o?0:_?_.size:0);en("fdrop-jfilters-btn",i,i>0)}function it(e,t,n){document.querySelectorAll(e).forEach(s=>{const a=s.querySelector("[data-count]");if(a){const o=n[s.getAttribute(t)]|0;a.textContent=o||""}})}function en(e,t,n){const s=document.getElementById(e);if(!s)return;s.classList.toggle("is-active",n);const a=s.querySelector(".fdrop-count");if(a){const o=n&&t>0;a.textContent=o?t:"",a.style.display=o?"":"none"}}function ue(e,t){e.classList.toggle("is-checked",t),e.setAttribute("aria-checked",String(t))}function rs(){const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=e.querySelector('.fdrop-all[data-all="stage"]'),n=e.querySelector('.fdrop-all[data-all="status"]');t&&(t.textContent=["",...l.applicationStages].every(s=>E.has(s))?"none":"all"),n&&(n.textContent=["",...l.outreachStatuses].every(s=>_.has(s))?"none":"all")}function ct(){document.querySelectorAll(".fdrop.is-open").forEach(e=>{e.classList.remove("is-open");const t=e.querySelector(".fdrop-btn");t&&t.setAttribute("aria-expanded","false")})}function tn(e){const t=e.querySelector(".fdrop-btn"),n=e.querySelector(".fdrop-menu");if(!t||!n)return;const s=t.getBoundingClientRect();n.style.left=Math.round(s.left)+"px",n.style.top=Math.round(s.bottom+4)+"px",n.style.minWidth=Math.round(s.width)+"px",n.style.maxHeight=Math.max(160,Math.round(window.innerHeight-s.bottom-12))+"px"}function ds(e){const t=e.querySelector(".fdrop-btn");e.classList.add("is-open"),t&&t.setAttribute("aria-expanded","true"),tn(e)}function nn(){const e=document.querySelector(".fdrop.is-open");e&&tn(e)}window.addEventListener("scroll",nn,!0),window.addEventListener("resize",nn);const us=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function ps(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(us),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}const Ae=()=>new Date().toISOString().slice(0,10);function sn(e){const t=[["","none"]];for(const n of l.applicationStages)t.push([n,n]);return e&&!l.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function an(e){const t=[["","none"]];for(const n of l.outreachStatuses)t.push([n,n]);return e&&!l.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const ms=8;function on(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%ms}function Pe(e){return on(e,l.applicationStages)}function cn(e){return on(e,l.outreachStatuses)}function hs(e){const t=ps(e);return t.length?t.map(n=>{const s=c(n.position||n.email);if(!n.email)return s;const a=c(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${c(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function ln(e){const t=e.application_status||"";if(!t)return l.applicationStages.length+1;const n=l.applicationStages.indexOf(t);return n<0?l.applicationStages.length:n}function fs(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return ln(e)-ln(t);if(n==="followups_due")return(t.followups_due|0)-(e.followups_due|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-l.jsort.dir:l.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function gs(){const e=document.getElementById("jobs-followup-nav");if(!e)return;let t=0,n=0;for(const a of l.jobs)a.next_up&&t++,n+=a.followups_due|0;if(t||(re=!1),n||(de=!1),!t&&!n){e.style.display="none",e.innerHTML="";return}e.style.display="";const s=[];t&&s.push(`<button class="queue-nav-btn queue-nav-btn--nextup${re?" is-active":""}" data-q="nextup" title="${re?"showing only these — click to show all jobs":"show only jobs queued next up for outreach"}"><span class="fn-icon">${qa}</span><span class="fn-text"><strong>${t}</strong> next up</span></button>`),n&&s.push(`<button class="queue-nav-btn${de?" is-active":""}" data-q="due" title="${de?"showing only these — click to show all jobs":"show only jobs owing a follow-up"}"><span class="fn-icon">${Gn}</span><span class="fn-text"><strong>${n}</strong> follow-up${n>1?"s":""} due</span></button>`),e.innerHTML=s.join(""),e.querySelectorAll(".queue-nav-btn").forEach(a=>{a.onclick=()=>{a.dataset.q==="nextup"?re=!re:de=!de,M()}})}function M(){const e=document.querySelector("#jt tbody");e.innerHTML="",gs();const t=ls().sort((a,o)=>l.jsort.dir*fs(a,o,l.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",Xt();const n=E&&!E.has("rejected")?l.jobs.filter(a=>(a.application_status||"")==="rejected").length:0,s=document.getElementById("jobs-hidden-note");s.style.display=n?"":"none",n&&(s.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{E.add("rejected"),ot(),M()});for(const a of t){const o=a.application_status||"",i=document.createElement("tr");i.dataset.id=a.posting_id;const d=sn(o).map(([m,f])=>`<option value="${c(m)}"${o===m?" selected":""}>${c(f)}</option>`).join(""),u=a.outreach_status||"",p=an(u).map(([m,f])=>`<option value="${c(m)}"${u===m?" selected":""}>${c(f)}</option>`).join("");i.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${a.next_up?" is-on":""}" title="${a.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 12.5v-9M4.5 7L8 3.5 11.5 7"/></svg></button><div class="jt-namecol"><span class="row-name">${c(a.title||a.company)}</span>${ys(a.outreach_draft_status)}${a.title?`<div class="small dim">${c(a.company)}</div>`:""}</div></div></td>
      <td data-col="application"><div class="jt-stage"><select class="jt-stage-sel ${Pe(o)}" title="application stage">${d}</select>${o&&a.application_status_at?`<span class="jt-stage-at" title="stage last changed">${c(a.application_status_at.slice(0,10))}</span>`:""}</div></td>
    ...
    <section class="pane-section">
      ...
      <div class="company-row">
        ...
        ${e.verdict?`<span class="role-verdict"><span class="role-verdict-label">fit</span><span class="${g(e.verdict)}" title="scout's company-fit verdict">${c(e.verdict)}</span></span>`:""}
      </div>
    </section>
  ...
  `,Ms();const i=document.getElementById("pursuit-company-link");i&&i.addEventListener("click",()=>ge(e.company_id)),ks(e),J(document.getElementById("pursuit-title-input"),p=>dn(e,"title",p)),J(document.getElementById("pursuit-url-input"),p=>bs(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>ws(e,d)),J(document.getElementById("pursuit-notes-input"),p=>As(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>J(p,m=>dn(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>Ss(e)),he(),Ie(),t&&(t.scrollTop=s),hn=e.posting_id}function js(e){return`
  ...
  </div>`}function Os(){const e=h.row,t=Hs(),n=e.last_outreach_at?`<div class="outreach-meta"><span>last outreach ${c(e.last_outreach_at)}</span></div>`:"";if(!h.contactsLoaded)return`<div class="contacts-mgr">${t}${n}<div class="loading-row"><span class="spinner"></span><span>loading contacts…</span></div></div>`;const s=h.contacts.map(Ns).join(""),a=h.contacts.length?"":`<div class="cc-empty dim">No contacts yet — add the people you're reaching out to at ${c(e.company)}.</div>`;return`<div class="contacts-mgr">
    ${t}
    ${n}
  ...
  </div>`}function Ns(e){const t=h.outreach.filter(o=>o.contact_id===e.id),n=t[0]||null,s=e.role?`<span class="cc-role">${c(e.role)}</span>`:"",a=e.email?`<a class="cc-mail" href="mailto:${c(e.email)}" title="${c(e.email)}">${c(e.email)}</a>`:"";return`<div class="contact-card" data-cid="${e.id}">
    <div class="cc-head">
    ...
    ${xe(e)}
  </div>`}function ft(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function xe(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,p)=>p?`<div class="tr-line"><span class="tr-key">${u}:</span> ${c(String(p))}</div>`:"",o=n.role||{},i=Array.isArray(n.hooks)?n.hooks:[],d=i.map(u=>`
      <div class="tr-line">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${c(aa(e.fail_reason))}</div>`:""}
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",Un);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",In)}function _n(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function ge(e){var d,u;const t=document.getElementById("pane"),n=document.getElementById("scrim"),s=l.openId===e&&t.classList.contains("open"),a=s?((d=document.getElementById("pane-body"))==null?void 0:d.scrollTop)??0:0,o=s?(u=document.getElementById("trace-body"))==null?void 0:u.innerHTML:null;l.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),rn("company"),s||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let i;try{const p=await fetch(`/api/companies/${e}`);if(!p.ok)throw new Error(`HTTP ${p.status}`);i=await p.json()}catch(p){s||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${c(p.message)}</div>`);return}if(l.openId===e){if(_e(i),s){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const p=document.getElementById("pane-body");p&&(p.scrollTop=a)}Te(e)}}function Se(){l.openId=null,l.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function _e(e){l.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${c(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
    <span class="${g(e.has_verdict?e.verdict:"")}">${c(e.has_verdict?e.verdict:"unscored")}</span>
  `;const t=document.getElementById("pane-chat");t&&(t.style.display=l.meta&&l.meta.chat?"":"none",t.onclick=()=>Ot("company",e.company_id,e.name));const n=e.model==="manual",s=e.has_verdict?`
    <dl class="kv">
  ...
  `:'<div class="muted">No enrichment yet. Run <code>scout enrich</code>.</div>',i=!l.meta||l.meta.control!==!1,d=i&&l.meta&&l.meta.verdict?'<button class="h3-action" id="rescore-btn" title="re-score just this company — replaces the current verdict, manual or not">↻ re-score</button>':"",u=i&&e.domain?'<button class="h3-action" id="reenrich-btn" title="re-fetch this company’s site now">↻ re-enrich</button>':"",p=Object.keys(e.raw_json||{}).sort(),m=p.length===0?"":`
    <details class="raw-json">
      ...
      <table><tbody>
        ${p.map(b=>`<tr><td class="k">${c(b)}</td><td>${c(e.raw_json[b])}</td></tr>`).join("")}
      </tbody></table>
    </details>
  `,f=`
    <div class="flag-bar">
  ...
  `;const y=document.getElementById("posting-add-btn");y&&y.addEventListener("click",()=>ha(e)),vt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(C=>C.classList.remove("is-on")),b.classList.add("is-on")})});const A=document.getElementById("ve-save-btn");A&&A.addEventListener("click",()=>ma(e)),J(document.getElementById("pane-title-input"),b=>mn(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>J(b,C=>mn(e,b.dataset.k,C))),J(document.getElementById("pane-domain-input"),b=>Bs(e,b)),J(document.getElementById("pane-notes-input"),b=>Cs(e,b),{multiline:!0});const x=document.getElementById("flag-toggle-btn");x&&x.addEventListener("click",()=>Tn(e.company_id));const I=document.getElementById("review-stamp-btn");I&&I.addEventListener("click",()=>pa(e.company_id));const B=document.getElementById("rescore-btn");B&&B.addEventListener("click",()=>kt("verdict",{company_ids:[e.company_id]}));const R=document.getElementById("reenrich-btn");R&&R.addEventListener("click",()=>kt("enrich",{company_ids:[e.company_id]}));const N=document.getElementById("company-delete-btn");N&&N.addEventListener("click",()=>xs(e))}function ua(e){return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ye(e,t){const n=l.openDetail;if(!n||!l.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=yt(n),vt())}function vt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||lt(e.dataset.pid)})})}async function ha(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){r("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){r(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");r(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const i=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==i.id),e.postings.unshift(i);const d=document.getElementById("postings-list");d&&(d.innerHTML=yt(e),vt()),t.value="",n.value="",s.disabled=!1,k(),r("link added")}async function Te(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){Re(`<div class="muted">Failed to load trail: ${c(s.message)}</div>`);return}if(!t.ok){Re(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Re('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Re(n.map(fa).join(""))}function fa(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(c);return e.run_id&&t.push("run "+c(e.run_id.slice(0,8))),`
...
`):a.content||""}catch{s.value=""}s.dataset.orig=s.value,s.dataset.loaded="1",s.addEventListener("blur",()=>Ra(e))}async function Ra(e){const t=e.dataset.kind,n=e.dataset.list==="1",s=e.querySelector(".set-textarea");if(s.dataset.loaded!=="1"||s.value===s.dataset.orig)return;const a=n?{statuses:s.value.split(/\r?\n/).map(i=>i.trim()).filter(Boolean)}:{content:s.value};let o;try{o=await fetch(`/api/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}catch(i){r(`save failed: ${i.message}`);return}if(!o.ok){r(`save failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}s.dataset.orig=s.value,jt(e),t==="followup-template"&&(l.followupTemplate=s.value),n&&(await q(),M())}function Mt(e){e.querySelectorAll(".set-field[data-kind]").forEach(Na)}function Da(e){e.innerHTML=Q("outreach-subject","Email subject","The send subject — {{role}} / {{company}} substitution, no LLM.",2,!1)+Q("outreach-template","Email body","Verbatim prose with the writer's fill-in holes.",16,!1)+Q("outreach-signature","Email signature","A fixed sign-off appended to every sent email (blank = none).",3,!1)+Q("followup-template","Follow-up template","Copy-paste follow-up — {{contact_name}}, {{role}}, {{company}}, {{last_sent}}, {{last_message}}.",9,!1)+`<div class="set-field">
    ...
    </div>`,Mt(e);const t=e.querySelector(".set-fu-interval");t&&t.addEventListener("change",async()=>{const n=Math.max(0,Math.min(90,parseInt(t.value,10)||0));t.value=String(n),await W("PUT","/api/followup-interval",{days:n})&&(l.followupInterval=n,r("follow-up interval saved"))})}function Ua(e){e.innerHTML=Oa.map(([t,n,s])=>`
  ...
  </details>`}function za(e){const t=l.anthropicKey||{};let n="Not set — verdict, capture & outreach disabled.";t.key_source==="db"?n="Set here · active.":t.key_source==="env"&&(n="Using the ANTHROPIC_API_KEY environment variable.");const s=l.gmail||{},a=!!s.connected,o=!!s.configured,i=a?"ok":o?"warn":"off",d=a?`Connected as ${c(s.email||"your account")}`:o?"Not connected":"Not set up";e.innerHTML=`
    <div class="set-field">
...
`))+"</code></pre>");continue}const d=i.match(/^(#{1,6})\s+(.*)$/);if(d){a();const f=d[1].length;n.push("<h"+f+">"+Qe(c(d[2]))+"</h"+f+">"),o++;continue}const u=i.match(/^\s*[-*]\s+(.*)$/);if(u){s!=="ul"&&(a(),n.push("<ul>"),s="ul"),n.push("<li>"+Qe(c(u[1]))+"</li>"),o++;continue}const p=i.match(/^\s*\d+\.\s+(.*)$/);if(p){s!=="ol"&&(a(),n.push("<ol>"),s="ol"),n.push("<li>"+Qe(c(p[1]))+"</li>"),o++;continue}if(i.trim()===""){a(),o++;continue}a();const m=[];for(;o<t.length&&t[o].trim()!==""&&!/^```|^#{1,6}\s|^\s*[-*]\s+|^\s*\d+\.\s+/.test(t[o]);)m.push(Qe(c(t[o]))),o++;n.push("<p>"+m.join("<br>")+"</p>")}return a(),n.join("")}function Xe(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,e==="assistant"?n.innerHTML=fo(t||""):n.textContent=t||"",n}function Ht(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function go(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=l.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(l.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Zn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=mo(n.content);if(n.role==="user")s&&t.appendChild(Xe("user",s));else if(n.role==="assistant"){const a=ho(n.content);if(!s&&!a.length)continue;const o=Xe("assistant",s);if(a.length){const i=document.createElement("div");i.className="chat-tools",i.textContent="· used "+a.join(", "),o.appendChild(i)}t.appendChild(o)}}t.children.length||t.appendChild(go()),Ht()}async function Ot(e,t,n){if(!l.meta||!l.meta.chat){r("chat needs ANTHROPIC_API_KEY in the server env");return}l.chat.es&&(l.chat.es.close(),l.chat.es=null),l.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),i=await fetch("/api/chat/threads?"+o);if(!i.ok)throw new Error((await i.text().catch(()=>"")).trim()||"HTTP "+i.status);const d=await i.json();l.chat.threadId=d.thread.id,Zn(d.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+c(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Nt(){l.chat.es&&(l.chat.es.close(),l.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function et(e){l.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Qn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Xn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||l.chat.streaming||!l.chat.threadId)return;e.value="",Qn(),et(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Xe("user",t));const a=Xe("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),Ht();let o="";const i=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,et(!1)},d=l.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){i(m.message);return}if(!u.ok){i((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");l.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,a.textContent=o,Ht()}),p.addEventListener("end",async m=>{p.close(),l.chat.es===p&&(l.chat.es=null),a.classList.remove("chat-streaming"),et(!1),l.chat.threadId===d&&await yo(),vo(),typeof m.data=="string"&&m.data.indexOf("error")===0&&r("chat: "+m.data)}),p.onerror=()=>{p.close(),l.chat.es===p&&(l.chat.es=null),a.classList.remove("chat-streaming"),et(!1)}}async function yo(){const e=l.chat.scope,t=l.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Zn(a.messages||[])}catch{}}function vo(){H(),k(),j(),l.openId&&ge(l.openId)}document.getElementById("open-chat").onclick=()=>Ot("global","",""),document.getElementById("chat-close").onclick=Nt,document.getElementById("chat-scrim").onclick=Nt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Xn()}),document.getElementById("chat-input").addEventListener("input",Qn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Xn())}),Kt("#t tbody",os),Kt("#jt tbody",is);const bo=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();L(bo==="jobs"?"jobs":"companies",{render:!1});function es(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||es()}),window.addEventListener("pageshow",e=>{e.persisted&&es()}),H(),k(),j(),Le(),wt(),Bt(),At(),ve(),oe(),setInterval(oe,9e4),q(),function(){const t=/[?&]gmail=(connected|error)/.exec(location.search);t&&(r(t[1]==="connected"?"Gmail connected":"Gmail connection failed"),history.replaceState(null,"",location.pathname+location.hash))}()}$o({"":{view:()=>({mount(w){w.innerHTML=To,Lo()}}),chrome:!1}},{title:"scout"});So();_o();
