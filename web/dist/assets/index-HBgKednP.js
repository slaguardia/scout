...
`;function xo(w){const b={k:"verdict",dir:1},x={k:"created_at",dir:1},c={rows:[],sort:{...b},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...x},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null,gmail:null,notifications:{notifications:[],unread:0,followups:[]},settingsGroup:"outreach"},y=e=>"pill pill-"+(e||"none"),I='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',r=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),A=e=>/^https?:\/\//i.test(String(e??""))?r(e):"#";async function q(){const t=await(await fetch("/api/companies")).json();c.rows=t.rows||[],F()}async function $(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}c.jobs=e.rows||[],j(),R(),ye()}function R(){if(!h.postingId)return;const e=c.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&X())}let Y=null;function ye(){const e=c.jobs.some(t=>t.outreach_draft_status==="researching");e&&!Y?Y=setInterval(Ce,4e3):!e&&Y&&(clearInterval(Y),Y=null)}async function Ce(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(c.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==c.jobs.length;c.jobs=t,s&&(j(),R()),ye()}async function P(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(c.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(c.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(c.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(c.followupTemplate=e.content)}).catch(()=>{})]),st(),c.view==="jobs"&&j()}function L(e,{render:t=!0}={}){c.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none";const n=(d,u)=>{const p=document.getElementById(d);p&&(p.style.display=u?"":"none")};n("settings-view",e==="settings"),n("inbox-view",e==="inbox"),n("docs-view",e==="docs"),document.getElementById("open-settings").classList.toggle("is-active",e==="settings");const s=document.getElementById("open-notifications");s&&s.classList.toggle("is-active",e==="inbox");const a=document.getElementById("open-docs");a&&a.classList.toggle("is-active",e==="docs");const o=e==="companies"||e==="jobs";document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none";const i=document.getElementById("block-columns");i&&(i.style.display=o?"":"none"),Ut(),t&&(e==="jobs"?j():e==="settings"?J():e==="inbox"?(zn(),te()):e==="docs"?Ka():F())}async function C(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}c.stats=e,Z()}function Z(){J()}function Xe(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function je(e){return e.slice().sort((t,n)=>c.sort.dir*Xe(t,n,c.sort.k))}function ve(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(s=>{s.getAttribute(t)===n.k?s.dataset.sort=n.dir<0?"desc":"asc":delete s.dataset.sort})}const U=new Set;let ne=!1,se=!1;const Qn=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function Xn(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+Qn.map(([t,n,s])=>Q("data-v",t,n,s,U.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+Q("data-toggle","flagged","⚑ Flagged","",ne)+Q("data-toggle","enriched","Enriched","",se),Ht())}function Ht(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const i of c.rows){const d=i.verdict||"__none__";e[d]=(e[d]|0)+1,i.flagged&&t++,i.enriched&&n++}at("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=U.size+(ne?1:0)+(se?1:0);Zt("fdrop-cfilters-btn",o,o>0)}function Ot(){const e=document.getElementById("q").value.trim().toLowerCase();return c.rows.filter(t=>!(U.size&&!U.has(t.verdict||"__none__")||ne&&!t.flagged||se&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const es=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],ts=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Nt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Rt=Nt("scout-hidden-cols"),Dt=Nt("scout-hidden-jcols");function et(){return c.view==="jobs"?{cols:ts,hidden:Dt,key:"scout-hidden-jcols"}:{cols:es,hidden:Rt,key:"scout-hidden-cols"}}function ae(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Rt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Dt.has(e.dataset.col)?"none":""})}function Ut(){const e=et(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>Q("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Vt())}function Vt(){const e=et(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Ft(e){return`
    ...
    `}function Jt(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>In(t.dataset.id))}const ns=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],ss=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function Gt(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let i=0;i<n;i++){const d=t.map(([u,p])=>{const m=p.endsWith("%")?Math.round(parseFloat(p)*a[i%a.length])+"%":p;return`<td${u?` data-col="${u}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${d}</tr>`}s.innerHTML=o,ae()}function F(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=je(Ot());Ht(),document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=Ft(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||me(s.dataset.id)}),e.appendChild(s),Jt(s)}ve("t","data-k",c.sort),ae()}async function as(e){const n=await(await fetch("/api/companies")).json();c.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=je(Ot()).map(i=>i.company_id),o=[...s.querySelectorAll("tr")].map(i=>i.dataset.id);if(a.length!==o.length||a.some((i,d)=>i!==o[d])){F();return}for(const i of e){const d=c.rows.find(p=>p.company_id===i),u=s.querySelector(`tr[data-id="${CSS.escape(i)}"]`);if(!d||!u){F();return}u.innerHTML=Ft(d),Jt(u)}ae()}let E=null,tt=null,oe=!1,ie=!1,_=null,nt=null;function zt(){const e=c.applicationStages;if(E===null)E=new Set(["",...e]);else{for(const t of[...E])t!==""&&!e.includes(t)&&E.delete(t);if(tt)for(const t of e)tt.has(t)||E.add(t)}tt=new Set(e)}function Wt(){const e=c.outreachStatuses;if(_===null)_=new Set(["",...e]);else{for(const t of[..._])t!==""&&!e.includes(t)&&_.delete(t);if(nt)for(const t of e)nt.has(t)||_.add(t)}nt=new Set(e)}function os(){zt(),Wt();const e=document.getElementById("jq").value.trim().toLowerCase();return c.jobs.filter(t=>{const n=t.application_status||"";return!(!E.has(n)||oe&&!t.next_up||ie&&!(t.followups_due|0)||!_.has(t.outreach_status||"")||e&&!(t.title+" "+t.company+" "+(t.location||"")+" "+(t.description||"")+" "+(t.contacts||"")).toLowerCase().includes(e))})}function Q(e,t,n,s,a){return`<button class="fdrop-item${a?" is-checked":""}" ${e}="${r(t)}" role="menuitemcheckbox" aria-checked="${a}"><span class="fdrop-check" aria-hidden="true"></span>`+(s?`<span class="fdrop-dot ${s}"></span>`:"")+`<span class="fdrop-label">${r(n)}</span><span class="fdrop-item-count" data-count></span></button>`}function Kt(e,t,n){return`<div class="fdrop-head fdrop-head--toggle"><span>${e}</span><button type="button" class="fdrop-all" data-all="${t}">${n?"none":"all"}</button></div>`}function st(){zt(),Wt();const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=["",...c.applicationStages],n=["",...c.outreachStatuses];e.innerHTML=Kt("Application stage","stage",t.every(s=>E.has(s)))+Q("data-stage","","not applied","",E.has(""))+c.applicationStages.map(s=>Q("data-stage",s,s,Ae(s),E.has(s))).join("")+'<div class="fdrop-sep"></div>'+Kt("Reply status","status",n.every(s=>_.has(s)))+[["","not reached out",""]].concat(c.outreachStatuses.map(s=>[s,s,sn(s)])).map(([s,a,o])=>Q("data-status",s,a,o,_.has(s))).join(""),Yt()}function Yt(){const e={},t={};for(const d of c.jobs){const u=d.application_status||"";e[u]=(e[u]|0)+1;const p=d.outreach_status||"";t[p]=(t[p]|0)+1}at("#fdrop-jfilters-menu [data-stage]","data-stage",e),at("#fdrop-jfilters-menu [data-status]","data-status",t);const n=["",...c.applicationStages],s=E&&E.size===n.length&&n.every(d=>E.has(d)),a=["",...c.outreachStatuses],o=_&&_.size===a.length&&a.every(d=>_.has(d)),i=(s?0:E?E.size:0)+(o?0:_?_.size:0);Zt("fdrop-jfilters-btn",i,i>0)}function at(e,t,n){document.querySelectorAll(e).forEach(s=>{const a=s.querySelector("[data-count]");if(a){const o=n[s.getAttribute(t)]|0;a.textContent=o||""}})}function Zt(e,t,n){const s=document.getElementById(e);if(!s)return;s.classList.toggle("is-active",n);const a=s.querySelector(".fdrop-count");if(a){const o=n&&t>0;a.textContent=o?t:"",a.style.display=o?"":"none"}}function ce(e,t){e.classList.toggle("is-checked",t),e.setAttribute("aria-checked",String(t))}function is(){const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=e.querySelector('.fdrop-all[data-all="stage"]'),n=e.querySelector('.fdrop-all[data-all="status"]');t&&(t.textContent=["",...c.applicationStages].every(s=>E.has(s))?"none":"all"),n&&(n.textContent=["",...c.outreachStatuses].every(s=>_.has(s))?"none":"all")}function ot(){document.querySelectorAll(".fdrop.is-open").forEach(e=>{e.classList.remove("is-open");const t=e.querySelector(".fdrop-btn");t&&t.setAttribute("aria-expanded","false")})}function Qt(e){const t=e.querySelector(".fdrop-btn"),n=e.querySelector(".fdrop-menu");if(!t||!n)return;const s=t.getBoundingClientRect();n.style.left=Math.round(s.left)+"px",n.style.top=Math.round(s.bottom+4)+"px",n.style.minWidth=Math.round(s.width)+"px",n.style.maxHeight=Math.max(160,Math.round(window.innerHeight-s.bottom-12))+"px"}function cs(e){const t=e.querySelector(".fdrop-btn");e.classList.add("is-open"),t&&t.setAttribute("aria-expanded","true"),Qt(e)}function Xt(){const e=document.querySelector(".fdrop.is-open");e&&Qt(e)}window.addEventListener("scroll",Xt,!0),window.addEventListener("resize",Xt);const rs=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function ls(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(rs),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}const Me=()=>new Date().toISOString().slice(0,10);function en(e){const t=[["","none"]];for(const n of c.applicationStages)t.push([n,n]);return e&&!c.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function tn(e){const t=[["","none"]];for(const n of c.outreachStatuses)t.push([n,n]);return e&&!c.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const ds=8;function nn(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%ds}function Ae(e){return nn(e,c.applicationStages)}function sn(e){return nn(e,c.outreachStatuses)}function us(e){const t=ls(e);return t.length?t.map(n=>{const s=r(n.position||n.email);if(!n.email)return s;const a=r(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${r(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function an(e){const t=e.application_status||"";if(!t)return c.applicationStages.length+1;const n=c.applicationStages.indexOf(t);return n<0?c.applicationStages.length:n}function ps(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return an(e)-an(t);if(n==="followups_due")return(t.followups_due|0)-(e.followups_due|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-c.jsort.dir:c.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function ms(){const e=document.getElementById("jobs-followup-nav");if(!e)return;let t=0,n=0;for(const a of c.jobs)a.next_up&&t++,n+=a.followups_due|0;if(t||(oe=!1),n||(ie=!1),!t&&!n){e.style.display="none",e.innerHTML="";return}e.style.display="";const s=[];t&&s.push(`<button class="queue-nav-btn queue-nav-btn--nextup${oe?" is-active":""}" data-q="nextup" title="${oe?"showing only these — click to show all jobs":"show only jobs queued next up for outreach"}"><span class="fn-icon">${Ca}</span><span class="fn-text"><strong>${t}</strong> next up</span></button>`),n&&s.push(`<button class="queue-nav-btn${ie?" is-active":""}" data-q="due" title="${ie?"showing only these — click to show all jobs":"show only jobs owing a follow-up"}"><span class="fn-icon">${Vn}</span><span class="fn-text"><strong>${n}</strong> follow-up${n>1?"s":""} due</span></button>`),e.innerHTML=s.join(""),e.querySelectorAll(".queue-nav-btn").forEach(a=>{a.onclick=()=>{a.dataset.q==="nextup"?oe=!oe:ie=!ie,j()}})}function j(){const e=document.querySelector("#jt tbody");e.innerHTML="",ms();const t=os().sort((a,o)=>c.jsort.dir*ps(a,o,c.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",Yt();const n=E&&!E.has("rejected")?c.jobs.filter(a=>(a.application_status||"")==="rejected").length:0,s=document.getElementById("jobs-hidden-note");s.style.display=n?"":"none",n&&(s.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{E.add("rejected"),st(),j()});for(const a of t){const o=a.application_status||"",i=document.createElement("tr");i.dataset.id=a.posting_id;const d=en(o).map(([m,f])=>`<option value="${r(m)}"${o===m?" selected":""}>${r(f)}</option>`).join(""),u=a.outreach_status||"",p=tn(u).map(([m,f])=>`<option value="${r(m)}"${u===m?" selected":""}>${r(f)}</option>`).join("");i.innerHTML=`
      ...
      <td data-col="application"><div class="jt-stage"><select class="jt-stage-sel ${Ae(o)}" title="application stage">${d}</select>${o&&a.application_status_at?`<span class="jt-stage-at" title="stage last changed">${r(a.application_status_at.slice(0,10))}</span>`:""}</div></td>
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row">
          ...
          <select class="input pl-appstatus" title="application stage">
            ${en(e.application_status||"").map(([p,m])=>`<option value="${r(p)}"${(e.application_status||"")===p?" selected":""}>${r(m)}</option>`).join("")}
          </select>
          ${e.application_status&&e.application_status_at?`<span class="pl-at" title="stage last changed">since ${r(e.application_status_at.slice(0,10))}</span>`:""}
        </div>
        <div class="pipeline-row">
          ...
          <select class="input pl-ostatus" title="outreach reply status — separate from the application stage">
            ${tn(e.outreach_status||"").map(([p,m])=>`<option value="${r(p)}"${(e.outreach_status||"")===p?" selected":""}>${r(m)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
  ...
  `,Ts();const i=document.getElementById("pursuit-company-link");i&&i.addEventListener("click",()=>me(e.company_id)),vs(e),V(document.getElementById("pursuit-title-input"),p=>cn(e,"title",p)),V(document.getElementById("pursuit-url-input"),p=>gs(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>ys(e,d)),V(document.getElementById("pursuit-notes-input"),p=>Ls(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>V(p,m=>cn(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>Es(e)),de(),Ee(),t&&(t.scrollTop=s),un=e.posting_id}function _s(e){return`
    ...
    <div class="role-meta">
      ...
      <span class="role-company-wrap">
        ...
        ${e.verdict?`<span class="${y(e.verdict)}" title="scout's company-fit verdict">${r(e.verdict)}</span>`:""}
  ...
  </div>`}function Ms(){const e=h.row,t=js(),n=e.last_outreach_at?`<div class="outreach-meta"><span>last outreach ${r(e.last_outreach_at)}</span></div>`:"";if(!h.contactsLoaded)return`<div class="contacts-mgr">${t}${n}<div class="loading-row"><span class="spinner"></span><span>loading contacts…</span></div></div>`;const s=h.contacts.map(As).join(""),a=h.contacts.length?"":`<div class="cc-empty dim">No contacts yet — add the people you're reaching out to at ${r(e.company)}.</div>`;return`<div class="contacts-mgr">
    ${t}
    ${n}
  ...
  </div>`}function As(e){const t=h.outreach.filter(o=>o.contact_id===e.id),n=t[0]||null,s=e.role?`<span class="cc-role">${r(e.role)}</span>`:"",a=e.email?`<a class="cc-mail" href="mailto:${r(e.email)}" title="${r(e.email)}">${r(e.email)}</a>`:"";return`<div class="contact-card" data-cid="${e.id}">
    <div class="cc-head">
    ...
    ${t.length?`<details class="cc-history"><summary>${t.length} send${t.length>1?"s":""}</summary><div class="cc-entries">${t.map(qs).join("")}</div></details>`:""}
  </div>`}function Ps(e){if(!e)return'<span class="dim">no outreach logged yet</span>';const t=`last ${r(e.sent_at)}`,n=e.id,s=e.followup_due_at,a=s&&s<=Me();return e.followup_done_at?a?`${t} · <span class="fu-escalate">no reply — try another contact</span> <button class="cc-fu-dismiss" data-eid="${n}" type="button" title="dismiss — stop reminding me about this contact">dismiss</button>`:`${t} · <label class="cc-fu-check fu-done"><input class="cc-fu-toggle" type="checkbox" data-eid="${n}" checked> followed up</label>`:s?`${t} · <label class="cc-fu-check${a?" fu-overdue":""}"><input class="cc-fu-toggle" type="checkbox" data-eid="${n}"> follow up</label> <button class="cc-fu-stop" data-eid="${n}" type="button" title="discontinue follow-ups for this contact">stop</button>`:`${t} · <span class="fu-stopped">follow-up stopped</span> <button class="cc-fu-resume" data-eid="${n}" type="button">resume</button>`}function qs(e){const t=e.followup_done_at?'<span class="fu-done">followed up</span>':e.followup_due_at?`<span class="fu-mini">↳ follow up ${r(e.followup_due_at)}</span>`:"",n=e.gmail_message_id?'<span class="cc-e-prov prov-gmail" title="sent via Gmail — replies auto-sync">via Gmail ✓</span>':'<span class="cc-e-prov prov-manual" title="logged by hand — not tracked in Gmail">logged manually</span>',s=e.body?`<details class="cc-e-body"><summary>email sent</summary><pre>${r(e.body)}</pre></details>`:"";return`<div class="cc-entry-wrap">
      ...
      ${s}
    </div>`}async function ee(e,t,n){let s;try{s=await fetch(t,{method:e,headers:n?{"Content-Type":"application/json"}:{},body:n?JSON.stringify(n):void 0})}catch(a){return l(`save failed: ${a.message}`),null}if(!s.ok){const a=(await s.text().catch(()=>"")).trim();return l(`save failed: ${a||"HTTP "+s.status}`),null}try{return await s.json()}catch{return{}}}async function ue(){await $(),await be()}function Hs(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.postingId,n=e.querySelector(".cc-sync-now");n&&n.addEventListener("click",async()=>{n.disabled=!0;const a=n.textContent;n.textContent="Syncing…";try{const o=await fetch("/api/gmail/sync",{method:"POST"});if(!o.ok){l(`sync failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}await fe(),await be(),l("synced with Gmail")}catch(o){l(`sync failed: ${o.message}`)}finally{n.disabled=!1,n.textContent=a}});const s=e.querySelector(".cc-addwrap");if(s){const a=s.querySelector(".cc-addform");s.querySelector(".cc-addbtn").addEventListener("click",()=>{a.style.display="",s.querySelector(".cc-addbtn").style.display="none",a.querySelector(".cc-f-name").focus()}),s.querySelector(".cc-f-cancel").addEventListener("click",()=>de()),s.querySelector(".cc-f-save").addEventListener("click",async()=>{const o={name:a.querySelector(".cc-f-name").value,role:a.querySelector(".cc-f-role").value,email:a.querySelector(".cc-f-email").value};await ee("POST",`/api/companies/${h.row.company_id}/contacts`,o)&&(l("contact added"),ue())})}e.querySelectorAll(".contact-card").forEach(a=>{const o=a.dataset.cid,i=a.querySelector(".cc-editform");a.querySelector(".cc-edit").addEventListener("click",()=>{i.style.display=i.style.display==="none"?"":"none",i.style.display!=="none"&&i.querySelector(".cc-e-name").focus()});const d=a.querySelector(".cc-e-cancel");d&&d.addEventListener("click",()=>{i.style.display="none"});const u=a.querySelector(".cc-e-save");u&&u.addEventListener("click",async()=>{const g={name:i.querySelector(".cc-e-name").value,role:i.querySelector(".cc-e-role").value,email:i.querySelector(".cc-e-email").value};await ee("PUT",`/api/contacts/${o}`,g)&&(l("contact saved"),ue())}),a.querySelector(".cc-arch").addEventListener("click",async()=>{await ee("DELETE",`/api/contacts/${o}`)&&(l("contact removed"),ue())});const p=a.querySelector(".cc-logform"),m=a.querySelector(".cc-log");m&&m.addEventListener("click",()=>{p.style.display=p.style.display==="none"?"":"none",p.style.display!=="none"&&p.querySelector(".cc-l-date").focus()});const f=a.querySelector(".cc-l-cancel");f&&f.addEventListener("click",()=>{p.style.display="none"});const v=a.querySelector(".cc-l-save");v&&v.addEventListener("click",async()=>{const g={contact_id:o,sent_at:p.querySelector(".cc-l-date").value||Me(),body:p.querySelector(".cc-l-body").value};await ee("POST",`/api/postings/${t}/outreach-log`,g)&&(l("outreach logged"),ue())});const M=a.querySelector(".cc-followup");M&&M.addEventListener("click",()=>{const g=h.contacts.find(ge=>String(ge.id)===String(o)),D=h.outreach.filter(ge=>String(ge.contact_id)===String(o))[0]||null;Re(Bs(g,D),"follow-up copied — paste into your email")});const S=async(g,D,ge)=>{const Be=h.outreach.find(fo=>String(fo.id)===String(g))||{};await ee("PUT",`/api/outreach-log/${g}`,{sent_at:Be.sent_at||"",body:Be.body||"",note:Be.note||"",followup_due_at:Be.followup_due_at||"",done:!!Be.followup_done_at,...D})&&(l(ge),ue())},k=a.querySelector(".cc-fu-toggle");k&&k.addEventListener("change",()=>S(k.dataset.eid,{done:k.checked},k.checked?"marked followed up":"follow-up reopened"));const B=a.querySelector(".cc-fu-stop");B&&B.addEventListener("click",()=>S(B.dataset.eid,{followup_due_at:"",done:!1},"follow-up stopped"));const N=a.querySelector(".cc-fu-resume");N&&N.addEventListener("click",()=>S(N.dataset.eid,{followup_due_at:Me(),done:!1},"follow-up resumed"));const O=a.querySelector(".cc-fu-dismiss");O&&O.addEventListener("click",()=>S(O.dataset.eid,{followup_due_at:"",done:!0},"escalation dismissed")),a.querySelectorAll(".cc-e-del").forEach(g=>g.addEventListener("click",async()=>{if(!confirm("Delete this logged send? Its follow-up is removed too. This can't be undone."))return;const D=g.closest(".cc-entry").dataset.eid;await ee("DELETE",`/api/outreach-log/${D}`)&&(l("send deleted"),ue())}))})}function Os(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const gn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',yn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',vn=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${gn}</button>`,ut=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],Ns='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function Rs(e){let t=ut.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    ...
    <div class="draft-head">
      ...
      <div class="draft-sentbody">${r(pt(e)||"(empty)")}</div>
      ${$e(e)}
    ...
    ${$e(e)}
  </div>`}function pt(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function $e(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,p)=>p?`<div class="tr-line"><span class="tr-key">${u}:</span> ${r(String(p))}</div>`:"",o=n.role||{},i=Array.isArray(n.hooks)?n.hooks:[],d=i.map(u=>`
      <div class="tr-line">
  ...
  `:'<div class="muted">Not yet scored. Run <code>scout verdict</code>, or set one by hand below.</div>',a=`
    ...
    <dl class="kv">
      <dt>url</dt><dd>${e.website_url?`<a href="${A(e.website_url)}" target="_blank" rel="noopener">${r(e.website_url)} ↗</a>`:'<span class="muted">—</span>'}</dd>
  ...
  `;const v=document.getElementById("posting-add-btn");v&&v.addEventListener("click",()=>la(e)),ft(),document.querySelectorAll("#ve-pick .ve-opt").forEach(g=>{g.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(D=>D.classList.remove("is-on")),g.classList.add("is-on")})});const M=document.getElementById("ve-save-btn");M&&M.addEventListener("click",()=>ra(e)),V(document.getElementById("pane-title-input"),g=>dn(e,"name",g)),document.querySelectorAll("#facts-body [data-k]").forEach(g=>V(g,D=>dn(e,g.dataset.k,D))),V(document.getElementById("pane-domain-input"),g=>Is(e,g)),V(document.getElementById("pane-notes-input"),g=>Ss(e,g),{multiline:!0});const S=document.getElementById("flag-toggle-btn");S&&S.addEventListener("click",()=>In(e.company_id));const k=document.getElementById("review-stamp-btn");k&&k.addEventListener("click",()=>ca(e.company_id));const B=document.getElementById("rescore-btn");B&&B.addEventListener("click",()=>vt("verdict",{company_ids:[e.company_id]}));const N=document.getElementById("reenrich-btn");N&&N.addEventListener("click",()=>vt("enrich",{company_ids:[e.company_id]}));const O=document.getElementById("company-delete-btn");O&&O.addEventListener("click",()=>ks(e))}function ia(e){return`
    ...
    </dl>`}async function ca(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){l(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");l(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=c.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,F()),c.openId===e&&(Ie(s),Se(e)),l("reviewed")}async function In(e){const t=c.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){l(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");l(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,F()),c.openId===e&&(Ie(a),Se(e)),$(),l(a.flagged?"flagged":"unflagged")}async function ra(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){l("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(d){l(`save failed: ${d.message}`),a.disabled=!1;return}if(!o.ok){const d=await o.text().catch(()=>"");l(`save failed: HTTP ${o.status}${d?" — "+d:""}`),a.disabled=!1;return}const i=await o.json();Ie(i),Se(i.company_id),q(),C(),$(),l("verdict saved")}function ht(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(r).join(" · "),a=n.application_status||"",o=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?Ae(a)||"pill-stage":"pill-none"}">${r(a||"—")}</span>`,`<span class="pt-meta">${a?"tracked":"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${r(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    <div class="brain-node posting-card" data-pid="${r(n.id)}" title="open the pursuit — tracking, outreach, drafts">
      <div class="n"><a href="${A(n.url)}" target="_blank" rel="noopener">${r(n.title||n.url)} ↗</a></div>
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function he(e,t){const n=c.openDetail;if(!n||!c.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=ht(n),ft())}function ft(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||it(e.dataset.pid)})})}async function la(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const i=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==i.id),e.postings.unshift(i);const d=document.getElementById("postings-list");d&&(d.innerHTML=ht(e),ft()),t.value="",n.value="",s.disabled=!1,$(),l("link added")}async function Se(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){Oe(`<div class="muted">Failed to load trail: ${r(s.message)}</div>`);return}if(!t.ok){Oe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Oe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Oe(n.map(da).join(""))}function da(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(r);return e.run_id&&t.push("run "+r(e.run_id.slice(0,8))),`
...
`)):document.getElementById("editor-text").value=i.content||"",a&&(document.getElementById("editor-enabled").checked=i.enabled!==!1),i.taste_version&&(document.getElementById("editor-ver").textContent="version "+i.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Fe(){document.getElementById("editor-scrim").classList.remove("open"),H=null}let T=null,kt=[],$t=[];const va=["location.allowed","verticals.excluded","verticals.allowed"],Et={"verticals.excluded":"pf-vertical-tags","verticals.allowed":"pf-vertical-tags"};function W(e){const[t,n]=e.split(".");return T[t]&&T[t][n]||[]}function Je(e,t){const[n,s]=e.split(".");(T[n]=T[n]||{})[s]=t}function Ge(e,t){const n=t.toLowerCase();return e.some(s=>String(s).toLowerCase()===n)}function xt(){va.forEach(e=>{const t=document.querySelector(`.pf-chips[data-field="${e}"]`);if(!t)return;const n=Et[e]?` list="${Et[e]}"`:"",s=Et[e]?"type to search…":"type &amp; Enter…";t.innerHTML=W(e).map((a,o)=>`<span class="pf-chip">${r(a)}<button class="pf-chip-x" data-field="${e}" data-i="${o}" title="remove" aria-label="remove ${r(a)}">×</button></span>`).join("")+`<input class="pf-chip-input" data-field="${e}"${n} type="text" placeholder="${s}" spellcheck="false" autocomplete="off" />`})}function ba(e,t){var s;const n=(t||"").trim();if(n){const a=W(e);Ge(a,n)||Je(e,[...a,n])}xt(),(s=document.querySelector(`.pf-chip-input[data-field="${e}"]`))==null||s.focus()}function qn(e,t){const n=W(e).slice();n.splice(t,1),Je(e,n),xt()}function Hn(){const e=document.getElementById("pf-stages");if(!e)return;const t=W("funding_stage.allowed");e.innerHTML=$t.map(n=>{const s=Ge(t,n.value),a=n.count?` <span class="pf-stage-n">${n.count}</span>`:"";return`<button class="pf-stage${s?" is-on":""}" data-stage="${r(n.value)}">${r(n.value)}${a}</button>`}).join("")}function wa(e){const t=W("funding_stage.allowed");Je("funding_stage.allowed",Ge(t,e)?t.filter(n=>String(n).toLowerCase()!==e.toLowerCase()):[...t,e]),Hn()}function ka(){const e=document.getElementById("pf-vertical-tags");e&&(e.innerHTML=kt.map(t=>`<option value="${r(t.value)}" label="${t.count}"></option>`).join(""))}function $a(){return{location:{allowed:[],remote_ok:!0},headcount:{min:0,max:0},verticals:{allowed:[],excluded:[]},funding_stage:{allowed:[]}}}function Ea(){return`<div class="set-field pf-inline">
    ...
    <div class="set-field-label">${r(t)}</div>
    <div class="set-field-desc">${r(n)}</div>
...
`):a.content||""}catch{s.value=""}s.dataset.orig=s.value,s.dataset.loaded="1",s.addEventListener("blur",()=>Pa(e))}async function Pa(e){const t=e.dataset.kind,n=e.dataset.list==="1",s=e.querySelector(".set-textarea");if(s.dataset.loaded!=="1"||s.value===s.dataset.orig)return;const a=n?{statuses:s.value.split(/\r?\n/).map(i=>i.trim()).filter(Boolean)}:{content:s.value};let o;try{o=await fetch(`/api/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}catch(i){l(`save failed: ${i.message}`);return}if(!o.ok){l(`save failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}s.dataset.orig=s.value,Lt(e),t==="followup-template"&&(c.followupTemplate=s.value),n&&(await P(),j())}function Bt(e){e.querySelectorAll(".set-field[data-kind]").forEach(Aa)}function qa(e){e.innerHTML=K("outreach-subject","Email subject","The send subject — {{role}} / {{company}} substitution, no LLM.",2,!1)+K("outreach-template","Email body","Verbatim prose with the writer's fill-in holes.",16,!1)+K("outreach-signature","Email signature","A fixed sign-off appended to every sent email (blank = none).",3,!1)+K("followup-template","Follow-up template","Copy-paste follow-up — {{contact_name}}, {{role}}, {{company}}, {{last_sent}}, {{last_message}}.",9,!1)+`<div class="set-field">
    ...
    </div>`,Bt(e);const t=e.querySelector(".set-fu-interval");t&&t.addEventListener("change",async()=>{const n=Math.max(0,Math.min(90,parseInt(t.value,10)||0));t.value=String(n),await ee("PUT","/api/followup-interval",{days:n})&&(c.followupInterval=n,l("follow-up interval saved"))})}function Ha(e){e.innerHTML=Ma.map(([t,n,s])=>`
    <div class="set-field" data-prompt="${t}">
      <div class="set-field-label">${r(n)}</div>
      <div class="set-field-desc">${r(s)}</div>
      ...
      <pre class="set-readonly">${r(a?t.body:"(no brief yet — Refresh to distill from the brain)")}</pre>
    ...
    <div class="set-help-body">
      <ol class="set-steps">
        ...
        <li><strong>Configure the OAuth consent screen.</strong> Add these scopes, then <strong>Publish app</strong> (self-hosting your own mailbox needs no Google verification) — or add your account under <strong>Test users</strong>:
          <ul class="set-help-scopes">${n.map(s=>`<li><code>${r(s)}</code></li>`).join("")}</ul>
        </li>
    ...
    <div class="set-field">
      ...
      <div class="set-subfields">
        ...
        <input class="input" id="set-gm-id" placeholder="…apps.googleusercontent.com" autocomplete="off" spellcheck="false" value="${r(s.client_id||"")}">
        ...
        <input class="input" id="set-gm-redirect" placeholder="https://…/api/gmail/callback" autocomplete="off" spellcheck="false" value="${r(s.redirect_uri||"")}">
      </div>
    ...
    </div>`;const u=e.querySelector("#set-ak-save");u&&u.addEventListener("click",async()=>{const k=e.querySelector("#set-ak-input").value.trim();if(!k){l("paste a key first");return}const B=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:k})});if(!B.ok){l((await B.text().catch(()=>"")).trim()||`HTTP ${B.status}`);return}l("Anthropic key saved"),await _e(),await Ct()});const p=e.querySelector("#set-ak-remove");p&&p.addEventListener("click",async()=>{const k=await fetch("/api/integrations/anthropic",{method:"DELETE"});if(!k.ok){l(`HTTP ${k.status}`);return}l("Anthropic key removed"),await _e(),await Ct()});const m=e.querySelector("#set-gm-save");m&&m.addEventListener("click",async()=>{const k=e.querySelector("#set-gm-id").value.trim(),B=e.querySelector("#set-gm-secret").value,N=e.querySelector("#set-gm-redirect").value.trim();if(!k){l("client ID is required");return}const O=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:k,client_secret:B,redirect_uri:N})});if(!O.ok){l((await O.text().catch(()=>"")).trim()||`HTTP ${O.status}`);return}l("Gmail OAuth client saved"),await fe()});const f=e.querySelector("#set-gm-connect");f&&f.addEventListener("click",Va);const v=e.querySelector("#set-gm-disconnect");v&&v.addEventListener("click",Fa);const M=e.querySelector("#gm-copy-cb");M&&M.addEventListener("click",()=>Re(c.gmail&&c.gmail.callback_uri||"","redirect URI copied"));const S=e.querySelector("#set-autoflip");S&&S.addEventListener("change",async()=>{let k=!1;try{k=(await fetch("/api/gmail/autoflip",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:S.checked})})).ok}catch{k=!1}k?(c.gmail&&(c.gmail.autoflip=S.checked),l(`auto-update ${S.checked?"on":"off"}`)):(S.checked=!S.checked,l("failed to save"))})}async function Ct(){try{c.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{c.anthropicKey=null}J()}async function fe(){try{c.gmail=await(await fetch("/api/gmail/status")).json()}catch{c.gmail=null}J()}async function Va(){let e;try{e=await fetch("/api/gmail/connect")}catch(n){l(`connect failed: ${n.message}`);return}if(!e.ok){l((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}let t={};try{t=await e.json()}catch{}t.auth_url?window.location.href=t.auth_url:l("could not start the Gmail connect flow")}async function Fa(){if(!confirm("Disconnect Gmail? Sending and sync stop; already-synced data stays."))return;let e;try{e=await fetch("/api/gmail/disconnect",{method:"DELETE"})}catch(t){l(`disconnect failed: ${t.message}`);return}if(!e.ok){l((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}l("Gmail disconnected"),await fe()}function Le(){document.getElementById("gmail-config-scrim").classList.remove("open")}async function Ja(){const e=document.getElementById("gmail-client-id").value.trim(),t=document.getElementById("gmail-client-secret").value,n=document.getElementById("gmail-redirect").value.trim();if(!e){l("client ID is required");return}let s;try{s=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:e,client_secret:t,redirect_uri:n})})}catch(a){l(`save failed: ${a.message}`);return}if(!s.ok){l((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`);return}l("Gmail OAuth client saved — click Connect"),Le(),await fe()}async function Ga(){if(!confirm("Remove the stored Google OAuth client? Connecting needs it re-entered (or set via env)."))return;let e;try{e=await fetch("/api/gmail/config",{method:"DELETE"})}catch(t){l(`remove failed: ${t.message}`);return}if(!e.ok){l((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}l("OAuth client removed"),Le(),await fe()}function Jn(){const e=c.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&c.meta&&(c.meta.outreach===!1||c.meta.chat===!1);s.style.display=a?"":"none"}}function jt(){document.getElementById("key-scrim").classList.remove("open")}async function Gn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){l("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){l(`save failed: ${a.message}`),n();return}if(!s.ok){l((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}c.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),l("Anthropic key saved"),await _e(),Jn(),J()}async function za(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){l(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){l((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}c.anthropicKey=await t.json(),l(c.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await _e(),Jn(),J()}async function Wa(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),_t();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),_t();return}c.profile=await t.json(),J(),l("company-fit brief refreshed"),C()}function Mt(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Mt,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Mt()};function Ka(){const e=document.querySelector("#docs-nav a");Ke(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ke(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Ya(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ke(e)}document.getElementById("open-docs").onclick=()=>L("docs");function Za(){L("settings")}document.getElementById("open-settings").onclick=Za,document.getElementById("gmail-config-cancel").onclick=Le,document.getElementById("gmail-config-save").onclick=Ja,document.getElementById("gmail-config-remove").onclick=Ga,document.getElementById("gmail-config-scrim").onclick=e=>{e.target.id==="gmail-config-scrim"&&Le()};function Qa(){const e=document.getElementById("notif-badge");if(!e)return;const t=(c.notifications&&c.notifications.unread)|0;t>0?(e.textContent=t>99?"99+":String(t),e.style.display=""):e.style.display="none"}async function te(){try{c.notifications=await(await fetch("/api/notifications")).json()}catch{return}Qa(),c.view==="inbox"&&zn()}function Xa(){return'<option value="">link to role…</option>'+(c.jobs||[]).map(t=>`<option value="${r(t.posting_id)}">${r((t.company||"")+" — "+(t.title||"(untitled)"))}</option>`).join("")}function eo(e){const t=e.company||e.role?`<div class="notif-ctx">${r([e.company,e.role].filter(Boolean).join(" · "))}</div>`:'<div class="notif-ctx dim">not linked to a role</div>',n=e.created_at?`<span class="notif-when">${r((e.created_at||"").replace("T"," ").slice(0,16))}</span>`:"",s=e.kind==="app_status"&&e.suggested_status&&!e.actioned&&e.posting_id?`<button class="btn btn-primary notif-apply" data-id="${e.id}">Apply: ${r(e.suggested_status)}</button>`:"",a=e.posting_id?"":`<select class="input notif-link" data-id="${e.id}" title="link this to a role">${Xa()}</select>`;return`<div class="notif-item${e.seen?"":" is-unread"}" data-id="${e.id}" data-seen="${e.seen?1:0}">
    <div class="notif-main">
      <div class="notif-title">${e.seen?"":'<span class="notif-dot" aria-label="unread"></span>'}${r(e.title)}</div>
      ${t}
      ${e.detail?`<div class="notif-detail">${r(e.detail)}</div>`:""}
    </div>
    <div class="notif-side">${n}<div class="notif-acts">${s}${a}</div></div>
  </div>`}function to(e){return`<div class="notif-item notif-followup">
    <div class="notif-main">
      <div class="notif-title">Follow up: ${r(e.contact_name||"contact")}</div>
      <div class="notif-ctx">${r([e.company,e.role].filter(Boolean).join(" · "))}</div>
      <div class="notif-detail dim">due ${r(e.due_at||"")}</div>
    </div>
    <div class="notif-side"><button class="btn notif-open" data-pid="${r(e.posting_id)}">Open</button></div>
  </div>`}function zn(){const e=document.getElementById("notifications-body");if(!e)return;const t=c.notifications||{notifications:[],followups:[]},n=t.notifications||[],s=t.followups||[];if(!n.length&&!s.length){e.innerHTML='<div class="cc-empty dim">Nothing here yet. Replies, application updates, and follow-ups show up as Gmail syncs.</div>';return}let a="";n.length&&(a+='<div class="settings-group-h">Updates</div>'+n.map(eo).join("")),s.length&&(a+='<div class="settings-group-h">Follow-ups due</div>'+s.map(to).join("")),e.innerHTML=a,no()}function no(){const e=document.getElementById("notifications-body");e&&(e.querySelectorAll(".notif-item[data-id]").forEach(t=>{const n=t.dataset.id,s=t.querySelector(".notif-main");s&&t.dataset.seen==="0"&&s.addEventListener("click",()=>so(n))}),e.querySelectorAll(".notif-apply").forEach(t=>t.addEventListener("click",n=>{n.stopPropagation(),ao(t.dataset.id)})),e.querySelectorAll(".notif-link").forEach(t=>t.addEventListener("change",n=>{n.stopPropagation(),t.value&&oo(t.dataset.id,t.value)})),e.querySelectorAll(".notif-open").forEach(t=>t.addEventListener("click",()=>{const n=t.dataset.pid;L("jobs"),it(n)})))}async function so(e){try{await fetch(`/api/notifications/${e}/seen`,{method:"POST"})}catch{return}await te()}async function ao(e){let t;try{t=await fetch(`/api/notifications/${e}/apply`,{method:"POST"})}catch(s){l(`apply failed: ${s.message}`);return}if(!t.ok){l((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}const n=await t.json().catch(()=>({}));l(`status set to ${n.applied||"updated"}`),await te(),await $()}async function oo(e,t){let n;try{n=await fetch(`/api/notifications/${e}/link`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({posting_id:t})})}catch(s){l(`link failed: ${s.message}`);return}if(!n.ok){l((await n.text().catch(()=>"")).trim()||`HTTP ${n.status}`);return}l("linked to role"),await te()}async function io(){const e=document.getElementById("notifications-sync");e&&(e.disabled=!0,e.textContent="Syncing…");try{const t=await fetch("/api/gmail/sync",{method:"POST"});l(t.ok?"synced":(await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`)}catch(t){l(`sync failed: ${t.message}`)}e&&(e.disabled=!1,e.textContent="Sync now"),await te(),await $()}document.getElementById("open-notifications").onclick=()=>L("inbox"),document.getElementById("notifications-sync").onclick=io,document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ke(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&Ke(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),c.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function co(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function ro(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ye(e){return e.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+|mailto:[^\s)]+)\)/g,(t,n,s)=>`<a href="${s}" target="_blank" rel="noopener noreferrer">${n}</a>`).replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*\n]+)\*/g,"$1<em>$2</em>")}function lo(e){const t=String(e||"").split(`
`),n=[];let s=null;const a=()=>{s&&(n.push("</"+s+">"),s=null)};let o=0;for(;o<t.length;){const i=t[o];if(/^```/.test(i)){a(),o++;const f=[];for(;o<t.length&&!/^```/.test(t[o]);)f.push(t[o]),o++;o++,n.push("<pre><code>"+r(f.join(`
`))+"</code></pre>");continue}const d=i.match(/^(#{1,6})\s+(.*)$/);if(d){a();const f=d[1].length;n.push("<h"+f+">"+Ye(r(d[2]))+"</h"+f+">"),o++;continue}const u=i.match(/^\s*[-*]\s+(.*)$/);if(u){s!=="ul"&&(a(),n.push("<ul>"),s="ul"),n.push("<li>"+Ye(r(u[1]))+"</li>"),o++;continue}const p=i.match(/^\s*\d+\.\s+(.*)$/);if(p){s!=="ol"&&(a(),n.push("<ol>"),s="ol"),n.push("<li>"+Ye(r(p[1]))+"</li>"),o++;continue}if(i.trim()===""){a(),o++;continue}a();const m=[];for(;o<t.length&&t[o].trim()!==""&&!/^```|^#{1,6}\s|^\s*[-*]\s+|^\s*\d+\.\s+/.test(t[o]);)m.push(Ye(r(t[o]))),o++;n.push("<p>"+m.join("<br>")+"</p>")}return a(),n.join("")}function Ze(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,e==="assistant"?n.innerHTML=lo(t||""):n.textContent=t||"",n}function At(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function uo(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=c.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(c.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Wn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=co(n.content);if(n.role==="user")s&&t.appendChild(Ze("user",s));else if(n.role==="assistant"){const a=ro(n.content);if(!s&&!a.length)continue;const o=Ze("assistant",s);if(a.length){const i=document.createElement("div");i.className="chat-tools",i.textContent="· used "+a.join(", "),o.appendChild(i)}t.appendChild(o)}}t.children.length||t.appendChild(uo()),At()}async function Pt(e,t,n){if(!c.meta||!c.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}c.chat.es&&(c.chat.es.close(),c.chat.es=null),c.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),i=await fetch("/api/chat/threads?"+o);if(!i.ok)throw new Error((await i.text().catch(()=>"")).trim()||"HTTP "+i.status);const d=await i.json();c.chat.threadId=d.thread.id,Wn(d.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+r(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function qt(){c.chat.es&&(c.chat.es.close(),c.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Qe(e){c.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Kn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Yn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||c.chat.streaming||!c.chat.threadId)return;e.value="",Kn(),Qe(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Ze("user",t));const a=Ze("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),At();let o="";const i=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Qe(!1)},d=c.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){i(m.message);return}if(!u.ok){i((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");c.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,a.textContent=o,At()}),p.addEventListener("end",async m=>{p.close(),c.chat.es===p&&(c.chat.es=null),a.classList.remove("chat-streaming"),Qe(!1),c.chat.threadId===d&&await po(),mo(),typeof m.data=="string"&&m.data.indexOf("error")===0&&l("chat: "+m.data)}),p.onerror=()=>{p.close(),c.chat.es===p&&(c.chat.es=null),a.classList.remove("chat-streaming"),Qe(!1)}}async function po(){const e=c.chat.scope,t=c.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Wn(a.messages||[])}catch{}}function mo(){q(),$(),C(),c.openId&&me(c.openId)}document.getElementById("open-chat").onclick=()=>Pt("global","",""),document.getElementById("chat-close").onclick=qt,document.getElementById("chat-scrim").onclick=qt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Yn()}),document.getElementById("chat-input").addEventListener("input",Kn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Yn())}),Gt("#t tbody",ns),Gt("#jt tbody",ss);const ho=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();L(ho==="jobs"?"jobs":"companies",{render:!1});function Zn(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||Zn()}),window.addEventListener("pageshow",e=>{e.persisted&&Zn()}),q(),$(),C(),_e(),yt(),_t(),Ct(),fe(),te(),setInterval(te,9e4),P(),function(){const t=/[?&]gmail=(connected|error)/.exec(location.search);t&&(l(t[1]==="connected"?"Gmail connected":"Gmail connection failed"),history.replaceState(null,"",location.pathname+location.hash))}()}vo({"":{view:()=>({mount(w){w.innerHTML=Eo,xo()}}),chrome:!1}},{title:"scout"});ko();$o();
