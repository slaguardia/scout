(function(){const b=document.createElement("link").relList;if(b&&b.supports&&b.supports("modulepreload"))return;for(const y of document.querySelectorAll('link[rel="modulepreload"]'))i(y);new MutationObserver(y=>{for(const S of y)if(S.type==="childList")for(const c of S.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&i(c)}).observe(document,{childList:!0,subtree:!0});function x(y){const S={};return y.integrity&&(S.integrity=y.integrity),y.referrerPolicy&&(S.referrerPolicy=y.referrerPolicy),y.crossOrigin==="use-credentials"?S.credentials="include":y.crossOrigin==="anonymous"?S.credentials="omit":S.credentials="same-origin",S}function i(y){if(y.ep)return;y.ep=!0;const S=x(y);fetch(y.href,S)}})();function mo(k,b){const x=b.replace(/^#/,"");let i=null;for(const y of Object.keys(k))(x===y||x.startsWith(y))&&(i===null||y.length>i.length)&&(i=y);return i===null&&""in k&&(i=""),i}function ho(k){return typeof k=="function"?{view:k,chrome:!0}:{view:k.view,chrome:k.chrome!==!1}}function fo(k,b={}){const x=b.root??document.body,i=b.title??document.title??"",y=b.brandHref??"#",S=document.createElement("main"),c=document.createElement("header");c.className="cap-head";const A=document.createElement("a");A.className="brand",A.href=y,A.textContent=i,A.setAttribute("aria-label",`${i} — home`),c.appendChild(A);const q=document.createElement("nav");q.className="cap-nav",q.setAttribute("aria-label","Views");for(const P of b.nav??[]){const L=document.createElement("a");L.href=P.href,L.textContent=P.label,P.ariaLabel&&L.setAttribute("aria-label",P.ariaLabel),q.appendChild(L)}c.appendChild(q);const E=document.createElement("section");E.className="tk-content",S.appendChild(c),S.appendChild(E);const R=document.createElement("div");R.className="tk-bleed";const Z=P=>{var L;for(const C of Array.from(q.querySelectorAll("a"))){const Q=((L=C.getAttribute("href"))==null?void 0:L.replace(/^#/,""))??"";C.toggleAttribute("aria-current",P!==null&&P!==""&&Q===P),C.hasAttribute("aria-current")&&C.setAttribute("aria-current","page")}};let fe=0;const Le=()=>{const P=mo(k,location.hash);if(Z(P),P===null){R.isConnected&&R.remove(),S.isConnected||x.appendChild(S),go(E,"Not found.");return}const{view:L,chrome:C}=ho(k[P]),Q=C?E:R;C?(R.isConnected&&R.remove(),S.isConnected||x.appendChild(S)):(S.isConnected&&S.remove(),R.isConnected||x.appendChild(R)),Q.replaceChildren();const Ze=L(),Be=++fe,ge=Ze.mount(Q);ge instanceof Promise&&ge.catch(D=>{Be===fe&&yo(Q,String(D))})};window.addEventListener("hashchange",Le),Le()}function go(k,b){k.replaceChildren();const x=document.createElement("div");x.className="tk-empty",x.textContent=b,k.appendChild(x)}function yo(k,b){k.replaceChildren();const x=document.createElement("div");x.className="tk-error",x.textContent=b,k.appendChild(x)}function vo(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(b=>{for(const x of b)x.unregister()}),window.caches&&caches.keys().then(b=>{for(const x of b)caches.delete(x)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function bo(){let k;try{k=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!k.ok)return null;let b;try{b=await k.json()}catch{return null}return typeof b.email=="string"&&b.email?{email:b.email}:null}const wo=`
<div class="layout">
<aside class="sidebar">
...
`;function ko(k){const b={k:"verdict",dir:1},x={k:"created_at",dir:1},i={rows:[],sort:{...b},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...x},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null,gmail:null,notifications:{notifications:[],unread:0,followups:[]},settingsGroup:"outreach"},y=e=>"pill pill-"+(e||"none"),S='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',c=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),A=e=>/^https?:\/\//i.test(String(e??""))?c(e):"#";async function q(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],F()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],j(),R(),fe()}function R(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&ee())}let Z=null;function fe(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!Z?Z=setInterval(Le,4e3):!e&&Z&&(clearInterval(Z),Z=null)}async function Le(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&(j(),R()),fe()}async function P(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(i.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(i.followupTemplate=e.content)}).catch(()=>{})]),tt(),i.view==="jobs"&&j()}function L(e,{render:t=!0}={}){i.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none";const n=(d,u)=>{const p=document.getElementById(d);p&&(p.style.display=u?"":"none")};n("settings-view",e==="settings"),n("inbox-view",e==="inbox"),n("docs-view",e==="docs"),document.getElementById("open-settings").classList.toggle("is-active",e==="settings");const s=document.getElementById("open-notifications");s&&s.classList.toggle("is-active",e==="inbox");const a=document.getElementById("open-docs");a&&a.classList.toggle("is-active",e==="docs");const o=e==="companies"||e==="jobs";document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none";const l=document.getElementById("block-columns");l&&(l.style.display=o?"":"none"),Rt(),t&&(e==="jobs"?j():e==="settings"?J():e==="inbox"?(zn(),ne()):e==="docs"?za():F())}async function C(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}i.stats=e,Q()}function Q(){J()}function Ze(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function Be(e){return e.slice().sort((t,n)=>i.sort.dir*Ze(t,n,i.sort.k))}function ge(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(s=>{s.getAttribute(t)===n.k?s.dataset.sort=n.dir<0?"desc":"asc":delete s.dataset.sort})}const D=new Set;let se=!1,ae=!1;const Zn=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function Qn(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+Zn.map(([t,n,s])=>z("data-v",t,n,s,D.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+z("data-toggle","flagged","⚑ Flagged","",se)+z("data-toggle","enriched","Enriched","",ae),Pt())}function Pt(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const l of i.rows){const d=l.verdict||"__none__";e[d]=(e[d]|0)+1,l.flagged&&t++,l.enriched&&n++}nt("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=D.size+(se?1:0)+(ae?1:0);Kt("fdrop-cfilters-btn",o,o>0)}function qt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(D.size&&!D.has(t.verdict||"__none__")||se&&!t.flagged||ae&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Xn=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],es=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Ht(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Ot=Ht("scout-hidden-cols"),Nt=Ht("scout-hidden-jcols");function Qe(){return i.view==="jobs"?{cols:es,hidden:Nt,key:"scout-hidden-jcols"}:{cols:Xn,hidden:Ot,key:"scout-hidden-cols"}}function oe(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Ot.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Nt.has(e.dataset.col)?"none":""})}function Rt(){const e=Qe(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>z("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Dt())}function Dt(){const e=Qe(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Ut(e){return`
  ...
  `,_s();const l=document.getElementById("pursuit-company-link");l&&l.addEventListener("click",()=>me(e.company_id)),vs(e),U(document.getElementById("pursuit-title-input"),p=>on(e,"title",p)),U(document.getElementById("pursuit-url-input"),p=>gs(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>ys(e,d)),U(document.getElementById("pursuit-notes-input"),p=>Ls(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>U(p,m=>on(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>$s(e)),de(),we(),t&&(t.scrollTop=s),dn=e.posting_id}function Ts(e){return`
  ...
  `;const w=document.getElementById("posting-add-btn");w&&w.addEventListener("click",()=>ca(e)),mt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(g=>{g.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(V=>V.classList.remove("is-on")),g.classList.add("is-on")})});const I=document.getElementById("ve-save-btn");I&&I.addEventListener("click",()=>ia(e)),U(document.getElementById("pane-title-input"),g=>rn(e,"name",g)),document.querySelectorAll("#facts-body [data-k]").forEach(g=>U(g,V=>rn(e,g.dataset.k,V))),U(document.getElementById("pane-domain-input"),g=>xs(e,g)),U(document.getElementById("pane-notes-input"),g=>Ss(e,g),{multiline:!0});const v=document.getElementById("flag-toggle-btn");v&&v.addEventListener("click",()=>In(e.company_id));const M=document.getElementById("review-stamp-btn");M&&M.addEventListener("click",()=>oa(e.company_id));const N=document.getElementById("rescore-btn");N&&N.addEventListener("click",()=>gt("verdict",{company_ids:[e.company_id]}));const O=document.getElementById("reenrich-btn");O&&O.addEventListener("click",()=>gt("enrich",{company_ids:[e.company_id]}));const B=document.getElementById("company-delete-btn");B&&B.addEventListener("click",()=>ks(e))}function aa(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function he(e,t){const n=i.openDetail;if(!n||!i.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=pt(n),mt())}function mt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||at(e.dataset.pid)})})}async function ca(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){r("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){r(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");r(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const l=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==l.id),e.postings.unshift(l);const d=document.getElementById("postings-list");d&&(d.innerHTML=pt(e),mt()),t.value="",n.value="",s.disabled=!1,E(),r("link added")}async function $e(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){qe(`<div class="muted">Failed to load trail: ${c(s.message)}</div>`);return}if(!t.ok){qe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){qe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}qe(n.map(la).join(""))}function la(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(c);return e.run_id&&t.push("run "+c(e.run_id.slice(0,8))),`
...
`)):document.getElementById("editor-text").value=l.content||"",a&&(document.getElementById("editor-enabled").checked=l.enabled!==!1),l.taste_version&&(document.getElementById("editor-ver").textContent="version "+l.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Ue(){document.getElementById("editor-scrim").classList.remove("open"),H=null}let _=null,bt=[],wt=[];const ga=["location.allowed","verticals.excluded","verticals.allowed"],kt={"verticals.excluded":"pf-vertical-tags","verticals.allowed":"pf-vertical-tags"};function K(e){const[t,n]=e.split(".");return _[t]&&_[t][n]||[]}function Ve(e,t){const[n,s]=e.split(".");(_[n]=_[n]||{})[s]=t}function Fe(e,t){const n=t.toLowerCase();return e.some(s=>String(s).toLowerCase()===n)}function Et(){ga.forEach(e=>{const t=document.querySelector(`.pf-chips[data-field="${e}"]`);if(!t)return;const n=kt[e]?` list="${kt[e]}"`:"",s=kt[e]?"type to search…":"type &amp; Enter…";t.innerHTML=K(e).map((a,o)=>`<span class="pf-chip">${c(a)}<button class="pf-chip-x" data-field="${e}" data-i="${o}" title="remove" aria-label="remove ${c(a)}">×</button></span>`).join("")+`<input class="pf-chip-input" data-field="${e}"${n} type="text" placeholder="${s}" spellcheck="false" autocomplete="off" />`})}function ya(e,t){var s;const n=(t||"").trim();if(n){const a=K(e);Fe(a,n)||Ve(e,[...a,n])}Et(),(s=document.querySelector(`.pf-chip-input[data-field="${e}"]`))==null||s.focus()}function Pn(e,t){const n=K(e).slice();n.splice(t,1),Ve(e,n),Et()}function qn(){const e=document.getElementById("pf-stages");if(!e)return;const t=K("funding_stage.allowed");e.innerHTML=wt.map(n=>{const s=Fe(t,n.value),a=n.count?` <span class="pf-stage-n">${n.count}</span>`:"";return`<button class="pf-stage${s?" is-on":""}" data-stage="${c(n.value)}">${c(n.value)}${a}</button>`}).join("")}function va(e){const t=K("funding_stage.allowed");Ve("funding_stage.allowed",Fe(t,e)?t.filter(n=>String(n).toLowerCase()!==e.toLowerCase()):[...t,e]),qn()}function ba(){const e=document.getElementById("pf-vertical-tags");e&&(e.innerHTML=bt.map(t=>`<option value="${c(t.value)}" label="${t.count}"></option>`).join(""))}function wa(){return{location:{allowed:[],remote_ok:!0},headcount:{min:0,max:0},verticals:{allowed:[],excluded:[]},funding_stage:{allowed:[]}}}function ka(){return`<div class="set-field pf-inline">
  ...
  </div>`}async function Hn(e=!1){if(document.getElementById("pf-enabled"))try{const[t,n]=await Promise.all([(await fetch("/api/taste-filter"+(e?"?default=1":""))).json(),bt.length||wt.length?Promise.resolve(null):fetch("/api/filter-options").then(s=>s.json()).catch(()=>null)]);n&&(bt=n.verticals||[],wt=n.stages||[]),_=Object.assign(wa(),t.rules||{}),_.location=Object.assign({allowed:[],remote_ok:!0},_.location),_.headcount=Object.assign({min:0,max:0},_.headcount),_.verticals=Object.assign({allowed:[],excluded:[]},_.verticals),_.funding_stage=Object.assign({allowed:[]},_.funding_stage),e||(document.getElementById("pf-enabled").checked=t.enabled!==!1),document.getElementById("pf-remote-ok").checked=!!_.location.remote_ok,document.getElementById("pf-hc-min").value=String(_.headcount.min||0),document.getElementById("pf-hc-max").value=String(_.headcount.max||0),ba(),Et(),qn()}catch(t){r(`failed to load pre-filter: ${t.message}`)}}async function Ea(){if(!_)return;_.location.remote_ok=document.getElementById("pf-remote-ok").checked,_.headcount.min=Math.max(0,parseInt(document.getElementById("pf-hc-min").value,10)||0),_.headcount.max=Math.max(0,parseInt(document.getElementById("pf-hc-max").value,10)||0),document.querySelectorAll(".pf-chip-input").forEach(n=>{const s=n.value.trim();s&&!Fe(K(n.dataset.field),s)&&Ve(n.dataset.field,[...K(n.dataset.field),s])});const e=document.getElementById("pf-enabled").checked;let t;try{t=await fetch("/api/taste-filter",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({rules:_,enabled:e})})}catch(n){r(`save failed: ${n.message}`);return}if(!t.ok){r(`save failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`);return}r("pre-filter saved"),C()}const $a=[{key:"experience",hard:!0},{key:"voice",hard:!1},{key:"logistics",hard:!1}];async function On(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{Ia(await(await fetch("/api/outreach/sources")).json())}catch(e){r(`failed to load sources: ${e.message}`)}}function $t(){document.getElementById("sources-scrim").classList.remove("open")}function Ia(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):$a,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],l=o.length?o.map(d=>`<li><span class="src-title">${c(d.title||d.page_id)}</span></li>`).join(""):`<li class="dim small">${a.hard?"none yet — add an experience page to your brain":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${c(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${l}</ul></div>`}).join("")}async function xa(){if(!H)return;const e=document.getElementById("editor-text").value;let t;vt(H)?t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)}:(t={content:e},H.startsWith("outreach-prompts/")&&H!=="outreach-prompts/fill"&&(t.enabled=document.getElementById("editor-enabled").checked));let n;try{n=await fetch(`/api/${H}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){r(`save failed: ${o.message}`);return}if(!n.ok){r(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const s=await n.json();s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version);const a=vt(H);H==="followup-template"&&(i.followupTemplate=e),r(`${De(H)} saved`),Ue(),a&&P(),C()}async function Sa(){if(!H||!H.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${H}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){r(`reset failed: ${n.message}`);return}if(!e.ok){r(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",r(`${De(H)} reset to default`)}function Nn(e,t,n){e.k!==t?(e.k=t,e.dir=1):e.dir===1?e.dir=-1:Object.assign(e,n)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{Nn(i.sort,e.dataset.k,b),F()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{Nn(i.jsort,e.dataset.jk,x),j()}}),document.getElementById("tab-companies").onclick=()=>L("companies"),document.getElementById("tab-jobs").onclick=()=>L("jobs"),document.getElementById("q").oninput=F,document.getElementById("fdrop-cfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.dataset.toggle==="flagged")se=!se,X(t,se);else if(t.dataset.toggle==="enriched")ae=!ae,X(t,ae);else if(t.hasAttribute("data-v")){const n=t.getAttribute("data-v");D.has(n)?D.delete(n):D.add(n),X(t,D.has(n))}else return;F()}}),document.getElementById("fdrop-columns-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item[data-col]");if(!t)return;const n=Qe(),s=t.getAttribute("data-col");n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),X(t,!n.hidden.has(s)),oe(),Dt()}),document.getElementById("jq").oninput=j;for(const e of["fdrop-cfilters","fdrop-columns","fdrop-jfilters"]){const t=document.getElementById(e);t.querySelector(".fdrop-btn").addEventListener("click",s=>{s.stopPropagation();const a=t.classList.contains("is-open");st(),a||cs(t)}),t.querySelector(".fdrop-menu").addEventListener("click",s=>s.stopPropagation())}document.addEventListener("click",st),document.getElementById("fdrop-jfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-all");if(t){if(t.getAttribute("data-all")==="stage"){const s=["",...i.applicationStages];$=s.every(a=>$.has(a))?new Set:new Set(s)}else{const s=["",...i.outreachStatuses];T=T&&s.every(a=>T.has(a))?new Set:new Set(s)}tt(),j();return}const n=e.target.closest(".fdrop-item");if(n){if(n.hasAttribute("data-stage")){const s=n.getAttribute("data-stage");$.has(s)?$.delete(s):$.add(s),X(n,$.has(s))}else if(n.dataset.toggle==="nextup")ie=!ie,X(n,ie);else if(n.hasAttribute("data-status")){const s=n.getAttribute("data-status");T.has(s)?T.delete(s):T.add(s),X(n,T.has(s))}else return;is(),j()}}),Qn(),Rt(),oe(),document.getElementById("pane-close").onclick=ke,document.getElementById("scrim").onclick=ke,document.getElementById("pursuit-close").onclick=ye,document.getElementById("pursuit-scrim").onclick=ye,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.querySelector(".fdrop.is-open")){st();return}if(document.getElementById("chat-pane").classList.contains("open")){At();return}if(document.getElementById("profile-scrim").classList.contains("open")){Ct();return}if(document.getElementById("add-scrim").classList.contains("open")){Re();return}if(document.getElementById("run-scrim").classList.contains("open")){Je();return}if(document.getElementById("help-scrim").classList.contains("open")){ze();return}if(document.getElementById("relink-scrim").classList.contains("open")){re();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){Me();return}if(document.getElementById("deljob-scrim").classList.contains("open")){Ae();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(ot==="pursuit"&&n){ye();return}if(ot==="company"&&t){ke();return}if(t){ke();return}ye();return}if(document.getElementById("key-scrim").classList.contains("open")){Bt();return}if(document.getElementById("sources-scrim").classList.contains("open")){$t();return}if(document.getElementById("editor-scrim").classList.contains("open")){Ue();return}if(document.getElementById("gmail-config-scrim").classList.contains("open")){Te();return}});let It=null;const Ta={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Rn(e){if(i.meta&&i.meta.control===!1){r("control surface disabled");return}It=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Ta[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Je(){document.getElementById("run-scrim").classList.remove("open"),It=null}document.getElementById("btn-enrich").onclick=()=>Rn("enrich"),document.getElementById("btn-verdict").onclick=()=>Rn("verdict"),document.getElementById("run-cancel").onclick=Je,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Je()},document.getElementById("run-go").onclick=()=>{const e=It,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(Je(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),gt(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=pa;const _a={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Dn(e){const t=_a[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const l=document.createElement("div");l.className="help-item-desc",l.textContent=s.desc;const d=document.createElement("a");d.className="help-link",d.textContent="Learn more →",d.onclick=()=>{ze(),L("docs"),Ga(s.sec)},a.appendChild(o),a.appendChild(l),a.appendChild(d),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function ze(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Dn("add"),document.getElementById("help-run").onclick=()=>Dn("run"),document.getElementById("help-close").onclick=ze,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&ze()},document.getElementById("add-cancel").onclick=Re,document.getElementById("add-save").onclick=Mn,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Re()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>_n(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Ln),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Mn()))}),document.getElementById("add-vertical-filter").addEventListener("input",Bn),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&da(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=ma,document.getElementById("drawer-close").onclick=Sn,(()=>{const e=document.getElementById("drawer");e.addEventListener("mouseenter",He),e.addEventListener("mouseleave",()=>{!xe&&e.classList.contains("open")&&Tn()})})(),document.getElementById("editor-cancel").onclick=Ue,document.getElementById("editor-save").onclick=xa,document.getElementById("editor-reset").onclick=Sa,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Ue()},document.getElementById("sources-close").onclick=$t,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&$t()},document.addEventListener("click",e=>{var a;const t=e.target.closest(".pf-stage");if(t){va(t.dataset.stage);return}const n=e.target.closest(".pf-chip-x");if(n){Pn(n.dataset.field,parseInt(n.dataset.i,10));return}const s=e.target.closest(".pf-chips");s&&e.target===s&&((a=s.querySelector(".pf-chip-input"))==null||a.focus())}),document.addEventListener("keydown",e=>{const t=e.target.closest(".pf-chip-input");if(t){if(e.key==="Enter"||e.key===",")e.preventDefault(),ya(t.dataset.field,t.value);else if(e.key==="Backspace"&&!t.value){const n=K(t.dataset.field);n.length&&Pn(t.dataset.field,n.length-1)}}}),document.getElementById("key-cancel").onclick=Bt,document.getElementById("key-save").onclick=Jn,document.getElementById("key-remove").onclick=Fa,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&Bt()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),Jn())}),document.getElementById("delcompany-cancel").onclick=Me,document.getElementById("delcompany-confirm").onclick=Es,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&Me()},document.getElementById("deljob-cancel").onclick=Ae,document.getElementById("deljob-confirm").onclick=Is,document.getElementById("deljob-scrim").onclick=e=>{e.target.id==="deljob-scrim"&&Ae()},document.getElementById("relink-cancel").onclick=re,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&re()},document.getElementById("relink-search").addEventListener("input",e=>{cn(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&ln(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&ln(t.dataset.id)});async function xt(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}J()}const St='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',Un='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4.2 7a3.8 3.8 0 0 1 7.6 0c0 3 1.2 4 1.2 4H3s1.2-1 1.2-4z"/><path d="M6.7 13a1.5 1.5 0 0 0 2.6 0"/></svg>',La=[["outreach","Outreach"],["pipeline","Outreach pipeline"],["tracking","Tracking"],["job-hunting","Job hunting"],["integrations","Integrations"]],Ba=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]];function J(){const e=document.getElementById("criteria-stats");if(!e)return;const t=document.getElementById("settings-view");if(t&&t.style.display==="none")return;const n=i.settingsGroup||"outreach";e.innerHTML=`<div class="settings-shell">
...
`):a.content||""}catch{s.value=""}s.dataset.orig=s.value,s.dataset.loaded="1",s.addEventListener("blur",()=>ja(e))}async function ja(e){const t=e.dataset.kind,n=e.dataset.list==="1",s=e.querySelector(".set-textarea");if(s.dataset.loaded!=="1"||s.value===s.dataset.orig)return;const a=n?{statuses:s.value.split(/\r?\n/).map(l=>l.trim()).filter(Boolean)}:{content:s.value};let o;try{o=await fetch(`/api/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}catch(l){r(`save failed: ${l.message}`);return}if(!o.ok){r(`save failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}s.dataset.orig=s.value,Tt(e),t==="followup-template"&&(i.followupTemplate=s.value),n&&(await P(),j())}function _t(e){e.querySelectorAll(".set-field[data-kind]").forEach(Ca)}function Ma(e){e.innerHTML=Y("outreach-subject","Email subject","The send subject — {{role}} / {{company}} substitution, no LLM.",2,!1)+Y("outreach-template","Email body","Verbatim prose with the writer's fill-in holes.",16,!1)+Y("outreach-signature","Email signature","A fixed sign-off appended to every sent email (blank = none).",3,!1)+Y("followup-template","Follow-up template","Copy-paste follow-up — {{contact_name}}, {{role}}, {{company}}, {{last_sent}}, {{last_message}}.",9,!1)+`<div class="set-field">
    ...
    </div>`,_t(e);const t=e.querySelector(".set-fu-interval");t&&t.addEventListener("change",async()=>{const n=Math.max(0,Math.min(90,parseInt(t.value,10)||0));t.value=String(n),await te("PUT","/api/followup-interval",{days:n})&&(i.followupInterval=n,r("follow-up interval saved"))})}function Aa(e){e.innerHTML=Ba.map(([t,n,s])=>`
    ...
    </div>`:o=Y("taste","Taste (local fallback)","Local fallback criteria used when the brain is unreachable.",12,!1),e.innerHTML=o+Y("playbook","Playbook","How scout judges — the reasoning rules behind every verdict.",12,!1)+ka(),_t(e);const l=e.querySelector("#brief-refresh");l&&l.addEventListener("click",Ja),Hn();const d=e.querySelector("#pf-save");d&&d.addEventListener("click",Ea);const u=e.querySelector("#pf-reset");u&&u.addEventListener("click",()=>Hn(!0))}function Oa(e){const t=e.callback_uri||"(your scout URL)/api/gmail/callback",n=e.scopes||["openid","email","https://www.googleapis.com/auth/gmail.send","https://www.googleapis.com/auth/gmail.readonly"];return`<details class="set-help"${e.configured?"":" open"}>
    ...
    </div>`;const d=e.querySelector("#set-ak-save");d&&d.addEventListener("click",async()=>{const v=e.querySelector("#set-ak-input").value.trim();if(!v){r("paste a key first");return}const M=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:v})});if(!M.ok){r((await M.text().catch(()=>"")).trim()||`HTTP ${M.status}`);return}r("Anthropic key saved"),await Ie(),await Lt()});const u=e.querySelector("#set-ak-remove");u&&u.addEventListener("click",async()=>{const v=await fetch("/api/integrations/anthropic",{method:"DELETE"});if(!v.ok){r(`HTTP ${v.status}`);return}r("Anthropic key removed"),await Ie(),await Lt()});const p=e.querySelector("#set-gm-save");p&&p.addEventListener("click",async()=>{const v=e.querySelector("#set-gm-id").value.trim(),M=e.querySelector("#set-gm-secret").value,N=e.querySelector("#set-gm-redirect").value.trim();if(!v){r("client ID is required");return}const O=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:v,client_secret:M,redirect_uri:N})});if(!O.ok){r((await O.text().catch(()=>"")).trim()||`HTTP ${O.status}`);return}r("Gmail OAuth client saved"),await Se()});const m=e.querySelector("#set-gm-connect");m&&m.addEventListener("click",Ra);const f=e.querySelector("#set-gm-disconnect");f&&f.addEventListener("click",Da);const w=e.querySelector("#gm-copy-cb");w&&w.addEventListener("click",()=>Oe(i.gmail&&i.gmail.callback_uri||"","redirect URI copied"));const I=e.querySelector("#set-autoflip");I&&I.addEventListener("change",async()=>{let v=!1;try{v=(await fetch("/api/gmail/autoflip",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:I.checked})})).ok}catch{v=!1}v?(i.gmail&&(i.gmail.autoflip=I.checked),r(`auto-update ${I.checked?"on":"off"}`)):(I.checked=!I.checked,r("failed to save"))})}async function Lt(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}J()}async function Se(){try{i.gmail=await(await fetch("/api/gmail/status")).json()}catch{i.gmail=null}J()}async function Ra(){let e;try{e=await fetch("/api/gmail/connect")}catch(n){r(`connect failed: ${n.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}let t={};try{t=await e.json()}catch{}t.auth_url?window.location.href=t.auth_url:r("could not start the Gmail connect flow")}async function Da(){if(!confirm("Disconnect Gmail? Sending and sync stop; already-synced data stays."))return;let e;try{e=await fetch("/api/gmail/disconnect",{method:"DELETE"})}catch(t){r(`disconnect failed: ${t.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}r("Gmail disconnected"),await Se()}function Te(){document.getElementById("gmail-config-scrim").classList.remove("open")}async function Ua(){const e=document.getElementById("gmail-client-id").value.trim(),t=document.getElementById("gmail-client-secret").value,n=document.getElementById("gmail-redirect").value.trim();if(!e){r("client ID is required");return}let s;try{s=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:e,client_secret:t,redirect_uri:n})})}catch(a){r(`save failed: ${a.message}`);return}if(!s.ok){r((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`);return}r("Gmail OAuth client saved — click Connect"),Te(),await Se()}async function Va(){if(!confirm("Remove the stored Google OAuth client? Connecting needs it re-entered (or set via env)."))return;let e;try{e=await fetch("/api/gmail/config",{method:"DELETE"})}catch(t){r(`remove failed: ${t.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}r("OAuth client removed"),Te(),await Se()}function Fn(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function Bt(){document.getElementById("key-scrim").classList.remove("open")}async function Jn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){r("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){r(`save failed: ${a.message}`),n();return}if(!s.ok){r((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),r("Anthropic key saved"),await Ie(),Fn(),J()}async function Fa(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){r(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){r((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),r(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await Ie(),Fn(),J()}async function Ja(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){r(`refresh failed: ${n.message}`),xt();return}if(!t.ok){const n=await t.text().catch(()=>"");r(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),xt();return}i.profile=await t.json(),J(),r("company-fit brief refreshed"),C()}function Ct(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ct,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ct()};function za(){const e=document.querySelector("#docs-nav a");Ge(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ge(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Ga(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ge(e)}document.getElementById("open-docs").onclick=()=>L("docs");function Wa(){L("settings")}document.getElementById("open-settings").onclick=Wa,document.getElementById("gmail-config-cancel").onclick=Te,document.getElementById("gmail-config-save").onclick=Ua,document.getElementById("gmail-config-remove").onclick=Va,document.getElementById("gmail-config-scrim").onclick=e=>{e.target.id==="gmail-config-scrim"&&Te()};function Ka(){const e=document.getElementById("notif-badge");if(!e)return;const t=(i.notifications&&i.notifications.unread)|0;t>0?(e.textContent=t>99?"99+":String(t),e.style.display=""):e.style.display="none"}async function ne(){try{i.notifications=await(await fetch("/api/notifications")).json()}catch{return}Ka(),i.view==="inbox"&&zn()}function Ya(){return'<option value="">link to role…</option>'+(i.jobs||[]).map(t=>`<option value="${c(t.posting_id)}">${c((t.company||"")+" — "+(t.title||"(untitled)"))}</option>`).join("")}function Za(e){const t=e.company||e.role?`<div class="notif-ctx">${c([e.company,e.role].filter(Boolean).join(" · "))}</div>`:'<div class="notif-ctx dim">not linked to a role</div>',n=e.created_at?`<span class="notif-when">${c((e.created_at||"").replace("T"," ").slice(0,16))}</span>`:"",s=e.kind==="app_status"&&e.suggested_status&&!e.actioned&&e.posting_id?`<button class="btn btn-primary notif-apply" data-id="${e.id}">Apply: ${c(e.suggested_status)}</button>`:"",a=e.posting_id?"":`<select class="input notif-link" data-id="${e.id}" title="link this to a role">${Ya()}</select>`;return`<div class="notif-item${e.seen?"":" is-unread"}" data-id="${e.id}" data-seen="${e.seen?1:0}">
...
`))+"</code></pre>");continue}const d=l.match(/^(#{1,6})\s+(.*)$/);if(d){a();const f=d[1].length;n.push("<h"+f+">"+We(c(d[2]))+"</h"+f+">"),o++;continue}const u=l.match(/^\s*[-*]\s+(.*)$/);if(u){s!=="ul"&&(a(),n.push("<ul>"),s="ul"),n.push("<li>"+We(c(u[1]))+"</li>"),o++;continue}const p=l.match(/^\s*\d+\.\s+(.*)$/);if(p){s!=="ol"&&(a(),n.push("<ol>"),s="ol"),n.push("<li>"+We(c(p[1]))+"</li>"),o++;continue}if(l.trim()===""){a(),o++;continue}a();const m=[];for(;o<t.length&&t[o].trim()!==""&&!/^```|^#{1,6}\s|^\s*[-*]\s+|^\s*\d+\.\s+/.test(t[o]);)m.push(We(c(t[o]))),o++;n.push("<p>"+m.join("<br>")+"</p>")}return a(),n.join("")}function Ke(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,e==="assistant"?n.innerHTML=io(t||""):n.textContent=t||"",n}function jt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function co(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Gn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=ao(n.content);if(n.role==="user")s&&t.appendChild(Ke("user",s));else if(n.role==="assistant"){const a=oo(n.content);if(!s&&!a.length)continue;const o=Ke("assistant",s);if(a.length){const l=document.createElement("div");l.className="chat-tools",l.textContent="· used "+a.join(", "),o.appendChild(l)}t.appendChild(o)}}t.children.length||t.appendChild(co()),jt()}async function Mt(e,t,n){if(!i.meta||!i.meta.chat){r("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),l=await fetch("/api/chat/threads?"+o);if(!l.ok)throw new Error((await l.text().catch(()=>"")).trim()||"HTTP "+l.status);const d=await l.json();i.chat.threadId=d.thread.id,Gn(d.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+c(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function At(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Ye(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Wn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Kn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",Wn(),Ye(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Ke("user",t));const a=Ke("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),jt();let o="";const l=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Ye(!1)},d=i.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){l(m.message);return}if(!u.ok){l((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");i.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,a.textContent=o,jt()}),p.addEventListener("end",async m=>{p.close(),i.chat.es===p&&(i.chat.es=null),a.classList.remove("chat-streaming"),Ye(!1),i.chat.threadId===d&&await lo(),ro(),typeof m.data=="string"&&m.data.indexOf("error")===0&&r("chat: "+m.data)}),p.onerror=()=>{p.close(),i.chat.es===p&&(i.chat.es=null),a.classList.remove("chat-streaming"),Ye(!1)}}async function lo(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Gn(a.messages||[])}catch{}}function ro(){q(),E(),C(),i.openId&&me(i.openId)}document.getElementById("open-chat").onclick=()=>Mt("global","",""),document.getElementById("chat-close").onclick=At,document.getElementById("chat-scrim").onclick=At,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Kn()}),document.getElementById("chat-input").addEventListener("input",Wn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Kn())}),Ft("#t tbody",ts),Ft("#jt tbody",ns);const uo=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();L(uo==="jobs"?"jobs":"companies",{render:!1});function Yn(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||Yn()}),window.addEventListener("pageshow",e=>{e.persisted&&Yn()}),q(),E(),C(),Ie(),ft(),xt(),Lt(),Se(),ne(),setInterval(ne,9e4),P(),function(){const t=/[?&]gmail=(connected|error)/.exec(location.search);t&&(r(t[1]==="connected"?"Gmail connected":"Gmail connection failed"),history.replaceState(null,"",location.pathname+location.hash))}()}fo({"":{view:()=>({mount(k){k.innerHTML=wo,ko()}}),chrome:!1}},{title:"scout"});vo();bo();
