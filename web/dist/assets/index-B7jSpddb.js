(function(){const v=document.createElement("link").relList;if(v&&v.supports&&v.supports("modulepreload"))return;for(const y of document.querySelectorAll('link[rel="modulepreload"]'))i(y);new MutationObserver(y=>{for(const x of y)if(x.type==="childList")for(const c of x.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&i(c)}).observe(document,{childList:!0,subtree:!0});function $(y){const x={};return y.integrity&&(x.integrity=y.integrity),y.referrerPolicy&&(x.referrerPolicy=y.referrerPolicy),y.crossOrigin==="use-credentials"?x.credentials="include":y.crossOrigin==="anonymous"?x.credentials="omit":x.credentials="same-origin",x}function i(y){if(y.ep)return;y.ep=!0;const x=$(y);fetch(y.href,x)}})();function To(w,v){const $=v.replace(/^#/,"");let i=null;for(const y of Object.keys(w))($===y||$.startsWith(y))&&(i===null||y.length>i.length)&&(i=y);return i===null&&""in w&&(i=""),i}function So(w){return typeof w=="function"?{view:w,chrome:!0}:{view:w.view,chrome:w.chrome!==!1}}function Co(w,v={}){const $=v.root??document.body,i=v.title??document.title??"",y=v.brandHref??"#",x=document.createElement("main"),c=document.createElement("header");c.className="cap-head";const M=document.createElement("a");M.className="brand",M.href=y,M.textContent=i,M.setAttribute("aria-label",`${i} — home`),c.appendChild(M);const N=document.createElement("nav");N.className="cap-nav",N.setAttribute("aria-label","Views");for(const P of v.nav??[]){const q=document.createElement("a");q.href=P.href,q.textContent=P.label,P.ariaLabel&&q.setAttribute("aria-label",P.ariaLabel),N.appendChild(q)}c.appendChild(N);const k=document.createElement("section");k.className="tk-content",x.appendChild(c),x.appendChild(k);const F=document.createElement("div");F.className="tk-bleed";const ne=P=>{var q;for(const C of Array.from(N.querySelectorAll("a"))){const ae=((q=C.getAttribute("href"))==null?void 0:q.replace(/^#/,""))??"";C.toggleAttribute("aria-current",P!==null&&P!==""&&ae===P),C.hasAttribute("aria-current")&&C.setAttribute("aria-current","page")}};let ke=0;const He=()=>{const P=To(w,location.hash);if(ne(P),P===null){F.isConnected&&F.remove(),x.isConnected||$.appendChild(x),jo(k,"Not found.");return}const{view:q,chrome:C}=So(w[P]),ae=C?k:F;C?(F.isConnected&&F.remove(),x.isConnected||$.appendChild(x)):(x.isConnected&&x.remove(),F.isConnected||$.appendChild(F)),ae.replaceChildren();const rt=q(),Pe=++ke,Ee=rt.mount(ae);Ee instanceof Promise&&Ee.catch(J=>{Pe===ke&&Mo(ae,String(J))})};window.addEventListener("hashchange",He),He()}function jo(w,v){w.replaceChildren();const $=document.createElement("div");$.className="tk-empty",$.textContent=v,w.appendChild($)}function Mo(w,v){w.replaceChildren();const $=document.createElement("div");$.className="tk-error",$.textContent=v,w.appendChild($)}function Ao(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(v=>{for(const $ of v)$.unregister()}),window.caches&&caches.keys().then(v=>{for(const $ of v)caches.delete($)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Ho(){let w;try{w=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!w.ok)return null;let v;try{v=await w.json()}catch{return null}return typeof v.email=="string"&&v.email?{email:v.email}:null}const Po=`
<div class="layout">
<aside class="sidebar">
...
`;function qo(w){const v={k:"verdict",dir:1},$={k:"created_at",dir:1},i={rows:[],sort:{...v},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...$},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null,gmail:null,notifications:{notifications:[],unread:0,followups:[]}},y=e=>"pill pill-"+(e||"none"),x='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',c=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),M=e=>/^https?:\/\//i.test(String(e??""))?c(e):"#";async function N(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],W()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],A(),F(),ke()}function F(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&oe())}let ne=null;function ke(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!ne?ne=setInterval(He,4e3):!e&&ne&&(clearInterval(ne),ne=null)}async function He(){let e;try{const s=await fetch("/api/postings");if(!s.ok)return;e=await s.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(s=>[s.posting_id,s.outreach_draft_status])),a=t.some(s=>n.get(s.posting_id)!==s.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,a&&(A(),F()),ke()}async function P(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(i.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(i.followupTemplate=e.content)}).catch(()=>{})]),mt(),i.view==="jobs"&&A()}function q(e,{render:t=!0}={}){i.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none";const n=document.getElementById("settings-view");n&&(n.style.display=e==="settings"?"":"none");const a=document.getElementById("open-settings");a&&a.classList.toggle("is-active",e==="settings"),document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none";const s=document.getElementById("block-columns");s&&(s.style.display=e==="settings"?"none":""),an(),t&&(e==="jobs"?A():e==="settings"?ee():W())}async function C(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}i.stats=e,ae()}function ae(){ee()}function rt(e,t,n){const a=e[n]??"",s=t[n]??"";if(n==="headcount")return(a|0)-(s|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[a]??3)-(o[s]??3)}return String(a).localeCompare(String(s))}function Pe(e){return e.slice().sort((t,n)=>i.sort.dir*rt(t,n,i.sort.k))}function Ee(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(a=>{a.getAttribute(t)===n.k?a.dataset.sort=n.dir<0?"desc":"asc":delete a.dataset.sort})}const J=new Set;let re=!1,de=!1;const ha=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function fa(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+ha.map(([t,n,a])=>Y("data-v",t,n,a,J.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+Y("data-toggle","flagged","⚑ Flagged","",re)+Y("data-toggle","enriched","Enriched","",de),Qt())}function Qt(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const l of i.rows){const d=l.verdict||"__none__";e[d]=(e[d]|0)+1,l.flagged&&t++,l.enriched&&n++}ht("#fdrop-cfilters-menu [data-v]","data-v",e);const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');a&&(a.textContent=t||"");const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');s&&(s.textContent=n||"");const o=J.size+(re?1:0)+(de?1:0);mn("fdrop-cfilters-btn",o,o>0)}function Xt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(J.size&&!J.has(t.verdict||"__none__")||re&&!t.flagged||de&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const ga=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],ya=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function en(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const tn=en("scout-hidden-cols"),nn=en("scout-hidden-jcols");function dt(){return i.view==="jobs"?{cols:ya,hidden:nn,key:"scout-hidden-jcols"}:{cols:ga,hidden:tn,key:"scout-hidden-cols"}}function ue(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=tn.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=nn.has(e.dataset.col)?"none":""})}function an(){const e=dt(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>Y("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),sn())}function sn(){const e=dt(),t=e.cols.filter(a=>e.hidden.has(a.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function on(e){return`
    ...
    `}function cn(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>Dn(t.dataset.id))}const va=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],ba=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function ln(e,t,n=7){const a=document.querySelector(e);if(!a)return;const s=[1,.82,.7,.95,.76,.9,.85];let o="";for(let l=0;l<n;l++){const d=t.map(([u,p])=>{const m=p.endsWith("%")?Math.round(parseFloat(p)*s[l%s.length])+"%":p;return`<td${u?` data-col="${u}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${d}</tr>`}a.innerHTML=o,ue()}function W(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=Pe(Xt());Qt(),document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const a=document.createElement("tr");a.dataset.id=n.company_id,a.innerHTML=on(n),a.addEventListener("click",s=>{s.target.closest("a, .flag-btn")||be(a.dataset.id)}),e.appendChild(a),cn(a)}Ee("t","data-k",i.sort),ue()}async function wa(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const a=document.querySelector("#t tbody"),s=Pe(Xt()).map(l=>l.company_id),o=[...a.querySelectorAll("tr")].map(l=>l.dataset.id);if(s.length!==o.length||s.some((l,d)=>l!==o[d])){W();return}for(const l of e){const d=i.rows.find(p=>p.company_id===l),u=a.querySelector(`tr[data-id="${CSS.escape(l)}"]`);if(!d||!u){W();return}u.innerHTML=on(d),cn(u)}ue()}let E=null,ut=null,pe=!1,me=!1,L=null,pt=null;function rn(){const e=i.applicationStages;if(E===null)E=new Set(["",...e.filter(t=>t!=="rejected")]);else{for(const t of[...E])t!==""&&!e.includes(t)&&E.delete(t);if(ut)for(const t of e)t!=="rejected"&&!ut.has(t)&&E.add(t)}ut=new Set(e)}function dn(){const e=i.outreachStatuses;if(L===null)L=new Set(["",...e]);else{for(const t of[...L])t!==""&&!e.includes(t)&&L.delete(t);if(pt)for(const t of e)pt.has(t)||L.add(t)}pt=new Set(e)}function ka(){rn(),dn();const e=document.getElementById("jq").value.trim().toLowerCase();return i.jobs.filter(t=>{const n=t.application_status||"";return!(!E.has(n)||pe&&!t.next_up||me&&!(t.followups_due|0)||!L.has(t.outreach_status||"")||e&&!(t.title+" "+t.company+" "+(t.location||"")+" "+(t.description||"")+" "+(t.contacts||"")).toLowerCase().includes(e))})}function Y(e,t,n,a,s){return`<button class="fdrop-item${s?" is-checked":""}" ${e}="${c(t)}" role="menuitemcheckbox" aria-checked="${s}"><span class="fdrop-check" aria-hidden="true"></span>`+(a?`<span class="fdrop-dot ${a}"></span>`:"")+`<span class="fdrop-label">${c(n)}</span><span class="fdrop-item-count" data-count></span></button>`}function un(e,t,n){return`<div class="fdrop-head fdrop-head--toggle"><span>${e}</span><button type="button" class="fdrop-all" data-all="${t}">${n?"none":"all"}</button></div>`}function mt(){rn(),dn();const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=["",...i.applicationStages],n=["",...i.outreachStatuses];e.innerHTML=un("Application stage","stage",t.every(a=>E.has(a)))+Y("data-stage","","not applied","",E.has(""))+i.applicationStages.map(a=>Y("data-stage",a,a,Oe(a),E.has(a))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Outreach queue</div>'+Y("data-toggle","nextup","★ Next up","",pe)+'<div class="fdrop-sep"></div>'+un("Reply status","status",n.every(a=>L.has(a)))+[["","not reached out",""]].concat(i.outreachStatuses.map(a=>[a,a,bn(a)])).map(([a,s,o])=>Y("data-status",a,s,o,L.has(a))).join(""),pn()}function pn(){const e={},t={};let n=0;for(const u of i.jobs){const p=u.application_status||"";e[p]=(e[p]|0)+1;const m=u.outreach_status||"";t[m]=(t[m]|0)+1,u.next_up&&n++}ht("#fdrop-jfilters-menu [data-stage]","data-stage",e),ht("#fdrop-jfilters-menu [data-status]","data-status",t),Ea("nextup",n);const a=["",...i.applicationStages.filter(u=>u!=="rejected")],s=E&&E.size===a.length&&a.every(u=>E.has(u)),o=["",...i.outreachStatuses],l=L&&L.size===o.length&&o.every(u=>L.has(u)),d=(s?0:E?E.size:0)+(pe?1:0)+(l?0:L?L.size:0);mn("fdrop-jfilters-btn",d,d>0)}function ht(e,t,n){document.querySelectorAll(e).forEach(a=>{const s=a.querySelector("[data-count]");if(s){const o=n[a.getAttribute(t)]|0;s.textContent=o||""}})}function Ea(e,t){const n=document.querySelector(`#fdrop-jfilters-menu [data-toggle="${e}"] [data-count]`);n&&(n.textContent=t||"")}function mn(e,t,n){const a=document.getElementById(e);if(!a)return;a.classList.toggle("is-active",n);const s=a.querySelector(".fdrop-count");if(s){const o=n&&t>0;s.textContent=o?t:"",s.style.display=o?"":"none"}}function se(e,t){e.classList.toggle("is-checked",t),e.setAttribute("aria-checked",String(t))}function $a(){const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=e.querySelector('.fdrop-all[data-all="stage"]'),n=e.querySelector('.fdrop-all[data-all="status"]');t&&(t.textContent=["",...i.applicationStages].every(a=>E.has(a))?"none":"all"),n&&(n.textContent=["",...i.outreachStatuses].every(a=>L.has(a))?"none":"all")}function ft(){document.querySelectorAll(".fdrop.is-open").forEach(e=>{e.classList.remove("is-open");const t=e.querySelector(".fdrop-btn");t&&t.setAttribute("aria-expanded","false")})}function hn(e){const t=e.querySelector(".fdrop-btn"),n=e.querySelector(".fdrop-menu");if(!t||!n)return;const a=t.getBoundingClientRect();n.style.left=Math.round(a.left)+"px",n.style.top=Math.round(a.bottom+4)+"px",n.style.minWidth=Math.round(a.width)+"px",n.style.maxHeight=Math.max(160,Math.round(window.innerHeight-a.bottom-12))+"px"}function Ia(e){const t=e.querySelector(".fdrop-btn");e.classList.add("is-open"),t&&t.setAttribute("aria-expanded","true"),hn(e)}function fn(){const e=document.querySelector(".fdrop.is-open");e&&hn(e)}window.addEventListener("scroll",fn,!0),window.addEventListener("resize",fn);const xa=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function Ba(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(xa),a=n?n[0]:"";let s=a?t.replace(a,""):t;return s=s.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:s,email:a}})}const qe=()=>new Date().toISOString().slice(0,10);function gn(e){const t=[["","none"]];for(const n of i.applicationStages)t.push([n,n]);return e&&!i.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function yn(e){const t=[["","none"]];for(const n of i.outreachStatuses)t.push([n,n]);return e&&!i.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const _a=8;function vn(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%_a}function Oe(e){return vn(e,i.applicationStages)}function bn(e){return vn(e,i.outreachStatuses)}function La(e){const t=Ba(e);return t.length?t.map(n=>{const a=c(n.position||n.email);if(!n.email)return a;const s=c(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${c(n.email)}" title="${s}">${a}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function wn(e){const t=e.application_status||"";if(!t)return i.applicationStages.length+1;const n=i.applicationStages.indexOf(t);return n<0?i.applicationStages.length:n}function Ta(e,t,n){if(n==="verdict"){const a={yes:0,maybe:1,no:2,"":3};return(a[e.verdict]??3)-(a[t.verdict]??3)}if(n==="application")return wn(e)-wn(t);if(n==="followups_due")return(t.followups_due|0)-(e.followups_due|0);if(n==="created_at"||n==="last_outreach_at"){const a=e[n]||"",s=t[n]||"";return!a&&!s?0:a?s?String(s).localeCompare(String(a)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function Sa(){const e=document.getElementById("jobs-followup-nav");if(!e)return;const t=i.jobs.reduce((n,a)=>n+(a.followups_due|0),0);if(!t){e.style.display="none",me=!1;return}e.style.display="",e.innerHTML=`<button class="followup-nav-btn${me?" is-active":""}" title="${me?"showing only these — click to show all jobs":"show only jobs owing a follow-up"}"><span class="fn-icon">${et}</span><span class="fn-text"><strong>${t}</strong> follow-up${t>1?"s":""} due</span></button>`,e.querySelector(".followup-nav-btn").onclick=()=>{me=!me,A()}}function A(){const e=document.querySelector("#jt tbody");e.innerHTML="",Sa();const t=ka().sort((s,o)=>i.jsort.dir*Ta(s,o,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",pn();const n=E&&!E.has("rejected")?i.jobs.filter(s=>(s.application_status||"")==="rejected").length:0,a=document.getElementById("jobs-hidden-note");a.style.display=n?"":"none",n&&(a.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{E.add("rejected"),mt(),A()});for(const s of t){const o=s.application_status||"",l=document.createElement("tr");l.dataset.id=s.posting_id;const d=gn(o).map(([m,f])=>`<option value="${c(m)}"${o===m?" selected":""}>${c(f)}</option>`).join(""),u=s.outreach_status||"",p=yn(u).map(([m,f])=>`<option value="${c(m)}"${u===m?" selected":""}>${c(f)}</option>`).join("");l.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${s.next_up?" is-on":""}" title="${s.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 12.5v-9M4.5 7L8 3.5 11.5 7"/></svg></button><div class="jt-namecol"><span class="row-name">${c(s.title||s.company)}</span>${Ca(s.outreach_draft_status)}${s.title?`<div class="small dim">${c(s.company)}</div>`:""}</div></div></td>
      <td data-col="application"><div class="jt-stage"><select class="jt-stage-sel ${Oe(o)}" title="application stage">${d}</select></div></td>
      <td class="small" data-col="outreach"><div class="jt-out"><select class="jt-ostatus ${bn(u)}" title="outreach reply status">${p}</select>${s.followups_due?`<span class="followup-badge" title="${s.followups_due} follow-up${s.followups_due>1?"s":""} due — open to act">${et}${s.followups_due}</span>`:""}</div></td>
      <td class="small" data-col="last_outreach">${s.last_outreach_at?c(s.last_outreach_at):'<span class="dim">—</span>'}</td>
      <td class="small td-contacts" data-col="contacts">${La(s.contacts)}</td>
      <td data-col="link"><a href="${M(s.url)}" target="_blank" rel="noopener" title="open posting" aria-label="open posting">↗</a></td>
    `,l.querySelector(".jt-nextup").onclick=()=>Ln(s,!1),l.querySelector(".jt-stage-sel").onchange=m=>Cn(s,{application_status:m.target.value}),l.querySelector(".jt-ostatus").onchange=m=>Cn(s,{outreach_status:m.target.value}),e.appendChild(l)}Ee("jt","data-jk",i.jsort),ue(),e.querySelectorAll("tr").forEach(s=>{s.addEventListener("click",o=>{o.target.closest("a, button, select")||gt(s.dataset.id)})})}function Ca(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1,contacts:[],outreach:[],contactsLoaded:!1};async function gt(e){let t=i.jobs.find(n=>n.posting_id===e);if(t||(await k(),t=i.jobs.find(n=>n.posting_id===e)),!t){r("posting not found — refresh");return}vt(),$t(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.contacts=[],h.outreach=[],h.contactsLoaded=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),En("pursuit"),oe(),he(),ve(),kn()}async function kn(){const e=h.postingId,t=h.row&&h.row.company_id;if(!(!e||!t)){try{const[n,a]=await Promise.all([fetch(`/api/companies/${t}/contacts`).then(s=>s.ok?s.json():[]),fetch(`/api/postings/${e}/outreach-log`).then(s=>s.ok?s.json():[])]);if(h.postingId!==e)return;h.contacts=Array.isArray(n)?n:[],h.outreach=Array.isArray(a)?a:[]}catch{}h.contactsLoaded=!0,ge()}}let yt=null;function En(e){yt=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function $e(){vt(),$t(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function vt(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function he(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){ge();return}e=await n.json()}catch{ge();return}h.drafts=e.drafts||[],ge();const t=h.drafts[0];t&&t.status==="researching"?ja():vt()}function ja(){h.poll||(h.poll=setInterval(he,4e3))}function G(e,t,{multiline:n=!1}={}){if(!e)return;let a=e.value;e.addEventListener("focus",()=>{a=e.value}),e.addEventListener("keydown",s=>{s.key==="Escape"?(s.preventDefault(),e.value=a,e.blur()):s.key==="Enter"&&(!n||s.metaKey||s.ctrlKey)&&(s.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const s=e.value.trim();if(s===a.trim()){e.value=a;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(s),a=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=a,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),r(`save failed: ${o.message}`)}})}async function $n(e,t,n){const a={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:n},s=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const o=await s.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),A(),we(e.posting_id,{title:o.title,location:o.location})}async function Ma(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();e.url=a.url;const s=document.querySelector("#role-body .role-url-open");s&&s.setAttribute("href",M(e.url)),we(e.posting_id,{url:a.url})}async function Aa(e,t){if(t.disabled)return;const n=t.textContent;t.disabled=!0,t.textContent="re-enriching…";let a;try{a=await fetch(`/api/postings/${e.posting_id}/recapture`,{method:"POST"})}catch(o){t.disabled=!1,t.textContent=n,r(`re-enrich failed: ${o.message}`);return}if(!a.ok){const o=(await a.text().catch(()=>"")).trim();let l=o||"HTTP "+a.status;try{l=JSON.parse(o).error||l}catch{}t.disabled=!1,t.textContent=n,r(`re-enrich failed: ${l}`);return}const s=await a.json();Object.assign(e,{title:s.title,location:s.location,employment_type:s.employment_type,workplace_type:s.workplace_type,department:s.department,comp_range:s.comp_range,description:s.description,posted_at:s.posted_at,url:s.url,questions_status:s.questions_status}),A(),oe(),we(e.posting_id,{title:s.title,location:s.location,url:s.url}),r("re-enriched from the posting link")}function Ha(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>qa(e))}async function Pa(e,t){const n=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();e.company_id=a.company_id,e.company=a.company_name,oe(),k()}let Ie=null;function qa(e){Ie=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const n=document.getElementById("relink-search");n&&(n.value=""),In(""),document.getElementById("relink-scrim").classList.add("open"),n&&n.focus()}function fe(){document.getElementById("relink-scrim").classList.remove("open"),Ie=null}let bt=null;function Oa(e){bt=e;const t=(e.postings||[]).length,n=t?` and its ${t} job ${t===1?"posting":"postings"}`:"",a=document.getElementById("delcompany-summary");a&&(a.innerHTML=`Delete <strong>${c(e.name||"this company")}</strong>${n}?`);const s=document.getElementById("delcompany-confirm");s&&(s.disabled=!1),document.getElementById("delcompany-scrim").classList.add("open")}function Ne(){document.getElementById("delcompany-scrim").classList.remove("open"),bt=null}async function Na(){const e=bt;if(!e)return;const t=document.getElementById("delcompany-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e.company_id}`,{method:"DELETE"})}catch(s){r(`delete failed: ${s.message}`),t&&(t.disabled=!1);return}if(!n.ok){const s=await n.text().catch(()=>"");r(`delete failed: HTTP ${n.status}${s?" — "+s:""}`),t&&(t.disabled=!1);return}const a=e.name||"company";Ne(),i.openId===e.company_id&&_e(),N(),k(),C(),r(`deleted ${a}`)}let wt=null;function Ra(e){wt=e;const t=(e.title||"").trim()||"this posting",n=e.company?` at <strong>${c(e.company)}</strong>`:"",a=document.getElementById("deljob-summary");a&&(a.innerHTML=`Delete <strong>${c(t)}</strong>${n}?`);const s=document.getElementById("deljob-confirm");s&&(s.disabled=!1),document.getElementById("deljob-scrim").classList.add("open")}function Re(){document.getElementById("deljob-scrim").classList.remove("open"),wt=null}async function Da(){const e=wt;if(!e)return;const t=document.getElementById("deljob-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${e.posting_id}`,{method:"DELETE"})}catch(s){r(`delete failed: ${s.message}`),t&&(t.disabled=!1);return}if(!n.ok){const s=await n.text().catch(()=>"");r(`delete failed: HTTP ${n.status}${s?" — "+s:""}`),t&&(t.disabled=!1);return}const a=(e.title||"").trim()||"posting";Re(),$e(),k(),i.openId===e.company_id&&be(e.company_id),r(`deleted ${a}`)}function In(e){const t=document.getElementById("relink-results");if(!t)return;const n=e.trim().toLowerCase();let a=(i.rows||[]).slice();if(n?(a=a.filter(o=>(o.name||"").toLowerCase().includes(n)),a.sort((o,l)=>{const d=(o.name||"").toLowerCase().startsWith(n)?0:1,u=(l.name||"").toLowerCase().startsWith(n)?0:1;return d-u||(o.name||"").localeCompare(l.name||"")})):a.sort((o,l)=>(o.name||"").localeCompare(l.name||"")),a=a.slice(0,60),!a.length){t.innerHTML=`<div class="relink-empty">${(i.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const s=Ie?Ie.company_id:"";t.innerHTML=a.map(o=>{const l=o.company_id===s,d=[o.vertical,o.location].filter(Boolean).map(c).join(" · ");return`<button type="button" class="relink-result${l?" is-current":""}"
      ...
      </button>`}).join("")}async function xn(e){const t=Ie;if(!t){fe();return}if(e===t.company_id){fe();return}try{await Pa(t,e),fe(),r(`moved to ${t.company}`)}catch(n){r(`move failed: ${n.message}`)}}async function Bn(e,t,n){const a={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(a.name).trim())throw new Error("name is required");const s=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const o=await s.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),N(),k()}async function Va(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();i.openId=a.company_id,Le(a),Te(a.company_id),N(),k()}async function Ua(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json();e.notes=a.notes}let _n=null;function oe(){const e=h.row;if(!e)return;const t=document.getElementById("pursuit-body"),a=!!t&&_n===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${c(e.title||"")}">`;const s=e.application_status||"";document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${s?Oe(s)||"pill-stage":"pill-none"}">${c(s||"—")}</span>`+(e.verdict?` <span class="${y(e.verdict)}">${c(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>Kt("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    <section class="pane-section role-head">
      <div id="role-body">${Fa(e)}</div>
    </section>
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row">
          ...
          <select class="input pl-appstatus" title="application stage">
            ${gn(e.application_status||"").map(([p,m])=>`<option value="${c(p)}"${(e.application_status||"")===p?" selected":""}>${c(m)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
          ...
          <select class="input pl-ostatus" title="outreach reply status — separate from the application stage">
            ${yn(e.outreach_status||"").map(([p,m])=>`<option value="${c(p)}"${(e.outreach_status||"")===p?" selected":""}>${c(m)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
  ...
  `,za();const l=document.getElementById("pursuit-company-link");l&&l.addEventListener("click",()=>be(e.company_id)),Ha(e),G(document.getElementById("pursuit-title-input"),p=>$n(e,"title",p)),G(document.getElementById("pursuit-url-input"),p=>Ma(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>Aa(e,d)),G(document.getElementById("pursuit-notes-input"),p=>Ja(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>G(p,m=>$n(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>Ra(e)),ge(),Be(),t&&(t.scrollTop=a),_n=e.posting_id}function Fa(e){return`
    ...
    </div>`}function za(){const e=document.querySelector("#pursuit-body .pl-appstatus");e&&e.addEventListener("change",a=>Sn({application_status:a.target.value}));const t=document.querySelector("#pursuit-body .pl-ostatus");t&&t.addEventListener("change",a=>Sn({outreach_status:a.target.value}));const n=document.querySelector("#pursuit-body .pt-nextup");n&&n.addEventListener("click",()=>Ln(h.row,!0))}async function Ln(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(s){r(`save failed: ${s.message}`);return}if(!n.ok){const s=(await n.text().catch(()=>"")).trim();r(`save failed: ${s||"HTTP "+n.status}`);return}const a=await n.json();e.next_up=a.next_up,A(),we(e.posting_id,{next_up:a.next_up}),t&&oe(),r(e.next_up?"queued next up":"removed from the queue")}async function Tn(e,t){const n={application_status:e.application_status||"",outreach_status:e.outreach_status||"",notes:e.notes||"",...t};let a;try{a=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return r(`save failed: ${o.message}`),null}if(!a.ok){const o=(await a.text().catch(()=>"")).trim();return r(`save failed: ${o||"HTTP "+a.status}`),null}const s=await a.json();return Object.assign(e,{application_status:s.application_status,outreach_count:s.outreach_count,last_outreach_at:s.last_outreach_at,outreach_status:s.outreach_status,contacts:s.contacts,notes:s.notes,next_up:s.next_up}),we(e.posting_id,{application_status:s.application_status,outreach_count:s.outreach_count,last_outreach_at:s.last_outreach_at,next_up:s.next_up}),s}async function Ja(e){const t=h.row,n={application_status:t.application_status||"",outreach_status:t.outreach_status||"",notes:e},a=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const s=await a.json();t.notes=s.notes,A()}async function Sn(e){await Tn(h.row,e)&&(A(),oe(),r("tracking saved"))}async function Cn(e,t){await Tn(e,t)&&(A(),h.postingId===e.posting_id&&(h.row=e,oe()),r("tracking saved"))}function ge(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.drafts,n=t[0]||null,a=t.slice(1),o=n&&(Xa(n.status)||n.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${n?"Draft again":"Draft outreach"}</button><label class="draft-skip-research" title="Skip the web-research stage — write straight from the template; the opener becomes a plain intro."><input type="checkbox" id="draft-skip-research"> skip research</label>`,l=a.length?`
    <details class="draft-history" ${h.openHist?"open":""}>
      <summary>${a.length} earlier draft${a.length>1?"s":""}</summary>
      <div id="draft-history-body">${a.map(d=>Hn(d,!0)).join("")}</div>
    </details>`:"";e.innerHTML=Ka()+`<div class="outreach-drafts-head">Drafts</div><div id="draft-current">${n?Hn(n,!1):""}</div><div class="draft-actions">${o}</div>`+l,Qa(),ss()}function Ga(e,t){const n=h.row||{},a={company:n.company||"",role:n.title||"",contact_name:e&&e.name||"",contact_role:e&&e.role||"",last_sent:t&&t.sent_at||"",last_message:t&&t.body||""};return(i.followupTemplate||"").replace(/\{\{\s*([A-Za-z][A-Za-z0-9_]*)\s*\}\}/g,(s,o)=>o in a?a[o]:s)}function Ka(){const e=h.row,t=e.last_outreach_at?`<div class="outreach-meta"><span>last outreach ${c(e.last_outreach_at)}</span></div>`:"";if(!h.contactsLoaded)return`<div class="contacts-mgr">${t}<div class="loading-row"><span class="spinner"></span><span>loading contacts…</span></div></div>`;const n=h.contacts.map(Wa).join(""),a=h.contacts.length?"":`<div class="cc-empty dim">No contacts yet — add the people you're reaching out to at ${c(e.company)}.</div>`;return`<div class="contacts-mgr">
    ${t}
  ...
  </div>`}function Wa(e){const t=h.outreach.filter(o=>o.contact_id===e.id),n=t[0]||null,a=e.role?`<span class="cc-role">${c(e.role)}</span>`:"",s=e.email?`<a class="cc-mail" href="mailto:${c(e.email)}" title="${c(e.email)}">${c(e.email)}</a>`:"";return`<div class="contact-card" data-cid="${e.id}">
    <div class="cc-head">
    ...
    ${t.length?`<details class="cc-history"><summary>${t.length} send${t.length>1?"s":""}</summary><div class="cc-entries">${t.map(Za).join("")}</div></details>`:""}
      ...
      ${n}
    </div>`}async function ie(e,t,n){let a;try{a=await fetch(t,{method:e,headers:n?{"Content-Type":"application/json"}:{},body:n?JSON.stringify(n):void 0})}catch(s){return r(`save failed: ${s.message}`),null}if(!a.ok){const s=(await a.text().catch(()=>"")).trim();return r(`save failed: ${s||"HTTP "+a.status}`),null}try{return await a.json()}catch{return{}}}async function ye(){await k(),await kn()}function Qa(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.postingId,n=e.querySelector(".cc-addwrap");if(n){const a=n.querySelector(".cc-addform");n.querySelector(".cc-addbtn").addEventListener("click",()=>{a.style.display="",n.querySelector(".cc-addbtn").style.display="none",a.querySelector(".cc-f-name").focus()}),n.querySelector(".cc-f-cancel").addEventListener("click",()=>ge()),n.querySelector(".cc-f-save").addEventListener("click",async()=>{const s={name:a.querySelector(".cc-f-name").value,role:a.querySelector(".cc-f-role").value,email:a.querySelector(".cc-f-email").value};await ie("POST",`/api/companies/${h.row.company_id}/contacts`,s)&&(r("contact added"),ye())})}e.querySelectorAll(".contact-card").forEach(a=>{const s=a.dataset.cid,o=a.querySelector(".cc-editform");a.querySelector(".cc-edit").addEventListener("click",()=>{o.style.display=o.style.display==="none"?"":"none",o.style.display!=="none"&&o.querySelector(".cc-e-name").focus()});const l=a.querySelector(".cc-e-cancel");l&&l.addEventListener("click",()=>{o.style.display="none"});const d=a.querySelector(".cc-e-save");d&&d.addEventListener("click",async()=>{const _={name:o.querySelector(".cc-e-name").value,role:o.querySelector(".cc-e-role").value,email:o.querySelector(".cc-e-email").value};await ie("PUT",`/api/contacts/${s}`,_)&&(r("contact saved"),ye())}),a.querySelector(".cc-arch").addEventListener("click",async()=>{await ie("DELETE",`/api/contacts/${s}`)&&(r("contact removed"),ye())});const u=a.querySelector(".cc-logform"),p=a.querySelector(".cc-log");p&&p.addEventListener("click",()=>{u.style.display=u.style.display==="none"?"":"none",u.style.display!=="none"&&u.querySelector(".cc-l-date").focus()});const m=a.querySelector(".cc-l-cancel");m&&m.addEventListener("click",()=>{u.style.display="none"});const f=a.querySelector(".cc-l-save");f&&f.addEventListener("click",async()=>{const _={contact_id:s,sent_at:u.querySelector(".cc-l-date").value||qe(),body:u.querySelector(".cc-l-body").value};await ie("POST",`/api/postings/${t}/outreach-log`,_)&&(r("outreach logged"),ye())});const b=a.querySelector(".cc-followup");b&&b.addEventListener("click",()=>{const _=h.contacts.find(O=>String(O.id)===String(s)),g=h.outreach.filter(O=>String(O.contact_id)===String(s))[0]||null;_t(Ga(_,g),"follow-up copied — paste into your email")});const T=async(_,g,O)=>{const te=h.outreach.find(Yt=>String(Yt.id)===String(_))||{};await ie("PUT",`/api/outreach-log/${_}`,{sent_at:te.sent_at||"",body:te.body||"",note:te.note||"",followup_due_at:te.followup_due_at||"",done:!!te.followup_done_at,...g})&&(r(O),ye())},S=a.querySelector(".cc-fu-toggle");S&&S.addEventListener("change",()=>T(S.dataset.eid,{done:S.checked},S.checked?"marked followed up":"follow-up reopened"));const U=a.querySelector(".cc-fu-stop");U&&U.addEventListener("click",()=>T(U.dataset.eid,{followup_due_at:"",done:!1},"follow-up stopped"));const H=a.querySelector(".cc-fu-resume");H&&H.addEventListener("click",()=>T(H.dataset.eid,{followup_due_at:qe(),done:!1},"follow-up resumed"));const D=a.querySelector(".cc-fu-dismiss");D&&D.addEventListener("click",()=>T(D.dataset.eid,{followup_due_at:"",done:!0},"escalation dismissed")),a.querySelectorAll(".cc-e-del").forEach(_=>_.addEventListener("click",async()=>{const g=_.closest(".cc-entry").dataset.eid;await ie("DELETE",`/api/outreach-log/${g}`)&&(r("send deleted"),ye())}))})}function Xa(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const jn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',Mn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',An=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${jn}</button>`,kt=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],es='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function ts(e){let t=kt.findIndex(a=>a.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${kt.map((a,s)=>{const o=s<t?"is-done":s===t?"is-active":"is-pending",l=s<t?es:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${l}</span><span class="dp-name">${a.label}</span></div>`}).join("")}</div>
    ...
    <button class="btn btn-primary draft-gmail-btn" title="send this email from your Gmail and log it">${Mn}Send via Gmail</button>
  </div>`:'<div class="draft-note dim">Add a contact with an email to send via Gmail.</div>'}function Hn(e,t){const n=(p,m,f="")=>`
    ...
    </div>`;if(e.status==="failed"){const p=as(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
    ...
    <div class="draft-head">${o}${a?An:""}</div>
    ${d}
    ${u?`<textarea class="draft-textarea" id="draft-edit-${e.id}" spellcheck="false">${c(a)}</textarea>
    ${Pn(e.lint)}
    <div class="draft-actions">
      <button class="btn btn-primary draft-sent-btn" title="mark this email sent — bumps the outreach count">${Mn}Mark sent</button>
    ...
    ${xe(e)}
      ...
      <div class="trace-body">
        ...
        ${M(s.source_url)!=="#"?`<div class="tr-line"><a href="${M(s.source_url)}" target="_blank" rel="noopener">source</a></div>`:""}
        ${a.reasoning?`<div class="tr-line"><span class="tr-key">reasoning:</span> ${c(a.reasoning)}</div>`:""}
      </div></details>`}return t}function Pn(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${c(n.message||"")}"><code>${c(n.code||"")}</code>${c(n.message||"")}</span>`).join("")+"</div>":""}function as(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${c(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${c(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function ss(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#draft-start-btn");t&&t.addEventListener("click",()=>De(!1,os())),e.querySelectorAll(".draft-retry-btn").forEach(a=>a.addEventListener("click",()=>De())),e.querySelectorAll(".draft-regen-btn").forEach(a=>a.addEventListener("click",()=>De(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(a=>{const s=a.dataset.did,o=a.querySelector(".draft-textarea");o&&G(o,p=>cs(s,p),{multiline:!0});const l=a.querySelector(".draft-sent-btn");l&&l.addEventListener("click",()=>rs(s));const d=a.querySelector(".draft-gmail-btn");d&&d.addEventListener("click",()=>{const p=a.querySelector(".draft-gmail-to");ls(s,p?p.value:"",d)});const u=a.querySelector(".draft-copy-btn");u&&u.addEventListener("click",()=>{const p=a.querySelector(".draft-textarea"),m=a.querySelector(".draft-sentbody"),f=p?p.value:m?m.textContent:"";_t(f,"email copied")})});const n=e.querySelector("details.draft-history");n&&n.addEventListener("toggle",()=>{h.openHist=n.open})}function os(){const e=document.getElementById("draft-skip-research");return!!(e&&e.checked)}async function De(e=!1,t=!1){const n=document.getElementById("outreach-section"),a=n&&(n.querySelector("#draft-start-btn")||n.querySelector(".draft-retry-btn")||n.querySelector(".draft-regen-btn"));a&&(a.disabled=!0);let s;try{const l=new URLSearchParams;e&&l.set("regenerate","1"),t&&l.set("research","0");const d=l.toString()?`?${l.toString()}`:"";s=await fetch(`/api/postings/${h.postingId}/outreach${d}`,{method:"POST"})}catch(l){r(`draft failed: ${l.message}`),a&&(a.disabled=!1);return}if(s.status===202){let l={};try{l=await s.json()}catch{}Array.isArray(l.degraded)&&l.degraded.length&&r(`drafting without ${l.degraded.join(", ")} — quality degrades, integrity unaffected`),await he(),k();return}if(s.status===409){await he(),r("a draft is already active");return}if(s.status===412){let l={};try{l=await s.json()}catch{}is(l.need,l.error),a&&(a.disabled=!1);return}if(s.status===503){const l=document.getElementById("outreach-section");if(l){const d=document.createElement("div");d.className="draft-note",d.textContent="Outreach engine not running in this build.",l.appendChild(d)}a&&(a.disabled=!1);return}const o=(await s.text().catch(()=>"")).trim();r(`draft failed: ${o||"HTTP "+s.status}`),a&&(a.disabled=!1)}function is(e,t){const n=document.getElementById("outreach-section");if(!n)return;const a=n.querySelector(".draft-actions"),s=e==="template",o=s?"Write email template":"View brain knowledge",l=document.createElement("div");l.className="blocks-gate",l.innerHTML=`
    ...
    </div>`,a?a.replaceWith(l):n.appendChild(l);const d=l.querySelector("#gate-fix-btn");d&&d.addEventListener("click",()=>s?K("outreach-template"):ta());const u=l.querySelector("#gate-retry-btn");u&&u.addEventListener("click",De)}async function cs(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json(),s=h.drafts.findIndex(d=>String(d.id)===String(e));s>=0&&(h.drafts[s]=a);const o=document.getElementById(`draft-edit-${e}`),l=o&&o.closest(".draft-card");if(l){const d=l.querySelector(".lint-chips"),u=Pn(a.lint);d?d.outerHTML=u||"":u&&o.insertAdjacentHTML("afterend",u)}}async function ls(e,t,n){n&&(n.disabled=!0,n.dataset.t=n.textContent||"",n.textContent="Sending…");const a=()=>{n&&(n.disabled=!1,n.textContent=n.dataset.t||"Send via Gmail")};let s;try{s=await fetch(`/api/outreach/drafts/${e}/send-gmail`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:t||""})})}catch(l){r(`send failed: ${l.message}`),a();return}if(!s.ok){const l=(await s.text().catch(()=>"")).trim();r(`send failed: ${l||"HTTP "+s.status}`),a();return}let o={};try{o=await s.json()}catch{}r(o.to?`sent via Gmail to ${o.to}`:"sent via Gmail"),await he(),await k()}async function rs(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(a){r(`failed: ${a.message}`);return}if(!t.ok){const a=(await t.text().catch(()=>"")).trim();r(`failed: ${a||"HTTP "+t.status}`);return}r("marked sent"),await he(),await k();const n=i.jobs.find(a=>a.posting_id===h.postingId);n&&we(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function ve(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){Be();return}e=await t.json()}catch{Be();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",Be(),h.answers.some(t=>t.status==="generating")?ds():$t()}function ds(){h.answersPoll||(h.answersPoll=setInterval(ve,4e3))}function $t(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function Be(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,a=t.some(p=>p.status==="generating"),s=t.length?`<div class="answers-list">${t.map(ms).join("")}</div>`:"",o=!!h.detecting,l=a||o?" disabled":"",d=p=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":p}</button>`;let u;n==="ok"&&t.length?u=(t.some(m=>!qn(m)&&m.status!=="generating")?`<button class="btn" id="answers-start-btn"${l}>${a?"Drafting…":"Draft all blank"}</button>`:"")+d("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${l}>${a?"Drafting…":"Draft answers"}</button>`+d("Re-detect questions"):u=d("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${c(us(n,t.length))}</div>`+s+`<div class="answers-actions">${u}</div>`,fs()}function us(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function qn(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function ps(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function ms(e){const t=qn(e),n=e.edited&&e.edited.trim(),a=e.status==="generating",s=t.length,o=e.max_length&&s>e.max_length,l=e.max_length?`<span class="answer-count${o?" over":""}">${s} / ${e.max_length}</span>`:`<span class="answer-count">${s} chars</span>`,d=!!t,u=d?"Regenerate":"Generate",p=d?"re-draft this answer (discards the current text)":"draft an answer to just this question";return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    <div class="answer-foot">
      ...
      ${a||!d?"":`<button class="answer-copy-btn dh-copy" title="copy this answer to the clipboard" aria-label="copy answer">${jn}</button>`}
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${c(hs(e.fail_reason))}</div>`:""}
  </div>`}function hs(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function fs(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",On);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",ys),e.querySelectorAll(".answer-card[data-aid]").forEach(a=>{const s=a.dataset.aid,o=a.querySelector(".answer-textarea");o&&(G(o,p=>vs(s,p),{multiline:!0}),o.addEventListener("input",()=>gs(a,o)));const l=a.querySelector(".answer-regen-btn");l&&l.addEventListener("click",()=>bs(s));const d=a.querySelector(".answer-copy-btn");d&&d.addEventListener("click",()=>{o&&_t(o.value,"answer copied")});const u=a.querySelector(".answer-remove-btn");u&&u.addEventListener("click",()=>ws(s))})}function gs(e,t){const n=e.querySelector(".answer-count");if(!n)return;const a=t.value.length,s=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=s?`${a} / ${s}`:`${a} chars`,n.classList.toggle("over",!!s&&a>s)}async function On(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(s){r(`draft failed: ${s.message}`),t&&(t.disabled=!1);return}if(n.status===202){await ve();return}if(n.status===412){let s={};try{s=await n.json()}catch{}Nn(s.error),t&&(t.disabled=!1);return}if(n.status===503){Rn("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const a=(await n.text().catch(()=>"")).trim();r(`draft failed: ${a||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function ys(){h.detecting=!0,Be();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();r(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){r(`detect failed: ${e.message}`)}h.detecting=!1,await ve()}async function vs(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const a=await n.json(),s=h.answers.findIndex(o=>String(o.id)===String(e));s>=0&&(h.answers[s]=a)}async function bs(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){r(`regenerate failed: ${n.message}`);return}if(t.status===503){Rn("Answer generation isn't running in this build.");return}if(t.status===412){let n={};try{n=await t.json()}catch{}Nn(n.error);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();r(`regenerate failed: ${n||"HTTP "+t.status}`);return}await ve()}async function ws(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"DELETE"})}catch(n){r(`remove failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();r(`remove failed: ${n||"HTTP "+t.status}`);return}await ve()}function Nn(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),a=document.createElement("div");a.className="blocks-gate",a.innerHTML=`
    ...
    </div>`,n?n.replaceWith(a):t.appendChild(a);const s=a.querySelector("#answers-fix-btn");s&&s.addEventListener("click",ta);const o=a.querySelector("#answers-retry-btn");o&&o.addEventListener("click",On)}function Rn(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function be(e){var d,u;const t=document.getElementById("pane"),n=document.getElementById("scrim"),a=i.openId===e&&t.classList.contains("open"),s=a?((d=document.getElementById("pane-body"))==null?void 0:d.scrollTop)??0:0,o=a?(u=document.getElementById("trace-body"))==null?void 0:u.innerHTML:null;i.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),En("company"),a||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let l;try{const p=await fetch(`/api/companies/${e}`);if(!p.ok)throw new Error(`HTTP ${p.status}`);l=await p.json()}catch(p){a||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${c(p.message)}</div>`);return}if(i.openId===e){if(Le(l),a){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const p=document.getElementById("pane-body");p&&(p.scrollTop=s)}Te(e)}}function _e(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function Le(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${c(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
    <span class="${y(e.has_verdict?e.verdict:"")}">${c(e.has_verdict?e.verdict:"unscored")}</span>
  `;const t=document.getElementById("pane-chat");t&&(t.style.display=i.meta&&i.meta.chat?"":"none",t.onclick=()=>Kt("company",e.company_id,e.name));const n=e.model==="manual",a=e.has_verdict?`
    <dl class="kv">
  ...
  `;const b=document.getElementById("posting-add-btn");b&&b.addEventListener("click",()=>Is(e)),xt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(g=>{g.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(O=>O.classList.remove("is-on")),g.classList.add("is-on")})});const T=document.getElementById("ve-save-btn");T&&T.addEventListener("click",()=>$s(e)),G(document.getElementById("pane-title-input"),g=>Bn(e,"name",g)),document.querySelectorAll("#facts-body [data-k]").forEach(g=>G(g,O=>Bn(e,g.dataset.k,O))),G(document.getElementById("pane-domain-input"),g=>Va(e,g)),G(document.getElementById("pane-notes-input"),g=>Ua(e,g),{multiline:!0});const S=document.getElementById("flag-toggle-btn");S&&S.addEventListener("click",()=>Dn(e.company_id));const U=document.getElementById("review-stamp-btn");U&&U.addEventListener("click",()=>Es(e.company_id));const H=document.getElementById("rescore-btn");H&&H.addEventListener("click",()=>St("verdict",{company_ids:[e.company_id]}));const D=document.getElementById("reenrich-btn");D&&D.addEventListener("click",()=>St("enrich",{company_ids:[e.company_id]}));const _=document.getElementById("company-delete-btn");_&&_.addEventListener("click",()=>Oa(e))}function ks(e){return`
    <div class="ie-grid">
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function we(e,t){const n=i.openDetail;if(!n||!i.openId)return;const a=(n.postings||[]).find(o=>String(o.id)===String(e));if(!a)return;Object.assign(a,t);const s=document.getElementById("postings-list");s&&(s.innerHTML=It(n),xt())}function xt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||gt(e.dataset.pid)})})}async function Is(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),a=document.getElementById("posting-add-btn"),s=t.value.trim();if(!s){r("Enter a URL first."),t.focus();return}a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:s,title:n.value.trim()})})}catch(u){r(`add failed: ${u.message}`),a.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");r(`add failed: HTTP ${o.status}${u?" — "+u:""}`),a.disabled=!1;return}const l=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==l.id),e.postings.unshift(l);const d=document.getElementById("postings-list");d&&(d.innerHTML=It(e),xt()),t.value="",n.value="",a.disabled=!1,k(),r("link added")}async function Te(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(a){Ve(`<div class="muted">Failed to load trail: ${c(a.message)}</div>`);return}if(!t.ok){Ve(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Ve('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Ve(n.map(xs).join(""))}function xs(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(c);return e.run_id&&t.push("run "+c(e.run_id.slice(0,8))),`
    ...
    </div>`}function Ve(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let Vn;function r(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(Vn),Vn=setTimeout(()=>t.classList.remove("show"),2200)}let Bt;const Bs=6e3;function Ue(){clearTimeout(Bt),Bt=void 0}function Un(){Ue(),document.getElementById("drawer").classList.remove("open")}function Fn(){Ue(),Bt=setTimeout(Un,Bs)}async function _t(e,t="copied"){if(!e){r("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}r(t)}catch(n){r(`copy failed: ${n.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function Lt(){try{const a=await fetch("/api/meta");if(!a.ok)return;i.meta=await a.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=i.meta.chat?"":"none")}async function Tt(){let e;try{const a=await fetch("/api/runs");if(!a.ok)return;e=await a.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Se=null;async function St(e,t){if(i.meta&&i.meta.control===!1){r("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(s){r(`run failed: ${s.message}`);return}if(n.status===409){r("a job is already running");return}if(n.status===412){const s=await n.text();r(s.trim());return}if(!n.ok){r(`run failed: HTTP ${n.status}`);return}const{job_id:a}=await n.json();Zn(e,a,t)}async function _s(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(s){r(`upload failed: ${s.message}`);return}if(n.status===409){r("a job is already running");return}if(!n.ok){r(`upload failed: HTTP ${n.status}`);return}const{job_id:a}=await n.json();Zn("ingest",a)}const Ls=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let Fe=[],Z=new Set,Q="company";function zn(e){Q=e,document.querySelectorAll("#add-kind .v-chip").forEach(a=>a.classList.toggle("is-on",a.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",Jn()}function Ct(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function Jn(){const e=document.getElementById("add-note");Ct()?e.innerHTML=Q==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and description — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=Q==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function Ts(){Ls.forEach(a=>{document.getElementById(a).value=""}),document.getElementById("add-vertical-filter").value="",Z=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),zn(i.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(a=>`<option value="${c(a.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const a=await(await fetch("/api/facets")).json();(a.funding_stages||[]).forEach(s=>{const o=document.createElement("option");o.value=s,o.textContent=s,n.appendChild(o)}),Fe=a.verticals||[]}catch{Fe=[]}Gn()}function ze(){document.getElementById("add-scrim").classList.remove("open")}function Gn(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=Fe.filter(a=>!t||a.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(a=>`<button type="button" class="vchip${Z.has(a)?" sel":""}" data-v="${c(a)}">${c(a)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(a=>a.addEventListener("click",()=>{const s=a.dataset.v;Z.has(s)?Z.delete(s):Z.add(s),a.classList.toggle("sel"),Kn()}))):e.innerHTML=`<div class="none">${Fe.length?"no match":"no verticals in the set yet"}</div>`,Kn()}function Kn(){const e=Z.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Wn(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Yn(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){r(Q==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),a=n.textContent;n.disabled=!0,Ct()&&(n.textContent="reading page…");const s=()=>{n.disabled=!1,n.textContent=a},o=f=>document.getElementById(f).value.trim(),l=Ct();let d,u;l?(d="/api/capture",u={url:Wn(t),kind:Q==="company"?"company_page":"job_posting",fields:Q==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...Z].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):Q==="company"?(d="/api/companies",u={website:t,name:o("add-name"),vertical:[...Z].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(d="/api/postings",u={url:Wn(t),title:o("add-title"),company:o("add-job-company")});let p;try{p=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(f){r(`add failed: ${f.message}`),s();return}if(!p.ok){let f=`HTTP ${p.status}`;try{const b=await p.text();try{f=JSON.parse(b).error||f}catch{f=b.trim()||f}}catch{}if(s(),p.status===409){r(f||"That company is already in the list."),e.focus(),e.select();return}r(`add failed: ${f}`);return}const m=await p.json();if(s(),l&&!m.company_id){r(m.note||"couldn't classify that page");return}if(ze(),N(),C(),k(),Q==="job"){const f=m.posting&&m.posting.title||"job link";r(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),q("jobs")}else l?(r(m.note||(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`)),be(m.company_id)):r("company added")}function Zn(e,t,n){Se=t,Ue();const a=document.getElementById("drawer"),s=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",s.innerHTML="";const o=document.getElementById("drawer-summary");o.hidden=!0,o.innerHTML="",a.classList.add("open"),Tt();const l={yes:0,maybe:0,no:0},d=/^(.+?)\s*→\s*(yes|maybe|no)\s*—\s*([\s\S]*)$/i,u=new EventSource(`/api/jobs/${t}/stream`),p=(m,f)=>{const b=document.createElement("div");let T;if(!f&&(T=m.match(d))){const S=T[2].toLowerCase();l[S]++,b.className="ln ln-verdict";const U=document.createElement("span");U.className="pill pill-"+S,U.textContent=S;const H=document.createElement("span");H.className="lv-text";const D=document.createElement("span");D.className="lv-name",D.textContent=T[1].trim(),H.appendChild(D);const _=(T[3]||"").trim();if(_){const g=document.createElement("span");g.className="lv-reason",g.textContent=_,H.append(" ",g)}b.append(U,H)}else if(!f&&/^(scoring|enriching|ingesting)\b/i.test(m))b.className="ln ln-head",b.textContent=m;else if(!f&&/^·\s/.test(m))b.className="ln ln-pick",b.textContent=m;else{const S=!f&&/^\s*warn:/i.test(m);b.className="ln"+(f?" ln-err":S?" ln-warn":""),b.textContent=S?m.replace(/^\s*warn:\s*/i,"⚠ "):m}s.appendChild(b),s.scrollTop=s.scrollHeight};u.addEventListener("line",m=>p(m.data,/error|failed/i.test(m.data))),u.addEventListener("end",m=>{if(u.close(),Se=null,p(`— ${m.data} —`,m.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",l.yes+l.maybe+l.no>0){for(const b of["yes","maybe","no"]){if(!l[b])continue;const T=document.createElement("span");T.className="pill pill-"+b,T.textContent=`${l[b]} ${b}`,o.appendChild(T)}o.hidden=!1}Fn(),r(`${e} ${m.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?wa(n.company_ids):N(),C(),Tt(),k(),i.openId&&be(i.openId)}),u.onerror=()=>{u.close()}}async function Ss(){if(Se)try{await fetch(`/api/jobs/${Se}/cancel`,{method:"POST"})}catch{}}let R=null;const Cs={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function jt(e){return e==="application-stages"||e==="outreach-statuses"}function Je(e){if(e==="outreach-template")return"email body";if(e==="outreach-subject")return"email subject";if(e==="outreach-signature")return"email signature";if(e==="followup-template")return"follow-up template";if(e==="playbook")return"playbook";if(e==="application-stages")return"application stages";if(e==="outreach-statuses")return"outreach statuses";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(Cs[t]||t)+" prompt"}return e+".md"}async function K(e){R=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Je(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=!!e&&e.startsWith("outreach-prompts/"),a=n?e.slice(17):"",s=n&&a!=="fill";document.getElementById("editor-toggle-row").style.display=s?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",s&&(document.getElementById("editor-toggle-label").textContent="Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const d=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${d||"HTTP "+o.status}`;return}const l=await o.json();jt(e)?(document.getElementById("editor-title").textContent="edit "+Je(e)+" — one per line",document.getElementById("editor-text").value=(l.statuses||[]).join(`
`)):document.getElementById("editor-text").value=l.content||"",s&&(document.getElementById("editor-enabled").checked=l.enabled!==!1),l.taste_version&&(document.getElementById("editor-ver").textContent="version "+l.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Ge(){document.getElementById("editor-scrim").classList.remove("open"),R=null}let B=null,Mt=[],At=[];const js=["location.allowed","verticals.excluded","verticals.allowed"],Ht={"verticals.excluded":"pf-vertical-tags","verticals.allowed":"pf-vertical-tags"};function X(e){const[t,n]=e.split(".");return B[t]&&B[t][n]||[]}function Ke(e,t){const[n,a]=e.split(".");(B[n]=B[n]||{})[a]=t}function We(e,t){const n=t.toLowerCase();return e.some(a=>String(a).toLowerCase()===n)}function Pt(){js.forEach(e=>{const t=document.querySelector(`.pf-chips[data-field="${e}"]`);if(!t)return;const n=Ht[e]?` list="${Ht[e]}"`:"",a=Ht[e]?"type to search…":"type &amp; Enter…";t.innerHTML=X(e).map((s,o)=>`<span class="pf-chip">${c(s)}<button class="pf-chip-x" data-field="${e}" data-i="${o}" title="remove" aria-label="remove ${c(s)}">×</button></span>`).join("")+`<input class="pf-chip-input" data-field="${e}"${n} type="text" placeholder="${a}" spellcheck="false" autocomplete="off" />`})}function Ms(e,t){var a;const n=(t||"").trim();if(n){const s=X(e);We(s,n)||Ke(e,[...s,n])}Pt(),(a=document.querySelector(`.pf-chip-input[data-field="${e}"]`))==null||a.focus()}function Qn(e,t){const n=X(e).slice();n.splice(t,1),Ke(e,n),Pt()}function Xn(){const e=document.getElementById("pf-stages");if(!e)return;const t=X("funding_stage.allowed");e.innerHTML=At.map(n=>{const a=We(t,n.value),s=n.count?` <span class="pf-stage-n">${n.count}</span>`:"";return`<button class="pf-stage${a?" is-on":""}" data-stage="${c(n.value)}">${c(n.value)}${s}</button>`}).join("")}function As(e){const t=X("funding_stage.allowed");Ke("funding_stage.allowed",We(t,e)?t.filter(n=>String(n).toLowerCase()!==e.toLowerCase()):[...t,e]),Xn()}function Hs(){const e=document.getElementById("pf-vertical-tags");e&&(e.innerHTML=Mt.map(t=>`<option value="${c(t.value)}" label="${t.count}"></option>`).join(""))}function Ps(){return{location:{allowed:[],remote_ok:!0},headcount:{min:0,max:0},verticals:{allowed:[],excluded:[]},funding_stage:{allowed:[]}}}async function ea(e=!1){document.getElementById("prefilter-scrim").classList.add("open");try{const[t,n]=await Promise.all([(await fetch("/api/taste-filter"+(e?"?default=1":""))).json(),Mt.length||At.length?Promise.resolve(null):fetch("/api/filter-options").then(a=>a.json()).catch(()=>null)]);n&&(Mt=n.verticals||[],At=n.stages||[]),B=Object.assign(Ps(),t.rules||{}),B.location=Object.assign({allowed:[],remote_ok:!0},B.location),B.headcount=Object.assign({min:0,max:0},B.headcount),B.verticals=Object.assign({allowed:[],excluded:[]},B.verticals),B.funding_stage=Object.assign({allowed:[]},B.funding_stage),e||(document.getElementById("pf-enabled").checked=t.enabled!==!1),document.getElementById("pf-remote-ok").checked=!!B.location.remote_ok,document.getElementById("pf-hc-min").value=String(B.headcount.min||0),document.getElementById("pf-hc-max").value=String(B.headcount.max||0),Hs(),Pt(),Xn()}catch(t){r(`failed to load pre-filter: ${t.message}`)}}function Ye(){document.getElementById("prefilter-scrim").classList.remove("open"),B=null}async function qs(){if(!B)return;B.location.remote_ok=document.getElementById("pf-remote-ok").checked,B.headcount.min=Math.max(0,parseInt(document.getElementById("pf-hc-min").value,10)||0),B.headcount.max=Math.max(0,parseInt(document.getElementById("pf-hc-max").value,10)||0),document.querySelectorAll(".pf-chip-input").forEach(n=>{const a=n.value.trim();a&&!We(X(n.dataset.field),a)&&Ke(n.dataset.field,[...X(n.dataset.field),a])});const e=document.getElementById("pf-enabled").checked;let t;try{t=await fetch("/api/taste-filter",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({rules:B,enabled:e})})}catch(n){r(`save failed: ${n.message}`);return}if(!t.ok){r(`save failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`);return}r("pre-filter saved"),Ye(),C()}const Os=[{key:"experience",hard:!0},{key:"voice",hard:!1},{key:"logistics",hard:!1}];async function ta(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{Ns(await(await fetch("/api/outreach/sources")).json())}catch(e){r(`failed to load sources: ${e.message}`)}}function qt(){document.getElementById("sources-scrim").classList.remove("open")}function Ns(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(s=>({key:s.Key||s.key,hard:s.Hard??s.hard})):Os,a={};(e&&e.sources||[]).forEach(s=>{(a[s.need]=a[s.need]||[]).push(s)}),t.innerHTML=n.map(s=>{const o=a[s.key]||[],l=o.length?o.map(d=>`<li><span class="src-title">${c(d.title||d.page_id)}</span></li>`).join(""):`<li class="dim small">${s.hard?"none yet — add an experience page to your brain":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${c(s.key)}${s.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${l}</ul></div>`}).join("")}async function Rs(){if(!R)return;const e=document.getElementById("editor-text").value;let t;jt(R)?t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)}:(t={content:e},R.startsWith("outreach-prompts/")&&R!=="outreach-prompts/fill"&&(t.enabled=document.getElementById("editor-enabled").checked));let n;try{n=await fetch(`/api/${R}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){r(`save failed: ${o.message}`);return}if(!n.ok){r(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const a=await n.json();a.taste_version&&(document.getElementById("editor-ver").textContent="version "+a.taste_version);const s=jt(R);R==="followup-template"&&(i.followupTemplate=e),r(`${Je(R)} saved`),Ge(),s&&P(),C()}async function Ds(){if(!R||!R.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${R}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){r(`reset failed: ${n.message}`);return}if(!e.ok){r(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",r(`${Je(R)} reset to default`)}function na(e,t,n){e.k!==t?(e.k=t,e.dir=1):e.dir===1?e.dir=-1:Object.assign(e,n)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{na(i.sort,e.dataset.k,v),W()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{na(i.jsort,e.dataset.jk,$),A()}}),document.getElementById("tab-companies").onclick=()=>q("companies"),document.getElementById("tab-jobs").onclick=()=>q("jobs"),document.getElementById("q").oninput=W,document.getElementById("fdrop-cfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.dataset.toggle==="flagged")re=!re,se(t,re);else if(t.dataset.toggle==="enriched")de=!de,se(t,de);else if(t.hasAttribute("data-v")){const n=t.getAttribute("data-v");J.has(n)?J.delete(n):J.add(n),se(t,J.has(n))}else return;W()}}),document.getElementById("fdrop-columns-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item[data-col]");if(!t)return;const n=dt(),a=t.getAttribute("data-col");n.hidden.has(a)?n.hidden.delete(a):n.hidden.add(a),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),se(t,!n.hidden.has(a)),ue(),sn()}),document.getElementById("jq").oninput=A;for(const e of["fdrop-cfilters","fdrop-columns","fdrop-jfilters"]){const t=document.getElementById(e);t.querySelector(".fdrop-btn").addEventListener("click",a=>{a.stopPropagation();const s=t.classList.contains("is-open");ft(),s||Ia(t)}),t.querySelector(".fdrop-menu").addEventListener("click",a=>a.stopPropagation())}document.addEventListener("click",ft),document.getElementById("fdrop-jfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-all");if(t){if(t.getAttribute("data-all")==="stage"){const a=["",...i.applicationStages];E=a.every(s=>E.has(s))?new Set:new Set(a)}else{const a=["",...i.outreachStatuses];L=L&&a.every(s=>L.has(s))?new Set:new Set(a)}mt(),A();return}const n=e.target.closest(".fdrop-item");if(n){if(n.hasAttribute("data-stage")){const a=n.getAttribute("data-stage");E.has(a)?E.delete(a):E.add(a),se(n,E.has(a))}else if(n.dataset.toggle==="nextup")pe=!pe,se(n,pe);else if(n.hasAttribute("data-status")){const a=n.getAttribute("data-status");L.has(a)?L.delete(a):L.add(a),se(n,L.has(a))}else return;$a(),A()}}),fa(),an(),ue(),document.getElementById("pane-close").onclick=_e,document.getElementById("scrim").onclick=_e,document.getElementById("pursuit-close").onclick=$e,document.getElementById("pursuit-scrim").onclick=$e,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.querySelector(".fdrop.is-open")){ft();return}if(document.getElementById("chat-pane").classList.contains("open")){Wt();return}if(no()){zt();return}if(document.getElementById("profile-scrim").classList.contains("open")){Ft();return}if(document.getElementById("add-scrim").classList.contains("open")){ze();return}if(document.getElementById("run-scrim").classList.contains("open")){Ze();return}if(document.getElementById("help-scrim").classList.contains("open")){Qe();return}if(document.getElementById("relink-scrim").classList.contains("open")){fe();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){Ne();return}if(document.getElementById("deljob-scrim").classList.contains("open")){Re();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(yt==="pursuit"&&n){$e();return}if(yt==="company"&&t){_e();return}if(t){_e();return}$e();return}if(document.getElementById("key-scrim").classList.contains("open")){Ut();return}if(document.getElementById("sources-scrim").classList.contains("open")){qt();return}if(document.getElementById("prefilter-scrim").classList.contains("open")){Ye();return}if(document.getElementById("editor-scrim").classList.contains("open")){Ge();return}if(document.getElementById("gmail-config-scrim").classList.contains("open")){Me();return}});let Ot=null;const Vs={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function aa(e){if(i.meta&&i.meta.control===!1){r("control surface disabled");return}Ot=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Vs[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},a=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&a>0?(document.getElementById("run-warn-text").textContent=`${a} ${a===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${a===1?"it":"them"}. Run Enrich first to include ${a===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Ze(){document.getElementById("run-scrim").classList.remove("open"),Ot=null}document.getElementById("btn-enrich").onclick=()=>aa("enrich"),document.getElementById("btn-verdict").onclick=()=>aa("verdict"),document.getElementById("run-cancel").onclick=Ze,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Ze()},document.getElementById("run-go").onclick=()=>{const e=Ot,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(Ze(),!e)return;const a={};t&&(a.only_blanks=!0),n>0&&(a.workers=n),St(e,a)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=Ts;const Us={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function sa(e){const t=Us[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const a=document.createElement("p");a.className="help-intro",a.textContent=t.intro,n.appendChild(a)}t.items.forEach(a=>{const s=document.createElement("div");s.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=a.name;const l=document.createElement("div");l.className="help-item-desc",l.textContent=a.desc;const d=document.createElement("a");d.className="help-link",d.textContent="Learn more →",d.onclick=()=>{Qe(),la(),ao(a.sec)},s.appendChild(o),s.appendChild(l),s.appendChild(d),n.appendChild(s)}),document.getElementById("help-scrim").classList.add("open")}function Qe(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>sa("add"),document.getElementById("help-run").onclick=()=>sa("run"),document.getElementById("help-close").onclick=Qe,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Qe()},document.getElementById("add-cancel").onclick=ze,document.getElementById("add-save").onclick=Yn,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&ze()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>zn(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Jn),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Yn()))}),document.getElementById("add-vertical-filter").addEventListener("input",Gn),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&_s(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=Ss,document.getElementById("drawer-close").onclick=Un,(()=>{const e=document.getElementById("drawer");e.addEventListener("mouseenter",Ue),e.addEventListener("mouseleave",()=>{!Se&&e.classList.contains("open")&&Fn()})})(),document.getElementById("editor-cancel").onclick=Ge,document.getElementById("editor-save").onclick=Rs,document.getElementById("editor-reset").onclick=Ds,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Ge()},document.getElementById("sources-close").onclick=qt,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&qt()},document.getElementById("pf-cancel").onclick=Ye,document.getElementById("pf-save").onclick=qs,document.getElementById("pf-reset").onclick=()=>ea(!0),document.getElementById("prefilter-scrim").addEventListener("click",e=>{var s;if(e.target.id==="prefilter-scrim"){Ye();return}const t=e.target.closest(".pf-stage");if(t){As(t.dataset.stage);return}const n=e.target.closest(".pf-chip-x");if(n){Qn(n.dataset.field,parseInt(n.dataset.i,10));return}const a=e.target.closest(".pf-chips");a&&e.target===a&&((s=a.querySelector(".pf-chip-input"))==null||s.focus())}),document.getElementById("prefilter-scrim").addEventListener("keydown",e=>{const t=e.target.closest(".pf-chip-input");if(t){if(e.key==="Enter"||e.key===",")e.preventDefault(),Ms(t.dataset.field,t.value);else if(e.key==="Backspace"&&!t.value){const n=X(t.dataset.field);n.length&&Qn(t.dataset.field,n.length-1)}}}),document.getElementById("key-cancel").onclick=Ut,document.getElementById("key-save").onclick=ca,document.getElementById("key-remove").onclick=Xs,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&Ut()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),ca())}),document.getElementById("delcompany-cancel").onclick=Ne,document.getElementById("delcompany-confirm").onclick=Na,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&Ne()},document.getElementById("deljob-cancel").onclick=Re,document.getElementById("deljob-confirm").onclick=Da,document.getElementById("deljob-scrim").onclick=e=>{e.target.id==="deljob-scrim"&&Re()},document.getElementById("relink-cancel").onclick=fe,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&fe()},document.getElementById("relink-search").addEventListener("input",e=>{In(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&xn(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&xn(t.dataset.id)});function Nt(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const a=Math.round(n/60);return a<48?`${a}h ago`:`${Math.round(a/24)}d ago`}async function Rt(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}ee()}const z='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',Xe='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',Fs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',oa='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',zs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',Ce='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',Dt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',Js='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>',et='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4.2 7a3.8 3.8 0 0 1 7.6 0c0 3 1.2 4 1.2 4H3s1.2-1 1.2-4z"/><path d="M6.7 13a1.5 1.5 0 0 0 2.6 0"/></svg>';function V(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${c(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"",a=e.act?`<button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>`:"";return`<div class="settings-item">
  ...
  </div>`,b=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],T=b.map(([I,j,Ae])=>V({icon:Dt,nameHTML:`<span class="edit-link" data-act="edit-prompt-${I}" title="edit the ${j.replace(/^\d+ · /,"")} prompt">${j}</span>`,desc:Ae,act:`edit-prompt-${I}`,actIcon:z,actTitle:`edit the ${j} prompt`,actLabel:`edit ${j} prompt`})).join(""),S=!i.stats||i.stats.taste_filter_enabled!==!1,U=V({icon:Js,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate that narrows bulk verdict runs before the paid LLM — location, headcount, vertical, stage. Re-scoring one company by hand ignores it.",dot:S?"ok":"off",note:S?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:z,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),H=i.anthropicKey;let D="off",_="not set — verdict, capture & outreach disabled";H&&H.key_source==="db"?(D="ok",_="set here · active"):H&&H.key_source==="env"&&(D="ok",_="from the environment");const g=V({icon:Fs,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:D,note:_,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:z,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),O=i.gmail||{},te=!!O.configured,ot=!!O.connected,Yt=O.config_source||"",$o=ot?"ok":"off";let it;te?ot?it=`connected as ${O.email||"(unknown)"}`:it=Yt==="env"?"configured (env) — not connected":"configured — not connected":it="not set up — add your Google OAuth client to connect";let ct;te?ot?ct='<button class="btn" data-act="gmail-config" title="edit the OAuth client">Credentials</button><button class="btn" data-act="gmail-disconnect">Disconnect</button>':ct='<button class="btn" data-act="gmail-config" title="edit the OAuth client">Credentials</button><button class="btn btn-primary" data-act="gmail-connect">Connect</button>':ct='<button class="btn btn-primary" data-act="gmail-config">Set up</button>';const Io=`<div class="settings-item">
    ...
    <div class="settings-item-main">
      ...
      <div class="crit-status"><span class="pf-dot ${$o}"></span><span class="crit-note-t">${c(it)}</span></div>
    </div>
    <div class="gmail-acts">${ct}</div>
  </div>`,xo=!!(i.gmail&&i.gmail.autoflip),Bo=`<div class="settings-item">
    <span class="settings-item-icon">${et}</span>
    <div class="settings-item-main">
    ...
    <input type="checkbox" class="set-autoflip" ${xo?"checked":""} title="auto-update application status" aria-label="auto-update application status">
  </div>`,_o=V({icon:Dt,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:z,actTitle:"edit the application stages",actLabel:"edit application stages"}),Lo=V({icon:Dt,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:z,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${g}${Io}${Bo}
     </div>`;const Zt={"view-profile":()=>to(i.profile),"refresh-profile":eo,"edit-taste":()=>K("taste"),"edit-taste-filter":()=>ea(),"edit-application-stages":()=>K("application-stages"),"edit-outreach-statuses":()=>K("outreach-statuses"),"edit-playbook":()=>K("playbook"),"edit-template":()=>K("outreach-template"),"edit-subject":()=>K("outreach-subject"),"edit-signature":()=>K("outreach-signature"),"edit-followup-template":()=>K("followup-template"),"edit-anthropic-key":Qs,"gmail-config":Ws,"gmail-connect":Gs,"gmail-disconnect":Ks};for(const[I]of b)Zt[`edit-prompt-${I}`]=()=>K(`outreach-prompts/${I}`);e.querySelectorAll("[data-act]").forEach(I=>{const j=I.dataset.act;j&&Zt[j]&&(I.onclick=Zt[j])});const lt=e.querySelector(".set-fu-interval");lt&&lt.addEventListener("change",async()=>{const I=Math.max(0,Math.min(90,parseInt(lt.value,10)||0));lt.value=String(I),await ie("PUT","/api/followup-interval",{days:I})&&(i.followupInterval=I,r("follow-up interval saved"))});const le=e.querySelector(".set-autoflip");le&&le.addEventListener("change",async()=>{let I=!1;try{I=(await fetch("/api/gmail/autoflip",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:le.checked})})).ok}catch{I=!1}I?(i.gmail&&(i.gmail.autoflip=le.checked),r(`auto-update application status ${le.checked?"on":"off"}`)):(le.checked=!le.checked,r("failed to save"))})}async function ia(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}ee()}async function je(){try{i.gmail=await(await fetch("/api/gmail/status")).json()}catch{i.gmail=null}ee()}async function Gs(){let e;try{e=await fetch("/api/gmail/connect")}catch(n){r(`connect failed: ${n.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}let t={};try{t=await e.json()}catch{}t.auth_url?window.location.href=t.auth_url:r("could not start the Gmail connect flow")}async function Ks(){if(!confirm("Disconnect Gmail? Sending and sync stop; already-synced data stays."))return;let e;try{e=await fetch("/api/gmail/disconnect",{method:"DELETE"})}catch(t){r(`disconnect failed: ${t.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}r("Gmail disconnected"),await je()}async function Ws(){await je();const e=i.gmail||{};document.getElementById("gmail-config-scrim").classList.add("open"),document.getElementById("gmail-client-id").value=e.client_id||"",document.getElementById("gmail-client-secret").value="",document.getElementById("gmail-redirect").value=e.redirect_uri||"";const t=document.getElementById("gmail-config-remove");t&&(t.style.display=e.config_source==="db"?"":"none");const n=document.getElementById("gmail-client-id");n&&n.focus()}function Me(){document.getElementById("gmail-config-scrim").classList.remove("open")}async function Ys(){const e=document.getElementById("gmail-client-id").value.trim(),t=document.getElementById("gmail-client-secret").value,n=document.getElementById("gmail-redirect").value.trim();if(!e){r("client ID is required");return}let a;try{a=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:e,client_secret:t,redirect_uri:n})})}catch(s){r(`save failed: ${s.message}`);return}if(!a.ok){r((await a.text().catch(()=>"")).trim()||`HTTP ${a.status}`);return}r("Gmail OAuth client saved — click Connect"),Me(),await je()}async function Zs(){if(!confirm("Remove the stored Google OAuth client? Connecting needs it re-entered (or set via env)."))return;let e;try{e=await fetch("/api/gmail/config",{method:"DELETE"})}catch(t){r(`remove failed: ${t.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}r("OAuth client removed"),Me(),await je()}async function Qs(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await ia(),Vt();const e=document.getElementById("key-input");e&&e.focus()}function Vt(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const a=document.getElementById("key-restart-hint");if(a){const s=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);a.style.display=s?"":"none"}}function Ut(){document.getElementById("key-scrim").classList.remove("open")}async function ca(){const e=(document.getElementById("key-input").value||"").trim();if(!e){r("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let a;try{a=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(s){r(`save failed: ${s.message}`),n();return}if(!a.ok){r((await a.text().catch(()=>"")).trim()||`HTTP ${a.status}`),n();return}i.anthropicKey=await a.json(),document.getElementById("key-input").value="",n(),r("Anthropic key saved"),await Lt(),Vt(),ee()}async function Xs(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){r(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){r((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),r(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await Lt(),Vt(),ee()}async function eo(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){r(`refresh failed: ${n.message}`),Rt();return}if(!t.ok){const n=await t.text().catch(()=>"");r(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Rt();return}i.profile=await t.json(),ee(),r("company-fit brief refreshed"),C()}function to(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${Nt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Ft(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ft,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ft()};function la(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");tt(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function zt(){document.getElementById("docs-scrim").classList.remove("open")}function no(){return document.getElementById("docs-scrim").classList.contains("open")}function tt(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function ao(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),tt(e)}document.getElementById("open-docs").onclick=la,document.getElementById("docs-close").onclick=zt,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&zt()};function so(){q("settings")}document.getElementById("open-settings").onclick=so,document.getElementById("gmail-config-cancel").onclick=Me,document.getElementById("gmail-config-save").onclick=Ys,document.getElementById("gmail-config-remove").onclick=Zs,document.getElementById("gmail-config-scrim").onclick=e=>{e.target.id==="gmail-config-scrim"&&Me()};function oo(){const e=document.getElementById("notif-badge");if(!e)return;const t=(i.notifications&&i.notifications.unread)|0;t>0?(e.textContent=t>99?"99+":String(t),e.style.display=""):e.style.display="none"}async function ce(){try{i.notifications=await(await fetch("/api/notifications")).json()}catch{return}oo(),document.getElementById("notifications-scrim").classList.contains("open")&&ra()}function io(){document.getElementById("notifications-scrim").classList.add("open"),ra(),ce()}function Jt(){document.getElementById("notifications-scrim").classList.remove("open")}function co(){return'<option value="">link to role…</option>'+(i.jobs||[]).map(t=>`<option value="${c(t.posting_id)}">${c((t.company||"")+" — "+(t.title||"(untitled)"))}</option>`).join("")}function lo(e){const t=e.company||e.role?`<div class="notif-ctx">${c([e.company,e.role].filter(Boolean).join(" · "))}</div>`:'<div class="notif-ctx dim">not linked to a role</div>',n=e.created_at?`<span class="notif-when">${c((e.created_at||"").replace("T"," ").slice(0,16))}</span>`:"",a=e.kind==="app_status"&&e.suggested_status&&!e.actioned&&e.posting_id?`<button class="btn btn-primary notif-apply" data-id="${e.id}">Apply: ${c(e.suggested_status)}</button>`:"",s=e.posting_id?"":`<select class="input notif-link" data-id="${e.id}" title="link this to a role">${co()}</select>`;return`<div class="notif-item${e.seen?"":" is-unread"}" data-id="${e.id}" data-seen="${e.seen?1:0}">
  ...
  </div>`}function ro(e){return`<div class="notif-item notif-followup">
    <div class="notif-main">
  ...
  </div>`}function ra(){const e=document.getElementById("notifications-body");if(!e)return;const t=i.notifications||{notifications:[],followups:[]},n=t.notifications||[],a=t.followups||[];if(!n.length&&!a.length){e.innerHTML='<div class="cc-empty dim">Nothing here yet. Replies, application updates, and follow-ups show up as Gmail syncs.</div>';return}let s="";n.length&&(s+='<div class="settings-group-h">Updates</div>'+n.map(lo).join("")),a.length&&(s+='<div class="settings-group-h">Follow-ups due</div>'+a.map(ro).join("")),e.innerHTML=s,uo()}function uo(){const e=document.getElementById("notifications-body");e&&(e.querySelectorAll(".notif-item[data-id]").forEach(t=>{const n=t.dataset.id,a=t.querySelector(".notif-main");a&&t.dataset.seen==="0"&&a.addEventListener("click",()=>po(n))}),e.querySelectorAll(".notif-apply").forEach(t=>t.addEventListener("click",n=>{n.stopPropagation(),mo(t.dataset.id)})),e.querySelectorAll(".notif-link").forEach(t=>t.addEventListener("change",n=>{n.stopPropagation(),t.value&&ho(t.dataset.id,t.value)})),e.querySelectorAll(".notif-open").forEach(t=>t.addEventListener("click",()=>{const n=t.dataset.pid;Jt(),gt(n)})))}async function po(e){try{await fetch(`/api/notifications/${e}/seen`,{method:"POST"})}catch{return}await ce()}async function mo(e){let t;try{t=await fetch(`/api/notifications/${e}/apply`,{method:"POST"})}catch(a){r(`apply failed: ${a.message}`);return}if(!t.ok){r((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}const n=await t.json().catch(()=>({}));r(`status set to ${n.applied||"updated"}`),await ce(),await k()}async function ho(e,t){let n;try{n=await fetch(`/api/notifications/${e}/link`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({posting_id:t})})}catch(a){r(`link failed: ${a.message}`);return}if(!n.ok){r((await n.text().catch(()=>"")).trim()||`HTTP ${n.status}`);return}r("linked to role"),await ce()}async function fo(){const e=document.getElementById("notifications-sync");e&&(e.disabled=!0,e.textContent="Syncing…");try{const t=await fetch("/api/gmail/sync",{method:"POST"});r(t.ok?"synced":(await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`)}catch(t){r(`sync failed: ${t.message}`)}e&&(e.disabled=!1,e.textContent="Sync now"),await ce(),await k()}document.getElementById("open-notifications").onclick=io,document.getElementById("notifications-close").onclick=Jt,document.getElementById("notifications-sync").onclick=fo,document.getElementById("notifications-scrim").onclick=e=>{e.target.id==="notifications-scrim"&&Jt()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),tt(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const a=n.filter(s=>s.isIntersecting).sort((s,o)=>s.boundingClientRect.top-o.boundingClientRect.top);a.length&&tt(a[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function go(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function yo(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function nt(e){return e.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+|mailto:[^\s)]+)\)/g,(t,n,a)=>`<a href="${a}" target="_blank" rel="noopener noreferrer">${n}</a>`).replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*\n]+)\*/g,"$1<em>$2</em>")}function vo(e){const t=String(e||"").split(`
...
`))+"</code></pre>");continue}const d=l.match(/^(#{1,6})\s+(.*)$/);if(d){s();const f=d[1].length;n.push("<h"+f+">"+nt(c(d[2]))+"</h"+f+">"),o++;continue}const u=l.match(/^\s*[-*]\s+(.*)$/);if(u){a!=="ul"&&(s(),n.push("<ul>"),a="ul"),n.push("<li>"+nt(c(u[1]))+"</li>"),o++;continue}const p=l.match(/^\s*\d+\.\s+(.*)$/);if(p){a!=="ol"&&(s(),n.push("<ol>"),a="ol"),n.push("<li>"+nt(c(p[1]))+"</li>"),o++;continue}if(l.trim()===""){s(),o++;continue}s();const m=[];for(;o<t.length&&t[o].trim()!==""&&!/^```|^#{1,6}\s|^\s*[-*]\s+|^\s*\d+\.\s+/.test(t[o]);)m.push(nt(c(t[o]))),o++;n.push("<p>"+m.join("<br>")+"</p>")}return s(),n.join("")}function at(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,e==="assistant"?n.innerHTML=vo(t||""):n.textContent=t||"",n}function Gt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function bo(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function da(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const a=go(n.content);if(n.role==="user")a&&t.appendChild(at("user",a));else if(n.role==="assistant"){const s=yo(n.content);if(!a&&!s.length)continue;const o=at("assistant",a);if(s.length){const l=document.createElement("div");l.className="chat-tools",l.textContent="· used "+s.join(", "),o.appendChild(l)}t.appendChild(o)}}t.children.length||t.appendChild(bo()),Gt()}async function Kt(e,t,n){if(!i.meta||!i.meta.chat){r("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const a=document.getElementById("chat-messages");a.innerHTML='<div class="chat-empty">loading…</div>';const s=document.getElementById("chat-pane");s.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),s.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),l=await fetch("/api/chat/threads?"+o);if(!l.ok)throw new Error((await l.text().catch(()=>"")).trim()||"HTTP "+l.status);const d=await l.json();i.chat.threadId=d.thread.id,da(d.messages||[])}catch(o){a.innerHTML='<div class="chat-empty">Failed to open chat: '+c(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Wt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function st(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function ua(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function pa(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",ua(),st(!0);const n=document.getElementById("chat-messages"),a=n.querySelector(".chat-empty");a&&a.remove(),n.appendChild(at("user",t));const s=at("assistant","");s.classList.add("chat-streaming"),n.appendChild(s),Gt();let o="";const l=m=>{s.classList.remove("chat-streaming"),s.textContent="⚠ "+m,st(!1)},d=i.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){l(m.message);return}if(!u.ok){l((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");i.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,s.textContent=o,Gt()}),p.addEventListener("end",async m=>{p.close(),i.chat.es===p&&(i.chat.es=null),s.classList.remove("chat-streaming"),st(!1),i.chat.threadId===d&&await wo(),ko(),typeof m.data=="string"&&m.data.indexOf("error")===0&&r("chat: "+m.data)}),p.onerror=()=>{p.close(),i.chat.es===p&&(i.chat.es=null),s.classList.remove("chat-streaming"),st(!1)}}async function wo(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const a=await fetch("/api/chat/threads?"+n);if(!a.ok)return;const s=await a.json();da(s.messages||[])}catch{}}function ko(){N(),k(),C(),i.openId&&be(i.openId)}document.getElementById("open-chat").onclick=()=>Kt("global","",""),document.getElementById("chat-close").onclick=Wt,document.getElementById("chat-scrim").onclick=Wt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),pa()}),document.getElementById("chat-input").addEventListener("input",ua),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),pa())}),ln("#t tbody",va),ln("#jt tbody",ba);const Eo=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();q(Eo==="jobs"?"jobs":"companies",{render:!1});function ma(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||ma()}),window.addEventListener("pageshow",e=>{e.persisted&&ma()}),N(),k(),C(),Lt(),Tt(),Rt(),ia(),je(),ce(),setInterval(ce,9e4),P(),function(){const t=/[?&]gmail=(connected|error)/.exec(location.search);t&&(r(t[1]==="connected"?"Gmail connected":"Gmail connection failed"),history.replaceState(null,"",location.pathname+location.hash))}()}Co({"":{view:()=>({mount(w){w.innerHTML=Po,qo()}}),chrome:!1}},{title:"scout"});Ao();Ho();
