(function(){const v=document.createElement("link").relList;if(v&&v.supports&&v.supports("modulepreload"))return;for(const g of document.querySelectorAll('link[rel="modulepreload"]'))c(g);new MutationObserver(g=>{for(const x of g)if(x.type==="childList")for(const l of x.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&c(l)}).observe(document,{childList:!0,subtree:!0});function I(g){const x={};return g.integrity&&(x.integrity=g.integrity),g.referrerPolicy&&(x.referrerPolicy=g.referrerPolicy),g.crossOrigin==="use-credentials"?x.credentials="include":g.crossOrigin==="anonymous"?x.credentials="omit":x.credentials="same-origin",x}function c(g){if(g.ep)return;g.ep=!0;const x=I(g);fetch(g.href,x)}})();function So(w,v){const I=v.replace(/^#/,"");let c=null;for(const g of Object.keys(w))(I===g||I.startsWith(g))&&(c===null||g.length>c.length)&&(c=g);return c===null&&""in w&&(c=""),c}function _o(w){return typeof w=="function"?{view:w,chrome:!0}:{view:w.view,chrome:w.chrome!==!1}}function To(w,v={}){const I=v.root??document.body,c=v.title??document.title??"",g=v.brandHref??"#",x=document.createElement("main"),l=document.createElement("header");l.className="cap-head";const A=document.createElement("a");A.className="brand",A.href=g,A.textContent=c,A.setAttribute("aria-label",`${c} — home`),l.appendChild(A);const O=document.createElement("nav");O.className="cap-nav",O.setAttribute("aria-label","Views");for(const P of v.nav??[]){const T=document.createElement("a");T.href=P.href,T.textContent=P.label,P.ariaLabel&&T.setAttribute("aria-label",P.ariaLabel),O.appendChild(T)}l.appendChild(O);const E=document.createElement("section");E.className="tk-content",x.appendChild(l),x.appendChild(E);const V=document.createElement("div");V.className="tk-bleed";const ee=P=>{var T;for(const C of Array.from(O.querySelectorAll("a"))){const te=((T=C.getAttribute("href"))==null?void 0:T.replace(/^#/,""))??"";C.toggleAttribute("aria-current",P!==null&&P!==""&&te===P),C.hasAttribute("aria-current")&&C.setAttribute("aria-current","page")}};let ke=0;const qe=()=>{const P=So(w,location.hash);if(ee(P),P===null){V.isConnected&&V.remove(),x.isConnected||I.appendChild(x),Bo(E,"Not found.");return}const{view:T,chrome:C}=_o(w[P]),te=C?E:V;C?(V.isConnected&&V.remove(),x.isConnected||I.appendChild(x)):(x.isConnected&&x.remove(),V.isConnected||I.appendChild(V)),te.replaceChildren();const at=T(),He=++ke,Ee=at.mount(te);Ee instanceof Promise&&Ee.catch(J=>{He===ke&&Lo(te,String(J))})};window.addEventListener("hashchange",qe),qe()}function Bo(w,v){w.replaceChildren();const I=document.createElement("div");I.className="tk-empty",I.textContent=v,w.appendChild(I)}function Lo(w,v){w.replaceChildren();const I=document.createElement("div");I.className="tk-error",I.textContent=v,w.appendChild(I)}function Co(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(v=>{for(const I of v)I.unregister()}),window.caches&&caches.keys().then(v=>{for(const I of v)caches.delete(I)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function jo(){let w;try{w=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!w.ok)return null;let v;try{v=await w.json()}catch{return null}return typeof v.email=="string"&&v.email?{email:v.email}:null}const Mo=`
<div class="layout">
<aside class="sidebar">
...
`;function Ao(w){const v={k:"verdict",dir:1},I={k:"created_at",dir:1},c={rows:[],sort:{...v},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...I},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null,gmail:null,notifications:{notifications:[],unread:0,followups:[]},settingsGroup:"outreach"},g=e=>"pill pill-"+(e||"none"),x='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',l=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),A=e=>/^https?:\/\//i.test(String(e??""))?l(e):"#";async function O(){const t=await(await fetch("/api/companies")).json();c.rows=t.rows||[],z()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}c.jobs=e.rows||[],j(),V(),ke()}function V(){if(!f.postingId)return;const e=c.jobs.find(t=>t.posting_id===f.postingId);e&&(f.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&se())}let ee=null;function ke(){const e=c.jobs.some(t=>t.outreach_draft_status==="researching");e&&!ee?ee=setInterval(qe,4e3):!e&&ee&&(clearInterval(ee),ee=null)}async function qe(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(c.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==c.jobs.length;c.jobs=t,s&&(j(),V()),ke()}async function P(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(c.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(c.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(c.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(c.followupTemplate=e.content)}).catch(()=>{})]),lt(),c.view==="jobs"&&j()}function T(e,{render:t=!0}={}){c.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("tab-inbox").classList.toggle("active",e==="inbox"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none";const n=(i,d)=>{const u=document.getElementById(i);u&&(u.style.display=d?"":"none")};n("settings-view",e==="settings"),n("inbox-view",e==="inbox"),n("docs-view",e==="docs"),document.getElementById("open-settings").classList.toggle("is-active",e==="settings");const s=document.getElementById("open-docs");s&&s.classList.toggle("is-active",e==="docs");for(const i of["btn-enrich","btn-verdict"]){const d=document.getElementById(i);d&&(d.style.display=e==="companies"?"":"none")}const a=e==="companies"||e==="jobs";document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none";const o=document.getElementById("block-columns");o&&(o.style.display=a?"":"none"),Wt(),t&&(e==="jobs"?j():e==="settings"?K():e==="inbox"?(ts(),oe()):e==="docs"?io():z())}async function C(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}c.stats=e,te()}function te(){K()}function at(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function He(e){return e.slice().sort((t,n)=>c.sort.dir*at(t,n,c.sort.k))}function Ee(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(s=>{s.getAttribute(t)===n.k?s.dataset.sort=n.dir<0?"desc":"asc":delete s.dataset.sort})}const J=new Set;let ce=!1,le=!1;const is=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function cs(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+is.map(([t,n,s])=>ne("data-v",t,n,s,J.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+ne("data-toggle","flagged","⚑ Flagged","",ce)+ne("data-toggle","enriched","Enriched","",le),Vt())}function Vt(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const i of c.rows){const d=i.verdict||"__none__";e[d]=(e[d]|0)+1,i.flagged&&t++,i.enriched&&n++}rt("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=J.size+(ce?1:0)+(le?1:0);sn("fdrop-cfilters-btn",o,o>0)}function Ft(){const e=document.getElementById("q").value.trim().toLowerCase();return c.rows.filter(t=>!(J.size&&!J.has(t.verdict||"__none__")||ce&&!t.flagged||le&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const ls=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],rs=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Jt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Gt=Jt("scout-hidden-cols"),zt=Jt("scout-hidden-jcols");function ot(){return c.view==="jobs"?{cols:rs,hidden:zt,key:"scout-hidden-jcols"}:{cols:ls,hidden:Gt,key:"scout-hidden-cols"}}function re(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Gt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=zt.has(e.dataset.col)?"none":""})}function Wt(){const e=ot(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>ne("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Kt())}function Kt(){const e=ot(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Yt(e){return`
  ...
  `,Rs();const i=document.getElementById("pursuit-company-link");i&&i.addEventListener("click",()=>ye(e.company_id)),Ss(e),G(document.getElementById("pursuit-title-input"),p=>mn(e,"title",p)),G(document.getElementById("pursuit-url-input"),p=>Is(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>xs(e,d)),G(document.getElementById("pursuit-notes-input"),p=>Ds(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>G(p,m=>mn(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>Cs(e)),he(),_e(),t&&(t.scrollTop=s),yn=e.posting_id}function Ns(e){return`
    ...
    <div class="draft-head">
      ...
      <div class="draft-sentbody">${l(bt(e)||"(empty)")}</div>
      ${Se(e)}
      ...
      <div class="draft-sentbody">${l(bt(e)||"(empty)")}</div>
      ${Se(e)}
    </div>`;const s=bt(e),a=e.status==="no_hook",o=a?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let i="";if(a)try{i=JSON.parse(e.hook||"{}").reasoning||""}catch{}const d=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${i?" "+l(i):""}</div>`:"";if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    ${Se(e)}
  </div>`}function bt(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function Se(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,p)=>p?`<div class="tr-line"><span class="tr-key">${u}:</span> ${l(String(p))}</div>`:"",o=n.role||{},i=Array.isArray(n.hooks)?n.hooks:[],d=i.map(u=>`
      <div class="tr-line">
  ...
  `;const y=document.getElementById("posting-add-btn");y&&y.addEventListener("click",()=>wa(e)),Et(),document.querySelectorAll("#ve-pick .ve-opt").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(ie=>ie.classList.remove("is-on")),b.classList.add("is-on")})});const M=document.getElementById("ve-save-btn");M&&M.addEventListener("click",()=>ba(e)),G(document.getElementById("pane-title-input"),b=>gn(e,"name",b)),document.querySelectorAll("#facts-body [data-k]").forEach(b=>G(b,ie=>gn(e,b.dataset.k,ie))),G(document.getElementById("pane-domain-input"),b=>Hs(e,b)),G(document.getElementById("pane-notes-input"),b=>Os(e,b),{multiline:!0});const B=document.getElementById("flag-toggle-btn");B&&B.addEventListener("click",()=>jn(e.company_id));const k=document.getElementById("review-stamp-btn");k&&k.addEventListener("click",()=>va(e.company_id));const L=document.getElementById("rescore-btn");L&&L.addEventListener("click",()=>xt("verdict",{company_ids:[e.company_id]}));const U=document.getElementById("reenrich-btn");U&&U.addEventListener("click",()=>xt("enrich",{company_ids:[e.company_id]}));const R=document.getElementById("company-delete-btn");R&&R.addEventListener("click",()=>Bs(e))}function ya(e){return`
    <div class="ie-grid">
...
`)):document.getElementById("editor-text").value=i.content||"",a&&(document.getElementById("editor-enabled").checked=i.enabled!==!1),i.taste_version&&(document.getElementById("editor-ver").textContent="version "+i.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Ye(){document.getElementById("editor-scrim").classList.remove("open"),N=null}let _=null,Tt=[],Bt=[];const La=["location.allowed","verticals.excluded","verticals.allowed"],Lt={"verticals.excluded":"pf-vertical-tags","verticals.allowed":"pf-vertical-tags"};function Q(e){const[t,n]=e.split(".");return _[t]&&_[t][n]||[]}function Ze(e,t){const[n,s]=e.split(".");(_[n]=_[n]||{})[s]=t}function Qe(e,t){const n=t.toLowerCase();return e.some(s=>String(s).toLowerCase()===n)}function Ct(){La.forEach(e=>{const t=document.querySelector(`.pf-chips[data-field="${e}"]`);if(!t)return;const n=Lt[e]?` list="${Lt[e]}"`:"",s=Lt[e]?"type to search…":"type &amp; Enter…";t.innerHTML=Q(e).map((a,o)=>`<span class="pf-chip">${l(a)}<button class="pf-chip-x" data-field="${e}" data-i="${o}" title="remove" aria-label="remove ${l(a)}">×</button></span>`).join("")+`<input class="pf-chip-input" data-field="${e}"${n} type="text" placeholder="${s}" spellcheck="false" autocomplete="off" />`})}function Ca(e,t){var s;const n=(t||"").trim();if(n){const a=Q(e);Qe(a,n)||Ze(e,[...a,n])}Ct(),(s=document.querySelector(`.pf-chip-input[data-field="${e}"]`))==null||s.focus()}function Fn(e,t){const n=Q(e).slice();n.splice(t,1),Ze(e,n),Ct()}function Jn(){const e=document.getElementById("pf-stages");if(!e)return;const t=Q("funding_stage.allowed");e.innerHTML=Bt.map(n=>{const s=Qe(t,n.value),a=n.count?` <span class="pf-stage-n">${n.count}</span>`:"";return`<button class="pf-stage${s?" is-on":""}" data-stage="${l(n.value)}">${l(n.value)}${a}</button>`}).join("")}function ja(e){const t=Q("funding_stage.allowed");Ze("funding_stage.allowed",Qe(t,e)?t.filter(n=>String(n).toLowerCase()!==e.toLowerCase()):[...t,e]),Jn()}function Ma(){const e=document.getElementById("pf-vertical-tags");e&&(e.innerHTML=Tt.map(t=>`<option value="${l(t.value)}" label="${t.count}"></option>`).join(""))}function Aa(){return{location:{allowed:[],remote_ok:!0},headcount:{min:0,max:0},verticals:{allowed:[],excluded:[]},funding_stage:{allowed:[]}}}function Pa(){return`<div class="set-field pf-inline">
  ...
  </div>`}function Pt(e){const t=e.querySelector(".set-saved");t&&(t.classList.add("show"),setTimeout(()=>t.classList.remove("show"),1500))}async function Ja(e){const t=e.dataset.kind,n=e.dataset.list==="1",s=e.querySelector(".set-textarea");try{const a=await(await fetch(`/api/${t}`)).json();s.value=n?(a.statuses||[]).join(`
`):a.content||""}catch{s.value=""}s.dataset.orig=s.value,s.dataset.loaded="1",s.addEventListener("blur",()=>Ga(e))}async function Ga(e){const t=e.dataset.kind,n=e.dataset.list==="1",s=e.querySelector(".set-textarea");if(s.dataset.loaded!=="1"||s.value===s.dataset.orig)return;const a=n?{statuses:s.value.split(/\r?\n/).map(i=>i.trim()).filter(Boolean)}:{content:s.value};let o;try{o=await fetch(`/api/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}catch(i){r(`save failed: ${i.message}`);return}if(!o.ok){r(`save failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}s.dataset.orig=s.value,Pt(e),t==="followup-template"&&(c.followupTemplate=s.value),n&&(await P(),j())}function qt(e){e.querySelectorAll(".set-field[data-kind]").forEach(Ja)}function za(e){e.innerHTML=ae("outreach-subject","Email subject","The send subject — {{role}} / {{company}} substitution, no LLM.",2,!1)+ae("outreach-template","Email body","Verbatim prose with the writer's fill-in holes. Put your sign-off at the bottom — markdown links render as real links on send.",18,!1)+ae("followup-template","Follow-up body","The full follow-up, sign-off included — {{contact_name}}, {{role}}, {{company}}, {{last_sent}}, {{last_message}}.",9,!1)+`<div class="set-field">
    ...
    </div>`,qt(e);const t=e.querySelector(".set-fu-interval");t&&t.addEventListener("change",async()=>{const n=Math.max(0,Math.min(90,parseInt(t.value,10)||0));t.value=String(n),await W("PUT","/api/followup-interval",{days:n})&&(c.followupInterval=n,r("follow-up interval saved"))})}function Wa(e){e.innerHTML=Fa.map(([t,n,s])=>`
