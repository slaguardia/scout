...
`;function wo(k){const w={k:"verdict",dir:1},x={k:"created_at",dir:1},i={rows:[],sort:{...w},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...x},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null,gmail:null,notifications:{notifications:[],unread:0,followups:[]},settingsGroup:"outreach"},v=e=>"pill pill-"+(e||"none"),I='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',c=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),M=e=>/^https?:\/\//i.test(String(e??""))?c(e):"#";async function q(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],F()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],j(),N(),fe()}function N(){if(!h.postingId)return;const e=i.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&ee())}let Z=null;function fe(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!Z?Z=setInterval(Le,4e3):!e&&Z&&(clearInterval(Z),Z=null)}async function Le(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&(j(),N()),fe()}async function A(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(i.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(i.followupTemplate=e.content)}).catch(()=>{})]),et(),i.view==="jobs"&&j()}function L(e,{render:t=!0}={}){i.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none";const n=(d,u)=>{const p=document.getElementById(d);p&&(p.style.display=u?"":"none")};n("settings-view",e==="settings"),n("inbox-view",e==="inbox"),n("docs-view",e==="docs"),document.getElementById("open-settings").classList.toggle("is-active",e==="settings");const s=document.getElementById("open-notifications");s&&s.classList.toggle("is-active",e==="inbox");const a=document.getElementById("open-docs");a&&a.classList.toggle("is-active",e==="docs");const o=e==="companies"||e==="jobs";document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none";const l=document.getElementById("block-columns");l&&(l.style.display=o?"":"none"),Rt(),t&&(e==="jobs"?j():e==="settings"?J():e==="inbox"?(zn(),ne()):e==="docs"?Ja():F())}async function C(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}i.stats=e,Q()}function Q(){J()}function Ye(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function Be(e){return e.slice().sort((t,n)=>i.sort.dir*Ye(t,n,i.sort.k))}function ge(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(s=>{s.getAttribute(t)===n.k?s.dataset.sort=n.dir<0?"desc":"asc":delete s.dataset.sort})}const R=new Set;let se=!1,ae=!1;const Zn=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function Qn(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+Zn.map(([t,n,s])=>z("data-v",t,n,s,R.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+z("data-toggle","flagged","⚑ Flagged","",se)+z("data-toggle","enriched","Enriched","",ae),Pt())}function Pt(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const l of i.rows){const d=l.verdict||"__none__";e[d]=(e[d]|0)+1,l.flagged&&t++,l.enriched&&n++}tt("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=R.size+(se?1:0)+(ae?1:0);Wt("fdrop-cfilters-btn",o,o>0)}function qt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(R.size&&!R.has(t.verdict||"__none__")||se&&!t.flagged||ae&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const Xn=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],es=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Ht(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Ot=Ht("scout-hidden-cols"),Nt=Ht("scout-hidden-jcols");function Ze(){return i.view==="jobs"?{cols:es,hidden:Nt,key:"scout-hidden-jcols"}:{cols:Xn,hidden:Ot,key:"scout-hidden-cols"}}function oe(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Ot.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Nt.has(e.dataset.col)?"none":""})}function Rt(){const e=Ze(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>z("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Dt())}function Dt(){const e=Ze(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Ut(e){return`
    ...
    `}function Vt(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>xn(t.dataset.id))}const ts=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],ns=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function Ft(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let l=0;l<n;l++){const d=t.map(([u,p])=>{const m=p.endsWith("%")?Math.round(parseFloat(p)*a[l%a.length])+"%":p;return`<td${u?` data-col="${u}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${d}</tr>`}s.innerHTML=o,oe()}function F(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=Be(qt());Pt(),document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=Ut(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||me(s.dataset.id)}),e.appendChild(s),Vt(s)}ge("t","data-k",i.sort),oe()}async function ss(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=Be(qt()).map(l=>l.company_id),o=[...s.querySelectorAll("tr")].map(l=>l.dataset.id);if(a.length!==o.length||a.some((l,d)=>l!==o[d])){F();return}for(const l of e){const d=i.rows.find(p=>p.company_id===l),u=s.querySelector(`tr[data-id="${CSS.escape(l)}"]`);if(!d||!u){F();return}u.innerHTML=Ut(d),Vt(u)}oe()}let $=null,Qe=null,ie=!1,ce=!1,_=null,Xe=null;function Jt(){const e=i.applicationStages;if($===null)$=new Set(["",...e.filter(t=>t!=="rejected")]);else{for(const t of[...$])t!==""&&!e.includes(t)&&$.delete(t);if(Qe)for(const t of e)t!=="rejected"&&!Qe.has(t)&&$.add(t)}Qe=new Set(e)}function zt(){const e=i.outreachStatuses;if(_===null)_=new Set(["",...e]);else{for(const t of[..._])t!==""&&!e.includes(t)&&_.delete(t);if(Xe)for(const t of e)Xe.has(t)||_.add(t)}Xe=new Set(e)}function as(){Jt(),zt();const e=document.getElementById("jq").value.trim().toLowerCase();return i.jobs.filter(t=>{const n=t.application_status||"";return!(!$.has(n)||ie&&!t.next_up||ce&&!(t.followups_due|0)||!_.has(t.outreach_status||"")||e&&!(t.title+" "+t.company+" "+(t.location||"")+" "+(t.description||"")+" "+(t.contacts||"")).toLowerCase().includes(e))})}function z(e,t,n,s,a){return`<button class="fdrop-item${a?" is-checked":""}" ${e}="${c(t)}" role="menuitemcheckbox" aria-checked="${a}"><span class="fdrop-check" aria-hidden="true"></span>`+(s?`<span class="fdrop-dot ${s}"></span>`:"")+`<span class="fdrop-label">${c(n)}</span><span class="fdrop-item-count" data-count></span></button>`}function Gt(e,t,n){return`<div class="fdrop-head fdrop-head--toggle"><span>${e}</span><button type="button" class="fdrop-all" data-all="${t}">${n?"none":"all"}</button></div>`}function et(){Jt(),zt();const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=["",...i.applicationStages],n=["",...i.outreachStatuses];e.innerHTML=Gt("Application stage","stage",t.every(s=>$.has(s)))+z("data-stage","","not applied","",$.has(""))+i.applicationStages.map(s=>z("data-stage",s,s,je(s),$.has(s))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Outreach queue</div>'+z("data-toggle","nextup","★ Next up","",ie)+'<div class="fdrop-sep"></div>'+Gt("Reply status","status",n.every(s=>_.has(s)))+[["","not reached out",""]].concat(i.outreachStatuses.map(s=>[s,s,tn(s)])).map(([s,a,o])=>z("data-status",s,a,o,_.has(s))).join(""),Kt()}function Kt(){const e={},t={};let n=0;for(const u of i.jobs){const p=u.application_status||"";e[p]=(e[p]|0)+1;const m=u.outreach_status||"";t[m]=(t[m]|0)+1,u.next_up&&n++}tt("#fdrop-jfilters-menu [data-stage]","data-stage",e),tt("#fdrop-jfilters-menu [data-status]","data-status",t),os("nextup",n);const s=["",...i.applicationStages.filter(u=>u!=="rejected")],a=$&&$.size===s.length&&s.every(u=>$.has(u)),o=["",...i.outreachStatuses],l=_&&_.size===o.length&&o.every(u=>_.has(u)),d=(a?0:$?$.size:0)+(ie?1:0)+(l?0:_?_.size:0);Wt("fdrop-jfilters-btn",d,d>0)}function tt(e,t,n){document.querySelectorAll(e).forEach(s=>{const a=s.querySelector("[data-count]");if(a){const o=n[s.getAttribute(t)]|0;a.textContent=o||""}})}function os(e,t){const n=document.querySelector(`#fdrop-jfilters-menu [data-toggle="${e}"] [data-count]`);n&&(n.textContent=t||"")}function Wt(e,t,n){const s=document.getElementById(e);if(!s)return;s.classList.toggle("is-active",n);const a=s.querySelector(".fdrop-count");if(a){const o=n&&t>0;a.textContent=o?t:"",a.style.display=o?"":"none"}}function X(e,t){e.classList.toggle("is-checked",t),e.setAttribute("aria-checked",String(t))}function is(){const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=e.querySelector('.fdrop-all[data-all="stage"]'),n=e.querySelector('.fdrop-all[data-all="status"]');t&&(t.textContent=["",...i.applicationStages].every(s=>$.has(s))?"none":"all"),n&&(n.textContent=["",...i.outreachStatuses].every(s=>_.has(s))?"none":"all")}function nt(){document.querySelectorAll(".fdrop.is-open").forEach(e=>{e.classList.remove("is-open");const t=e.querySelector(".fdrop-btn");t&&t.setAttribute("aria-expanded","false")})}function Yt(e){const t=e.querySelector(".fdrop-btn"),n=e.querySelector(".fdrop-menu");if(!t||!n)return;const s=t.getBoundingClientRect();n.style.left=Math.round(s.left)+"px",n.style.top=Math.round(s.bottom+4)+"px",n.style.minWidth=Math.round(s.width)+"px",n.style.maxHeight=Math.max(160,Math.round(window.innerHeight-s.bottom-12))+"px"}function cs(e){const t=e.querySelector(".fdrop-btn");e.classList.add("is-open"),t&&t.setAttribute("aria-expanded","true"),Yt(e)}function Zt(){const e=document.querySelector(".fdrop.is-open");e&&Yt(e)}window.addEventListener("scroll",Zt,!0),window.addEventListener("resize",Zt);const ls=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function rs(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(ls),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}const Ce=()=>new Date().toISOString().slice(0,10);function Qt(e){const t=[["","none"]];for(const n of i.applicationStages)t.push([n,n]);return e&&!i.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function Xt(e){const t=[["","none"]];for(const n of i.outreachStatuses)t.push([n,n]);return e&&!i.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const ds=8;function en(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%ds}function je(e){return en(e,i.applicationStages)}function tn(e){return en(e,i.outreachStatuses)}function us(e){const t=rs(e);return t.length?t.map(n=>{const s=c(n.position||n.email);if(!n.email)return s;const a=c(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${c(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function nn(e){const t=e.application_status||"";if(!t)return i.applicationStages.length+1;const n=i.applicationStages.indexOf(t);return n<0?i.applicationStages.length:n}function ps(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return nn(e)-nn(t);if(n==="followups_due")return(t.followups_due|0)-(e.followups_due|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function ms(){const e=document.getElementById("jobs-followup-nav");if(!e)return;const t=i.jobs.reduce((n,s)=>n+(s.followups_due|0),0);if(!t){e.style.display="none",ce=!1;return}e.style.display="",e.innerHTML=`<button class="followup-nav-btn${ce?" is-active":""}" title="${ce?"showing only these — click to show all jobs":"show only jobs owing a follow-up"}"><span class="fn-icon">${Un}</span><span class="fn-text"><strong>${t}</strong> follow-up${t>1?"s":""} due</span></button>`,e.querySelector(".followup-nav-btn").onclick=()=>{ce=!ce,j()}}function j(){const e=document.querySelector("#jt tbody");e.innerHTML="",ms();const t=as().sort((a,o)=>i.jsort.dir*ps(a,o,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",Kt();const n=$&&!$.has("rejected")?i.jobs.filter(a=>(a.application_status||"")==="rejected").length:0,s=document.getElementById("jobs-hidden-note");s.style.display=n?"":"none",n&&(s.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{$.add("rejected"),et(),j()});for(const a of t){const o=a.application_status||"",l=document.createElement("tr");l.dataset.id=a.posting_id;const d=Qt(o).map(([m,f])=>`<option value="${c(m)}"${o===m?" selected":""}>${c(f)}</option>`).join(""),u=a.outreach_status||"",p=Xt(u).map(([m,f])=>`<option value="${c(m)}"${u===m?" selected":""}>${c(f)}</option>`).join("");l.innerHTML=`
  ...
  `,Ts();const l=document.getElementById("pursuit-company-link");l&&l.addEventListener("click",()=>me(e.company_id)),vs(e),D(document.getElementById("pursuit-title-input"),p=>on(e,"title",p)),D(document.getElementById("pursuit-url-input"),p=>gs(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>ys(e,d)),D(document.getElementById("pursuit-notes-input"),p=>Ls(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>D(p,m=>on(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>$s(e)),de(),we(),t&&(t.scrollTop=s),dn=e.posting_id}function _s(e){return`
    ...
    <div class="draft-head">
      ...
      <div class="draft-sentbody">${c(rt(e)||"(empty)")}</div>
      ${be(e)}
      ...
      <div class="draft-sentbody">${c(rt(e)||"(empty)")}</div>
      ${be(e)}
    </div>`;const s=rt(e),a=e.status==="no_hook",o=a?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let l="";if(a)try{l=JSON.parse(e.hook||"{}").reasoning||""}catch{}const d=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${l?" "+c(l):""}</div>`:"";if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    ${be(e)}
  </div>`}function rt(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function be(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,p)=>p?`<div class="tr-line"><span class="tr-key">${u}:</span> ${c(String(p))}</div>`:"",o=n.role||{},l=Array.isArray(n.hooks)?n.hooks:[],d=l.map(u=>`
      <div class="tr-line">
    ...
    </div>`,s?s.replaceWith(l):n.appendChild(l);const d=l.querySelector("#gate-fix-btn");d&&d.addEventListener("click",()=>a?fa("outreach-template"):On());const u=l.querySelector("#gate-retry-btn");u&&u.addEventListener("click",Pe)}async function Fs(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.drafts.findIndex(d=>String(d.id)===String(e));a>=0&&(h.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),l=o&&o.closest(".draft-card");if(l){const d=l.querySelector(".lint-chips"),u=bn(s.lint);d?d.outerHTML=u||"":u&&o.insertAdjacentHTML("afterend",u)}}async function Js(e,t,n){n&&(n.disabled=!0,n.dataset.t=n.textContent||"",n.textContent="Sending…");const s=()=>{n&&(n.disabled=!1,n.textContent=n.dataset.t||"Send via Gmail")};let a;try{a=await fetch(`/api/outreach/drafts/${e}/send-gmail`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:t||""})})}catch(l){r(`send failed: ${l.message}`),s();return}if(!a.ok){const l=(await a.text().catch(()=>"")).trim();r(`send failed: ${l||"HTTP "+a.status}`),s();return}let o={};try{o=await a.json()}catch{}r(o.to?`sent via Gmail to ${o.to}`:"sent via Gmail"),await le(),await E()}async function zs(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){r(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();r(`failed: ${s||"HTTP "+t.status}`);return}r("marked sent"),await le(),await E();const n=i.jobs.find(s=>s.posting_id===h.postingId);n&&he(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function pe(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){we();return}e=await t.json()}catch{we();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",we(),h.answers.some(t=>t.status==="generating")?Gs():dt()}function Gs(){h.answersPoll||(h.answersPoll=setInterval(pe,4e3))}function dt(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function we(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(p=>p.status==="generating"),a=t.length?`<div class="answers-list">${t.map(Ys).join("")}</div>`:"",o=!!h.detecting,l=s||o?" disabled":"",d=p=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":p}</button>`;let u;n==="ok"&&t.length?u=(t.some(m=>!wn(m)&&m.status!=="generating")?`<button class="btn" id="answers-start-btn"${l}>${s?"Drafting…":"Draft all blank"}</button>`:"")+d("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${l}>${s?"Drafting…":"Draft answers"}</button>`+d("Re-detect questions"):u=d("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${c(Ks(n,t.length))}</div>`+a+`<div class="answers-actions">${u}</div>`,Qs()}function Ks(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function wn(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function Ws(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function Ys(e){const t=wn(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,l=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`,d=!!t,u=d?"Regenerate":"Generate",p=d?"re-draft this answer (discards the current text)":"draft an answer to just this question";return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
  ...
  </div>`}function Zs(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function Qs(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",kn);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",ea),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(D(o,p=>ta(a,p),{multiline:!0}),o.addEventListener("input",()=>Xs(s,o)));const l=s.querySelector(".answer-regen-btn");l&&l.addEventListener("click",()=>na(a));const d=s.querySelector(".answer-copy-btn");d&&d.addEventListener("click",()=>{o&&ht(o.value,"answer copied")});const u=s.querySelector(".answer-remove-btn");u&&u.addEventListener("click",()=>sa(a))})}function Xs(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function kn(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){r(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await pe();return}if(n.status===412){let a={};try{a=await n.json()}catch{}En(a.error),t&&(t.disabled=!1);return}if(n.status===503){$n("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();r(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function ea(){h.detecting=!0,we();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();r(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){r(`detect failed: ${e.message}`)}h.detecting=!1,await pe()}async function ta(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=s)}async function na(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){r(`regenerate failed: ${n.message}`);return}if(t.status===503){$n("Answer generation isn't running in this build.");return}if(t.status===412){let n={};try{n=await t.json()}catch{}En(n.error);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();r(`regenerate failed: ${n||"HTTP "+t.status}`);return}await pe()}async function sa(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"DELETE"})}catch(n){r(`remove failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();r(`remove failed: ${n||"HTTP "+t.status}`);return}await pe()}function En(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
  ...
  `;const g=document.getElementById("posting-add-btn");g&&g.addEventListener("click",()=>ca(e)),pt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(y=>{y.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(V=>V.classList.remove("is-on")),y.classList.add("is-on")})});const b=document.getElementById("ve-save-btn");b&&b.addEventListener("click",()=>ia(e)),D(document.getElementById("pane-title-input"),y=>rn(e,"name",y)),document.querySelectorAll("#facts-body [data-k]").forEach(y=>D(y,V=>rn(e,y.dataset.k,V))),D(document.getElementById("pane-domain-input"),y=>Is(e,y)),D(document.getElementById("pane-notes-input"),y=>Ss(e,y),{multiline:!0});const S=document.getElementById("flag-toggle-btn");S&&S.addEventListener("click",()=>xn(e.company_id));const O=document.getElementById("review-stamp-btn");O&&O.addEventListener("click",()=>oa(e.company_id));const P=document.getElementById("rescore-btn");P&&P.addEventListener("click",()=>gt("verdict",{company_ids:[e.company_id]}));const U=document.getElementById("reenrich-btn");U&&U.addEventListener("click",()=>gt("enrich",{company_ids:[e.company_id]}));const B=document.getElementById("company-delete-btn");B&&B.addEventListener("click",()=>ks(e))}function aa(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function oa(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){r(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");r(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=i.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,F()),i.openId===e&&(Ee(s),$e(e)),r("reviewed")}async function xn(e){const t=i.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){r(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");r(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,F()),i.openId===e&&(Ee(a),$e(e)),E(),r(a.flagged?"flagged":"unflagged")}async function ia(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){r("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(d){r(`save failed: ${d.message}`),a.disabled=!1;return}if(!o.ok){const d=await o.text().catch(()=>"");r(`save failed: HTTP ${o.status}${d?" — "+d:""}`),a.disabled=!1;return}const l=await o.json();Ee(l),$e(l.company_id),q(),C(),E(),r("verdict saved")}function ut(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(c).join(" · "),a=n.application_status||"",o=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?je(a)||"pill-stage":"pill-none"}">${c(a||"—")}</span>`,`<span class="pt-meta">${a?"tracked":"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${c(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function he(e,t){const n=i.openDetail;if(!n||!i.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=ut(n),pt())}function pt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||st(e.dataset.pid)})})}async function ca(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){r("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){r(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");r(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const l=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==l.id),e.postings.unshift(l);const d=document.getElementById("postings-list");d&&(d.innerHTML=ut(e),pt()),t.value="",n.value="",s.disabled=!1,E(),r("link added")}async function $e(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){qe(`<div class="muted">Failed to load trail: ${c(s.message)}</div>`);return}if(!t.ok){qe(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){qe('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}qe(n.map(la).join(""))}function la(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(c);return e.run_id&&t.push("run "+c(e.run_id.slice(0,8))),`
...
`)):document.getElementById("editor-text").value=l.content||"",a&&(document.getElementById("editor-enabled").checked=l.enabled!==!1),l.taste_version&&(document.getElementById("editor-ver").textContent="version "+l.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function De(){document.getElementById("editor-scrim").classList.remove("open"),H=null}let T=null,bt=[],wt=[];const ga=["location.allowed","verticals.excluded","verticals.allowed"],kt={"verticals.excluded":"pf-vertical-tags","verticals.allowed":"pf-vertical-tags"};function W(e){const[t,n]=e.split(".");return T[t]&&T[t][n]||[]}function Ue(e,t){const[n,s]=e.split(".");(T[n]=T[n]||{})[s]=t}function Ve(e,t){const n=t.toLowerCase();return e.some(s=>String(s).toLowerCase()===n)}function Et(){ga.forEach(e=>{const t=document.querySelector(`.pf-chips[data-field="${e}"]`);if(!t)return;const n=kt[e]?` list="${kt[e]}"`:"",s=kt[e]?"type to search…":"type &amp; Enter…";t.innerHTML=W(e).map((a,o)=>`<span class="pf-chip">${c(a)}<button class="pf-chip-x" data-field="${e}" data-i="${o}" title="remove" aria-label="remove ${c(a)}">×</button></span>`).join("")+`<input class="pf-chip-input" data-field="${e}"${n} type="text" placeholder="${s}" spellcheck="false" autocomplete="off" />`})}function ya(e,t){var s;const n=(t||"").trim();if(n){const a=W(e);Ve(a,n)||Ue(e,[...a,n])}Et(),(s=document.querySelector(`.pf-chip-input[data-field="${e}"]`))==null||s.focus()}function Pn(e,t){const n=W(e).slice();n.splice(t,1),Ue(e,n),Et()}function qn(){const e=document.getElementById("pf-stages");if(!e)return;const t=W("funding_stage.allowed");e.innerHTML=wt.map(n=>{const s=Ve(t,n.value),a=n.count?` <span class="pf-stage-n">${n.count}</span>`:"";return`<button class="pf-stage${s?" is-on":""}" data-stage="${c(n.value)}">${c(n.value)}${a}</button>`}).join("")}function va(e){const t=W("funding_stage.allowed");Ue("funding_stage.allowed",Ve(t,e)?t.filter(n=>String(n).toLowerCase()!==e.toLowerCase()):[...t,e]),qn()}function ba(){const e=document.getElementById("pf-vertical-tags");e&&(e.innerHTML=bt.map(t=>`<option value="${c(t.value)}" label="${t.count}"></option>`).join(""))}function wa(){return{location:{allowed:[],remote_ok:!0},headcount:{min:0,max:0},verticals:{allowed:[],excluded:[]},funding_stage:{allowed:[]}}}function ka(){return`<div class="set-field pf-inline">
  ...
  </div>`}async function Hn(e=!1){if(document.getElementById("pf-enabled"))try{const[t,n]=await Promise.all([(await fetch("/api/taste-filter"+(e?"?default=1":""))).json(),bt.length||wt.length?Promise.resolve(null):fetch("/api/filter-options").then(s=>s.json()).catch(()=>null)]);n&&(bt=n.verticals||[],wt=n.stages||[]),T=Object.assign(wa(),t.rules||{}),T.location=Object.assign({allowed:[],remote_ok:!0},T.location),T.headcount=Object.assign({min:0,max:0},T.headcount),T.verticals=Object.assign({allowed:[],excluded:[]},T.verticals),T.funding_stage=Object.assign({allowed:[]},T.funding_stage),e||(document.getElementById("pf-enabled").checked=t.enabled!==!1),document.getElementById("pf-remote-ok").checked=!!T.location.remote_ok,document.getElementById("pf-hc-min").value=String(T.headcount.min||0),document.getElementById("pf-hc-max").value=String(T.headcount.max||0),ba(),Et(),qn()}catch(t){r(`failed to load pre-filter: ${t.message}`)}}async function Ea(){if(!T)return;T.location.remote_ok=document.getElementById("pf-remote-ok").checked,T.headcount.min=Math.max(0,parseInt(document.getElementById("pf-hc-min").value,10)||0),T.headcount.max=Math.max(0,parseInt(document.getElementById("pf-hc-max").value,10)||0),document.querySelectorAll(".pf-chip-input").forEach(n=>{const s=n.value.trim();s&&!Ve(W(n.dataset.field),s)&&Ue(n.dataset.field,[...W(n.dataset.field),s])});const e=document.getElementById("pf-enabled").checked;let t;try{t=await fetch("/api/taste-filter",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({rules:T,enabled:e})})}catch(n){r(`save failed: ${n.message}`);return}if(!t.ok){r(`save failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`);return}r("pre-filter saved"),C()}const $a=[{key:"experience",hard:!0},{key:"voice",hard:!1},{key:"logistics",hard:!1}];async function On(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{xa(await(await fetch("/api/outreach/sources")).json())}catch(e){r(`failed to load sources: ${e.message}`)}}function $t(){document.getElementById("sources-scrim").classList.remove("open")}function xa(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):$a,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],l=o.length?o.map(d=>`<li><span class="src-title">${c(d.title||d.page_id)}</span></li>`).join(""):`<li class="dim small">${a.hard?"none yet — add an experience page to your brain":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${c(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${l}</ul></div>`}).join("")}async function Ia(){if(!H)return;const e=document.getElementById("editor-text").value;let t;vt(H)?t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)}:(t={content:e},H.startsWith("outreach-prompts/")&&H!=="outreach-prompts/fill"&&(t.enabled=document.getElementById("editor-enabled").checked));let n;try{n=await fetch(`/api/${H}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){r(`save failed: ${o.message}`);return}if(!n.ok){r(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const s=await n.json();s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version);const a=vt(H);H==="followup-template"&&(i.followupTemplate=e),r(`${Re(H)} saved`),De(),a&&A(),C()}async function Sa(){if(!H||!H.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${H}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){r(`reset failed: ${n.message}`);return}if(!e.ok){r(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",r(`${Re(H)} reset to default`)}function Nn(e,t,n){e.k!==t?(e.k=t,e.dir=1):e.dir===1?e.dir=-1:Object.assign(e,n)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{Nn(i.sort,e.dataset.k,w),F()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{Nn(i.jsort,e.dataset.jk,x),j()}}),document.getElementById("tab-companies").onclick=()=>L("companies"),document.getElementById("tab-jobs").onclick=()=>L("jobs"),document.getElementById("q").oninput=F,document.getElementById("fdrop-cfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.dataset.toggle==="flagged")se=!se,X(t,se);else if(t.dataset.toggle==="enriched")ae=!ae,X(t,ae);else if(t.hasAttribute("data-v")){const n=t.getAttribute("data-v");R.has(n)?R.delete(n):R.add(n),X(t,R.has(n))}else return;F()}}),document.getElementById("fdrop-columns-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item[data-col]");if(!t)return;const n=Ze(),s=t.getAttribute("data-col");n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),X(t,!n.hidden.has(s)),oe(),Dt()}),document.getElementById("jq").oninput=j;for(const e of["fdrop-cfilters","fdrop-columns","fdrop-jfilters"]){const t=document.getElementById(e);t.querySelector(".fdrop-btn").addEventListener("click",s=>{s.stopPropagation();const a=t.classList.contains("is-open");nt(),a||cs(t)}),t.querySelector(".fdrop-menu").addEventListener("click",s=>s.stopPropagation())}document.addEventListener("click",nt),document.getElementById("fdrop-jfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-all");if(t){if(t.getAttribute("data-all")==="stage"){const s=["",...i.applicationStages];$=s.every(a=>$.has(a))?new Set:new Set(s)}else{const s=["",...i.outreachStatuses];_=_&&s.every(a=>_.has(a))?new Set:new Set(s)}et(),j();return}const n=e.target.closest(".fdrop-item");if(n){if(n.hasAttribute("data-stage")){const s=n.getAttribute("data-stage");$.has(s)?$.delete(s):$.add(s),X(n,$.has(s))}else if(n.dataset.toggle==="nextup")ie=!ie,X(n,ie);else if(n.hasAttribute("data-status")){const s=n.getAttribute("data-status");_.has(s)?_.delete(s):_.add(s),X(n,_.has(s))}else return;is(),j()}}),Qn(),Rt(),oe(),document.getElementById("pane-close").onclick=ke,document.getElementById("scrim").onclick=ke,document.getElementById("pursuit-close").onclick=ye,document.getElementById("pursuit-scrim").onclick=ye,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.querySelector(".fdrop.is-open")){nt();return}if(document.getElementById("chat-pane").classList.contains("open")){At();return}if(document.getElementById("profile-scrim").classList.contains("open")){Ct();return}if(document.getElementById("add-scrim").classList.contains("open")){Ne();return}if(document.getElementById("run-scrim").classList.contains("open")){Fe();return}if(document.getElementById("help-scrim").classList.contains("open")){Je();return}if(document.getElementById("relink-scrim").classList.contains("open")){re();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){Me();return}if(document.getElementById("deljob-scrim").classList.contains("open")){Ae();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(at==="pursuit"&&n){ye();return}if(at==="company"&&t){ke();return}if(t){ke();return}ye();return}if(document.getElementById("key-scrim").classList.contains("open")){Bt();return}if(document.getElementById("sources-scrim").classList.contains("open")){$t();return}if(document.getElementById("editor-scrim").classList.contains("open")){De();return}if(document.getElementById("gmail-config-scrim").classList.contains("open")){_e();return}});let xt=null;const _a={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Rn(e){if(i.meta&&i.meta.control===!1){r("control surface disabled");return}xt=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=_a[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Fe(){document.getElementById("run-scrim").classList.remove("open"),xt=null}document.getElementById("btn-enrich").onclick=()=>Rn("enrich"),document.getElementById("btn-verdict").onclick=()=>Rn("verdict"),document.getElementById("run-cancel").onclick=Fe,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Fe()},document.getElementById("run-go").onclick=()=>{const e=xt,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(Fe(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),gt(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=pa;const Ta={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Dn(e){const t=Ta[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const l=document.createElement("div");l.className="help-item-desc",l.textContent=s.desc;const d=document.createElement("a");d.className="help-link",d.textContent="Learn more →",d.onclick=()=>{Je(),L("docs"),za(s.sec)},a.appendChild(o),a.appendChild(l),a.appendChild(d),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Je(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Dn("add"),document.getElementById("help-run").onclick=()=>Dn("run"),document.getElementById("help-close").onclick=Je,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Je()},document.getElementById("add-cancel").onclick=Ne,document.getElementById("add-save").onclick=Mn,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Ne()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Tn(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",Ln),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Mn()))}),document.getElementById("add-vertical-filter").addEventListener("input",Bn),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&da(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=ma,document.getElementById("drawer-close").onclick=Sn,(()=>{const e=document.getElementById("drawer");e.addEventListener("mouseenter",He),e.addEventListener("mouseleave",()=>{!Ie&&e.classList.contains("open")&&_n()})})(),document.getElementById("editor-cancel").onclick=De,document.getElementById("editor-save").onclick=Ia,document.getElementById("editor-reset").onclick=Sa,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&De()},document.getElementById("sources-close").onclick=$t,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&$t()},document.addEventListener("click",e=>{var a;const t=e.target.closest(".pf-stage");if(t){va(t.dataset.stage);return}const n=e.target.closest(".pf-chip-x");if(n){Pn(n.dataset.field,parseInt(n.dataset.i,10));return}const s=e.target.closest(".pf-chips");s&&e.target===s&&((a=s.querySelector(".pf-chip-input"))==null||a.focus())}),document.addEventListener("keydown",e=>{const t=e.target.closest(".pf-chip-input");if(t){if(e.key==="Enter"||e.key===",")e.preventDefault(),ya(t.dataset.field,t.value);else if(e.key==="Backspace"&&!t.value){const n=W(t.dataset.field);n.length&&Pn(t.dataset.field,n.length-1)}}}),document.getElementById("key-cancel").onclick=Bt,document.getElementById("key-save").onclick=Jn,document.getElementById("key-remove").onclick=Va,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&Bt()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),Jn())}),document.getElementById("delcompany-cancel").onclick=Me,document.getElementById("delcompany-confirm").onclick=Es,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&Me()},document.getElementById("deljob-cancel").onclick=Ae,document.getElementById("deljob-confirm").onclick=xs,document.getElementById("deljob-scrim").onclick=e=>{e.target.id==="deljob-scrim"&&Ae()},document.getElementById("relink-cancel").onclick=re,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&re()},document.getElementById("relink-search").addEventListener("input",e=>{cn(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&ln(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&ln(t.dataset.id)});async function It(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}J()}const St='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',Un='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4.2 7a3.8 3.8 0 0 1 7.6 0c0 3 1.2 4 1.2 4H3s1.2-1 1.2-4z"/><path d="M6.7 13a1.5 1.5 0 0 0 2.6 0"/></svg>',La=[["outreach","Outreach"],["pipeline","Outreach pipeline"],["tracking","Tracking"],["job-hunting","Job hunting"],["integrations","Integrations"]],Ba=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]];function J(){const e=document.getElementById("criteria-stats");if(!e)return;const t=document.getElementById("settings-view");if(t&&t.style.display==="none")return;const n=i.settingsGroup||"outreach";e.innerHTML=`<div class="settings-shell">
...
`):a.content||""}catch{s.value=""}s.dataset.orig=s.value,s.dataset.loaded="1",s.addEventListener("blur",()=>ja(e))}async function ja(e){const t=e.dataset.kind,n=e.dataset.list==="1",s=e.querySelector(".set-textarea");if(s.dataset.loaded!=="1"||s.value===s.dataset.orig)return;const a=n?{statuses:s.value.split(/\r?\n/).map(l=>l.trim()).filter(Boolean)}:{content:s.value};let o;try{o=await fetch(`/api/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}catch(l){r(`save failed: ${l.message}`);return}if(!o.ok){r(`save failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}s.dataset.orig=s.value,_t(e),t==="followup-template"&&(i.followupTemplate=s.value),n&&(await A(),j())}function Tt(e){e.querySelectorAll(".set-field[data-kind]").forEach(Ca)}function Ma(e){e.innerHTML=Y("outreach-subject","Email subject","The send subject — {{role}} / {{company}} substitution, no LLM.",2,!1)+Y("outreach-template","Email body","Verbatim prose with the writer's fill-in holes.",16,!1)+Y("outreach-signature","Email signature","A fixed sign-off appended to every sent email (blank = none).",3,!1)+Y("followup-template","Follow-up template","Copy-paste follow-up — {{contact_name}}, {{role}}, {{company}}, {{last_sent}}, {{last_message}}.",9,!1)+`<div class="set-field">
    ...
    </div>`,Tt(e);const t=e.querySelector(".set-fu-interval");t&&t.addEventListener("change",async()=>{const n=Math.max(0,Math.min(90,parseInt(t.value,10)||0));t.value=String(n),await te("PUT","/api/followup-interval",{days:n})&&(i.followupInterval=n,r("follow-up interval saved"))})}function Aa(e){e.innerHTML=Ba.map(([t,n,s])=>`
    ...
    </div>`:o=Y("taste","Taste (local fallback)","Local fallback criteria used when the brain is unreachable.",12,!1),e.innerHTML=o+Y("playbook","Playbook","How scout judges — the reasoning rules behind every verdict.",12,!1)+ka(),Tt(e);const l=e.querySelector("#brief-refresh");l&&l.addEventListener("click",Fa),Hn();const d=e.querySelector("#pf-save");d&&d.addEventListener("click",Ea);const u=e.querySelector("#pf-reset");u&&u.addEventListener("click",()=>Hn(!0))}function Oa(e){const t=i.anthropicKey||{};let n="Not set — verdict, capture & outreach disabled.";t.key_source==="db"?n="Set here · active.":t.key_source==="env"&&(n="Using the ANTHROPIC_API_KEY environment variable.");const s=i.gmail||{},a=!!s.connected,o=!!s.configured;let l;a?l=`Connected as ${s.email||"(unknown)"}.`:o?l="Configured — not connected. Click Connect.":l="Not set up — paste your Google OAuth client below, then Connect.",e.innerHTML=`
    <div class="set-field">
    ...
    </div>`;const d=e.querySelector("#set-ak-save");d&&d.addEventListener("click",async()=>{const b=e.querySelector("#set-ak-input").value.trim();if(!b){r("paste a key first");return}const S=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:b})});if(!S.ok){r((await S.text().catch(()=>"")).trim()||`HTTP ${S.status}`);return}r("Anthropic key saved"),await xe(),await Lt()});const u=e.querySelector("#set-ak-remove");u&&u.addEventListener("click",async()=>{const b=await fetch("/api/integrations/anthropic",{method:"DELETE"});if(!b.ok){r(`HTTP ${b.status}`);return}r("Anthropic key removed"),await xe(),await Lt()});const p=e.querySelector("#set-gm-save");p&&p.addEventListener("click",async()=>{const b=e.querySelector("#set-gm-id").value.trim(),S=e.querySelector("#set-gm-secret").value,O=e.querySelector("#set-gm-redirect").value.trim();if(!b){r("client ID is required");return}const P=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:b,client_secret:S,redirect_uri:O})});if(!P.ok){r((await P.text().catch(()=>"")).trim()||`HTTP ${P.status}`);return}r("Gmail OAuth client saved"),await Se()});const m=e.querySelector("#set-gm-connect");m&&m.addEventListener("click",Na);const f=e.querySelector("#set-gm-disconnect");f&&f.addEventListener("click",Ra);const g=e.querySelector("#set-autoflip");g&&g.addEventListener("change",async()=>{let b=!1;try{b=(await fetch("/api/gmail/autoflip",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:g.checked})})).ok}catch{b=!1}b?(i.gmail&&(i.gmail.autoflip=g.checked),r(`auto-update ${g.checked?"on":"off"}`)):(g.checked=!g.checked,r("failed to save"))})}async function Lt(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}J()}async function Se(){try{i.gmail=await(await fetch("/api/gmail/status")).json()}catch{i.gmail=null}J()}async function Na(){let e;try{e=await fetch("/api/gmail/connect")}catch(n){r(`connect failed: ${n.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}let t={};try{t=await e.json()}catch{}t.auth_url?window.location.href=t.auth_url:r("could not start the Gmail connect flow")}async function Ra(){if(!confirm("Disconnect Gmail? Sending and sync stop; already-synced data stays."))return;let e;try{e=await fetch("/api/gmail/disconnect",{method:"DELETE"})}catch(t){r(`disconnect failed: ${t.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}r("Gmail disconnected"),await Se()}function _e(){document.getElementById("gmail-config-scrim").classList.remove("open")}async function Da(){const e=document.getElementById("gmail-client-id").value.trim(),t=document.getElementById("gmail-client-secret").value,n=document.getElementById("gmail-redirect").value.trim();if(!e){r("client ID is required");return}let s;try{s=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:e,client_secret:t,redirect_uri:n})})}catch(a){r(`save failed: ${a.message}`);return}if(!s.ok){r((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`);return}r("Gmail OAuth client saved — click Connect"),_e(),await Se()}async function Ua(){if(!confirm("Remove the stored Google OAuth client? Connecting needs it re-entered (or set via env)."))return;let e;try{e=await fetch("/api/gmail/config",{method:"DELETE"})}catch(t){r(`remove failed: ${t.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}r("OAuth client removed"),_e(),await Se()}function Fn(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function Bt(){document.getElementById("key-scrim").classList.remove("open")}async function Jn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){r("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){r(`save failed: ${a.message}`),n();return}if(!s.ok){r((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),r("Anthropic key saved"),await xe(),Fn(),J()}async function Va(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){r(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){r((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),r(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await xe(),Fn(),J()}async function Fa(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){r(`refresh failed: ${n.message}`),It();return}if(!t.ok){const n=await t.text().catch(()=>"");r(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),It();return}i.profile=await t.json(),J(),r("company-fit brief refreshed"),C()}function Ct(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Ct,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Ct()};function Ja(){const e=document.querySelector("#docs-nav a");ze(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function ze(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function za(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),ze(e)}document.getElementById("open-docs").onclick=()=>L("docs");function Ga(){L("settings")}document.getElementById("open-settings").onclick=Ga,document.getElementById("gmail-config-cancel").onclick=_e,document.getElementById("gmail-config-save").onclick=Da,document.getElementById("gmail-config-remove").onclick=Ua,document.getElementById("gmail-config-scrim").onclick=e=>{e.target.id==="gmail-config-scrim"&&_e()};function Ka(){const e=document.getElementById("notif-badge");if(!e)return;const t=(i.notifications&&i.notifications.unread)|0;t>0?(e.textContent=t>99?"99+":String(t),e.style.display=""):e.style.display="none"}async function ne(){try{i.notifications=await(await fetch("/api/notifications")).json()}catch{return}Ka(),i.view==="inbox"&&zn()}function Wa(){return'<option value="">link to role…</option>'+(i.jobs||[]).map(t=>`<option value="${c(t.posting_id)}">${c((t.company||"")+" — "+(t.title||"(untitled)"))}</option>`).join("")}function Ya(e){const t=e.company||e.role?`<div class="notif-ctx">${c([e.company,e.role].filter(Boolean).join(" · "))}</div>`:'<div class="notif-ctx dim">not linked to a role</div>',n=e.created_at?`<span class="notif-when">${c((e.created_at||"").replace("T"," ").slice(0,16))}</span>`:"",s=e.kind==="app_status"&&e.suggested_status&&!e.actioned&&e.posting_id?`<button class="btn btn-primary notif-apply" data-id="${e.id}">Apply: ${c(e.suggested_status)}</button>`:"",a=e.posting_id?"":`<select class="input notif-link" data-id="${e.id}" title="link this to a role">${Wa()}</select>`;return`<div class="notif-item${e.seen?"":" is-unread"}" data-id="${e.id}" data-seen="${e.seen?1:0}">
...
`))+"</code></pre>");continue}const d=l.match(/^(#{1,6})\s+(.*)$/);if(d){a();const f=d[1].length;n.push("<h"+f+">"+Ge(c(d[2]))+"</h"+f+">"),o++;continue}const u=l.match(/^\s*[-*]\s+(.*)$/);if(u){s!=="ul"&&(a(),n.push("<ul>"),s="ul"),n.push("<li>"+Ge(c(u[1]))+"</li>"),o++;continue}const p=l.match(/^\s*\d+\.\s+(.*)$/);if(p){s!=="ol"&&(a(),n.push("<ol>"),s="ol"),n.push("<li>"+Ge(c(p[1]))+"</li>"),o++;continue}if(l.trim()===""){a(),o++;continue}a();const m=[];for(;o<t.length&&t[o].trim()!==""&&!/^```|^#{1,6}\s|^\s*[-*]\s+|^\s*\d+\.\s+/.test(t[o]);)m.push(Ge(c(t[o]))),o++;n.push("<p>"+m.join("<br>")+"</p>")}return a(),n.join("")}function Ke(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,e==="assistant"?n.innerHTML=oo(t||""):n.textContent=t||"",n}function jt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function io(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Gn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=so(n.content);if(n.role==="user")s&&t.appendChild(Ke("user",s));else if(n.role==="assistant"){const a=ao(n.content);if(!s&&!a.length)continue;const o=Ke("assistant",s);if(a.length){const l=document.createElement("div");l.className="chat-tools",l.textContent="· used "+a.join(", "),o.appendChild(l)}t.appendChild(o)}}t.children.length||t.appendChild(io()),jt()}async function Mt(e,t,n){if(!i.meta||!i.meta.chat){r("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),l=await fetch("/api/chat/threads?"+o);if(!l.ok)throw new Error((await l.text().catch(()=>"")).trim()||"HTTP "+l.status);const d=await l.json();i.chat.threadId=d.thread.id,Gn(d.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+c(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function At(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function We(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Kn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Wn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",Kn(),We(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Ke("user",t));const a=Ke("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),jt();let o="";const l=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,We(!1)},d=i.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){l(m.message);return}if(!u.ok){l((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");i.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,a.textContent=o,jt()}),p.addEventListener("end",async m=>{p.close(),i.chat.es===p&&(i.chat.es=null),a.classList.remove("chat-streaming"),We(!1),i.chat.threadId===d&&await co(),lo(),typeof m.data=="string"&&m.data.indexOf("error")===0&&r("chat: "+m.data)}),p.onerror=()=>{p.close(),i.chat.es===p&&(i.chat.es=null),a.classList.remove("chat-streaming"),We(!1)}}async function co(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Gn(a.messages||[])}catch{}}function lo(){q(),E(),C(),i.openId&&me(i.openId)}document.getElementById("open-chat").onclick=()=>Mt("global","",""),document.getElementById("chat-close").onclick=At,document.getElementById("chat-scrim").onclick=At,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Wn()}),document.getElementById("chat-input").addEventListener("input",Kn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Wn())}),Ft("#t tbody",ts),Ft("#jt tbody",ns);const ro=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();L(ro==="jobs"?"jobs":"companies",{render:!1});function Yn(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||Yn()}),window.addEventListener("pageshow",e=>{e.persisted&&Yn()}),q(),E(),C(),xe(),ft(),It(),Lt(),Se(),ne(),setInterval(ne,9e4),A(),function(){const t=/[?&]gmail=(connected|error)/.exec(location.search);t&&(r(t[1]==="connected"?"Gmail connected":"Gmail connection failed"),history.replaceState(null,"",location.pathname+location.hash))}()}ho({"":{view:()=>({mount(k){k.innerHTML=bo,wo()}}),chrome:!1}},{title:"scout"});yo();vo();
