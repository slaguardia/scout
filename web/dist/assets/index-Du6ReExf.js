(function(){const v=document.createElement("link").relList;if(v&&v.supports&&v.supports("modulepreload"))return;for(const g of document.querySelectorAll('link[rel="modulepreload"]'))c(g);new MutationObserver(g=>{for(const $ of g)if($.type==="childList")for(const G of $.addedNodes)G.tagName==="LINK"&&G.rel==="modulepreload"&&c(G)}).observe(document,{childList:!0,subtree:!0});function E(g){const $={};return g.integrity&&($.integrity=g.integrity),g.referrerPolicy&&($.referrerPolicy=g.referrerPolicy),g.crossOrigin==="use-credentials"?$.credentials="include":g.crossOrigin==="anonymous"?$.credentials="omit":$.credentials="same-origin",$}function c(g){if(g.ep)return;g.ep=!0;const $=E(g);fetch(g.href,$)}})();function To(b,v){const E=v.replace(/^#/,"");let c=null;for(const g of Object.keys(b))(E===g||E.startsWith(g))&&(c===null||g.length>c.length)&&(c=g);return c===null&&""in b&&(c=""),c}function Bo(b){return typeof b=="function"?{view:b,chrome:!0}:{view:b.view,chrome:b.chrome!==!1}}function Lo(b,v={}){const E=v.root??document.body,c=v.title??document.title??"",g=v.brandHref??"#",$=document.createElement("main"),G=document.createElement("header");G.className="cap-head";const ae=document.createElement("a");ae.className="brand",ae.href=g,ae.textContent=c,ae.setAttribute("aria-label",`${c} — home`),G.appendChild(ae);const l=document.createElement("nav");l.className="cap-nav",l.setAttribute("aria-label","Views");for(const H of v.nav??[]){const U=document.createElement("a");U.href=H.href,U.textContent=H.label,H.ariaLabel&&U.setAttribute("aria-label",H.ariaLabel),l.appendChild(U)}G.appendChild(l);const q=document.createElement("section");q.className="tk-content",$.appendChild(G),$.appendChild(q);const C=document.createElement("div");C.className="tk-bleed";const x=H=>{var U;for(const V of Array.from(l.querySelectorAll("a"))){const A=((U=V.getAttribute("href"))==null?void 0:U.replace(/^#/,""))??"";V.toggleAttribute("aria-current",H!==null&&H!==""&&A===H),V.hasAttribute("aria-current")&&V.setAttribute("aria-current","page")}};let $e=0;const Q=()=>{const H=To(b,location.hash);if(x(H),H===null){C.isConnected&&C.remove(),$.isConnected||E.appendChild($),Co(q,"Not found.");return}const{view:U,chrome:V}=Bo(b[H]),A=V?q:C;V?(C.isConnected&&C.remove(),$.isConnected||E.appendChild($)):($.isConnected&&$.remove(),C.isConnected||E.appendChild(C)),A.replaceChildren();const F=U(),it=++$e,He=F.mount(A);He instanceof Promise&&He.catch(Oe=>{it===$e&&jo(A,String(Oe))})};window.addEventListener("hashchange",Q),Q()}function Co(b,v){b.replaceChildren();const E=document.createElement("div");E.className="tk-empty",E.textContent=v,b.appendChild(E)}function jo(b,v){b.replaceChildren();const E=document.createElement("div");E.className="tk-error",E.textContent=v,b.appendChild(E)}function Mo(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(v=>{for(const E of v)E.unregister()}),window.caches&&caches.keys().then(v=>{for(const E of v)caches.delete(E)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function Ao(){let b;try{b=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!b.ok)return null;let v;try{v=await b.json()}catch{return null}return typeof v.email=="string"&&v.email?{email:v.email}:null}const Po=`
<div class="layout">
<aside class="sidebar">
...
`))+"</code></pre>");continue}const d=i.match(/^(#{1,6})\s+(.*)$/);if(d){a();const h=d[1].length;n.push("<h"+h+">"+st(l(d[2]))+"</h"+h+">"),o++;continue}const u=i.match(/^\s*[-*]\s+(.*)$/);if(u){s!=="ul"&&(a(),n.push("<ul>"),s="ul"),n.push("<li>"+st(l(u[1]))+"</li>"),o++;continue}const p=i.match(/^\s*\d+\.\s+(.*)$/);if(p){s!=="ol"&&(a(),n.push("<ol>"),s="ol"),n.push("<li>"+st(l(p[1]))+"</li>"),o++;continue}if(i.trim()===""){a(),o++;continue}a();const m=[];for(;o<t.length&&t[o].trim()!==""&&!/^```|^#{1,6}\s|^\s*[-*]\s+|^\s*\d+\.\s+/.test(t[o]);)m.push(st(l(t[o]))),o++;n.push("<p>"+m.join("<br>")+"</p>")}return a(),n.join("")}function at(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,e==="assistant"?n.innerHTML=Eo(t||""):n.textContent=t||"",n}function Dt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function $o(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=c.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(c.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function as(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=wo(n.content);if(n.role==="user")s&&t.appendChild(at("user",s));else if(n.role==="assistant"){const a=ko(n.content);if(!s&&!a.length)continue;const o=at("assistant",s);if(a.length){const i=document.createElement("div");i.className="chat-tools",i.textContent="· used "+a.join(", "),o.appendChild(i)}t.appendChild(o)}}t.children.length||t.appendChild($o()),Dt()}async function Ut(e,t,n){if(!c.meta||!c.meta.chat){r("chat needs ANTHROPIC_API_KEY in the server env");return}c.chat.es&&(c.chat.es.close(),c.chat.es=null),c.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),i=await fetch("/api/chat/threads?"+o);if(!i.ok)throw new Error((await i.text().catch(()=>"")).trim()||"HTTP "+i.status);const d=await i.json();c.chat.threadId=d.thread.id,as(d.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+l(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Vt(){c.chat.es&&(c.chat.es.close(),c.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function ot(e){c.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function os(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function is(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||c.chat.streaming||!c.chat.threadId)return;e.value="",os(),ot(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(at("user",t));const a=at("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),Dt();let o="";const i=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,ot(!1)},d=c.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){i(m.message);return}if(!u.ok){i((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");c.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,a.textContent=o,Dt()}),p.addEventListener("end",async m=>{p.close(),c.chat.es===p&&(c.chat.es=null),a.classList.remove("chat-streaming"),ot(!1),c.chat.threadId===d&&await Io(),xo(),typeof m.data=="string"&&m.data.indexOf("error")===0&&r("chat: "+m.data)}),p.onerror=()=>{p.close(),c.chat.es===p&&(c.chat.es=null),a.classList.remove("chat-streaming"),ot(!1)}}async function Io(){const e=c.chat.scope,t=c.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();as(a.messages||[])}catch{}}function xo(){C(),x(),F(),c.openId&&be(c.openId)}document.getElementById("open-chat").onclick=()=>Ut("global","",""),document.getElementById("chat-close").onclick=Vt,document.getElementById("chat-scrim").onclick=Vt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),is()}),document.getElementById("chat-input").addEventListener("input",os),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),is())}),en("#t tbody",ps),en("#jt tbody",ms);const So=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();A(So==="jobs"?"jobs":"companies",{render:!1});function cs(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||cs()}),window.addEventListener("pageshow",e=>{e.persisted&&cs()}),C(),x(),F(),je(),St(),At(),Ot(),Ee(),le(),setInterval(le,9e4),V(),function(){const t=/[?&]gmail=(connected|error)/.exec(location.search);t&&(r(t[1]==="connected"?"Gmail connected":"Gmail connection failed"),history.replaceState(null,"",location.pathname+location.hash))}()}Lo({"":{view:()=>({mount(b){b.innerHTML=Po,qo()}}),chrome:!1}},{title:"scout"});Mo();Ao();
