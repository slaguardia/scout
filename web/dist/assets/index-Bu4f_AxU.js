(function(){const b=document.createElement("link").relList;if(b&&b.supports&&b.supports("modulepreload"))return;for(const y of document.querySelectorAll('link[rel="modulepreload"]'))l(y);new MutationObserver(y=>{for(const I of y)if(I.type==="childList")for(const c of I.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&l(c)}).observe(document,{childList:!0,subtree:!0});function x(y){const I={};return y.integrity&&(I.integrity=y.integrity),y.referrerPolicy&&(I.referrerPolicy=y.referrerPolicy),y.crossOrigin==="use-credentials"?I.credentials="include":y.crossOrigin==="anonymous"?I.credentials="omit":I.credentials="same-origin",I}function l(y){if(y.ep)return;y.ep=!0;const I=x(y);fetch(y.href,I)}})();function wo(w,b){const x=b.replace(/^#/,"");let l=null;for(const y of Object.keys(w))(x===y||x.startsWith(y))&&(l===null||y.length>l.length)&&(l=y);return l===null&&""in w&&(l=""),l}function ko(w){return typeof w=="function"?{view:w,chrome:!0}:{view:w.view,chrome:w.chrome!==!1}}function Eo(w,b={}){const x=b.root??document.body,l=b.title??document.title??"",y=b.brandHref??"#",I=document.createElement("main"),c=document.createElement("header");c.className="cap-head";const A=document.createElement("a");A.className="brand",A.href=y,A.textContent=l,A.setAttribute("aria-label",`${l} — home`),c.appendChild(A);const q=document.createElement("nav");q.className="cap-nav",q.setAttribute("aria-label","Views");for(const P of b.nav??[]){const L=document.createElement("a");L.href=P.href,L.textContent=P.label,P.ariaLabel&&L.setAttribute("aria-label",P.ariaLabel),q.appendChild(L)}c.appendChild(q);const E=document.createElement("section");E.className="tk-content",I.appendChild(c),I.appendChild(E);const U=document.createElement("div");U.className="tk-bleed";const X=P=>{var L;for(const C of Array.from(q.querySelectorAll("a"))){const ee=((L=C.getAttribute("href"))==null?void 0:L.replace(/^#/,""))??"";C.toggleAttribute("aria-current",P!==null&&P!==""&&ee===P),C.hasAttribute("aria-current")&&C.setAttribute("aria-current","page")}};let ve=0;const Ce=()=>{const P=wo(w,location.hash);if(X(P),P===null){U.isConnected&&U.remove(),I.isConnected||x.appendChild(I),$o(E,"Not found.");return}const{view:L,chrome:C}=ko(w[P]),ee=C?E:U;C?(U.isConnected&&U.remove(),I.isConnected||x.appendChild(I)):(I.isConnected&&I.remove(),U.isConnected||x.appendChild(U)),ee.replaceChildren();const et=L(),je=++ve,be=et.mount(ee);be instanceof Promise&&be.catch(V=>{je===ve&&xo(ee,String(V))})};window.addEventListener("hashchange",Ce),Ce()}function $o(w,b){w.replaceChildren();const x=document.createElement("div");x.className="tk-empty",x.textContent=b,w.appendChild(x)}function xo(w,b){w.replaceChildren();const x=document.createElement("div");x.className="tk-error",x.textContent=b,w.appendChild(x)}function Io(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(b=>{for(const x of b)x.unregister()}),window.caches&&caches.keys().then(b=>{for(const x of b)caches.delete(x)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function So(){let w;try{w=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!w.ok)return null;let b;try{b=await w.json()}catch{return null}return typeof b.email=="string"&&b.email?{email:b.email}:null}const _o=`
<div class="layout">
<aside class="sidebar">
...
`;function To(w){const b={k:"verdict",dir:1},x={k:"created_at",dir:1},l={rows:[],sort:{...b},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...x},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null,gmail:null,notifications:{notifications:[],unread:0,followups:[]},settingsGroup:"outreach"},y=e=>"pill pill-"+(e||"none"),I='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',c=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),A=e=>/^https?:\/\//i.test(String(e??""))?c(e):"#";async function q(){const t=await(await fetch("/api/companies")).json();l.rows=t.rows||[],J()}async function E(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}l.jobs=e.rows||[],j(),U(),ve()}function U(){if(!h.postingId)return;const e=l.jobs.find(t=>t.posting_id===h.postingId);e&&(h.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&ne())}let X=null;function ve(){const e=l.jobs.some(t=>t.outreach_draft_status==="researching");e&&!X?X=setInterval(Ce,4e3):!e&&X&&(clearInterval(X),X=null)}async function Ce(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(l.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==l.jobs.length;l.jobs=t,s&&(j(),U()),ve()}async function P(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(l.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(l.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(l.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(l.followupTemplate=e.content)}).catch(()=>{})]),at(),l.view==="jobs"&&j()}function L(e,{render:t=!0}={}){l.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("tab-inbox").classList.toggle("active",e==="inbox"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none";const n=(i,d)=>{const u=document.getElementById(i);u&&(u.style.display=d?"":"none")};n("settings-view",e==="settings"),n("inbox-view",e==="inbox"),n("docs-view",e==="docs"),document.getElementById("open-settings").classList.toggle("is-active",e==="settings");const s=document.getElementById("open-docs");s&&s.classList.toggle("is-active",e==="docs");const a=e==="companies"||e==="jobs";document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none";const o=document.getElementById("block-columns");o&&(o.style.display=a?"":"none"),Ft(),t&&(e==="jobs"?j():e==="settings"?G():e==="inbox"?(Kn(),ae()):e==="docs"?Xa():J())}async function C(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}l.stats=e,ee()}function ee(){G()}function et(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function je(e){return e.slice().sort((t,n)=>l.sort.dir*et(t,n,l.sort.k))}function be(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(s=>{s.getAttribute(t)===n.k?s.dataset.sort=n.dir<0?"desc":"asc":delete s.dataset.sort})}const V=new Set;let oe=!1,ie=!1;const es=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function ts(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+es.map(([t,n,s])=>te("data-v",t,n,s,V.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+te("data-toggle","flagged","⚑ Flagged","",oe)+te("data-toggle","enriched","Enriched","",ie),Nt())}function Nt(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const i of l.rows){const d=i.verdict||"__none__";e[d]=(e[d]|0)+1,i.flagged&&t++,i.enriched&&n++}ot("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=V.size+(oe?1:0)+(ie?1:0);Xt("fdrop-cfilters-btn",o,o>0)}function Rt(){const e=document.getElementById("q").value.trim().toLowerCase();return l.rows.filter(t=>!(V.size&&!V.has(t.verdict||"__none__")||oe&&!t.flagged||ie&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const ns=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],ss=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Dt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Ut=Dt("scout-hidden-cols"),Vt=Dt("scout-hidden-jcols");function tt(){return l.view==="jobs"?{cols:ss,hidden:Vt,key:"scout-hidden-jcols"}:{cols:ns,hidden:Ut,key:"scout-hidden-cols"}}function ce(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Ut.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Vt.has(e.dataset.col)?"none":""})}function Ft(){const e=tt(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>te("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Jt())}function Jt(){const e=tt(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Gt(e){return`
    ...
    `}function zt(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>_n(t.dataset.id))}const as=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],os=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function Wt(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let i=0;i<n;i++){const d=t.map(([u,p])=>{const m=p.endsWith("%")?Math.round(parseFloat(p)*a[i%a.length])+"%":p;return`<td${u?` data-col="${u}"`:""}><span class="skel" style="width:${m}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${d}</tr>`}s.innerHTML=o,ce()}function J(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=je(Rt());Nt(),document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=Gt(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||fe(s.dataset.id)}),e.appendChild(s),zt(s)}be("t","data-k",l.sort),ce()}async function is(e){const n=await(await fetch("/api/companies")).json();l.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=je(Rt()).map(i=>i.company_id),o=[...s.querySelectorAll("tr")].map(i=>i.dataset.id);if(a.length!==o.length||a.some((i,d)=>i!==o[d])){J();return}for(const i of e){const d=l.rows.find(p=>p.company_id===i),u=s.querySelector(`tr[data-id="${CSS.escape(i)}"]`);if(!d||!u){J();return}u.innerHTML=Gt(d),zt(u)}ce()}let $=null,nt=null,le=!1,re=!1,_=null,st=null;function Kt(){const e=l.applicationStages;if($===null)$=new Set(["",...e]);else{for(const t of[...$])t!==""&&!e.includes(t)&&$.delete(t);if(nt)for(const t of e)nt.has(t)||$.add(t)}nt=new Set(e)}function Yt(){const e=l.outreachStatuses;if(_===null)_=new Set(["",...e]);else{for(const t of[..._])t!==""&&!e.includes(t)&&_.delete(t);if(st)for(const t of e)st.has(t)||_.add(t)}st=new Set(e)}function cs(){Kt(),Yt();const e=document.getElementById("jq").value.trim().toLowerCase();return l.jobs.filter(t=>{const n=t.application_status||"";return!(!$.has(n)||le&&!t.next_up||re&&!(t.followups_due|0)||!_.has(t.outreach_status||"")||e&&!(t.title+" "+t.company+" "+(t.location||"")+" "+(t.description||"")+" "+(t.contacts||"")).toLowerCase().includes(e))})}function te(e,t,n,s,a){return`<button class="fdrop-item${a?" is-checked":""}" ${e}="${c(t)}" role="menuitemcheckbox" aria-checked="${a}"><span class="fdrop-check" aria-hidden="true"></span>`+(s?`<span class="fdrop-dot ${s}"></span>`:"")+`<span class="fdrop-label">${c(n)}</span><span class="fdrop-item-count" data-count></span></button>`}function Zt(e,t,n){return`<div class="fdrop-head fdrop-head--toggle"><span>${e}</span><button type="button" class="fdrop-all" data-all="${t}">${n?"none":"all"}</button></div>`}function at(){Kt(),Yt();const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=["",...l.applicationStages],n=["",...l.outreachStatuses];e.innerHTML=Zt("Application stage","stage",t.every(s=>$.has(s)))+te("data-stage","","not applied","",$.has(""))+l.applicationStages.map(s=>te("data-stage",s,s,Ae(s),$.has(s))).join("")+'<div class="fdrop-sep"></div>'+Zt("Reply status","status",n.every(s=>_.has(s)))+[["","not reached out",""]].concat(l.outreachStatuses.map(s=>[s,s,on(s)])).map(([s,a,o])=>te("data-status",s,a,o,_.has(s))).join(""),Qt()}function Qt(){const e={},t={};for(const d of l.jobs){const u=d.application_status||"";e[u]=(e[u]|0)+1;const p=d.outreach_status||"";t[p]=(t[p]|0)+1}ot("#fdrop-jfilters-menu [data-stage]","data-stage",e),ot("#fdrop-jfilters-menu [data-status]","data-status",t);const n=["",...l.applicationStages],s=$&&$.size===n.length&&n.every(d=>$.has(d)),a=["",...l.outreachStatuses],o=_&&_.size===a.length&&a.every(d=>_.has(d)),i=(s?0:$?$.size:0)+(o?0:_?_.size:0);Xt("fdrop-jfilters-btn",i,i>0)}function ot(e,t,n){document.querySelectorAll(e).forEach(s=>{const a=s.querySelector("[data-count]");if(a){const o=n[s.getAttribute(t)]|0;a.textContent=o||""}})}function Xt(e,t,n){const s=document.getElementById(e);if(!s)return;s.classList.toggle("is-active",n);const a=s.querySelector(".fdrop-count");if(a){const o=n&&t>0;a.textContent=o?t:"",a.style.display=o?"":"none"}}function de(e,t){e.classList.toggle("is-checked",t),e.setAttribute("aria-checked",String(t))}function ls(){const e=document.getElementById("fdrop-jfilters-menu");if(!e)return;const t=e.querySelector('.fdrop-all[data-all="stage"]'),n=e.querySelector('.fdrop-all[data-all="status"]');t&&(t.textContent=["",...l.applicationStages].every(s=>$.has(s))?"none":"all"),n&&(n.textContent=["",...l.outreachStatuses].every(s=>_.has(s))?"none":"all")}function it(){document.querySelectorAll(".fdrop.is-open").forEach(e=>{e.classList.remove("is-open");const t=e.querySelector(".fdrop-btn");t&&t.setAttribute("aria-expanded","false")})}function en(e){const t=e.querySelector(".fdrop-btn"),n=e.querySelector(".fdrop-menu");if(!t||!n)return;const s=t.getBoundingClientRect();n.style.left=Math.round(s.left)+"px",n.style.top=Math.round(s.bottom+4)+"px",n.style.minWidth=Math.round(s.width)+"px",n.style.maxHeight=Math.max(160,Math.round(window.innerHeight-s.bottom-12))+"px"}function rs(e){const t=e.querySelector(".fdrop-btn");e.classList.add("is-open"),t&&t.setAttribute("aria-expanded","true"),en(e)}function tn(){const e=document.querySelector(".fdrop.is-open");e&&en(e)}window.addEventListener("scroll",tn,!0),window.addEventListener("resize",tn);const ds=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function us(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(ds),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}const Me=()=>new Date().toISOString().slice(0,10);function nn(e){const t=[["","none"]];for(const n of l.applicationStages)t.push([n,n]);return e&&!l.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function sn(e){const t=[["","none"]];for(const n of l.outreachStatuses)t.push([n,n]);return e&&!l.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const ps=8;function an(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%ps}function Ae(e){return an(e,l.applicationStages)}function on(e){return an(e,l.outreachStatuses)}function ms(e){const t=us(e);return t.length?t.map(n=>{const s=c(n.position||n.email);if(!n.email)return s;const a=c(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${c(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function cn(e){const t=e.application_status||"";if(!t)return l.applicationStages.length+1;const n=l.applicationStages.indexOf(t);return n<0?l.applicationStages.length:n}function hs(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return cn(e)-cn(t);if(n==="followups_due")return(t.followups_due|0)-(e.followups_due|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-l.jsort.dir:l.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function fs(){const e=document.getElementById("jobs-followup-nav");if(!e)return;let t=0,n=0;for(const a of l.jobs)a.next_up&&t++,n+=a.followups_due|0;if(t||(le=!1),n||(re=!1),!t&&!n){e.style.display="none",e.innerHTML="";return}e.style.display="";const s=[];t&&s.push(`<button class="queue-nav-btn queue-nav-btn--nextup${le?" is-active":""}" data-q="nextup" title="${le?"showing only these — click to show all jobs":"show only jobs queued next up for outreach"}"><span class="fn-icon">${Pa}</span><span class="fn-text"><strong>${t}</strong> next up</span></button>`),n&&s.push(`<button class="queue-nav-btn${re?" is-active":""}" data-q="due" title="${re?"showing only these — click to show all jobs":"show only jobs owing a follow-up"}"><span class="fn-icon">${Jn}</span><span class="fn-text"><strong>${n}</strong> follow-up${n>1?"s":""} due</span></button>`),e.innerHTML=s.join(""),e.querySelectorAll(".queue-nav-btn").forEach(a=>{a.onclick=()=>{a.dataset.q==="nextup"?le=!le:re=!re,j()}})}function j(){const e=document.querySelector("#jt tbody");e.innerHTML="",fs();const t=cs().sort((a,o)=>l.jsort.dir*hs(a,o,l.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",Qt();const n=$&&!$.has("rejected")?l.jobs.filter(a=>(a.application_status||"")==="rejected").length:0,s=document.getElementById("jobs-hidden-note");s.style.display=n?"":"none",n&&(s.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{$.add("rejected"),at(),j()});for(const a of t){const o=a.application_status||"",i=document.createElement("tr");i.dataset.id=a.posting_id;const d=nn(o).map(([m,f])=>`<option value="${c(m)}"${o===m?" selected":""}>${c(f)}</option>`).join(""),u=a.outreach_status||"",p=sn(u).map(([m,f])=>`<option value="${c(m)}"${u===m?" selected":""}>${c(f)}</option>`).join("");i.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${a.next_up?" is-on":""}" title="${a.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 12.5v-9M4.5 7L8 3.5 11.5 7"/></svg></button><div class="jt-namecol"><span class="row-name">${c(a.title||a.company)}</span>${gs(a.outreach_draft_status)}${a.title?`<div class="small dim">${c(a.company)}</div>`:""}</div></div></td>
      <td data-col="application"><div class="jt-stage"><select class="jt-stage-sel ${Ae(o)}" title="application stage">${d}</select>${o&&a.application_status_at?`<span class="jt-stage-at" title="stage last changed">${c(a.application_status_at.slice(0,10))}</span>`:""}</div></td>
      <td class="small" data-col="outreach"><div class="jt-out"><select class="jt-ostatus ${on(u)}" title="outreach reply status">${p}</select>${a.followups_due?`<span class="followup-badge" title="${a.followups_due} follow-up${a.followups_due>1?"s":""} due — open to act">${Jn}${a.followups_due}</span>`:""}</div></td>
      <td class="small" data-col="last_outreach">${a.last_outreach_at?c(a.last_outreach_at):'<span class="dim">—</span>'}</td>
      <td class="small td-contacts" data-col="contacts">${ms(a.contacts)}</td>
      <td data-col="link"><a href="${A(a.url)}" target="_blank" rel="noopener" title="open posting" aria-label="open posting">↗</a></td>
    `,i.querySelector(".jt-nextup").onclick=()=>hn(a,!1),i.querySelector(".jt-stage-sel").onchange=m=>yn(a,{application_status:m.target.value}),i.querySelector(".jt-ostatus").onchange=m=>yn(a,{outreach_status:m.target.value}),e.appendChild(i)}be("jt","data-jk",l.jsort),ce(),e.querySelectorAll("tr").forEach(a=>{a.addEventListener("click",o=>{o.target.closest("a, button, select")||ct(a.dataset.id)})})}function gs(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const h={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1,contacts:[],outreach:[],contactsLoaded:!1};async function ct(e){let t=l.jobs.find(n=>n.posting_id===e);if(t||(await E(),t=l.jobs.find(n=>n.posting_id===e)),!t){r("posting not found — refresh");return}rt(),ft(),h.postingId=e,h.row=t,h.drafts=[],h.openHist=!1,h.answers=[],h.detecting=!1,h.contacts=[],h.outreach=[],h.contactsLoaded=!1,h.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),ln("pursuit"),ne(),ue(),he(),we()}async function we(){const e=h.postingId,t=h.row&&h.row.company_id;if(!(!e||!t)){try{const[n,s]=await Promise.all([fetch(`/api/companies/${t}/contacts`).then(a=>a.ok?a.json():[]),fetch(`/api/postings/${e}/outreach-log`).then(a=>a.ok?a.json():[])]);if(h.postingId!==e)return;h.contacts=Array.isArray(n)?n:[],h.outreach=Array.isArray(s)?s:[]}catch{}h.contactsLoaded=!0,me()}}let lt=null;function ln(e){lt=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function ke(){rt(),ft(),h.postingId=null,h.row=null,h.drafts=[],h.answers=[],h.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function rt(){h.poll&&(clearInterval(h.poll),h.poll=null)}async function ue(){if(!h.postingId)return;let e;try{const n=await fetch(`/api/postings/${h.postingId}/outreach`);if(!n.ok){me();return}e=await n.json()}catch{me();return}h.drafts=e.drafts||[],me();const t=h.drafts[0];t&&t.status==="researching"?ys():rt()}function ys(){h.poll||(h.poll=setInterval(ue,4e3))}function F(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),r(`save failed: ${o.message}`)}})}async function rn(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),j(),ge(e.posting_id,{title:o.title,location:o.location})}async function vs(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",A(e.url)),ge(e.posting_id,{url:s.url})}async function bs(e,t){if(t.disabled)return;const n=t.textContent;t.disabled=!0,t.textContent="re-enriching…";let s;try{s=await fetch(`/api/postings/${e.posting_id}/recapture`,{method:"POST"})}catch(o){t.disabled=!1,t.textContent=n,r(`re-enrich failed: ${o.message}`);return}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();let i=o||"HTTP "+s.status;try{i=JSON.parse(o).error||i}catch{}t.disabled=!1,t.textContent=n,r(`re-enrich failed: ${i}`);return}const a=await s.json();Object.assign(e,{title:a.title,location:a.location,employment_type:a.employment_type,workplace_type:a.workplace_type,department:a.department,comp_range:a.comp_range,description:a.description,posted_at:a.posted_at,url:a.url,questions_status:a.questions_status}),j(),ne(),ge(e.posting_id,{title:a.title,location:a.location,url:a.url}),r("re-enriched from the posting link")}function ws(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>Es(e))}async function ks(e,t){const n=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.company_id=s.company_id,e.company=s.company_name,ne(),E()}let Ee=null;function Es(e){Ee=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const n=document.getElementById("relink-search");n&&(n.value=""),dn(""),document.getElementById("relink-scrim").classList.add("open"),n&&n.focus()}function pe(){document.getElementById("relink-scrim").classList.remove("open"),Ee=null}let dt=null;function $s(e){dt=e;const t=(e.postings||[]).length,n=t?` and its ${t} job ${t===1?"posting":"postings"}`:"",s=document.getElementById("delcompany-summary");s&&(s.innerHTML=`Delete <strong>${c(e.name||"this company")}</strong>${n}?`);const a=document.getElementById("delcompany-confirm");a&&(a.disabled=!1),document.getElementById("delcompany-scrim").classList.add("open")}function Pe(){document.getElementById("delcompany-scrim").classList.remove("open"),dt=null}async function xs(){const e=dt;if(!e)return;const t=document.getElementById("delcompany-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e.company_id}`,{method:"DELETE"})}catch(a){r(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");r(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=e.name||"company";Pe(),l.openId===e.company_id&&Ie(),q(),E(),C(),r(`deleted ${s}`)}let ut=null;function Is(e){ut=e;const t=(e.title||"").trim()||"this posting",n=e.company?` at <strong>${c(e.company)}</strong>`:"",s=document.getElementById("deljob-summary");s&&(s.innerHTML=`Delete <strong>${c(t)}</strong>${n}?`);const a=document.getElementById("deljob-confirm");a&&(a.disabled=!1),document.getElementById("deljob-scrim").classList.add("open")}function qe(){document.getElementById("deljob-scrim").classList.remove("open"),ut=null}async function Ss(){const e=ut;if(!e)return;const t=document.getElementById("deljob-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${e.posting_id}`,{method:"DELETE"})}catch(a){r(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");r(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=(e.title||"").trim()||"posting";qe(),ke(),E(),l.openId===e.company_id&&fe(e.company_id),r(`deleted ${s}`)}let pt=null;function _s(e,t,n){pt=e;const s=document.getElementById("delcontact-summary");s&&(s.innerHTML=`Remove <strong>${c(t)}</strong>?`);const a=document.getElementById("delcontact-note");a&&(a.textContent=`You've logged ${n} email${n===1?"":"s"} to this contact — removing them takes that send history off this posting.`);const o=document.getElementById("delcontact-confirm");o&&(o.disabled=!1),document.getElementById("delcontact-scrim").classList.add("open")}function He(){document.getElementById("delcontact-scrim").classList.remove("open"),pt=null}async function Ts(){const e=pt;if(!e)return;const t=document.getElementById("delcontact-confirm");t&&(t.disabled=!0);const n=await z("DELETE",`/api/contacts/${e}`);He(),n&&(r("contact removed"),se())}function dn(e){const t=document.getElementById("relink-results");if(!t)return;const n=e.trim().toLowerCase();let s=(l.rows||[]).slice();if(n?(s=s.filter(o=>(o.name||"").toLowerCase().includes(n)),s.sort((o,i)=>{const d=(o.name||"").toLowerCase().startsWith(n)?0:1,u=(i.name||"").toLowerCase().startsWith(n)?0:1;return d-u||(o.name||"").localeCompare(i.name||"")})):s.sort((o,i)=>(o.name||"").localeCompare(i.name||"")),s=s.slice(0,60),!s.length){t.innerHTML=`<div class="relink-empty">${(l.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const a=Ee?Ee.company_id:"";t.innerHTML=s.map(o=>{const i=o.company_id===a,d=[o.vertical,o.location].filter(Boolean).map(c).join(" · ");return`<button type="button" class="relink-result${i?" is-current":""}"
      ...
      </button>`}).join("")}async function un(e){const t=Ee;if(!t){pe();return}if(e===t.company_id){pe();return}try{await ks(t,e),pe(),r(`moved to ${t.company}`)}catch(n){r(`move failed: ${n.message}`)}}async function pn(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),q(),E()}async function Ls(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();l.openId=s.company_id,Se(s),_e(s.company_id),q(),E()}async function Bs(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}let mn=null;function ne(){const e=h.row;if(!e)return;const t=document.getElementById("pursuit-body"),s=!!t&&mn===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${c(e.title||"")}">`;const a=e.application_status||"";document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a?Ae(a)||"pill-stage":"pill-none"}">${c(a||"—")}</span>`;const o=document.getElementById("pursuit-chat");o&&(o.style.display=l.meta&&l.meta.chat?"":"none",o.onclick=()=>Ht("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
    <section class="pane-section role-head">
      <div id="role-body">${Cs(e)}</div>
    </section>
    ...
    <section class="pane-section">
      ...
      <div class="company-row">
        ...
        ${e.verdict?`<span class="role-verdict"><span class="role-verdict-label">fit</span><span class="${y(e.verdict)}" title="scout's company-fit verdict">${c(e.verdict)}</span></span>`:""}
      </div>
    </section>
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row">
          ...
          <select class="input pl-appstatus" title="application stage">
            ${nn(e.application_status||"").map(([p,m])=>`<option value="${c(p)}"${(e.application_status||"")===p?" selected":""}>${c(m)}</option>`).join("")}
          </select>
        ...
        <div class="pipeline-row">
          ...
          <select class="input pl-ostatus" title="outreach reply status — separate from the application stage">
            ${sn(e.outreach_status||"").map(([p,m])=>`<option value="${c(p)}"${(e.outreach_status||"")===p?" selected":""}>${c(m)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
  ...
  `,js();const i=document.getElementById("pursuit-company-link");i&&i.addEventListener("click",()=>fe(e.company_id)),ws(e),F(document.getElementById("pursuit-title-input"),p=>rn(e,"title",p)),F(document.getElementById("pursuit-url-input"),p=>vs(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>bs(e,d)),F(document.getElementById("pursuit-notes-input"),p=>Ms(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>F(p,m=>rn(e,p.dataset.k,m),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>Is(e)),me(),xe(),t&&(t.scrollTop=s),mn=e.posting_id}function Cs(e){return`
    ...
    ${e.posted_at?`<div class="role-posted">posted ${c(e.posted_at)}</div>`:""}`}function js(){const e=document.querySelector("#pursuit-body .pl-appstatus");e&&e.addEventListener("change",s=>gn({application_status:s.target.value}));const t=document.querySelector("#pursuit-body .pl-ostatus");t&&t.addEventListener("change",s=>gn({outreach_status:s.target.value}));const n=document.querySelector("#pursuit-body .pt-nextup");n&&n.addEventListener("click",()=>hn(h.row,!0))}async function hn(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){r(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();r(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,j(),ge(e.posting_id,{next_up:s.next_up}),t&&ne(),r(e.next_up?"queued next up":"removed from the queue")}async function fn(e,t){const n={application_status:e.application_status||"",outreach_status:e.outreach_status||"",notes:e.notes||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return r(`save failed: ${o.message}`),null}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();return r(`save failed: ${o||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{application_status:a.application_status,application_status_at:a.application_status_at,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,outreach_status:a.outreach_status,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),ge(e.posting_id,{application_status:a.application_status,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function Ms(e){const t=h.row,n={application_status:t.application_status||"",outreach_status:t.outreach_status||"",notes:e},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,j()}async function gn(e){await fn(h.row,e)&&(j(),ne(),r("tracking saved"))}async function yn(e,t){await fn(e,t)&&(j(),h.postingId===e.posting_id&&(h.row=e,ne()),r("tracking saved"))}function me(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.drafts,n=t[0]||null,s=t.slice(1),o=n&&(Us(n.status)||n.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${n?"Draft again":"Draft outreach"}</button><label class="draft-skip-research" title="Skip the web-research stage — write straight from the template; the opener becomes a plain intro."><input type="checkbox" id="draft-skip-research"> skip research</label>`,i=s.length?`
    <details class="draft-history" ${h.openHist?"open":""}>
      <summary>${s.length} earlier draft${s.length>1?"s":""}</summary>
      <div id="draft-history-body">${s.map(d=>kn(d,!0)).join("")}</div>
    </details>`:"";e.innerHTML=Hs()+`<div class="outreach-drafts-head">Drafts</div><div id="draft-current">${n?kn(n,!1):""}</div><div class="draft-actions">${o}</div>`+i,Ds(),zs()}function As(e,t){const n=h.row||{},s={company:n.company||"",role:n.title||"",contact_name:e&&e.name||"",contact_role:e&&e.role||"",last_sent:t&&t.sent_at||"",last_message:t&&t.body||""};return(l.followupTemplate||"").replace(/\{\{\s*([A-Za-z][A-Za-z0-9_]*)\s*\}\}/g,(a,o)=>o in s?s[o]:a)}function Ps(e){const t=Date.parse(e.replace(" ","T")+"Z");if(isNaN(t))return e;const n=Math.max(0,Math.round((Date.now()-t)/1e3));return n<60?"just now":n<3600?`${Math.floor(n/60)}m ago`:n<86400?`${Math.floor(n/3600)}h ago`:`${Math.floor(n/86400)}d ago`}function qs(){const e=l.gmail||{};if(!e.connected)return`<div class="cc-gmailbar cc-gmailbar-off dim">Gmail not connected — sends are logged by hand and replies don't auto-sync. Connect it in Settings → Gmail.</div>`;const t=e.last_sync_at?`synced ${c(Ps(e.last_sync_at))}`:"not synced yet";return`<div class="cc-gmailbar">
    <span class="cc-gmail-on" title="${c(e.email||"")}">Gmail tracking on${e.email?` · ${c(e.email)}`:""}</span>
  ...
  </div>`}function Hs(){const e=h.row,t=qs(),n=e.last_outreach_at?`<div class="outreach-meta"><span>last outreach ${c(e.last_outreach_at)}</span></div>`:"";if(!h.contactsLoaded)return`<div class="contacts-mgr">${t}${n}<div class="loading-row"><span class="spinner"></span><span>loading contacts…</span></div></div>`;const s=h.contacts.map(Os).join(""),a=h.contacts.length?"":`<div class="cc-empty dim">No contacts yet — add the people you're reaching out to at ${c(e.company)}.</div>`;return`<div class="contacts-mgr">
    ${t}
    ${n}
  ...
  </div>`}function Os(e){const t=h.outreach.filter(o=>o.contact_id===e.id),n=t[0]||null,s=e.role?`<span class="cc-role">${c(e.role)}</span>`:"",a=e.email?`<a class="cc-mail" href="mailto:${c(e.email)}" title="${c(e.email)}">${c(e.email)}</a>`:"";return`<div class="contact-card" data-cid="${e.id}">
    <div class="cc-head">
    ...
    ${t.length?`<details class="cc-history"><summary>${t.length} email${t.length===1?"":"s"} sent</summary><div class="cc-entries">${t.map(Rs).join("")}</div></details>`:""}
  </div>`}function Ns(e){if(!e)return'<span class="dim">no outreach logged yet</span>';const t=`last ${c(e.sent_at)}`,n=e.id,s=e.followup_due_at,a=s&&s<=Me();return e.followup_done_at?a?`${t} · <span class="fu-escalate">no reply — try another contact</span> <button class="cc-fu-dismiss" data-eid="${n}" type="button" title="dismiss — stop reminding me about this contact">dismiss</button>`:`${t} · <label class="cc-fu-check fu-done"><input class="cc-fu-toggle" type="checkbox" data-eid="${n}" checked> followed up</label>`:s?`${t} · <label class="cc-fu-check${a?" fu-overdue":""}"><input class="cc-fu-toggle" type="checkbox" data-eid="${n}"> follow up</label> <button class="cc-fu-stop" data-eid="${n}" type="button" title="discontinue follow-ups for this contact">stop</button>`:`${t} · <span class="fu-stopped">follow-up stopped</span> <button class="cc-fu-resume" data-eid="${n}" type="button">resume</button>`}function Rs(e){const t=e.followup_done_at?'<span class="fu-done">followed up</span>':e.followup_due_at?`<span class="fu-mini">→ follow up ${c(e.followup_due_at)}</span>`:"",n=e.gmail_message_id?'<span class="cc-e-prov prov-gmail" title="sent via Gmail — replies auto-sync">via Gmail ✓</span>':'<span class="cc-e-prov prov-manual" title="logged by hand — not tracked in Gmail">logged manually</span>',s=e.gmail_message_id?"":`<button class="cc-e-del" type="button" data-eid="${e.id}" title="delete this logged send (and its follow-up)" aria-label="delete this send">×</button>`,a=e.body?'<span class="cc-e-view"></span>':"",o=a||s?`<span class="cc-e-actions">${a}${s}</span>`:"",i=`<span class="cc-e-date">${c(e.sent_at)}</span>
        ${n}
        ${e.note?`<span class="cc-e-note">${c(e.note)}</span>`:""}
        ${t}`;return e.body?`<details class="cc-entry-d">
        <summary class="cc-entry">${i}${o}</summary>
        <pre class="cc-e-body">${c(e.body)}</pre>
      </details>`:`<div class="cc-entry">${i}${o}</div>`}async function z(e,t,n){let s;try{s=await fetch(t,{method:e,headers:n?{"Content-Type":"application/json"}:{},body:n?JSON.stringify(n):void 0})}catch(a){return r(`save failed: ${a.message}`),null}if(!s.ok){const a=(await s.text().catch(()=>"")).trim();return r(`save failed: ${a||"HTTP "+s.status}`),null}try{return await s.json()}catch{return{}}}async function se(){await E(),await we()}function Ds(){const e=document.getElementById("outreach-section");if(!e)return;const t=h.postingId,n=e.querySelector(".cc-sync-now");n&&n.addEventListener("click",async()=>{n.disabled=!0;const a=n.textContent;n.innerHTML='<span class="spinner spinner-xs"></span> Syncing…';try{const o=await fetch("/api/gmail/sync?reconcile=1",{method:"POST"});if(!o.ok){r(`sync failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}await ye(),await we(),r("synced with Gmail")}catch(o){r(`sync failed: ${o.message}`)}finally{n.disabled=!1,n.textContent=a}});const s=e.querySelector(".cc-addwrap");if(s){const a=s.querySelector(".cc-addform");s.querySelector(".cc-addbtn").addEventListener("click",()=>{a.style.display="",s.querySelector(".cc-addbtn").style.display="none",a.querySelector(".cc-f-name").focus()}),s.querySelector(".cc-f-cancel").addEventListener("click",()=>me()),s.querySelector(".cc-f-save").addEventListener("click",async()=>{const o={name:a.querySelector(".cc-f-name").value,role:a.querySelector(".cc-f-role").value,email:a.querySelector(".cc-f-email").value};await z("POST",`/api/companies/${h.row.company_id}/contacts`,o)&&(r("contact added"),se())})}e.querySelectorAll(".contact-card").forEach(a=>{const o=a.dataset.cid,i=a.querySelector(".cc-editform");a.querySelector(".cc-edit").addEventListener("click",()=>{i.style.display=i.style.display==="none"?"":"none",i.style.display!=="none"&&i.querySelector(".cc-e-name").focus()});const d=a.querySelector(".cc-e-cancel");d&&d.addEventListener("click",()=>{i.style.display="none"});const u=a.querySelector(".cc-e-save");u&&u.addEventListener("click",async()=>{const g={name:i.querySelector(".cc-e-name").value,role:i.querySelector(".cc-e-role").value,email:i.querySelector(".cc-e-email").value};await z("PUT",`/api/contacts/${o}`,g)&&(r("contact saved"),se())}),a.querySelector(".cc-arch").addEventListener("click",async()=>{const g=h.outreach.filter(D=>String(D.contact_id)===String(o)).length;if(g>0){const D=h.contacts.find(Q=>String(Q.id)===String(o));_s(o,D&&D.name||"this contact",g);return}await z("DELETE",`/api/contacts/${o}`)&&(r("contact removed"),se())});const p=a.querySelector(".cc-logform"),m=a.querySelector(".cc-log");m&&m.addEventListener("click",()=>{p.style.display=p.style.display==="none"?"":"none",p.style.display!=="none"&&p.querySelector(".cc-l-date").focus()});const f=a.querySelector(".cc-l-cancel");f&&f.addEventListener("click",()=>{p.style.display="none"});const v=a.querySelector(".cc-l-save");v&&v.addEventListener("click",async()=>{const g={contact_id:o,sent_at:p.querySelector(".cc-l-date").value||Me(),body:p.querySelector(".cc-l-body").value};await z("POST",`/api/postings/${t}/outreach-log`,g)&&(r("outreach logged"),se())});const M=a.querySelector(".cc-followup");M&&M.addEventListener("click",()=>{const g=h.contacts.find(D=>String(D.id)===String(o)),N=h.outreach.filter(D=>String(D.contact_id)===String(o))[0]||null;De(As(g,N),"follow-up copied — paste into your email")});const S=async(g,N,D)=>{const Q=h.outreach.find(bo=>String(bo.id)===String(g))||{};await z("PUT",`/api/outreach-log/${g}`,{sent_at:Q.sent_at||"",body:Q.body||"",note:Q.note||"",followup_due_at:Q.followup_due_at||"",done:!!Q.followup_done_at,...N})&&(r(D),se())},k=a.querySelector(".cc-fu-toggle");k&&k.addEventListener("change",()=>S(k.dataset.eid,{done:k.checked},k.checked?"marked followed up":"follow-up reopened"));const B=a.querySelector(".cc-fu-stop");B&&B.addEventListener("click",()=>S(B.dataset.eid,{followup_due_at:"",done:!1},"follow-up stopped"));const R=a.querySelector(".cc-fu-resume");R&&R.addEventListener("click",()=>S(R.dataset.eid,{followup_due_at:Me(),done:!1},"follow-up resumed"));const O=a.querySelector(".cc-fu-dismiss");O&&O.addEventListener("click",()=>S(O.dataset.eid,{followup_due_at:"",done:!0},"escalation dismissed")),a.querySelectorAll(".cc-e-del").forEach(g=>g.addEventListener("click",async N=>{if(N.preventDefault(),N.stopPropagation(),!confirm("Delete this logged send? Its follow-up is removed too. This can't be undone."))return;const D=g.dataset.eid;await z("DELETE",`/api/outreach-log/${D}`)&&(r("send deleted"),se())}))})}function Us(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const vn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',bn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',wn=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${vn}</button>`,mt=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],Vs='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function Fs(e){let t=mt.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${mt.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",i=a<t?Vs:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${i}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${mt[t].active}…</span></div>
  </div>`}function Js(){const e=!!(l.gmail&&l.gmail.connected),t=(h.contacts||[]).filter(d=>d.email),n=t.length?`<select class="input draft-recipient" title="recipient" aria-label="recipient">${t.map(d=>`<option value="${d.id}">${c(d.name||d.email)}${d.email?` &lt;${c(d.email)}&gt;`:""}</option>`).join("")}</select>`:"",s=e&&t.length?`<button class="btn btn-primary draft-gmail-btn" title="send this email from your Gmail now, log it, and arm a follow-up">${bn}Send via Gmail</button>`:"",o=`<button class="btn draft-sent-btn" title="${t.length?"I sent this myself — log it to the chosen contact and arm a follow-up":"mark this draft sent (no contact to log against — add one to track follow-ups)"}">${bn}Mark sent${t.length?" (log it)":""}</button>`,i=t.length?"":'<div class="draft-note dim">Add a contact with an email to log the send + arm a follow-up.</div>';return`<div class="draft-gmail-row">${n}${s}${o}</div>${i}`}function kn(e,t){const n=(p,m,f="")=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const p=Gs(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ${n("pill pill-no","failed")}
      ${e.fail_reason?`<div class="draft-note">${c(e.fail_reason)}</div>`:""}
      ${p}
      ${$e(e)}
      ${t?"":`<div class="draft-actions"><button class="btn btn-primary draft-retry-btn">${Bt}Retry</button></div>`}
      ...
      <div class="draft-sentbody">${c(ht(e)||"(empty)")}</div>
      ${$e(e)}
    </div>`;if(e.status==="sent")return`<div class="draft-card dc-sent" data-did="${e.id}">
      ${n("pill pill-yes","sent",t?"":wn)}
      ${e.sent_at?`<div class="draft-note">Sent ${c((e.sent_at||"").replace("T"," ").slice(0,16))}</div>`:""}
      <div class="draft-sentbody">${c(ht(e)||"(empty)")}</div>
      ${$e(e)}
    </div>`;const s=ht(e),a=e.status==="no_hook",o=a?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let i="";if(a)try{i=JSON.parse(e.hook||"{}").reasoning||""}catch{}const d=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${i?" "+c(i):""}</div>`:"";if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    <div class="draft-head">${o}${s?wn:""}</div>
    ${d}
    ${u?`<textarea class="draft-textarea" id="draft-edit-${e.id}" spellcheck="false">${c(s)}</textarea>
    ${En(e.lint)}
    ${Js()}
    <div class="draft-actions">
      <button class="btn draft-regen-btn" title="discard this draft (kept in history) and re-run — picks up backfilled info">${Bt}Regenerate</button>
      ...
      <button class="btn draft-regen-btn" title="re-run the draft — picks up backfilled info">${Bt}Regenerate</button>
      <label class="draft-skip-research" title="Regenerate without web research — drops the carried research and writes a plain intro."><input type="checkbox" class="draft-regen-skip"> skip research</label>
    </div>`}
    ${$e(e)}
  </div>`}function ht(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function $e(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,p)=>p?`<div class="tr-line"><span class="tr-key">${u}:</span> ${c(String(p))}</div>`:"",o=n.role||{},i=Array.isArray(n.hooks)?n.hooks:[],d=i.map(u=>`
      <div class="tr-line">
      ...
      </div></details>`}return t}function En(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${c(n.message||"")}"><code>${c(n.code||"")}</code>${c(n.message||"")}</span>`).join("")+"</div>":""}function Gs(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${c(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${c(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function zs(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#draft-start-btn");t&&t.addEventListener("click",()=>Oe(!1,Ws())),e.querySelectorAll(".draft-retry-btn").forEach(s=>s.addEventListener("click",()=>Oe())),e.querySelectorAll(".draft-regen-btn").forEach(s=>s.addEventListener("click",a=>{const o=a.currentTarget.closest(".draft-card"),i=o?o.querySelector(".draft-regen-skip"):null;Oe(!0,!!(i&&i.checked))})),e.querySelectorAll(".draft-card[data-did]").forEach(s=>{const a=s.dataset.did,o=s.querySelector(".draft-textarea");o&&F(o,m=>Ys(a,m),{multiline:!0});const i=()=>s.querySelector(".draft-recipient"),d=s.querySelector(".draft-sent-btn");d&&d.addEventListener("click",()=>{const m=i();Qs(a,m?m.value:"")});const u=s.querySelector(".draft-gmail-btn");u&&u.addEventListener("click",()=>{const m=i();Zs(a,m?m.value:"",u)});const p=s.querySelector(".draft-copy-btn");p&&p.addEventListener("click",()=>{const m=s.querySelector(".draft-textarea"),f=s.querySelector(".draft-sentbody"),v=m?m.value:f?f.textContent:"";De(v,"email copied")})});const n=e.querySelector("details.draft-history");n&&n.addEventListener("toggle",()=>{h.openHist=n.open})}function Ws(){const e=document.getElementById("draft-skip-research");return!!(e&&e.checked)}async function Oe(e=!1,t=!1){const n=document.getElementById("outreach-section"),s=n&&(n.querySelector("#draft-start-btn")||n.querySelector(".draft-retry-btn")||n.querySelector(".draft-regen-btn"));s&&(s.disabled=!0);let a;try{const i=new URLSearchParams;e&&i.set("regenerate","1"),t&&i.set("research","0");const d=i.toString()?`?${i.toString()}`:"";a=await fetch(`/api/postings/${h.postingId}/outreach${d}`,{method:"POST"})}catch(i){r(`draft failed: ${i.message}`),s&&(s.disabled=!1);return}if(a.status===202){let i={};try{i=await a.json()}catch{}Array.isArray(i.degraded)&&i.degraded.length&&r(`drafting without ${i.degraded.join(", ")} — quality degrades, integrity unaffected`),await ue(),E();return}if(a.status===409){await ue(),r("a draft is already active");return}if(a.status===412){let i={};try{i=await a.json()}catch{}Ks(i.need,i.error),s&&(s.disabled=!1);return}if(a.status===503){const i=document.getElementById("outreach-section");if(i){const d=document.createElement("div");d.className="draft-note",d.textContent="Outreach engine not running in this build.",i.appendChild(d)}s&&(s.disabled=!1);return}const o=(await a.text().catch(()=>"")).trim();r(`draft failed: ${o||"HTTP "+a.status}`),s&&(s.disabled=!1)}function Ks(e,t){const n=document.getElementById("outreach-section");if(!n)return;const s=n.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"View brain knowledge",i=document.createElement("div");i.className="blocks-gate",i.innerHTML=`
    ...
    </div>`,s?s.replaceWith(i):n.appendChild(i);const d=i.querySelector("#gate-fix-btn");d&&d.addEventListener("click",()=>a?ka("outreach-template"):Dn());const u=i.querySelector("#gate-retry-btn");u&&u.addEventListener("click",Oe)}async function Ys(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.drafts.findIndex(d=>String(d.id)===String(e));a>=0&&(h.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),i=o&&o.closest(".draft-card");if(i){const d=i.querySelector(".lint-chips"),u=En(s.lint);d?d.outerHTML=u||"":u&&o.insertAdjacentHTML("afterend",u)}}async function Zs(e,t,n){n&&(n.disabled=!0,n.dataset.t=n.textContent||"",n.textContent="Sending…");const s=()=>{n&&(n.disabled=!1,n.textContent=n.dataset.t||"Send via Gmail")};let a;try{a=await fetch(`/api/outreach/drafts/${e}/send-gmail`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:t||""})})}catch(i){r(`send failed: ${i.message}`),s();return}if(!a.ok){const i=(await a.text().catch(()=>"")).trim();let d=i||"HTTP "+a.status;try{const u=JSON.parse(i);u&&u.error&&(d=u.error)}catch{}r(`send failed: ${d}`),s();return}let o={};try{o=await a.json()}catch{}r(o.to?`sent via Gmail to ${o.to}`:"sent via Gmail"),await ue(),await E(),await we()}async function Qs(e,t=""){let n;try{n=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:t||""})})}catch(a){r(`failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();let o=a||"HTTP "+n.status;try{const i=JSON.parse(a);i&&i.error&&(o=i.error)}catch{}r(`failed: ${o}`);return}r(t?"marked sent — follow-up armed":"marked sent"),await ue(),await E(),t&&await we();const s=l.jobs.find(a=>a.posting_id===h.postingId);s&&ge(s.posting_id,{outreach_count:s.outreach_count,last_outreach_at:s.last_outreach_at,next_up:s.next_up})}async function he(){if(!h.postingId)return;let e;try{const t=await fetch(`/api/postings/${h.postingId}/answers`);if(!t.ok){xe();return}e=await t.json()}catch{xe();return}h.answers=e.answers||[],h.answersStatus=e.questions_status||"",xe(),h.answers.some(t=>t.status==="generating")?Xs():ft()}function Xs(){h.answersPoll||(h.answersPoll=setInterval(he,4e3))}function ft(){h.answersPoll&&(clearInterval(h.answersPoll),h.answersPoll=null)}function xe(){const e=document.getElementById("answers-section");if(!e)return;const t=h.answers,n=h.answersStatus,s=t.some(p=>p.status==="generating"),a=t.length?`<div class="answers-list">${t.map(na).join("")}</div>`:"",o=!!h.detecting,i=s||o?" disabled":"",d=p=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":p}</button>`;let u;n==="ok"&&t.length?u=(t.some(m=>!$n(m)&&m.status!=="generating")?`<button class="btn" id="answers-start-btn"${i}>${s?"Drafting…":"Draft all blank"}</button>`:"")+d("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${i}>${s?"Drafting…":"Draft answers"}</button>`+d("Re-detect questions"):u=d("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${c(ea(n,t.length))}</div>`+a+`<div class="answers-actions">${u}</div>`,aa()}function ea(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function $n(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function ta(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function na(e){const t=$n(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,i=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`,d=!!t,u=d?"Regenerate":"Generate",p=d?"re-draft this answer (discards the current text)":"draft an answer to just this question";return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    <div class="answer-foot">
      ...
      ${s||!d?"":`<button class="answer-copy-btn dh-copy" title="copy this answer to the clipboard" aria-label="copy answer">${vn}</button>`}
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${c(sa(e.fail_reason))}</div>`:""}
  </div>`}function sa(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function aa(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",xn);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",ia),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(F(o,p=>ca(a,p),{multiline:!0}),o.addEventListener("input",()=>oa(s,o)));const i=s.querySelector(".answer-regen-btn");i&&i.addEventListener("click",()=>la(a));const d=s.querySelector(".answer-copy-btn");d&&d.addEventListener("click",()=>{o&&De(o.value,"answer copied")});const u=s.querySelector(".answer-remove-btn");u&&u.addEventListener("click",()=>ra(a))})}function oa(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function xn(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${h.postingId}/answers`,{method:"POST"})}catch(a){r(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await he();return}if(n.status===412){let a={};try{a=await n.json()}catch{}In(a.error),t&&(t.disabled=!1);return}if(n.status===503){Sn("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();r(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function ia(){h.detecting=!0,xe();try{const e=await fetch(`/api/postings/${h.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();r(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){r(`detect failed: ${e.message}`)}h.detecting=!1,await he()}async function ca(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=h.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(h.answers[a]=s)}async function la(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){r(`regenerate failed: ${n.message}`);return}if(t.status===503){Sn("Answer generation isn't running in this build.");return}if(t.status===412){let n={};try{n=await t.json()}catch{}In(n.error);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();r(`regenerate failed: ${n||"HTTP "+t.status}`);return}await he()}async function ra(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"DELETE"})}catch(n){r(`remove failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();r(`remove failed: ${n||"HTTP "+t.status}`);return}await he()}function In(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",Dn);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",xn)}function Sn(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function fe(e){var d,u;const t=document.getElementById("pane"),n=document.getElementById("scrim"),s=l.openId===e&&t.classList.contains("open"),a=s?((d=document.getElementById("pane-body"))==null?void 0:d.scrollTop)??0:0,o=s?(u=document.getElementById("trace-body"))==null?void 0:u.innerHTML:null;l.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),ln("company"),s||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let i;try{const p=await fetch(`/api/companies/${e}`);if(!p.ok)throw new Error(`HTTP ${p.status}`);i=await p.json()}catch(p){s||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${c(p.message)}</div>`);return}if(l.openId===e){if(Se(i),s){if(o!=null){const m=document.getElementById("trace-body");m&&(m.innerHTML=o)}const p=document.getElementById("pane-body");p&&(p.scrollTop=a)}_e(e)}}function Ie(){l.openId=null,l.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function Se(e){l.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${c(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
    <span class="${y(e.has_verdict?e.verdict:"")}">${c(e.has_verdict?e.verdict:"unscored")}</span>
  `;const t=document.getElementById("pane-chat");t&&(t.style.display=l.meta&&l.meta.chat?"":"none",t.onclick=()=>Ht("company",e.company_id,e.name));const n=e.model==="manual",s=e.has_verdict?`
    <dl class="kv">
  ...
  `:'<div class="muted">No enrichment yet. Run <code>scout enrich</code>.</div>',i=!l.meta||l.meta.control!==!1,d=i&&l.meta&&l.meta.verdict?'<button class="h3-action" id="rescore-btn" title="re-score just this company — replaces the current verdict, manual or not">↻ re-score</button>':"",u=i&&e.domain?'<button class="h3-action" id="reenrich-btn" title="re-fetch this company’s site now">↻ re-enrich</button>':"",p=Object.keys(e.raw_json||{}).sort(),m=p.length===0?"":`
  ...
  `;const v=document.getElementById("posting-add-btn");v&&v.addEventListener("click",()=>ma(e)),yt(),document.querySelectorAll("#ve-pick .ve-opt").forEach(g=>{g.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(N=>N.classList.remove("is-on")),g.classList.add("is-on")})});const M=document.getElementById("ve-save-btn");M&&M.addEventListener("click",()=>pa(e)),F(document.getElementById("pane-title-input"),g=>pn(e,"name",g)),document.querySelectorAll("#facts-body [data-k]").forEach(g=>F(g,N=>pn(e,g.dataset.k,N))),F(document.getElementById("pane-domain-input"),g=>Ls(e,g)),F(document.getElementById("pane-notes-input"),g=>Bs(e,g),{multiline:!0});const S=document.getElementById("flag-toggle-btn");S&&S.addEventListener("click",()=>_n(e.company_id));const k=document.getElementById("review-stamp-btn");k&&k.addEventListener("click",()=>ua(e.company_id));const B=document.getElementById("rescore-btn");B&&B.addEventListener("click",()=>wt("verdict",{company_ids:[e.company_id]}));const R=document.getElementById("reenrich-btn");R&&R.addEventListener("click",()=>wt("enrich",{company_ids:[e.company_id]}));const O=document.getElementById("company-delete-btn");O&&O.addEventListener("click",()=>$s(e))}function da(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function ua(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){r(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");r(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=l.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,J()),l.openId===e&&(Se(s),_e(e)),r("reviewed")}async function _n(e){const t=l.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){r(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");r(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,J()),l.openId===e&&(Se(a),_e(e)),E(),r(a.flagged?"flagged":"unflagged")}async function pa(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){r("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(d){r(`save failed: ${d.message}`),a.disabled=!1;return}if(!o.ok){const d=await o.text().catch(()=>"");r(`save failed: HTTP ${o.status}${d?" — "+d:""}`),a.disabled=!1;return}const i=await o.json();Se(i),_e(i.company_id),q(),C(),E(),r("verdict saved")}function gt(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(c).join(" · "),a=n.application_status||"",o=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?Ae(a)||"pill-stage":"pill-none"}">${c(a||"—")}</span>`,`<span class="pt-meta">${a?"tracked":"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${c(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function ge(e,t){const n=l.openDetail;if(!n||!l.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=gt(n),yt())}function yt(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||ct(e.dataset.pid)})})}async function ma(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){r("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){r(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");r(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const i=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==i.id),e.postings.unshift(i);const d=document.getElementById("postings-list");d&&(d.innerHTML=gt(e),yt()),t.value="",n.value="",s.disabled=!1,E(),r("link added")}async function _e(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){Ne(`<div class="muted">Failed to load trail: ${c(s.message)}</div>`);return}if(!t.ok){Ne(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Ne('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Ne(n.map(ha).join(""))}function ha(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(c);return e.run_id&&t.push("run "+c(e.run_id.slice(0,8))),`
    ...
    </div>`}function Ne(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let Tn;function r(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(Tn),Tn=setTimeout(()=>t.classList.remove("show"),2200)}let vt;const fa=6e3;function Re(){clearTimeout(vt),vt=void 0}function Ln(){Re(),document.getElementById("drawer").classList.remove("open")}function Bn(){Re(),vt=setTimeout(Ln,fa)}async function De(e,t="copied"){if(!e){r("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}r(t)}catch(n){r(`copy failed: ${n.message}`)}}l.meta={control:!1,brain:!1,verdict:!1};async function Te(){try{const s=await fetch("/api/meta");if(!s.ok)return;l.meta=await s.json()}catch{return}const e=l.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!l.meta.verdict,t.title=l.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=l.meta.chat?"":"none")}async function bt(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Le=null;async function wt(e,t){if(l.meta&&l.meta.control===!1){r("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){r(`run failed: ${a.message}`);return}if(n.status===409){r("a job is already running");return}if(n.status===412){const a=await n.text();r(a.trim());return}if(!n.ok){r(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Hn(e,s,t)}async function ga(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){r(`upload failed: ${a.message}`);return}if(n.status===409){r("a job is already running");return}if(!n.ok){r(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Hn("ingest",s)}const ya=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let Ue=[],W=new Set,K="company";function Cn(e){K=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",jn()}function kt(){return!!l.meta.capture&&document.getElementById("add-enrich").checked}function jn(){const e=document.getElementById("add-note");kt()?e.innerHTML=K==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and description — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=K==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function va(){ya.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",W=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!l.meta.capture,t.classList.toggle("disabled",!l.meta.capture),t.title=l.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",l.meta.capture||(e.checked=!1),Cn(l.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(l.rows||[]).map(s=>`<option value="${c(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),Ue=s.verticals||[]}catch{Ue=[]}Mn()}function Ve(){document.getElementById("add-scrim").classList.remove("open")}function Mn(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=Ue.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${W.has(s)?" sel":""}" data-v="${c(s)}">${c(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;W.has(a)?W.delete(a):W.add(a),s.classList.toggle("sel"),An()}))):e.innerHTML=`<div class="none">${Ue.length?"no match":"no verticals in the set yet"}</div>`,An()}function An(){const e=W.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Pn(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function qn(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){r(K==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,kt()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=f=>document.getElementById(f).value.trim(),i=kt();let d,u;i?(d="/api/capture",u={url:Pn(t),kind:K==="company"?"company_page":"job_posting",fields:K==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...W].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):K==="company"?(d="/api/companies",u={website:t,name:o("add-name"),vertical:[...W].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(d="/api/postings",u={url:Pn(t),title:o("add-title"),company:o("add-job-company")});let p;try{p=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(f){r(`add failed: ${f.message}`),a();return}if(!p.ok){let f=`HTTP ${p.status}`;try{const v=await p.text();try{f=JSON.parse(v).error||f}catch{f=v.trim()||f}}catch{}if(a(),p.status===409){r(f||"That company is already in the list."),e.focus(),e.select();return}r(`add failed: ${f}`);return}const m=await p.json();if(a(),i&&!m.company_id){r(m.note||"couldn't classify that page");return}if(Ve(),q(),C(),E(),K==="job"){const f=m.posting&&m.posting.title||"job link";r(`tracking: ${f} @ ${m.company_name}${m.posting_updated?" (refreshed)":""}`),L("jobs")}else i?(r(m.note||(m.company_created?`company added: ${m.company_name}`:`${m.company_name} is already in the list`)),fe(m.company_id)):r("company added")}function Hn(e,t,n){Le=t,Re();const s=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="";const o=document.getElementById("drawer-summary");o.hidden=!0,o.innerHTML="",s.classList.add("open"),bt();const i={yes:0,maybe:0,no:0},d=/^(.+?)\s*→\s*(yes|maybe|no)\s*—\s*([\s\S]*)$/i,u=new EventSource(`/api/jobs/${t}/stream`),p=(m,f)=>{const v=document.createElement("div");let M;if(!f&&(M=m.match(d))){const S=M[2].toLowerCase();i[S]++,v.className="ln ln-verdict";const k=document.createElement("span");k.className="pill pill-"+S,k.textContent=S;const B=document.createElement("span");B.className="lv-text";const R=document.createElement("span");R.className="lv-name",R.textContent=M[1].trim(),B.appendChild(R);const O=(M[3]||"").trim();if(O){const g=document.createElement("span");g.className="lv-reason",g.textContent=O,B.append(" ",g)}v.append(k,B)}else if(!f&&/^(scoring|enriching|ingesting)\b/i.test(m))v.className="ln ln-head",v.textContent=m;else if(!f&&/^·\s/.test(m))v.className="ln ln-pick",v.textContent=m;else{const S=!f&&/^\s*warn:/i.test(m);v.className="ln"+(f?" ln-err":S?" ln-warn":""),v.textContent=S?m.replace(/^\s*warn:\s*/i,"⚠ "):m}a.appendChild(v),a.scrollTop=a.scrollHeight};u.addEventListener("line",m=>p(m.data,/error|failed/i.test(m.data))),u.addEventListener("end",m=>{if(u.close(),Le=null,p(`— ${m.data} —`,m.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",i.yes+i.maybe+i.no>0){for(const v of["yes","maybe","no"]){if(!i[v])continue;const M=document.createElement("span");M.className="pill pill-"+v,M.textContent=`${i[v]} ${v}`,o.appendChild(M)}o.hidden=!1}Bn(),r(`${e} ${m.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?is(n.company_ids):q(),C(),bt(),E(),l.openId&&fe(l.openId)}),u.onerror=()=>{u.close()}}async function ba(){if(Le)try{await fetch(`/api/jobs/${Le}/cancel`,{method:"POST"})}catch{}}let H=null;const wa={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function Et(e){return e==="application-stages"||e==="outreach-statuses"}function Fe(e){if(e==="outreach-template")return"email body";if(e==="outreach-subject")return"email subject";if(e==="outreach-signature")return"email signature";if(e==="followup-template")return"follow-up template";if(e==="playbook")return"playbook";if(e==="application-stages")return"application stages";if(e==="outreach-statuses")return"outreach statuses";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return(wa[t]||t)+" prompt"}return e+".md"}async function ka(e){H=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Fe(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=e.startsWith("outreach-prompts/"),s=n?e.slice(17):"",a=n&&s!=="fill";document.getElementById("editor-toggle-row").style.display=a?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",a&&(document.getElementById("editor-toggle-label").textContent="Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const d=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${d||"HTTP "+o.status}`;return}const i=await o.json();Et(e)?(document.getElementById("editor-title").textContent="edit "+Fe(e)+" — one per line",document.getElementById("editor-text").value=(i.statuses||[]).join(`
`)):document.getElementById("editor-text").value=i.content||"",a&&(document.getElementById("editor-enabled").checked=i.enabled!==!1),i.taste_version&&(document.getElementById("editor-ver").textContent="version "+i.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Je(){document.getElementById("editor-scrim").classList.remove("open"),H=null}let T=null,$t=[],xt=[];const Ea=["location.allowed","verticals.excluded","verticals.allowed"],It={"verticals.excluded":"pf-vertical-tags","verticals.allowed":"pf-vertical-tags"};function Y(e){const[t,n]=e.split(".");return T[t]&&T[t][n]||[]}function Ge(e,t){const[n,s]=e.split(".");(T[n]=T[n]||{})[s]=t}function ze(e,t){const n=t.toLowerCase();return e.some(s=>String(s).toLowerCase()===n)}function St(){Ea.forEach(e=>{const t=document.querySelector(`.pf-chips[data-field="${e}"]`);if(!t)return;const n=It[e]?` list="${It[e]}"`:"",s=It[e]?"type to search…":"type &amp; Enter…";t.innerHTML=Y(e).map((a,o)=>`<span class="pf-chip">${c(a)}<button class="pf-chip-x" data-field="${e}" data-i="${o}" title="remove" aria-label="remove ${c(a)}">×</button></span>`).join("")+`<input class="pf-chip-input" data-field="${e}"${n} type="text" placeholder="${s}" spellcheck="false" autocomplete="off" />`})}function $a(e,t){var s;const n=(t||"").trim();if(n){const a=Y(e);ze(a,n)||Ge(e,[...a,n])}St(),(s=document.querySelector(`.pf-chip-input[data-field="${e}"]`))==null||s.focus()}function On(e,t){const n=Y(e).slice();n.splice(t,1),Ge(e,n),St()}function Nn(){const e=document.getElementById("pf-stages");if(!e)return;const t=Y("funding_stage.allowed");e.innerHTML=xt.map(n=>{const s=ze(t,n.value),a=n.count?` <span class="pf-stage-n">${n.count}</span>`:"";return`<button class="pf-stage${s?" is-on":""}" data-stage="${c(n.value)}">${c(n.value)}${a}</button>`}).join("")}function xa(e){const t=Y("funding_stage.allowed");Ge("funding_stage.allowed",ze(t,e)?t.filter(n=>String(n).toLowerCase()!==e.toLowerCase()):[...t,e]),Nn()}function Ia(){const e=document.getElementById("pf-vertical-tags");e&&(e.innerHTML=$t.map(t=>`<option value="${c(t.value)}" label="${t.count}"></option>`).join(""))}function Sa(){return{location:{allowed:[],remote_ok:!0},headcount:{min:0,max:0},verticals:{allowed:[],excluded:[]},funding_stage:{allowed:[]}}}function _a(){return`<div class="set-field pf-inline">
  ...
  </div>`}async function Rn(e=!1){if(document.getElementById("pf-enabled"))try{const[t,n]=await Promise.all([(await fetch("/api/taste-filter"+(e?"?default=1":""))).json(),$t.length||xt.length?Promise.resolve(null):fetch("/api/filter-options").then(s=>s.json()).catch(()=>null)]);n&&($t=n.verticals||[],xt=n.stages||[]),T=Object.assign(Sa(),t.rules||{}),T.location=Object.assign({allowed:[],remote_ok:!0},T.location),T.headcount=Object.assign({min:0,max:0},T.headcount),T.verticals=Object.assign({allowed:[],excluded:[]},T.verticals),T.funding_stage=Object.assign({allowed:[]},T.funding_stage),e||(document.getElementById("pf-enabled").checked=t.enabled!==!1),document.getElementById("pf-remote-ok").checked=!!T.location.remote_ok,document.getElementById("pf-hc-min").value=String(T.headcount.min||0),document.getElementById("pf-hc-max").value=String(T.headcount.max||0),Ia(),St(),Nn()}catch(t){r(`failed to load pre-filter: ${t.message}`)}}async function Ta(){if(!T)return;T.location.remote_ok=document.getElementById("pf-remote-ok").checked,T.headcount.min=Math.max(0,parseInt(document.getElementById("pf-hc-min").value,10)||0),T.headcount.max=Math.max(0,parseInt(document.getElementById("pf-hc-max").value,10)||0),document.querySelectorAll(".pf-chip-input").forEach(n=>{const s=n.value.trim();s&&!ze(Y(n.dataset.field),s)&&Ge(n.dataset.field,[...Y(n.dataset.field),s])});const e=document.getElementById("pf-enabled").checked;let t;try{t=await fetch("/api/taste-filter",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({rules:T,enabled:e})})}catch(n){r(`save failed: ${n.message}`);return}if(!t.ok){r(`save failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`);return}r("pre-filter saved"),C()}const La=[{key:"experience",hard:!0},{key:"voice",hard:!1},{key:"logistics",hard:!1}];async function Dn(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{Ba(await(await fetch("/api/outreach/sources")).json())}catch(e){r(`failed to load sources: ${e.message}`)}}function _t(){document.getElementById("sources-scrim").classList.remove("open")}function Ba(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):La,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],i=o.length?o.map(d=>`<li><span class="src-title">${c(d.title||d.page_id)}</span></li>`).join(""):`<li class="dim small">${a.hard?"none yet — add an experience page to your brain":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${c(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${i}</ul></div>`}).join("")}async function Ca(){if(!H)return;const e=document.getElementById("editor-text").value;let t;Et(H)?t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)}:(t={content:e},H.startsWith("outreach-prompts/")&&H!=="outreach-prompts/fill"&&(t.enabled=document.getElementById("editor-enabled").checked));let n;try{n=await fetch(`/api/${H}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){r(`save failed: ${o.message}`);return}if(!n.ok){r(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const s=await n.json();s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version);const a=Et(H);H==="followup-template"&&(l.followupTemplate=e),r(`${Fe(H)} saved`),Je(),a&&P(),C()}async function ja(){if(!H||!H.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${H}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){r(`reset failed: ${n.message}`);return}if(!e.ok){r(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",r(`${Fe(H)} reset to default`)}function Un(e,t,n){e.k!==t?(e.k=t,e.dir=1):e.dir===1?e.dir=-1:Object.assign(e,n)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{Un(l.sort,e.dataset.k,b),J()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{Un(l.jsort,e.dataset.jk,x),j()}}),document.getElementById("tab-companies").onclick=()=>L("companies"),document.getElementById("tab-jobs").onclick=()=>L("jobs"),document.getElementById("tab-inbox").onclick=()=>L("inbox"),document.getElementById("q").oninput=J,document.getElementById("fdrop-cfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.dataset.toggle==="flagged")oe=!oe,de(t,oe);else if(t.dataset.toggle==="enriched")ie=!ie,de(t,ie);else if(t.hasAttribute("data-v")){const n=t.getAttribute("data-v");V.has(n)?V.delete(n):V.add(n),de(t,V.has(n))}else return;J()}}),document.getElementById("fdrop-columns-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item[data-col]");if(!t)return;const n=tt(),s=t.getAttribute("data-col");n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),de(t,!n.hidden.has(s)),ce(),Jt()}),document.getElementById("jq").oninput=j;for(const e of["fdrop-cfilters","fdrop-columns","fdrop-jfilters"]){const t=document.getElementById(e);t.querySelector(".fdrop-btn").addEventListener("click",s=>{s.stopPropagation();const a=t.classList.contains("is-open");it(),a||rs(t)}),t.querySelector(".fdrop-menu").addEventListener("click",s=>s.stopPropagation())}document.addEventListener("click",it),document.getElementById("fdrop-jfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-all");if(t){if(t.getAttribute("data-all")==="stage"){const s=["",...l.applicationStages];$=s.every(a=>$.has(a))?new Set:new Set(s)}else{const s=["",...l.outreachStatuses];_=_&&s.every(a=>_.has(a))?new Set:new Set(s)}at(),j();return}const n=e.target.closest(".fdrop-item");if(n){if(n.hasAttribute("data-stage")){const s=n.getAttribute("data-stage");$.has(s)?$.delete(s):$.add(s),de(n,$.has(s))}else if(n.hasAttribute("data-status")){const s=n.getAttribute("data-status");_.has(s)?_.delete(s):_.add(s),de(n,_.has(s))}else return;ls(),j()}}),ts(),Ft(),ce(),document.getElementById("pane-close").onclick=Ie,document.getElementById("scrim").onclick=Ie,document.getElementById("pursuit-close").onclick=ke,document.getElementById("pursuit-scrim").onclick=ke,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.querySelector(".fdrop.is-open")){it();return}if(document.getElementById("chat-pane").classList.contains("open")){Ot();return}if(document.getElementById("profile-scrim").classList.contains("open")){Pt();return}if(document.getElementById("add-scrim").classList.contains("open")){Ve();return}if(document.getElementById("run-scrim").classList.contains("open")){We();return}if(document.getElementById("help-scrim").classList.contains("open")){Ke();return}if(document.getElementById("relink-scrim").classList.contains("open")){pe();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){Pe();return}if(document.getElementById("deljob-scrim").classList.contains("open")){qe();return}if(document.getElementById("delcontact-scrim").classList.contains("open")){He();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(lt==="pursuit"&&n){ke();return}if(lt==="company"&&t){Ie();return}if(t){Ie();return}ke();return}if(document.getElementById("key-scrim").classList.contains("open")){At();return}if(document.getElementById("sources-scrim").classList.contains("open")){_t();return}if(document.getElementById("editor-scrim").classList.contains("open")){Je();return}if(document.getElementById("gmail-config-scrim").classList.contains("open")){Be();return}});let Tt=null;const Ma={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Vn(e){if(l.meta&&l.meta.control===!1){r("control surface disabled");return}Tt=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Ma[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=l.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function We(){document.getElementById("run-scrim").classList.remove("open"),Tt=null}document.getElementById("btn-enrich").onclick=()=>Vn("enrich"),document.getElementById("btn-verdict").onclick=()=>Vn("verdict"),document.getElementById("run-cancel").onclick=We,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&We()},document.getElementById("run-go").onclick=()=>{const e=Tt,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(We(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),wt(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=va;const Aa={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Fn(e){const t=Aa[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const i=document.createElement("div");i.className="help-item-desc",i.textContent=s.desc;const d=document.createElement("a");d.className="help-link",d.textContent="Learn more →",d.onclick=()=>{Ke(),L("docs"),eo(s.sec)},a.appendChild(o),a.appendChild(i),a.appendChild(d),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function Ke(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Fn("add"),document.getElementById("help-run").onclick=()=>Fn("run"),document.getElementById("help-close").onclick=Ke,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&Ke()},document.getElementById("add-cancel").onclick=Ve,document.getElementById("add-save").onclick=qn,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Ve()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>Cn(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",jn),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),qn()))}),document.getElementById("add-vertical-filter").addEventListener("input",Mn),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&ga(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=ba,document.getElementById("drawer-close").onclick=Ln,(()=>{const e=document.getElementById("drawer");e.addEventListener("mouseenter",Re),e.addEventListener("mouseleave",()=>{!Le&&e.classList.contains("open")&&Bn()})})(),document.getElementById("editor-cancel").onclick=Je,document.getElementById("editor-save").onclick=Ca,document.getElementById("editor-reset").onclick=ja,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Je()},document.getElementById("sources-close").onclick=_t,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&_t()},document.addEventListener("click",e=>{var a;const t=e.target.closest(".pf-stage");if(t){xa(t.dataset.stage);return}const n=e.target.closest(".pf-chip-x");if(n){On(n.dataset.field,parseInt(n.dataset.i,10));return}const s=e.target.closest(".pf-chips");s&&e.target===s&&((a=s.querySelector(".pf-chip-input"))==null||a.focus())}),document.addEventListener("keydown",e=>{const t=e.target.closest(".pf-chip-input");if(t){if(e.key==="Enter"||e.key===",")e.preventDefault(),$a(t.dataset.field,t.value);else if(e.key==="Backspace"&&!t.value){const n=Y(t.dataset.field);n.length&&On(t.dataset.field,n.length-1)}}}),document.getElementById("key-cancel").onclick=At,document.getElementById("key-save").onclick=Wn,document.getElementById("key-remove").onclick=Za,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&At()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),Wn())}),document.getElementById("delcompany-cancel").onclick=Pe,document.getElementById("delcompany-confirm").onclick=xs,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&Pe()},document.getElementById("deljob-cancel").onclick=qe,document.getElementById("deljob-confirm").onclick=Ss,document.getElementById("deljob-scrim").onclick=e=>{e.target.id==="deljob-scrim"&&qe()},document.getElementById("delcontact-cancel").onclick=He,document.getElementById("delcontact-confirm").onclick=Ts,document.getElementById("delcontact-scrim").onclick=e=>{e.target.id==="delcontact-scrim"&&He()},document.getElementById("relink-cancel").onclick=pe,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&pe()},document.getElementById("relink-search").addEventListener("input",e=>{dn(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&un(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&un(t.dataset.id)});async function Lt(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);l.profile=await e.json()}catch{l.profile=null}G()}const Bt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',Jn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4.2 7a3.8 3.8 0 0 1 7.6 0c0 3 1.2 4 1.2 4H3s1.2-1 1.2-4z"/><path d="M6.7 13a1.5 1.5 0 0 0 2.6 0"/></svg>',Pa='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 12.5v-9M4.5 7L8 3.5 11.5 7"/></svg>',qa=[["outreach","Outreach"],["pipeline","Outreach pipeline"],["tracking","Tracking"],["job-hunting","Job hunting"],["integrations","Integrations"]],Ha=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]];function G(){const e=document.getElementById("criteria-stats");if(!e)return;const t=document.getElementById("settings-view");if(t&&t.style.display==="none")return;const n=l.settingsGroup||"outreach";e.innerHTML=`<div class="settings-shell">
    <nav class="settings-nav">
      ${qa.map(([a,o])=>`<a data-grp="${a}" class="${a===n?"active":""}">${c(o)}</a>`).join("")}
    </nav>
    <div class="settings-content" id="settings-content"></div>
  </div>`,e.querySelectorAll("[data-grp]").forEach(a=>{a.onclick=()=>{l.settingsGroup!==a.dataset.grp&&(l.settingsGroup=a.dataset.grp,G())}});const s=document.getElementById("settings-content");s&&(n==="pipeline"?Da(s):n==="tracking"?Va(s):n==="job-hunting"?Fa(s):n==="integrations"?Ga(s):Ra(s))}function Z(e,t,n,s,a){return`<div class="set-field" data-kind="${e}" data-list="${a?1:0}">
  ...
  </div>`}function Ct(e){const t=e.querySelector(".set-saved");t&&(t.classList.add("show"),setTimeout(()=>t.classList.remove("show"),1500))}async function Oa(e){const t=e.dataset.kind,n=e.dataset.list==="1",s=e.querySelector(".set-textarea");try{const a=await(await fetch(`/api/${t}`)).json();s.value=n?(a.statuses||[]).join(`
`):a.content||""}catch{s.value=""}s.dataset.orig=s.value,s.dataset.loaded="1",s.addEventListener("blur",()=>Na(e))}async function Na(e){const t=e.dataset.kind,n=e.dataset.list==="1",s=e.querySelector(".set-textarea");if(s.dataset.loaded!=="1"||s.value===s.dataset.orig)return;const a=n?{statuses:s.value.split(/\r?\n/).map(i=>i.trim()).filter(Boolean)}:{content:s.value};let o;try{o=await fetch(`/api/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}catch(i){r(`save failed: ${i.message}`);return}if(!o.ok){r(`save failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}s.dataset.orig=s.value,Ct(e),t==="followup-template"&&(l.followupTemplate=s.value),n&&(await P(),j())}function jt(e){e.querySelectorAll(".set-field[data-kind]").forEach(Oa)}function Ra(e){e.innerHTML=Z("outreach-subject","Email subject","The send subject — {{role}} / {{company}} substitution, no LLM.",2,!1)+Z("outreach-template","Email body","Verbatim prose with the writer's fill-in holes.",16,!1)+Z("outreach-signature","Email signature","A fixed sign-off appended to every sent email (blank = none).",3,!1)+Z("followup-template","Follow-up template","Copy-paste follow-up — {{contact_name}}, {{role}}, {{company}}, {{last_sent}}, {{last_message}}.",9,!1)+`<div class="set-field">
      ...
      <input class="input set-fu-interval" type="number" min="0" max="90" value="${l.followupInterval}" style="margin-top:8px;width:90px">
    </div>`,jt(e);const t=e.querySelector(".set-fu-interval");t&&t.addEventListener("change",async()=>{const n=Math.max(0,Math.min(90,parseInt(t.value,10)||0));t.value=String(n),await z("PUT","/api/followup-interval",{days:n})&&(l.followupInterval=n,r("follow-up interval saved"))})}function Da(e){e.innerHTML=Ha.map(([t,n,s])=>`
    ...
    </div>`).join(""),e.querySelectorAll(".set-field[data-prompt]").forEach(Ua)}async function Ua(e){const t=e.dataset.prompt,n=e.querySelector(".set-textarea"),s=e.querySelector(".pl-enabled");try{const o=await(await fetch(`/api/outreach-prompts/${t}`)).json();n.value=o.content||"",s&&(s.checked=o.enabled!==!1)}catch{n.value=""}n.dataset.orig=n.value,n.dataset.loaded="1",n.addEventListener("blur",()=>Gn(e)),s&&s.addEventListener("change",()=>Gn(e));const a=e.querySelector(".pl-reset");a&&a.addEventListener("click",async()=>{const o=await fetch(`/api/outreach-prompts/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})});if(!o.ok){r(`reset failed: HTTP ${o.status}`);return}const i=await o.json();n.value=i.content||"",n.dataset.orig=n.value,s&&(s.checked=i.enabled!==!1),Ct(e),r("reset to default")})}async function Gn(e){const t=e.dataset.prompt,n=e.querySelector(".set-textarea"),s=e.querySelector(".pl-enabled");if(n.dataset.loaded!=="1")return;const a={content:n.value};s&&(a.enabled=s.checked);let o;try{o=await fetch(`/api/outreach-prompts/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})}catch(i){r(`save failed: ${i.message}`);return}if(!o.ok){r(`save failed: ${(await o.text().catch(()=>"")).trim()||"HTTP "+o.status}`);return}n.dataset.orig=n.value,Ct(e)}function Va(e){e.innerHTML=Z("application-stages","Application stages","The application pipeline labels (applied, screening, interview…). One per line.",6,!0)+Z("outreach-statuses","Outreach statuses","The outreach reply labels (initial contact, no response, replied…). One per line.",6,!0),jt(e)}function Fa(e){const t=l.profile,s=(t&&t.active_source||l.stats&&l.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;s?o=`<div class="set-field">
    ...
    </div>`:o=Z("taste","Taste (local fallback)","Local fallback criteria used when the brain is unreachable.",12,!1),e.innerHTML=o+Z("playbook","Playbook","How scout judges — the reasoning rules behind every verdict.",12,!1)+_a(),jt(e);const i=e.querySelector("#brief-refresh");i&&i.addEventListener("click",Qa),Rn();const d=e.querySelector("#pf-save");d&&d.addEventListener("click",Ta);const u=e.querySelector("#pf-reset");u&&u.addEventListener("click",()=>Rn(!0))}function Ja(e){const t=e.callback_uri||"(your scout URL)/api/gmail/callback",n=e.scopes||["openid","email","https://www.googleapis.com/auth/gmail.send","https://www.googleapis.com/auth/gmail.readonly"];return`<details class="set-help"${e.configured?"":" open"}>
  ...
  </details>`}function Ga(e){const t=l.anthropicKey||{};let n="Not set — verdict, capture & outreach disabled.";t.key_source==="db"?n="Set here · active.":t.key_source==="env"&&(n="Using the ANTHROPIC_API_KEY environment variable.");const s=l.gmail||{},a=!!s.connected,o=!!s.configured,i=a?"ok":o?"warn":"off",d=a?`Connected as ${c(s.email||"your account")}`:o?"Not connected":"Not set up";e.innerHTML=`
    <div class="set-field">
    ...
    </div>`;const u=e.querySelector("#set-ak-save");u&&u.addEventListener("click",async()=>{const k=e.querySelector("#set-ak-input").value.trim();if(!k){r("paste a key first");return}const B=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:k})});if(!B.ok){r((await B.text().catch(()=>"")).trim()||`HTTP ${B.status}`);return}r("Anthropic key saved"),await Te(),await Mt()});const p=e.querySelector("#set-ak-remove");p&&p.addEventListener("click",async()=>{const k=await fetch("/api/integrations/anthropic",{method:"DELETE"});if(!k.ok){r(`HTTP ${k.status}`);return}r("Anthropic key removed"),await Te(),await Mt()});const m=e.querySelector("#set-gm-save");m&&m.addEventListener("click",async()=>{const k=e.querySelector("#set-gm-id").value.trim(),B=e.querySelector("#set-gm-secret").value,R=e.querySelector("#set-gm-redirect").value.trim();if(!k){r("client ID is required");return}const O=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:k,client_secret:B,redirect_uri:R})});if(!O.ok){r((await O.text().catch(()=>"")).trim()||`HTTP ${O.status}`);return}r("Gmail OAuth client saved"),await ye()});const f=e.querySelector("#set-gm-connect");f&&f.addEventListener("click",za);const v=e.querySelector("#set-gm-disconnect");v&&v.addEventListener("click",Wa);const M=e.querySelector("#gm-copy-cb");M&&M.addEventListener("click",()=>De(l.gmail&&l.gmail.callback_uri||"","redirect URI copied"));const S=e.querySelector("#set-autoflip");S&&S.addEventListener("change",async()=>{let k=!1;try{k=(await fetch("/api/gmail/autoflip",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:S.checked})})).ok}catch{k=!1}k?(l.gmail&&(l.gmail.autoflip=S.checked),r(`auto-update ${S.checked?"on":"off"}`)):(S.checked=!S.checked,r("failed to save"))})}async function Mt(){try{l.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{l.anthropicKey=null}G()}async function ye(){try{l.gmail=await(await fetch("/api/gmail/status")).json()}catch{l.gmail=null}G()}async function za(){let e;try{e=await fetch("/api/gmail/connect")}catch(n){r(`connect failed: ${n.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}let t={};try{t=await e.json()}catch{}t.auth_url?window.location.href=t.auth_url:r("could not start the Gmail connect flow")}async function Wa(){if(!confirm("Disconnect Gmail? Sending and sync stop; already-synced data stays."))return;let e;try{e=await fetch("/api/gmail/disconnect",{method:"DELETE"})}catch(t){r(`disconnect failed: ${t.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}r("Gmail disconnected"),await ye()}function Be(){document.getElementById("gmail-config-scrim").classList.remove("open")}async function Ka(){const e=document.getElementById("gmail-client-id").value.trim(),t=document.getElementById("gmail-client-secret").value,n=document.getElementById("gmail-redirect").value.trim();if(!e){r("client ID is required");return}let s;try{s=await fetch("/api/gmail/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_id:e,client_secret:t,redirect_uri:n})})}catch(a){r(`save failed: ${a.message}`);return}if(!s.ok){r((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`);return}r("Gmail OAuth client saved — click Connect"),Be(),await ye()}async function Ya(){if(!confirm("Remove the stored Google OAuth client? Connecting needs it re-entered (or set via env)."))return;let e;try{e=await fetch("/api/gmail/config",{method:"DELETE"})}catch(t){r(`remove failed: ${t.message}`);return}if(!e.ok){r((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}r("OAuth client removed"),Be(),await ye()}function zn(){const e=l.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&l.meta&&(l.meta.outreach===!1||l.meta.chat===!1);s.style.display=a?"":"none"}}function At(){document.getElementById("key-scrim").classList.remove("open")}async function Wn(){const e=(document.getElementById("key-input").value||"").trim();if(!e){r("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){r(`save failed: ${a.message}`),n();return}if(!s.ok){r((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}l.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),r("Anthropic key saved"),await Te(),zn(),G()}async function Za(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){r(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){r((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}l.anthropicKey=await t.json(),r(l.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await Te(),zn(),G()}async function Qa(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){r(`refresh failed: ${n.message}`),Lt();return}if(!t.ok){const n=await t.text().catch(()=>"");r(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),Lt();return}l.profile=await t.json(),G(),r("company-fit brief refreshed"),C()}function Pt(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Pt,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Pt()};function Xa(){const e=document.querySelector("#docs-nav a");Ye(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function Ye(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function eo(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ye(e)}document.getElementById("open-docs").onclick=()=>L("docs");function to(){L("settings")}document.getElementById("open-settings").onclick=to,document.getElementById("gmail-config-cancel").onclick=Be,document.getElementById("gmail-config-save").onclick=Ka,document.getElementById("gmail-config-remove").onclick=Ya,document.getElementById("gmail-config-scrim").onclick=e=>{e.target.id==="gmail-config-scrim"&&Be()};function no(){const e=document.getElementById("notif-badge");if(!e)return;const t=(l.notifications&&l.notifications.unread)|0;t>0?(e.textContent=t>99?"99+":String(t),e.style.display=""):e.style.display="none"}async function ae(){try{l.notifications=await(await fetch("/api/notifications")).json()}catch{return}no(),l.view==="inbox"&&Kn()}function so(){return'<option value="">link to role…</option>'+(l.jobs||[]).map(t=>`<option value="${c(t.posting_id)}">${c((t.company||"")+" — "+(t.title||"(untitled)"))}</option>`).join("")}function ao(e){const t=e.company||e.role?`<div class="notif-ctx">${c([e.company,e.role].filter(Boolean).join(" · "))}</div>`:'<div class="notif-ctx dim">not linked to a role</div>',n=e.created_at?`<span class="notif-when">${c((e.created_at||"").replace("T"," ").slice(0,16))}</span>`:"",s=e.kind==="app_status"&&e.suggested_status&&!e.actioned&&e.posting_id?`<button class="btn btn-primary notif-apply" data-id="${e.id}">Apply: ${c(e.suggested_status)}</button>`:"",a=e.posting_id?"":`<select class="input notif-link" data-id="${e.id}" title="link this to a role">${so()}</select>`;return`<div class="notif-item${e.seen?"":" is-unread"}" data-id="${e.id}" data-seen="${e.seen?1:0}">
  ...
  </div>`}function Kn(){const e=document.getElementById("notifications-body");if(!e)return;const t=l.notifications||{notifications:[],followups:[]},n=t.notifications||[],s=t.followups||[];if(!n.length&&!s.length){e.innerHTML='<div class="cc-empty dim">Nothing here yet. Replies, application updates, and follow-ups show up as Gmail syncs.</div>';return}let a="";n.length&&(a+='<div class="settings-group-h">Updates</div>'+n.map(ao).join("")),s.length&&(a+='<div class="settings-group-h">Follow-ups due</div>'+s.map(oo).join("")),e.innerHTML=a,io()}function io(){const e=document.getElementById("notifications-body");e&&(e.querySelectorAll(".notif-item[data-id]").forEach(t=>{const n=t.dataset.id,s=t.querySelector(".notif-main");s&&t.dataset.seen==="0"&&s.addEventListener("click",()=>co(n))}),e.querySelectorAll(".notif-apply").forEach(t=>t.addEventListener("click",n=>{n.stopPropagation(),lo(t.dataset.id)})),e.querySelectorAll(".notif-link").forEach(t=>t.addEventListener("change",n=>{n.stopPropagation(),t.value&&ro(t.dataset.id,t.value)})),e.querySelectorAll(".notif-open").forEach(t=>t.addEventListener("click",()=>{const n=t.dataset.pid;L("jobs"),ct(n)})))}async function co(e){try{await fetch(`/api/notifications/${e}/seen`,{method:"POST"})}catch{return}await ae()}async function lo(e){let t;try{t=await fetch(`/api/notifications/${e}/apply`,{method:"POST"})}catch(s){r(`apply failed: ${s.message}`);return}if(!t.ok){r((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}const n=await t.json().catch(()=>({}));r(`status set to ${n.applied||"updated"}`),await ae(),await E()}async function ro(e,t){let n;try{n=await fetch(`/api/notifications/${e}/link`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({posting_id:t})})}catch(s){r(`link failed: ${s.message}`);return}if(!n.ok){r((await n.text().catch(()=>"")).trim()||`HTTP ${n.status}`);return}r("linked to role"),await ae()}async function uo(){const e=document.getElementById("notifications-sync");e&&(e.disabled=!0,e.textContent="Syncing…");try{const t=await fetch("/api/gmail/sync",{method:"POST"});r(t.ok?"synced":(await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`)}catch(t){r(`sync failed: ${t.message}`)}e&&(e.disabled=!1,e.textContent="Sync now"),await ae(),await E()}document.getElementById("notifications-sync").onclick=uo,document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ye(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&Ye(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),l.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function po(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function mo(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Ze(e){return e.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+|mailto:[^\s)]+)\)/g,(t,n,s)=>`<a href="${s}" target="_blank" rel="noopener noreferrer">${n}</a>`).replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*\n]+)\*/g,"$1<em>$2</em>")}function ho(e){const t=String(e||"").split(`
`),n=[];let s=null;const a=()=>{s&&(n.push("</"+s+">"),s=null)};let o=0;for(;o<t.length;){const i=t[o];if(/^```/.test(i)){a(),o++;const f=[];for(;o<t.length&&!/^```/.test(t[o]);)f.push(t[o]),o++;o++,n.push("<pre><code>"+c(f.join(`
`))+"</code></pre>");continue}const d=i.match(/^(#{1,6})\s+(.*)$/);if(d){a();const f=d[1].length;n.push("<h"+f+">"+Ze(c(d[2]))+"</h"+f+">"),o++;continue}const u=i.match(/^\s*[-*]\s+(.*)$/);if(u){s!=="ul"&&(a(),n.push("<ul>"),s="ul"),n.push("<li>"+Ze(c(u[1]))+"</li>"),o++;continue}const p=i.match(/^\s*\d+\.\s+(.*)$/);if(p){s!=="ol"&&(a(),n.push("<ol>"),s="ol"),n.push("<li>"+Ze(c(p[1]))+"</li>"),o++;continue}if(i.trim()===""){a(),o++;continue}a();const m=[];for(;o<t.length&&t[o].trim()!==""&&!/^```|^#{1,6}\s|^\s*[-*]\s+|^\s*\d+\.\s+/.test(t[o]);)m.push(Ze(c(t[o]))),o++;n.push("<p>"+m.join("<br>")+"</p>")}return a(),n.join("")}function Qe(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,e==="assistant"?n.innerHTML=ho(t||""):n.textContent=t||"",n}function qt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function fo(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=l.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(l.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function Yn(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=po(n.content);if(n.role==="user")s&&t.appendChild(Qe("user",s));else if(n.role==="assistant"){const a=mo(n.content);if(!s&&!a.length)continue;const o=Qe("assistant",s);if(a.length){const i=document.createElement("div");i.className="chat-tools",i.textContent="· used "+a.join(", "),o.appendChild(i)}t.appendChild(o)}}t.children.length||t.appendChild(fo()),qt()}async function Ht(e,t,n){if(!l.meta||!l.meta.chat){r("chat needs ANTHROPIC_API_KEY in the server env");return}l.chat.es&&(l.chat.es.close(),l.chat.es=null),l.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),i=await fetch("/api/chat/threads?"+o);if(!i.ok)throw new Error((await i.text().catch(()=>"")).trim()||"HTTP "+i.status);const d=await i.json();l.chat.threadId=d.thread.id,Yn(d.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+c(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Ot(){l.chat.es&&(l.chat.es.close(),l.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Xe(e){l.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function Zn(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function Qn(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||l.chat.streaming||!l.chat.threadId)return;e.value="",Zn(),Xe(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Qe("user",t));const a=Qe("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),qt();let o="";const i=m=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+m,Xe(!1)},d=l.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(m){i(m.message);return}if(!u.ok){i((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");l.chat.es=p,p.addEventListener("delta",m=>{o+=m.data,a.textContent=o,qt()}),p.addEventListener("end",async m=>{p.close(),l.chat.es===p&&(l.chat.es=null),a.classList.remove("chat-streaming"),Xe(!1),l.chat.threadId===d&&await go(),yo(),typeof m.data=="string"&&m.data.indexOf("error")===0&&r("chat: "+m.data)}),p.onerror=()=>{p.close(),l.chat.es===p&&(l.chat.es=null),a.classList.remove("chat-streaming"),Xe(!1)}}async function go(){const e=l.chat.scope,t=l.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();Yn(a.messages||[])}catch{}}function yo(){q(),E(),C(),l.openId&&fe(l.openId)}document.getElementById("open-chat").onclick=()=>Ht("global","",""),document.getElementById("chat-close").onclick=Ot,document.getElementById("chat-scrim").onclick=Ot,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),Qn()}),document.getElementById("chat-input").addEventListener("input",Zn),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),Qn())}),Wt("#t tbody",as),Wt("#jt tbody",os);const vo=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();L(vo==="jobs"?"jobs":"companies",{render:!1});function Xn(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||Xn()}),window.addEventListener("pageshow",e=>{e.persisted&&Xn()}),q(),E(),C(),Te(),bt(),Lt(),Mt(),ye(),ae(),setInterval(ae,9e4),P(),function(){const t=/[?&]gmail=(connected|error)/.exec(location.search);t&&(r(t[1]==="connected"?"Gmail connected":"Gmail connection failed"),history.replaceState(null,"",location.pathname+location.hash))}()}Eo({"":{view:()=>({mount(w){w.innerHTML=_o,To()}}),chrome:!1}},{title:"scout"});Io();So();
