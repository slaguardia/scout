(function(){const v=document.createElement("link").relList;if(v&&v.supports&&v.supports("modulepreload"))return;for(const g of document.querySelectorAll('link[rel="modulepreload"]'))i(g);new MutationObserver(g=>{for(const $ of g)if($.type==="childList")for(const c of $.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&i(c)}).observe(document,{childList:!0,subtree:!0});function E(g){const $={};return g.integrity&&($.integrity=g.integrity),g.referrerPolicy&&($.referrerPolicy=g.referrerPolicy),g.crossOrigin==="use-credentials"?$.credentials="include":g.crossOrigin==="anonymous"?$.credentials="omit":$.credentials="same-origin",$}function i(g){if(g.ep)return;g.ep=!0;const $=E(g);fetch(g.href,$)}})();function ho(w,v){const E=v.replace(/^#/,"");let i=null;for(const g of Object.keys(w))(E===g||E.startsWith(g))&&(i===null||g.length>i.length)&&(i=g);return i===null&&""in w&&(i=""),i}function fo(w){return typeof w=="function"?{view:w,chrome:!0}:{view:w.view,chrome:w.chrome!==!1}}function yo(w,v={}){const E=v.root??document.body,i=v.title??document.title??"",g=v.brandHref??"#",$=document.createElement("main"),c=document.createElement("header");c.className="cap-head";const C=document.createElement("a");C.className="brand",C.href=g,C.textContent=i,C.setAttribute("aria-label",`${i} — home`),c.appendChild(C);const q=document.createElement("nav");q.className="cap-nav",q.setAttribute("aria-label","Views");for(const M of v.nav??[]){const O=document.createElement("a");O.href=M.href,O.textContent=M.label,M.ariaLabel&&O.setAttribute("aria-label",M.ariaLabel),q.appendChild(O)}c.appendChild(q);const k=document.createElement("section");k.className="tk-content",$.appendChild(c),$.appendChild(k);const V=document.createElement("div");V.className="tk-bleed";const ne=M=>{var O;for(const S of Array.from(q.querySelectorAll("a"))){const se=((O=S.getAttribute("href"))==null?void 0:O.replace(/^#/,""))??"";S.toggleAttribute("aria-current",M!==null&&M!==""&&se===M),S.hasAttribute("aria-current")&&S.setAttribute("aria-current","page")}};let we=0;const je=()=>{const M=ho(w,location.hash);if(ne(M),M===null){V.isConnected&&V.remove(),$.isConnected||E.appendChild($),go(k,"Not found.");return}const{view:O,chrome:S}=fo(w[M]),se=S?k:V;S?(V.isConnected&&V.remove(),$.isConnected||E.appendChild($)):($.isConnected&&$.remove(),V.isConnected||E.appendChild(V)),se.replaceChildren();const tt=O(),Me=++we,ke=tt.mount(se);ke instanceof Promise&&ke.catch(F=>{Me===we&&vo(se,String(F))})};window.addEventListener("hashchange",je),je()}function go(w,v){w.replaceChildren();const E=document.createElement("div");E.className="tk-empty",E.textContent=v,w.appendChild(E)}function vo(w,v){w.replaceChildren();const E=document.createElement("div");E.className="tk-error",E.textContent=v,w.appendChild(E)}function bo(){if(!("serviceWorker"in navigator))return;if(["localhost","127.0.0.1","[::1]",""].includes(location.hostname)){navigator.serviceWorker.getRegistrations().then(v=>{for(const E of v)E.unregister()}),window.caches&&caches.keys().then(v=>{for(const E of v)caches.delete(E)});return}window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(()=>{})})}async function wo(){let w;try{w=await fetch("/api/me",{headers:{Accept:"application/json"}})}catch{return null}if(!w.ok)return null;let v;try{v=await w.json()}catch{return null}return typeof v.email=="string"&&v.email?{email:v.email}:null}const ko=`
<div class="layout">
<aside class="sidebar">
...
`;function Eo(w){const v={k:"verdict",dir:1},E={k:"created_at",dir:1},i={rows:[],sort:{...v},openId:null,stats:null,profile:null,view:"companies",jobs:[],jsort:{...E},applicationStages:["applied","screening","interview","offer","rejected"],outreachStatuses:["initial contact","no response","replied","followed up"],followupInterval:5,followupTemplate:"",openDetail:null,anthropicKey:null,gmail:null,notifications:{notifications:[],unread:0,followups:[]}},g=e=>"pill pill-"+(e||"none"),$='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 14V2.5"/><path d="M3.5 2.5c3-1.2 6 1.2 9 0V9c-3 1.2-6-1.2-9 0z"/></svg>',c=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),C=e=>/^https?:\/\//i.test(String(e??""))?c(e):"#";async function q(){const t=await(await fetch("/api/companies")).json();i.rows=t.rows||[],G()}async function k(){let e;try{const t=await fetch("/api/postings");if(!t.ok)return;e=await t.json()}catch{return}i.jobs=e.rows||[],A(),V(),we()}function V(){if(!m.postingId)return;const e=i.jobs.find(t=>t.posting_id===m.postingId);e&&(m.row=e,document.getElementById("pursuit-pane").classList.contains("open")&&oe())}let ne=null;function we(){const e=i.jobs.some(t=>t.outreach_draft_status==="researching");e&&!ne?ne=setInterval(je,4e3):!e&&ne&&(clearInterval(ne),ne=null)}async function je(){let e;try{const a=await fetch("/api/postings");if(!a.ok)return;e=await a.json()}catch{return}const t=e.rows||[],n=new Map(i.jobs.map(a=>[a.posting_id,a.outreach_draft_status])),s=t.some(a=>n.get(a.posting_id)!==a.outreach_draft_status)||t.length!==i.jobs.length;i.jobs=t,s&&(A(),V()),we()}async function M(){await Promise.all([fetch("/api/application-stages").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.applicationStages=e.statuses)}).catch(()=>{}),fetch("/api/outreach-statuses").then(e=>e.ok?e.json():null).then(e=>{e&&Array.isArray(e.statuses)&&e.statuses.length&&(i.outreachStatuses=e.statuses)}).catch(()=>{}),fetch("/api/followup-interval").then(e=>e.ok?e.json():null).then(e=>{e&&Number.isInteger(e.days)&&(i.followupInterval=e.days)}).catch(()=>{}),fetch("/api/followup-template").then(e=>e.ok?e.json():null).then(e=>{e&&typeof e.content=="string"&&(i.followupTemplate=e.content)}).catch(()=>{})]),nn(),i.view==="jobs"&&A()}function O(e,{render:t=!0}={}){i.view=e;try{localStorage.setItem("scout-view",e)}catch{}document.getElementById("tab-companies").classList.toggle("active",e==="companies"),document.getElementById("tab-jobs").classList.toggle("active",e==="jobs"),document.getElementById("companies-view").style.display=e==="companies"?"":"none",document.getElementById("jobs-view").style.display=e==="jobs"?"":"none",document.getElementById("block-filter-companies").style.display=e==="companies"?"":"none",document.getElementById("block-filter-jobs").style.display=e==="jobs"?"":"none",Yt(),t&&(e==="jobs"?A():G())}async function S(){let e;try{const t=await fetch("/api/stats");if(!t.ok)throw new Error(`HTTP ${t.status}`);e=await t.json()}catch(t){console.warn(`stats failed: ${t.message}`);return}i.stats=e,se()}function se(){te()}function tt(e,t,n){const s=e[n]??"",a=t[n]??"";if(n==="headcount")return(s|0)-(a|0);if(n==="verdict"){const o={yes:0,maybe:1,no:2,"":3};return(o[s]??3)-(o[a]??3)}return String(s).localeCompare(String(a))}function Me(e){return e.slice().sort((t,n)=>i.sort.dir*tt(t,n,i.sort.k))}function ke(e,t,n){document.querySelectorAll(`#${e} thead th[${t}]`).forEach(s=>{s.getAttribute(t)===n.k?s.dataset.sort=n.dir<0?"desc":"asc":delete s.dataset.sort})}const F=new Set;let re=!1,le=!1;const cs=[["yes","yes","fdrop-dot--yes"],["maybe","maybe","fdrop-dot--maybe"],["no","no","fdrop-dot--no"],["__none__","unscored","fdrop-dot--none"]];function rs(){const e=document.getElementById("fdrop-cfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Verdict</div>'+cs.map(([t,n,s])=>Y("data-v",t,n,s,F.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Flags</div>'+Y("data-toggle","flagged","⚑ Flagged","",re)+Y("data-toggle","enriched","Enriched","",le),zt())}function zt(){const e={yes:0,maybe:0,no:0,__none__:0};let t=0,n=0;for(const r of i.rows){const d=r.verdict||"__none__";e[d]=(e[d]|0)+1,r.flagged&&t++,r.enriched&&n++}at("#fdrop-cfilters-menu [data-v]","data-v",e);const s=document.querySelector('#fdrop-cfilters-menu [data-toggle="flagged"] [data-count]');s&&(s.textContent=t||"");const a=document.querySelector('#fdrop-cfilters-menu [data-toggle="enriched"] [data-count]');a&&(a.textContent=n||"");const o=F.size+(re?1:0)+(le?1:0);an("fdrop-cfilters-btn",o,o>0)}function Jt(){const e=document.getElementById("q").value.trim().toLowerCase();return i.rows.filter(t=>!(F.size&&!F.has(t.verdict||"__none__")||re&&!t.flagged||le&&!t.enriched||e&&!(t.name+" "+(t.vertical||"")+" "+(t.reason||"")).toLowerCase().includes(e)))}const ls=[{k:"flag",label:"flag"},{k:"verdict",label:"verdict"},{k:"reason",label:"reason"},{k:"vertical",label:"vertical"},{k:"location",label:"location"},{k:"hc",label:"hc"},{k:"stage",label:"stage"},{k:"reviewed",label:"reviewed"},{k:"site",label:"site"}],ds=[{k:"application",label:"application"},{k:"outreach",label:"outreach"},{k:"last_outreach",label:"last outreach"},{k:"contacts",label:"contacts"},{k:"link",label:"link"}];function Gt(e){try{return new Set(JSON.parse(localStorage.getItem(e)||"[]"))}catch{return new Set}}const Kt=Gt("scout-hidden-cols"),Wt=Gt("scout-hidden-jcols");function nt(){return i.view==="jobs"?{cols:ds,hidden:Wt,key:"scout-hidden-jcols"}:{cols:ls,hidden:Kt,key:"scout-hidden-cols"}}function de(){document.querySelectorAll("#t [data-col]").forEach(e=>{e.style.display=Kt.has(e.dataset.col)?"none":""}),document.querySelectorAll("#jt [data-col]").forEach(e=>{e.style.display=Wt.has(e.dataset.col)?"none":""})}function Yt(){const e=nt(),t=document.getElementById("fdrop-columns-menu");t&&(t.innerHTML='<div class="fdrop-head">Visible columns</div>'+e.cols.map(n=>Y("data-col",n.k,n.label,"",!e.hidden.has(n.k))).join(""),Zt())}function Zt(){const e=nt(),t=e.cols.filter(s=>e.hidden.has(s.k)).length,n=document.querySelector("#fdrop-columns-btn .fdrop-count");n&&(n.textContent=t||"",n.style.display=t?"":"none")}function Qt(e){return`
      ...
      <td data-col="verdict"><span class="${g(e.verdict)}">${c(e.verdict||"—")}</span></td>
      ...
      <td data-col="site">${e.website_url?`<a href="${C(e.website_url)}" target="_blank" rel="noopener" title="open website" aria-label="open website">↗</a>`:""}</td>
    `}function Xt(e){const t=e.querySelector(".flag-btn");t&&t.addEventListener("click",()=>Mn(t.dataset.id))}const us=[["flag","14px"],["verdict","46px"],[null,"62%"],["reason","85%"],["vertical","70%"],["location","60%"],["hc","26px"],["stage","55%"],["reviewed","44px"],["site","38px"]],ps=[[null,"72%"],["applied","58px"],["response","54px"],["outreach","22px"],["last_outreach","58px"],["contacts","55%"],["link","32px"]];function en(e,t,n=7){const s=document.querySelector(e);if(!s)return;const a=[1,.82,.7,.95,.76,.9,.85];let o="";for(let r=0;r<n;r++){const d=t.map(([u,p])=>{const h=p.endsWith("%")?Math.round(parseFloat(p)*a[r%a.length])+"%":p;return`<td${u?` data-col="${u}"`:""}><span class="skel" style="width:${h}"></span></td>`}).join("");o+=`<tr class="skel-row" aria-hidden="true">${d}</tr>`}s.innerHTML=o,de()}function G(){const e=document.querySelector("#t tbody");e.innerHTML="";const t=Me(Jt());zt(),document.getElementById("empty").style.display=t.length?"none":"block";for(const n of t){const s=document.createElement("tr");s.dataset.id=n.company_id,s.innerHTML=Qt(n),s.addEventListener("click",a=>{a.target.closest("a, .flag-btn")||ve(s.dataset.id)}),e.appendChild(s),Xt(s)}ke("t","data-k",i.sort),de()}async function ms(e){const n=await(await fetch("/api/companies")).json();i.rows=n.rows||[];const s=document.querySelector("#t tbody"),a=Me(Jt()).map(r=>r.company_id),o=[...s.querySelectorAll("tr")].map(r=>r.dataset.id);if(a.length!==o.length||a.some((r,d)=>r!==o[d])){G();return}for(const r of e){const d=i.rows.find(p=>p.company_id===r),u=s.querySelector(`tr[data-id="${CSS.escape(r)}"]`);if(!d||!u){G();return}u.innerHTML=Qt(d),Xt(u)}de()}let _=null,st=null,ue=!1,pe=!1;const W=new Set;function tn(){const e=i.applicationStages;if(_===null)_=new Set(["",...e.filter(t=>t!=="rejected")]);else{for(const t of[..._])t!==""&&!e.includes(t)&&_.delete(t);if(st)for(const t of e)t!=="rejected"&&!st.has(t)&&_.add(t)}st=new Set(e)}function hs(){tn();const e=document.getElementById("jq").value.trim().toLowerCase();return i.jobs.filter(t=>{const n=t.application_status||"";return!(!_.has(n)||ue&&!t.next_up||pe&&!(t.followups_due|0)||W.size&&!W.has(t.outreach_status||"")||e&&!(t.title+" "+t.company+" "+(t.location||"")+" "+(t.description||"")+" "+(t.contacts||"")).toLowerCase().includes(e))})}function Y(e,t,n,s,a){return`<button class="fdrop-item${a?" is-checked":""}" ${e}="${c(t)}" role="menuitemcheckbox" aria-checked="${a}"><span class="fdrop-check" aria-hidden="true"></span>`+(s?`<span class="fdrop-dot ${s}"></span>`:"")+`<span class="fdrop-label">${c(n)}</span><span class="fdrop-item-count" data-count></span></button>`}function nn(){tn();const e=document.getElementById("fdrop-jfilters-menu");e&&(e.innerHTML='<div class="fdrop-head">Application stage</div>'+Y("data-stage","","not applied","",_.has(""))+i.applicationStages.map(t=>Y("data-stage",t,t,He(t),_.has(t))).join("")+'<div class="fdrop-sep"></div><div class="fdrop-head">Outreach queue</div>'+Y("data-toggle","nextup","★ Next up","",ue)+'<div class="fdrop-sep"></div><div class="fdrop-head">Reply status</div>'+[["","none",""]].concat(i.outreachStatuses.map(t=>[t,t,un(t)])).map(([t,n,s])=>Y("data-status",t,n,s,W.has(t))).join(""),sn())}function sn(){const e={},t={};let n=0;for(const r of i.jobs){const d=r.application_status||"";e[d]=(e[d]|0)+1;const u=r.outreach_status||"";t[u]=(t[u]|0)+1,r.next_up&&n++}at("#fdrop-jfilters-menu [data-stage]","data-stage",e),at("#fdrop-jfilters-menu [data-status]","data-status",t),fs("nextup",n);const s=["",...i.applicationStages.filter(r=>r!=="rejected")],o=(_&&_.size===s.length&&s.every(r=>_.has(r))?0:_?_.size:0)+(ue?1:0)+W.size;an("fdrop-jfilters-btn",o,o>0)}function at(e,t,n){document.querySelectorAll(e).forEach(s=>{const a=s.querySelector("[data-count]");if(a){const o=n[s.getAttribute(t)]|0;a.textContent=o||""}})}function fs(e,t){const n=document.querySelector(`#fdrop-jfilters-menu [data-toggle="${e}"] [data-count]`);n&&(n.textContent=t||"")}function an(e,t,n){const s=document.getElementById(e);if(!s)return;s.classList.toggle("is-active",n);const a=s.querySelector(".fdrop-count");if(a){const o=n&&t>0;a.textContent=o?t:"",a.style.display=o?"":"none"}}function ae(e,t){e.classList.toggle("is-checked",t),e.setAttribute("aria-checked",String(t))}function ot(){document.querySelectorAll(".fdrop.is-open").forEach(e=>{e.classList.remove("is-open");const t=e.querySelector(".fdrop-btn");t&&t.setAttribute("aria-expanded","false")})}function on(e){const t=e.querySelector(".fdrop-btn"),n=e.querySelector(".fdrop-menu");if(!t||!n)return;const s=t.getBoundingClientRect();n.style.left=Math.round(s.left)+"px",n.style.top=Math.round(s.bottom+4)+"px",n.style.minWidth=Math.round(s.width)+"px",n.style.maxHeight=Math.max(160,Math.round(window.innerHeight-s.bottom-12))+"px"}function ys(e){const t=e.querySelector(".fdrop-btn");e.classList.add("is-open"),t&&t.setAttribute("aria-expanded","true"),on(e)}function cn(){const e=document.querySelector(".fdrop.is-open");e&&on(e)}window.addEventListener("scroll",cn,!0),window.addEventListener("resize",cn);const gs=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;function vs(e){if(e=String(e||"").trim(),!e)return[];if(e[0]==="[")try{const t=JSON.parse(e);if(Array.isArray(t))return t.map(n=>({position:String((n==null?void 0:n.position)||"").trim(),email:String((n==null?void 0:n.email)||"").trim()})).filter(n=>n.position||n.email)}catch{}return e.split(",").map(t=>t.trim()).filter(Boolean).map(t=>{const n=t.match(gs),s=n?n[0]:"";let a=s?t.replace(s,""):t;return a=a.replace(/[<>()]/g,"").replace(/[\s:–—-]+$/,"").trim(),{position:a,email:s}})}const Ae=()=>new Date().toISOString().slice(0,10);function rn(e){const t=[["","none"]];for(const n of i.applicationStages)t.push([n,n]);return e&&!i.applicationStages.includes(e)&&t.push([e,e+" (removed)"]),t}function ln(e){const t=[["","none"]];for(const n of i.outreachStatuses)t.push([n,n]);return e&&!i.outreachStatuses.includes(e)&&t.push([e,e+" (removed)"]),t}const bs=8;function dn(e,t){const n=(t||[]).indexOf(e);return n<0?"":"sc-"+n%bs}function He(e){return dn(e,i.applicationStages)}function un(e){return dn(e,i.outreachStatuses)}function ws(e){const t=vs(e);return t.length?t.map(n=>{const s=c(n.position||n.email);if(!n.email)return s;const a=c(n.position?`${n.position} — ${n.email}`:n.email);return`<a href="mailto:${c(n.email)}" title="${a}">${s}</a>`}).join('<span class="dim">, </span>'):'<span class="dim">—</span>'}function pn(e){const t=e.application_status||"";if(!t)return i.applicationStages.length+1;const n=i.applicationStages.indexOf(t);return n<0?i.applicationStages.length:n}function ks(e,t,n){if(n==="verdict"){const s={yes:0,maybe:1,no:2,"":3};return(s[e.verdict]??3)-(s[t.verdict]??3)}if(n==="application")return pn(e)-pn(t);if(n==="followups_due")return(t.followups_due|0)-(e.followups_due|0);if(n==="created_at"||n==="last_outreach_at"){const s=e[n]||"",a=t[n]||"";return!s&&!a?0:s?a?String(a).localeCompare(String(s)):-i.jsort.dir:i.jsort.dir}return String(e[n]??"").localeCompare(String(t[n]??""))}function Es(){const e=document.getElementById("jobs-followup-nav");if(!e)return;const t=i.jobs.reduce((n,s)=>n+(s.followups_due|0),0);if(!t){e.style.display="none",pe=!1;return}e.style.display="",e.innerHTML=`<button class="followup-nav-btn${pe?" is-active":""}" title="${pe?"showing only these — click to show all jobs":"show only jobs owing a follow-up"}"><span class="fn-icon">${Mt}</span><span class="fn-text"><strong>${t}</strong> follow-up${t>1?"s":""} due</span></button>`,e.querySelector(".followup-nav-btn").onclick=()=>{pe=!pe,A()}}function A(){const e=document.querySelector("#jt tbody");e.innerHTML="",Es();const t=hs().sort((a,o)=>i.jsort.dir*ks(a,o,i.jsort.k));document.getElementById("jobs-empty").style.display=t.length?"none":"block",sn();const n=_&&!_.has("rejected")?i.jobs.filter(a=>(a.application_status||"")==="rejected").length:0,s=document.getElementById("jobs-hidden-note");s.style.display=n?"":"none",n&&(s.innerHTML=`${n} rejected application${n>1?"s":""} hidden — <a id="show-rejected-link">show</a>`,document.getElementById("show-rejected-link").onclick=()=>{_.add("rejected"),nn(),A()});for(const a of t){const o=a.application_status||"",r=document.createElement("tr");r.dataset.id=a.posting_id;const d=rn(o).map(([h,f])=>`<option value="${c(h)}"${o===h?" selected":""}>${c(f)}</option>`).join(""),u=a.outreach_status||"",p=ln(u).map(([h,f])=>`<option value="${c(h)}"${u===h?" selected":""}>${c(f)}</option>`).join("");r.innerHTML=`
      <td><div class="jt-namecell"><button class="jt-nextup${a.next_up?" is-on":""}" title="${a.next_up?"queued next up for outreach — click to remove":"mark next up for outreach"}" aria-label="next up"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 12.5v-9M4.5 7L8 3.5 11.5 7"/></svg></button><div class="jt-namecol"><span class="row-name">${c(a.title||a.company)}</span>${$s(a.outreach_draft_status)}${a.title?`<div class="small dim">${c(a.company)}</div>`:""}</div></div></td>
      <td data-col="application"><div class="jt-stage"><select class="jt-stage-sel ${He(o)}" title="application stage">${d}</select></div></td>
      <td class="small" data-col="outreach"><div class="jt-out"><select class="jt-ostatus ${un(u)}" title="outreach reply status">${p}</select>${a.followups_due?`<span class="followup-badge" title="${a.followups_due} follow-up${a.followups_due>1?"s":""} due — open to act">${Mt}${a.followups_due}</span>`:""}</div></td>
      <td class="small" data-col="last_outreach">${a.last_outreach_at?c(a.last_outreach_at):'<span class="dim">—</span>'}</td>
      <td class="small td-contacts" data-col="contacts">${ws(a.contacts)}</td>
      <td data-col="link"><a href="${C(a.url)}" target="_blank" rel="noopener" title="open posting" aria-label="open posting">↗</a></td>
    `,r.querySelector(".jt-nextup").onclick=()=>wn(a,!1),r.querySelector(".jt-stage-sel").onchange=h=>$n(a,{application_status:h.target.value}),r.querySelector(".jt-ostatus").onchange=h=>$n(a,{outreach_status:h.target.value}),e.appendChild(r)}ke("jt","data-jk",i.jsort),de(),e.querySelectorAll("tr").forEach(a=>{a.addEventListener("click",o=>{o.target.closest("a, button, select")||it(a.dataset.id)})})}function $s(e){return e==="researching"?'<span class="draft-badge db-researching" title="drafting outreach…"><span class="spinner spinner-xs"></span>drafting</span>':e==="awaiting_review"?'<span class="draft-badge" title="an outreach draft is ready to review">draft ready</span>':e==="no_hook"?'<span class="draft-badge db-nohook" title="no honest hook — scout recommends not emailing">no hook</span>':""}const m={postingId:null,row:null,drafts:[],poll:null,openHist:!1,answers:[],answersStatus:"",answersPoll:null,detecting:!1,contacts:[],outreach:[],contactsLoaded:!1};async function it(e){let t=i.jobs.find(n=>n.posting_id===e);if(t||(await k(),t=i.jobs.find(n=>n.posting_id===e)),!t){l("posting not found — refresh");return}rt(),mt(),m.postingId=e,m.row=t,m.drafts=[],m.openHist=!1,m.answers=[],m.detecting=!1,m.contacts=[],m.outreach=[],m.contactsLoaded=!1,m.answersStatus=t.questions_status||"",document.getElementById("pursuit-pane").classList.add("open"),document.getElementById("pursuit-scrim").classList.add("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","false"),hn("pursuit"),oe(),me(),ge(),mn()}async function mn(){const e=m.postingId,t=m.row&&m.row.company_id;if(!(!e||!t)){try{const[n,s]=await Promise.all([fetch(`/api/companies/${t}/contacts`).then(a=>a.ok?a.json():[]),fetch(`/api/postings/${e}/outreach-log`).then(a=>a.ok?a.json():[])]);if(m.postingId!==e)return;m.contacts=Array.isArray(n)?n:[],m.outreach=Array.isArray(s)?s:[]}catch{}m.contactsLoaded=!0,fe()}}let ct=null;function hn(e){ct=e;const t=e==="company";document.getElementById("scrim").style.zIndex=t?"54":"52",document.getElementById("pane").style.zIndex=t?"55":"53",document.getElementById("pursuit-scrim").style.zIndex=t?"52":"54",document.getElementById("pursuit-pane").style.zIndex=t?"53":"55"}function Ee(){rt(),mt(),m.postingId=null,m.row=null,m.drafts=[],m.answers=[],m.answersStatus="",document.getElementById("pursuit-pane").classList.remove("open"),document.getElementById("pursuit-scrim").classList.remove("open"),document.getElementById("pursuit-pane").setAttribute("aria-hidden","true")}function rt(){m.poll&&(clearInterval(m.poll),m.poll=null)}async function me(){if(!m.postingId)return;let e;try{const n=await fetch(`/api/postings/${m.postingId}/outreach`);if(!n.ok){fe();return}e=await n.json()}catch{fe();return}m.drafts=e.drafts||[],fe();const t=m.drafts[0];t&&t.status==="researching"?Is():rt()}function Is(){m.poll||(m.poll=setInterval(me,4e3))}function z(e,t,{multiline:n=!1}={}){if(!e)return;let s=e.value;e.addEventListener("focus",()=>{s=e.value}),e.addEventListener("keydown",a=>{a.key==="Escape"?(a.preventDefault(),e.value=s,e.blur()):a.key==="Enter"&&(!n||a.metaKey||a.ctrlKey)&&(a.preventDefault(),e.blur())}),e.addEventListener("blur",async()=>{const a=e.value.trim();if(a===s.trim()){e.value=s;return}e.classList.remove("is-saved","is-error"),e.classList.add("is-saving");try{await t(a),s=e.value,e.classList.remove("is-saving"),e.classList.add("is-saved"),setTimeout(()=>e.classList.remove("is-saved"),1200)}catch(o){e.value=s,e.classList.remove("is-saving"),e.classList.add("is-error"),setTimeout(()=>e.classList.remove("is-error"),1600),l(`save failed: ${o.message}`)}})}async function fn(e,t,n){const s={title:e.title||"",location:e.location||"",comp_range:e.comp_range||"",employment_type:e.employment_type||"",workplace_type:e.workplace_type||"",department:e.department||"",description:e.description||"",[t]:n},a=await fetch(`/api/postings/${e.posting_id}/details`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{title:o.title,location:o.location,employment_type:o.employment_type,workplace_type:o.workplace_type,department:o.department,comp_range:o.comp_range,description:o.description}),A(),be(e.posting_id,{title:o.title,location:o.location})}async function xs(e,t){const n=await fetch(`/api/postings/${e.posting_id}/url`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.url=s.url;const a=document.querySelector("#role-body .role-url-open");a&&a.setAttribute("href",C(e.url)),be(e.posting_id,{url:s.url})}async function _s(e,t){if(t.disabled)return;const n=t.textContent;t.disabled=!0,t.textContent="re-enriching…";let s;try{s=await fetch(`/api/postings/${e.posting_id}/recapture`,{method:"POST"})}catch(o){t.disabled=!1,t.textContent=n,l(`re-enrich failed: ${o.message}`);return}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();let r=o||"HTTP "+s.status;try{r=JSON.parse(o).error||r}catch{}t.disabled=!1,t.textContent=n,l(`re-enrich failed: ${r}`);return}const a=await s.json();Object.assign(e,{title:a.title,location:a.location,employment_type:a.employment_type,workplace_type:a.workplace_type,department:a.department,comp_range:a.comp_range,description:a.description,posted_at:a.posted_at,url:a.url,questions_status:a.questions_status}),A(),oe(),be(e.posting_id,{title:a.title,location:a.location,url:a.url}),l("re-enriched from the posting link")}function Ls(e){const t=document.getElementById("pursuit-company-edit");t&&t.addEventListener("click",()=>Ts(e))}async function Bs(e,t){const n=await fetch(`/api/postings/${e.posting_id}/company`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({company_id:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.company_id=s.company_id,e.company=s.company_name,oe(),k()}let $e=null;function Ts(e){$e=e;const t=document.getElementById("relink-current");t&&(t.textContent=e.company?`currently: ${e.company}`:"");const n=document.getElementById("relink-search");n&&(n.value=""),yn(""),document.getElementById("relink-scrim").classList.add("open"),n&&n.focus()}function he(){document.getElementById("relink-scrim").classList.remove("open"),$e=null}let lt=null;function Ss(e){lt=e;const t=(e.postings||[]).length,n=t?` and its ${t} job ${t===1?"posting":"postings"}`:"",s=document.getElementById("delcompany-summary");s&&(s.innerHTML=`Delete <strong>${c(e.name||"this company")}</strong>${n}?`);const a=document.getElementById("delcompany-confirm");a&&(a.disabled=!1),document.getElementById("delcompany-scrim").classList.add("open")}function Pe(){document.getElementById("delcompany-scrim").classList.remove("open"),lt=null}async function Cs(){const e=lt;if(!e)return;const t=document.getElementById("delcompany-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e.company_id}`,{method:"DELETE"})}catch(a){l(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");l(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=e.name||"company";Pe(),i.openId===e.company_id&&_e(),q(),k(),S(),l(`deleted ${s}`)}let dt=null;function js(e){dt=e;const t=(e.title||"").trim()||"this posting",n=e.company?` at <strong>${c(e.company)}</strong>`:"",s=document.getElementById("deljob-summary");s&&(s.innerHTML=`Delete <strong>${c(t)}</strong>${n}?`);const a=document.getElementById("deljob-confirm");a&&(a.disabled=!1),document.getElementById("deljob-scrim").classList.add("open")}function qe(){document.getElementById("deljob-scrim").classList.remove("open"),dt=null}async function Ms(){const e=dt;if(!e)return;const t=document.getElementById("deljob-confirm");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${e.posting_id}`,{method:"DELETE"})}catch(a){l(`delete failed: ${a.message}`),t&&(t.disabled=!1);return}if(!n.ok){const a=await n.text().catch(()=>"");l(`delete failed: HTTP ${n.status}${a?" — "+a:""}`),t&&(t.disabled=!1);return}const s=(e.title||"").trim()||"posting";qe(),Ee(),k(),i.openId===e.company_id&&ve(e.company_id),l(`deleted ${s}`)}function yn(e){const t=document.getElementById("relink-results");if(!t)return;const n=e.trim().toLowerCase();let s=(i.rows||[]).slice();if(n?(s=s.filter(o=>(o.name||"").toLowerCase().includes(n)),s.sort((o,r)=>{const d=(o.name||"").toLowerCase().startsWith(n)?0:1,u=(r.name||"").toLowerCase().startsWith(n)?0:1;return d-u||(o.name||"").localeCompare(r.name||"")})):s.sort((o,r)=>(o.name||"").localeCompare(r.name||"")),s=s.slice(0,60),!s.length){t.innerHTML=`<div class="relink-empty">${(i.rows||[]).length?"no company matches":"no companies yet — Add one first"}</div>`;return}const a=$e?$e.company_id:"";t.innerHTML=s.map(o=>{const r=o.company_id===a,d=[o.vertical,o.location].filter(Boolean).map(c).join(" · ");return`<button type="button" class="relink-result${r?" is-current":""}"
        data-id="${o.company_id}"${r?" disabled":""}>
        <span class="rr-main">
          <span class="rr-name">${c(o.name||"—")}</span>
          ${d?`<span class="rr-sub">${d}</span>`:""}
        </span>
        <span class="${g(o.verdict)} rr-verdict">${c(o.verdict||"—")}</span>
        ${r?'<span class="rr-current-tag">current</span>':""}
      </button>`}).join("")}async function gn(e){const t=$e;if(!t){he();return}if(e===t.company_id){he();return}try{await Bs(t,e),he(),l(`moved to ${t.company}`)}catch(n){l(`move failed: ${n.message}`)}}async function vn(e,t,n){const s={name:e.name||"",headcount:e.headcount||"",funding_stage:e.funding_stage||"",location:e.location||"",vertical:e.vertical||"",[t]:n};if(!String(s.name).trim())throw new Error("name is required");const a=await fetch(`/api/companies/${e.company_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)});if(!a.ok)throw new Error((await a.text().catch(()=>"")).trim()||"HTTP "+a.status);const o=await a.json();Object.assign(e,{name:o.name,headcount:o.headcount,funding_stage:o.funding_stage,location:o.location,vertical:o.vertical}),q(),k()}async function As(e,t){const n=await fetch(`/api/companies/${e.company_id}/domain`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({website:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();i.openId=s.company_id,Le(s),Be(s.company_id),q(),k()}async function Hs(e,t){const n=await fetch(`/api/companies/${e.company_id}/notes`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({notes:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json();e.notes=s.notes}let bn=null;function oe(){const e=m.row;if(!e)return;const t=document.getElementById("pursuit-body"),s=!!t&&bn===e.posting_id&&document.getElementById("pursuit-pane").classList.contains("open")&&t?t.scrollTop:0;document.getElementById("pursuit-title").innerHTML=`<input class="ie ie-title" id="pursuit-title-input" placeholder="role name" value="${c(e.title||"")}">`;const a=e.application_status||"";document.getElementById("pursuit-pills").innerHTML=`<span class="pill ${a?He(a)||"pill-stage":"pill-none"}">${c(a||"—")}</span>`+(e.verdict?` <span class="${g(e.verdict)}">${c(e.verdict)}</span>`:"");const o=document.getElementById("pursuit-chat");o&&(o.style.display=i.meta&&i.meta.chat?"":"none",o.onclick=()=>Dt("posting",e.posting_id,e.title||e.company)),document.getElementById("pursuit-body").innerHTML=`
      ...
      <div class="pipeline-grid">
        <div class="pipeline-row">
          ...
          <select class="input pl-appstatus" title="application stage">
            ${rn(e.application_status||"").map(([p,h])=>`<option value="${c(p)}"${(e.application_status||"")===p?" selected":""}>${c(h)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
          ...
          <select class="input pl-ostatus" title="outreach reply status — separate from the application stage">
            ${ln(e.outreach_status||"").map(([p,h])=>`<option value="${c(p)}"${(e.outreach_status||"")===p?" selected":""}>${c(h)}</option>`).join("")}
          </select>
        </div>
        <div class="pipeline-row">
  ...
  `,qs();const r=document.getElementById("pursuit-company-link");r&&r.addEventListener("click",()=>ve(e.company_id)),Ls(e),z(document.getElementById("pursuit-title-input"),p=>fn(e,"title",p)),z(document.getElementById("pursuit-url-input"),p=>xs(e,p));const d=document.getElementById("pursuit-reenrich");d&&d.addEventListener("click",()=>_s(e,d)),z(document.getElementById("pursuit-notes-input"),p=>Os(p),{multiline:!0}),document.querySelectorAll("#role-body [data-k]").forEach(p=>z(p,h=>fn(e,p.dataset.k,h),{multiline:p.tagName==="TEXTAREA"}));const u=document.getElementById("job-delete-btn");u&&u.addEventListener("click",()=>js(e)),fe(),xe(),t&&(t.scrollTop=s),bn=e.posting_id}function Ps(e){return`
    <div class="role-url ie-field">
      <div class="role-url-head">
        <label>link</label>
        <a class="role-url-open" href="${C(e.url)}" target="_blank" rel="noopener" title="open the posting">↗</a>
    ...
    </div>`}function qs(){const e=document.querySelector("#pursuit-body .pl-appstatus");e&&e.addEventListener("change",s=>En({application_status:s.target.value}));const t=document.querySelector("#pursuit-body .pl-ostatus");t&&t.addEventListener("change",s=>En({outreach_status:s.target.value}));const n=document.querySelector("#pursuit-body .pt-nextup");n&&n.addEventListener("click",()=>wn(m.row,!0))}async function wn(e,t){let n;try{n=await fetch(`/api/postings/${e.posting_id}/next-up`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({next_up:!e.next_up})})}catch(a){l(`save failed: ${a.message}`);return}if(!n.ok){const a=(await n.text().catch(()=>"")).trim();l(`save failed: ${a||"HTTP "+n.status}`);return}const s=await n.json();e.next_up=s.next_up,A(),be(e.posting_id,{next_up:s.next_up}),t&&oe(),l(e.next_up?"queued next up":"removed from the queue")}async function kn(e,t){const n={application_status:e.application_status||"",outreach_status:e.outreach_status||"",notes:e.notes||"",...t};let s;try{s=await fetch(`/api/postings/${e.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})}catch(o){return l(`save failed: ${o.message}`),null}if(!s.ok){const o=(await s.text().catch(()=>"")).trim();return l(`save failed: ${o||"HTTP "+s.status}`),null}const a=await s.json();return Object.assign(e,{application_status:a.application_status,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,outreach_status:a.outreach_status,contacts:a.contacts,notes:a.notes,next_up:a.next_up}),be(e.posting_id,{application_status:a.application_status,outreach_count:a.outreach_count,last_outreach_at:a.last_outreach_at,next_up:a.next_up}),a}async function Os(e){const t=m.row,n={application_status:t.application_status||"",outreach_status:t.outreach_status||"",notes:e},s=await fetch(`/api/postings/${t.posting_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!s.ok)throw new Error((await s.text().catch(()=>"")).trim()||"HTTP "+s.status);const a=await s.json();t.notes=a.notes,A()}async function En(e){await kn(m.row,e)&&(A(),oe(),l("tracking saved"))}async function $n(e,t){await kn(e,t)&&(A(),m.postingId===e.posting_id&&(m.row=e,oe()),l("tracking saved"))}function fe(){const e=document.getElementById("outreach-section");if(!e)return;const t=m.drafts,n=t[0]||null,s=t.slice(1),o=n&&(zs(n.status)||n.status==="failed")?"":`<button class="btn btn-primary" id="draft-start-btn">${n?"Draft again":"Draft outreach"}</button><label class="draft-skip-research" title="Skip the web-research stage — write straight from the template; the opener becomes a plain intro."><input type="checkbox" id="draft-skip-research"> skip research</label>`,r=s.length?`
    ...
    </details>`:"";e.innerHTML=Rs()+`<div class="outreach-drafts-head">Drafts</div><div id="draft-current">${n?Ln(n,!1):""}</div><div class="draft-actions">${o}</div>`+r,Fs(),Ys()}function Ns(e,t){const n=m.row||{},s={company:n.company||"",role:n.title||"",contact_name:e&&e.name||"",contact_role:e&&e.role||"",last_sent:t&&t.sent_at||"",last_message:t&&t.body||""};return(i.followupTemplate||"").replace(/\{\{\s*([A-Za-z][A-Za-z0-9_]*)\s*\}\}/g,(a,o)=>o in s?s[o]:a)}function Rs(){const e=m.row,t=e.last_outreach_at?`<div class="outreach-meta"><span>last outreach ${c(e.last_outreach_at)}</span></div>`:"";if(!m.contactsLoaded)return`<div class="contacts-mgr">${t}<div class="loading-row"><span class="spinner"></span><span>loading contacts…</span></div></div>`;const n=m.contacts.map(Ds).join(""),s=m.contacts.length?"":`<div class="cc-empty dim">No contacts yet — add the people you're reaching out to at ${c(e.company)}.</div>`;return`<div class="contacts-mgr">
    ${t}
  ...
  </div>`}function Ds(e){const t=m.outreach.filter(o=>o.contact_id===e.id),n=t[0]||null,s=e.role?`<span class="cc-role">${c(e.role)}</span>`:"",a=e.email?`<a class="cc-mail" href="mailto:${c(e.email)}" title="${c(e.email)}">${c(e.email)}</a>`:"";return`<div class="contact-card" data-cid="${e.id}">
    <div class="cc-head">
    ...
    ${t.length?`<details class="cc-history"><summary>${t.length} send${t.length>1?"s":""}</summary><div class="cc-entries">${t.map(Us).join("")}</div></details>`:""}
      ...
      ${n}
    </div>`}async function ie(e,t,n){let s;try{s=await fetch(t,{method:e,headers:n?{"Content-Type":"application/json"}:{},body:n?JSON.stringify(n):void 0})}catch(a){return l(`save failed: ${a.message}`),null}if(!s.ok){const a=(await s.text().catch(()=>"")).trim();return l(`save failed: ${a||"HTTP "+s.status}`),null}try{return await s.json()}catch{return{}}}async function ye(){await k(),await mn()}function Fs(){const e=document.getElementById("outreach-section");if(!e)return;const t=m.postingId,n=e.querySelector(".cc-addwrap");if(n){const s=n.querySelector(".cc-addform");n.querySelector(".cc-addbtn").addEventListener("click",()=>{s.style.display="",n.querySelector(".cc-addbtn").style.display="none",s.querySelector(".cc-f-name").focus()}),n.querySelector(".cc-f-cancel").addEventListener("click",()=>fe()),n.querySelector(".cc-f-save").addEventListener("click",async()=>{const a={name:s.querySelector(".cc-f-name").value,role:s.querySelector(".cc-f-role").value,email:s.querySelector(".cc-f-email").value};await ie("POST",`/api/companies/${m.row.company_id}/contacts`,a)&&(l("contact added"),ye())})}e.querySelectorAll(".contact-card").forEach(s=>{const a=s.dataset.cid,o=s.querySelector(".cc-editform");s.querySelector(".cc-edit").addEventListener("click",()=>{o.style.display=o.style.display==="none"?"":"none",o.style.display!=="none"&&o.querySelector(".cc-e-name").focus()});const r=s.querySelector(".cc-e-cancel");r&&r.addEventListener("click",()=>{o.style.display="none"});const d=s.querySelector(".cc-e-save");d&&d.addEventListener("click",async()=>{const x={name:o.querySelector(".cc-e-name").value,role:o.querySelector(".cc-e-role").value,email:o.querySelector(".cc-e-email").value};await ie("PUT",`/api/contacts/${a}`,x)&&(l("contact saved"),ye())}),s.querySelector(".cc-arch").addEventListener("click",async()=>{await ie("DELETE",`/api/contacts/${a}`)&&(l("contact removed"),ye())});const u=s.querySelector(".cc-logform"),p=s.querySelector(".cc-log");p&&p.addEventListener("click",()=>{u.style.display=u.style.display==="none"?"":"none",u.style.display!=="none"&&u.querySelector(".cc-l-date").focus()});const h=s.querySelector(".cc-l-cancel");h&&h.addEventListener("click",()=>{u.style.display="none"});const f=s.querySelector(".cc-l-save");f&&f.addEventListener("click",async()=>{const x={contact_id:a,sent_at:u.querySelector(".cc-l-date").value||Ae(),body:u.querySelector(".cc-l-body").value};await ie("POST",`/api/postings/${t}/outreach-log`,x)&&(l("outreach logged"),ye())});const b=s.querySelector(".cc-followup");b&&b.addEventListener("click",()=>{const x=m.contacts.find(P=>String(P.id)===String(a)),y=m.outreach.filter(P=>String(P.contact_id)===String(a))[0]||null;gt(Ns(x,y),"follow-up copied — paste into your email")});const T=async(x,y,P)=>{const K=m.outreach.find(Se=>String(Se.id)===String(x))||{};await ie("PUT",`/api/outreach-log/${x}`,{sent_at:K.sent_at||"",body:K.body||"",note:K.note||"",followup_due_at:K.followup_due_at||"",done:!!K.followup_done_at,...y})&&(l(P),ye())},L=s.querySelector(".cc-fu-toggle");L&&L.addEventListener("change",()=>T(L.dataset.eid,{done:L.checked},L.checked?"marked followed up":"follow-up reopened"));const R=s.querySelector(".cc-fu-stop");R&&R.addEventListener("click",()=>T(R.dataset.eid,{followup_due_at:"",done:!1},"follow-up stopped"));const H=s.querySelector(".cc-fu-resume");H&&H.addEventListener("click",()=>T(H.dataset.eid,{followup_due_at:Ae(),done:!1},"follow-up resumed"));const D=s.querySelector(".cc-fu-dismiss");D&&D.addEventListener("click",()=>T(D.dataset.eid,{followup_due_at:"",done:!0},"escalation dismissed")),s.querySelectorAll(".cc-e-del").forEach(x=>x.addEventListener("click",async()=>{const y=x.closest(".cc-entry").dataset.eid;await ie("DELETE",`/api/outreach-log/${y}`)&&(l("send deleted"),ye())}))})}function zs(e){return e==="researching"||e==="awaiting_review"||e==="needs_work"||e==="no_hook"}const In='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M2.5 10.5v-7a1 1 0 011-1h7"/></svg>',xn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2L7.3 8.7"/><path d="M14 2L9.7 14l-2.4-5.3L2 6.3z"/></svg>',_n=`<button class="dh-copy draft-copy-btn" title="copy the email to the clipboard" aria-label="copy email">${In}</button>`,ut=[{key:"research",label:"Research",active:"Researching the company"},{key:"fill",label:"Draft",active:"Writing the draft"},{key:"humanize",label:"Polish",active:"Polishing the voice"},{key:"honesty",label:"Fact-check",active:"Fact-checking against your experience"}],Js='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5l3 3 6-7"/></svg>';function Gs(e){let t=ut.findIndex(s=>s.key===e);return t<0&&(t=0),`<div class="draft-progress">
    <div class="dp-track">${ut.map((s,a)=>{const o=a<t?"is-done":a===t?"is-active":"is-pending",r=a<t?Js:"";return`<div class="dp-seg ${o}"><span class="dp-dot">${r}</span><span class="dp-name">${s.label}</span></div>`}).join("")}</div>
    <div class="dp-status"><span class="spinner"></span><span>${ut[t].active}…</span></div>
  </div>`}function Ks(){if(!(i.gmail&&i.gmail.connected))return"";const e=(m.contacts||[]).filter(n=>n.email);return e.length?`<div class="draft-gmail-row">
    ...
    <button class="btn btn-primary draft-gmail-btn" title="send this email from your Gmail and log it">${xn}Send via Gmail</button>
  </div>`:'<div class="draft-note dim">Add a contact with an email to send via Gmail.</div>'}function Ln(e,t){const n=(p,h,f="")=>`
    <div class="draft-head">
    ...
    </div>`;if(e.status==="failed"){const p=Ws(e.violations);return`<div class="draft-card dc-failed" data-did="${e.id}">
      ...
      <div class="draft-sentbody">${c(pt(e)||"(empty)")}</div>
      ${Ie(e)}
      ...
      <div class="draft-sentbody">${c(pt(e)||"(empty)")}</div>
      ${Ie(e)}
    </div>`;const s=pt(e),a=e.status==="no_hook",o=a?'<span class="pill pill-info">no honest hook</span>':'<span class="pill pill-maybe">awaiting review</span>';let r="";if(a)try{r=JSON.parse(e.hook||"{}").reasoning||""}catch{}const d=a?`<div class="draft-note">No honest hook found — nothing true to say yet; scout recommends not emailing.${r?" "+c(r):""}</div>`:"";if(t)return`<div class="draft-card ${a?"dc-nohook":"dc-review"}" data-did="${e.id}">
    ...
    <div class="draft-actions">
      <button class="btn btn-primary draft-sent-btn" title="mark this email sent — bumps the outreach count">${xn}Mark sent</button>
    ...
    ${Ks()}`:`<div class="draft-actions">
      <button class="btn draft-regen-btn" title="re-run the draft — picks up backfilled info">${Ye}Regenerate</button>
    </div>`}
    ${Ie(e)}
  </div>`}function pt(e){return e.edited&&e.edited.trim()?e.edited:e.draft||""}function Ie(e){let t="",n=null,s=null;try{n=JSON.parse(e.research||"null")}catch{}try{s=JSON.parse(e.hook||"null")}catch{}if(n&&typeof n=="object"){const a=(u,p)=>p?`<div class="tr-line"><span class="tr-key">${u}:</span> ${c(String(p))}</div>`:"",o=n.role||{},r=Array.isArray(n.hooks)?n.hooks:[],d=r.map(u=>`
      <div class="tr-line">
        <span class="tr-key">${c(u.type||"hook")}</span>
        ${C(u.source_url)!=="#"?` · <a href="${C(u.source_url)}" target="_blank" rel="noopener">source</a>`:""}
      ...
      <div class="trace-body">
        ...
        ${C(a.source_url)!=="#"?`<div class="tr-line"><a href="${C(a.source_url)}" target="_blank" rel="noopener">source</a></div>`:""}
        ${s.reasoning?`<div class="tr-line"><span class="tr-key">reasoning:</span> ${c(s.reasoning)}</div>`:""}
      </div></details>`}return t}function Bn(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<div class="lint-chips">'+t.map(n=>`<span class="lint-chip" title="${c(n.message||"")}"><code>${c(n.code||"")}</code>${c(n.message||"")}</span>`).join("")+"</div>":""}function Ws(e){let t=[];try{t=JSON.parse(e||"[]")||[]}catch{t=[]}return t.length?'<ul class="violation-list">'+t.map(n=>`<li>${c(n.claim||n.message||String(n))}${n.why?` <span class="vl-why">— ${c(n.why)}</span>`:""}</li>`).join("")+"</ul>":""}function Ys(){const e=document.getElementById("outreach-section");if(!e)return;const t=e.querySelector("#draft-start-btn");t&&t.addEventListener("click",()=>Oe(!1,Zs())),e.querySelectorAll(".draft-retry-btn").forEach(s=>s.addEventListener("click",()=>Oe())),e.querySelectorAll(".draft-regen-btn").forEach(s=>s.addEventListener("click",()=>Oe(!0))),e.querySelectorAll(".draft-card[data-did]").forEach(s=>{const a=s.dataset.did,o=s.querySelector(".draft-textarea");o&&z(o,p=>Xs(a,p),{multiline:!0});const r=s.querySelector(".draft-sent-btn");r&&r.addEventListener("click",()=>ta(a));const d=s.querySelector(".draft-gmail-btn");d&&d.addEventListener("click",()=>{const p=s.querySelector(".draft-gmail-to");ea(a,p?p.value:"",d)});const u=s.querySelector(".draft-copy-btn");u&&u.addEventListener("click",()=>{const p=s.querySelector(".draft-textarea"),h=s.querySelector(".draft-sentbody"),f=p?p.value:h?h.textContent:"";gt(f,"email copied")})});const n=e.querySelector("details.draft-history");n&&n.addEventListener("toggle",()=>{m.openHist=n.open})}function Zs(){const e=document.getElementById("draft-skip-research");return!!(e&&e.checked)}async function Oe(e=!1,t=!1){const n=document.getElementById("outreach-section"),s=n&&(n.querySelector("#draft-start-btn")||n.querySelector(".draft-retry-btn")||n.querySelector(".draft-regen-btn"));s&&(s.disabled=!0);let a;try{const r=new URLSearchParams;e&&r.set("regenerate","1"),t&&r.set("research","0");const d=r.toString()?`?${r.toString()}`:"";a=await fetch(`/api/postings/${m.postingId}/outreach${d}`,{method:"POST"})}catch(r){l(`draft failed: ${r.message}`),s&&(s.disabled=!1);return}if(a.status===202){let r={};try{r=await a.json()}catch{}Array.isArray(r.degraded)&&r.degraded.length&&l(`drafting without ${r.degraded.join(", ")} — quality degrades, integrity unaffected`),await me(),k();return}if(a.status===409){await me(),l("a draft is already active");return}if(a.status===412){let r={};try{r=await a.json()}catch{}Qs(r.need,r.error),s&&(s.disabled=!1);return}if(a.status===503){const r=document.getElementById("outreach-section");if(r){const d=document.createElement("div");d.className="draft-note",d.textContent="Outreach engine not running in this build.",r.appendChild(d)}s&&(s.disabled=!1);return}const o=(await a.text().catch(()=>"")).trim();l(`draft failed: ${o||"HTTP "+a.status}`),s&&(s.disabled=!1)}function Qs(e,t){const n=document.getElementById("outreach-section");if(!n)return;const s=n.querySelector(".draft-actions"),a=e==="template",o=a?"Write email template":"View brain knowledge",r=document.createElement("div");r.className="blocks-gate",r.innerHTML=`
    ...
    </div>`,s?s.replaceWith(r):n.appendChild(r);const d=r.querySelector("#gate-fix-btn");d&&d.addEventListener("click",()=>a?X("outreach-template"):Gn());const u=r.querySelector("#gate-retry-btn");u&&u.addEventListener("click",Oe)}async function Xs(e,t){const n=await fetch(`/api/outreach/drafts/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=m.drafts.findIndex(d=>String(d.id)===String(e));a>=0&&(m.drafts[a]=s);const o=document.getElementById(`draft-edit-${e}`),r=o&&o.closest(".draft-card");if(r){const d=r.querySelector(".lint-chips"),u=Bn(s.lint);d?d.outerHTML=u||"":u&&o.insertAdjacentHTML("afterend",u)}}async function ea(e,t,n){n&&(n.disabled=!0,n.dataset.t=n.textContent||"",n.textContent="Sending…");const s=()=>{n&&(n.disabled=!1,n.textContent=n.dataset.t||"Send via Gmail")};let a;try{a=await fetch(`/api/outreach/drafts/${e}/send-gmail`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:t||""})})}catch(r){l(`send failed: ${r.message}`),s();return}if(!a.ok){const r=(await a.text().catch(()=>"")).trim();l(`send failed: ${r||"HTTP "+a.status}`),s();return}let o={};try{o=await a.json()}catch{}l(o.to?`sent via Gmail to ${o.to}`:"sent via Gmail"),await me(),await k()}async function ta(e){let t;try{t=await fetch(`/api/outreach/drafts/${e}/sent`,{method:"POST"})}catch(s){l(`failed: ${s.message}`);return}if(!t.ok){const s=(await t.text().catch(()=>"")).trim();l(`failed: ${s||"HTTP "+t.status}`);return}l("marked sent"),await me(),await k();const n=i.jobs.find(s=>s.posting_id===m.postingId);n&&be(n.posting_id,{outreach_count:n.outreach_count,last_outreach_at:n.last_outreach_at,next_up:n.next_up})}async function ge(){if(!m.postingId)return;let e;try{const t=await fetch(`/api/postings/${m.postingId}/answers`);if(!t.ok){xe();return}e=await t.json()}catch{xe();return}m.answers=e.answers||[],m.answersStatus=e.questions_status||"",xe(),m.answers.some(t=>t.status==="generating")?na():mt()}function na(){m.answersPoll||(m.answersPoll=setInterval(ge,4e3))}function mt(){m.answersPoll&&(clearInterval(m.answersPoll),m.answersPoll=null)}function xe(){const e=document.getElementById("answers-section");if(!e)return;const t=m.answers,n=m.answersStatus,s=t.some(p=>p.status==="generating"),a=t.length?`<div class="answers-list">${t.map(oa).join("")}</div>`:"",o=!!m.detecting,r=s||o?" disabled":"",d=p=>`<button class="btn" id="answers-redetect-btn"${o?" disabled":""}>${o?"Detecting…":p}</button>`;let u;n==="ok"&&t.length?u=(t.some(h=>!Tn(h)&&h.status!=="generating")?`<button class="btn" id="answers-start-btn"${r}>${s?"Drafting…":"Draft all blank"}</button>`:"")+d("Re-detect"):n===""||n==="unreachable"?u=`<button class="btn btn-primary" id="answers-start-btn"${r}>${s?"Drafting…":"Draft answers"}</button>`+d("Re-detect questions"):u=d("Re-detect questions"),e.innerHTML=`<div class="answers-meta">${c(sa(n,t.length))}</div>`+a+`<div class="answers-actions">${u}</div>`,ca()}function sa(e,t){switch(e){case"":return"Not detected yet";case"ok":return`${t} question${t===1?"":"s"} found`;case"none":return"No essay questions on this form";case"unsupported":return"Couldn't read this form — apply on the site";case"unreachable":return"Couldn't reach the application form — try re-detecting";default:return"Couldn't read this form"}}function Tn(e){return e.edited&&e.edited.trim()?e.edited:e.answer||""}function aa(e){switch(e.status){case"ready":return'<span class="pill pill-yes">ready</span>';case"needs_review":return'<span class="pill pill-maybe">needs review</span>';case"failed":return'<span class="pill pill-no">failed</span>';case"generating":return'<span class="pill pill-info">drafting…</span>';default:return'<span class="pill pill-info">not drafted</span>'}}function oa(e){const t=Tn(e),n=e.edited&&e.edited.trim(),s=e.status==="generating",a=t.length,o=e.max_length&&a>e.max_length,r=e.max_length?`<span class="answer-count${o?" over":""}">${a} / ${e.max_length}</span>`:`<span class="answer-count">${a} chars</span>`,d=!!t,u=d?"Regenerate":"Generate",p=d?"re-draft this answer (discards the current text)":"draft an answer to just this question";return`<div class="answer-card ac-${e.status}" data-aid="${e.id}">
    ...
    ${e.status==="failed"&&e.fail_reason?`<div class="answer-note answer-fail">${c(ia(e.fail_reason))}</div>`:""}
  </div>`}function ia(e){return e=String(e||""),e.length>160?e.slice(0,160)+"…":e}function ca(){const e=document.getElementById("answers-section");if(!e)return;const t=e.querySelector("#answers-start-btn");t&&t.addEventListener("click",Sn);const n=e.querySelector("#answers-redetect-btn");n&&n.addEventListener("click",la),e.querySelectorAll(".answer-card[data-aid]").forEach(s=>{const a=s.dataset.aid,o=s.querySelector(".answer-textarea");o&&(z(o,p=>da(a,p),{multiline:!0}),o.addEventListener("input",()=>ra(s,o)));const r=s.querySelector(".answer-regen-btn");r&&r.addEventListener("click",()=>ua(a));const d=s.querySelector(".answer-copy-btn");d&&d.addEventListener("click",()=>{o&&gt(o.value,"answer copied")});const u=s.querySelector(".answer-remove-btn");u&&u.addEventListener("click",()=>pa(a))})}function ra(e,t){const n=e.querySelector(".answer-count");if(!n)return;const s=t.value.length,a=n.textContent.includes("/")?parseInt(n.textContent.split("/")[1],10):0;n.textContent=a?`${s} / ${a}`:`${s} chars`,n.classList.toggle("over",!!a&&s>a)}async function Sn(){const e=document.getElementById("answers-section"),t=e&&e.querySelector("#answers-start-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/postings/${m.postingId}/answers`,{method:"POST"})}catch(a){l(`draft failed: ${a.message}`),t&&(t.disabled=!1);return}if(n.status===202){await ge();return}if(n.status===412){let a={};try{a=await n.json()}catch{}Cn(a.error),t&&(t.disabled=!1);return}if(n.status===503){jn("Answer generation isn't running in this build."),t&&(t.disabled=!1);return}const s=(await n.text().catch(()=>"")).trim();l(`draft failed: ${s||"HTTP "+n.status}`),t&&(t.disabled=!1)}async function la(){m.detecting=!0,xe();try{const e=await fetch(`/api/postings/${m.postingId}/answers/redetect`,{method:"POST"});if(!e.ok){const t=(await e.text().catch(()=>"")).trim();l(`detect failed: ${t||"HTTP "+e.status}`)}}catch(e){l(`detect failed: ${e.message}`)}m.detecting=!1,await ge()}async function da(e,t){const n=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({edited:t})});if(!n.ok)throw new Error((await n.text().catch(()=>"")).trim()||"HTTP "+n.status);const s=await n.json(),a=m.answers.findIndex(o=>String(o.id)===String(e));a>=0&&(m.answers[a]=s)}async function ua(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({regenerate:!0})})}catch(n){l(`regenerate failed: ${n.message}`);return}if(t.status===503){jn("Answer generation isn't running in this build.");return}if(t.status===412){let n={};try{n=await t.json()}catch{}Cn(n.error);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`regenerate failed: ${n||"HTTP "+t.status}`);return}await ge()}async function pa(e){let t;try{t=await fetch(`/api/answers/${e}`,{method:"DELETE"})}catch(n){l(`remove failed: ${n.message}`);return}if(!t.ok){const n=(await t.text().catch(()=>"")).trim();l(`remove failed: ${n||"HTTP "+t.status}`);return}await ge()}function Cn(e){const t=document.getElementById("answers-section");if(!t)return;const n=t.querySelector(".answers-actions"),s=document.createElement("div");s.className="blocks-gate",s.innerHTML=`
    ...
    </div>`,n?n.replaceWith(s):t.appendChild(s);const a=s.querySelector("#answers-fix-btn");a&&a.addEventListener("click",Gn);const o=s.querySelector("#answers-retry-btn");o&&o.addEventListener("click",Sn)}function jn(e){const t=document.getElementById("answers-section");if(!t)return;const n=document.createElement("div");n.className="draft-note",n.textContent=e,t.appendChild(n)}async function ve(e){var d,u;const t=document.getElementById("pane"),n=document.getElementById("scrim"),s=i.openId===e&&t.classList.contains("open"),a=s?((d=document.getElementById("pane-body"))==null?void 0:d.scrollTop)??0:0,o=s?(u=document.getElementById("trace-body"))==null?void 0:u.innerHTML:null;i.openId=e,t.classList.add("open"),n.classList.add("open"),t.setAttribute("aria-hidden","false"),hn("company"),s||(document.getElementById("pane-title").textContent="loading…",document.getElementById("pane-pills").innerHTML="",document.getElementById("pane-body").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>');let r;try{const p=await fetch(`/api/companies/${e}`);if(!p.ok)throw new Error(`HTTP ${p.status}`);r=await p.json()}catch(p){s||(document.getElementById("pane-body").innerHTML=`<div class="muted">Failed to load detail: ${c(p.message)}</div>`);return}if(i.openId===e){if(Le(r),s){if(o!=null){const h=document.getElementById("trace-body");h&&(h.innerHTML=o)}const p=document.getElementById("pane-body");p&&(p.scrollTop=a)}Be(e)}}function _e(){i.openId=null,i.openDetail=null,document.getElementById("pane").classList.remove("open"),document.getElementById("scrim").classList.remove("open"),document.getElementById("pane").setAttribute("aria-hidden","true")}function Le(e){i.openDetail=e,document.getElementById("pane-title").innerHTML=`<input class="ie ie-title" id="pane-title-input" placeholder="company name" value="${c(e.name||"")}">`,document.getElementById("pane-pills").innerHTML=`
    <span class="${g(e.has_verdict?e.verdict:"")}">${c(e.has_verdict?e.verdict:"unscored")}</span>
  `;const t=document.getElementById("pane-chat");t&&(t.style.display=i.meta&&i.meta.chat?"":"none",t.onclick=()=>Dt("company",e.company_id,e.name));const n=e.model==="manual",s=e.has_verdict?`
    <dl class="kv">
      <dt>verdict</dt><dd><span class="${g(e.verdict)}">${c(e.verdict)}</span>${n?' <span class="small muted">· set by hand</span>':""}</dd>
  ...
  `:'<div class="muted">Not yet scored. Run <code>scout verdict</code>, or set one by hand below.</div>',a=`
    ...
    <dl class="kv">
      <dt>url</dt><dd>${e.website_url?`<a href="${C(e.website_url)}" target="_blank" rel="noopener">${c(e.website_url)} ↗</a>`:'<span class="muted">—</span>'}</dd>
  ...
  `:'<div class="muted">No enrichment yet. Run <code>scout enrich</code>.</div>',r=!i.meta||i.meta.control!==!1,d=r&&i.meta&&i.meta.verdict?'<button class="h3-action" id="rescore-btn" title="re-score just this company — replaces the current verdict, manual or not">↻ re-score</button>':"",u=r&&e.domain?'<button class="h3-action" id="reenrich-btn" title="re-fetch this company’s site now">↻ re-enrich</button>':"",p=Object.keys(e.raw_json||{}).sort(),h=p.length===0?"":`
    <details class="raw-json">
      ...
      <table><tbody>
        ${p.map(y=>`<tr><td class="k">${c(y)}</td><td>${c(e.raw_json[y])}</td></tr>`).join("")}
      </tbody></table>
    </details>
  `,f=`
    <div class="flag-bar">
  ...
  `;const b=document.getElementById("posting-add-btn");b&&b.addEventListener("click",()=>ya(e)),ft(),document.querySelectorAll("#ve-pick .ve-opt").forEach(y=>{y.addEventListener("click",()=>{document.querySelectorAll("#ve-pick .ve-opt").forEach(P=>P.classList.remove("is-on")),y.classList.add("is-on")})});const T=document.getElementById("ve-save-btn");T&&T.addEventListener("click",()=>fa(e)),z(document.getElementById("pane-title-input"),y=>vn(e,"name",y)),document.querySelectorAll("#facts-body [data-k]").forEach(y=>z(y,P=>vn(e,y.dataset.k,P))),z(document.getElementById("pane-domain-input"),y=>As(e,y)),z(document.getElementById("pane-notes-input"),y=>Hs(e,y),{multiline:!0});const L=document.getElementById("flag-toggle-btn");L&&L.addEventListener("click",()=>Mn(e.company_id));const R=document.getElementById("review-stamp-btn");R&&R.addEventListener("click",()=>ha(e.company_id));const H=document.getElementById("rescore-btn");H&&H.addEventListener("click",()=>wt("verdict",{company_ids:[e.company_id]}));const D=document.getElementById("reenrich-btn");D&&D.addEventListener("click",()=>wt("enrich",{company_ids:[e.company_id]}));const x=document.getElementById("company-delete-btn");x&&x.addEventListener("click",()=>Ss(e))}function ma(e){return`
    <div class="ie-grid">
    ...
    </dl>`}async function ha(e){const t=document.getElementById("review-stamp-btn");t&&(t.disabled=!0);let n;try{n=await fetch(`/api/companies/${e}/reviewed`,{method:"POST"})}catch(o){l(`failed: ${o.message}`),t&&(t.disabled=!1);return}if(!n.ok){const o=await n.text().catch(()=>"");l(`failed: HTTP ${n.status}${o?" — "+o:""}`),t&&(t.disabled=!1);return}const s=await n.json(),a=i.rows.find(o=>o.company_id===e);a&&(a.reviewed_at=s.reviewed_at,G()),i.openId===e&&(Le(s),Be(e)),l("reviewed")}async function Mn(e){const t=i.rows.find(o=>o.company_id===e),n=!(t&&t.flagged);let s;try{s=await fetch(`/api/companies/${e}/flagged`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagged:n})})}catch(o){l(`failed: ${o.message}`);return}if(!s.ok){const o=await s.text().catch(()=>"");l(`failed: HTTP ${s.status}${o?" — "+o:""}`);return}const a=await s.json();t&&(t.flagged=a.flagged,G()),i.openId===e&&(Le(a),Be(e)),k(),l(a.flagged?"flagged":"unflagged")}async function fa(e){const t=document.querySelector("#ve-pick .ve-opt.is-on");if(!t){l("Pick yes, maybe, or no.");return}const n=t.dataset.v,s=document.getElementById("ve-reason").value.trim(),a=document.getElementById("ve-save-btn");a.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/verdict`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({verdict:n,reason:s})})}catch(d){l(`save failed: ${d.message}`),a.disabled=!1;return}if(!o.ok){const d=await o.text().catch(()=>"");l(`save failed: HTTP ${o.status}${d?" — "+d:""}`),a.disabled=!1;return}const r=await o.json();Le(r),Be(r.company_id),q(),S(),k(),l("verdict saved")}function ht(e){const t=e.postings||[];return t.length?t.map(n=>{const s=[n.location,n.source==="capture"?"captured":"added",(n.created_at||"").slice(0,10)].filter(Boolean).map(c).join(" · "),a=n.application_status||"",o=[n.next_up?'<span class="draft-badge db-next" style="margin-left:0" title="queued next up for outreach">next up</span>':"",`<span class="pill ${a?He(a)||"pill-stage":"pill-none"}">${c(a||"—")}</span>`,`<span class="pt-meta">${a?"tracked":"not applied"}</span>`,`<span class="pt-meta">${n.outreach_count?`${n.outreach_count} sent · last ${c(n.last_outreach_at||"?")}`:"no outreach yet"}</span>`].filter(Boolean).join("");return`
    <div class="brain-node posting-card" data-pid="${c(n.id)}" title="open the pursuit — tracking, outreach, drafts">
      <div class="n"><a href="${C(n.url)}" target="_blank" rel="noopener">${c(n.title||n.url)} ↗</a></div>
    ...
    </div>`}).join(""):'<div class="muted">No job links yet.</div>'}function be(e,t){const n=i.openDetail;if(!n||!i.openId)return;const s=(n.postings||[]).find(o=>String(o.id)===String(e));if(!s)return;Object.assign(s,t);const a=document.getElementById("postings-list");a&&(a.innerHTML=ht(n),ft())}function ft(){document.querySelectorAll("#postings-list .posting-card").forEach(e=>{e.addEventListener("click",t=>{t.target.closest("a")||it(e.dataset.pid)})})}async function ya(e){const t=document.getElementById("posting-url"),n=document.getElementById("posting-title"),s=document.getElementById("posting-add-btn"),a=t.value.trim();if(!a){l("Enter a URL first."),t.focus();return}s.disabled=!0;let o;try{o=await fetch(`/api/companies/${e.company_id}/postings`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a,title:n.value.trim()})})}catch(u){l(`add failed: ${u.message}`),s.disabled=!1;return}if(!o.ok){const u=await o.text().catch(()=>"");l(`add failed: HTTP ${o.status}${u?" — "+u:""}`),s.disabled=!1;return}const r=await o.json();e.postings=(e.postings||[]).filter(u=>u.id!==r.id),e.postings.unshift(r);const d=document.getElementById("postings-list");d&&(d.innerHTML=ht(e),ft()),t.value="",n.value="",s.disabled=!1,k(),l("link added")}async function Be(e){let t;try{t=await fetch(`/api/companies/${e}/trace`)}catch(s){Ne(`<div class="muted">Failed to load trail: ${c(s.message)}</div>`);return}if(!t.ok){Ne(`<div class="muted">Failed to load trail: HTTP ${t.status}.</div>`);return}const n=(await t.json()).events||[];if(n.length===0){Ne('<div class="muted">No decision trail yet. Run <code>verdict</code> to record one — every scoring pass is captured here.</div>');return}Ne(n.map(ga).join(""))}function ga(e){const t=[e.criteria_source,e.taste_version].filter(Boolean).map(c);return e.run_id&&t.push("run "+c(e.run_id.slice(0,8))),`
    <div class="trail-event">
      <div class="trail-head">
        <span class="${g(e.verdict)}">${c(e.verdict)}</span>
    ...
    </div>`}function Ne(e){const t=document.getElementById("trace-body");t&&(t.innerHTML=e)}let An;function l(e){const t=document.getElementById("toast");t.textContent=e,t.classList.toggle("err",/\b(fail(ed)?|error|disabled|already running)\b/i.test(e)),t.classList.add("show"),clearTimeout(An),An=setTimeout(()=>t.classList.remove("show"),2200)}let yt;const va=6e3;function Re(){clearTimeout(yt),yt=void 0}function Hn(){Re(),document.getElementById("drawer").classList.remove("open")}function Pn(){Re(),yt=setTimeout(Hn,va)}async function gt(e,t="copied"){if(!e){l("nothing to copy");return}try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(e);else{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}l(t)}catch(n){l(`copy failed: ${n.message}`)}}i.meta={control:!1,brain:!1,verdict:!1};async function vt(){try{const s=await fetch("/api/meta");if(!s.ok)return;i.meta=await s.json()}catch{return}const e=i.meta.control;document.getElementById("btn-ingest").disabled=!e,document.getElementById("btn-enrich").disabled=!e;const t=document.getElementById("btn-verdict");t.disabled=!e||!i.meta.verdict,t.title=i.meta.verdict?"":"set ANTHROPIC_API_KEY in the server env to enable";const n=document.getElementById("open-chat");n&&(n.style.display=i.meta.chat?"":"none")}async function bt(){let e;try{const s=await fetch("/api/runs");if(!s.ok)return;e=await s.json()}catch{return}const t=e.busy_stage||"",n=document.getElementById("run-busy");t?(n.style.display="",document.getElementById("run-busy-label").textContent=t+" running…"):n.style.display="none",document.getElementById("btn-ingest").classList.toggle("busy",t==="ingest"),document.getElementById("btn-enrich").classList.toggle("busy",t==="enrich"),document.getElementById("btn-verdict").classList.toggle("busy",t==="verdict")}let Te=null;async function wt(e,t){if(i.meta&&i.meta.control===!1){l("control surface disabled");return}let n;try{n=await fetch(`/api/run/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t||{})})}catch(a){l(`run failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(n.status===412){const a=await n.text();l(a.trim());return}if(!n.ok){l(`run failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Un(e,s,t)}async function ba(e){const t=new FormData;t.append("csv",e);let n;try{n=await fetch("/api/ingest",{method:"POST",body:t})}catch(a){l(`upload failed: ${a.message}`);return}if(n.status===409){l("a job is already running");return}if(!n.ok){l(`upload failed: HTTP ${n.status}`);return}const{job_id:s}=await n.json();Un("ingest",s)}const wa=["add-url","add-name","add-location","add-headcount","add-title","add-job-company"];let De=[],Z=new Set,Q="company";function qn(e){Q=e,document.querySelectorAll("#add-kind .v-chip").forEach(s=>s.classList.toggle("is-on",s.dataset.kind===e)),document.getElementById("add-company-fields").style.display=e==="company"?"":"none",document.getElementById("add-job-fields").style.display=e==="job"?"":"none";const t=document.getElementById("add-url-label"),n=document.getElementById("add-url");e==="company"?(t.innerHTML='Website<span class="req">*</span>',n.placeholder="acme.com"):(t.innerHTML='Posting URL<span class="req">*</span>',n.placeholder="https://… the job posting"),document.getElementById("add-save").textContent=e==="company"?"Add company":"Add job",On()}function kt(){return!!i.meta.capture&&document.getElementById("add-enrich").checked}function On(){const e=document.getElementById("add-note");kt()?e.innerHTML=Q==="company"?"scout fetches the page and fills the blank fields — your values win. The page text also seeds enrichment, so the next Verdict can score it. Pages behind a login wall (LinkedIn) usually can't be fetched.":"scout fetches the posting and fills in the title, location and description — your values win. The job attaches to its company, adding it to the list first if needed. Pages behind a login wall (LinkedIn) usually can't be fetched.":e.innerHTML=Q==="company"?"Stored as source <code>manual</code>. Run Enrich then Verdict to score it. A website already in the list is rejected — manual adds never overwrite an existing company.":"Stored as-is, no fetch. The job attaches to the typed company, or to the link's own domain when the posting lives on the company's site — for an ATS link (greenhouse, lever, …), type the company."}async function ka(){wa.forEach(s=>{document.getElementById(s).value=""}),document.getElementById("add-vertical-filter").value="",Z=new Set;const e=document.getElementById("add-enrich"),t=document.getElementById("add-enrich-row");e.disabled=!i.meta.capture,t.classList.toggle("disabled",!i.meta.capture),t.title=i.meta.capture?"":"set ANTHROPIC_API_KEY in the server env to enable",i.meta.capture||(e.checked=!1),qn(i.view==="jobs"?"job":"company");const n=document.getElementById("add-stage");n.innerHTML='<option value="">—</option>',document.getElementById("add-vertical-chips").innerHTML='<div class="none">loading…</div>',document.getElementById("add-company-names").innerHTML=(i.rows||[]).map(s=>`<option value="${c(s.name)}">`).join(""),document.getElementById("add-scrim").classList.add("open"),document.getElementById("add-url").focus();try{const s=await(await fetch("/api/facets")).json();(s.funding_stages||[]).forEach(a=>{const o=document.createElement("option");o.value=a,o.textContent=a,n.appendChild(o)}),De=s.verticals||[]}catch{De=[]}Nn()}function Ve(){document.getElementById("add-scrim").classList.remove("open")}function Nn(){const e=document.getElementById("add-vertical-chips"),t=document.getElementById("add-vertical-filter").value.trim().toLowerCase(),n=De.filter(s=>!t||s.toLowerCase().includes(t));n.length?(e.innerHTML=n.map(s=>`<button type="button" class="vchip${Z.has(s)?" sel":""}" data-v="${c(s)}">${c(s)}</button>`).join(""),e.querySelectorAll(".vchip").forEach(s=>s.addEventListener("click",()=>{const a=s.dataset.v;Z.has(a)?Z.delete(a):Z.add(a),s.classList.toggle("sel"),Rn()}))):e.innerHTML=`<div class="none">${De.length?"no match":"no verticals in the set yet"}</div>`,Rn()}function Rn(){const e=Z.size;document.getElementById("add-vertical-count").textContent=e?`· ${e} selected`:""}function Dn(e){return/^https?:\/\//i.test(e)?e:"https://"+e}async function Vn(){const e=document.getElementById("add-url"),t=e.value.trim();if(!t){l(Q==="company"?"Website is required.":"Posting URL is required."),e.focus();return}const n=document.getElementById("add-save"),s=n.textContent;n.disabled=!0,kt()&&(n.textContent="reading page…");const a=()=>{n.disabled=!1,n.textContent=s},o=f=>document.getElementById(f).value.trim(),r=kt();let d,u;r?(d="/api/capture",u={url:Dn(t),kind:Q==="company"?"company_page":"job_posting",fields:Q==="company"?{name:o("add-name"),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value,vertical:[...Z].join(", ")}:{name:o("add-job-company"),title:o("add-title")}}):Q==="company"?(d="/api/companies",u={website:t,name:o("add-name"),vertical:[...Z].join(", "),location:o("add-location"),headcount:o("add-headcount"),funding_stage:document.getElementById("add-stage").value}):(d="/api/postings",u={url:Dn(t),title:o("add-title"),company:o("add-job-company")});let p;try{p=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})}catch(f){l(`add failed: ${f.message}`),a();return}if(!p.ok){let f=`HTTP ${p.status}`;try{const b=await p.text();try{f=JSON.parse(b).error||f}catch{f=b.trim()||f}}catch{}if(a(),p.status===409){l(f||"That company is already in the list."),e.focus(),e.select();return}l(`add failed: ${f}`);return}const h=await p.json();if(a(),r&&!h.company_id){l(h.note||"couldn't classify that page");return}if(Ve(),q(),S(),k(),Q==="job"){const f=h.posting&&h.posting.title||"job link";l(`tracking: ${f} @ ${h.company_name}${h.posting_updated?" (refreshed)":""}`),O("jobs")}else r?(l(h.note||(h.company_created?`company added: ${h.company_name}`:`${h.company_name} is already in the list`)),ve(h.company_id)):l("company added")}function Un(e,t,n){Te=t,Re();const s=document.getElementById("drawer"),a=document.getElementById("drawer-log");document.getElementById("drawer-title").textContent=e,document.getElementById("drawer-spinner").style.display="",document.getElementById("drawer-cancel").style.display="",document.getElementById("drawer-close").style.display="none",a.innerHTML="";const o=document.getElementById("drawer-summary");o.hidden=!0,o.innerHTML="",s.classList.add("open"),bt();const r={yes:0,maybe:0,no:0},d=/^(.+?)\s*→\s*(yes|maybe|no)\s*—\s*([\s\S]*)$/i,u=new EventSource(`/api/jobs/${t}/stream`),p=(h,f)=>{const b=document.createElement("div");let T;if(!f&&(T=h.match(d))){const L=T[2].toLowerCase();r[L]++,b.className="ln ln-verdict";const R=document.createElement("span");R.className="pill pill-"+L,R.textContent=L;const H=document.createElement("span");H.className="lv-text";const D=document.createElement("span");D.className="lv-name",D.textContent=T[1].trim(),H.appendChild(D);const x=(T[3]||"").trim();if(x){const y=document.createElement("span");y.className="lv-reason",y.textContent=x,H.append(" ",y)}b.append(R,H)}else if(!f&&/^(scoring|enriching|ingesting)\b/i.test(h))b.className="ln ln-head",b.textContent=h;else if(!f&&/^·\s/.test(h))b.className="ln ln-pick",b.textContent=h;else{const L=!f&&/^\s*warn:/i.test(h);b.className="ln"+(f?" ln-err":L?" ln-warn":""),b.textContent=L?h.replace(/^\s*warn:\s*/i,"⚠ "):h}a.appendChild(b),a.scrollTop=a.scrollHeight};u.addEventListener("line",h=>p(h.data,/error|failed/i.test(h.data))),u.addEventListener("end",h=>{if(u.close(),Te=null,p(`— ${h.data} —`,h.data==="failed"),document.getElementById("drawer-spinner").style.display="none",document.getElementById("drawer-cancel").style.display="none",document.getElementById("drawer-close").style.display="",r.yes+r.maybe+r.no>0){for(const b of["yes","maybe","no"]){if(!r[b])continue;const T=document.createElement("span");T.className="pill pill-"+b,T.textContent=`${r[b]} ${b}`,o.appendChild(T)}o.hidden=!1}Pn(),l(`${e} ${h.data}`),n&&Array.isArray(n.company_ids)&&n.company_ids.length>0?ms(n.company_ids):q(),S(),bt(),k(),i.openId&&ve(i.openId)}),u.onerror=()=>{u.close()}}async function Ea(){if(Te)try{await fetch(`/api/jobs/${Te}/cancel`,{method:"POST"})}catch{}}let N=null;const $a={researcher:"researcher",fill:"writer",humanizer:"humanizer",honesty:"honesty check"};function Et(e){return e==="application-stages"||e==="outreach-statuses"}function Ue(e){if(e==="outreach-template")return"outreach template";if(e==="followup-template")return"follow-up template";if(e==="playbook")return"playbook";if(e==="application-stages")return"application stages";if(e==="outreach-statuses")return"outreach statuses";if(e&&e.startsWith("outreach-prompts/")){const t=e.slice(17);return($a[t]||t)+" prompt"}return e+".md"}async function X(e){N=e;const t=document.getElementById("editor-scrim");document.getElementById("editor-title").textContent="edit "+Ue(e),document.getElementById("editor-text").value="loading…",document.getElementById("editor-ver").textContent="";const n=!!e&&e.startsWith("outreach-prompts/"),s=n?e.slice(17):"",a=n&&s!=="fill";document.getElementById("editor-toggle-row").style.display=a?"":"none",document.getElementById("editor-reset").style.display=n?"":"none",a&&(document.getElementById("editor-toggle-label").textContent="Run this stage (off → it is skipped in the pipeline)"),t.classList.add("open");try{const o=await fetch(`/api/${e}`);if(!o.ok){const d=(await o.text().catch(()=>"")).trim();document.getElementById("editor-text").value=o.status===404?"failed to load: HTTP 404 — this route is missing. Restart the scout server (your running binary may predate this feature).":`failed to load: ${d||"HTTP "+o.status}`;return}const r=await o.json();Et(e)?(document.getElementById("editor-title").textContent="edit "+Ue(e)+" — one per line",document.getElementById("editor-text").value=(r.statuses||[]).join(`
`)):document.getElementById("editor-text").value=r.content||"",a&&(document.getElementById("editor-enabled").checked=r.enabled!==!1),r.taste_version&&(document.getElementById("editor-ver").textContent="version "+r.taste_version)}catch(o){document.getElementById("editor-text").value="failed to load: "+o.message}}function Fe(){document.getElementById("editor-scrim").classList.remove("open"),N=null}let I=null,$t=[],It=[];const Ia=["location.allowed","verticals.excluded","verticals.allowed"],xt={"verticals.excluded":"pf-vertical-tags","verticals.allowed":"pf-vertical-tags"};function ee(e){const[t,n]=e.split(".");return I[t]&&I[t][n]||[]}function ze(e,t){const[n,s]=e.split(".");(I[n]=I[n]||{})[s]=t}function Je(e,t){const n=t.toLowerCase();return e.some(s=>String(s).toLowerCase()===n)}function _t(){Ia.forEach(e=>{const t=document.querySelector(`.pf-chips[data-field="${e}"]`);if(!t)return;const n=xt[e]?` list="${xt[e]}"`:"",s=xt[e]?"type to search…":"type &amp; Enter…";t.innerHTML=ee(e).map((a,o)=>`<span class="pf-chip">${c(a)}<button class="pf-chip-x" data-field="${e}" data-i="${o}" title="remove" aria-label="remove ${c(a)}">×</button></span>`).join("")+`<input class="pf-chip-input" data-field="${e}"${n} type="text" placeholder="${s}" spellcheck="false" autocomplete="off" />`})}function xa(e,t){var s;const n=(t||"").trim();if(n){const a=ee(e);Je(a,n)||ze(e,[...a,n])}_t(),(s=document.querySelector(`.pf-chip-input[data-field="${e}"]`))==null||s.focus()}function Fn(e,t){const n=ee(e).slice();n.splice(t,1),ze(e,n),_t()}function zn(){const e=document.getElementById("pf-stages");if(!e)return;const t=ee("funding_stage.allowed");e.innerHTML=It.map(n=>{const s=Je(t,n.value),a=n.count?` <span class="pf-stage-n">${n.count}</span>`:"";return`<button class="pf-stage${s?" is-on":""}" data-stage="${c(n.value)}">${c(n.value)}${a}</button>`}).join("")}function _a(e){const t=ee("funding_stage.allowed");ze("funding_stage.allowed",Je(t,e)?t.filter(n=>String(n).toLowerCase()!==e.toLowerCase()):[...t,e]),zn()}function La(){const e=document.getElementById("pf-vertical-tags");e&&(e.innerHTML=$t.map(t=>`<option value="${c(t.value)}" label="${t.count}"></option>`).join(""))}function Ba(){return{location:{allowed:[],remote_ok:!0},headcount:{min:0,max:0},verticals:{allowed:[],excluded:[]},funding_stage:{allowed:[]}}}async function Jn(e=!1){document.getElementById("prefilter-scrim").classList.add("open");try{const[t,n]=await Promise.all([(await fetch("/api/taste-filter"+(e?"?default=1":""))).json(),$t.length||It.length?Promise.resolve(null):fetch("/api/filter-options").then(s=>s.json()).catch(()=>null)]);n&&($t=n.verticals||[],It=n.stages||[]),I=Object.assign(Ba(),t.rules||{}),I.location=Object.assign({allowed:[],remote_ok:!0},I.location),I.headcount=Object.assign({min:0,max:0},I.headcount),I.verticals=Object.assign({allowed:[],excluded:[]},I.verticals),I.funding_stage=Object.assign({allowed:[]},I.funding_stage),e||(document.getElementById("pf-enabled").checked=t.enabled!==!1),document.getElementById("pf-remote-ok").checked=!!I.location.remote_ok,document.getElementById("pf-hc-min").value=String(I.headcount.min||0),document.getElementById("pf-hc-max").value=String(I.headcount.max||0),La(),_t(),zn()}catch(t){l(`failed to load pre-filter: ${t.message}`)}}function Ge(){document.getElementById("prefilter-scrim").classList.remove("open"),I=null}async function Ta(){if(!I)return;I.location.remote_ok=document.getElementById("pf-remote-ok").checked,I.headcount.min=Math.max(0,parseInt(document.getElementById("pf-hc-min").value,10)||0),I.headcount.max=Math.max(0,parseInt(document.getElementById("pf-hc-max").value,10)||0),document.querySelectorAll(".pf-chip-input").forEach(n=>{const s=n.value.trim();s&&!Je(ee(n.dataset.field),s)&&ze(n.dataset.field,[...ee(n.dataset.field),s])});const e=document.getElementById("pf-enabled").checked;let t;try{t=await fetch("/api/taste-filter",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({rules:I,enabled:e})})}catch(n){l(`save failed: ${n.message}`);return}if(!t.ok){l(`save failed: ${(await t.text().catch(()=>"")).trim()||"HTTP "+t.status}`);return}l("pre-filter saved"),Ge(),S()}const Sa=[{key:"experience",hard:!0},{key:"voice",hard:!1},{key:"logistics",hard:!1}];async function Gn(){document.getElementById("sources-scrim").classList.add("open"),document.getElementById("sources-list").innerHTML='<div class="loading-row"><span class="spinner"></span><span>loading…</span></div>';try{Ca(await(await fetch("/api/outreach/sources")).json())}catch(e){l(`failed to load sources: ${e.message}`)}}function Lt(){document.getElementById("sources-scrim").classList.remove("open")}function Ca(e){const t=document.getElementById("sources-list");if(!t)return;const n=e&&e.needs&&e.needs.length?e.needs.map(a=>({key:a.Key||a.key,hard:a.Hard??a.hard})):Sa,s={};(e&&e.sources||[]).forEach(a=>{(s[a.need]=s[a.need]||[]).push(a)}),t.innerHTML=n.map(a=>{const o=s[a.key]||[],r=o.length?o.map(d=>`<li><span class="src-title">${c(d.title||d.page_id)}</span></li>`).join(""):`<li class="dim small">${a.hard?"none yet — add an experience page to your brain":"none (optional)"}</li>`;return`<div class="src-need">
      <div class="src-need-h">${c(a.key)}${a.hard?' <span class="dim">required</span>':' <span class="dim">optional</span>'}</div>
      <ul class="src-items">${r}</ul></div>`}).join("")}async function ja(){if(!N)return;const e=document.getElementById("editor-text").value;let t;Et(N)?t={statuses:e.split(/\r?\n/).map(o=>o.trim()).filter(Boolean)}:(t={content:e},N.startsWith("outreach-prompts/")&&N!=="outreach-prompts/fill"&&(t.enabled=document.getElementById("editor-enabled").checked));let n;try{n=await fetch(`/api/${N}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch(o){l(`save failed: ${o.message}`);return}if(!n.ok){l(`save failed: ${(await n.text().catch(()=>"")).trim()||"HTTP "+n.status}`);return}const s=await n.json();s.taste_version&&(document.getElementById("editor-ver").textContent="version "+s.taste_version);const a=Et(N);N==="followup-template"&&(i.followupTemplate=e),l(`${Ue(N)} saved`),Fe(),a&&M(),S()}async function Ma(){if(!N||!N.startsWith("outreach-prompts/"))return;let e;try{e=await fetch(`/api/${N}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({reset:!0})})}catch(n){l(`reset failed: ${n.message}`);return}if(!e.ok){l(`reset failed: HTTP ${e.status}`);return}const t=await e.json();document.getElementById("editor-text").value=t.content||"",l(`${Ue(N)} reset to default`)}function Kn(e,t,n){e.k!==t?(e.k=t,e.dir=1):e.dir===1?e.dir=-1:Object.assign(e,n)}document.querySelectorAll("#t thead th[data-k]").forEach(e=>{e.onclick=()=>{Kn(i.sort,e.dataset.k,v),G()}}),document.querySelectorAll("#jt thead th[data-jk]").forEach(e=>{e.onclick=()=>{Kn(i.jsort,e.dataset.jk,E),A()}}),document.getElementById("tab-companies").onclick=()=>O("companies"),document.getElementById("tab-jobs").onclick=()=>O("jobs"),document.getElementById("q").oninput=G,document.getElementById("fdrop-cfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.dataset.toggle==="flagged")re=!re,ae(t,re);else if(t.dataset.toggle==="enriched")le=!le,ae(t,le);else if(t.hasAttribute("data-v")){const n=t.getAttribute("data-v");F.has(n)?F.delete(n):F.add(n),ae(t,F.has(n))}else return;G()}}),document.getElementById("fdrop-columns-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item[data-col]");if(!t)return;const n=nt(),s=t.getAttribute("data-col");n.hidden.has(s)?n.hidden.delete(s):n.hidden.add(s),localStorage.setItem(n.key,JSON.stringify([...n.hidden])),ae(t,!n.hidden.has(s)),de(),Zt()}),document.getElementById("jq").oninput=A;for(const e of["fdrop-cfilters","fdrop-columns","fdrop-jfilters"]){const t=document.getElementById(e);t.querySelector(".fdrop-btn").addEventListener("click",s=>{s.stopPropagation();const a=t.classList.contains("is-open");ot(),a||ys(t)}),t.querySelector(".fdrop-menu").addEventListener("click",s=>s.stopPropagation())}document.addEventListener("click",ot),document.getElementById("fdrop-jfilters-menu").addEventListener("click",e=>{const t=e.target.closest(".fdrop-item");if(t){if(t.hasAttribute("data-stage")){const n=t.getAttribute("data-stage");_.has(n)?_.delete(n):_.add(n),ae(t,_.has(n))}else if(t.dataset.toggle==="nextup")ue=!ue,ae(t,ue);else if(t.hasAttribute("data-status")){const n=t.getAttribute("data-status");W.has(n)?W.delete(n):W.add(n),ae(t,W.has(n))}else return;A()}}),rs(),Yt(),de(),document.getElementById("pane-close").onclick=_e,document.getElementById("scrim").onclick=_e,document.getElementById("pursuit-close").onclick=Ee,document.getElementById("pursuit-scrim").onclick=Ee,document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(document.querySelector(".fdrop.is-open")){ot();return}if(document.getElementById("chat-pane").classList.contains("open")){Vt();return}if(za()){qt();return}if(document.getElementById("profile-scrim").classList.contains("open")){Pt();return}if(document.getElementById("add-scrim").classList.contains("open")){Ve();return}if(document.getElementById("run-scrim").classList.contains("open")){Ke();return}if(document.getElementById("help-scrim").classList.contains("open")){We();return}if(document.getElementById("relink-scrim").classList.contains("open")){he();return}if(document.getElementById("delcompany-scrim").classList.contains("open")){Pe();return}if(document.getElementById("deljob-scrim").classList.contains("open")){qe();return}const t=document.getElementById("pane").classList.contains("open"),n=document.getElementById("pursuit-pane").classList.contains("open");if(t||n){if(ct==="pursuit"&&n){Ee();return}if(ct==="company"&&t){_e();return}if(t){_e();return}Ee();return}if(document.getElementById("key-scrim").classList.contains("open")){Ht();return}if(document.getElementById("sources-scrim").classList.contains("open")){Lt();return}if(document.getElementById("prefilter-scrim").classList.contains("open")){Ge();return}if(document.getElementById("editor-scrim").classList.contains("open")){Fe();return}if(document.getElementById("settings-scrim").classList.contains("open")){Ot();return}});let Bt=null;const Aa={enrich:"Fetches and summarizes each company's pages, filling its enrichment row.",verdict:"Scores every company against your criteria — one LLM call each. Only companies with a successful enrichment are scored."};function Wn(e){if(i.meta&&i.meta.control===!1){l("control surface disabled");return}Bt=e,document.getElementById("run-title").textContent="Run "+e,document.getElementById("run-desc").textContent=Aa[e]||"",document.getElementById("run-only-blanks").checked=!1,document.getElementById("run-workers-input").value=e==="verdict"?10:8;const t=document.getElementById("run-warn"),n=i.stats||{},s=Math.max(0,(n.total_companies||0)-(n.enriched_ok||0));e==="verdict"&&s>0?(document.getElementById("run-warn-text").textContent=`${s} ${s===1?"company isn't":"companies aren't"} enriched yet — verdict will skip ${s===1?"it":"them"}. Run Enrich first to include ${s===1?"it":"them"}.`,t.style.display=""):t.style.display="none",document.getElementById("run-scrim").classList.add("open")}function Ke(){document.getElementById("run-scrim").classList.remove("open"),Bt=null}document.getElementById("btn-enrich").onclick=()=>Wn("enrich"),document.getElementById("btn-verdict").onclick=()=>Wn("verdict"),document.getElementById("run-cancel").onclick=Ke,document.getElementById("run-scrim").onclick=e=>{e.target.id==="run-scrim"&&Ke()},document.getElementById("run-go").onclick=()=>{const e=Bt,t=document.getElementById("run-only-blanks").checked,n=parseInt(document.getElementById("run-workers-input").value,10);if(Ke(),!e)return;const s={};t&&(s.only_blanks=!0),n>0&&(s.workers=n),wt(e,s)},document.getElementById("btn-ingest").onclick=()=>document.getElementById("csv-file").click(),document.getElementById("btn-add").onclick=ka;const Ha={add:{title:"Add data",intro:"Two ways to get companies and jobs into scout.",items:[{name:"Ingest CSV",sec:"ingest",desc:"Bulk-import companies from a CSV export (e.g. Crunchbase). Columns are mapped to company fields and new rows are created."},{name:"Add",sec:"ingest",desc:"Add one company or job from its link. Tick “fill in the blanks” to let an ATS API (ashby/greenhouse/lever) or one cheap agent pass complete the details."}]},run:{title:"Run the pipeline",intro:"Enrich must run before Verdict — verdict only scores companies that already have a successful enrichment row.",items:[{name:"Enrich",sec:"enrich",desc:"Fetches and summarizes each company's web presence into an enrichment row. A prerequisite for Verdict."},{name:"Verdict",sec:"verdict",desc:"Scores each enriched company against your criteria with the LLM, producing a yes / maybe / no with reasoning."}]}};function Yn(e){const t=Ha[e];if(!t)return;document.getElementById("help-title").textContent=t.title;const n=document.getElementById("help-items");if(n.innerHTML="",t.intro){const s=document.createElement("p");s.className="help-intro",s.textContent=t.intro,n.appendChild(s)}t.items.forEach(s=>{const a=document.createElement("div");a.className="help-item";const o=document.createElement("div");o.className="help-item-name",o.textContent=s.name;const r=document.createElement("div");r.className="help-item-desc",r.textContent=s.desc;const d=document.createElement("a");d.className="help-link",d.textContent="Learn more →",d.onclick=()=>{We(),ts(),Ja(s.sec)},a.appendChild(o),a.appendChild(r),a.appendChild(d),n.appendChild(a)}),document.getElementById("help-scrim").classList.add("open")}function We(){document.getElementById("help-scrim").classList.remove("open")}document.getElementById("help-add").onclick=()=>Yn("add"),document.getElementById("help-run").onclick=()=>Yn("run"),document.getElementById("help-close").onclick=We,document.getElementById("help-scrim").onclick=e=>{e.target.id==="help-scrim"&&We()},document.getElementById("add-cancel").onclick=Ve,document.getElementById("add-save").onclick=Vn,document.getElementById("add-scrim").onclick=e=>{e.target.id==="add-scrim"&&Ve()},document.querySelectorAll("#add-kind .v-chip").forEach(e=>{e.onclick=()=>qn(e.dataset.kind)}),document.getElementById("add-enrich").addEventListener("change",On),document.getElementById("add-scrim").addEventListener("keydown",e=>{e.key==="Enter"&&(e.target.tagName!=="INPUT"||e.target.type==="checkbox"||e.target.id==="add-vertical-filter"||e.target.id==="add-job-company"||(e.preventDefault(),Vn()))}),document.getElementById("add-vertical-filter").addEventListener("input",Nn),document.getElementById("add-headcount").addEventListener("input",e=>{const t=e.target.value.replace(/[^0-9]/g,"");t!==e.target.value&&(e.target.value=t)}),document.getElementById("csv-file").onchange=e=>{const t=e.target.files&&e.target.files[0];t&&ba(t),e.target.value=""},document.getElementById("drawer-cancel").onclick=Ea,document.getElementById("drawer-close").onclick=Hn,(()=>{const e=document.getElementById("drawer");e.addEventListener("mouseenter",Re),e.addEventListener("mouseleave",()=>{!Te&&e.classList.contains("open")&&Pn()})})(),document.getElementById("editor-cancel").onclick=Fe,document.getElementById("editor-save").onclick=ja,document.getElementById("editor-reset").onclick=Ma,document.getElementById("editor-scrim").onclick=e=>{e.target.id==="editor-scrim"&&Fe()},document.getElementById("sources-close").onclick=Lt,document.getElementById("sources-scrim").onclick=e=>{e.target.id==="sources-scrim"&&Lt()},document.getElementById("pf-cancel").onclick=Ge,document.getElementById("pf-save").onclick=Ta,document.getElementById("pf-reset").onclick=()=>Jn(!0),document.getElementById("prefilter-scrim").addEventListener("click",e=>{var a;if(e.target.id==="prefilter-scrim"){Ge();return}const t=e.target.closest(".pf-stage");if(t){_a(t.dataset.stage);return}const n=e.target.closest(".pf-chip-x");if(n){Fn(n.dataset.field,parseInt(n.dataset.i,10));return}const s=e.target.closest(".pf-chips");s&&e.target===s&&((a=s.querySelector(".pf-chip-input"))==null||a.focus())}),document.getElementById("prefilter-scrim").addEventListener("keydown",e=>{const t=e.target.closest(".pf-chip-input");if(t){if(e.key==="Enter"||e.key===",")e.preventDefault(),xa(t.dataset.field,t.value);else if(e.key==="Backspace"&&!t.value){const n=ee(t.dataset.field);n.length&&Fn(t.dataset.field,n.length-1)}}}),document.getElementById("key-cancel").onclick=Ht,document.getElementById("key-save").onclick=es,document.getElementById("key-remove").onclick=Va,document.getElementById("key-scrim").onclick=e=>{e.target.id==="key-scrim"&&Ht()},document.getElementById("key-input").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),es())}),document.getElementById("delcompany-cancel").onclick=Pe,document.getElementById("delcompany-confirm").onclick=Cs,document.getElementById("delcompany-scrim").onclick=e=>{e.target.id==="delcompany-scrim"&&Pe()},document.getElementById("deljob-cancel").onclick=qe,document.getElementById("deljob-confirm").onclick=Ms,document.getElementById("deljob-scrim").onclick=e=>{e.target.id==="deljob-scrim"&&qe()},document.getElementById("relink-cancel").onclick=he,document.getElementById("relink-scrim").onclick=e=>{e.target.id==="relink-scrim"&&he()},document.getElementById("relink-search").addEventListener("input",e=>{yn(e.target.value)}),document.getElementById("relink-search").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();const t=document.querySelector("#relink-results .relink-result:not([disabled])");t&&gn(t.dataset.id)}}),document.getElementById("relink-results").addEventListener("click",e=>{const t=e.target.closest(".relink-result");t&&!t.disabled&&gn(t.dataset.id)});function Tt(e){if(e==null)return"—";let t=Math.max(0,e|0);if(t<90)return`${t}s ago`;const n=Math.round(t/60);if(n<90)return`${n}m ago`;const s=Math.round(n/60);return s<48?`${s}h ago`:`${Math.round(s/24)}d ago`}async function St(){try{const e=await fetch("/api/profile");if(!e.ok)throw new Error(`HTTP ${e.status}`);i.profile=await e.json()}catch{i.profile=null}te()}const J='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.6 2.4a1.2 1.2 0 0 1 1.7 1.7L5.6 11.8l-3 1 1-3z"/><path d="M10.4 3.6l2 2"/></svg>',Ye='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13.4 8a5.4 5.4 0 1 1-1.5-3.8"/><path d="M13.6 2.6V5.2H11"/></svg>',Pa='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11" r="2.6"/><path d="M6.9 9.1 13 3M11 5l1.6 1.6M9.3 6.7l1.6 1.6"/></svg>',Zn='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.2"/><circle cx="8" cy="8" r="2.4"/></svg>',qa='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3.2h7.2a1.6 1.6 0 0 1 1.6 1.6v8H4.6A1.6 1.6 0 0 1 3 11.2z"/><path d="M11.8 12.8h1.4v-9A1.6 1.6 0 0 0 11.6 2.4H5.4"/><path d="M5.4 5.8h3.6M5.4 8.2h3.6"/></svg>',Ct='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3.5" width="12" height="9" rx="1.6"/><path d="M2.6 4.6 8 8.8l5.4-4.2"/></svg>',jt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2.2h5.4l2.6 2.6v9H4z"/><path d="M9.4 2.2v2.6H12"/><path d="M6 7h4M6 9.2h4M6 11.4h2.4"/></svg>',Oa='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.4 3.4h11.2L9.4 8.4v4.2l-2.8 1.4V8.4z"/></svg>',Mt='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4.2 7a3.8 3.8 0 0 1 7.6 0c0 3 1.2 4 1.2 4H3s1.2-1 1.2-4z"/><path d="M6.7 13a1.5 1.5 0 0 0 2.6 0"/></svg>';function U(e){const t=e.dot||e.note?`<div class="crit-status">${e.dot?`<span class="pf-dot ${e.dot}"></span>`:""}${e.note?`<span class="crit-note-t">${c(e.note)}</span>`:""}</div>`:"",n=e.actID?` id="${e.actID}"`:"",s=e.act?`<button class="crit-edit"${n} data-act="${e.act}" title="${e.actTitle}" aria-label="${e.actLabel}">${e.actIcon}</button>`:"";return`<div class="settings-item">
    ...
    ${s}
  </div>`}function te(){const e=document.getElementById("criteria-stats");if(!e)return;const t=i.profile,s=(t&&t.active_source||i.stats&&i.stats.taste_source||"").startsWith("brain:"),a=t&&typeof t.body=="string";let o;if(s){let B="off",j="";const Ce=t&&t.criteria_state;Ce==="current"?(B="ok",j="current · verified "+Tt(t.verified_age_seconds)):Ce==="changed"?(B="warn",j="changed — re-distill"):Ce==="unverified"?(B="warn",j=t&&!t.reachable&&a?"brain offline · using cache":"unverified — re-distill"):t&&!t.reachable&&a?(B="warn",j="brain offline · using cache"):a&&(B="ok",j="fetched "+Tt(t.age_seconds)),o=U({icon:Zn,nameHTML:a?'<span class="edit-link" data-act="view-profile" title="view the company-fit brief">company-fit brief</span>':"company-fit brief",dot:B,note:j,desc:"The criteria scout feeds the verdict stage — distilled from the brain.",act:"refresh-profile",actID:"refresh-profile",actIcon:Ye,actTitle:"re-distill the company-fit brief from the brain",actLabel:"refresh company-fit brief"})}else o=U({icon:Zn,nameHTML:'<span class="edit-link" data-act="edit-taste" title="edit taste.md">taste</span>',note:t&&t.configured?"brain offline — local fallback":"",dot:t&&t.configured?"warn":"",desc:"Local fallback criteria used when the brain is unreachable.",act:"edit-taste",actIcon:J,actTitle:"edit taste.md",actLabel:"edit taste"});const r=U({icon:qa,nameHTML:'<span class="edit-link" data-act="edit-playbook" title="edit the verdict playbook">playbook</span>',desc:"How scout judges — the reasoning rules behind every verdict.",act:"edit-playbook",actIcon:J,actTitle:"edit the verdict playbook",actLabel:"edit playbook"}),d=U({icon:Ct,nameHTML:'<span class="edit-link" data-act="edit-template" title="edit the outreach email template">email template</span>',desc:"The outreach email format — verbatim prose with fill-in holes.",act:"edit-template",actIcon:J,actTitle:"edit the outreach email template",actLabel:"edit email template"}),u=U({icon:Ct,nameHTML:'<span class="edit-link" data-act="edit-followup-template" title="edit the follow-up template">follow-up template</span>',desc:"Copy-paste follow-up — variables {{contact_name}}, {{role}}, {{company}}, {{last_sent}}, {{last_message}}.",act:"edit-followup-template",actIcon:J,actTitle:"edit the follow-up template",actLabel:"edit follow-up template"}),p=`<div class="settings-item">
    <span class="settings-item-icon">${Mt}</span>
    <div class="settings-item-main">
    ...
    <input class="input set-fu-interval" type="number" min="0" max="90" value="${i.followupInterval}" title="business days (0 = off)" aria-label="follow-up reminder interval in business days">
  </div>`,h=[["researcher","1 · Researcher","Searches the web for true company facts and the best hooks to open with."],["fill","2 · Writer","Writes the email's blanks from the research, your experience, and your voice."],["humanizer","3 · Humanizer","Strips AI tells and matches your voice — never changes a fact."],["honesty","4 · Honesty check","Vetoes any claim about you beyond your documented experience."]],f=h.map(([B,j,Ce])=>U({icon:jt,nameHTML:`<span class="edit-link" data-act="edit-prompt-${B}" title="edit the ${j.replace(/^\d+ · /,"")} prompt">${j}</span>`,desc:Ce,act:`edit-prompt-${B}`,actIcon:J,actTitle:`edit the ${j} prompt`,actLabel:`edit ${j} prompt`})).join(""),b=!i.stats||i.stats.taste_filter_enabled!==!1,T=U({icon:Oa,nameHTML:'<span class="edit-link" data-act="edit-taste-filter" title="edit the pre-filter rules">pre-filter</span>',desc:"Cheap mechanical gate that narrows bulk verdict runs before the paid LLM — location, headcount, vertical, stage. Re-scoring one company by hand ignores it.",dot:b?"ok":"off",note:b?"active":"disabled — scoring everything",act:"edit-taste-filter",actIcon:J,actTitle:"edit the pre-filter rules",actLabel:"edit pre-filter rules"}),L=i.anthropicKey;let R="off",H="not set — verdict, capture & outreach disabled";L&&L.key_source==="db"?(R="ok",H="set here · active"):L&&L.key_source==="env"&&(R="ok",H="from the environment");const D=U({icon:Pa,nameHTML:'<span class="edit-link" data-act="edit-anthropic-key" title="set the Anthropic API key">Anthropic API key</span>',dot:R,note:H,desc:"Powers scoring, capture & outreach. Set here to run scout without the env var.",act:"edit-anthropic-key",actIcon:J,actTitle:"set the Anthropic API key",actLabel:"set Anthropic API key"}),x=i.gmail||{};let y="off",P="not connected",K="gmail-connect",Ut="connect Gmail",Se="connect a Gmail account to send + sync";x.configured?x.connected?(y="ok",P=`connected as ${x.email||"(unknown)"}`,K="gmail-disconnect",Ut="disconnect Gmail",Se="disconnect this Gmail account"):P="configured — not connected":P="OAuth client not set (GMAIL_CLIENT_ID / GMAIL_CLIENT_SECRET)";const uo=U({icon:Ct,nameHTML:`<span class="edit-link" data-act="${K}" title="${Se}">Gmail</span>`,dot:y,note:P,desc:"Send outreach from your Gmail and auto-sync replies + application status.",act:K,actIcon:J,actTitle:Se,actLabel:Ut}),po=U({icon:jt,nameHTML:'<span class="edit-link" data-act="edit-application-stages" title="edit the application stages">application stages</span>',desc:"The application pipeline labels you track (applied, screening, interview…). One per line.",act:"edit-application-stages",actIcon:J,actTitle:"edit the application stages",actLabel:"edit application stages"}),mo=U({icon:jt,nameHTML:'<span class="edit-link" data-act="edit-outreach-statuses" title="edit the outreach statuses">outreach statuses</span>',desc:"The outreach reply labels (initial contact, no response, replied…). One per line.",act:"edit-outreach-statuses",actIcon:J,actTitle:"edit the outreach statuses",actLabel:"edit outreach statuses"});e.innerHTML=`<div class="settings-section">
     ...
     <div class="settings-section">
       ...
       ${D}${uo}
     </div>`;const Ft={"view-profile":()=>Fa(i.profile),"refresh-profile":Ua,"edit-taste":()=>X("taste"),"edit-taste-filter":()=>Jn(),"edit-application-stages":()=>X("application-stages"),"edit-outreach-statuses":()=>X("outreach-statuses"),"edit-playbook":()=>X("playbook"),"edit-template":()=>X("outreach-template"),"edit-followup-template":()=>X("followup-template"),"edit-anthropic-key":Da,"gmail-connect":Na,"gmail-disconnect":Ra};for(const[B]of h)Ft[`edit-prompt-${B}`]=()=>X(`outreach-prompts/${B}`);e.querySelectorAll("[data-act]").forEach(B=>{const j=B.dataset.act;j&&Ft[j]&&(B.onclick=Ft[j])});const et=e.querySelector(".set-fu-interval");et&&et.addEventListener("change",async()=>{const B=Math.max(0,Math.min(90,parseInt(et.value,10)||0));et.value=String(B),await ie("PUT","/api/followup-interval",{days:B})&&(i.followupInterval=B,l("follow-up interval saved"))})}async function Qn(){try{i.anthropicKey=await(await fetch("/api/integrations/anthropic")).json()}catch{i.anthropicKey=null}te()}async function Xn(){try{i.gmail=await(await fetch("/api/gmail/status")).json()}catch{i.gmail=null}te()}async function Na(){let e;try{e=await fetch("/api/gmail/connect")}catch(n){l(`connect failed: ${n.message}`);return}if(!e.ok){l((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}let t={};try{t=await e.json()}catch{}t.auth_url?window.location.href=t.auth_url:l("could not start the Gmail connect flow")}async function Ra(){if(!confirm("Disconnect Gmail? Sending and sync stop; already-synced data stays."))return;let e;try{e=await fetch("/api/gmail/disconnect",{method:"DELETE"})}catch(t){l(`disconnect failed: ${t.message}`);return}if(!e.ok){l((await e.text().catch(()=>"")).trim()||`HTTP ${e.status}`);return}l("Gmail disconnected"),await Xn()}async function Da(){document.getElementById("key-scrim").classList.add("open"),document.getElementById("key-input").value="",await Qn(),At();const e=document.getElementById("key-input");e&&e.focus()}function At(){const e=i.anthropicKey||{},t=document.getElementById("key-status");t&&(t.textContent=e.key_source==="db"?"A key is set here (stored in scout).":e.key_source==="env"?"Using the ANTHROPIC_API_KEY environment variable. Saving a key here overrides it.":"No key set. Scoring, capture, and outreach are disabled until you add one.");const n=document.getElementById("key-remove");n&&(n.style.display=e.key_source==="db"?"":"none");const s=document.getElementById("key-restart-hint");if(s){const a=e.has_key&&i.meta&&(i.meta.outreach===!1||i.meta.chat===!1);s.style.display=a?"":"none"}}function Ht(){document.getElementById("key-scrim").classList.remove("open")}async function es(){const e=(document.getElementById("key-input").value||"").trim();if(!e){l("paste a key first");return}const t=document.getElementById("key-save");t&&(t.disabled=!0,t.textContent="Verifying…");const n=()=>{t&&(t.disabled=!1,t.textContent="Save key")};let s;try{s=await fetch("/api/integrations/anthropic",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:e})})}catch(a){l(`save failed: ${a.message}`),n();return}if(!s.ok){l((await s.text().catch(()=>"")).trim()||`HTTP ${s.status}`),n();return}i.anthropicKey=await s.json(),document.getElementById("key-input").value="",n(),l("Anthropic key saved"),await vt(),At(),te()}async function Va(){const e=document.getElementById("key-remove");e&&(e.disabled=!0);let t;try{t=await fetch("/api/integrations/anthropic",{method:"DELETE"})}catch(n){l(`remove failed: ${n.message}`),e&&(e.disabled=!1);return}if(e&&(e.disabled=!1),!t.ok){l((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}i.anthropicKey=await t.json(),l(i.anthropicKey.has_key?"removed — using the environment key":"Anthropic key removed"),await vt(),At(),te()}async function Ua(){const e=document.getElementById("refresh-profile");e&&(e.classList.add("spinning"),e.disabled=!0);let t;try{t=await fetch("/api/profile/refresh",{method:"POST"})}catch(n){l(`refresh failed: ${n.message}`),St();return}if(!t.ok){const n=await t.text().catch(()=>"");l(`refresh failed: ${(n||"").trim()||"HTTP "+t.status}`),St();return}i.profile=await t.json(),te(),l("company-fit brief refreshed"),S()}function Fa(e){!e||typeof e.body!="string"||(document.getElementById("profile-modal-meta").textContent=`${e.chars||0} chars · fetched ${Tt(e.age_seconds)}`,document.getElementById("profile-modal-body").textContent=e.body,document.getElementById("profile-scrim").classList.add("open"))}function Pt(){document.getElementById("profile-scrim").classList.remove("open")}document.getElementById("profile-modal-close").onclick=Pt,document.getElementById("profile-scrim").onclick=e=>{e.target.id==="profile-scrim"&&Pt()};function ts(){document.getElementById("docs-scrim").classList.add("open");const e=document.querySelector("#docs-nav a");Ze(e?e.dataset.sec:null);const t=document.getElementById("docs-body");t&&(t.scrollTop=0)}function qt(){document.getElementById("docs-scrim").classList.remove("open")}function za(){return document.getElementById("docs-scrim").classList.contains("open")}function Ze(e){document.querySelectorAll("#docs-nav a").forEach(t=>t.classList.toggle("active",t.dataset.sec===e))}function Ja(e){const t=document.getElementById("doc-"+e);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ze(e)}document.getElementById("open-docs").onclick=ts,document.getElementById("docs-close").onclick=qt,document.getElementById("docs-scrim").onclick=e=>{e.target.id==="docs-scrim"&&qt()};function Ga(){document.getElementById("settings-scrim").classList.add("open"),te()}function Ot(){document.getElementById("settings-scrim").classList.remove("open")}document.getElementById("open-settings").onclick=Ga,document.getElementById("settings-close").onclick=Ot,document.getElementById("settings-scrim").onclick=e=>{e.target.id==="settings-scrim"&&Ot()};function Ka(){const e=document.getElementById("notif-badge");if(!e)return;const t=(i.notifications&&i.notifications.unread)|0;t>0?(e.textContent=t>99?"99+":String(t),e.style.display=""):e.style.display="none"}async function ce(){try{i.notifications=await(await fetch("/api/notifications")).json()}catch{return}Ka(),document.getElementById("notifications-scrim").classList.contains("open")&&ns()}function Wa(){document.getElementById("notifications-scrim").classList.add("open"),ns(),ce()}function Nt(){document.getElementById("notifications-scrim").classList.remove("open")}function Ya(){return'<option value="">link to role…</option>'+(i.jobs||[]).map(t=>`<option value="${c(t.posting_id)}">${c((t.company||"")+" — "+(t.title||"(untitled)"))}</option>`).join("")}function Za(e){const t=e.company||e.role?`<div class="notif-ctx">${c([e.company,e.role].filter(Boolean).join(" · "))}</div>`:'<div class="notif-ctx dim">not linked to a role</div>',n=e.created_at?`<span class="notif-when">${c((e.created_at||"").replace("T"," ").slice(0,16))}</span>`:"",s=e.kind==="app_status"&&e.suggested_status&&!e.actioned&&e.posting_id?`<button class="btn btn-primary notif-apply" data-id="${e.id}">Apply: ${c(e.suggested_status)}</button>`:"",a=e.posting_id?"":`<select class="input notif-link" data-id="${e.id}" title="link this to a role">${Ya()}</select>`;return`<div class="notif-item${e.seen?"":" is-unread"}" data-id="${e.id}" data-seen="${e.seen?1:0}">
  ...
  </div>`}function ns(){const e=document.getElementById("notifications-body");if(!e)return;const t=i.notifications||{notifications:[],followups:[]},n=t.notifications||[],s=t.followups||[];if(!n.length&&!s.length){e.innerHTML='<div class="cc-empty dim">Nothing here yet. Replies, application updates, and follow-ups show up as Gmail syncs.</div>';return}let a="";n.length&&(a+='<div class="settings-group-h">Updates</div>'+n.map(Za).join("")),s.length&&(a+='<div class="settings-group-h">Follow-ups due</div>'+s.map(Qa).join("")),e.innerHTML=a,Xa()}function Xa(){const e=document.getElementById("notifications-body");e&&(e.querySelectorAll(".notif-item[data-id]").forEach(t=>{const n=t.dataset.id,s=t.querySelector(".notif-main");s&&t.dataset.seen==="0"&&s.addEventListener("click",()=>eo(n))}),e.querySelectorAll(".notif-apply").forEach(t=>t.addEventListener("click",n=>{n.stopPropagation(),to(t.dataset.id)})),e.querySelectorAll(".notif-link").forEach(t=>t.addEventListener("change",n=>{n.stopPropagation(),t.value&&no(t.dataset.id,t.value)})),e.querySelectorAll(".notif-open").forEach(t=>t.addEventListener("click",()=>{const n=t.dataset.pid;Nt(),it(n)})))}async function eo(e){try{await fetch(`/api/notifications/${e}/seen`,{method:"POST"})}catch{return}await ce()}async function to(e){let t;try{t=await fetch(`/api/notifications/${e}/apply`,{method:"POST"})}catch(s){l(`apply failed: ${s.message}`);return}if(!t.ok){l((await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`);return}const n=await t.json().catch(()=>({}));l(`status set to ${n.applied||"updated"}`),await ce(),await k()}async function no(e,t){let n;try{n=await fetch(`/api/notifications/${e}/link`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({posting_id:t})})}catch(s){l(`link failed: ${s.message}`);return}if(!n.ok){l((await n.text().catch(()=>"")).trim()||`HTTP ${n.status}`);return}l("linked to role"),await ce()}async function so(){const e=document.getElementById("notifications-sync");e&&(e.disabled=!0,e.textContent="Syncing…");try{const t=await fetch("/api/gmail/sync",{method:"POST"});l(t.ok?"synced":(await t.text().catch(()=>"")).trim()||`HTTP ${t.status}`)}catch(t){l(`sync failed: ${t.message}`)}e&&(e.disabled=!1,e.textContent="Sync now"),await ce(),await k()}document.getElementById("open-notifications").onclick=Wa,document.getElementById("notifications-close").onclick=Nt,document.getElementById("notifications-sync").onclick=so,document.getElementById("notifications-scrim").onclick=e=>{e.target.id==="notifications-scrim"&&Nt()},document.querySelectorAll("#docs-nav a").forEach(e=>{e.onclick=()=>{const t=document.getElementById("doc-"+e.dataset.sec);t&&t.scrollIntoView({behavior:"smooth",block:"start"}),Ze(e.dataset.sec)}}),function(){const e=document.getElementById("docs-body");if(!e||!("IntersectionObserver"in window))return;const t=new IntersectionObserver(n=>{const s=n.filter(a=>a.isIntersecting).sort((a,o)=>a.boundingClientRect.top-o.boundingClientRect.top);s.length&&Ze(s[0].target.id.replace(/^doc-/,""))},{root:e,rootMargin:"0px 0px -65% 0px",threshold:0});document.querySelectorAll("#docs-body section").forEach(n=>t.observe(n))}(),i.chat={scope:null,scopeId:"",threadId:null,streaming:!1,es:null};function ao(e){return(e||[]).filter(t=>t&&t.type==="text").map(t=>t.text||"").join("")}function oo(e){return(e||[]).filter(t=>t&&t.type==="tool_use").map(t=>t.name)}function Qe(e,t){const n=document.createElement("div");return n.className="chat-msg chat-"+e,n.textContent=t||"",n}function Rt(){const e=document.getElementById("chat-messages");e.scrollTop=e.scrollHeight}function io(){const e=document.createElement("div");return e.className="chat-empty",e.textContent=i.chat.scope==="global"?"Tell me about a job you applied to (paste the link), or ask what's already tracked.":"Ask about this "+(i.chat.scope==="company"?"company":"role")+" — I can research it on the web and update scout.",e}function ss(e){const t=document.getElementById("chat-messages");t.innerHTML="";for(const n of e||[]){const s=ao(n.content);if(n.role==="user")s&&t.appendChild(Qe("user",s));else if(n.role==="assistant"){const a=oo(n.content);if(!s&&!a.length)continue;const o=Qe("assistant",s);if(a.length){const r=document.createElement("div");r.className="chat-tools",r.textContent="· used "+a.join(", "),o.appendChild(r)}t.appendChild(o)}}t.children.length||t.appendChild(io()),Rt()}async function Dt(e,t,n){if(!i.meta||!i.meta.chat){l("chat needs ANTHROPIC_API_KEY in the server env");return}i.chat.es&&(i.chat.es.close(),i.chat.es=null),i.chat={scope:e,scopeId:t||"",threadId:null,streaming:!1,es:null},document.getElementById("chat-title").textContent=e==="global"?"Chat":e==="company"?"Chat · company":"Chat · role",document.getElementById("chat-sub").textContent=e==="global"?"":n||"";const s=document.getElementById("chat-messages");s.innerHTML='<div class="chat-empty">loading…</div>';const a=document.getElementById("chat-pane");a.classList.add("open"),document.getElementById("chat-scrim").classList.add("open"),a.setAttribute("aria-hidden","false");try{const o="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):""),r=await fetch("/api/chat/threads?"+o);if(!r.ok)throw new Error((await r.text().catch(()=>"")).trim()||"HTTP "+r.status);const d=await r.json();i.chat.threadId=d.thread.id,ss(d.messages||[])}catch(o){s.innerHTML='<div class="chat-empty">Failed to open chat: '+c(o.message)+"</div>";return}document.getElementById("chat-input").focus()}function Vt(){i.chat.es&&(i.chat.es.close(),i.chat.es=null);const e=document.getElementById("chat-pane");e.classList.remove("open"),document.getElementById("chat-scrim").classList.remove("open"),e.setAttribute("aria-hidden","true")}function Xe(e){i.chat.streaming=e,document.getElementById("chat-send").disabled=e;const t=document.getElementById("chat-input");t.disabled=e,e||t.focus()}function as(){const e=document.getElementById("chat-input");e.style.height="auto",e.style.height=Math.min(e.scrollHeight,160)+"px"}async function os(){const e=document.getElementById("chat-input"),t=e.value.trim();if(!t||i.chat.streaming||!i.chat.threadId)return;e.value="",as(),Xe(!0);const n=document.getElementById("chat-messages"),s=n.querySelector(".chat-empty");s&&s.remove(),n.appendChild(Qe("user",t));const a=Qe("assistant","");a.classList.add("chat-streaming"),n.appendChild(a),Rt();let o="";const r=h=>{a.classList.remove("chat-streaming"),a.textContent="⚠ "+h,Xe(!1)},d=i.chat.threadId;let u;try{u=await fetch("/api/chat/"+d+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})})}catch(h){r(h.message);return}if(!u.ok){r((await u.text().catch(()=>"")).trim()||"HTTP "+u.status);return}const p=new EventSource("/api/chat/"+d+"/stream");i.chat.es=p,p.addEventListener("delta",h=>{o+=h.data,a.textContent=o,Rt()}),p.addEventListener("end",async h=>{p.close(),i.chat.es===p&&(i.chat.es=null),a.classList.remove("chat-streaming"),Xe(!1),i.chat.threadId===d&&await co(),ro(),typeof h.data=="string"&&h.data.indexOf("error")===0&&l("chat: "+h.data)}),p.onerror=()=>{p.close(),i.chat.es===p&&(i.chat.es=null),a.classList.remove("chat-streaming"),Xe(!1)}}async function co(){const e=i.chat.scope,t=i.chat.scopeId,n="scope="+encodeURIComponent(e)+(t?"&scope_id="+encodeURIComponent(t):"");try{const s=await fetch("/api/chat/threads?"+n);if(!s.ok)return;const a=await s.json();ss(a.messages||[])}catch{}}function ro(){q(),k(),S(),i.openId&&ve(i.openId)}document.getElementById("open-chat").onclick=()=>Dt("global","",""),document.getElementById("chat-close").onclick=Vt,document.getElementById("chat-scrim").onclick=Vt,document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault(),os()}),document.getElementById("chat-input").addEventListener("input",as),document.getElementById("chat-input").addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),os())}),en("#t tbody",us),en("#jt tbody",ps);const lo=(()=>{try{return localStorage.getItem("scout-view")}catch{return null}})();O(lo==="jobs"?"jobs":"companies",{render:!1});function is(){const e=document.querySelector(".layout");e&&(e.style.transform="translateZ(0)",requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.transform=""})))}document.addEventListener("visibilitychange",()=>{document.hidden||is()}),window.addEventListener("pageshow",e=>{e.persisted&&is()}),q(),k(),S(),vt(),bt(),St(),Qn(),Xn(),ce(),setInterval(ce,9e4),M(),function(){const t=/[?&]gmail=(connected|error)/.exec(location.search);t&&(l(t[1]==="connected"?"Gmail connected":"Gmail connection failed"),history.replaceState(null,"",location.pathname+location.hash))}()}yo({"":{view:()=>({mount(w){w.innerHTML=ko,Eo()}}),chrome:!1}},{title:"scout"});bo();wo();
