"""Tests for scout.enrich — the challenge gate, not-found detection, fact parsing,
and the enrich run. The fetch tests use the threaded http stub
(tests/httpstub.py); the Enricher's `scheme` seam points it at http://."""
...
def test_looks_like_challenge():
    cases = [
        ...
        (
            ...
            long_string("Welcome to our site. ", 60) + " Just a moment... ",
            False,
        ),
        ("empty", "", False),
    ]
...
def test_looks_like_not_found():
    cases = [
        (
            "classic soft 404",
            "404 Error. The page you were looking for could not be found. Go back home.",
            True,
        ),
        ...
        (
            ...
            long_string("Welcome to our site. ", 60) + " page not found ",
            False,
        ),
        ("empty", "", False),
    ]
    ...
    def handle(req):
        ...
        if req.path == "/company":
            return (
                ...
                "<html><body>" + REAL_CONTENT + "</body></html>",
            )
    ...
    def handle(req):
        if req.path in ("/about", "/about-us", "/company"):
            return (
                200,
                {"Content-Type": "text/html"},
                "<html><body>Page not found. Try our homepage.</body></html>",
            )
...
def test_fetch_one_all_soft_404():
    ...
    def handle(req):
        return (
            200,
            {"Content-Type": "text/html"},
            "<html><body>404 Error. Page not found.</body></html>",
        )
    ...
    def handle(req):
        if req.path in ("/about", "/about-us", "/company"):
            return (
                200,
                {"Content-Type": "text/html"},
                "<html><head><title>undefined | Acme</title></head><body></body></html>",
            )
        return 200, {"Content-Type": "text/html"}, "<html><body>" + REAL_CONTENT + "</body></html>"
...
def test_parse_facts():
    f = parse_facts(
        '{"name":"Acme","vertical":"Robotics, AI","location":"Austin, TX","headcount":120,"funding_stage":"Series A"}'
    )
    assert (
        f.name == "Acme"
        and f.vertical == "Robotics, AI"
        and f.headcount == 120
        and f.funding_stage == "Series A"
    )
