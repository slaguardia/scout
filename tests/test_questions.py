"""Port of internal/capture/questions_test.go — application-question detection."""
...
def test_detect_greenhouse_questions(monkeypatch):
    ...
    with http_server(handle) as url:
        ...
        scan = detect_questions(_client(), "https://job-boards.greenhouse.io/parasail/jobs/4092794008")
        assert scan.status == QUESTIONS_OK and scan.source == "greenhouse"
        got = _prompts(scan)
        want = ["Cover Letter", "What inspires you at work?", "How would you approach our hardest problem?"]
        assert got == want
...
ASHBY_FIXTURE = json.dumps({"data": {"jobPosting": {
...
def test_detect_ashby_questions(monkeypatch):
    ...
    with http_server(handle) as url:
        ...
        scan = detect_questions(_client(), "https://jobs.ashbyhq.com/writer/634e0a00-dd96-4f5f-ba5f-4fa3aff4c6c9")
...
def test_detect_ashby_fails_soft(monkeypatch, body):
    ...
    with http_server(handle) as url:
        monkeypatch.setattr(questions, "ashby_graphql_base", url)
        scan = detect_questions(_client(), "https://jobs.ashbyhq.com/acme/634e0a00-dd96-4f5f-ba5f-4fa3aff4c6c9")
...
def test_detect_rippling_questions(monkeypatch):
    ...
    with http_server(handle) as url:
        ...
        for bad in ["First name", "Email", "Current company", "Location (city only)", "LinkedIn link", "Resume", "Phone number", "Pronouns"]:
            assert bad not in got
        assert scan.questions[0].key == "why_plenful"


def test_detect_rippling_no_essays(monkeypatch):
    body = json.dumps({"name": "X", "activeJobApplication": {"customQuestions": {"fields": [
...
DOVER_QUESTIONS_FIXTURE = json.dumps({
    "id": "job", "client_name": "Paratus", "title": "Founding Engineer",
...
def test_detect_dover_questions(monkeypatch):
    ...
    with http_server(handle) as url:
        ...
        for bad in ["Are you a U.S. citizen and eligible to obtain a U.S. security clearance?", "Resume Upload", "LinkedIn Profile URL", "Phone", "Internal hidden question"]:
            assert bad not in got
        assert scan.questions[0].key == "q-essay"


def test_detect_dover_no_essays(monkeypatch):
    body = json.dumps({"id": "job", "application_questions": [
        {"id": "q-resume", "question": "Resume Upload", "input_type": "FILE_UPLOAD", "question_type": "RESUME", "hidden": False},
        {"id": "q-phone", "question": "Phone", "input_type": "SHORT_ANSWER", "question_type": "PHONE_NUMBER", "hidden": False},
    ]})
