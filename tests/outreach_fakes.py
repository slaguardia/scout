"""Shared fakes for the outreach engine tests — fake Anthropic / fake brain test
servers.
    ...
    def handle(self, req):
        with self._lock:
            ...
            if self.calls >= len(self.replies):
                self.errors.append(
                    f"unexpected anthropic call #{self.calls + 1} "
...
class FakeBrain:
    ...
    cursor/changed signal) from an in-memory doc set. docs maps id -> a dict
    {id,title,path,version,text}."""
...
def seed_posting_draft(con: sqlite3.Connection) -> int:
    ...
    cid = companies.upsert_company(
        con, Company(source="test", name="Acme", domain="acme.com", raw_json="{}")
    )
