"""The link-capture pass, direct posting add, and the posting sub-resource edits
(details / company / + url / next-up smoke)."""
...
def test_capture_end_to_end(tmp_path, monkeypatch):
    ...
    def page(req):
        return (
            ...
            f"<html><body><h1>Platform Engineer</h1>{page_body}</body></html>",
        )
    ...
    with http_server(page) as page_url, http_server(llm) as llm_url:
        ...
        assert (
            row["company"] == "Acme"
            and row["title"] == "Platform Engineer"
            and row["source"] == "capture"
        )
...
def test_capture_fetch_failure_is_422(tmp_path, monkeypatch):
    ...
    with http_server(dead) as dead_url:
        client.app.state.scout.anthropic = anthropic.Client(
            api_key="test-key", endpoint="http://unused.invalid"
        )
...
def test_add_posting_direct(tmp_path, monkeypatch):
    ...
    res = rec.json()
    assert (
        res["company_id"] == cid and not res["company_created"] and res["posting"]["title"] == "SE"
    )
    ...
    rec = _post(
        client,
        "/api/postings",
        '{"url":"https://apply.workable.com/widgets/j/ABC123","company":"Widgets"}',
    )
    assert rec.status_code == 200, (rec.status_code, rec.text)
    res = rec.json()
    ...
    assert (
        _post(
            client, "/api/postings", '{"url":"https://apply.workable.com/mystery/j/DEF456"}'
        ).status_code
        == 400
    )
    con = open_db(db_path)
    ...
    assert (
        _post(client, "/api/postings", '{"url":"javascript:alert(1)","company":"Evil"}').status_code
        == 400
    )
    con = open_db(db_path)
    assert companies_store.count_companies(con) == before
...
def test_posting_details_api(tmp_path, monkeypatch):
    ...
    rec = _put(
        client,
        f"/api/postings/{p.id}/details",
        '{"title":"Staff Engineer","location":"Remote","comp_range":"$200k"}',
    )
    assert rec.status_code == 200, (rec.status_code, rec.text)
    got = rec.json()
    assert (
...
def test_posting_tracking_api(tmp_path, monkeypatch):
    ...
    rec = _put(
        client,
        f"/api/postings/{p.id}",
        '{"application_status":"screening","outreach_status":"initial contact"}',
    )
    assert rec.status_code == 200, (rec.status_code, rec.text)
    got = rec.json()
    ...
    assert (
        _put(client, f"/api/postings/{p.id}", '{"outreach_status":"' + "x" * 100 + '"}').status_code
        == 400
    )
    ...
    assert len(rows) == 1
    assert (
        rows[0]["application_status"] == "screening"
        and rows[0]["outreach_status"] == "initial contact"
    )
...
def test_posting_company_api(tmp_path, monkeypatch):
    ...
    assert (
        _put(client, f"/api/postings/{p.id}/company", '{"company_id":"does-not-exist"}').status_code
        == 400
    )
    assert _put(client, f"/api/postings/{p.id}/company", '{"company_id":""}').status_code == 400
    assert (
        _put(client, "/api/postings/nope/company", '{"company_id":"' + added + '"}').status_code
        == 404
    )
...
def test_posting_next_up_and_url(tmp_path, monkeypatch):
    """Smoke coverage for the two sub-routes."""
    client, cid, db_path = new_test_app(tmp_path, monkeypatch)
    con = open_db(db_path)
    p = postings_store.add_posting(con, cid, "https://acme.com/jobs/se", "SE")
    con.close()
    ...
    assert (
        _put(client, f"/api/postings/{p.id}/next-up", '{"next_up":true}').json()["next_up"] is True
    )
    assert (
        _put(client, f"/api/postings/{p.id}/next-up", '{"next_up":false}').json()["next_up"]
        is False
    )
    ...
    assert (
        _put(client, f"/api/postings/{p.id}/url", '{"url":"javascript:alert(1)"}').status_code
        == 400
    )
