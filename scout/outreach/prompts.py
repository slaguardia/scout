...
# (added by the engine) for news/site/posting hooks. Beyond the facts it produces
# an INTERPRETATION — the company's bet, what it implies, and inferences about
# their internal reality — which is what gives the fill step depth to write from.
RESEARCHER_SYSTEM = """You research companies for job-search outreach. Given a company name and job URL,
...
FILL_SYSTEM_DEFAULT = """You write one warm, human cold email for a job seeker, in his own voice. You fill
...
# removes the LLM's tells and matches the user's voice WITHOUT changing any fact —
# the honesty checker runs after it, so wording is all that may move.
HUMANIZE_SYSTEM = """You clean up AI tells in short cold-email paragraphs the user is about to send.
...
# honestyCheckerSystem is the Honesty checker. Veto power, single purpose. It sees
# only the experience document and the text to verify — never the intended hook —
# and is strict-fail biased.
HONESTY_CHECKER_SYSTEM = """You verify that a job-search email makes no claim about THE SENDER beyond their
